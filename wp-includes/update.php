 ...
 * @param bool    $force_check Whether to bypass the transient cache and force a fresh update check. Defaults to false,
 *                             true if $extra_stats is set.
 */
...
function wp_get_update_data() {
    ...
    if (($core = current_user_can('update_core')) && function_exists('get_core_updates')) {
        ...
        if (!empty($update_wordpress) && !in_array($update_wordpress[0]->response,
                                                   ['development', 'latest']) && current_user_can('update_core')
        ) {
            $counts['wordpress'] = 1;
        }
    }
