...
<div id="primary" class="sidebar-<?php echo $st_kb_sidebar_position; ?> clearfix">
	<div class="ht-container">
		...
		<section id="content" role="main">
			...
			$tax_terms = wp_list_filter($tax_terms, ['parent' => 0]);
			?>
			...
			<div class="kb-category-list row stacked">
				...
				foreach ($tax_terms as $tax_term) {
					...
					echo '<h3><span class="count">' . sprintf(__("%s Articles", "framework"),
					                                          $tax_term->count) . '</span><a href="' . esc_attr(get_term_link($tax_term,
					                                                                                                          'st_kb_category')) . '" title="' . sprintf(__('View all posts in %s',
					                                                                                                                                                        'framework'),
					                                                                                                                                                     $tax_term->name) . '" ' . '>' . $tax_term->name . ' <span>&rarr;</span></a></h3>';
