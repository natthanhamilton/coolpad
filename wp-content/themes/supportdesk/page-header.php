			...
			<p><?php
				...
				elseif (is_month()) :
					printf(__('Monthly Archives for %s', 'framework'),
					       '<span>' . get_the_date(_x('F Y', 'monthly archives date format', 'framework')) . '</span>');
				elseif (is_year()) :
					printf(__('Yearly Archives for %s', 'framework'),
					       '<span>' . get_the_date(_x('Y', 'yearly archives date format', 'framework')) . '</span>');
				else :
...
<?php } else { ?>
	...
	<div id="page-header" class="clearfix">
		<div class="ht-container">
			<h1><?php if (is_search()) {
					_e("Search: ", "framework");
				} ?><?php echo get_the_title($st_index_id); ?></h1>
