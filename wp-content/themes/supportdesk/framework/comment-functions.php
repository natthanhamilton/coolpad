            ...
            <li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
                <article id="comment-<?php comment_ID(); ?>" class="comment">
                    <div class="comment-action">
                        <?php edit_comment_link(__('Edit', 'framework'), '', ''); ?>
                        <?php comment_reply_link(array_merge($args, ['reply_text' => __('Reply',
                                                                                        'framework'), 'depth' => $depth, 'max_depth' => $args['max_depth']])); ?>
                    </div>
                    <!-- .comment-action -->
...
function st_comment_form_args($args) {
    ...
    $fields = [
        'author' => '<p class="form-author clearfix' . esc_attr($input_class) . '"><label for="author">' . __('Name',
                                                                                                              'framework') . $req . '</label> <input type="text" class="text-input" name="author" id="author" value="' . esc_attr($commenter['comment_author']) . '" size="40" /></p>',
        'email'  => '<p class="form-email clearfix' . esc_attr($input_class) . '"><label for="email">' . __('Email',
                                                                                                            'framework') . $req . '</label> <input type="text" class="text-input" name="email" id="email" value="' . esc_attr($commenter['comment_author_email']) . '" size="40" /></p>',
        'url'    => '<p class="form-url clearfix"><label for="url">' . __('Website',
                                                                          'framework') . '</label><input type="text" class="text-input" name="url" id="url" value="' . esc_attr($commenter['comment_author_url']) . '" size="40" /></p>'
    ];
    /* Sets the default arguments for displaying the comment form. */
    $args = [
        'fields'               => apply_filters('comment_form_default_fields', $fields),
        'comment_field'        => '<p class="form-textarea req clearfix"><label for="comment">' . __('Comment',
                                                                                                     'framework') . '</label><textarea name="comment" id="comment" cols="60" rows="10"></textarea></p>',
        'must_log_in'          => '<p class="alert">' . sprintf(__('You must be <a href="%1$s" title="Log in">logged in</a> to post a comment.',
                                                                   'framework'),
                                                                wp_login_url(get_permalink())) . '</p><!-- .alert -->',
        'logged_in_as'         => '<p class="log-in-out">' . sprintf(__('Logged in as <a href="%1$s" title="%2$s">%2$s</a>.',
                                                                        'framework'), admin_url('profile.php'),
                                                                     esc_attr($user_identity)) . ' <a href="' . wp_logout_url(get_permalink()) . '" title="' . esc_attr__('Log out of this account',
                                                                                                                                                                          'framework') . '">' . __('Log out &raquo;',
                                                                                                                                                                                                   'framework') . '</a></p><!-- .log-in-out -->',
