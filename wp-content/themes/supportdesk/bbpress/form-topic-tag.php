		...
		<fieldset class="bbp-form" id="bbp-edit-topic-tag">
			...
			<fieldset class="bbp-form" id="tag-rename">
				...
				<div class="bbp-template-notice">
					<p><?php _e('Changing the slug affects its permalink. Any links to the old slug will stop working.',
					            'bbpress'); ?></p>
				</div>
				...
				<form id="rename_tag" name="rename_tag" method="post" action="<?php the_permalink(); ?>">
					...
					<div>
						...
						<input type="text" id="tag-slug" name="tag-slug" size="20" maxlength="40"
						       tabindex="<?php bbp_tab_index(); ?>"
						       value="<?php echo esc_attr(apply_filters('editable_slug',
				...
				<form id="merge_tag" name="merge_tag" method="post" action="<?php the_permalink(); ?>">
					...
					<div class="bbp-submit-wrapper">
						<button type="submit" tabindex="<?php bbp_tab_index(); ?>" class="button submit"
						        onclick="return confirm('<?php echo esc_js(sprintf(__('Are you sure you want to merge the "%s" tag into the tag you specified?',
						                                                              'bbpress'),
						                                                           bbp_get_topic_tag_name())); ?>');"><?php esc_attr_e('Merge',
						                                                                                                               'bbpress'); ?></button>
			...
			<?php if (current_user_can('delete_topic_tags')) : ?>
				...
				<fieldset class="bbp-form" id="delete-tag">
					...
					<div class="bbp-template-notice info">
						<p><?php _e('This does not delete your topics. Only the tag itself is deleted.',
						            'bbpress'); ?></p>
					</div>
					<div class="bbp-template-notice">
					...
					<form id="delete_tag" name="delete_tag" method="post" action="<?php the_permalink(); ?>">
						...
						<div class="bbp-submit-wrapper">
							<button type="submit" tabindex="<?php bbp_tab_index(); ?>" class="button submit"
							        onclick="return confirm('<?php echo esc_js(sprintf(__('Are you sure you want to delete the "%s" tag? This is permanent and cannot be undone.',
							                                                              'bbpress'),
							                                                           bbp_get_topic_tag_name())); ?>');"><?php esc_attr_e('Delete',
							                                                                                                               'bbpress'); ?></button>
