...
$mobile_columns = '2';
if (isset($flatsome_opt['category_row_count_mobile'])) {
	$mobile_columns = $flatsome_opt['category_row_count_mobile'];
}
$class                    = isset($category->depth) ? 'product-category-level-' . ($category->depth + 2)
...
<style>
	.wc-nested-category-layout-category-title + img {
		display: none;
	}
...
<ul class="subcategory-products products small-block-grid-<?php echo $mobile_columns; ?> large-block-grid-<?php echo $woocommerce_loop["columns"]; ?> <?php echo $class; ?>">
	<?php } else {
	if (isset($flatsome_opt['category_row_count']))
	{ ?>
	<ul class="subcategory-products products small-block-grid-<?php echo $mobile_columns; ?> large-block-grid-<?php echo $flatsome_opt['category_row_count']; ?> <?php echo $class; ?>">
		...
		<?php if ($see_more) : ?>
			<a class="woocommerce-nested-category-layout-see-more clearfix button small alt-button"
			   href="<?php echo esc_attr(get_term_link($category)) ?>"><?php echo apply_filters('wc_nested_category_layout_see_more_message',
			                                                                                    __('See more',
			                                                                                       WC_Nested_Category_Layout::TEXT_DOMAIN),
			                                                                                    $category); ?></a>
		<?php endif; ?>
		<hr/>