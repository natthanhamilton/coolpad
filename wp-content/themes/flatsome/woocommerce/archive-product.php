...
<?php if ($flatsome_opt['masonry_grid']) { ?>
	<style>
		/* Masonery Grid style */
		.products li:nth-child(even) .front-image {
			margin-top: -30px;
		}

		.products li:nth-child(4) .front-image,
		.products li:nth-child(8) .front-image,
		.products li:nth-child(12) .front-image {
			margin-top: -50px;
		}
	</style>
<?php } ?>
	...
	<div class="large-12 columns">
		...
		<?php if ($flatsome_opt['category_sidebar'] == 'off-canvas') { ?>
			<div class="category-filtering mob-center">
				<a href="#shop-sidebar" class="off-canvas-overlay filter-button" data-pos="left"
				   data-color="light"><span class="icon-menu"></span> <?php echo __('Filter', 'woocommerce'); ?></a>
				<?php the_widget('WC_Widget_Layered_Nav_Filters'); ?>
			</div><!-- Category filtering -->
		<?php } else {
			if ($flatsome_opt['category_sidebar'] !== 'none') { ?>
				<div class="category-filtering mob-center show-for-small">
					<a href="#shop-sidebar" class="off-canvas-overlay filter-button" data-pos="left" data-color="light"><span
							class="icon-menu"></span> <?php echo __('Filter', 'woocommerce'); ?></a>
					<?php the_widget('WC_Widget_Layered_Nav_Filters'); ?>
				</div><!-- Category filtering -->
			<?php }
		} ?>
...
<?php if ($flatsome_opt['category_sidebar'] == 'right-sidebar') { ?>
	...
	<?php } else {
	if ($flatsome_opt['category_sidebar'] == 'left-sidebar') { ?>
...
<?php if ($flatsome_opt['search_result'] && get_search_query()) : ?>
	 ...
	 */
	query_posts(['post_type' => ['post', 'page'], 's' => get_search_query()]);
	if (have_posts()) {
	...
	while (have_posts()) : the_post();
		...
		if ($post->post_type == 'page') {
			foreach (['shop', 'cart', 'checkout', 'view_order', 'terms'] as $wc_page_type) {
...
<?php
$no_widgets_msg
	= '<p style="padding:30% 10%">You need to assign Widgets to <strong>"Shop Sidebar"</strong> in <a href="' . get_site_url() . '/wp-admin/widgets.php">Appearance > Widgets</a> to show anything here</p>'; ?>
<?php if ($flatsome_opt['category_sidebar'] == 'right-sidebar') { ?>
	<!-- Right Shop sidebar -->
...
<?php } else {
	...
	<?php } else {
		if ($flatsome_opt['category_sidebar'] == 'off-canvas') { ?>
