...
<?php if (in_array('nextend-facebook-connect/nextend-facebook-connect.php', apply_filters('active_plugins',
                                                                                          get_option('active_plugins'))) && $flatsome_opt['facebook_login'] && get_option('woocommerce_enable_myaccount_registration') == 'yes' && !is_user_logged_in()
) { ?>
	<div id="facebook-login" class="ux_banner dark" style="height:180px">
		<div class="banner-bg"
		     style="background-image:url('<?php echo $flatsome_opt['facebook_login_bg']; ?>');background-color:#ddd"></div>
		<div class="row">
