...
<div id="wrapper"<?php if ($flatsome_opt['box_shadow']) echo ' class="box-shadow"'; ?>>
	<div class="header-wrapper before-sticky">
						...
						<div class="right-text right">
							...
							<ul id="menu-top-bar-menu" class="top-bar-nav">
									...
									<li class="account-dropdown menu-parent-item">
										...
										if (is_user_logged_in()) { ?>
											<a href="<?php echo get_permalink(get_option('woocommerce_myaccount_page_id')); ?>"
											   class="nav-top-link nav-top-login">
												...
												<ul>
													...
													<?php } else {
														if (!function_exists('wc_get_account_menu_items')) { ?>
															<li>Define your My Account dropdown menu in <b>Appearance >
																	Menus</b></li>
														<?php }
													} ?>
													<?php if (function_exists('wc_get_account_menu_items') && $flatsome_opt['wc_account_links']) { ?>
														<?php foreach (wc_get_account_menu_items() as $endpoint =>
														               $label) : ?>
															<li>
								...
								<?php if (ux_is_woocommerce_active() && $flatsome_opt['show_cart'] == 'top_bar') { ?>
									<li class="mini-cart-top-bar">
										<a href="<?php echo esc_url($woocommerce->cart->get_cart_url()); ?>">
											...
											<span
												class="label"><?php echo $woocommerce->cart->cart_contents_count; ?></span>
										</a>
									</li><!-- .mini-cart -->
								<?php } ?>
								...
								<?php if ($flatsome_opt['topbar_right']) { ?>
									<li class="html-block">
										<div
											class="html-block-inner"><?php echo do_shortcode($flatsome_opt['topbar_right']); ?></div>
									</li>
								<?php } ?>
							</ul>
						</div><!-- top bar right -->
			...
			<div class="row">
				<div class="large-12 columns header-container">
						...
						<div id="logo" class="logo-left">
							<a href="<?php echo esc_url(home_url('/')); ?>"
							   title="<?php echo esc_attr(get_bloginfo('name',
							                                           'display')); ?> - <?php bloginfo('description'); ?>"
							   rel="home">
					...
					<div class="left-links">
						...
						<?php } else {
							if ($flatsome_opt['nav_position'] == 'bottom' || $flatsome_opt['nav_position'] == 'bottom_center') { ?>
					...
					<div class="right-links">
						<ul <?php if ($flatsome_opt['nav_position'] == 'top_right'){ ?>id="site-navigation"<?php } ?>
						    class="header-nav">
							...
							<?php if ($flatsome_opt['top_right_text']) { ?>
								<li class="html-block">
									<div
										class="html-block-inner hide-for-small"><?php echo do_shortcode($flatsome_opt['top_right_text']); ?></div>
								</li>
							<?php } ?>
									...
									<?php if (function_exists('wc_print_notices')) { ?>
										<li class="mini-cart <?php if ($flatsome_opt['show_cart'] == 'top_bar') echo 'hide-for-medium'; ?>">
											<div class="cart-inner">
												...
												<a href="<?php echo esc_url($woocommerce->cart->get_cart_url()); ?>"
												   class="cart-link">
													<strong class="cart-name hide-for-small"><?php _e('Cart',
													                                                  'woocommerce'); ?></strong>
													<span
														class="cart-price hide-for-small">/ <?php echo $woocommerce->cart->get_cart_subtotal(); ?></span>
													<!-- cart icon -->
													<div class="cart-icon">
														<?php if ($flatsome_opt['custom_cart_icon']) { ?>
															<div class="custom-cart-inner">
																<div
																	class="custom-cart-count"><?php echo $woocommerce->cart->cart_contents_count; ?></div>
																<img class="custom-cart-icon"
																     alt="<?php _e('Cart', 'woocommerce'); ?>"
																     src="<?php echo $flatsome_opt['custom_cart_icon'] ?>"/>
												...
												<div id="mini-cart-content" class="nav-dropdown">
													<div class="nav-dropdown-inner">
														...
														<?php if ($woocommerce->cart->cart_contents_count == 0) {
															echo '<p class="empty">' . __('No products in the cart.',
															                              'woocommerce') . '</p>';
															?>
		...
		<?php if ($flatsome_opt['nav_position'] == 'bottom' || $flatsome_opt['nav_position'] == 'bottom_center') { ?>
			...
			<div
				class="wide-nav <?php echo $flatsome_opt['nav_position_color']; ?> <?php if ($flatsome_opt['nav_position'] == 'bottom_center') {
					echo 'nav-center';
				} else {
					echo 'nav-left';
				} ?>">
