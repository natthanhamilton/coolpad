...
class Flatsome_Recent_Post_Widget extends WP_Widget {
	function Flatsome_Recent_Post_Widget() {
		$widget_ops
			= ['classname' => 'flatsome_recent_posts', 'description' => __('A widget that displays recent posts ',
			                                                               'flatsome')];
		$control_ops = ['width' => 300, 'height' => 350, 'id_base' => 'flatsome_recent_posts'];
		parent::__construct('flatsome_recent_posts', __('Flatsome Recent Posts', 'flatsome'), $widget_ops,
		                    $control_ops);
	}
	...
	function widget($args, $instance) {
		...
		$title = apply_filters('widget_title',
		                       empty($instance['title']) ? __('Recent Posts', 'flatsome') : $instance['title'],
		                       $instance, $this->id_base);
		if (empty($instance['number']) || !$number = absint($instance['number'])) {
			$number = 10;
		}
		$r = new WP_Query(apply_filters('widget_posts_args',
		                                ['posts_per_page' => $number, 'no_found_rows' => TRUE, 'post_status' => 'publish', 'ignore_sticky_posts' => TRUE]));
		if ($r->have_posts()) :
			?>
			...
			<ul>
				<?php while ($r->have_posts()) : $r->the_post(); ?>
					<li>
						...
						<a href="<?php the_permalink() ?>" title="<?php echo esc_attr(get_the_title() ? get_the_title()
							                                                              : get_the_ID()); ?>"><?php if (get_the_title()) the_title(); else the_ID(); ?></a>
						<div class="post_comments"><?php comments_popup_link(__('Leave a comment', 'flatsome'),
						                                                     __('<strong>1</strong> Comment',
						                                                        'flatsome'),
						                                                     __('<strong>%</strong> Comments',
						                                                        'flatsome')); ?></div>
					</li>
				<?php endwhile; ?>
			</ul>
