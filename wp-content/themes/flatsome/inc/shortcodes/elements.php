...
function ux_instagram_feed($atts, $content = NULL) {
    ...
    if (class_exists('null_instagram_widget')) {
        ...
        the_widget('null_instagram_widget',
                   ['username' => $username, 'target' => '_blank', 'number' => $photos, 'link' => $text],
                   ['before_widget' => '<div class="ux-instagram-feed">', 'after_wiget' => '</div>']);
        $w = ob_get_contents();
        ob_end_clean();
...
function ux_price_table($atts, $content = NULL) {
    ...
    <div class="ux_price_table text-center <?php if ($featured == 'true') { ?>featured-table box-shadow<?php } ?>">
        <ul class="pricing-table">
            ...
            <?php if ($button_style) { ?>
                <li class="cta-button"><a class="button <?php echo $button_style; ?>"
                                          href="<?php echo $button_link; ?>"><?php echo $button_text; ?></a></li>
            <?php } ?>
        </ul>
    </div>
...
function ux_logo($atts, $content = NULL) {
    ...
    $content
        = '<div class="ux_logo"><a title="' . $title . '" href="' . $link . '" style="width: ' . $width . 'px; padding: ' . $padding . ';"><img src="' . $org_img . '" title="' . $title . '" alt="' . $title . '" style="min-height:' . $height . '" /></a></div>';
...
function ux_image($atts, $content = NULL) {
    ...
    $content
        = '<div class="ux-img-container ' . $drop_shadow . '">' . $link_start . '<img src="' . $img . '" alt="' . $title . '" title="' . $title . '" style="bottom:-' . $image_pull . '"/>' . $link_end . '</div>';
...
function ux_phone($atts, $content = NULL) {
    ...
    $content
        = '<div class="ux-header-element element-phone"><a href="tel:' . $number . '" class="circle ' . $tooltip_class . '" title="' . $tooltip . '" style="border-width:' . $border . '"><span class="icon-phone"></span> ' . $number . '</a></div>';
...
function ux_header_button($atts, $content = NULL) {
    ...
    if ($tooltip) $tooltip_class = 'tip-top';
    $content
        = '<div class="ux-header-element header_button"><a href="' . $link . '" class="circle ' . $tooltip_class . '" title="' . $tooltip . '" target="' . $target . '" style="border-width:' . $border . '">' . $text . '</a></div>';
