...
function uxbannerShortcode($atts, $content = NULL) {
    ...
    if ($parallax) {
        ...
        $parallax       = 'data-velocity="0.' . $parallax . '"';
    }
    $text_parallax_class = '';
    if ($parallax_text) {
        $text_parallax_class = ' parallax_text';
        $parallax_text       = 'data-velocity="0.' . $parallax_text . '"';
    }
    // Background overlay;
    $background_overlay = '';
    ...
    ?>
    <div id="banner_<?php echo $bannerid; ?>"
         class="ux_banner <?php echo $color; ?> <?php echo $has_parallax; ?> <?php echo $class; ?> <?php if ($height == '100%') echo 'full-height'; ?> <?php echo $hover; ?>"
         style="height:<?php echo $height; ?>; <?php echo $bg_color; ?>" data-height="<?php echo $height; ?>"
         role="banner">
        <?php echo $start_link; ?>
        <?php if ($youtube) { ?>
        ...
        <?php if ($bg || $bg_color) { ?>
            ...
            <div
                class="banner-bg<?php echo $parallax_class; ?> <?php echo $slide_effect; ?>" <?php if ($parallax) echo $parallax; ?>
                style="background-image:url('<?php echo $background; ?>'); <?php echo $bg_color; ?>"><?php if ($background) { ?>
                    <img src="<?php echo $background; ?>"  alt="<?php echo $alt; ?>"
                         style="visibility:hidden;" /><?php } ?></div>
        <?php } ?>
        <?php if ($video_mp4 || $video_webm || $video_ogg) { ?>
            <video id="banner_video" class="ux-banner-video hide-for-small" <?php if ($parallax) echo $parallax; ?>
                   poster="<?php echo $background; ?>" preload="auto"
                   autoplay <?php if ($video_sound == 'false') echo "muted='muted'"; ?>
        ...
        <?php } ?>
        <?php if ($effect) {
            wp_enqueue_style('flatsome-effect'); ?>
            <div class="banner-effect effect-<?php echo $effect; ?>"></div>
        <?php } ?>
        <div
            class="row<?php if ($parallax_text) echo ' parallax_text'; ?><?php echo $text_parallax_class; ?>" <?php if ($parallax_text) echo $parallax_text; ?>>
            <div class="inner <?php echo $text_pos; ?> <?php echo $textalign; ?> <?php if ($text_bg) {
                echo 'text-boxed';
            } ?>
" style="width:<?php echo $text_width; ?>;">
                <div class="inner-wrap <?php echo $animated; ?>" style="<?php if ($text_bg) {
        ...
        <?php if ($mob_height || $tablet_height || $animation_duration) { ?>
            <style>
                <?php if($tablet_height ){ ?>@media only screen and (max-width: 768px) {
                    #banner_<?php echo $bannerid; ?> {
                        height: <?php echo $tablet_height; ?> !important
                    }
                }

                <?php } // mob height ?>
                <?php if($mob_height){ ?>@media only screen and (max-width: 480px) {
                    #banner_<?php echo $bannerid; ?> {
                        height: <?php echo $mob_height; ?> !important
                    }
                }

                <?php } // mob height ?>
                <?php if($animation_duration){ ?>
                #banner_<?php echo $bannerid; ?> .inner-animate {
                    animation-duration: <?php echo $animation_duration; ?>;
                    -webkit-animation-duration: <?php echo $animation_duration; ?>;
                    -moz-animation-duration: <?php echo $animation_duration; ?>;
                    -o-animation-duration: <?php echo $animation_duration; ?>;
                }
