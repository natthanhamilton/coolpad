...
function ux_tabgroup($params, $content = NULL) {
    ...
    if (is_array($GLOBALS['tabs'])) {
        foreach ($GLOBALS['tabs'] as $key => $tab) {
            ...
            $tabs[]
                    = '<li class="tab' . $active . '"><a href="#panel' . $randomid . $i . '">' . $tab['title'] . '</a></li>';
            $panes[]
                    = '<div class="panel' . $active . '" id="panel' . $randomid . $i . '">' . fixShortcode($tab['content']) . '</div>';
            $i++;
        }
...
function ux_tabgroup_vertical($params, $content = NULL) {
    ...
    if (is_array($GLOBALS['tabs'])) {
        foreach ($GLOBALS['tabs'] as $key => $tab) {
            ...
            $active  = $key == 0 ? ' active' : ''; // Set first tab active by default.
            $tabs[]
                     = '<li class="tab' . $current . '"><a href="#panel' . $randomid . $i . '">' . $tab['title'] . '</a></li>';
            $panes[]
                     = '<div class="tabs-inner' . $active . '" id="panel' . $randomid . $i . '">' . fixShortcode($tab['content']) . '</div>';
            $i++;
        }
        if ($title) $title = '<h3>' . $title . '</h3>';
        $return
...
function ux_tab($params, $content = NULL) {
    ...
    $x                     = $GLOBALS['tab_count'];
    $GLOBALS['tabs'][ $x ] = ['title' => sprintf($title, $GLOBALS['tab_count']), 'content' => $content];
    $GLOBALS['tab_count']++;
}
