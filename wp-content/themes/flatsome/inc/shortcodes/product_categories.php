...
function ux_product_categories($atts, $content = NULL) {
    ...
    if ($parent !== "") {
        $product_categories = wp_list_filter($product_categories, ['parent' => $parent]);
    }
    if ($number) {
              ...
              "contain": true
                ...
                if ($product_categories) {
                    foreach ($product_categories as $category) {
                        woocommerce_get_template('content-product_cat.php', [
                            'category' => $category,
                            'style'    => $style
                        ]);
                    }
...
function ux_product_categories_grid($atts, $content = NULL) {
    ...
    <?php if ($padding) {
        $padding_w = $padding / 2; ?>
        <style>
            #banner_grid_<?php echo $bannergridID ?> .ux_banner-grid {
                margin-left: -<?php echo $padding_w; ?>px !important;
                margin-right: -<?php echo $padding_w ?>px !important;
            }

            #banner_grid_<?php echo $bannergridID ?> .ux_banner-grid .columns {
                margin-bottom: <?php echo $padding; ?> !important
            }

            #banner_grid_<?php echo $bannergridID ?> .ux_banner-grid .columns > .column-inner {
                padding-left: <?php echo $padding_w; ?>px !important;
                padding-right: <?php echo $padding_w ?>px !important;
            }
        </style>
    <?php } ?>
                    ...
                    if (function_exists('wc_print_notices')) { ?>
                        ...
                        if ($product_categories) {
                            foreach ($product_categories as $category) {
                                ...
                                $image        = $image[0];
                                if ($category->count > 0) $cat_count
                                    = apply_filters('woocommerce_subcategory_count_html',
                                                    ' <span class="count">' . $category->count . ' ' . __('Products',
                                                                                                          'woocommerce') . '</span>',
                                                    $category);
                                $cat_link = get_term_link($category->slug, 'product_cat');
                                ?>
                                <div class="columns ux-grid-column <?php echo $cat_span; ?>"
                                     style="height:<?php echo $cat_height; ?>">
