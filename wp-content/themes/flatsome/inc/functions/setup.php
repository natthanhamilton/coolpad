...
function flatsome_scripts() {
    ...
    if (!isset($flatsome_opt['minified_flatsome']) || !$flatsome_opt['minified_flatsome']) {
        wp_enqueue_style('flatsome-icons', get_template_directory_uri() . '/css/fonts.css', [], $version, 'all');
        wp_enqueue_style('flatsome-animations', get_template_directory_uri() . '/css/animations.css', [], $version,
                         'all');
        wp_enqueue_style('flatsome-main-css', get_template_directory_uri() . '/css/foundation.css', [], $version,
                         'all');
        wp_register_style('flatsome-effect', get_template_directory_uri() . '/css/effects.css', [], $version, 'all');
    } else {
        wp_enqueue_style('flatsome-css-minified', get_template_directory_uri() . '/css/flatsome.min.css', [], $version,
                         'all');
    }
    /* Load Custom styles CSS */
    wp_enqueue_style('flatsome-style', get_stylesheet_uri(), [], $version, 'all');
    /* JS libaries */
    if (!isset($flatsome_opt['minified_flatsome']) || !$flatsome_opt['minified_flatsome']) {
        wp_enqueue_script('flatsome-modernizer', get_template_directory_uri() . '/js/modernizr.js', ['jquery'],
                          $version, TRUE);
        wp_enqueue_script('flatsome-plugins-js', get_template_directory_uri() . '/js/flatsome-plugins.js', ['jquery'],
                          $version, TRUE);
        wp_enqueue_script('flatsome-theme-js', get_template_directory_uri() . '/js/flatsome-theme.js', ['jquery'],
                          $version, TRUE);
        if (ux_is_woocommerce_active()) {
            wp_enqueue_script('flatsome-woocommerce-js', get_template_directory_uri() . '/js/flatsome-woocommerce.js',
                              ['jquery'], $version, TRUE);
        }
    } else {
        wp_enqueue_script('flatsome-theme-js-minified', get_template_directory_uri() . '/js/flatsome.min.js',
                          ['jquery'], $version, TRUE);
    }
    /* add JS variables to scripts */
    wp_localize_script('flatsome-theme-js', 'ajaxURL', ['ajaxurl' => admin_url('admin-ajax.php')]);
    wp_localize_script('flatsome-theme-js-minified', 'ajaxURL', ['ajaxurl' => admin_url('admin-ajax.php')]);
