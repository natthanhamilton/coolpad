...
if (isset($flatsome_opt['type_headings']) && !$flatsome_opt['disable_fonts']) {
    ...
    if (isset($flatsome_opt['type_subset'])) {
        $subsets = ['latin'];
        if (isset($flatsome_opt['type_subset']['latin-ext']) && $flatsome_opt['type_subset']['latin-ext']) {
            array_push($subsets, 'latin-ext');
        }
        if (isset($flatsome_opt['type_subset']['cyrillic-ext']) && $flatsome_opt['type_subset']['cyrillic-ext']) {
            array_push($subsets, 'cyrillic-ext');
        }
        if (isset($flatsome_opt['type_subset']['greek-ext']) && $flatsome_opt['type_subset']['greek-ext']) {
            array_push($subsets, 'greek-ext');
        }
        if (isset($flatsome_opt['type_subset']['greek']) && $flatsome_opt['type_subset']['greek']) {
            array_push($subsets, 'greek');
        }
        if (isset($flatsome_opt['type_subset']['vietnamese']) && $flatsome_opt['type_subset']['vietnamese']) {
            array_push($subsets, 'vietnamese');
        }
        if (isset($flatsome_opt['type_subset']['cyrillic']) && $flatsome_opt['type_subset']['cyrillic']) {
    ...
    if ($customfont != "") {
        function google_fonts() {
            ...
            wp_enqueue_style('flatsome-googlefonts',
                             "//fonts.googleapis.com/css?family=" . substr_replace($customfont, "",
                                                                                   -1) . $customfontset);
        }
