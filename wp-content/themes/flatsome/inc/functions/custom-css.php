...
function flatsome_custom_css() {
    ...
    <style type="text/css">
        ...
        .top-bar-nav a.nav-top-link, body, p, #top-bar, .cart-inner .nav-dropdown, .nav-dropdown {
            font-family: <?php echo $flatsome_opt['type_texts'] ?>, helvetica, arial, sans-serif
        }

        .header-nav a.nav-top-link, a.cart-link, .mobile-sidebar a {
            font-family: <?php echo $flatsome_opt['type_nav'] ?>, helvetica, arial, sans-serif
        }

        h1, h2, h3, h4, h5, h6 {
            font-family: <?php echo $flatsome_opt['type_headings'] ?>, helvetica, arial, sans-serif
        }

        .alt-font {
            font-family: <?php echo $flatsome_opt['type_alt'] ?>, Georgia, serif !important;
        }
        ...
        body.boxed, body.framed-layout, body {
            background-color: <?php echo $flatsome_opt['body_bg'] ?>;
            background-image: url("<?php echo $flatsome_opt['body_bg_image'] ?>");
        }
        ...
        #masthead {
            height: <?php echo $flatsome_opt['header_height']; ?>px;
        }

        <?php if($flatsome_opt['header_height'] > '70'){ ?>
        #logo a img {
            max-height: <?php $height = $flatsome_opt['header_height'];  $height = str_replace("px","", $height);  $height = ($height)-30; $height = $height.'px'; echo $height;?>
        }

        <?php } else { ?>
        #logo a img {
            max-height: <?php $height = $flatsome_opt['header_height'];  $height = str_replace("px","", $height);  $height = ($height)-10; $height = $height.'px'; echo $height;?>
        }
        ...
        .logo-center #masthead #logo {
            width: <?php echo $flatsome_opt['logo_width']*100/1080;?>%
        }

        .logo-center #masthead .left-links, .logo-center #masthead .right-links {
            width: <?php echo (100-($flatsome_opt['logo_width']*100/1080))/2; ?>%
        }

        #masthead #logo a {
            max-width: <?php echo $flatsome_opt['logo_width']; ?>px
        }

        <?php } else { ?>
        #masthead #logo {
            width: <?php echo $flatsome_opt['logo_width'] ?>px;
        }
        ...
        #masthead.stuck.move_down {
            height: <?php  echo $flatsome_opt['header_height_sticky']; ?>px;
        }

        .wide-nav.move_down {
            top: <?php  echo $flatsome_opt['header_height_sticky']; ?>px;
        }

        <?php if($flatsome_opt['header_height_sticky'] > '70'){ ?>
        #masthead.stuck.move_down #logo a img {
            max-height: <?php $height = $flatsome_opt['header_height_sticky'];  $height = str_replace("px","", $height);  $height = ($height)-30; $height = $height.'px'; echo $height;?>
        }

        <?php } else { ?>
        #masthead.stuck.move_down #logo a img {
            max-height: <?php $height = $flatsome_opt['header_height_sticky'];  $height = str_replace("px","", $height);  $height = ($height)-10; $height = $height.'px'; echo $height;?>
        }
        ...
        ul.header-nav li a {
            font-size: <?php echo $flatsome_opt['nav_size']; ?>
        }
        ...
        .slider-nav-reveal .flickity-prev-next-button, #main-content {
            background-color: <?php echo $flatsome_opt['content_bg']; ?> !important
        }
        ...
        .wide-nav {
            background-color: <?php echo $flatsome_opt['nav_position_bg']; ?>
        }
        ...
        #top-bar {
            background-color: <?php echo $flatsome_opt['topbar_bg'] ?>
        }

        <?php } else { ?>
        #top-bar {
            background-color: <?php echo $flatsome_opt['color_primary'] ?>
        }
        ...
        .ux-timer-text.primary span .alt-button.primary, .callout.style3 .inner .inner-text, .add-to-cart-grid .cart-icon strong, .tagcloud a, .navigation-paging a, .navigation-image a, ul.page-numbers a, ul.page-numbers li > span, #masthead .mobile-menu a, .alt-button, #logo a, li.mini-cart .cart-icon strong, .widget_product_tag_cloud a, .widget_tag_cloud a, .post-date, #masthead .mobile-menu a.mobile-menu a, .checkout-group h3, .order-review h3 {
            color: <?php echo $flatsome_opt['color_primary'] ?>;
        }
        ...
        button[disabled], .button[disabled], .slider-nav-circle .flickity-prev-next-button:hover svg, .slider-nav-circle .flickity-prev-next-button:hover .arrow, .ux-header-element a:hover, .featured-table.ux_price_table, .text-bordered-primary, .callout.style3 .inner, ul.page-numbers li > span, .add-to-cart-grid .cart-icon strong, .add-to-cart-grid .cart-icon-handle, .add-to-cart-grid.loading .cart-icon strong, .navigation-paging a, .navigation-image a, ul.page-numbers a, ul.page-numbers a:hover, .post.sticky, .widget_product_tag_cloud a, .widget_tag_cloud a, .next-prev-nav .prod-dropdown > a:hover, .iosSlider .sliderNav a:hover span, .column-slider .sliderNav a:hover, .woocommerce .order-review, .woocommerce-checkout form.login, .button, button, li.mini-cart .cart-icon strong, li.mini-cart .cart-icon .cart-icon-handle, .post-date {
            border-color: <?php echo $flatsome_opt['color_primary'] ?>;
        }

        .blockUI:before, .processing:before, .ux-loading {
            border-left-color: <?php echo $flatsome_opt['color_primary'] ?>;
        }

        /* -- alt buttons-- */
        .primary.alt-button:hover, .button.alt-button:hover {
            background-color: <?php echo $flatsome_opt['color_primary'] ?> !important
        }

        /* -- icons -- */
        .flickity-prev-next-button:hover svg, .flickity-prev-next-button:hover .arrow, .featured-box:hover svg, .featured-img svg:hover {
            fill: <?php echo $flatsome_opt['color_primary']; ?> !important;
        }
        ...
        .featured-box:hover .featured-img-circle {
            background-color: <?php echo $flatsome_opt['color_primary'] ?> !important;
            border-color: <?php echo $flatsome_opt['color_primary'] ?> !important;
        }
        ...
        .star-rating:before, .woocommerce-page .star-rating:before, .star-rating span:before {
            color: <?php echo $flatsome_opt['color_secondary'] ?>
        }

        .secondary.alt-button, li.menu-sale a {
            color: <?php echo $flatsome_opt['color_secondary'] ?> !important
        }
        ...
        .secondary-bg.button.alt-button.success:hover, .label-sale.menu-item a:after, .mini-cart:hover .custom-cart-count, .callout .inner, .button.secondary, .button.checkout, #submit.secondary, button.secondary, .button.secondary, input[type="submit"].secondary {
            background-color: <?php echo $flatsome_opt['color_secondary'] ?>
        }

        /* -- borders -- */
        .button.secondary, .button.secondary {
            border-color: <?php echo $flatsome_opt['color_secondary'] ?>;
        }

        /* -- alt buttons-- */
        .secondary.alt-button:hover {
            color: #FFF !important;
            background-color: <?php echo $flatsome_opt['color_secondary'] ?> !important
        }
        ...
        form.cart .button, .cart-inner .button.checkout, .checkout-button, input#place_order {
            background-color: <?php echo $flatsome_opt['color_checkout'] ?> !important
        }
        ...
        /* Sale bubble color */
        .callout .inner {
            background-color: <?php echo $flatsome_opt['color_sale'] ?> !important
        }

        .callout.style3 .inner {
            background: transparent !important;
            border-color: <?php echo $flatsome_opt['color_sale'] ?> !important
        }

        .callout.style3 .inner .inner-text {
            color: <?php echo $flatsome_opt['color_sale'] ?> !important;
        }
        ...
        .star-rating span:before, .star-rating:before, .woocommerce-page .star-rating:before {
            color: <?php echo $flatsome_opt['color_review'] ?> !important
        }
        ...
        /* LINK COLOR */
        a, .icons-row a.icon {
            color: <?php echo $flatsome_opt['color_links'] ?>
        }

        .cart_list_product_title {
            color: <?php echo $flatsome_opt['color_links'] ?> !important
        }

        .icons-row a.icon {
            border-color: <?php echo $flatsome_opt['color_links'] ?>;
        }

        .icons-row a.icon:hover {
            background-color: <?php echo $flatsome_opt['color_links'] ?>;
            border-color: <?php echo $flatsome_opt['color_links'] ?>;
        }
        ...
        input[type="submit"], .button {
            -webkit-border-radius: <?php echo $flatsome_opt['button_radius']; ?> !important;
            -moz-border-radius: <?php echo $flatsome_opt['button_radius']; ?> !important;
            border-radius: <?php echo $flatsome_opt['button_radius']; ?> !important;
        }
        ...
        .nav-dropdown {
            border-color: <?php echo $flatsome_opt['dropdown_border'] ?>;
        }

        .nav-dropdown > ul:after, .nav-dropdown > .row:after, .nav-dropdown-inner:after, .menu-item-language .sub-menu:after {
            border-bottom-color: <?php echo $flatsome_opt['dropdown_border'] ?>;
        }
        ...
        .nav-dropdown {
            background-color: <?php echo $flatsome_opt['dropdown_bg']; ?>;
        }

        <?php } ?>

        <?php if($flatsome_opt['dropdown_text'] == "dark") { ?>
        .mini-cart-item, .cart_list_product_title, .minicart_total_checkout, ul.top-bar-nav .nav-dropdown ul li, ul.top-bar-nav .nav-dropdown li a, ul.header-nav li .nav-dropdown li a {
            color: #eee !important;
            border-color: rgba(255, 255, 255, 0.1) !important;
            border-left: 0;
        }

        .nav-dropdown p.empty, ul.top-bar-nav .nav-dropdown li a:hover, ul.header-nav li .nav-dropdown li a:hover {
            color: #fff !important;
        }

        .remove .icon-close:hover, ul.header-nav li .nav-dropdown > ul > li.menu-parent-item > a {
            color: #EEE;
        ...
        .footer.footer-1 {
            background-image: url('<?php echo $flatsome_opt['footer_1_bg_image']; ?>');
        }
        ...
        .footer.footer-2 {
            background-image: url('<?php echo $flatsome_opt['footer_2_bg_image']; ?>');
        }
        ...
        .label-new.menu-item > a:after {
            content: "<?php _e('New','flatsome'); ?>";
        }

        .label-hot.menu-item > a:after {
            content: "<?php _e('Hot','flatsome'); ?>";
        }

        .label-sale.menu-item > a:after {
            content: "<?php _e('Sale','flatsome'); ?>";
        }

        .label-popular.menu-item > a:after {
            content: "<?php _e('Popular','flatsome'); ?>";
        }

        /* featured items */
        .featured_item_image {
            max-height: <?php echo $flatsome_opt['featured_items_related_height'];?>
        }

        /* BUBBLE COLORS */
        .callout .inner.callout-new-bg {
            background-color: <?php echo $flatsome_opt['color_new_bubble'];?> !important;
        }

        .callout.style3 .inner.callout-new-bg {
            background-color: none !important;
            border-color: <?php echo $flatsome_opt['color_new_bubble'];?> !important
        }
        ...
        #wpadminbar #wp-admin-bar-admin_bar_helper > .ab-item:before {
            content: "\f107";
            top: 2px;
        }
