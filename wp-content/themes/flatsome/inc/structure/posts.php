    ...
    function flatsome_content_nav($nav_id) {
        ...
        <nav role="navigation" id="<?php echo esc_attr($nav_id); ?>" class="<?php echo $nav_class; ?>">
            <?php if (is_single()) : // navigation links for single posts ?>
                ...
                <?php previous_post_link('<div class="nav-previous left">%link</div>',
                                         '<span class="icon-angle-left">' . _x('', 'Previous post link',
                                                                               'flatsome') . '</span> %title'); ?>
                <?php next_post_link('<div class="nav-next right">%link</div>',
                                     '%title <span class="icon-angle-right">' . _x('', 'Next post link',
                                                                                   'flatsome') . '</span>'); ?>
            ...
            <?php elseif ($wp_query->max_num_pages > 1 && (is_home() || is_archive() || is_search())) : // navigation links for home, archive, and search pages ?>
                ...
                <?php if (get_next_posts_link()) : ?>
                    <div
                        class="nav-previous"><?php next_posts_link(__('<span class="icon-angle-left"></span> Older posts',
                                                                      'flatsome')); ?></div>
                <?php endif; ?>

                <?php if (get_previous_posts_link()) : ?>
                    <div
                        class="nav-next"><?php previous_posts_link(__('Newer posts <span class="icon-angle-right"></span>',
                                                                      'flatsome')); ?></div>
                <?php endif; ?>
    ...
    function flatsome_comment($comment, $args, $depth) {
        ...
        switch ($comment->comment_type) :
            ...
            case 'trackback' :
                ...
                <p><?php _e('Pingback:',
                            'flatsome'); ?><?php comment_author_link(); ?><?php edit_comment_link(__('Edit',
                                                                                                     'flatsome'),
                ...
                <article id="comment-<?php comment_ID(); ?>" class="comment-inner">
                    ...
                    <div class="row collapse">
                        ...
                        <div class="large-10 columns">
                            <?php printf(__('%s <span class="says">says:</span>', 'flatsome'),
                            ...
                            <div class="comment-meta commentmetadata">
                                <a href="<?php echo esc_url(get_comment_link($comment->comment_ID)); ?>">
                                    <time datetime="<?php comment_time('c'); ?>">
                                        <?php printf(_x('%1$s at %2$s', '1: date, 2: time', 'flatsome'),
                                                     get_comment_date(), get_comment_time()); ?>
                                    </time>
                                </a>
                                <?php edit_comment_link(__('Edit', 'flatsome'), '<span class="edit-link">',
                                                        '<span>'); ?>
