...
function flatsome_favicons() {
    global $flatsome_opt;
    ?>
    <link rel="shortcut icon" href="<?php if (isset($flatsome_opt['site_favicon']) && $flatsome_opt['site_favicon']) {
        echo $flatsome_opt['site_favicon']; ?>
    <?php } else { ?><?php echo get_template_directory_uri(); ?>/favicon.png<?php } ?>"/>

    <!-- Retina/iOS favicon -->
    <link rel="apple-touch-icon-precomposed"
          href="<?php if (isset($flatsome_opt['site_favicon_large']) && $flatsome_opt['site_favicon_large']) {
              echo $flatsome_opt['site_favicon_large']; ?>
...
class FlatsomeNavDropdown extends Walker_Nav_Menu {
    function start_lvl(&$output, $depth = 0, $args = []) {
        ...
        if ($display_depth == '1') {
            ...
            $class_names = 'nav-column-links';
        }
        $indent = str_repeat("\t", $depth);
        $output .= "\n$indent<div class=" . $class_names . "><ul>\n";
    }

    function end_lvl(&$output, $depth = 1, $args = []) {
        $indent = str_repeat("\t", $depth);
            ...
            <ul class="mobile-main-menu">
                ...
                <?php if (ux_is_woocommerce_active() && $flatsome_opt['myaccount_dropdown']) { ?>
                    ...
                    <li class="menu-item menu-account-item menu-item-has-children">
                        ...
                        <?php } else { ?>
                            <a href="<?php echo get_permalink(get_option('woocommerce_myaccount_page_id')); ?>"><?php _e('Login',
                                                                                                                         'woocommerce'); ?></a>
                        <?php } ?>
                    </li>
