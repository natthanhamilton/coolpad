...
if ($flatsome_opt['maintenance_mode']) {
     ...
     */
    $smm_active_message = __('<strong>Maintenance mode</strong> is <strong>active</strong>!',
                             'flatsome-maintenance-mode');
    $smm_admin_notice
                        = '<div id="message" class="error fade"><p>' . $smm_active_message . ' <a href="themes.php?page=optionsframework&tab=of-option-globalsettings">' . __('Deactivate it, when work is done.',
                                                                                                                                                                              'flatsome-maintenance-mode') . '</a></p></div>';
    ...
    function flatsome_maintenance_mode() {
        ...
        if (!current_user_can('edit_themes') || !is_user_logged_in()) {
            wp_die('<center><img src="' . $flatsome_opt['site_logo'] . '"/ style="max-width:200px;"><h2>' . __('Maintenance',
                                                                                                               'flatsome-maintenance-mode') . '</h2><p>' . $flatsome_opt['maintenance_mode_text'] . '</p></center>',
                   __('Maintenance', 'flatsome-maintenance-mode'), ['response' => '503']);
        }
    }
