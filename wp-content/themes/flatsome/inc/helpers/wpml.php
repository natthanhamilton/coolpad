...
if (ux_is_woocommerce_active()) {
    ...
    if (function_exists('pll_get_post') || function_exists('icl_object_id')) {
        ...
        function ux_cartcache_enqueue_scripts() {
            ...
            wp_enqueue_script('wc-cart-fragments', get_template_directory_uri() . '/js/fixes/cart-fragments-wpml.js',
                              ['jquery', 'jquery-cookie'], '1.0', TRUE);
        }
    }
}
function wp_page_menu_filter2($items) {
    $items = str_replace('menu-item-language-current',
                         'menu-item-language-current has-dropdown  menu-item-has-children', $items);
