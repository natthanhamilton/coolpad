...
function ux_shortcode_button() {
    ...
    if (!in_array($typenow, ['post', 'page', 'blocks', 'product'])) {
        return;
    }
...
function ux_shortcode_add_tinymce_plugin($plugin_array) {
    $plugin_array['ux_shortcode_insert']
        = get_template_directory_uri() . '/inc/extensions/ux-shortcode-inserter/shortcode_insert.js'; // CHANGE THE BUTTON SCRIPT HERE
    return $plugin_array;
}
...
function ux_shortcode_css() {
    wp_enqueue_style('ux_shortcode_insert_css',
                     get_template_directory_uri() . '/inc/extensions/ux-shortcode-inserter/style.css');
}
