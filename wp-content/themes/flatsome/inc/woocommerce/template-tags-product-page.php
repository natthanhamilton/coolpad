...
if (!function_exists('flatsome_product_video_button')) {
    function flatsome_product_video_button() {
        ...
        if ($wc_cpdf->get_value(get_the_ID(), '_product_video')) { ?>
            <a class="product-video-popup tip-top"
               href="<?php echo $wc_cpdf->get_value(get_the_ID(), '_product_video'); ?>"
               title="<?php echo __('Video', 'flatsome'); ?>">
...
if (!function_exists('flatsome_product_zoom_button')) {
    function flatsome_product_zoom_button() { ?>
        <a href="#product-zoom" class="zoom-button tip-top hide-for-small"
           title="<?php echo __('Zoom', 'flatsome'); ?>">
    ...
    if (is_a($next_post, 'WP_Post')) { ?>
        <div class="prod-dropdown">
            ...
            <div class="nav-dropdown" style="display: none;">
                <a title="<?php echo get_the_title($next_post->ID); ?>"
...
function previous_post_link_product() {
    ...
    if (is_a($prev_post, 'WP_Post')) { ?>
        <div class="prod-dropdown">
            ...
            <div class="nav-dropdown" style="display: none;">
                <a title="<?php echo get_the_title($prev_post->ID); ?>"
...
function ProductShowReviews() {
    ...
    ");
        if ($count > 0) {
            ...
            echo '<a href="#tab-reviews" class="scroll-to-reviews"><div class="star-rating tip-top" title="' . $count . ' review(s)"><span style="width:' . ($average * 16) . 'px"><span itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating" class="rating"><span itemprop="ratingValue">' . $average . '</span><span itemprop="reviewCount" class="hidden">' . $count . '</span></span> ' . __('out of 5',
                                                                                                                                                                                                                                                                                                                                                                                                                              'woocommerce') . '</span></div></a>';
        }
    }
}
...
function flatsome_add_product_page_filter() {
    ...
    if ($flatsome_opt['product_offcanvas_sidebar']) { ?>
        <div class="product-page-filtering show-for-small">
            <a href="#product-sidebar" style="font-size: 1em;" class="off-canvas-overlay filter-button" data-pos="left"
               data-color="light"><span class="icon-menu"></span><?php echo __('Categories', 'woocommerce'); ?></a>
