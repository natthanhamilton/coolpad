	...
	class TGM_Plugin_Activation {
			...
			foreach ($this->plugins as $slug => $plugin) {
				if (FALSE === $this->can_plugin_activate($slug)) {
					add_filter('plugin_action_links_' . $plugin['file_path'],
					           [$this, 'filter_plugin_action_links_activate'], 20);
				}
				if (TRUE === $plugin['force_activation']) {
					add_filter('plugin_action_links_' . $plugin['file_path'],
					           [$this, 'filter_plugin_action_links_deactivate'], 20);
				}
				if (FALSE !== $this->does_plugin_require_update($slug)) {
					add_filter('plugin_action_links_' . $plugin['file_path'],
					           [$this, 'filter_plugin_action_links_update'], 20);
				}
			}
		}
		...
		protected function add_admin_menu(array $args) {
			...
			if ('themes.php' === $this->parent_slug) {
				...
				$this->page_hook = call_user_func("add_{$type}_page", $args['parent_slug'], $args['page_title'],
				                                  $args['menu_title'], $args['capability'], $args['menu_slug'],
				                                  $args['function']);
			}
		}
		...
		protected function do_plugin_install() {
			...
			if ((isset($_GET['tgmpa-install']) && 'install-plugin' === $_GET['tgmpa-install']) || (isset($_GET['tgmpa-update']) && 'update-plugin' === $_GET['tgmpa-update'])) {
				...
				$api           = ('repo' === $this->plugins[ $slug ]['source_type']) ? $this->get_plugins_api($slug)
					: NULL;
		...
		public function notices() {
			...
			if ($this->is_tgmpa_page() || get_user_meta(get_current_user_id(), 'tgmpa_dismissed_notice_' . $this->id,
			                                            TRUE)
			) {
				return;
			}
				...
				switch ($type) {
					...
					case 'install':
						$text = _n('To Install <span class="count">(%s)</span>',
						           'To Install <span class="count">(%s)</span>', $count, 'tgmpa');
						break;
					case 'update':
					...
					case 'activate':
						$text = _n('To Activate <span class="count">(%s)</span>',
						           'To Activate <span class="count">(%s)</span>', $count, 'tgmpa');
						break;
					default:
						$text = '';
						break;
				...
				if (is_wp_error($activate)) {
					...
					printf( // WPCS: xss ok.
						...
						esc_html(_n('The following plugin was activated successfully:',
						            'The following plugins were activated successfully:', $count, 'tgmpa')),
						$imploded
					);
							...
							if ($this->tgmpa->is_automatic) {
								...
								$this->upgrader->strings['skin_update_successful']
									= __('%1$s installed and activated successfully.',
									     'tgmpa') . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__('Show Details',
									                                                                                        'tgmpa') . '</span><span class="hidden">' . esc_html__('Hide Details',
									                                                                                                                                               'tgmpa') . '</span>.</a>';
								...
								$this->upgrader->strings['skin_upgrade_start']
									= __('The installation process is starting. This process may take a while on some hosts, so please be patient.',
									     'tgmpa');
								$this->upgrader->strings['skin_update_successful']
									= esc_html__('%1$s installed successfully.',
									             'tgmpa') . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__('Show Details',
									                                                                                                'tgmpa') . '</span><span class="hidden">' . esc_html__('Hide Details',
									                                                                                                                                                       'tgmpa') . '</span>.</a>';
								$this->upgrader->strings['skin_upgrade_end']
