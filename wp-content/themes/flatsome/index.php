...
if ($flatsome_opt['blog_header']) {
	echo do_shortcode($flatsome_opt['blog_header']);
}
?>
<div class="page-wrapper <?php echo $flatsome_opt['blog_style']; ?>  page-<?php if ($flatsome_opt['blog_layout']) {
	echo $flatsome_opt['blog_layout'];
} else {
	echo 'right-sidebar';
} ?>">
	<div class="row">
		...
		<?php if ($flatsome_opt['blog_layout'] == 'left-sidebar') {
			...
			if ($flatsome_opt['blog_layout'] == 'right-sidebar') {
				echo '<div id="content" class="large-9 left columns" role="main">';
			} else {
				if ($flatsome_opt['blog_layout'] == 'no-sidebar' && $flatsome_opt['blog_style'] == 'blog-pinterest') {
					echo '<div id="content" class="large-12 columns blog-pinterest-container" role="main">';
				} else {
					if ($flatsome_opt['blog_layout'] == 'no-sidebar') {
