...
if (!class_exists('BBP_Admin')) :
	...
	class BBP_Admin {
		...
		private function setup_actions() {
			...
			/** General Actions ***************************************************/
			add_action('bbp_admin_menu', [$this, 'admin_menus']); // Add menu item to settings menu
			add_action('bbp_admin_head', [$this, 'admin_head']); // Add some general styling to the admin area
			add_action('bbp_admin_notices', [$this, 'activation_notice']); // Add notice if not using a bbPress theme
			add_action('bbp_register_admin_style', [$this, 'register_admin_style']); // Add green admin style
			add_action('bbp_register_admin_settings', [$this, 'register_admin_settings']); // Add settings
			add_action('bbp_activation', [$this, 'new_install']); // Add menu item to settings menu
			add_action('admin_enqueue_scripts', [$this, 'enqueue_styles']); // Add enqueued CSS
			add_action('admin_enqueue_scripts', [$this, 'enqueue_scripts']); // Add enqueued JS
			add_action('wp_dashboard_setup',
			           [$this, 'dashboard_widget_right_now']); // Forums 'Right now' Dashboard widget
			add_action('admin_bar_menu', [$this, 'admin_bar_about_link'],
			           15); // Add a link to bbPress about page to the admin bar
			/** Ajax **************************************************************/
			// No _nopriv_ equivalent - users must be logged in
			add_action('wp_ajax_bbp_suggest_topic', [$this, 'suggest_topic']);
			add_action('wp_ajax_bbp_suggest_user', [$this, 'suggest_user']);
			/** Filters ***********************************************************/
			// Modify bbPress's admin links
			add_filter('plugin_action_links', [$this, 'modify_plugin_action_links'], 10, 2);
			// Map settings capabilities
			add_filter('bbp_map_meta_caps', [$this, 'map_settings_meta_caps'], 10, 4);
			// Hide the theme compat package selection
			add_filter('bbp_admin_get_settings_sections', [$this, 'hide_theme_compat_packages']);
			// Allow keymasters to save forums settings
			add_filter('option_page_capability_bbpress', [$this, 'option_page_capability_bbpress']);
			/** Network Admin *****************************************************/
			// Add menu item to settings menu
			add_action('network_admin_menu', [$this, 'network_admin_menus']);
			/** Dependencies ******************************************************/
			// Allow plugins to modify these actions
			do_action_ref_array('bbp_admin_loaded', [&$this]);
		}
			...
			foreach ((array)$sections as $section_id => $section) {
				...
				foreach ((array)$fields as $field_id => $field) {
					...
					if (!empty($field['callback']) && !empty($field['title'])) {
						add_settings_field($field_id, $field['title'], $field['callback'], $page, $section_id,
						                   $field['args']);
					}
		 ...
		 */
		public static function map_settings_meta_caps($caps = [], $cap = '', $user_id = 0, $args = []) {
			...
			switch ($cap) {
				...
				case 'bbp_settings_buddypress' :
					if ((is_plugin_active('buddypress/bp-loader.php') && defined('BP_VERSION') && bp_is_root_blog()) && is_super_admin()) {
						$caps = [bbpress()->admin->minimum_capability];
					} else {
						$caps = ['do_not_allow'];
					}
					break;
		...
		public function register_importers() {
			...
			// Load our importers
			$importers = apply_filters('bbp_importers', ['bbpress']);
		...
		public static function modify_plugin_action_links($links, $file) {
			...
			if (current_user_can('bbp_settings_page')) {
				$new_links['settings'] = '<a href="' . esc_url(add_query_arg(['page' => 'bbpress'],
				                                                             admin_url('options-general.php'))) . '">' . esc_html__('Settings',
				                                                                                                                    'bbpress') . '</a>';
			}
			// About page link
			if (current_user_can('bbp_about_page')) {
				$new_links['about'] = '<a href="' . esc_url(add_query_arg(['page' => 'bbp-about'],
				                                                          admin_url('index.php'))) . '">' . esc_html__('About',
				                                                                                                       'bbpress') . '</a>';
			}
		...
		public static function dashboard_widget_right_now() {
			wp_add_dashboard_widget('bbp-dashboard-right-now', __('Right Now in Forums', 'bbpress'),
		...
		public function admin_bar_about_link($wp_admin_bar) {
			if (is_user_logged_in()) {
				$wp_admin_bar->add_menu([
					                        ...
					                        'href'   => add_query_arg(['page' => 'bbp-about'], admin_url('index.php'))
				                        ]);
			}
		}
			...
			if ('post' === get_current_screen()->base) {
				switch (get_current_screen()->post_type) {
					...
					case bbp_get_topic_post_type() :
						...
						wp_enqueue_script('bbp-admin-common-js', $this->js_url . 'common.js', ['jquery'], $version);
						// Topics admin
						if (bbp_get_topic_post_type() === get_current_screen()->post_type) {
							wp_enqueue_script('bbp-admin-topics-js', $this->js_url . 'topics.js', ['jquery'], $version);
							// Replies admin
						} elseif (bbp_get_reply_post_type() === get_current_screen()->post_type) {
							wp_enqueue_script('bbp-admin-replies-js', $this->js_url . 'replies.js', ['jquery'],
							                  $version);
						}
						break;
				}
		...
		public function enqueue_styles() {
			wp_enqueue_style('bbp-admin-css', $this->css_url . 'admin.css', ['dashicons'], bbp_get_version());
		}
		...
		public function register_admin_style() {
			...
			wp_admin_css_color(
				...
				['#4f6d59', '#33834e', '#5FB37C', '#81c498'],
				['base' => '#f1f3f2', 'focus' => '#fff', 'current' => '#fff']
			);
			// Evergreen
			wp_admin_css_color(
				'bbp-evergreen',
		 ...
		 * @return array
		 */
		public function hide_theme_compat_packages($sections = []) {
		...
		public function suggest_topic() {
			...
			if (!empty($topics)) {
				foreach ((array)$topics as $post) {
					printf(esc_html__('%s - %s', 'bbpress'), bbp_get_topic_id($post->ID),
		...
		public function suggest_user() {
			...
			$users_query = new WP_User_Query([
				                                 ...
				                                 'search_columns' => ['ID', 'user_nicename', 'user_email'],
				                                 'orderby'        => 'ID'
			                                 ]);
			...
			if (!empty($users_query->results)) {
				foreach ((array)$users_query->results as $user) {
					printf(esc_html__('%s - %s', 'bbpress'), bbp_get_user_id($user->ID),
					       bbp_get_user_nicename($user->ID, ['force' => $user->user_nicename]) . "\n");
				}
			}
			die();
		}
		...
		public function about_screen() {
			...
			<div class="wrap about-wrap">
				...
				<div
					class="about-text"><?php printf(esc_html__('Thank you for updating! bbPress %s is bundled up and ready to weather the storm of users in your community!',
				                                               'bbpress'), $display_version); ?></div>
				<div class="bbp-badge"><?php printf(esc_html__('Version %s', 'bbpress'), $display_version); ?></div>

				<h2 class="nav-tab-wrapper">
					<a class="nav-tab nav-tab-active"
					   href="<?php echo esc_url(admin_url(add_query_arg(['page' => 'bbp-about'], 'index.php'))); ?>">
						<?php esc_html_e('What&#8217;s New', 'bbpress'); ?>
					</a><a class="nav-tab" href="<?php echo esc_url(admin_url(add_query_arg(['page' => 'bbp-credits'],
					                                                                        'index.php'))); ?>">
				...
				<div class="changelog">
					...
					<div class="feature-section col two-col">
						<div class="last-feature">
							...
							<p><?php esc_html_e('Now your users can subscribe to new topics in specific forums.',
							                    'bbpress'); ?></p>
						</div>

						<div>
							<h4><?php esc_html_e('Manage Subscriptions', 'bbpress'); ?></h4>
							<p><?php esc_html_e('Your users can manage all of their subscriptions in one convenient location.',
							                    'bbpress'); ?></p>
						</div>
					</div>
				</div>
				...
				<div class="changelog">
					...
					<div class="feature-section col three-col">
						<div>
							...
							<p><?php esc_html_e('Better handling of styles and scripts in the template stack.',
							                    'bbpress'); ?></p>
						</div>

						<div>
							<h4><?php esc_html_e('Polyglot support', 'bbpress'); ?></h4>
							<p><?php esc_html_e('bbPress fully supports automatic translation updates.',
							                    'bbpress'); ?></p>
						</div>

						<div class="last-feature">
							<h4><?php esc_html_e('User capabilities', 'bbpress'); ?></h4>
							<p><?php esc_html_e('Roles and capabilities have been swept through, cleaned up, and simplified.',
							                    'bbpress'); ?></p>
						</div>
					</div>
				</div>
				...
				<div class="return-to-dashboard">
					<a href="<?php echo esc_url(admin_url(add_query_arg(['page' => 'bbpress'],
					                                                    'options-general.php'))); ?>"><?php esc_html_e('Go to Forum Settings',
					                                                                                                   'bbpress'); ?></a>
				</div>
		...
		public function credits_screen() {
			...
			<div class="wrap about-wrap">
				<h1><?php printf(esc_html__('Welcome to bbPress %s', 'bbpress'), $display_version); ?></h1>
				<div
					class="about-text"><?php printf(esc_html__('Thank you for updating! bbPress %s is waxed, polished, and ready for you to take it for a lap or two around the block!',
				                                               'bbpress'), $display_version); ?></div>
				<div class="bbp-badge"><?php printf(esc_html__('Version %s', 'bbpress'), $display_version); ?></div>

				<h2 class="nav-tab-wrapper">
					<a href="<?php echo esc_url(admin_url(add_query_arg(['page' => 'bbp-about'], 'index.php'))); ?>"
					   class="nav-tab">
						<?php esc_html_e('What&#8217;s New', 'bbpress'); ?>
					</a><a
						href="<?php echo esc_url(admin_url(add_query_arg(['page' => 'bbp-credits'], 'index.php'))); ?>"
						class="nav-tab nav-tab-active">
						<?php esc_html_e('Credits', 'bbpress'); ?>
					</a>
				</h2>

				<p class="about-description"><?php esc_html_e('bbPress is created by a worldwide swarm of busy, busy bees.',
				                                              'bbpress'); ?></p>

				<h4 class="wp-people-group"><?php esc_html_e('Project Leaders', 'bbpress'); ?></h4>
				<ul class="wp-people-group " id="wp-people-group-project-leaders">
					<li class="wp-person" id="wp-person-matt">
						<a href="http://profiles.wordpress.org/matt"><img
								src="http://0.gravatar.com/avatar/767fc9c115a1b989744c755db47feb60?s=60"
								class="gravatar" alt="Matt Mullenweg"/></a>
					...
					<li class="wp-person" id="wp-person-johnjamesjacoby">
						<a href="http://profiles.wordpress.org/johnjamesjacoby"><img
								src="http://0.gravatar.com/avatar/81ec16063d89b162d55efe72165c105f?s=60"
								class="gravatar" alt="John James Jacoby"/></a>
					...
					<li class="wp-person" id="wp-person-jmdodd">
						<a href="http://profiles.wordpress.org/jmdodd"><img
								src="http://0.gravatar.com/avatar/6a7c997edea340616bcc6d0fe03f65dd?s=60"
								class="gravatar" alt="Jennifer M. Dodd"/></a>
					...
					<li class="wp-person" id="wp-person-netweb">
						<a href="http://profiles.wordpress.org/netweb"><img
								src="http://0.gravatar.com/avatar/97e1620b501da675315ba7cfb740e80f?s=60"
								class="gravatar" alt="Stephen Edgar"/></a>
				...
				<ul class="wp-people-group " id="wp-people-group-contributing-developers">
					<li class="wp-person" id="wp-person-jaredatch">
						<a href="http://profiles.wordpress.org/jaredatch"><img
								src="http://0.gravatar.com/avatar/e341eca9e1a85dcae7127044301b4363?s=60"
								class="gravatar" alt="Jared Atchison"/></a>
					...
					<li class="wp-person" id="wp-person-gautamgupta">
						<a href="http://profiles.wordpress.org/gautamgupta"><img
								src="http://0.gravatar.com/avatar/b0810422cbe6e4eead4def5ae7a90b34?s=60"
								class="gravatar" alt="Gautam Gupta"/></a>
		...
		public static function update_screen() {
			...
			switch ($action) {
				...
				default : ?>
					...
					<p><?php esc_html_e('You can update your forum through this page. Hit the link below to update.',
					                    'bbpress'); ?></p>
					<p><a class="button"
					      href="index.php?page=bbp-update&amp;action=bbp-update"><?php esc_html_e('Update Forum',
					                                                                              'bbpress'); ?></a></p>
		...
		public static function network_update_screen() {
			...
			case 'bbpress-update' :
				...
				$blogs
					= $wpdb->get_results("SELECT * FROM {$wpdb->blogs} WHERE site_id = '{$wpdb->siteid}' AND spam = '0' AND deleted = '0' AND archived = '0' ORDER BY registered DESC LIMIT {$n}, 5",
					                     ARRAY_A);
			...
			if (empty($blogs)) : ?>
				...
				<p><?php esc_html_e('All done!', 'bbpress'); ?></p>
				<a class="button" href="update-core.php?page=bbpress-update"><?php esc_html_e('Go Back',
				                                                                              'bbpress'); ?></a>
			...
			// Still have sites to loop through
				...
				<ul>
					...
					<?php foreach ((array)$blogs as $details) :
						...
						if (is_wp_error($response)) {
							wp_die(sprintf(__('Warning! Problem updating %1$s. Your server may not be able to connect to sites running on it. Error message: <em>%2$s</em>',
							                  'bbpress'), $siteurl, $response->get_error_message()));
						}
				...
				<p>
					<?php esc_html_e('If your browser doesn&#8217;t start loading the next page automatically, click this link:',
					                 'bbpress'); ?>
					<a class="button"
					   href="update-core.php?page=bbpress-update&amp;action=bbpress-update&amp;n=<?php echo($n + 5); ?>"><?php esc_html_e('Next Forums',
					                                                                                                                      'bbpress'); ?></a>
				</p>
			...
			default : ?>
				...
				<p><?php esc_html_e('You can update all the forums on your network through this page. It works by calling the update script of each site automatically. Hit the link below to update.',
				                    'bbpress'); ?></p>
				<p><a class="button"
				      href="update-core.php?page=bbpress-update&amp;action=bbpress-update"><?php esc_html_e('Update Forums',
				                                                                                            'bbpress'); ?></a>
				</p>
