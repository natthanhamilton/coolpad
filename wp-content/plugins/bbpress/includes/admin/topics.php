...
if (!class_exists('BBP_Topics_Admin')) :
	...
	class BBP_Topics_Admin {
		...
		private function setup_actions() {
			...
			add_action('bbp_admin_head', [$this, 'admin_head']);
			// Messages
			...
			add_action('add_meta_boxes', [$this, 'attributes_metabox']);
			add_action('save_post', [$this, 'attributes_metabox_save']);
			// Check if there are any bbp_toggle_topic_* requests on admin_init, also have a message displayed
			add_action('load-edit.php', [$this, 'toggle_topic']);
			add_action('admin_notices', [$this, 'toggle_topic_notice']);
			// Anonymous metabox actions
			add_action('add_meta_boxes', [$this, 'author_metabox']);
			// Add ability to filter topics and replies per forum
			add_filter('restrict_manage_posts', [$this, 'filter_dropdown']);
			add_filter('bbp_request', [$this, 'filter_post_rows']);
			// Contextual Help
			add_action('load-edit.php', [$this, 'edit_help']);
			add_action('load-post.php', [$this, 'new_help']);
			add_action('load-post-new.php', [$this, 'new_help']);
		}
		...
		public function edit_help() {
			...
			get_current_screen()->add_help_tab([
				                                   ...
				                                   'content' =>
					                                   '<p>' . __('This screen displays the individual topics on your site. You can customize the display of this screen to suit your workflow.',
					                                              'bbpress') . '</p>'
			                                   ]);
			// Screen Content
			get_current_screen()->add_help_tab([
				                                   ...
				                                   'content' =>
					                                   '<p>' . __('You can customize the display of this screen&#8217;s contents in a number of ways:',
					                                              'bbpress') . '</p>' .
					                                   '<ul>' .
					                                   '<li>' . __('You can hide/display columns based on your needs and decide how many topics to list per screen using the Screen Options tab.',
					                                               'bbpress') . '</li>' .
					                                   '<li>' . __('You can filter the list of topics by topic status using the text links in the upper left to show All, Published, or Trashed topics. The default view is to show all topics.',
					                                               'bbpress') . '</li>' .
					                                   '<li>' . __('You can refine the list to show only topics from a specific month by using the dropdown menus above the topics list. Click the Filter button after making your selection. You also can refine the list by clicking on the topic creator in the topics list.',
					                                               'bbpress') . '</li>' .
					                                   '</ul>'
			                                   ]);
			// Available Actions
			get_current_screen()->add_help_tab([
				                                   ...
				                                   'content' =>
					                                   '<p>' . __('Hovering over a row in the topics list will display action links that allow you to manage your topic. You can perform the following actions:',
					                                              'bbpress') . '</p>' .
					                                   '<ul>' .
					                                   '<li>' . __('<strong>Edit</strong> takes you to the editing screen for that topic. You can also reach that screen by clicking on the topic title.',
					                                               'bbpress') . '</li>' .
					                                   '<li>' . __('<strong>Trash</strong> removes your topic from this list and places it in the trash, from which you can permanently delete it.',
					                                               'bbpress') . '</li>' .
					                                   '<li>' . __('<strong>Spam</strong> removes your topic from this list and places it in the spam queue, from which you can permanently delete it.',
					                                               'bbpress') . '</li>' .
					                                   '<li>' . __('<strong>Preview</strong> will show you what your draft topic will look like if you publish it. View will take you to your live site to view the topic. Which link is available depends on your topic&#8217;s status.',
					                                               'bbpress') . '</li>' .
					                                   '<li>' . __('<strong>Close</strong> will mark the selected topic as &#8217;closed&#8217; and disable the option to post new replies to the topic.',
					                                               'bbpress') . '</li>' .
					                                   '<li>' . __('<strong>Stick</strong> will keep the selected topic &#8217;pinned&#8217; to the top the parent forum topic list.',
					                                               'bbpress') . '</li>' .
					                                   '<li>' . __('<strong>Stick <em>(to front)</em></strong> will keep the selected topic &#8217;pinned&#8217; to the top of ALL forums and be visable in any forums topics list.',
					                                               'bbpress') . '</li>' .
					                                   '</ul>'
			                                   ]);
			// Bulk Actions
			get_current_screen()->add_help_tab([
				                                   ...
				                                   'content' =>
					                                   '<p>' . __('You can also edit or move multiple topics to the trash at once. Select the topics you want to act on using the checkboxes, then select the action you want to take from the Bulk Actions menu and click Apply.',
					                                              'bbpress') . '</p>' .
					                                   '<p>' . __('When using Bulk Edit, you can change the metadata (categories, author, etc.) for all selected topics at once. To remove a topic from the grouping, just click the x next to its name in the Bulk Edit area that appears.',
					                                              'bbpress') . '</p>'
			                                   ]);
			// Help Sidebar
			get_current_screen()->set_help_sidebar(
				'<p><strong>' . __('For more information:', 'bbpress') . '</strong></p>' .
				'<p>' . __('<a href="http://codex.bbpress.org" target="_blank">bbPress Documentation</a>',
				           'bbpress') . '</p>' .
				'<p>' . __('<a href="http://bbpress.org/forums/" target="_blank">bbPress Support Forums</a>',
				           'bbpress') . '</p>'
			);
		}
		...
		public function new_help() {
			if ($this->bail()) return;
			$customize_display
				= '<p>' . __('The title field and the big topic editing Area are fixed in place, but you can reposition all the other boxes using drag and drop, and can minimize or expand them by clicking the title bar of each box. Use the Screen Options tab to unhide more boxes (Excerpt, Send Trackbacks, Custom Fields, Discussion, Slug, Author) or to choose a 1- or 2-column layout for this screen.',
				             'bbpress') . '</p>';
			get_current_screen()->add_help_tab([
			...
			get_current_screen()->add_help_tab([
				                                   ...
				                                   'content' =>
					                                   '<p>' . __('<strong>Title</strong> - Enter a title for your topic. After you enter a title, you&#8217;ll see the permalink below, which you can edit.',
					                                              'bbpress') . '</p>' .
			...
			if (current_theme_supports('topic-thumbnails') && post_type_supports('topic', 'thumbnail')) {
				$publish_box .= '<p>' . __('<strong>Featured Image</strong> - This allows you to associate an image with your topic without inserting it. This is usually useful only if your theme makes use of the featured image as a topic thumbnail on the home page, a custom header, etc.',
				                           'bbpress') . '</p>';
			}
			get_current_screen()->add_help_tab([
				                                   'id'      => 'topic-attributes',
				                                   ...
				                                   'content' =>
					                                   '<p>' . __('Select the attributes that your topic should have:',
					                                              'bbpress') . '</p>' .
					                                   '<ul>' .
					                                   '<li>' . __('<strong>Forum</strong> dropdown determines the parent forum that the topic belongs to. Select the forum or category from the dropdown, or leave the default (No Forum) to post the topic without an assigned forum.',
					                                               'bbpress') . '</li>' .
					                                   '<li>' . __('<strong>Topic Type</strong> dropdown indicates the sticky status of the topic. Selecting the super sticky option would stick the topic to the front of your forums, i.e. the topic index, sticky option would stick the topic to its respective forum. Selecting normal would not stick the topic anywhere.',
					                                               'bbpress') . '</li>' .
					                                   '</ul>'
			                                   ]);
			get_current_screen()->add_help_tab([
		...
		public function attributes_metabox_save($topic_id) {
			...
			// Nonce check
			if (empty($_POST['bbp_topic_metabox']) || !wp_verify_nonce($_POST['bbp_topic_metabox'],
			                                                           'bbp_topic_metabox_save')
			) {
				return $topic_id;
			}
		...
		public function toggle_topic() {
			...
			if (bbp_is_get_request() && !empty($_GET['action']) && in_array($_GET['action'],
			                                                                ['bbp_toggle_topic_close', 'bbp_toggle_topic_stick', 'bbp_toggle_topic_spam']) && !empty($_GET['topic_id'])
			) {
				...
				switch ($action) {
					...
					case 'bbp_toggle_topic_stick' :
						...
						$is_super  = FALSE === $is_sticky && !empty($_GET['super']) && ("1" === $_GET['super']) ? TRUE
							: FALSE;
						$message   = TRUE === $is_sticky ? 'unsticked' : 'sticked';
						$message   = TRUE === $is_super ? 'super_sticked' : $message;
						$success   = TRUE === $is_sticky ? bbp_unstick_topic($topic_id)
				...
				$message = ['bbp_topic_toggle_notice' => $message, 'topic_id' => $topic->ID];
				...
				$redirect = add_query_arg($message, remove_query_arg(['action', 'topic_id']));
		...
		public function toggle_topic_notice() {
			if ($this->bail()) return;
			// Only proceed if GET is a topic toggle action
			if (bbp_is_get_request() && !empty($_GET['bbp_topic_toggle_notice']) && in_array($_GET['bbp_topic_toggle_notice'],
			                                                                                 ['opened', 'closed', 'super_sticked', 'sticked', 'unsticked', 'spammed', 'unspammed']) && !empty($_GET['topic_id'])
			) {
				...
				switch ($notice) {
					case 'opened'    :
						$message = $is_failure === TRUE ? sprintf(__('There was a problem opening the topic "%1$s".',
						                                             'bbpress'), $topic_title)
							: sprintf(__('Topic "%1$s" successfully opened.', 'bbpress'), $topic_title);
						break;
					case 'closed'    :
						$message = $is_failure === TRUE ? sprintf(__('There was a problem closing the topic "%1$s".',
						                                             'bbpress'), $topic_title)
							: sprintf(__('Topic "%1$s" successfully closed.', 'bbpress'), $topic_title);
						break;
					case 'super_sticked' :
						$message = $is_failure === TRUE
							? sprintf(__('There was a problem sticking the topic "%1$s" to front.', 'bbpress'),
							          $topic_title)
							: sprintf(__('Topic "%1$s" successfully sticked to front.', 'bbpress'), $topic_title);
						break;
					case 'sticked'   :
						$message = $is_failure === TRUE ? sprintf(__('There was a problem sticking the topic "%1$s".',
						                                             'bbpress'), $topic_title)
							: sprintf(__('Topic "%1$s" successfully sticked.', 'bbpress'), $topic_title);
						break;
					case 'unsticked' :
						$message = $is_failure === TRUE ? sprintf(__('There was a problem unsticking the topic "%1$s".',
						                                             'bbpress'), $topic_title)
							: sprintf(__('Topic "%1$s" successfully unsticked.', 'bbpress'), $topic_title);
						break;
					case 'spammed'   :
						$message = $is_failure === TRUE
							? sprintf(__('There was a problem marking the topic "%1$s" as spam.', 'bbpress'),
							          $topic_title)
							: sprintf(__('Topic "%1$s" successfully marked as spam.', 'bbpress'), $topic_title);
						break;
					case 'unspammed' :
						$message = $is_failure === TRUE
							? sprintf(__('There was a problem unmarking the topic "%1$s" as spam.', 'bbpress'),
							          $topic_title)
		...
		public function row_actions($actions, $topic) {
			...
			if (empty($actions['view']) && (bbp_get_trash_status_id() === $topic->post_status) && current_user_can('view_trash')) {
				$actions['view']
					= '<a href="' . esc_url(bbp_get_topic_permalink($topic->ID)) . '" title="' . esc_attr(sprintf(__('View &#8220;%s&#8221;',
					                                                                                                 'bbpress'),
					                                                                                              bbp_get_topic_title($topic->ID))) . '" rel="permalink">' . esc_html__('View',
					                                                                                                                                                                    'bbpress') . '</a>';
			}
			...
			if (current_user_can('moderate', $topic->ID)) {
				...
				if (in_array($topic->post_status, [bbp_get_public_status_id(), bbp_get_closed_status_id()])) {
					$close_uri
						= wp_nonce_url(add_query_arg(['topic_id' => $topic->ID, 'action' => 'bbp_toggle_topic_close'],
						                             remove_query_arg(['bbp_topic_toggle_notice', 'topic_id', 'failed', 'super'])),
						               'close-topic_' . $topic->ID);
					if (bbp_is_topic_open($topic->ID)) {
						$actions['closed']
							= '<a href="' . esc_url($close_uri) . '" title="' . esc_attr__('Close this topic',
							                                                               'bbpress') . '">' . _x('Close',
							                                                                                      'Close a Topic',
							                                                                                      'bbpress') . '</a>';
					} else {
						$actions['closed']
							= '<a href="' . esc_url($close_uri) . '" title="' . esc_attr__('Open this topic',
							                                                               'bbpress') . '">' . _x('Open',
							                                                                                      'Open a Topic',
							                                                                                      'bbpress') . '</a>';
					}
				...
				if (!bbp_is_topic_spam($topic->ID) && !bbp_is_topic_trash($topic->ID)) {
					// Sticky
					$stick_uri
						= wp_nonce_url(add_query_arg(['topic_id' => $topic->ID, 'action' => 'bbp_toggle_topic_stick'],
						                             remove_query_arg(['bbp_topic_toggle_notice', 'topic_id', 'failed', 'super'])),
						               'stick-topic_' . $topic->ID);
					if (bbp_is_topic_sticky($topic->ID)) {
						$actions['stick']
							= '<a href="' . esc_url($stick_uri) . '" title="' . esc_attr__('Unstick this topic',
							                                                               'bbpress') . '">' . esc_html__('Unstick',
							                                                                                              'bbpress') . '</a>';
					} else {
						$super_uri
							= wp_nonce_url(add_query_arg(['topic_id' => $topic->ID, 'action' => 'bbp_toggle_topic_stick', 'super' => '1'],
							                             remove_query_arg(['bbp_topic_toggle_notice', 'topic_id', 'failed', 'super'])),
							               'stick-topic_' . $topic->ID);
						$actions['stick']
							= '<a href="' . esc_url($stick_uri) . '" title="' . esc_attr__('Stick this topic to its forum',
							                                                               'bbpress') . '">' . esc_html__('Stick',
							                                                                                              'bbpress') . '</a> <a href="' . esc_url($super_uri) . '" title="' . esc_attr__('Stick this topic to front',
							                                                                                                                                                                             'bbpress') . '">' . esc_html__('(to front)',
							                                                                                                                                                                                                            'bbpress') . '</a>';
					}
				}
				// Spam
				$spam_uri = wp_nonce_url(add_query_arg(['topic_id' => $topic->ID, 'action' => 'bbp_toggle_topic_spam'],
				                                       remove_query_arg(['bbp_topic_toggle_notice', 'topic_id', 'failed', 'super'])),
				                         'spam-topic_' . $topic->ID);
				if (bbp_is_topic_spam($topic->ID)) {
					$actions['spam']
						= '<a href="' . esc_url($spam_uri) . '" title="' . esc_attr__('Mark the topic as not spam',
						                                                              'bbpress') . '">' . esc_html__('Not spam',
						                                                                                             'bbpress') . '</a>';
				} else {
					$actions['spam']
						= '<a href="' . esc_url($spam_uri) . '" title="' . esc_attr__('Mark this topic as spam',
						                                                              'bbpress') . '">' . esc_html__('Spam',
						                                                                                             'bbpress') . '</a>';
				}
			}
			...
			if (current_user_can('delete_topic', $topic->ID)) {
				if (bbp_get_trash_status_id() === $topic->post_status) {
					...
					$actions['untrash'] = "<a title='" . esc_attr__('Restore this item from the Trash',
					                                                'bbpress') . "' href='" . esc_url(wp_nonce_url(add_query_arg(['_wp_http_referer' => add_query_arg(['post_type' => bbp_get_topic_post_type()],
					                                                                                                                                                  admin_url('edit.php'))],
					                                                                                                             admin_url(sprintf($post_type_object->_edit_link . '&amp;action=untrash',
					                                                                                                                               $topic->ID))),
					                                                                                               'untrash-' . $topic->post_type . '_' . $topic->ID)) . "'>" . esc_html__('Restore',
					                                                                                                                                                                       'bbpress') . "</a>";
				} elseif (EMPTY_TRASH_DAYS) {
					$actions['trash'] = "<a class='submitdelete' title='" . esc_attr__('Move this item to the Trash',
					                                                                   'bbpress') . "' href='" . esc_url(add_query_arg(['_wp_http_referer' => add_query_arg(['post_type' => bbp_get_topic_post_type()],
					                                                                                                                                                        admin_url('edit.php'))],
					                                                                                                                   get_delete_post_link($topic->ID))) . "'>" . esc_html__('Trash',
					                                                                                                                                                                          'bbpress') . "</a>";
				}
				if (bbp_get_trash_status_id() === $topic->post_status || !EMPTY_TRASH_DAYS) {
					$actions['delete'] = "<a class='submitdelete' title='" . esc_attr__('Delete this item permanently',
					                                                                    'bbpress') . "' href='" . esc_url(add_query_arg(['_wp_http_referer' => add_query_arg(['post_type' => bbp_get_topic_post_type()],
					                                                                                                                                                         admin_url('edit.php'))],
					                                                                                                                    get_delete_post_link($topic->ID,
					                                                                                                                                         '',
					                                                                                                                                         TRUE))) . "'>" . esc_html__('Delete Permanently',
					                                                                                                                                                                     'bbpress') . "</a>";
		...
		public function updated_messages($messages) {
			...
			$messages[ $this->post_type ] = [
				...
				5  => isset($_GET['revision'])
					? sprintf(__('Topic restored to revision from %s', 'bbpress'),
				...
				// Topic submitted
				8  => sprintf(__('Topic submitted. <a target="_blank" href="%s">Preview topic</a>', 'bbpress'),
				              esc_url(add_query_arg('preview', 'true', $topic_url))),
				// Topic scheduled
				9  => sprintf(__('Topic scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview topic</a>',
				                 'bbpress'),
					// translators: Publish box date format, see http://php.net/date
					          ...
					          $topic_url),
				// Topic draft updated
				10 => sprintf(__('Topic draft updated. <a target="_blank" href="%s">Preview topic</a>', 'bbpress'),
				              esc_url(add_query_arg('preview', 'true', $topic_url))),
			];
