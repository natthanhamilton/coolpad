...
function bbp_admin_repair() {
	...
	<div class="wrap">
		...
		<p><?php esc_html_e('bbPress keeps track of relationships between forums, topics, replies, and topic tags, and users. Occasionally these relationships become out of sync, most often after an import or migration. Use the tools below to manually recalculate these relationships.',
		                    'bbpress'); ?></p>
		<p class="description"><?php esc_html_e('Some of these tools create substantial database overhead. Avoid running more than 1 repair job at a time.',
		                                        'bbpress'); ?></p>
		...
		<form class="settings" method="post" action="">
			<table class="form-table">
				...
				<tr valign="top">
					...
					<td>
						<fieldset>
							<legend class="screen-reader-text"><span><?php esc_html_e('Repair', 'bbpress') ?></span>
							</legend>

							<?php foreach (bbp_admin_repair_list() as $item) : ?>

								<label><input type="checkbox" class="checkbox"
								              name="<?php echo esc_attr($item[0]) . '" id="' . esc_attr(str_replace('_',
								                                                                                    '-',
								                                                                                    $item[0])); ?>"
								              value="1"/> <?php echo esc_html($item[1]); ?></label><br/>
			...
			<fieldset class="submit">
				<input class="button-primary" type="submit" name="submit"
				       value="<?php esc_attr_e('Repair Items', 'bbpress'); ?>"/>
...
function bbp_admin_repair_list() {
	$repair_list = [
		0  => ['bbp-sync-topic-meta', __('Recalculate the parent topic for each post',
		                                 'bbpress'), 'bbp_admin_repair_topic_meta'],
		5  => ['bbp-sync-forum-meta', __('Recalculate the parent forum for each post',
		                                 'bbpress'), 'bbp_admin_repair_forum_meta'],
		10 => ['bbp-sync-forum-visibility', __('Recalculate private and hidden forums',
		                                       'bbpress'), 'bbp_admin_repair_forum_visibility'],
		15 => ['bbp-sync-all-topics-forums', __('Recalculate last activity in each topic and forum',
		                                        'bbpress'), 'bbp_admin_repair_freshness'],
		20 => ['bbp-sync-all-topics-sticky', __('Recalculate the sticky relationship of each topic',
		                                        'bbpress'), 'bbp_admin_repair_sticky'],
		25 => ['bbp-sync-all-reply-positions', __('Recalculate the position of each reply',
		                                          'bbpress'), 'bbp_admin_repair_reply_menu_order'],
		30 => ['bbp-group-forums', __('Repair BuddyPress Group Forum relationships',
		                              'bbpress'), 'bbp_admin_repair_group_forum_relationship'],
		35 => ['bbp-forum-topics', __('Count topics in each forum', 'bbpress'), 'bbp_admin_repair_forum_topic_count'],
		40 => ['bbp-forum-replies', __('Count replies in each forum', 'bbpress'), 'bbp_admin_repair_forum_reply_count'],
		45 => ['bbp-topic-replies', __('Count replies in each topic', 'bbpress'), 'bbp_admin_repair_topic_reply_count'],
		50 => ['bbp-topic-voices', __('Count voices in each topic', 'bbpress'), 'bbp_admin_repair_topic_voice_count'],
		55 => ['bbp-topic-hidden-replies', __('Count spammed & trashed replies in each topic',
		                                      'bbpress'), 'bbp_admin_repair_topic_hidden_reply_count'],
		60 => ['bbp-user-topics', __('Count topics for each user', 'bbpress'), 'bbp_admin_repair_user_topic_count'],
		65 => ['bbp-user-replies', __('Count replies for each user', 'bbpress'), 'bbp_admin_repair_user_reply_count'],
		70 => ['bbp-user-favorites', __('Remove trashed topics from user favorites',
		                                'bbpress'), 'bbp_admin_repair_user_favorites'],
		75 => ['bbp-user-topic-subscriptions', __('Remove trashed topics from user subscriptions',
		                                          'bbpress'), 'bbp_admin_repair_user_topic_subscriptions'],
		80 => ['bbp-user-forum-subscriptions', __('Remove trashed forums from user subscriptions',
		                                          'bbpress'), 'bbp_admin_repair_user_forum_subscriptions'],
		85 => ['bbp-user-role-map', __('Remap existing users to default forum roles',
		                               'bbpress'), 'bbp_admin_repair_user_roles']
	];
	ksort($repair_list);
...
function bbp_admin_repair_topic_reply_count() {
	...
	if (is_wp_error($wpdb->query($sql_delete))) {
		return [1, sprintf($statement, $result)];
	}
	...
	if (is_wp_error($wpdb->query($sql))) {
		return [2, sprintf($statement, $result)];
	}

	return [0, sprintf($statement, __('Complete!', 'bbpress'))];
}
...
function bbp_admin_repair_topic_hidden_reply_count() {
	...
	$sql
		= "INSERT INTO `{$wpdb->postmeta}` (`post_id`, `meta_key`, `meta_value`) (SELECT `post_parent`, '_bbp_reply_count_hidden', COUNT(`post_status`) as `meta_value` FROM `{$wpdb->posts}` WHERE `post_type` = '" . bbp_get_reply_post_type() . "' AND `post_status` IN ( '" . implode("','",
		                                                                                                                                                                                                                                                                                  [bbp_get_trash_status_id(), bbp_get_spam_status_id()]) . "') GROUP BY `post_parent`);";
	if (is_wp_error($wpdb->query($sql))) {
		return [2, sprintf($statement, $result)];
	}
...
function bbp_admin_repair_group_forum_relationship() {
	...
	if (is_wp_error($forum_ids) || empty($wpdb->last_result)) {
		return [2, sprintf($statement, __('Failed!', 'bbpress'))];
	}
	...
	foreach ($results as $group_forums) {
		...
		$updated
			= $wpdb->query("UPDATE `{$groups_meta_table}` SET `meta_value` = '{$group_forums->ID}' WHERE `meta_key` = 'forum_id' AND `meta_value` = '{$group_forums->meta_value}';");
		// Bump the count
		...
		$group_id
			= (int)$wpdb->get_var("SELECT `group_id` FROM `{$groups_meta_table}` WHERE `meta_key` = 'forum_id' AND `meta_value` = '{$group_forums->ID}';");
		if (!empty($group_id)) {
			// Update the group to forum meta connection in forums
			update_post_meta($group_forums->ID, '_bbp_group_ids', [$group_id]);
	...
	// Complete results
	$result = sprintf(__('Complete! %s groups updated; %s forums updated; %s forum statuses synced.', 'bbpress'),
	                  bbp_number_format($g_count), bbp_number_format($f_count), bbp_number_format($s_count));

	return [0, sprintf($statement, $result)];
}
...
function bbp_admin_repair_forum_topic_count() {
	...
	$forums = get_posts(['post_type' => bbp_get_forum_post_type(), 'numberposts' => -1]);
	if (!empty($forums)) {
...
function bbp_admin_repair_user_topic_count() {
	...
	$result      = __('Failed!', 'bbpress');
	$sql_select
	             = "SELECT `post_author`, COUNT(DISTINCT `ID`) as `_count` FROM `{$wpdb->posts}` WHERE `post_type` = '" . bbp_get_topic_post_type() . "' AND `post_status` = '" . bbp_get_public_status_id() . "' GROUP BY `post_author`;";
	...
	if (is_wp_error($wpdb->query($sql_delete))) {
		return [3, sprintf($statement, $result)];
	}
	foreach (array_chunk($insert_values, 10000) as $chunk) {
		...
		if (is_wp_error($wpdb->query($sql_insert))) {
			return [4, sprintf($statement, $result)];
		}
	}
...
function bbp_admin_repair_user_reply_count() {
	...
	$result      = __('Failed!', 'bbpress');
	$sql_select
	             = "SELECT `post_author`, COUNT(DISTINCT `ID`) as `_count` FROM `{$wpdb->posts}` WHERE `post_type` = '" . bbp_get_reply_post_type() . "' AND `post_status` = '" . bbp_get_public_status_id() . "' GROUP BY `post_author`;";
	$insert_rows = $wpdb->get_results($sql_select);
	if (is_wp_error($insert_rows)) {
		return [1, sprintf($statement, $result)];
	}
...
function bbp_admin_repair_user_favorites() {
	...
	$users
	           = $wpdb->get_results("SELECT `user_id`, `meta_value` AS `favorites` FROM `{$wpdb->usermeta}` WHERE `meta_key` = '{$key}';");
	if (is_wp_error($users)) {
		return [1, sprintf($statement, $result)];
	}
	$topics
		= $wpdb->get_col("SELECT `ID` FROM `{$wpdb->posts}` WHERE `post_type` = '" . bbp_get_topic_post_type() . "' AND `post_status` = '" . bbp_get_public_status_id() . "';");
	...
	foreach (array_chunk($values, 10000) as $chunk) {
		...
		$sql_insert = "INSERT INTO `$wpdb->usermeta` (`user_id`, `meta_key`, `meta_value`) VALUES $chunk;";
		if (is_wp_error($wpdb->query($sql_insert))) {
			return [5, sprintf($statement, $result)];
		}
	}
...
function bbp_admin_repair_user_topic_subscriptions() {
	...
	$users
	           = $wpdb->get_results("SELECT `user_id`, `meta_value` AS `subscriptions` FROM `{$wpdb->usermeta}` WHERE `meta_key` = '{$key}';");
	if (is_wp_error($users)) {
		return [1, sprintf($statement, $result)];
	}
	$topics
...
function bbp_admin_repair_user_forum_subscriptions() {
	...
	$forums
		= $wpdb->get_col("SELECT `ID` FROM `{$wpdb->posts}` WHERE `post_type` = '" . bbp_get_forum_post_type() . "' AND `post_status` = '" . bbp_get_public_status_id() . "';");
...
function bbp_admin_repair_user_roles() {
	...
	if (empty($role_map)) {
		return [1, sprintf($statement, __('Failed!', 'bbpress'))];
	}
...
function bbp_admin_repair_freshness() {
	...
	if (is_wp_error($wpdb->query("INSERT INTO `$wpdb->postmeta` (`post_id`, `meta_key`, `meta_value`)
			...
			WHERE `reply`.`post_status` IN ( '" . bbp_get_public_status_id() . "' ) AND `topic`.`post_type` = 'topic' AND `reply`.`post_type` = 'reply'
			GROUP BY `topic`.`ID` );"))) {
		return [6, sprintf($statement, $result)];
	}
	...
	if (is_wp_error($wpdb->query("INSERT INTO `$wpdb->postmeta` (`post_id`, `meta_key`, `meta_value`)
			...
			WHERE `reply`.`meta_id` IS NULL AND `topic`.`post_type` = 'topic' );"))) {
		return [7, sprintf($statement, $result)];
	}
	...
	if (is_wp_error($wpdb->query("INSERT INTO `$wpdb->postmeta` (`post_id`, `meta_key`, `meta_value`)
			...
			WHERE `reply`.`post_status` IN ( '" . bbp_get_public_status_id() . "' ) AND `topic`.`post_type` = 'topic' AND `reply`.`post_type` = 'reply'
			GROUP BY `topic`.`ID` );"))) {
		return [8, sprintf($statement, $result)];
	}
	...
	if (is_wp_error($wpdb->query("INSERT INTO `$wpdb->postmeta` (`post_id`, `meta_key`, `meta_value`)
			...
			WHERE `reply`.`meta_id` IS NULL AND `topic`.`post_type` = 'topic' );"))) {
		return [9, sprintf($statement, $result)];
	}
	// Forums need to know what their last active item is as well. Now it gets a bit more complex to do in the database.
	$forums
		= $wpdb->get_col("SELECT `ID` FROM `$wpdb->posts` WHERE `post_type` = 'forum' and `post_status` != 'auto-draft';");
	if (is_wp_error($forums)) {
		return [10, sprintf($statement, $result)];
	}
	// Loop through forums
...
function bbp_admin_reset() {
	...
	<div class="wrap">
		...
		<p><?php esc_html_e('Revert your forums back to a brand new installation. This process cannot be undone.',
		                    'bbpress'); ?></p>
		...
		<form class="settings" method="post" action="">
			<table class="form-table">
				...
				<tr valign="top">
					...
					<td>
						<fieldset>
							<legend class="screen-reader-text"><span><?php esc_html_e("Say it ain't so!",
							                                                          'bbpress'); ?></span></legend>
							<label><input type="checkbox" class="checkbox" name="bbpress-delete-imported-users"
							              id="bbpress-delete-imported-users"
							              value="1"/> <?php esc_html_e('This option will delete all previously imported users, and cannot be undone.',
							                                           'bbpress'); ?></label>
							<p class="description"><?php esc_html_e('Note: Resetting without this checked will delete the meta-data necessary to delete these users.',
							                                        'bbpress'); ?></p>
						</fieldset>
					</td>
				</tr>
				<tr valign="top">
					...
					<td>
						<fieldset>
							<legend class="screen-reader-text"><span><?php esc_html_e("Say it ain't so!",
							                                                          'bbpress'); ?></span></legend>
							<label><input type="checkbox" class="checkbox" name="bbpress-are-you-sure"
							              id="bbpress-are-you-sure"
							              value="1"/> <?php esc_html_e('This process cannot be undone.', 'bbpress'); ?>
							</label>
							<p class="description"><?php esc_html_e('Human sacrifice, dogs and cats living together... mass hysteria!',
							                                        'bbpress'); ?></p>
						</fieldset>
					</td>
				</tr>
			...
			<fieldset class="submit">
				<input class="button-primary" type="submit" name="submit"
				       value="<?php esc_attr_e('Reset bbPress', 'bbpress'); ?>"/>
...
function bbp_admin_reset_handler() {
	...
	$sql_posts
	            = $wpdb->get_results("SELECT `ID` FROM `{$wpdb->posts}` WHERE `post_type` IN ('forum', 'topic', 'reply')",
	                                 OBJECT_K);
	...
	$sql_delete
	            = "DELETE a,b,c FROM `{$wpdb->terms}` AS a LEFT JOIN `{$wpdb->term_taxonomy}` AS c ON a.term_id = c.term_id LEFT JOIN `{$wpdb->term_relationships}` AS b ON b.term_taxonomy_id = c.term_taxonomy_id WHERE c.taxonomy = 'topic-tag';";
	$result     = is_wp_error($wpdb->query($sql_delete)) ? $failed : $success;
	$messages[] = sprintf($statement, $result);
	/** User ******************************************************************/
	// Delete users
	if (!empty($_POST['bbpress-delete-imported-users'])) {
		$sql_users = $wpdb->get_results("SELECT `user_id` FROM `{$wpdb->usermeta}` WHERE `meta_key` = '_bbp_user_id'",
		                                OBJECT_K);
		if (!empty($sql_users)) {
			$sql_meta = [];
