		...
		<table>
			...
			<tr class="first">
				...
				if (current_user_can('publish_forums')) {
					$link = add_query_arg(['post_type' => bbp_get_forum_post_type()], get_admin_url(NULL, 'edit.php'));
			...
			<tr>
				...
				if (current_user_can('publish_topics')) {
					$link = add_query_arg(['post_type' => bbp_get_topic_post_type()], get_admin_url(NULL, 'edit.php'));
					$num  = '<a href="' . esc_url($link) . '">' . $num . '</a>';
					$text = '<a href="' . esc_url($link) . '">' . $text . '</a>';
				}
				?>
			...
			<tr>
				...
				if (current_user_can('publish_replies')) {
					$link = add_query_arg(['post_type' => bbp_get_reply_post_type()], get_admin_url(NULL, 'edit.php'));
					$num  = '<a href="' . esc_url($link) . '">' . $num . '</a>';
					$text = '<a href="' . esc_url($link) . '">' . $text . '</a>';
				}
				?>
			...
			<?php if (bbp_allow_topic_tags()) : ?>
				...
				<tr>
					...
					if (current_user_can('manage_topic_tags')) {
						$link
							  = add_query_arg(['taxonomy' => bbp_get_topic_tag_tax_id(), 'post_type' => bbp_get_topic_post_type()],
		...
		<table>
			...
			<?php if (isset($r['topic_count_hidden'])) : ?>
				...
				<tr>
					...
					$link = add_query_arg(['post_type' => bbp_get_topic_post_type()], get_admin_url(NULL, 'edit.php'));
					if ('0' !== $num) {
						$link = add_query_arg(['post_status' => bbp_get_spam_status_id()], $link);
					}
					$num
						= '<a href="' . esc_url($link) . '" title="' . esc_attr($r['hidden_topic_title']) . '">' . $num . '</a>';
					$text
						= '<a class="waiting" href="' . esc_url($link) . '" title="' . esc_attr($r['hidden_topic_title']) . '">' . $text . '</a>';
					?>
			...
			<?php if (isset($r['reply_count_hidden'])) : ?>
				...
				<tr>
					...
					$num
						= '<a href="' . esc_url($link) . '" title="' . esc_attr($r['hidden_reply_title']) . '">' . $num . '</a>';
					$text
						= '<a class="waiting" href="' . esc_url($link) . '" title="' . esc_attr($r['hidden_reply_title']) . '">' . $text . '</a>';
					?>
...
function bbp_forum_metabox() {
	...
	<p>
		...
		<?php bbp_form_forum_type_dropdown(['forum_id' => $post_id]); ?>
	</p>
	...
	<p>
		...
		<label class="screen-reader-text" for="bbp_forum_status_select"><?php esc_html_e('Status:',
		                                                                                 'bbpress') ?></label>
		<?php bbp_form_forum_status_dropdown(['forum_id' => $post_id]); ?>
	</p>
	...
	<p>
		...
		<label class="screen-reader-text" for="bbp_forum_visibility_select"><?php esc_html_e('Visibility:',
		                                                                                     'bbpress') ?></label>
		<?php bbp_form_forum_visibility_dropdown(['forum_id' => $post_id]); ?>
	</p>
	...
	<p>
		...
		<input name="menu_order" type="number" step="1" size="4" id="menu_order"
...
function bbp_topic_metabox() {
	...
	<p>
		...
		<?php bbp_form_topic_type_dropdown(['topic_id' => $post_id]); ?>
	</p>
	...
	<p>
		...
		<label class="screen-reader-text"
		       for="bbp_open_close_topic"><?php esc_html_e('Select whether to open or close the topic.',
		                                                   'bbpress'); ?></label>
		<?php bbp_form_topic_status_dropdown(['select_id' => 'post_status', 'topic_id' => $post_id]); ?>
	</p>
...
function bbp_reply_metabox() {
	...
	<p>
		...
		<input name="parent_id" id="bbp_topic_id" type="text" value="<?php echo esc_attr($reply_topic_id); ?>"
		       data-ajax-url="<?php echo esc_url(wp_nonce_url(add_query_arg(['action' => 'bbp_suggest_topic'],
		                                                                    admin_url('admin-ajax.php', 'relative'))),
		                                         'bbp_suggest_topic_nonce'); ?>"/>
	</p>
...
function bbp_author_metabox() {
	...
	if (bbp_is_reply_anonymous($post_id) || bbp_is_topic_anonymous($post_id)) : ?>
		...
		<p>
			...
			<input type="text" id="bbp_anonymous_name" name="bbp_anonymous_name"
			       value="<?php echo esc_attr(get_post_meta($post_id, '_bbp_anonymous_name', TRUE)); ?>"/>
		</p>
		...
		<p>
			...
			<input type="text" id="bbp_anonymous_email" name="bbp_anonymous_email"
			       value="<?php echo esc_attr(get_post_meta($post_id, '_bbp_anonymous_email', TRUE)); ?>"/>
		</p>

		<p>
			<strong class="label"><?php esc_html_e('Website:', 'bbpress'); ?></strong>
			<label class="screen-reader-text" for="bbp_anonymous_website"><?php esc_html_e('Website',
			                                                                               'bbpress'); ?></label>
			<input type="text" id="bbp_anonymous_website" name="bbp_anonymous_website"
			       value="<?php echo esc_attr(get_post_meta($post_id, '_bbp_anonymous_website', TRUE)); ?>"/>
		</p>
	...
	<?php else : ?>
		...
		<p>
			...
			<input type="text" id="bbp_author_id" name="post_author_override"
			       value="<?php echo esc_attr(bbp_get_global_post_field('post_author')); ?>"
			       data-ajax-url="<?php echo esc_url(wp_nonce_url(add_query_arg(['action' => 'bbp_suggest_user'],
			                                                                    admin_url('admin-ajax.php',
			                                                                              'relative'))),
			                                         'bbp_suggest_user_nonce'); ?>"/>
		</p>
	...
	<p>
		...
		<label class="screen-reader-text" for="bbp_author_ip_address"><?php esc_html_e('IP Address',
		                                                                               'bbpress'); ?></label>
		<input type="text" id="bbp_author_ip_address" name="bbp_author_ip_address"
		       value="<?php echo esc_attr(get_post_meta($post_id, '_bbp_author_ip', TRUE)); ?>" disabled="disabled"/>
	</p>
