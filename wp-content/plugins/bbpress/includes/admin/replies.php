...
if (!class_exists('BBP_Replies_Admin')) :
	...
	class BBP_Replies_Admin {
		...
		private function setup_actions() {
			...
			add_action('load-edit.php', [$this, 'toggle_reply']);
			add_action('admin_notices', [$this, 'toggle_reply_notice']);
		...
		public function edit_help() {
			...
			get_current_screen()->add_help_tab([
				                                   ...
				                                   'content' =>
					                                   '<p>' . __('This screen provides access to all of your replies. You can customize the display of this screen to suit your workflow.',
					                                              'bbpress') . '</p>'
			                                   ]);
			// Screen Content
			get_current_screen()->add_help_tab([
				                                   ...
				                                   'content' =>
					                                   '<p>' . __('You can customize the display of this screen&#8217;s contents in a number of ways:',
					                                              'bbpress') . '</p>' .
					                                   '<ul>' .
					                                   '<li>' . __('You can hide/display columns based on your needs and decide how many replies to list per screen using the Screen Options tab.',
					                                               'bbpress') . '</li>' .
					                                   '<li>' . __('You can filter the list of replies by reply status using the text links in the upper left to show All, Published, Draft, or Trashed replies. The default view is to show all replies.',
					                                               'bbpress') . '</li>' .
					                                   '<li>' . __('You can view replies in a simple title list or with an excerpt. Choose the view you prefer by clicking on the icons at the top of the list on the right.',
					                                               'bbpress') . '</li>' .
					                                   '<li>' . __('You can refine the list to show only replies in a specific category or from a specific month by using the dropdown menus above the replies list. Click the Filter button after making your selection. You also can refine the list by clicking on the reply author, category or tag in the replies list.',
					                                               'bbpress') . '</li>' .
					                                   '</ul>'
			                                   ]);
			// Available Actions
			get_current_screen()->add_help_tab([
				                                   ...
				                                   'content' =>
					                                   '<p>' . __('Hovering over a row in the replies list will display action links that allow you to manage your reply. You can perform the following actions:',
					                                              'bbpress') . '</p>' .
					                                   '<ul>' .
					                                   '<li>' . __('<strong>Edit</strong> takes you to the editing screen for that reply. You can also reach that screen by clicking on the reply title.',
					                                               'bbpress') . '</li>' .
					                                   ...
					                                   '<li>' . __('<strong>Trash</strong> removes your reply from this list and places it in the trash, from which you can permanently delete it.',
					                                               'bbpress') . '</li>' .
					                                   '<li>' . __('<strong>Spam</strong> removes your reply from this list and places it in the spam queue, from which you can permanently delete it.',
					                                               'bbpress') . '</li>' .
					                                   '<li>' . __('<strong>Preview</strong> will show you what your draft reply will look like if you publish it. View will take you to your live site to view the reply. Which link is available depends on your reply&#8217;s status.',
					                                               'bbpress') . '</li>' .
					                                   '</ul>'
			                                   ]);
			// Bulk Actions
			get_current_screen()->add_help_tab([
				                                   ...
				                                   'content' =>
					                                   '<p>' . __('You can also edit or move multiple replies to the trash at once. Select the replies you want to act on using the checkboxes, then select the action you want to take from the Bulk Actions menu and click Apply.',
					                                              'bbpress') . '</p>' .
					                                   '<p>' . __('When using Bulk Edit, you can change the metadata (categories, author, etc.) for all selected replies at once. To remove a reply from the grouping, just click the x next to its name in the Bulk Edit area that appears.',
					                                              'bbpress') . '</p>'
			                                   ]);
			// Help Sidebar
		...
		public function new_help() {
			...
			get_current_screen()->add_help_tab([
				                                   ...
				                                   'content' =>
					                                   '<p>' . __('<strong>Title</strong> - Enter a title for your reply. After you enter a title, you&#8217;ll see the permalink below, which you can edit.',
					                                              'bbpress') . '</p>' .
			...
			if (current_theme_supports('reply-thumbnails') && post_type_supports('reply', 'thumbnail')) {
				$publish_box .= '<p>' . __('<strong>Featured Image</strong> - This allows you to associate an image with your reply without inserting it. This is usually useful only if your theme makes use of the featured image as a reply thumbnail on the home page, a custom header, etc.',
				                           'bbpress') . '</p>';
			}
			get_current_screen()->add_help_tab([
				                                   'id'      => 'reply-attributes',
				                                   ...
				                                   'content' =>
					                                   '<p>' . __('Select the attributes that your reply should have:',
					                                              'bbpress') . '</p>' .
					                                   '<ul>' .
					                                   '<li>' . __('<strong>Forum</strong> dropdown determines the parent forum that the reply belongs to. Select the forum, or leave the default (Use Forum of Topic) to post the reply in forum of the topic.',
					                                               'bbpress') . '</li>' .
					                                   '<li>' . __('<strong>Topic</strong> determines the parent topic that the reply belongs to.',
					                                               'bbpress') . '</li>' .
					                                   '<li>' . __('<strong>Reply To</strong> determines the threading of the reply.',
					                                               'bbpress') . '</li>' .
					                                   '</ul>'
			                                   ]);
			get_current_screen()->add_help_tab([
		...
		public function attributes_metabox_save($reply_id) {
			...
			// Nonce check
			if (empty($_POST['bbp_reply_metabox']) || !wp_verify_nonce($_POST['bbp_reply_metabox'],
			                                                           'bbp_reply_metabox_save')
			) {
				return $reply_id;
			}
			...
			$forum_id = !empty($_POST['bbp_forum_id']) ? (int)$_POST['bbp_forum_id']
		...
		public function toggle_reply() {
			...
			if (bbp_is_get_request() && !empty($_GET['action']) && in_array($_GET['action'],
			                                                                ['bbp_toggle_reply_spam']) && !empty($_GET['reply_id'])
			) {
				$action    = $_GET['action'];            // What action is taking place?
				$reply_id  = (int)$_GET['reply_id'];    // What's the reply id?
				$success   = FALSE;                      // Flag
				$post_data = ['ID' => $reply_id]; // Prelim array
				...
				$message = ['bbp_reply_toggle_notice' => $message, 'reply_id' => $reply->ID];
				...
				$redirect = add_query_arg($message, remove_query_arg(['action', 'reply_id']));
		...
		public function toggle_reply_notice() {
			if ($this->bail()) return;
			// Only proceed if GET is a reply toggle action
			if (bbp_is_get_request() && !empty($_GET['bbp_reply_toggle_notice']) && in_array($_GET['bbp_reply_toggle_notice'],
			                                                                                 ['spammed', 'unspammed']) && !empty($_GET['reply_id'])
			) {
				...
				switch ($notice) {
					case 'spammed' :
						$message = $is_failure === TRUE
							? sprintf(__('There was a problem marking the reply "%1$s" as spam.', 'bbpress'),
							          $reply_title)
							: sprintf(__('Reply "%1$s" successfully marked as spam.', 'bbpress'), $reply_title);
						break;
					case 'unspammed' :
						$message = $is_failure === TRUE
							? sprintf(__('There was a problem unmarking the reply "%1$s" as spam.', 'bbpress'),
							          $reply_title)
					...
					if (!empty($reply_forum_id)) {
						...
						if ($reply_forum_id !== $topic_forum_id) {
							if (current_user_can('edit_others_replies') || current_user_can('moderate')) {
								$forum_title .= '<div class="attention">' . esc_html__('(Mismatch)',
								                                                       'bbpress') . '</div>';
							}
						}
						// Output the title
		...
		public function row_actions($actions, $reply) {
			...
			$actions['view']
				= '<a href="' . esc_url(bbp_get_reply_url($reply->ID)) . '" title="' . esc_attr(sprintf(__('View &#8220;%s&#8221;',
				                                                                                           'bbpress'),
				                                                                                        bbp_get_reply_title($reply->ID))) . '" rel="permalink">' . esc_html__('View',
				                                                                                                                                                              'bbpress') . '</a>';
			...
			if (current_user_can('moderate', $reply->ID)) {
				if (in_array($reply->post_status, [bbp_get_public_status_id(), bbp_get_spam_status_id()])) {
					$spam_uri
						= wp_nonce_url(add_query_arg(['reply_id' => $reply->ID, 'action' => 'bbp_toggle_reply_spam'],
						                             remove_query_arg(['bbp_reply_toggle_notice', 'reply_id', 'failed', 'super'])),
						               'spam-reply_' . $reply->ID);
					if (bbp_is_reply_spam($reply->ID)) {
						$actions['spam']
							= '<a href="' . esc_url($spam_uri) . '" title="' . esc_attr__('Mark the reply as not spam',
							                                                              'bbpress') . '">' . esc_html__('Not spam',
							                                                                                             'bbpress') . '</a>';
					} else {
						$actions['spam']
							= '<a href="' . esc_url($spam_uri) . '" title="' . esc_attr__('Mark this reply as spam',
			...
			if (current_user_can('delete_reply', $reply->ID)) {
				if (bbp_get_trash_status_id() === $reply->post_status) {
					...
					$actions['untrash'] = "<a title='" . esc_attr__('Restore this item from the Trash',
					                                                'bbpress') . "' href='" . esc_url(add_query_arg(['_wp_http_referer' => add_query_arg(['post_type' => bbp_get_reply_post_type()],
					                                                                                                                                     admin_url('edit.php'))],
					                                                                                                wp_nonce_url(admin_url(sprintf($post_type_object->_edit_link . '&amp;action=untrash',
					                                                                                                                               $reply->ID)),
					                                                                                                             'untrash-' . $reply->post_type . '_' . $reply->ID))) . "'>" . esc_html__('Restore',
					                                                                                                                                                                                      'bbpress') . "</a>";
				} elseif (EMPTY_TRASH_DAYS) {
					$actions['trash'] = "<a class='submitdelete' title='" . esc_attr__('Move this item to the Trash',
					                                                                   'bbpress') . "' href='" . esc_url(add_query_arg(['_wp_http_referer' => add_query_arg(['post_type' => bbp_get_reply_post_type()],
					                                                                                                                                                        admin_url('edit.php'))],
					                                                                                                                   get_delete_post_link($reply->ID))) . "'>" . esc_html__('Trash',
					                                                                                                                                                                          'bbpress') . "</a>";
				}
		...
		public function updated_messages($messages) {
			...
			$messages[ $this->post_type ] = [
				...
				5  => isset($_GET['revision'])
					? sprintf(__('Reply restored to revision from %s', 'bbpress'),
				...
				// Reply submitted
				8  => sprintf(__('Reply submitted. <a target="_blank" href="%s">Preview topic</a>', 'bbpress'),
				              esc_url(add_query_arg('preview', 'true', $topic_url))),
				// Reply scheduled
				9  => sprintf(__('Reply scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview topic</a>',
				                 'bbpress'),
					// translators: Publish box date format, see http://php.net/date
					          ...
					          $topic_url),
				// Reply draft updated
				10 => sprintf(__('Reply draft updated. <a target="_blank" href="%s">Preview topic</a>', 'bbpress'),
				              esc_url(add_query_arg('preview', 'true', $topic_url))),
			];
