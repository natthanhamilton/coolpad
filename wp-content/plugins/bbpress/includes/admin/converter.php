...
class BBP_Converter {
	...
	private function setup_actions() {
		...
		add_action('bbp_register_admin_settings', [$this, 'register_admin_settings']);
		// Attach to the admin ajax request to process cycles
		add_action('wp_ajax_bbconverter_process', [$this, 'process_callback']);
	}
	...
	public function register_admin_settings() {
		...
		add_settings_section('bbpress_converter_main', __('Database Settings', 'bbpress'),
		                     'bbp_converter_setting_callback_main_section', 'bbpress_converter');
		// System Select
		add_settings_field('_bbp_converter_platform', __('Select Platform', 'bbpress'),
		                   'bbp_converter_setting_callback_platform', 'bbpress_converter', 'bbpress_converter_main');
		register_setting('bbpress_converter_main', '_bbp_converter_platform', 'sanitize_title');
		// Database Server
		add_settings_field('_bbp_converter_db_server', __('Database Server', 'bbpress'),
		                   'bbp_converter_setting_callback_dbserver', 'bbpress_converter', 'bbpress_converter_main');
		register_setting('bbpress_converter_main', '_bbp_converter_db_server', 'sanitize_title');
		// Database Server Port
		add_settings_field('_bbp_converter_db_port', __('Database Port', 'bbpress'),
		                   'bbp_converter_setting_callback_dbport', 'bbpress_converter', 'bbpress_converter_main');
		register_setting('bbpress_converter_main', '_bbp_converter_db_port', 'sanitize_title');
		// Database Name
		add_settings_field('_bbp_converter_db_name', __('Database Name', 'bbpress'),
		                   'bbp_converter_setting_callback_dbname', 'bbpress_converter', 'bbpress_converter_main');
		register_setting('bbpress_converter_main', '_bbp_converter_db_name', 'sanitize_title');
		// Database User
		add_settings_field('_bbp_converter_db_user', __('Database User', 'bbpress'),
		                   'bbp_converter_setting_callback_dbuser', 'bbpress_converter', 'bbpress_converter_main');
		register_setting('bbpress_converter_main', '_bbp_converter_db_user', 'sanitize_title');
		// Database Pass
		add_settings_field('_bbp_converter_db_pass', __('Database Password', 'bbpress'),
		                   'bbp_converter_setting_callback_dbpass', 'bbpress_converter', 'bbpress_converter_main');
		register_setting('bbpress_converter_main', '_bbp_converter_db_pass', 'sanitize_title');
		// Database Prefix
		add_settings_field('_bbp_converter_db_prefix', __('Table Prefix', 'bbpress'),
		                   'bbp_converter_setting_callback_dbprefix', 'bbpress_converter', 'bbpress_converter_main');
		register_setting('bbpress_converter_main', '_bbp_converter_db_prefix', 'sanitize_title');
		// Add the options section
		add_settings_section('bbpress_converter_opt', __('Options', 'bbpress'),
		                     'bbp_converter_setting_callback_options_section', 'bbpress_converter');
		// Rows Limit
		add_settings_field('_bbp_converter_rows', __('Rows Limit', 'bbpress'), 'bbp_converter_setting_callback_rows',
		                   'bbpress_converter', 'bbpress_converter_opt');
		register_setting('bbpress_converter_opt', '_bbp_converter_rows', 'intval');
		// Delay Time
		add_settings_field('_bbp_converter_delay_time', __('Delay Time', 'bbpress'),
		                   'bbp_converter_setting_callback_delay_time', 'bbpress_converter', 'bbpress_converter_opt');
		register_setting('bbpress_converter_opt', '_bbp_converter_delay_time', 'intval');
		// Convert Users ?
		add_settings_field('_bbp_converter_convert_users', __('Convert Users', 'bbpress'),
		                   'bbp_converter_setting_callback_convert_users', 'bbpress_converter',
		                   'bbpress_converter_opt');
		register_setting('bbpress_converter_opt', '_bbp_converter_convert_users', 'intval');
		// Restart
		add_settings_field('_bbp_converter_restart', __('Start Over', 'bbpress'),
		                   'bbp_converter_setting_callback_restart', 'bbpress_converter', 'bbpress_converter_opt');
		register_setting('bbpress_converter_opt', '_bbp_converter_restart', 'intval');
		// Clean
		add_settings_field('_bbp_converter_clean', __('Purge Previous Import', 'bbpress'),
		                   'bbp_converter_setting_callback_clean', 'bbpress_converter', 'bbpress_converter_opt');
	...
	public function admin_head() { ?>
		...
		<script language="javascript">
			...
			function bbconverter_success(response) {
				...
				if (response == '<p class="loading"><?php esc_html_e('Conversion Complete',
				                                                     'bbpress'); ?></p>' || response.indexOf('error') > -1) {
	...
	public function process_callback() {
		...
		$platform = !empty($_POST['_bbp_converter_platform']) ? $_POST['_bbp_converter_platform']
		...
		switch ($step) {
			...
			case 1 :
				if (!empty($_POST['_bbp_converter_clean'])) {
					if ($converter->clean($start)) {
						...
						$this->converter_output(sprintf(__('Deleting previously converted data (%1$s - %2$s)',
						                                   'bbpress'), $min, $max));
					}
				} else {
					update_option('_bbp_converter_step', $step + 1);
			...
			case 3 :
				if (!empty($_POST['_bbp_converter_convert_users'])) {
					if ($converter->clean_passwords($start)) {
						...
						update_option('_bbp_converter_start', $max + 1);
						$this->converter_output(sprintf(__('Delete users WordPress default passwords (%1$s - %2$s)',
						                                   'bbpress'), $min, $max));
					}
				} else {
					update_option('_bbp_converter_step', $step + 1);
			...
			case 5 :
				if ($converter->convert_forum_parents($start)) {
					...
					update_option('_bbp_converter_start', $max + 1);
					$this->converter_output(sprintf(__('Calculating forum hierarchy (%1$s - %2$s)', 'bbpress'), $min,
					                                $max));
				}
				break;
			...
			case 7 :
				if ($converter->convert_topic_stickies($start)) {
					...
					update_option('_bbp_converter_start', $max + 1);
					$this->converter_output(sprintf(__('Calculating topic stickies (%1$s - %2$s)', 'bbpress'), $min,
					                                $max));
				}
				break;
			...
			case 8 :
				if ($converter->convert_topic_super_stickies($start)) {
					...
					update_option('_bbp_converter_start', $max + 1);
					$this->converter_output(sprintf(__('Calculating topic super stickies (%1$s - %2$s)', 'bbpress'),
					                                $min, $max));
				}
				break;
			// STEP 9. Convert tags.
			...
			case 11 :
				if ($converter->convert_reply_to_parents($start)) {
					...
					update_option('_bbp_converter_start', $max + 1);
					$this->converter_output(sprintf(__('Calculating reply_to parents (%1$s - %2$s)', 'bbpress'), $min,
					                                $max));
				}
				break;
			default :
...
abstract class BBP_Converter_Base {
	 ...
	 */
	private $map_topicid_to_forumid = [];
	/**
	...
	private function setup_globals() {
		...
		$this->opdb         = new wpdb($_POST['_bbp_converter_db_user'], $_POST['_bbp_converter_db_pass'],
		                               $_POST['_bbp_converter_db_name'], $_POST['_bbp_converter_db_server']);
	...
	public function convert_table($to_type, $start) {
		...
		$has_insert     = FALSE;
		$from_tablename = '';
		$field_list     = $from_tables = $tablefield_array = [];
		...
		foreach ($this->field_map as $item) {
			...
			if (($item['to_type'] == $to_type) && !empty($item['from_tablename'])) {
				...
				if (!empty($from_tablename)) {
					// Doing some joining
					if (!in_array($item['from_tablename'], $from_tables) && in_array($item['join_tablename'],
					                                                                 $from_tables)
					) {
				...
				$field_list[]
				               = 'convert(' . $item['from_tablename'] . '.' . $item['from_fieldname'] . ' USING "' . $this->charset . '") AS ' . $item['from_fieldname'];
			}
		}
		/** Step 2 ************************************************************/
		...
		if (!empty($from_tablename)) {
			...
			$forum_query = 'SELECT ' . implode(',',
			                                   $field_list) . ' FROM ' . $this->opdb->prefix . $from_tablename . ' LIMIT ' . $start . ', ' . $this->max_rows;
			...
			if (!empty($forum_array)) {
				...
				foreach ((array)$forum_array as $forum) {
					// Reset some defaults
					$insert_post = $insert_postmeta = $insert_data = [];
							...
							if (in_array($row['to_fieldname'], $tablefield_array)) {
								...
								if (isset($row['default'])) {
									...
									if (('callback_userid' == $row['callback_method']) && empty($_POST['_bbp_converter_convert_users'])) {
										...
										$insert_post[ $row['to_fieldname'] ]
											= call_user_func_array([$this, $row['callback_method']],
											                       [$forum[ $row['from_fieldname'] ], $forum]);
									}
								...
								if (isset($row['default'])) {
									...
									if (($row['callback_method'] == 'callback_userid') && (0 == $_POST['_bbp_converter_convert_users'])) {
										...
										$insert_postmeta[ $row['to_fieldname'] ]
											= call_user_func_array([$this, $row['callback_method']],
											                       [$forum[ $row['from_fieldname'] ], $forum]);
									}
									// Maps the field from the old forum.
					...
					if (count($insert_post) > 0 || ($to_type == 'tags' && count($insert_postmeta) > 0)) {
						switch ($to_type) {
								...
								if (is_numeric($post_id)) {
									foreach ($insert_postmeta as $key => $value) {
										...
										if ('_id' == substr($key, -3) && (TRUE === $this->sync_table)) {
											$this->wpdb->insert($this->sync_table_name,
											                    ['value_type' => 'user', 'value_id' => $post_id, 'meta_key' => $key, 'meta_value' => $value]);
										}
									}
								}
								break;
							/** New Topic-Tag *********************************/
							case 'tags':
								$post_id = wp_set_object_terms($insert_postmeta['objectid'], $insert_postmeta['name'],
								                               'topic-tag', TRUE);
								$term    = get_term_by('name', $insert_postmeta['name'], 'topic-tag');
								if (FALSE !== $term) {
									wp_update_term($term->term_id, 'topic-tag', [
								...
								if (is_numeric($post_id)) {
									foreach ($insert_postmeta as $key => $value) {
										...
										if (('topic' == $to_type) && ('_bbp_topic_id' == $key)) {
											...
											if ('_id' == substr($key, -3) && (TRUE === $this->sync_table)) {
												$this->wpdb->insert($this->sync_table_name,
												                    ['value_type' => 'post', 'value_id' => $post_id, 'meta_key' => '_bbp_topic_id', 'meta_value' => $post_id]);
												$this->wpdb->insert($this->sync_table_name,
												                    ['value_type' => 'post', 'value_id' => $post_id, 'meta_key' => '_bbp_old_topic_id', 'meta_value' => $value]);
											}
										} elseif ('_id' == substr($key, -3) && (TRUE === $this->sync_table)) {
											$this->wpdb->insert($this->sync_table_name,
											                    ['value_type' => 'post', 'value_id' => $post_id, 'meta_key' => $key, 'meta_value' => $value]);
										}
	...
	public function convert_forum_parents($start) {
		...
		if (!empty($this->sync_table)) {
			$query
				= 'SELECT value_id, meta_value FROM ' . $this->sync_table_name . ' WHERE meta_key = "_bbp_forum_parent_id" AND meta_value > 0 LIMIT ' . $start . ', ' . $this->max_rows;
		} else {
			$query
				= 'SELECT post_id AS value_id, meta_value FROM ' . $this->wpdb->postmeta . ' WHERE meta_key = "_bbp_forum_parent_id" AND meta_value > 0 LIMIT ' . $start . ', ' . $this->max_rows;
		}
	...
	private function callback_forumid($field) {
		if (!isset($this->map_forumid[ $field ])) {
			if (!empty($this->sync_table)) {
				$row
					= $this->wpdb->get_row($this->wpdb->prepare('SELECT value_id, meta_value FROM ' . $this->sync_table_name . ' WHERE meta_key = "_bbp_forum_id" AND meta_value = "%s" LIMIT 1',
					                                            $field));
			} else {
				$row
					= $this->wpdb->get_row($this->wpdb->prepare('SELECT post_id AS value_id FROM ' . $this->wpdb->postmeta . ' WHERE meta_key = "_bbp_forum_id" AND meta_value = "%s" LIMIT 1',
					                                            $field));
			}
			if (!is_null($row)) {
	...
	public function convert_topic_stickies($start) {
		...
		if (!empty($this->sync_table)) {
			$query
				= 'SELECT value_id, meta_value FROM ' . $this->sync_table_name . ' WHERE meta_key = "_bbp_old_sticky_status" AND meta_value = "sticky" LIMIT ' . $start . ', ' . $this->max_rows;
		} else {
			$query
				= 'SELECT post_id AS value_id, meta_value FROM ' . $this->wpdb->postmeta . ' WHERE meta_key = "_bbp_old_sticky_status" AND meta_value = "sticky" LIMIT ' . $start . ', ' . $this->max_rows;
		}
		update_option('_bbp_converter_query', $query);
	...
	public function convert_topic_super_stickies($start) {
		...
		if (!empty($this->sync_table)) {
			$query
				= 'SELECT value_id, meta_value FROM ' . $this->sync_table_name . ' WHERE meta_key = "_bbp_old_sticky_status" AND meta_value = "super-sticky" LIMIT ' . $start . ', ' . $this->max_rows;
		} else {
			$query
				= 'SELECT post_id AS value_id, meta_value FROM ' . $this->wpdb->postmeta . ' WHERE meta_key = "_bbp_old_sticky_status" AND meta_value = "super-sticky" LIMIT ' . $start . ', ' . $this->max_rows;
		}
		update_option('_bbp_converter_query', $query);
		$sticky_array = $this->wpdb->get_results($query);
		foreach ((array)$sticky_array as $row) {
	...
	public function convert_reply_to_parents($start) {
		...
		if (!empty($this->sync_table)) {
			$query
				= 'SELECT value_id, meta_value FROM ' . $this->sync_table_name . ' WHERE meta_key = "_bbp_old_reply_to" AND meta_value > 0 LIMIT ' . $start . ', ' . $this->max_rows;
		} else {
			$query
				= 'SELECT post_id AS value_id, meta_value FROM ' . $this->wpdb->postmeta . ' WHERE meta_key = "_bbp_old_reply_to" AND meta_value > 0 LIMIT ' . $start . ', ' . $this->max_rows;
		}
		update_option('_bbp_converter_query', $query);
	...
	private function callback_reply_to($field) {
		if (!isset($this->map_reply_to[ $field ])) {
			if (!empty($this->sync_table)) {
				$row
					= $this->wpdb->get_row($this->wpdb->prepare('SELECT value_id, meta_value FROM ' . $this->sync_table_name . ' WHERE meta_key = "_bbp_post_id" AND meta_value = "%s" LIMIT 1',
					                                            $field));
			} else {
				$row
					= $this->wpdb->get_row($this->wpdb->prepare('SELECT post_id AS value_id FROM ' . $this->wpdb->postmeta . ' WHERE meta_key = "_bbp_post_id" AND meta_value = "%s" LIMIT 1',
					                                            $field));
			}
			if (!is_null($row)) {
	...
	public function clean($start) {
		...
		if (TRUE === $this->sync_table) {
			...
			$query
				= 'SELECT post_id AS value_id FROM ' . $this->wpdb->postmeta . ' WHERE meta_key LIKE "_bbp_%" GROUP BY post_id ORDER BY post_id DESC LIMIT ' . $this->max_rows;
		}
		update_option('_bbp_converter_query', $query);
		...
		if (TRUE === $this->sync_table) {
			$query
				= 'SELECT value_id FROM ' . $this->sync_table_name . ' INNER JOIN ' . $this->wpdb->users . ' ON(value_id = ID) WHERE meta_key = "_bbp_user_id" AND value_type = "user" LIMIT ' . $this->max_rows;
		} else {
			$query
				= 'SELECT user_id AS value_id FROM ' . $this->wpdb->usermeta . ' WHERE meta_key = "_bbp_user_id" LIMIT ' . $this->max_rows;
		}
		update_option('_bbp_converter_query', $query);
	...
	public function callback_pass($username, $password) {
		$user
			= $this->wpdb->get_row($this->wpdb->prepare('SELECT * FROM ' . $this->wpdb->users . ' WHERE user_login = "%s" AND user_pass = "" LIMIT 1',
			                                            $username));
		if (!empty($user)) {
			$usermeta
				= $this->wpdb->get_row('SELECT * FROM ' . $this->wpdb->usermeta . ' WHERE meta_key = "_bbp_password" AND user_id = "' . $user->ID . '" LIMIT 1');
			if (!empty($usermeta)) {
	...
	private function callback_userid($field) {
		if (!isset($this->map_userid[ $field ])) {
			if (!empty($this->sync_table)) {
				$row
					= $this->wpdb->get_row($this->wpdb->prepare('SELECT value_id, meta_value FROM ' . $this->sync_table_name . ' WHERE meta_key = "_bbp_user_id" AND meta_value = "%s" LIMIT 1',
					                                            $field));
			} else {
				$row
					= $this->wpdb->get_row($this->wpdb->prepare('SELECT user_id AS value_id FROM ' . $this->wpdb->usermeta . ' WHERE meta_key = "_bbp_user_id" AND meta_value = "%s" LIMIT 1',
					                                            $field));
			}
			if (!is_null($row)) {
	...
	private function callback_topicid_to_forumid($field) {
		...
		if (empty($topicid)) {
			...
			$row
				= $this->wpdb->get_row('SELECT post_parent FROM ' . $this->wpdb->posts . ' WHERE ID = "' . $topicid . '" LIMIT 1');
			if (!is_null($row)) {
	...
	private function callback_topicid($field) {
		if (!isset($this->map_topicid[ $field ])) {
			if (!empty($this->sync_table)) {
				$row
					= $this->wpdb->get_row($this->wpdb->prepare('SELECT value_id, meta_value FROM ' . $this->sync_table_name . ' WHERE meta_key = "_bbp_old_topic_id" AND meta_value = "%s" LIMIT 1',
					                                            $field));
			} else {
				$row
					= $this->wpdb->get_row($this->wpdb->prepare('SELECT post_id AS value_id FROM ' . $this->wpdb->postmeta . ' WHERE meta_key = "_bbp_old_topic_id" AND meta_value = "%s" LIMIT 1',
					                                            $field));
			}
			if (!is_null($row)) {
