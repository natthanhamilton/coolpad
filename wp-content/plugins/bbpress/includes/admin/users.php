...
if (!class_exists('BBP_Users_Admin')) :
	...
	class BBP_Users_Admin {
		...
		function setup_actions() {
			...
			add_action('edit_user_profile', [$this, 'secondary_role_display']);
			// WordPress user screen
			add_action('restrict_manage_users', [$this, 'user_role_bulk_dropdown']);
			add_filter('manage_users_columns', [$this, 'user_role_column']);
			add_filter('manage_users_custom_column', [$this, 'user_role_row'], 10, 3);
			// Process bulk role change
			add_action('load-users.php', [$this, 'user_role_bulk_change']);
		}
					...
					<td>
						...
						<select name="bbp-forums-role" id="bbp-forums-role">
							...
							<?php if (!empty($user_role)) : ?>
								...
								<option value=""><?php esc_html_e('&mdash; No role for these forums &mdash;',
								                                  'bbpress'); ?></option>

							<?php else : ?>

								<option value=""
								        selected="selected"><?php esc_html_e('&mdash; No role for these forums &mdash;',
								                                             'bbpress'); ?></option>
		...
		public static function user_role_bulk_dropdown() {
			...
			<label class="screen-reader-text" for="bbp-new-role"><?php esc_html_e('Change forum role to&hellip;',
			                                                                      'bbpress') ?></label>
		...
		public function user_role_bulk_change() {
			...
			foreach ((array)$_REQUEST['users'] as $user_id) {
				...
				// Only keymasters can set other keymasters
				if (in_array(bbp_get_keymaster_role(), [$user_role, $new_role]) && !bbp_is_user_keymaster()) {
					continue;
				}
		 ...
		 */
		public static function user_role_column($columns = []) {
