...
function bbp_get_topic_post_type_labels() {
	return apply_filters('bbp_get_topic_post_type_labels', [
...
function bbp_get_topic_post_type_rewrite() {
	return apply_filters('bbp_get_topic_post_type_rewrite', [
...
function bbp_get_topic_post_type_supports() {
	return apply_filters('bbp_get_topic_post_type_supports', [
		'title',
		'editor',
		'revisions'
	]);
...
function bbp_has_topics($args = '') {
	...
	$default_show_stickies
	                      = (bool)(bbp_is_single_forum() || bbp_is_topic_archive()) && (FALSE === $default_topic_search);
	...
	if (!empty($r['show_stickies']) && $r['paged'] <= 1) {
		...
		if (is_array($stickies) && !empty($stickies)) {
			...
			foreach ($stickies as $sticky_index => $sticky_ID) {
				...
				$post_offsets = wp_filter_object_list($bbp->topic_query->posts, ['ID' => $sticky_ID], 'OR', 'ID');
				...
				foreach (array_keys($post_offsets) as $post_offset) {
					...
					array_splice($bbp->topic_query->posts, $sticky_offset, 0, [$sticky]);
					// Cleanup
	...
	if (((int)$bbp->topic_query->post_count || (int)$bbp->topic_query->found_posts) && (int)$bbp->topic_query->posts_per_page) {
		...
		$bbp_topic_pagination = apply_filters('bbp_topic_pagination', [
			'base'      => $base,
			'format'    => '',
			'total'     => $r['posts_per_page'] === $bbp->topic_query->found_posts ? 1
		...
		$bbp->topic_query->pagination_links = str_replace($wp_rewrite->pagination_base . "/1/'", "'",
		                                                  $bbp->topic_query->pagination_links);
	}
...
function bbp_get_topic_pagination($args = '') {
	...
	if ($wp_rewrite->using_permalinks()) {
		$base
			= trailingslashit(get_permalink($r['topic_id'])) . user_trailingslashit($wp_rewrite->pagination_base . '/%#%/');
	} else {
	...
	$pagination = [
		...
		'mid_size'  => 2,
		'end_size'  => 3,
		'add_args'  => (bbp_get_view_all()) ? ['view' => 'all'] : FALSE
	];
	// Add pagination to query object
...
function bbp_topic_content_append_revisions($content = '', $topic_id = 0) {
	...
	return apply_filters('bbp_topic_append_revisions', $content . bbp_get_topic_revision_log($topic_id), $content,
	                     $topic_id);
}
...
function bbp_get_topic_revision_log($topic_id = 0) {
	...
	foreach ((array)$revisions as $revision) {
		...
		$author
			   = bbp_get_author_link(['size' => 14, 'link_text' => bbp_get_topic_author_display_name($revision->ID), 'post_id' => $revision->ID]);
		$since = bbp_get_time_since(bbp_convert_date($revision->post_modified));
		$r .= "\t" . '<li id="bbp-topic-revision-log-' . esc_attr($topic_id) . '-item-' . esc_attr($revision->ID) . '" class="bbp-topic-revision-log-item">' . "\n";
		if (!empty($reason)) {
			$r .= "\t\t" . sprintf(__('This topic was modified %1$s by %2$s. Reason: %3$s', 'bbpress'),
			                       esc_html($since), $author, esc_html($reason)) . "\n";
		} else {
			$r .= "\t\t" . sprintf(__('This topic was modified %1$s by %2$s.', 'bbpress'), esc_html($since),
			                       $author) . "\n";
		}
		$r .= "\t" . '</li>' . "\n";
	}
...
function bbp_get_topic_revisions($topic_id = 0) {
	$topic_id  = bbp_get_topic_id($topic_id);
	$revisions = wp_get_post_revisions($topic_id, ['order' => 'ASC']);
...
function bbp_get_topic_author_link($args = '') {
	...
	if (!empty($topic_id)) {
		...
		if (empty($r['link_title'])) {
			$link_title = sprintf(empty($anonymous) ? __('View %s\'s profile', 'bbpress')
		...
		if (empty($anonymous) && bbp_user_has_profile(bbp_get_topic_author_id($topic_id))) {
			...
			foreach ($author_links as $link => $link_text) {
				...
				$author_link[] = sprintf('<a href="%1$s"%2$s%3$s>%4$s</a>', esc_url($author_url), $link_title,
 ...
 */
function bbp_topic_author_role($args = []) {
 ...
 */
function bbp_topic_subscription_link($args = []) {
 ...
 */
function bbp_get_topic_subscription_link($args = []) {
	// No link
	$retval = FALSE;
	// Parse the arguments
	$r = bbp_parse_args($args, [
 ...
 */
function bbp_topic_favorite_link($args = []) {
 ...
 */
function bbp_get_topic_favorite_link($args = []) {
	// No link
	$retval = FALSE;
	// Parse the arguments
	$r = bbp_parse_args($args, [
...
function bbp_get_topic_last_reply_permalink($topic_id = 0) {
	...
	return apply_filters('bbp_get_topic_last_reply_permalink',
	                     bbp_get_reply_permalink(bbp_get_topic_last_reply_id($topic_id)));
}
...
function bbp_get_topic_freshness_link($topic_id = 0) {
	...
	if (!empty($time_since)) {
		$anchor
			= '<a href="' . esc_url($link_url) . '" title="' . esc_attr($title) . '">' . esc_html($time_since) . '</a>';
	} else {
...
function bbp_get_topic_replies_link($topic_id = 0) {
	...
	$topic_id = $topic->ID;
	$replies  = sprintf(_n('%s reply', '%s replies', bbp_get_topic_reply_count($topic_id, TRUE), 'bbpress'),
	                    bbp_get_topic_reply_count($topic_id));
	$retval   = '';
	...
	if (!empty($deleted) && current_user_can('edit_others_replies')) {
		...
		if (bbp_get_view_all()) {
			...
			$retval .= " <a href='" . esc_url(bbp_add_view_all(bbp_get_topic_permalink($topic_id),
			                                                   TRUE)) . "'>$extra</a>";
		}
	}
 ...
 */
function bbp_topic_class($topic_id = 0, $classes = []) {
 ...
 */
function bbp_get_topic_class($topic_id = 0, $classes = []) {
	$bbp       = bbpress();
	$topic_id  = bbp_get_topic_id($topic_id);
...
function bbp_get_topic_admin_links($args = []) {
	...
	if (in_array($topic_status, [bbp_get_spam_status_id(), bbp_get_trash_status_id()])) {
...
function bbp_get_topic_edit_link($args = '') {
	...
	$retval
		= $r['link_before'] . '<a href="' . esc_url($uri) . '" class="bbp-topic-edit-link">' . $r['edit_text'] . '</a>' . $r['link_after'];
...
function bbp_get_topic_edit_url($topic_id = 0) {
	...
	if ($wp_rewrite->using_permalinks()) {
		...
		$url = add_query_arg([bbp_get_topic_post_type() => $topic->post_name, $bbp->edit_id => '1'], $topic_link);
	}
	// Maybe add view=all
...
function bbp_get_topic_trash_link($args = '') {
	...
	if (bbp_is_topic_trash($topic->ID)) {
		$actions['untrash'] = '<a title="' . esc_attr__('Restore this item from the Trash',
		                                                'bbpress') . '" href="' . esc_url(wp_nonce_url(add_query_arg(['action' => 'bbp_toggle_topic_trash', 'sub_action' => 'untrash', 'topic_id' => $topic->ID]),
		                                                                                               'untrash-' . $topic->post_type . '_' . $topic->ID)) . '" class="bbp-topic-restore-link">' . $r['restore_text'] . '</a>';
	} elseif (EMPTY_TRASH_DAYS) {
		$actions['trash'] = '<a title="' . esc_attr__('Move this item to the Trash',
		                                              'bbpress') . '" href="' . esc_url(wp_nonce_url(add_query_arg(['action' => 'bbp_toggle_topic_trash', 'sub_action' => 'trash', 'topic_id' => $topic->ID]),
		                                                                                             'trash-' . $topic->post_type . '_' . $topic->ID)) . '" class="bbp-topic-trash-link">' . $r['trash_text'] . '</a>';
	}
	if (bbp_is_topic_trash($topic->ID) || !EMPTY_TRASH_DAYS) {
		$actions['delete'] = '<a title="' . esc_attr__('Delete this item permanently',
		                                               'bbpress') . '" href="' . esc_url(wp_nonce_url(add_query_arg(['action' => 'bbp_toggle_topic_trash', 'sub_action' => 'delete', 'topic_id' => $topic->ID]),
		                                                                                              'delete-' . $topic->post_type . '_' . $topic->ID)) . '" onclick="return confirm(\'' . esc_js(__('Are you sure you want to delete that permanently?',
		                                                                                                                                                                                              'bbpress')) . '\' );" class="bbp-topic-delete-link">' . $r['delete_text'] . '</a>';
	}
	// Process the admin links
...
function bbp_get_topic_close_link($args = '') {
	...
	$uri     = add_query_arg(['action' => 'bbp_toggle_topic_close', 'topic_id' => $topic->ID]);
	$uri     = wp_nonce_url($uri, 'close-topic_' . $topic->ID);
	$retval
	         = $r['link_before'] . '<a href="' . esc_url($uri) . '" class="bbp-topic-close-link">' . $display . '</a>' . $r['link_after'];
...
function bbp_get_topic_merge_link($args = '') {
	...
	$uri = add_query_arg(['action' => 'merge'], bbp_get_topic_edit_url($topic->ID));
	$retval
	     = $r['link_before'] . '<a href="' . esc_url($uri) . '" class="bbp-topic-merge-link">' . $r['merge_text'] . '</a>' . $r['link_after'];
...
function bbp_get_topic_spam_link($args = '') {
	...
	$uri     = add_query_arg(['action' => 'bbp_toggle_topic_spam', 'topic_id' => $topic->ID]);
	$uri     = wp_nonce_url($uri, 'spam-topic_' . $topic->ID);
	$retval
	         = $r['link_before'] . '<a href="' . esc_url($uri) . '" class="bbp-topic-spam-link">' . $display . '</a>' . $r['link_after'];
 ...
 */
function bbp_topic_reply_link($args = []) {
 ...
 */
function bbp_get_topic_reply_link($args = []) {
	// Parse arguments against default values
	$r = bbp_parse_args($args, [
		'id'          => 0,
		'link_before' => '',
	...
	$r['uri'] = $uri;
	$retval
	          = $r['link_before'] . '<a href="' . esc_url($r['uri']) . '" class="bbp-topic-reply-link">' . $r['reply_text'] . '</a>' . $r['link_after'];
...
function bbp_get_forum_pagination_count() {
	...
	$to_num
	           = bbp_number_format(($start_num + ($bbp->topic_query->posts_per_page - 1) > $bbp->topic_query->found_posts)
		                               ? $bbp->topic_query->found_posts
		                               : $start_num + ($bbp->topic_query->posts_per_page - 1));
	$total_int = (int)!empty($bbp->topic_query->found_posts) ? $bbp->topic_query->found_posts
		: $bbp->topic_query->post_count;
	...
	if (empty($to_num)) {
		...
		$retstr = sprintf(_n('Viewing topic %2$s (of %4$s total)',
		                     'Viewing %1$s topics - %2$s through %3$s (of %4$s total)', $total_int, 'bbpress'),
		                  $bbp->topic_query->post_count, $from_num, $to_num, $total);
	}
	...
	<select name="<?php echo esc_attr($r['select_id']); ?>"
		...
		<?php foreach (bbp_get_topic_types() as $key => $label) : ?>
			...
			<option value="<?php echo esc_attr($key); ?>"<?php selected($key,
			                                                            $r['selected']); ?>><?php echo esc_html($label); ?></option>
...
function bbp_get_single_topic_description($args = '') {
	...
	if (!empty($last_reply)) {
		$last_updated_by = bbp_get_author_link(['post_id' => $last_reply, 'size' => $r['size']]);
		$retstr          = sprintf(esc_html__('This topic contains %1$s, has %2$s, and was last updated by %3$s %4$s.',
		                                      'bbpress'), $reply_count, $voice_count, $last_updated_by, $time_since);
...
function bbp_get_topic_tag_tax_labels() {
	return apply_filters('bbp_get_topic_tag_tax_labels', [
...
function bbp_get_topic_tag_tax_rewrite() {
	return apply_filters('bbp_get_topic_tag_tax_rewrite', [
...
function bbp_get_topic_tag_edit_link($tag = '') {
	...
	if (!empty($term->term_id)) {
		...
		if ($wp_rewrite->using_permalinks()) {
			...
			$retval = add_query_arg([$bbp->edit_id => '1'], bbp_get_topic_tag_link());
		}
		// No link
	} else {
		$retval = '';
 ...
 */
function bbp_topic_tag_description($args = []) {
 ...
 * @return string Term Name
 */
function bbp_get_topic_tag_description($args = []) {
	// Parse arguments against default values
	$r = bbp_parse_args($args, [
