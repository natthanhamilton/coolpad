 ...
 */
function bbp_insert_topic($topic_data = [], $topic_meta = []) {
	// Parse arguments against default values
	$topic_data = bbp_parse_args($topic_data, [
	...
	// Parse arguments against default values
	$topic_meta = bbp_parse_args($topic_meta, [
	...
	if (!empty($forum_id)) {
		bbp_update_forum(['forum_id' => $forum_id]);
	}
...
function bbp_new_topic_handler($action = '') {
	...
	if (!bbp_verify_nonce_request('bbp-new-topic')) {
		bbp_add_error('bbp_new_topic_nonce',
		              __('<strong>ERROR</strong>: Are you sure you wanted to do that?', 'bbpress'));
	...
	if (bbp_is_anonymous()) {
		...
		if (!current_user_can('publish_topics')) {
			bbp_add_error('bbp_topic_permissions',
			              __('<strong>ERROR</strong>: You do not have permission to create new topics.', 'bbpress'));
	...
	if (isset($_POST['bbp_forum_id'])) {
		...
		if (empty($_POST['bbp_forum_id'])) {
			...
			if (0 === $posted_forum_id) {
				...
				bbp_add_error('bbp_topic_forum_id',
				              __('<strong>ERROR</strong>: Forum ID cannot be a negative number.', 'bbpress'));
	...
	if (!empty($forum_id)) {
		...
		if (bbp_is_forum_category($forum_id)) {
			bbp_add_error('bbp_new_topic_forum_category',
			              __('<strong>ERROR</strong>: This forum is a category. No topics can be created in this forum.',
			                 'bbpress'));
			// Forum is not a category
		} else {
			// Forum is closed and user cannot access
			if (bbp_is_forum_closed($forum_id) && !current_user_can('edit_forum', $forum_id)) {
				bbp_add_error('bbp_new_topic_forum_closed',
				              __('<strong>ERROR</strong>: This forum has been closed to new topics.', 'bbpress'));
			}
			// Forum is private and user cannot access
			if (bbp_is_forum_private($forum_id)) {
				if (!current_user_can('read_private_forums')) {
					bbp_add_error('bbp_new_topic_forum_private',
					              __('<strong>ERROR</strong>: This forum is private and you do not have the capability to read or create new topics in it.',
					                 'bbpress'));
				}
				// Forum is hidden and user cannot access
			} elseif (bbp_is_forum_hidden($forum_id)) {
				if (!current_user_can('read_hidden_forums')) {
					bbp_add_error('bbp_new_topic_forum_hidden',
					              __('<strong>ERROR</strong>: This forum is hidden and you do not have the capability to read or create new topics in it.',
					                 'bbpress'));
				}
			}
		}
	...
	/** Topic Duplicate *******************************************************/
	if (!bbp_check_for_duplicate(['post_type' => bbp_get_topic_post_type(), 'post_author' => $topic_author, 'post_content' => $topic_content, 'anonymous_data' => $anonymous_data])) {
		bbp_add_error('bbp_topic_duplicate',
		              __('<strong>ERROR</strong>: Duplicate topic detected; it looks as though you&#8217;ve already said that!',
		                 'bbpress'));
	}
	/** Topic Blacklist *******************************************************/
	if (!bbp_check_for_blacklist($anonymous_data, $topic_author, $topic_title, $topic_content)) {
		bbp_add_error('bbp_topic_blacklist',
		              __('<strong>ERROR</strong>: Your topic cannot be created at this time.', 'bbpress'));
	}
	/** Topic Status **********************************************************/
	...
	} elseif (!empty($_POST['bbp_topic_status']) && in_array($_POST['bbp_topic_status'],
	...
	if (bbp_allow_topic_tags() && !empty($_POST['bbp_topic_tags'])) {
		...
		$terms = [bbp_get_topic_tag_tax_id() => $terms];
	}
	...
	$topic_data = apply_filters('bbp_new_topic_pre_insert', [
	...
	if (!empty($topic_id) && !is_wp_error($topic_id)) {
		...
		if ((get_post_field('post_status',
		                    $forum_id) === bbp_get_trash_status_id()) || ($topic_data['post_status'] === bbp_get_trash_status_id())
		) {
			// Trash the reply
		...
		if (!empty($_POST['bbp_stick_topic']) && in_array($_POST['bbp_stick_topic'], ['stick', 'super', 'unstick'])) {
			// What's the caps?
		...
		$append_error = (is_wp_error($topic_id) && $topic_id->get_error_message())
			? $topic_id->get_error_message() . ' ' : '';
		bbp_add_error('bbp_topic_error',
		              __('<strong>ERROR</strong>: The following problem(s) have been found with your topic:' . $append_error,
		                 'bbpress'));
	}
}
...
function bbp_edit_topic_handler($action = '') {
	...
	if (empty($topic)) {
		bbp_add_error('bbp_edit_topic_not_found',
		              __('<strong>ERROR</strong>: The topic you want to edit was not found.', 'bbpress'));
		...
		if (!bbp_is_topic_anonymous($topic_id)) {
			...
			if (!current_user_can('edit_topic', $topic_id)) {
				bbp_add_error('bbp_edit_topic_permissions',
				              __('<strong>ERROR</strong>: You do not have permission to edit that topic.', 'bbpress'));
			}
			// Set topic author
			...
			// Filter anonymous data
			$anonymous_data = bbp_filter_anonymous_post_data([], TRUE);
		}
	}
	// Nonce check
	if (!bbp_verify_nonce_request('bbp-edit-topic_' . $topic_id)) {
		bbp_add_error('bbp_edit_topic_nonce',
		              __('<strong>ERROR</strong>: Are you sure you wanted to do that?', 'bbpress'));
	...
	if (!empty($forum_id) && ($forum_id !== $current_forum_id)) {
		// Forum is a category
		if (bbp_is_forum_category($forum_id)) {
			bbp_add_error('bbp_edit_topic_forum_category',
			              __('<strong>ERROR</strong>: This forum is a category. No topics can be created in it.',
			                 'bbpress'));
			// Forum is not a category
		} else {
			// Forum is closed and user cannot access
			if (bbp_is_forum_closed($forum_id) && !current_user_can('edit_forum', $forum_id)) {
				bbp_add_error('bbp_edit_topic_forum_closed',
				              __('<strong>ERROR</strong>: This forum has been closed to new topics.', 'bbpress'));
			}
			// Forum is private and user cannot access
			if (bbp_is_forum_private($forum_id)) {
				if (!current_user_can('read_private_forums')) {
					bbp_add_error('bbp_edit_topic_forum_private',
					              __('<strong>ERROR</strong>: This forum is private and you do not have the capability to read or create new topics in it.',
					                 'bbpress'));
				}
				// Forum is hidden and user cannot access
	...
	if (!bbp_check_for_blacklist($anonymous_data, $topic_author, $topic_title, $topic_content)) {
		bbp_add_error('bbp_topic_blacklist',
		              __('<strong>ERROR</strong>: Your topic cannot be edited at this time.', 'bbpress'));
	}
	/** Topic Status **********************************************************/
	// Maybe put into moderation
	if (!bbp_check_for_moderation($anonymous_data, $topic_author, $topic_title, $topic_content)) {
		// Set post status to pending if public or closed
		if (in_array($topic->post_status, [bbp_get_public_status_id(), bbp_get_closed_status_id()])) {
			$topic_status = bbp_get_pending_status_id();
		}
		// Check a whitelist of possible topic status ID's
	} elseif (!empty($_POST['bbp_topic_status']) && in_array($_POST['bbp_topic_status'],
	...
	if (bbp_allow_topic_tags() && current_user_can('assign_topic_tags') && !empty($_POST['bbp_topic_tags'])) {
		...
		$terms = [bbp_get_topic_tag_tax_id() => explode(',', bbp_get_topic_tag_names($topic_id, ','))];
	}
	/** Additional Actions (Before Save) **************************************/
	...
	$topic_data = apply_filters('bbp_edit_topic_pre_insert', [
		'ID'           => $topic_id,
		'post_title'   => $topic_title,
		'post_content' => $topic_content,
		'post_status'  => $topic_status,
	...
	if (!empty($topic_id) && !is_wp_error($topic_id)) {
		...
		if (!empty($_POST['bbp_stick_topic']) && in_array($_POST['bbp_stick_topic'],
		                                                  array_keys(bbp_get_topic_types()))
		) {
			// What's the caps?
			if (current_user_can('moderate')) {
				// What's the haps?
		...
		$append_error = (is_wp_error($topic_id) && $topic_id->get_error_message())
			? $topic_id->get_error_message() . ' ' : '';
		bbp_add_error('bbp_topic_error',
		              __('<strong>ERROR</strong>: The following problem(s) have been found with your topic:' . $append_error . 'Please try again.',
		                 'bbpress'));
	}
}
...
function bbp_update_topic($topic_id = 0, $forum_id = 0, $anonymous_data = FALSE, $author_id = 0, $is_edit = FALSE) {
	...
	if (!empty($anonymous_data) && is_array($anonymous_data)) {
		// Parse arguments against default values
		$r = bbp_parse_args($anonymous_data, [
			'bbp_anonymous_name'    => '',
			'bbp_anonymous_email'   => '',
	...
	if (bbp_is_subscriptions_active() && !empty($author_id)) {
		...
		$subscheck
		            = (!empty($_POST['bbp_topic_subscription']) && ('bbp_subscribe' === $_POST['bbp_topic_subscription']))
			? TRUE : FALSE;
...
function bbp_update_topic_walker($topic_id, $last_active_time = '', $forum_id = 0, $reply_id = 0, $refresh = TRUE) {
	...
	// Get topic ancestors
	$ancestors = array_values(array_unique(array_merge([$forum_id], (array)get_post_ancestors($topic_id))));
	// Topic status
...
function bbp_move_topic_handler($topic_id, $old_forum_id, $new_forum_id) {
	...
	/** Old forum_id **********************************************************/
	// Get topic ancestors
	$old_forum_ancestors = array_values(array_unique(array_merge([$old_forum_id],
	                                                             (array)get_post_ancestors($old_forum_id))));
	...
	if (!in_array($new_forum_id, $old_forum_ancestors)) {
		// Get topic ancestors
		$new_forum_ancestors = array_values(array_unique(array_merge([$new_forum_id],
		                                                             (array)get_post_ancestors($new_forum_id))));
		// Make sure we're not walking twice
...
function bbp_merge_topic_handler($action = '') {
	...
	$subscribers     = $favoriters = $replies = [];
	/** Source Topic **********************************************************/
	// Topic id
	if (empty($_POST['bbp_topic_id'])) {
	...
	if (!bbp_verify_nonce_request('bbp-merge-topic_' . $source_topic_id)) {
		bbp_add_error('bbp_merge_topic_nonce',
		              __('<strong>ERROR</strong>: Are you sure you wanted to do that?', 'bbpress'));

		return;
		// Source topic not found
	} elseif (!$source_topic = bbp_get_topic($source_topic_id)) {
		bbp_add_error('bbp_merge_topic_source_not_found',
		              __('<strong>ERROR</strong>: The topic you want to merge was not found.', 'bbpress'));

		return;
	}
	// Cannot edit source topic
	if (!current_user_can('edit_topic', $source_topic->ID)) {
		bbp_add_error('bbp_merge_topic_source_permission',
		              __('<strong>ERROR</strong>: You do not have the permissions to edit the source topic.',
		                 'bbpress'));
	...
	if (empty($_POST['bbp_destination_topic'])) {
		bbp_add_error('bbp_merge_topic_destination_id',
		              __('<strong>ERROR</strong>: Destination topic ID not found.', 'bbpress'));
	} else {
	...
	if (!$destination_topic = bbp_get_topic($destination_topic_id)) {
		bbp_add_error('bbp_merge_topic_destination_not_found',
		              __('<strong>ERROR</strong>: The topic you want to merge to was not found.', 'bbpress'));
	}
	// Cannot edit destination topic
	if (!current_user_can('edit_topic', $destination_topic->ID)) {
		bbp_add_error('bbp_merge_topic_destination_permission',
		              __('<strong>ERROR</strong>: You do not have the permissions to edit the destination topic.',
		                 'bbpress'));
	}
	// Bail if errors
	if (bbp_has_errors()) {
	...
	$source_topic_tags = wp_get_post_terms($source_topic->ID, bbp_get_topic_tag_tax_id(), ['fields' => 'names']);
...
function bbp_split_topic_handler($action = '') {
	...
	if (empty($_POST['bbp_reply_id'])) {
		bbp_add_error('bbp_split_topic_reply_id',
		              __('<strong>ERROR</strong>: Reply ID to split the topic from not found!', 'bbpress'));
	} else {
	...
	// Reply exists
	if (empty($from_reply)) {
		bbp_add_error('bbp_split_topic_r_not_found',
		              __('<strong>ERROR</strong>: The reply you want to split from was not found.', 'bbpress'));
	}
	/** Topic to Split ********************************************************/
	...
	if (empty($source_topic)) {
		bbp_add_error('bbp_split_topic_source_not_found',
		              __('<strong>ERROR</strong>: The topic you want to split was not found.', 'bbpress'));
	}
	// Nonce check failed
	if (!bbp_verify_nonce_request('bbp-split-topic_' . $source_topic->ID)) {
		bbp_add_error('bbp_split_topic_nonce',
		              __('<strong>ERROR</strong>: Are you sure you wanted to do that?', 'bbpress'));
	...
	if (!current_user_can('edit_topic', $source_topic->ID)) {
		bbp_add_error('bbp_split_topic_source_permission',
		              __('<strong>ERROR</strong>: You do not have the permissions to edit the source topic.',
		                 'bbpress'));
	}
	// How to Split
	...
	if (empty($split_option) || !in_array($split_option, ['existing', 'reply'])) {
		bbp_add_error('bbp_split_topic_option',
		              __('<strong>ERROR</strong>: You need to choose a valid split option.', 'bbpress'));
		// Valid Split Option
	} else {
		// What kind of split
		switch ($split_option) {
			...
			case 'existing' :
				...
				if (empty($destination_topic)) {
					bbp_add_error('bbp_split_topic_destination_not_found',
					              __('<strong>ERROR</strong>: The topic you want to split to was not found!',
					                 'bbpress'));
				}
				// User cannot edit the destination topic
				if (!current_user_can('edit_topic', $destination_topic->ID)) {
					bbp_add_error('bbp_split_topic_destination_permission',
					              __('<strong>ERROR</strong>: You do not have the permissions to edit the destination topic!',
					                 'bbpress'));
				}
				break;
			...
			default :
				...
				if (current_user_can('publish_topics')) {
					...
					if (FALSE === $destination_topic_id || is_wp_error($destination_topic_id) || empty($destination_topic)) {
						bbp_add_error('bbp_split_topic_destination_reply',
						              __('<strong>ERROR</strong>: There was a problem converting the reply into the topic. Please try again.',
						                 'bbpress'));
					}
					// User cannot publish posts
				} else {
					bbp_add_error('bbp_split_topic_destination_permission',
					              __('<strong>ERROR</strong>: You do not have the permissions to create new topics. The reply could not be converted into a topic.',
					                 'bbpress'));
				}
				break;
		}
	...
	$replies
		= (array)$wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->posts} WHERE {$wpdb->posts}.post_date >= %s AND {$wpdb->posts}.post_parent = %d AND {$wpdb->posts}.post_type = %s ORDER BY {$wpdb->posts}.post_date ASC",
		                                           $from_reply->post_date, $source_topic->ID,
...
function bbp_edit_topic_tag_handler($action = '') {
	...
	if (is_wp_error($tag) && $tag->get_error_message()) {
		bbp_add_error('bbp_manage_topic_invalid_tag',
		              sprintf(__('<strong>ERROR</strong>: The following problem(s) have been found while getting the tag: %s',
		                         'bbpress'), $tag->get_error_message()));
	...
	switch ($action) {
		...
		case 'bbp-update-topic-tag' :
			...
			if (!bbp_verify_nonce_request('update-tag_' . $tag_id)) {
				bbp_add_error('bbp_manage_topic_tag_update_nonce',
				              __('<strong>ERROR</strong>: Are you sure you wanted to do that?', 'bbpress'));
			...
			if (!current_user_can('edit_topic_tags')) {
				bbp_add_error('bbp_manage_topic_tag_update_permissions',
				              __('<strong>ERROR</strong>: You do not have the permissions to edit the topic tags.',
				                 'bbpress'));
			...
			if (empty($_POST['tag-name']) || !$name = $_POST['tag-name']) {
				bbp_add_error('bbp_manage_topic_tag_update_name',
				              __('<strong>ERROR</strong>: You need to enter a tag name.', 'bbpress'));

				return;
			}
			// Attempt to update the tag
			$slug = !empty($_POST['tag-slug']) ? $_POST['tag-slug'] : '';
			$tag  = wp_update_term($tag_id, bbp_get_topic_tag_tax_id(), ['name' => $name, 'slug' => $slug]);
			// Cannot update tag
			if (is_wp_error($tag) && $tag->get_error_message()) {
				bbp_add_error('bbp_manage_topic_tag_update_error',
				              sprintf(__('<strong>ERROR</strong>: The following problem(s) have been found while updating the tag: %s',
				                         'bbpress'), $tag->get_error_message()));
		...
		case 'bbp-merge-topic-tag'  :
			...
			if (!bbp_verify_nonce_request('merge-tag_' . $tag_id)) {
				bbp_add_error('bbp_manage_topic_tag_merge_nonce',
				              __('<strong>ERROR</strong>: Are you sure you wanted to do that?', 'bbpress'));
			...
			// Can user edit topic tags?
			if (!current_user_can('edit_topic_tags')) {
				bbp_add_error('bbp_manage_topic_tag_merge_permissions',
				              __('<strong>ERROR</strong>: You do not have the permissions to edit the topic tags.',
				                 'bbpress'));
			...
			if (empty($_POST['tag-existing-name']) || !$name = $_POST['tag-existing-name']) {
				bbp_add_error('bbp_manage_topic_tag_merge_name',
				              __('<strong>ERROR</strong>: You need to enter a tag name.', 'bbpress'));
			...
			if (is_wp_error($tag) && $tag->get_error_message()) {
				bbp_add_error('bbp_manage_topic_tag_merge_error',
				              sprintf(__('<strong>ERROR</strong>: The following problem(s) have been found while merging the tags: %s',
				                         'bbpress'), $tag->get_error_message()));
			...
			if ($tag_id === $to_tag) {
				bbp_add_error('bbp_manage_topic_tag_merge_same',
				              __('<strong>ERROR</strong>: The tags which are being merged can not be the same.',
				                 'bbpress'));

				return;
			}
			// Delete the old term
			$tag = wp_delete_term($tag_id, bbp_get_topic_tag_tax_id(), ['default' => $to_tag, 'force_default' => TRUE]);
		...
		case 'bbp-delete-topic-tag' :
			...
			if (!bbp_verify_nonce_request('delete-tag_' . $tag_id)) {
				bbp_add_error('bbp_manage_topic_tag_delete_nonce',
				              __('<strong>ERROR</strong>: Are you sure you wanted to do that?', 'bbpress'));
			...
			if (!current_user_can('delete_topic_tags')) {
				bbp_add_error('bbp_manage_topic_tag_delete_permissions',
				              __('<strong>ERROR</strong>: You do not have the permissions to delete the topic tags.',
				                 'bbpress'));
			...
			// Error deleting term
			if (is_wp_error($tag) && $tag->get_error_message()) {
				bbp_add_error('bbp_manage_topic_tag_delete_error',
				              sprintf(__('<strong>ERROR</strong>: The following problem(s) have been found while deleting the tag: %s',
				                         'bbpress'), $tag->get_error_message()));
...
function bbp_get_topic_statuses() {
	return apply_filters('bbp_get_topic_statuses', [
...
function bbp_get_topic_types() {
	return apply_filters('bbp_get_topic_types', [
...
function bbp_get_stickies($forum_id = 0) {
	...
	$stickies = (empty($stickies) || !is_array($stickies)) ? [] : $stickies;
...
function bbp_toggle_topic_handler($action = '') {
	...
	$success   = FALSE;                      // Flag
	$post_data = ['ID' => $topic_id]; // Prelim array
	...
	if (!current_user_can('edit_topic',
	                      $topic->ID) || ('bbp_toggle_topic_trash' === $action && !current_user_can('delete_topic',
	                                                                                                $topic->ID))
	) {
		bbp_add_error('bbp_toggle_topic_permission',
		              __('<strong>ERROR:</strong> You do not have the permission to do that.', 'bbpress'));
	...
	switch ($action) {
		...
		case 'bbp_toggle_topic_close' :
			...
			$failure = TRUE === $is_open ? __('<strong>ERROR</strong>: There was a problem closing the topic.',
			                                  'bbpress')
		...
		case 'bbp_toggle_topic_stick' :
			...
			$failure   = TRUE === $is_sticky ? __('<strong>ERROR</strong>: There was a problem unsticking the topic.',
			                                      'bbpress')
		...
		case 'bbp_toggle_topic_trash' :
			$sub_action = !empty($_GET['sub_action']) && in_array($_GET['sub_action'], ['trash', 'untrash', 'delete'])
				? $_GET['sub_action'] : FALSE;
			if (empty($sub_action)) {
				break;
			}
			switch ($sub_action) {
...
function bbp_update_topic_reply_count_hidden($topic_id = 0, $reply_count = 0) {
	...
	if (empty($reply_count)) {
		$post_status = "'" . implode("','", [bbp_get_trash_status_id(), bbp_get_spam_status_id()]) . "'";
		$reply_count
		             = $wpdb->get_var($wpdb->prepare("SELECT COUNT(ID) FROM {$wpdb->posts} WHERE post_parent = %d AND post_status IN ( {$post_status} ) AND post_type = '%s';",
...
function bbp_update_topic_voice_count($topic_id = 0) {
	...
	$voices
		= $wpdb->get_col($wpdb->prepare("SELECT COUNT( DISTINCT post_author ) FROM {$wpdb->posts} WHERE ( post_parent = %d AND post_status = '%s' AND post_type = '%s' ) OR ( ID = %d AND post_type = '%s' );",
		                                $topic_id, bbp_get_public_status_id(), bbp_get_reply_post_type(), $topic_id,
...
function bbp_update_topic_anonymous_reply_count($topic_id = 0) {
	...
	$anonymous_replies
		= (int)$wpdb->get_var($wpdb->prepare("SELECT COUNT( ID ) FROM {$wpdb->posts} WHERE ( post_parent = %d AND post_status = '%s' AND post_type = '%s' AND post_author = 0 ) OR ( ID = %d AND post_type = '%s' AND post_author = 0 );",
...
function bbp_update_topic_revision_log($args = '') {
	...
	$revision_log[ $r['revision_id'] ] = ['author' => $r['author_id'], 'reason' => $r['reason']];
...
function bbp_spam_topic($topic_id = 0) {
	...
	if (!empty($terms)) {
		...
		if (!empty($term_names)) {
			...
			$topic->tax_input = [bbp_get_topic_tag_tax_id() => ''];
		}
	}
...
function bbp_unspam_topic($topic_id = 0) {
	...
	if (!empty($terms)) {
		...
		$topic->tax_input = [bbp_get_topic_tag_tax_id() => $terms];
...
function bbp_stick_topic($topic_id = 0, $super = FALSE) {
	...
	$success  = !empty($super) ? update_option('_bbp_super_sticky_topics', $stickies)
...
function bbp_unstick_topic($topic_id = 0) {
	...
	if (empty($stickies)) {
		...
		if (empty($stickies)) {
			$success = !empty($super) ? delete_option('_bbp_super_sticky_topics')
...
function bbp_topic_content_autoembed() {
	...
	if (bbp_use_autoembed() && is_a($wp_embed, 'WP_Embed')) {
		add_filter('bbp_get_topic_content', [$wp_embed, 'autoembed'], 2);
	}
}
 ...
 */
function bbp_display_topics_feed_rss2($topics_query = []) {
	// User cannot access this forum
	if (bbp_is_single_forum() && !bbp_user_can_view_forum(['forum_id' => bbp_get_forum_id()])) {
		return;
	}
	// Display the feed
				...
				<?php while (bbp_topics()) : bbp_the_topic(); ?>
					...
					<item>
						...
						<pubDate><?php echo mysql2date('D, d M Y H:i:s +0000',
						                               get_post_meta(bbp_get_topic_id(), '_bbp_last_active_time',
						                                             TRUE)); ?></pubDate>
						...
						<?php if (!post_password_required()) : ?>
							...
							<description>
								<![CDATA[
								<p><?php printf(esc_html__('Replies: %s', 'bbpress'),
								                bbp_get_topic_reply_count()); ?></p>
