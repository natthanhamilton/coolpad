...
if (!class_exists('BBP_Shortcodes')) :
	...
	class BBP_Shortcodes {
		...
		private function setup_globals() {
			...
			$this->codes = apply_filters('bbp_shortcodes', [
				/** Forums ********************************************************/
				'bbp-forum-index'  => [$this, 'display_forum_index'], // Forum Index
				'bbp-forum-form'   => [$this, 'display_forum_form'], // Topic form
				'bbp-single-forum' => [$this, 'display_forum'], // Specific forum - pass an 'id' attribute
				/** Topics ********************************************************/
				'bbp-topic-index'  => [$this, 'display_topic_index'], // Topic index
				'bbp-topic-form'   => [$this, 'display_topic_form'], // Topic form
				'bbp-single-topic' => [$this, 'display_topic'], // Specific topic - pass an 'id' attribute
				/** Topic Tags ****************************************************/
				'bbp-topic-tags' => [$this, 'display_topic_tags'], // All topic tags in a cloud
				'bbp-single-tag' => [$this, 'display_topics_of_tag'], // Topics of Tag
				/** Replies *******************************************************/
				'bbp-reply-form'   => [$this, 'display_reply_form'], // Reply form
				'bbp-single-reply' => [$this, 'display_reply'], // Specific reply - pass an 'id' attribute
				/** Views *********************************************************/
				'bbp-single-view' => [$this, 'display_view'], // Single view
				/** Search ********************************************************/
				'bbp-search-form' => [$this, 'display_search_form'], // Search form
				'bbp-search'      => [$this, 'display_search'], // Search
				/** Account *******************************************************/
				'bbp-login'     => [$this, 'display_login'], // Login
				'bbp-register'  => [$this, 'display_register'], // Register
				'bbp-lost-pass' => [$this, 'display_lost_pass'], // Lost Password
				/** Others *******************************************************/
				'bbp-stats' => [$this, 'display_stats'], // Stats
			]);
		}
		...
		public function display_forum($attr, $content = '') {
			...
			// Check forum caps
			if (bbp_user_can_view_forum(['forum_id' => $forum_id])) {
		...
		public function display_topic_form($attr = [], $content = '') {
			...
			if (empty($forum_id) || bbp_user_can_view_forum(['forum_id' => $forum_id])) {
		...
		public function display_topics_of_tag($attr, $content = '') {
			...
			// Filter the query
			if (!bbp_is_topic_tag()) {
				add_filter('bbp_before_has_topics_parse_args', [$this, 'display_topics_of_tag_query']);
			}
			// Start output buffer
		 ...
		 * @return array
		 */
		public function display_topic_index_query($args = []) {
			$args['author']        = 0;
		 ...
		 * @return array
		 */
		public function display_topics_of_tag_query($args = []) {
			$args['tax_query'] = [[
				                      'taxonomy' => bbp_get_topic_tag_tax_id(),
				                      'field'    => 'id',
