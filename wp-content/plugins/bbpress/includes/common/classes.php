...
if (!class_exists('BBP_Component')) :
	...
	class BBP_Component {
		...
		private function setup_actions() {
			add_action('bbp_register_post_types', [$this, 'register_post_types'], 10, 2); // Register post types
			add_action('bbp_register_taxonomies', [$this, 'register_taxonomies'], 10, 2); // Register taxonomies
			add_action('bbp_add_rewrite_tags', [$this, 'add_rewrite_tags'], 10, 2); // Add the rewrite tags
			add_action('bbp_generate_rewrite_rules', [$this, 'generate_rewrite_rules'], 10,
			           2); // Generate rewrite rules
...
if (class_exists('Walker')) :
	...
	class BBP_Walker_Dropdown extends Walker {
		 ...
		 * @var array
		 */
		var $db_fields = ['parent' => 'post_parent', 'id' => 'ID'];
		/** Methods ***************************************************************/
		/**
		 * Set the tree_type
		 *
		...
		public function start_el(&$output, $object, $depth = 0, $args = [], $current_object_id = 0) {
			...
			$output .= '>';
			$title = apply_filters('bbp_walker_dropdown_post_title', $object->post_title, $output, $object, $depth,
			                       $args);
	...
	class BBP_Walker_Reply extends Walker {
		 ...
		 * @since bbPress (r4944)
		 */
		public function display_element($element = FALSE, &$children_elements = [], $max_depth = 0, $depth = 0,
		                                $args = [], &$output = '') {
			if (empty($element)) {
				return;
			}
		 ...
		 * @since bbPress (r4944)
		 */
		public function end_el(&$output = '', $object = FALSE, $depth = 0, $args = []) {
			// Check for a callback and use it if specified
