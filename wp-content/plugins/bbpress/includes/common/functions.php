...
function bbp_number_format($number = 0, $decimals = FALSE, $dec_point = '.', $thousands_sep = ',') {
	...
	return apply_filters('bbp_number_format', number_format($number, $decimals, $dec_point, $thousands_sep), $number,
...
function bbp_get_time_since($older_date, $newer_date = FALSE, $gmt = FALSE) {
	...
	$chunks = [
		[60 * 60 * 24 * 365, __('year', 'bbpress'), __('years', 'bbpress')],
		[60 * 60 * 24 * 30, __('month', 'bbpress'), __('months', 'bbpress')],
		[60 * 60 * 24 * 7, __('week', 'bbpress'), __('weeks', 'bbpress')],
		[60 * 60 * 24, __('day', 'bbpress'), __('days', 'bbpress')],
		[60 * 60, __('hour', 'bbpress'), __('hours', 'bbpress')],
		[60, __('minute', 'bbpress'), __('minutes', 'bbpress')],
		[1, __('second', 'bbpress'), __('seconds', 'bbpress')]
	];
	if (!empty($older_date) && !is_numeric($older_date)) {
		$time_chunks = explode(':', str_replace(' ', ':', $older_date));
		$date_chunks = explode('-', str_replace(' ', '-', $older_date));
		$older_date  = gmmktime((int)$time_chunks[1], (int)$time_chunks[2], (int)$time_chunks[3], (int)$date_chunks[1],
	...
	if (0 > $since) {
		...
		if (!isset($chunks[ $i ])) {
			...
			if ($i + 2 < $j) {
				...
				// Add to output var
				if (0 != $count2) {
					$output .= (1 == $count2) ? _x(',', 'Separator in time since', 'bbpress') . ' 1 ' . $name2
						: _x(',', 'Separator in time since', 'bbpress') . ' ' . $count2 . ' ' . $chunks[ $i + 1 ][2];
				}
			}
...
function bbp_add_view_all($original_link = '', $force = FALSE) {
	...
	if (bbp_get_view_all() || !empty($force)) {
		$link = add_query_arg(['view' => 'all'], $original_link);
	} else {
		$link = $original_link;
	}
 ...
 * @return array Data
 */
function bbp_fix_post_author($data = [], $postarr = []) {
	...
	if (!in_array($data['post_type'], [bbp_get_topic_post_type(), bbp_get_reply_post_type()])) {
		return $data;
	}
...
function bbp_get_statistics($args = '') {
	...
	if (!empty($r['count_topics'])) {
		...
		if (current_user_can('read_private_topics') || current_user_can('edit_others_topics') || current_user_can('view_trash')) {
			...
			$topic_titles[] = !empty($topics['private']) ? sprintf(__('Private: %s', 'bbpress'),
			                                                       number_format_i18n($topics['private'])) : '';
			$topic_titles[] = !empty($topics['spammed']) ? sprintf(__('Spammed: %s', 'bbpress'),
			                                                       number_format_i18n($topics['spammed'])) : '';
			$topic_titles[] = !empty($topics['trashed']) ? sprintf(__('Trashed: %s', 'bbpress'),
			                                                       number_format_i18n($topics['trashed'])) : '';
	...
	if (!empty($r['count_replies'])) {
		...
		if (current_user_can('read_private_replies') || current_user_can('edit_others_replies') || current_user_can('view_trash')) {
			...
			$replies['private'] = (!empty($r['count_private_replies']) && current_user_can('read_private_replies'))
				? (int)$all_replies->{$private} : 0;
			// Spam
			$replies['spammed'] = (!empty($r['count_spammed_replies']) && current_user_can('edit_others_replies'))
				? (int)$all_replies->{$spam} : 0;
			// Trash
			$replies['trashed'] = (!empty($r['count_trashed_replies']) && current_user_can('view_trash'))
				? (int)$all_replies->{$trash} : 0;
			// Total hidden (private + spam + trash)
			$reply_count_hidden = $replies['private'] + $replies['spammed'] + $replies['trashed'];
			// Generate the hidden topic count's title attribute
			$reply_titles[] = !empty($replies['private']) ? sprintf(__('Private: %s', 'bbpress'),
			                                                        number_format_i18n($replies['private'])) : '';
			$reply_titles[] = !empty($replies['spammed']) ? sprintf(__('Spammed: %s', 'bbpress'),
			                                                        number_format_i18n($replies['spammed'])) : '';
			$reply_titles[] = !empty($replies['trashed']) ? sprintf(__('Trashed: %s', 'bbpress'),
			                                                        number_format_i18n($replies['trashed'])) : '';
...
function bbp_check_for_duplicate($post_data = []) {
	...
	$query
		= $wpdb->prepare("SELECT ID FROM {$wpdb->posts} {$join} WHERE post_type = %s AND post_status != %s AND post_author = %d AND post_content = %s {$where}",
		                 $r['post_type'], $r['post_status'], $r['post_author'], $r['post_content']);
 ...
 */
function bbp_notify_topic_subscribers($reply_id = 0, $topic_id = 0, $forum_id = 0, $anonymous_data = FALSE,
                                      $reply_author = 0) {
...
Login and visit the topic to unsubscribe from these emails.', 'bbpress'),
	...
	$subject = apply_filters('bbp_subscription_mail_title', '[' . $blog_name . '] ' . $topic_title, $reply_id,
	                         $topic_id);
	if (empty($subject)) {
		return;
	}
...
Login and visit the topic to unsubscribe from these emails.', 'bbpress'),
	...
	// For plugins to filter titles per reply/topic/user
	$subject = apply_filters('bbp_forum_subscription_mail_title', '[' . $blog_name . '] ' . $topic_title, $topic_id,
	                         $forum_id, $user_id);
	if (empty($subject)) {
		return;
	}
 ...
 * @return bool True on success, false on failure
 */
function bbp_notify_subscribers($reply_id = 0, $topic_id = 0, $forum_id = 0, $anonymous_data = FALSE,
                                $reply_author = 0) {
...
function bbp_logout_url($url = '', $redirect_to = '') {
	...
	if (empty($redirect_to) && !strstr($url, 'redirect_to')) {
		...
		$redirect_to = add_query_arg(['loggedout' => 'true'], esc_url($redirect_to));
...
function bbp_get_public_child_last_id($parent_id = 0, $post_type = 'post') {
	...
	if (FALSE === $child_id) {
		...
		$child_id
			= $wpdb->get_var($wpdb->prepare("SELECT ID FROM {$wpdb->posts} WHERE post_parent = %d AND post_status IN ( {$post_status} ) AND post_type = '%s' ORDER BY ID DESC LIMIT 1;",
			                                $parent_id, $post_type));
...
function bbp_get_public_child_ids($parent_id = 0, $post_type = 'post') {
	...
	if (FALSE === $child_ids) {
		...
		$post_status = "'" . implode("', '", $post_status) . "'";
		$child_ids
			= $wpdb->get_col($wpdb->prepare("SELECT ID FROM {$wpdb->posts} WHERE post_parent = %d AND post_status IN ( {$post_status} ) AND post_type = '%s' ORDER BY ID DESC;",
			                                $parent_id, $post_type));
				...
				switch ($post_type) {
					...
					case bbp_get_forum_post_type() :
						...
						if (!empty($_GET['type']) && (bbp_get_reply_post_type() === $_GET['type'])) {
							...
							if (empty($select_query_vars)) {
								$meta_query = [bbp_exclude_forum_ids('meta_query')];
							}
							// The query
							$the_query = [
								'author'         => 0,
								'feed'           => TRUE,
								'post_type'      => [bbp_get_reply_post_type(), bbp_get_topic_post_type()],
								'post_parent'    => 'any',
								'post_status'    => [bbp_get_public_status_id(), bbp_get_closed_status_id()],
								'posts_per_page' => bbp_get_replies_per_rss_page(),
								'order'          => 'DESC',
					...
					case bbp_get_topic_post_type() :
						...
						if (!empty($select_query_vars)) {
							...
							// Output the feed
							bbp_display_replies_feed_rss2(['feed' => TRUE]);
							// All topics
						} else {
							// The query
							$the_query = [
