...
function bbp_get_wp_login_action($args = '') {
	...
	if (!empty($r['action'])) {
		$login_url = add_query_arg(['action' => $r['action']], $r['url']);
		// No query arg
	} else {
		$login_url = $r['url'];
	}
...
function bbp_redirect_to_field($redirect_to = '') {
	...
	$redirect_field
	             = '<input type="hidden" id="bbp_redirect_to" name="redirect_to" value="' . esc_url($redirect_to) . '" />';
 ...
 */
function bbp_get_the_content($args = []) {
	// Parse arguments against default values
	$r = bbp_parse_args($args, [
		'context'           => 'topic',
	...
	if (bbp_use_wp_editor()) :
		...
		// Output the editor
		wp_editor($post_content, 'bbp_' . $r['context'] . '_content', [
			'wpautop'           => $r['wpautop'],
	 ...
	 */
		...
		<textarea id="bbp_<?php echo esc_attr($r['context']); ?>_content"
		          class="<?php echo esc_attr($r['editor_class']); ?>"
		          name="bbp_<?php echo esc_attr($r['context']); ?>_content" cols="60"
		          rows="<?php echo esc_attr($r['textarea_rows']); ?>"
		          tabindex="<?php echo esc_attr($r['tabindex']); ?>"><?php echo $post_content; ?></textarea>
 ...
 * @return array
 */
function bbp_get_tiny_mce_plugins($plugins = []) {
	// Unset fullscreen
 ...
 * @return array
 */
function bbp_get_teeny_mce_buttons($buttons = []) {
 ...
 */
function bbp_get_quicktags_settings($settings = []) {
...
function bbp_get_view_url($view = FALSE) {
	...
	if ($wp_rewrite->using_permalinks()) {
		...
		$url = add_query_arg([bbp_get_view_rewrite_id() => $view], home_url('/'));
	}
 ...
 */
function bbp_title_breadcrumb($args = []) {
 ...
 */
function bbp_get_breadcrumb($args = []) {
	...
	if (bbp_is_search()) {
		...
		if (current_user_can('manage_topic_tags')) {
			$tag_data[]
				= '<a href="' . esc_url(bbp_get_topic_tag_edit_link()) . '" class="bbp-edit-topic-tag-link">' . esc_html__('(Edit)',
				                                                                                                           'bbpress') . '</a>';
		}
	...
	if (!empty($r['include_home']) || empty($r['home_text'])) {
		$crumbs[]
			= '<a href="' . trailingslashit(home_url()) . '" class="bbp-breadcrumb-home">' . $r['home_text'] . '</a>';
	}
	...
	if (!empty($ancestors)) {
		...
		foreach ((array)$ancestors as $parent_id) {
			...
			switch ($parent->post_type) {
				...
				case bbp_get_forum_post_type() :
					$crumbs[]
						= '<a href="' . esc_url(bbp_get_forum_permalink($parent->ID)) . '" class="bbp-breadcrumb-forum">' . bbp_get_forum_title($parent->ID) . '</a>';
					break;
				// Topic
				case bbp_get_topic_post_type() :
					$crumbs[]
						= '<a href="' . esc_url(bbp_get_topic_permalink($parent->ID)) . '" class="bbp-breadcrumb-topic">' . bbp_get_topic_title($parent->ID) . '</a>';
					break;
				// Reply (Note: not in most themes)
				case bbp_get_reply_post_type() :
					$crumbs[]
						= '<a href="' . esc_url(bbp_get_reply_permalink($parent->ID)) . '" class="bbp-breadcrumb-reply">' . bbp_get_reply_title($parent->ID) . '</a>';
					break;
				// WordPress Post/Page/Other
				default :
					$crumbs[]
						= '<a href="' . esc_url(get_permalink($parent->ID)) . '" class="bbp-breadcrumb-item">' . get_the_title($parent->ID) . '</a>';
					break;
			}
		}
		// Edit topic tag
	} elseif (bbp_is_topic_tag_edit()) {
		$crumbs[] = '<a href="' . esc_url(get_term_link(bbp_get_topic_tag_id(),
		                                                bbp_get_topic_tag_tax_id())) . '" class="bbp-breadcrumb-topic-tag">' . sprintf(__('Topic Tag: %s',
		                                                                                                                                  'bbpress'),
		                                                                                                                               bbp_get_topic_tag_name()) . '</a>';
		// Search
	} elseif (bbp_is_search() && bbp_get_search_terms()) {
		$crumbs[]
			= '<a href="' . esc_url(bbp_get_search_url()) . '" class="bbp-breadcrumb-search">' . esc_html__('Search',
			                                                                                                'bbpress') . '</a>';
	}
	/** Current ***********************************************************/
	...
	// Build the trail
	$trail = !empty($crumbs)
		? ($r['before'] . $r['crumb_before'] . implode($sep . $r['crumb_after'] . $r['crumb_before'],
		                                               $crumbs) . $r['crumb_after'] . $r['after']) : '';
...
function bbp_get_logout_link($redirect_to = '') {
	return apply_filters('bbp_get_logout_link',
	                     '<a href="' . wp_logout_url($redirect_to) . '" class="button logout-link">' . esc_html__('Log Out',
	                                                                                                              'bbpress') . '</a>',
	                     $redirect_to);
}
...
function bbp_title($title = '', $sep = '&raquo;', $seplocation = '') {
	...
	if (bbp_is_forum_archive()) {
		...
		if (bbp_is_user_home()) {
			...
			$new_title['text'] = sprintf(esc_attr_x("%s's", 'User viewing another users profile', 'bbpress'),
			                             get_userdata(bbp_get_user_id())->display_name);
		}
		// User topics created
	...
	$new_title = bbp_parse_args($new_title, [
		'text'   => $title,
		'format' => '%s'
	], 'title');
