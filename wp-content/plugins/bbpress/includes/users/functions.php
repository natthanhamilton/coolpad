 ...
 */
function bbp_set_current_anonymous_user_data($anonymous_data = []) {
	if (empty($anonymous_data) || !is_array($anonymous_data)) {
		return;
	}
	$comment_cookie_lifetime = apply_filters('comment_cookie_lifetime', 30000000);
	setcookie('comment_author_' . COOKIEHASH, $anonymous_data['bbp_anonymous_name'], time() + $comment_cookie_lifetime,
	          COOKIEPATH, COOKIE_DOMAIN);
	setcookie('comment_author_email_' . COOKIEHASH, $anonymous_data['bbp_anonymous_email'],
	          time() + $comment_cookie_lifetime, COOKIEPATH, COOKIE_DOMAIN);
	setcookie('comment_author_url_' . COOKIEHASH, $anonymous_data['bbp_anonymous_website'],
	          time() + $comment_cookie_lifetime, COOKIEPATH, COOKIE_DOMAIN);
}
...
function bbp_get_topic_favoriters($topic_id = 0) {
	...
	if (FALSE === $users) {
		$users
			= $wpdb->get_col("SELECT user_id FROM {$wpdb->usermeta} WHERE meta_key = '{$key}' and FIND_IN_SET('{$topic_id}', meta_value) > 0");
...
function bbp_get_user_favorites($user_id = 0) {
	...
	if (!empty($favorites)) {
		$query = bbp_has_topics(['post__in' => $favorites]);
	} else {
		$query = FALSE;
	}
...
function bbp_favorites_handler($action = '') {
	...
	if (empty($topic_id)) {
		bbp_add_error('bbp_favorite_topic_id',
		              __('<strong>ERROR</strong>: No topic was found! Which topic are you marking/unmarking as favorite?',
		                 'bbpress'));
		// Check nonce
	} elseif (!bbp_verify_nonce_request('toggle-favorite_' . $topic_id)) {
		bbp_add_error('bbp_favorite_nonce',
		              __('<strong>ERROR</strong>: Are you sure you wanted to do that?', 'bbpress'));
		// Check current user's ability to edit the user
	} elseif (!current_user_can('edit_user', $user_id)) {
		bbp_add_error('bbp_favorite_permissions',
		              __('<strong>ERROR</strong>: You don\'t have the permission to edit favorites of that user!',
		                 'bbpress'));
	}
	// Bail if errors
	if (bbp_has_errors()) {
	...
	if (TRUE === $success) {
		...
		bbp_add_error('bbp_favorite_remove',
		              __('<strong>ERROR</strong>: There was a problem removing that topic from favorites!', 'bbpress'));
	} elseif (FALSE === $is_favorite && 'bbp_favorite_add' === $action) {
		bbp_add_error('bbp_favorite_add',
		              __('<strong>ERROR</strong>: There was a problem favoriting that topic!', 'bbpress'));
	}
}
...
function bbp_get_forum_subscribers($forum_id = 0) {
	...
	if (FALSE === $users) {
		$users
			= $wpdb->get_col("SELECT user_id FROM {$wpdb->usermeta} WHERE meta_key = '{$key}' and FIND_IN_SET('{$forum_id}', meta_value) > 0");
...
function bbp_get_user_topic_subscriptions($user_id = 0) {
	...
	if (!empty($subscriptions)) {
		$query = bbp_has_topics(['post__in' => $subscriptions]);
	} else {
		$query = FALSE;
	}
...
function bbp_get_user_forum_subscriptions($user_id = 0) {
	...
	if (!empty($subscriptions)) {
		$query = bbp_has_forums(['post__in' => $subscriptions]);
	} else {
		$query = FALSE;
	}
 ...
 */
function bbp_is_user_subscribed_to_forum($user_id = 0, $forum_id = 0, $subscribed_ids = []) {
	// Assume user is not subscribed
	$retval = FALSE;
	// Validate user
	$user_id = bbp_get_user_id($user_id, TRUE, TRUE);
 ...
 */
function bbp_is_user_subscribed_to_topic($user_id = 0, $topic_id = 0, $subscribed_ids = []) {
	// Assume user is not subscribed
	$retval = FALSE;
	// Validate user
	$user_id = bbp_get_user_id($user_id, TRUE, TRUE);
...
function bbp_forum_subscriptions_handler($action = '') {
	...
	if (empty($forum_id)) {
		bbp_add_error('bbp_subscription_forum_id',
		              __('<strong>ERROR</strong>: No forum was found! Which forum are you subscribing/unsubscribing to?',
		                 'bbpress'));
		// Check nonce
		...
		bbp_add_error('bbp_subscription_permissions',
		              __('<strong>ERROR</strong>: You don\'t have the permission to edit favorites of that user!',
		                 'bbpress'));
	}
	...
	if (TRUE === $success) {
		...
		bbp_add_error('bbp_unsubscribe',
		              __('<strong>ERROR</strong>: There was a problem unsubscribing from that forum!', 'bbpress'));
	} elseif (FALSE === $is_subscription && 'bbp_subscribe' === $action) {
		bbp_add_error('bbp_subscribe',
		              __('<strong>ERROR</strong>: There was a problem subscribing to that forum!', 'bbpress'));
	}
}
...
function bbp_subscriptions_handler($action = '') {
	...
	// Check for empty topic
	if (empty($topic_id)) {
		bbp_add_error('bbp_subscription_topic_id',
		              __('<strong>ERROR</strong>: No topic was found! Which topic are you subscribing/unsubscribing to?',
		                 'bbpress'));
		// Check nonce
	...
	if (TRUE === $success) {
		...
		bbp_add_error('bbp_unsubscribe',
		              __('<strong>ERROR</strong>: There was a problem unsubscribing from that topic!', 'bbpress'));
	} elseif (FALSE === $is_subscription && 'bbp_subscribe' === $action) {
		bbp_add_error('bbp_subscribe',
		              __('<strong>ERROR</strong>: There was a problem subscribing to that topic!', 'bbpress'));
	}
}
...
function bbp_edit_user_handler($action = '') {
	...
	if (is_multisite() && bbp_is_user_home_edit() && isset($_GET['newuseremail'])) {
		...
		if (hash_equals($new_email['hash'], $_GET['newuseremail'])) {
			...
			if ($wpdb->get_var($wpdb->prepare("SELECT user_login FROM {$wpdb->signups} WHERE user_login = %s",
			                                  bbp_get_displayed_user_field('user_login', 'raw')))
			) {
				$wpdb->query($wpdb->prepare("UPDATE {$wpdb->signups} SET user_email = %s WHERE user_login = %s",
				                            $user->user_email, bbp_get_displayed_user_field('user_login', 'raw')));
			}
			wp_update_user(get_object_vars($user));
			delete_option($user_id . '_new_email');
			wp_safe_redirect(add_query_arg(['updated' => 'true'], bbp_get_user_profile_edit_url($user_id)));
			exit();
		}
	...
	if (!bbp_verify_nonce_request('update-user_' . $user_id)) {
		bbp_add_error('bbp_update_user_nonce',
		              __('<strong>ERROR</strong>: Are you sure you wanted to do that?', 'bbpress'));
	...
	if (!current_user_can('edit_user', $user_id)) {
		bbp_add_error('bbp_update_user_capability',
		              __('<strong>ERROR</strong>: Are you sure you wanted to do that?', 'bbpress'));
	...
	if (is_wp_error($edit_user)) {
		...
		$redirect = add_query_arg(['updated' => 'true'], bbp_get_user_profile_edit_url($edit_user));
		wp_safe_redirect($redirect);
		exit;
	}
}
...
function bbp_sanitize_displayed_user_field($value = '', $field = '', $context = 'display') {
	...
	if (!in_array($context, ['edit', 'display'])) {
		return $value;
	}
...
function bbp_user_maybe_convert_pass() {
	...
	$row
		= $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->users} INNER JOIN {$wpdb->usermeta} ON user_id = ID WHERE meta_key = '_bbp_class' AND user_login = '%s' LIMIT 1",
		                                $username));
