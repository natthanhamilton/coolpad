 ...
 */
function bbp_map_primary_meta_caps($caps = [], $cap = '', $user_id = 0, $args = []) {
...
function bbp_get_user_role_map() {
	...
	return (array)apply_filters('bbp_get_user_role_map', [
...
function bbp_make_spam_user($user_id = 0) {
	...
	$post_types = [bbp_get_topic_post_type(), bbp_get_reply_post_type()];
	...
	foreach ((array)array_keys($blogs) as $blog_id) {
		...
		$posts
			= $wpdb->get_col($wpdb->prepare("SELECT ID FROM {$wpdb->posts} WHERE post_author = %d AND post_status = '%s' AND post_type IN ( {$post_types} )",
			                                $user_id, bbp_get_public_status_id()));
...
function bbp_make_ham_user($user_id = 0) {
	...
	foreach ((array)array_keys($blogs) as $blog_id) {
		...
		$posts
			= $wpdb->get_col($wpdb->prepare("SELECT ID FROM {$wpdb->posts} WHERE post_author = %d AND post_status = '%s' AND post_type IN ( {$post_types} )",
			                                $user_id, bbp_get_spam_status_id()));
		// Loop through posts and spam them
		if (!empty($posts)) {
			foreach ($posts as $post_id) {
				// The routines for topics ang replies are different, so use the
