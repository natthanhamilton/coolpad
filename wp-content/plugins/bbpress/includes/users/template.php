...
function bbp_get_user_profile_link($user_id = 0) {
	...
	$user_link
	      = '<a href="' . esc_url(bbp_get_user_profile_url($user_id)) . '">' . esc_html($user->display_name) . '</a>';
 ...
 */
function bbp_user_nicename($user_id = 0, $args = []) {
 ...
 */
function bbp_get_user_nicename($user_id = 0, $args = []) {
...
function bbp_get_user_profile_url($user_id = 0, $user_nicename = '') {
	...
	if ($wp_rewrite->using_permalinks()) {
		...
		$url = add_query_arg([bbp_get_user_rewrite_id() => $user_id], home_url('/'));
	}
...
function bbp_get_user_profile_edit_url($user_id = 0, $user_nicename = '') {
	...
	if ($wp_rewrite->using_permalinks()) {
		...
		$url = add_query_arg([$bbp->user_id => $user_id, $bbp->edit_id => '1'], home_url('/'));
	}
...
function bbp_get_favorites_permalink($user_id = 0) {
	...
	if ($wp_rewrite->using_permalinks()) {
		$url
			  = $wp_rewrite->root . bbp_get_user_slug() . '/%' . bbp_get_user_rewrite_id() . '%/%' . bbp_get_user_favorites_rewrite_id() . '%';
		$user = get_userdata($user_id);
		if (!empty($user->user_nicename)) {
			$user_nicename = $user->user_nicename;
		} else {
 ...
 */
function bbp_user_favorites_link($args = [], $user_id = 0, $wrap = TRUE) {
...
function bbp_get_user_favorites_link($args = '', $user_id = 0, $wrap = TRUE) {
	...
	if (!empty($is_fav)) {
		$text       = $r['favorited'];
		$query_args = ['action' => 'bbp_favorite_remove', 'topic_id' => $topic_id];
	} else {
		$text       = $r['favorite'];
		$query_args = ['action' => 'bbp_favorite_add', 'topic_id' => $topic_id];
	}
	...
	$html = sprintf('%s<span id="favorite-%d"  %s><a href="%s" class="favorite-toggle" data-topic="%d">%s</a></span>%s',
	                $r['before'], $topic_id, $sub, $url, $topic_id, $text, $r['after']);
...
function bbp_get_subscriptions_permalink($user_id = 0) {
	...
	if ($wp_rewrite->using_permalinks()) {
		$url
			  = $wp_rewrite->root . bbp_get_user_slug() . '/%' . bbp_get_user_rewrite_id() . '%/%' . bbp_get_user_subscriptions_rewrite_id() . '%';
		$user = get_userdata($user_id);
		if (!empty($user->user_nicename)) {
			$user_nicename = $user->user_nicename;
		} else {
		...
		$url = str_replace('%' . bbp_get_user_rewrite_id() . '%', $user_nicename, $url);
		$url = str_replace('%' . bbp_get_user_subscriptions_rewrite_id() . '%', bbp_get_user_subscriptions_slug(),
		                   $url);
		$url = home_url(user_trailingslashit($url));
		// Unpretty permalinks
	} else {
...
function bbp_get_user_subscribe_link($args = '', $user_id = 0, $wrap = TRUE) {
	...
	if (empty($topic_id) && !empty($forum_id)) {
		...
		if (!empty($is_subscribed)) {
			$text       = $r['unsubscribe'];
			$query_args = ['action' => 'bbp_unsubscribe', 'forum_id' => $forum_id];
		} else {
			$text       = $r['subscribe'];
			$query_args = ['action' => 'bbp_subscribe', 'forum_id' => $forum_id];
		}
		...
		$html
		     = sprintf('%s<span id="subscribe-%d"  %s><a href="%s" class="subscription-toggle" data-forum="%d">%s</a></span>%s',
		               $r['before'], $forum_id, $sub, $url, $forum_id, $text, $r['after']);
		// Initial output is wrapped in a span, ajax output is hooked to this
		if (!empty($wrap)) {
		...
		if (!empty($is_subscribed)) {
			$text       = $r['unsubscribe'];
			$query_args = ['action' => 'bbp_unsubscribe', 'topic_id' => $topic_id];
		} else {
			$text       = $r['subscribe'];
			$query_args = ['action' => 'bbp_subscribe', 'topic_id' => $topic_id];
		}
		// Create the link based where the user is and if the user is
		// subscribed already
		if (bbp_is_subscriptions()) {
		...
		$sub = $is_subscribed ? ' class="is-subscribed"' : '';
		$html
		     = sprintf('%s<span id="subscribe-%d"  %s><a href="%s" class="subscription-toggle" data-topic="%d">%s</a></span>%s',
		               $r['before'], $topic_id, $sub, $url, $topic_id, $text, $r['after']);
		// Initial output is wrapped in a span, ajax output is hooked to this
		if (!empty($wrap)) {
			$html = '<span id="subscription-toggle">' . $html . '</span>';
...
function bbp_notice_edit_user_is_super_admin() {
	if (is_multisite() && (bbp_is_single_user() || bbp_is_single_user_edit()) && current_user_can('manage_network_options') && is_super_admin(bbp_get_displayed_user_id())) : ?>
		...
		<div class="bbp-template-notice important">
			<p><?php bbp_is_user_home() || bbp_is_user_home_edit() ? esc_html_e('You have super admin privileges.',
			                                                                    'bbpress')
					: esc_html_e('This user has super admin privileges.', 'bbpress'); ?></p>
		</div>
...
function bbp_edit_user_display_name() {
	...
	if (!in_array($bbp->displayed_user->display_name,
	              $public_display)
	) // Only add this if it isn't duplicated elsewhere
	{
		$public_display = ['display_displayname' => $bbp->displayed_user->display_name] + $public_display;
	}
	...
	<select name="display_name" id="display_name">
		...
		<?php foreach ($public_display as $id => $item) : ?>
			...
			<option id="<?php echo $id; ?>"
			        value="<?php echo esc_attr($item); ?>"<?php selected($bbp->displayed_user->display_name,
	...
	<select name="role" id="role">
		...
		<?php foreach ($blog_roles as $role => $details) : ?>
			...
			<option <?php selected($user_role, $role); ?>
				value="<?php echo esc_attr($role); ?>"><?php echo translate_user_role($details['name']); ?></option>
...
function bbp_get_user_topics_created_url($user_id = 0) {
	...
	if ($wp_rewrite->using_permalinks()) {
		$url
			  = $wp_rewrite->root . bbp_get_user_slug() . '/%' . bbp_get_user_rewrite_id() . '%/' . bbp_get_topic_archive_slug();
		$user = get_userdata($user_id);
		if (!empty($user->user_nicename)) {
			$user_nicename = $user->user_nicename;
		} else {
...
function bbp_get_user_replies_created_url($user_id = 0) {
	...
	if ($wp_rewrite->using_permalinks()) {
		$url
			  = $wp_rewrite->root . bbp_get_user_slug() . '/%' . bbp_get_user_rewrite_id() . '%/' . bbp_get_reply_archive_slug();
		$user = get_userdata($user_id);
		if (!empty($user->user_nicename)) {
			$user_nicename = $user->user_nicename;
		} else {
...
function bbp_login_notices() {
	...
	if (!empty($_GET['loggedout']) && (TRUE === $_GET['loggedout'])) {
	...
	} elseif (!empty($_GET['checkemail']) && in_array($_GET['checkemail'], ['confirm', 'newpass', 'registered'])) {
		switch ($_GET['checkemail']) {
			...
			case 'registered' :
				bbp_add_error('registered', __('Registration complete. Please check your e-mail.', 'bbpress'),
				              'message');
				break;
		}
	}
...
function bbp_user_register_fields() {
	...
	bbp_redirect_to_field(add_query_arg(['checkemail' => 'registered'], $redirect_to));
...
function bbp_user_lost_pass_fields() {
	...
	bbp_redirect_to_field(add_query_arg(['checkemail' => 'confirm'], $redirect_to));
...
function bbp_get_author_link($args = '') {
	...
	if (!empty($r['post_id'])) {
		...
		if (empty($r['link_title'])) {
			$r['link_title'] = sprintf(!bbp_is_reply_anonymous($r['post_id']) ? __('View %s\'s profile', 'bbpress')
 ...
 * @return type
 */
function bbp_get_forums_for_current_user($args = []) {
	// Setup arrays
	$private = $hidden = $post__not_in = [];
	// Private forums
