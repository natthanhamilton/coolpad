...
function bbp_get_forum_post_type_labels() {
	return apply_filters('bbp_get_forum_post_type_labels', [
...
function bbp_get_forum_post_type_rewrite() {
	return apply_filters('bbp_get_forum_post_type_rewrite', [
...
function bbp_get_forum_post_type_supports() {
	return apply_filters('bbp_get_forum_post_type_supports', [
		'title',
		'editor',
		'revisions'
	]);
...
function bbp_forum_get_subforums($args = '') {
	...
	if (bbp_is_user_keymaster()) {
		$post_stati = [bbp_get_public_status_id(), bbp_get_private_status_id(), bbp_get_hidden_status_id()];
 ...
 */
function bbp_forum_subscription_link($args = []) {
 ...
 */
function bbp_get_forum_subscription_link($args = []) {
	// No link
	$retval = FALSE;
	// Parse the arguments
	$r = bbp_parse_args($args, [
...
function bbp_get_forum_last_topic_permalink($forum_id = 0) {
	...
	return apply_filters('bbp_get_forum_last_topic_permalink',
	                     bbp_get_topic_permalink(bbp_get_forum_last_topic_id($forum_id)), $forum_id);
}
...
function bbp_get_forum_last_reply_title($forum_id = 0) {
	...
	return apply_filters('bbp_get_forum_last_reply_title', bbp_get_reply_title(bbp_get_forum_last_reply_id($forum_id)),
	                     $forum_id);
}
...
function bbp_get_forum_last_reply_permalink($forum_id = 0) {
	...
	return apply_filters('bbp_get_forum_last_reply_permalink',
...
function bbp_get_forum_topics_link($forum_id = 0) {
	$forum    = bbp_get_forum($forum_id);
	$forum_id = $forum->ID;
	$topics   = sprintf(_n('%s topic', '%s topics', bbp_get_forum_topic_count($forum_id, TRUE, FALSE), 'bbpress'),
	                    bbp_get_forum_topic_count($forum_id));
	$retval   = '';
	...
	if (!empty($deleted) && current_user_can('edit_others_topics')) {
		...
		if (bbp_get_view_all()) {
			...
			$retval .= " <a href='" . esc_url(bbp_add_view_all(bbp_get_forum_permalink($forum_id),
			                                                   TRUE)) . "'>" . esc_html($extra) . "</a>";
		}
	}
 ...
 */
function bbp_forum_class($forum_id = 0, $classes = []) {
 ...
 */
function bbp_get_forum_class($forum_id = 0, $classes = []) {
	$bbp      = bbpress();
	$forum_id = bbp_get_forum_id($forum_id);
...
function bbp_get_single_forum_description($args = '') {
	...
	if (!empty($last_active)) {
		...
		$last_updated_by = bbp_get_author_link(['post_id' => $last_active, 'size' => $r['size']]);
	...
	if (!empty($last_active)) {
		if (!empty($reply_count)) {
			if (bbp_is_forum_category($forum_id)) {
				$retstr = sprintf(esc_html__('This category contains %1$s and %2$s, and was last updated by %3$s %4$s.',
				                             'bbpress'), $topic_text, $reply_text, $last_updated_by, $time_since);
			} else {
				$retstr = sprintf(esc_html__('This forum contains %1$s and %2$s, and was last updated by %3$s %4$s.',
				                             'bbpress'), $topic_text, $reply_text, $last_updated_by, $time_since);
			}
		} else {
			if (bbp_is_forum_category($forum_id)) {
				$retstr = sprintf(esc_html__('This category contains %1$s, and was last updated by %2$s %3$s.',
				                             'bbpress'), $topic_text, $last_updated_by, $time_since);
			} else {
				$retstr = sprintf(esc_html__('This forum contains %1$s, and was last updated by %2$s %3$s.', 'bbpress'),
				                  $topic_text, $last_updated_by, $time_since);
			}
		}
		// Forum has no last active data
	} else {
		if (!empty($reply_count)) {
			if (bbp_is_forum_category($forum_id)) {
				$retstr = sprintf(esc_html__('This category contains %1$s and %2$s.', 'bbpress'), $topic_text,
				                  $reply_text);
			} else {
				$retstr = sprintf(esc_html__('This forum contains %1$s and %2$s.', 'bbpress'), $topic_text,
				                  $reply_text);
			}
		} else {
...
function bbp_get_forum_topics_feed_link($forum_id = 0) {
	...
	if (!empty($forum_id)) {
		...
		$link = '<a href="' . esc_url($url) . '" class="bbp-forum-rss-link topics"><span>' . esc_attr__('Topics',
		                                                                                                'bbpress') . '</span></a>';
	}
...
function bbp_get_forum_replies_feed_link($forum_id = 0) {
	...
	if (!empty($forum_id)) {
		...
		if (get_option('permalink_structure')) {
			...
			$url = trailingslashit(bbp_get_forum_permalink($forum_id)) . 'feed';
			$url = user_trailingslashit($url, 'single_feed');
			$url = add_query_arg(['type' => 'reply'], $url);
			// Unpretty permalinks
		} else {
			$url = home_url(add_query_arg([
				                              'type'                    => 'reply',
		...
		$link = '<a href="' . esc_url($url) . '" class="bbp-forum-rss-link replies"><span>' . esc_html__('Replies',
		                                                                                                 'bbpress') . '</span></a>';
	}
