 ...
 */
function bbp_insert_forum($forum_data = [], $forum_meta = []) {
	// Forum
	$forum_data = bbp_parse_args($forum_data, [
	...
	// Forum meta
	$forum_meta = bbp_parse_args($forum_meta, [
		'reply_count'          => 0,
		'topic_count'          => 0,
...
function bbp_new_forum_handler($action = '') {
	...
	if (!bbp_verify_nonce_request('bbp-new-forum')) {
		bbp_add_error('bbp_new_forum_nonce',
	...
	if (!current_user_can('publish_forums')) {
		bbp_add_error('bbp_forum_permissions',
		              __('<strong>ERROR</strong>: You do not have permission to create new forums.', 'bbpress'));
	...
	if (empty($forum_content)) {
		bbp_add_error('bbp_forum_content',
		              __('<strong>ERROR</strong>: Your forum description cannot be empty.', 'bbpress'));
	}
	/** Forum Parent **********************************************************/
	...
	if (empty($forum_parent_id)) {
		bbp_add_error('bbp_new_forum_missing_parent',
		              __('<strong>ERROR</strong>: Your forum must have a parent.', 'bbpress'));
		// Forum exists
	} elseif (!empty($forum_parent_id)) {
		// Forum is a category
		if (bbp_is_forum_category($forum_parent_id)) {
			bbp_add_error('bbp_new_forum_forum_category',
			              __('<strong>ERROR</strong>: This forum is a category. No forums can be created in this forum.',
			                 'bbpress'));
		}
		// Forum is closed and user cannot access
		if (bbp_is_forum_closed($forum_parent_id) && !current_user_can('edit_forum', $forum_parent_id)) {
			bbp_add_error('bbp_new_forum_forum_closed',
			              __('<strong>ERROR</strong>: This forum has been closed to new forums.', 'bbpress'));
		}
		// Forum is private and user cannot access
		if (bbp_is_forum_private($forum_parent_id) && !current_user_can('read_private_forums')) {
			bbp_add_error('bbp_new_forum_forum_private',
			              __('<strong>ERROR</strong>: This forum is private and you do not have the capability to read or create new forums in it.',
			                 'bbpress'));
		}
		// Forum is hidden and user cannot access
		if (bbp_is_forum_hidden($forum_parent_id) && !current_user_can('read_hidden_forums')) {
			bbp_add_error('bbp_new_forum_forum_hidden',
			              __('<strong>ERROR</strong>: This forum is hidden and you do not have the capability to read or create new forums in it.',
			                 'bbpress'));
		}
	}
	/** Forum Flooding ********************************************************/
	...
	/** Forum Duplicate *******************************************************/
	if (!bbp_check_for_duplicate(['post_type' => bbp_get_forum_post_type(), 'post_author' => $forum_author, 'post_content' => $forum_content, 'anonymous_data' => $anonymous_data])) {
	...
	if (!bbp_check_for_blacklist($anonymous_data, $forum_author, $forum_title, $forum_content)) {
		bbp_add_error('bbp_forum_blacklist',
		              __('<strong>ERROR</strong>: Your forum cannot be created at this time.', 'bbpress'));
	}
	/** Forum Moderation ******************************************************/
	$post_status = bbp_get_public_status_id();
	...
	$forum_data = apply_filters('bbp_new_forum_pre_insert', [
	...
	if (!empty($forum_id) && !is_wp_error($forum_id)) {
		...
		if ((get_post_field('post_status',
		                    $forum_id) === bbp_get_trash_status_id()) || ($forum_data['post_status'] === bbp_get_trash_status_id())
		) {
			// Trash the reply
		...
		$append_error = (is_wp_error($forum_id) && $forum_id->get_error_message())
			? $forum_id->get_error_message() . ' ' : '';
		bbp_add_error('bbp_forum_error',
		              __('<strong>ERROR</strong>: The following problem(s) have been found with your forum:' . $append_error,
		                 'bbpress'));
	}
}
...
function bbp_edit_forum_handler($action = '') {
	...
	if (!bbp_verify_nonce_request('bbp-edit-forum_' . $forum_id)) {
		bbp_add_error('bbp_edit_forum_nonce',
		              __('<strong>ERROR</strong>: Are you sure you wanted to do that?', 'bbpress'));

		return;
		// Forum does not exist
	} elseif (empty($forum)) {
		bbp_add_error('bbp_edit_forum_not_found',
		              __('<strong>ERROR</strong>: The forum you want to edit was not found.', 'bbpress'));

		return;
		// User cannot edit this forum
	} elseif (!current_user_can('edit_forum', $forum_id)) {
		bbp_add_error('bbp_edit_forum_permissions',
		              __('<strong>ERROR</strong>: You do not have permission to edit that forum.', 'bbpress'));
	...
	if (!empty($forum_parent_id) && ($forum_parent_id !== $current_parent_forum_id)) {
		// Forum is closed and user cannot access
		if (bbp_is_forum_closed($forum_parent_id) && !current_user_can('edit_forum', $forum_parent_id)) {
			bbp_add_error('bbp_edit_forum_forum_closed',
			              __('<strong>ERROR</strong>: This forum has been closed to new forums.', 'bbpress'));
		}
		// Forum is private and user cannot access
		if (bbp_is_forum_private($forum_parent_id) && !current_user_can('read_private_forums')) {
			bbp_add_error('bbp_edit_forum_forum_private',
			              __('<strong>ERROR</strong>: This forum is private and you do not have the capability to read or create new forums in it.',
			                 'bbpress'));
		}
		// Forum is hidden and user cannot access
		if (bbp_is_forum_hidden($forum_parent_id) && !current_user_can('read_hidden_forums')) {
			bbp_add_error('bbp_edit_forum_forum_hidden',
			              __('<strong>ERROR</strong>: This forum is hidden and you do not have the capability to read or create new forums in it.',
			                 'bbpress'));
		}
	}
	...
	// No forum content
	if (empty($forum_content)) {
		bbp_add_error('bbp_edit_forum_content',
		              __('<strong>ERROR</strong>: Your forum description cannot be empty.', 'bbpress'));
	}
	/** Forum Blacklist *******************************************************/
	if (!bbp_check_for_blacklist($anonymous_data, bbp_get_forum_author_id($forum_id), $forum_title, $forum_content)) {
		bbp_add_error('bbp_forum_blacklist',
		              __('<strong>ERROR</strong>: Your forum cannot be edited at this time.', 'bbpress'));
	}
	/** Forum Moderation ******************************************************/
	$post_status = bbp_get_public_status_id();
	...
	$forum_data = apply_filters('bbp_edit_forum_pre_insert', [
		'ID'           => $forum_id,
		'post_title'   => $forum_title,
		'post_content' => $forum_content,
		'post_status'  => $post_status,
	...
	if (!empty($forum_id) && !is_wp_error($forum_id)) {
		...
		$append_error = (is_wp_error($forum_id) && $forum_id->get_error_message())
			? $forum_id->get_error_message() . ' ' : '';
		bbp_add_error('bbp_forum_error',
		              __('<strong>ERROR</strong>: The following problem(s) have been found with your forum:' . $append_error . 'Please try again.',
		                 'bbpress'));
	}
}
...
function bbp_save_forum_extras($forum_id = 0) {
	...
	/** Forum Status ******************************************************/
	if (!empty($_POST['bbp_forum_status']) && in_array($_POST['bbp_forum_status'], ['open', 'closed'])) {
	...
	/** Forum Type ********************************************************/
	if (!empty($_POST['bbp_forum_type']) && in_array($_POST['bbp_forum_type'], ['forum', 'category'])) {
	...
	/** Forum Visibility **************************************************/
	if (!empty($_POST['bbp_forum_visibility']) && in_array($_POST['bbp_forum_visibility'],
	                                                       [bbp_get_public_status_id(), bbp_get_private_status_id(), bbp_get_hidden_status_id()])
	) {
...
function bbp_publicize_forum($forum_id = 0, $current_visibility = '') {
	...
	if (bbp_get_public_status_id() !== $current_visibility) {
		...
		global $wpdb;
		$wpdb->update($wpdb->posts, ['post_status' => bbp_get_public_status_id()], ['ID' => $forum_id]);
...
function bbp_privatize_forum($forum_id = 0, $current_visibility = '') {
	...
	if (bbp_get_private_status_id() !== $current_visibility) {
		...
		// Update forums visibility setting
		global $wpdb;
		$wpdb->update($wpdb->posts, ['post_status' => bbp_get_private_status_id()], ['ID' => $forum_id]);
...
function bbp_hide_forum($forum_id = 0, $current_visibility = '') {
	...
	if (bbp_get_hidden_status_id() !== $current_visibility) {
		...
		// Update forums visibility setting
		global $wpdb;
		$wpdb->update($wpdb->posts, ['post_status' => bbp_get_hidden_status_id()], ['ID' => $forum_id]);
	...
	if (TRUE === $update_ancestors) {
		...
		if (!empty($ancestors)) {
			foreach ((array)$ancestors as $parent_forum_id) {
				...
				update_post_meta($parent_forum_id, '_bbp_total_topic_count',
				                 (int)$parent_total_topic_count + (int)$difference);
			}
		}
	}

	return (int)apply_filters('bbp_bump_forum_topic_count', (int)$total_topic_count + (int)$difference, $forum_id,
	                          (int)$difference, (bool)$update_ancestors);
}
	...
	if (TRUE === $update_ancestors) {
		...
		if (!empty($ancestors)) {
			foreach ((array)$ancestors as $parent_forum_id) {
				...
				update_post_meta($parent_forum_id, '_bbp_total_reply_count',
				                 (int)$parent_total_reply_count + (int)$difference);
			}
		}
	}

	return (int)apply_filters('bbp_bump_forum_reply_count', (int)$total_reply_count + (int)$difference, $forum_id,
	                          (int)$difference, (bool)$update_ancestors);
}
		...
		if (empty($topic_count)) {
			...
			$topic_count
			             = $wpdb->get_var($wpdb->prepare("SELECT COUNT(ID) FROM {$wpdb->posts} WHERE post_parent = %d AND post_status IN ( {$post_status} ) AND post_type = '%s';",
			                                             $forum_id, bbp_get_topic_post_type()));
		}
		// Update the count
...
function bbp_update_forum_reply_count($forum_id = 0) {
	...
	if (!empty($topic_ids)) {
		...
		$reply_count
		           = (int)$wpdb->get_var($wpdb->prepare("SELECT COUNT(ID) FROM {$wpdb->posts} WHERE post_parent IN ( {$topic_ids} ) AND post_status = '%s' AND post_type = '%s';",
...
function bbp_get_forum_statuses() {
	return apply_filters('bbp_get_forum_statuses', [
...
function bbp_get_forum_types() {
	return apply_filters('bbp_get_forum_types', [
...
function bbp_get_forum_visibilities() {
	return apply_filters('bbp_get_forum_visibilities', [
...
function bbp_get_hidden_forum_ids() {
	$forum_ids = get_option('_bbp_hidden_forums', []);
...
function bbp_get_private_forum_ids() {
	$forum_ids = get_option('_bbp_private_forums', []);
...
function bbp_exclude_forum_ids($type = 'string') {
	// Setup arrays
	$private = $hidden = $meta_query = $forum_ids = [];
...
function bbp_pre_get_posts_normalize_forum_visibility($posts_query = NULL) {
	...
	// Topics Or Replies
	if (array_intersect([bbp_get_topic_post_type(), bbp_get_reply_post_type()], $post_types)) {
...
function bbp_forum_query_last_reply_id($forum_id, $topic_ids = 0) {
	...
	if (FALSE === $reply_id) {
		...
		if (!empty($topic_ids)) {
			$topic_ids = implode(',', wp_parse_id_list($topic_ids));
			$reply_id
			           = (int)$wpdb->get_var($wpdb->prepare("SELECT ID FROM {$wpdb->posts} WHERE post_parent IN ( {$topic_ids} ) AND post_status = '%s' AND post_type = '%s' ORDER BY ID DESC LIMIT 1;",
			                                                bbp_get_public_status_id(), bbp_get_reply_post_type()));
