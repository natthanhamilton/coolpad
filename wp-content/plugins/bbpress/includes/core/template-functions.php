...
function bbp_locate_template($template_names, $load = FALSE, $require_once = TRUE) {
	 ...
	 */
	do_action('bbp_locate_template', $located, $template_name, $template_names, $template_locations, $load,
	          $require_once);
 ...
 */
function bbp_enqueue_style($handle = '', $file = '', $dependencies = [], $version = FALSE, $media = 'all') {
	// No file found yet
	$located = FALSE;
	// Trim off any slashes from the template name
	$file = ltrim($file, '/');
 ...
 *                               is used to ensure that the correct version is sent to the client regardless of
 *                               caching,
 *                               and so should be included if a version number is available and makes sense for the
 *                               script.
 ...
 */
function bbp_enqueue_script($handle = '', $file = '', $dependencies = [], $version = FALSE, $in_footer = 'all') {
	// No file found yet
	$located = FALSE;
	// Trim off any slashes from the template name
	$file = ltrim($file, '/');
 ...
 */
function bbp_get_query_template($type, $templates = []) {
 ...
 */
function bbp_get_template_locations($templates = []) {
	$locations = [
		'bbpress',
		'forums',
		''
 ...
 * @return array()
 */
function bbp_add_template_stack_locations($stacks = []) {
	$retval = [];
...
function bbp_parse_query($posts_query) {
	...
	} elseif (is_post_type_archive([bbp_get_forum_post_type(), bbp_get_topic_post_type()]) && ('topics' === bbp_show_on_root())) {
