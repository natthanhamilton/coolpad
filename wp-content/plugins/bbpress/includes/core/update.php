...
function bbp_is_activation($basename = '') {
	...
	if (empty($action) || !in_array($action, ['activate', 'activate-selected'])) {
		return FALSE;
	}
	// The plugin(s) being activated
	if ($action === 'activate') {
		$plugins = isset($_GET['plugin']) ? [$_GET['plugin']] : [];
	} else {
...
function bbp_is_deactivation($basename = '') {
	...
	if (empty($action) || !in_array($action, ['deactivate', 'deactivate-selected'])) {
		return FALSE;
	}
 ...
 */
function bbp_create_initial_content($args = []) {
