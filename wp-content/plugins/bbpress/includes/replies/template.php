...
function bbp_get_reply_post_type_labels() {
	return apply_filters('bbp_get_reply_post_type_labels', [
...
function bbp_get_reply_post_type_rewrite() {
	return apply_filters('bbp_get_reply_post_type_rewrite', [
...
function bbp_get_reply_post_type_supports() {
	return apply_filters('bbp_get_reply_post_type_supports', [
		'title',
		'editor',
		'revisions'
	]);
...
function bbp_has_replies($args = '') {
	...
	$default_post_type      = (bbp_is_single_topic() && bbp_show_lead_topic()) ? bbp_get_reply_post_type()
		: [bbp_get_topic_post_type(), bbp_get_reply_post_type()];
	...
	if ((int)$bbp->reply_query->found_posts && (int)$bbp->reply_query->posts_per_page) {
		...
		if (TRUE === $r['hierarchical']) {
			...
			$total_pages
			        = ceil((int)$walker->get_number_of_root_elements($bbp->reply_query->posts) / (int)$replies_per_page);
		} else {
			$total_pages = ceil((int)$bbp->reply_query->found_posts / (int)$replies_per_page);
			// Add pagination to query object
			$bbp->reply_query->pagination_links = paginate_links(apply_filters('bbp_replies_pagination', [
				'base'      => $base,
				'format'    => '',
				'total'     => $total_pages,
			...
			if ($wp_rewrite->using_permalinks()) {
				$bbp->reply_query->pagination_links = str_replace($wp_rewrite->pagination_base . '/1/', '',
				                                                  $bbp->reply_query->pagination_links);
			} else {
				$bbp->reply_query->pagination_links = str_replace('&#038;paged=1', '',
				                                                  $bbp->reply_query->pagination_links);
			}
		}
	}
...
function bbp_get_reply_url($reply_id = 0, $redirect_to = '') {
	...
	if (1 >= $reply_page) {
		...
		if ($wp_rewrite->using_permalinks()) {
			$url
				= trailingslashit($topic_url) . trailingslashit($wp_rewrite->pagination_base) . trailingslashit($reply_page) . $reply_hash;
			// Yucky links
		} else {
...
function bbp_reply_content_append_revisions($content = '', $reply_id = 0) {
	...
	return apply_filters('bbp_reply_append_revisions', $content . bbp_get_reply_revision_log($reply_id), $content,
	                     $reply_id);
}
...
function bbp_get_reply_revision_log($reply_id = 0) {
	...
	foreach ((array)$revisions as $revision) {
		...
		$author
			   = bbp_get_author_link(['size' => 14, 'link_text' => bbp_get_reply_author_display_name($revision->ID), 'post_id' => $revision->ID]);
		$since = bbp_get_time_since(bbp_convert_date($revision->post_modified));
		$r .= "\t" . '<li id="bbp-reply-revision-log-' . esc_attr($reply_id) . '-item-' . esc_attr($revision->ID) . '" class="bbp-reply-revision-log-item">' . "\n";
		if (!empty($reason)) {
			$r .= "\t\t" . sprintf(esc_html__('This reply was modified %1$s by %2$s. Reason: %3$s', 'bbpress'),
			                       esc_html($since), $author, esc_html($reason)) . "\n";
		} else {
			$r .= "\t\t" . sprintf(esc_html__('This reply was modified %1$s by %2$s.', 'bbpress'), esc_html($since),
			                       $author) . "\n";
		}
		$r .= "\t" . '</li>' . "\n";
	}
...
function bbp_get_reply_revisions($reply_id = 0) {
	$reply_id  = bbp_get_reply_id($reply_id);
	$revisions = wp_get_post_revisions($reply_id, ['order' => 'ASC']);
 ...
 */
function bbp_reply_author_role($args = []) {
 ...
 */
function bbp_get_reply_author_role($args = []) {
	// Parse arguments against default values
	$r = bbp_parse_args($args, [
		'reply_id' => 0,
	...
	$author_role = sprintf('%1$s<div class="%2$s">%3$s</div>%4$s', $r['before'], esc_attr($r['class']), esc_html($role),
	                       $r['after']);
...
function bbp_get_reply_topic_id($reply_id = 0) {
	...
	if ($reply_id = bbp_get_reply_id($reply_id)) // Get topic_id from reply
	{
		if ($topic_id = get_post_meta($reply_id, '_bbp_topic_id', TRUE)) // Validate the topic_id
		{
...
function bbp_get_reply_forum_id($reply_id = 0) {
	...
	$forum_id = 0;
	// Check that reply_id is valid
	if ($reply_id = bbp_get_reply_id($reply_id)) // Get forum_id from reply
	{
		if ($forum_id = get_post_meta($reply_id, '_bbp_forum_id', TRUE)) // Validate the forum_id
		{
 ...
 */
function bbp_reply_to_link($args = []) {
 ...
 */
function bbp_get_reply_to_link($args = []) {
	// Parse arguments against default values
	$r = bbp_parse_args($args, [
		'id'          => 0,
		'link_before' => '',
	...
	$uri = remove_query_arg(['bbp_reply_to']);
	$uri = add_query_arg(['bbp_reply_to' => $reply->ID]);
	...
	$r['uri'] = $uri;
	$retval
	          = $r['link_before'] . '<a href="' . esc_url($r['uri']) . '" class="bbp-reply-to-link"' . $onclick . '>' . esc_html($r['reply_text']) . '</a>' . $r['link_after'];
...
function bbp_get_cancel_reply_to_link($text = '') {
	...
	$link  = remove_query_arg(['bbp_reply_to', '_wpnonce']) . '#post-' . $reply_to;
	$retval
	       = '<a rel="nofollow" id="bbp-cancel-reply-to-link" href="' . esc_url($link) . '"' . $style . '>' . esc_html($text) . '</a>';
...
function bbp_get_reply_admin_links($args = []) {
	...
	if (in_array($reply_status, [bbp_get_spam_status_id(), bbp_get_trash_status_id()])) {
...
function bbp_get_reply_edit_link($args = '') {
	...
	$retval
		= $r['link_before'] . '<a href="' . esc_url($uri) . '" class="bbp-reply-edit-link">' . $r['edit_text'] . '</a>' . $r['link_after'];
...
function bbp_get_reply_edit_url($reply_id = 0) {
	...
	if ($wp_rewrite->using_permalinks()) {
		...
		$url = add_query_arg([bbp_get_reply_post_type() => $reply->post_name, $bbp->edit_id => '1'], $reply_link);
	}
	// Maybe add view all
	$url = bbp_add_view_all($url);
...
function bbp_get_reply_trash_link($args = '') {
	...
	if (bbp_is_reply_trash($reply->ID)) {
		$actions['untrash'] = '<a title="' . esc_attr__('Restore this item from the Trash',
		                                                'bbpress') . '" href="' . esc_url(wp_nonce_url(add_query_arg(['action' => 'bbp_toggle_reply_trash', 'sub_action' => 'untrash', 'reply_id' => $reply->ID]),
		                                                                                               'untrash-' . $reply->post_type . '_' . $reply->ID)) . '" class="bbp-reply-restore-link">' . $r['restore_text'] . '</a>';
	} elseif (EMPTY_TRASH_DAYS) {
		$actions['trash'] = '<a title="' . esc_attr__('Move this item to the Trash',
		                                              'bbpress') . '" href="' . esc_url(wp_nonce_url(add_query_arg(['action' => 'bbp_toggle_reply_trash', 'sub_action' => 'trash', 'reply_id' => $reply->ID]),
		                                                                                             'trash-' . $reply->post_type . '_' . $reply->ID)) . '" class="bbp-reply-trash-link">' . $r['trash_text'] . '</a>';
	}
	if (bbp_is_reply_trash($reply->ID) || !EMPTY_TRASH_DAYS) {
		$actions['delete'] = '<a title="' . esc_attr__('Delete this item permanently',
		                                               'bbpress') . '" href="' . esc_url(wp_nonce_url(add_query_arg(['action' => 'bbp_toggle_reply_trash', 'sub_action' => 'delete', 'reply_id' => $reply->ID]),
		                                                                                              'delete-' . $reply->post_type . '_' . $reply->ID)) . '" onclick="return confirm(\'' . esc_js(__('Are you sure you want to delete that permanently?',
		                                                                                                                                                                                              'bbpress')) . '\' );" class="bbp-reply-delete-link">' . $r['delete_text'] . '</a>';
	}
	// Process the admin links
...
function bbp_get_reply_spam_link($args = '') {
	...
	$uri     = add_query_arg(['action' => 'bbp_toggle_reply_spam', 'reply_id' => $reply->ID]);
	$uri     = wp_nonce_url($uri, 'spam-reply_' . $reply->ID);
	$retval
	         = $r['link_before'] . '<a href="' . esc_url($uri) . '" class="bbp-reply-spam-link">' . $display . '</a>' . $r['link_after'];
...
function bbp_get_reply_move_link($args = '') {
	...
	$retval
		= $r['link_before'] . '<a href="' . esc_url($uri) . '" title="' . $r['split_title'] . '" class="bbp-reply-move-link">' . $r['split_text'] . '</a>' . $r['link_after'];
...
function bbp_get_topic_split_link($args = '') {
	...
	$retval
		= $r['link_before'] . '<a href="' . esc_url($uri) . '" title="' . $r['split_title'] . '" class="bbp-topic-split-link">' . $r['split_text'] . '</a>' . $r['link_after'];
 ...
 */
function bbp_reply_class($reply_id = 0, $classes = []) {
 ...
 */
function bbp_get_reply_class($reply_id = 0, $classes = []) {
	$bbp       = bbpress();
	$reply_id  = bbp_get_reply_id($reply_id);
	...
	$classes[] = (bbp_get_reply_author_id($reply_id) === bbp_get_topic_author_id(bbp_get_reply_topic_id($reply_id))
		? 'topic-author' : '');
...
function bbp_get_topic_pagination_count() {
	...
	$to_num
	           = bbp_number_format(($start_num + ($bbp->reply_query->posts_per_page - 1) > $bbp->reply_query->found_posts)
		                               ? $bbp->reply_query->found_posts
		                               : $start_num + ($bbp->reply_query->posts_per_page - 1));
	...
	if (bbp_thread_replies() && bbp_is_single_topic()) {
		...
		// Adjust for topic
		$threads--;
		$retstr = sprintf(_n('Viewing %1$s reply thread', 'Viewing %1$s reply threads', $threads, 'bbbpress'),
		                  bbp_number_format($threads));
		...
		if (empty($to_num)) {
			...
			$retstr = sprintf(_n('Viewing %2$s replies (of %4$s total)',
			                     'Viewing %1$s replies - %2$s through %3$s (of %4$s total)',
			                     $bbp->reply_query->post_count, 'bbpress'), $bbp->reply_query->post_count, $from_num,
			                  $to_num, $total);
		}
		...
		if (empty($to_num)) {
			...
			$retstr = sprintf(_n('Viewing %2$s post (of %4$s total)',
