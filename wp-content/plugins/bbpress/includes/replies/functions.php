 ...
 */
function bbp_insert_reply($reply_data = [], $reply_meta = []) {
	// Forum
	$reply_data = bbp_parse_args($reply_data, [
	...
	// Forum meta
	$reply_meta = bbp_parse_args($reply_meta, [
...
function bbp_new_reply_handler($action = '') {
	...
	if (!bbp_verify_nonce_request('bbp-new-reply')) {
		bbp_add_error('bbp_new_reply_nonce',
	...
	if (bbp_is_anonymous()) {
		...
		if (!current_user_can('publish_replies')) {
			bbp_add_error('bbp_reply_permissions',
			              __('<strong>ERROR</strong>: You do not have permission to reply.', 'bbpress'));
		}
	...
	if (empty($_POST['bbp_topic_id'])) {
		...
		if (0 > $posted_topic_id) {
			bbp_add_error('bbp_reply_topic_id',
			              __('<strong>ERROR</strong>: Topic ID cannot be a negative number.', 'bbpress'));
	...
	if (!empty($forum_id)) {
		...
		if (bbp_is_forum_category($forum_id)) {
			bbp_add_error('bbp_new_reply_forum_category',
			              __('<strong>ERROR</strong>: This forum is a category. No replies can be created in this forum.',
			                 'bbpress'));
			// Forum is not a category
		} else {
			// Forum is closed and user cannot access
			if (bbp_is_forum_closed($forum_id) && !current_user_can('edit_forum', $forum_id)) {
				bbp_add_error('bbp_new_reply_forum_closed',
				              __('<strong>ERROR</strong>: This forum has been closed to new replies.', 'bbpress'));
			}
			// Forum is private and user cannot access
			if (bbp_is_forum_private($forum_id)) {
				if (!current_user_can('read_private_forums')) {
					bbp_add_error('bbp_new_reply_forum_private',
					              __('<strong>ERROR</strong>: This forum is private and you do not have the capability to read or create new replies in it.',
					                 'bbpress'));
				}
				// Forum is hidden and user cannot access
			} elseif (bbp_is_forum_hidden($forum_id)) {
				if (!current_user_can('read_hidden_forums')) {
					bbp_add_error('bbp_new_reply_forum_hidden',
					              __('<strong>ERROR</strong>: This forum is hidden and you do not have the capability to read or create new replies in it.',
					                 'bbpress'));
				}
			}
		}
	...
	/** Reply Duplicate *******************************************************/
	if (!bbp_check_for_duplicate(['post_type' => bbp_get_reply_post_type(), 'post_author' => $reply_author, 'post_content' => $reply_content, 'post_parent' => $topic_id, 'anonymous_data' => $anonymous_data])) {
		bbp_add_error('bbp_reply_duplicate',
		              __('<strong>ERROR</strong>: Duplicate reply detected; it looks as though you&#8217;ve already said that!',
		                 'bbpress'));
	}
	/** Reply Blacklist *******************************************************/
	if (!bbp_check_for_blacklist($anonymous_data, $reply_author, $reply_title, $reply_content)) {
		bbp_add_error('bbp_reply_blacklist',
		              __('<strong>ERROR</strong>: Your reply cannot be created at this time.', 'bbpress'));
	}
	/** Reply Status **********************************************************/
	...
	$reply_data = apply_filters('bbp_new_reply_pre_insert', [
	...
	if (!empty($reply_id) && !is_wp_error($reply_id)) {
		...
		if (is_wp_error($terms)) {
			bbp_add_error('bbp_reply_tags',
			              __('<strong>ERROR</strong>: There was a problem adding the tags to the topic.', 'bbpress'));
		}
		/** Trash Check *******************************************************/
		...
		bbp_add_error('bbp_reply_error',
		              __('<strong>ERROR</strong>: The following problem(s) have been found with your reply:' . $append_error . 'Please try again.',
		                 'bbpress'));
	}
}
...
function bbp_edit_reply_handler($action = '') {
	...
	if (!bbp_verify_nonce_request('bbp-edit-reply_' . $reply_id)) {
		bbp_add_error('bbp_edit_reply_nonce',
		              __('<strong>ERROR</strong>: Are you sure you wanted to do that?', 'bbpress'));
	...
	if (empty($reply)) {
		bbp_add_error('bbp_edit_reply_not_found',
		              __('<strong>ERROR</strong>: The reply you want to edit was not found.', 'bbpress'));
		...
		if (!bbp_is_reply_anonymous($reply_id)) {
			...
			if (!current_user_can('edit_reply', $reply_id)) {
				bbp_add_error('bbp_edit_reply_permissions',
				              __('<strong>ERROR</strong>: You do not have permission to edit that reply.', 'bbpress'));
	...
	if (!empty($forum_id) && ($forum_id !== bbp_get_reply_forum_id($reply_id))) {
		// Forum is a category
		if (bbp_is_forum_category($forum_id)) {
			bbp_add_error('bbp_edit_reply_forum_category',
			              __('<strong>ERROR</strong>: This forum is a category. No replies can be created in this forum.',
			                 'bbpress'));
			// Forum is not a category
		} else {
			// Forum is closed and user cannot access
			if (bbp_is_forum_closed($forum_id) && !current_user_can('edit_forum', $forum_id)) {
				bbp_add_error('bbp_edit_reply_forum_closed',
				              __('<strong>ERROR</strong>: This forum has been closed to new replies.', 'bbpress'));
			}
			// Forum is private and user cannot access
			if (bbp_is_forum_private($forum_id)) {
				if (!current_user_can('read_private_forums')) {
					bbp_add_error('bbp_edit_reply_forum_private',
					              __('<strong>ERROR</strong>: This forum is private and you do not have the capability to read or create new replies in it.',
					                 'bbpress'));
				}
				// Forum is hidden and user cannot access
				...
				if (!current_user_can('read_hidden_forums')) {
					bbp_add_error('bbp_edit_reply_forum_hidden',
					              __('<strong>ERROR</strong>: This forum is hidden and you do not have the capability to read or create new replies in it.',
					                 'bbpress'));
				}
			}
	...
	if (!bbp_check_for_blacklist($anonymous_data, $reply_author, $reply_title, $reply_content)) {
		bbp_add_error('bbp_reply_blacklist',
		              __('<strong>ERROR</strong>: Your reply cannot be edited at this time.', 'bbpress'));
	}
	/** Reply Status **********************************************************/
	// Maybe put into moderation
	if (!bbp_check_for_moderation($anonymous_data, $reply_author, $reply_title, $reply_content)) {
	...
	$reply_data = apply_filters('bbp_edit_reply_pre_insert', [
		'ID'           => $reply_id,
		'post_title'   => $reply_title,
		'post_content' => $reply_content,
		'post_status'  => $reply_status,
 ...
 */
function bbp_update_reply($reply_id = 0, $topic_id = 0, $forum_id = 0, $anonymous_data = FALSE, $author_id = 0,
                          $is_edit = FALSE, $reply_to = 0) {
...
function bbp_update_reply_walker($reply_id, $last_active_time = '', $forum_id = 0, $topic_id = 0, $refresh = TRUE) {
	...
	$ancestors = array_values(array_unique(array_merge([$topic_id, $forum_id], (array)get_post_ancestors($topic_id))));
...
function bbp_move_reply_handler($action = '') {
	...
	// Reply exists
	if (empty($move_reply)) {
		bbp_add_error('bbp_mover_reply_r_not_found',
		              __('<strong>ERROR</strong>: The reply you want to move was not found.', 'bbpress'));
	}
	/** Topic to Move From ***************************************************/
	...
	if (empty($source_topic)) {
		bbp_add_error('bbp_move_reply_source_not_found',
		              __('<strong>ERROR</strong>: The topic you want to move from was not found.', 'bbpress'));
	}
	// Nonce check failed
	if (!bbp_verify_nonce_request('bbp-move-reply_' . $move_reply->ID)) {
		bbp_add_error('bbp_move_reply_nonce',
		              __('<strong>ERROR</strong>: Are you sure you wanted to do that?', 'bbpress'));
	...
	if (!current_user_can('edit_topic', $source_topic->ID)) {
		bbp_add_error('bbp_move_reply_source_permission',
	...
	// Invalid move option
	if (empty($move_option) || !in_array($move_option, ['existing', 'topic'])) {
		bbp_add_error('bbp_move_reply_option',
		              __('<strong>ERROR</strong>: You need to choose a valid move option.', 'bbpress'));
		// Valid move option
	} else {
		// What kind of move
		switch ($move_option) {
			...
			case 'existing' :
				...
				if (empty($_POST['bbp_destination_topic'])) {
					bbp_add_error('bbp_move_reply_destination_id',
				...
				if (empty($destination_topic)) {
					bbp_add_error('bbp_move_reply_destination_not_found',
					              __('<strong>ERROR</strong>: The topic you want to move to was not found!',
					                 'bbpress'));
				}
				// User cannot edit the destination topic
				if (!current_user_can('edit_topic', $destination_topic->ID)) {
					bbp_add_error('bbp_move_reply_destination_permission',
				...
				wp_update_post([
					               'ID'          => $move_reply->ID,
					               'post_title'  => sprintf(__('Reply To: %s', 'bbpress'),
			...
			default :
				...
				if (current_user_can('publish_topics')) {
					...
					if (FALSE === $destination_topic_id || is_wp_error($destination_topic_id) || empty($destination_topic)) {
						bbp_add_error('bbp_move_reply_destination_reply',
...
function bbp_toggle_reply_handler($action = '') {
	...
	if (!current_user_can('edit_reply',
	                      $reply->ID) || ('bbp_toggle_reply_trash' === $action && !current_user_can('delete_reply',
	                                                                                                $reply->ID))
	) {
		bbp_add_error('bbp_toggle_reply_permission',
	...
	switch ($action) {
		...
		case 'bbp_toggle_reply_spam' :
			...
			$failure  = $is_spam ? __('<strong>ERROR</strong>: There was a problem unmarking the reply as spam!',
			                          'bbpress')
				: __('<strong>ERROR</strong>: There was a problem marking the reply as spam!', 'bbpress');
			$view_all = !$is_spam;
			break;
		// Toggle trash
		case 'bbp_toggle_reply_trash' :
			$sub_action = in_array($_GET['sub_action'], ['trash', 'untrash', 'delete']) ? $_GET['sub_action'] : FALSE;
			if (empty($sub_action)) {
				break;
			}
			switch ($sub_action) {
...
function bbp_reply_content_autoembed() {
	...
	if (bbp_use_autoembed() && is_a($wp_embed, 'WP_Embed')) {
		add_filter('bbp_get_reply_content', [$wp_embed, 'autoembed'], 2);
	}
}
...
function _bbp_has_replies_where($where = '', $query = FALSE) {
	...
	if ([bbp_get_topic_post_type(), bbp_get_reply_post_type()] !== $query->get('post_type')) {
		return $where;
	}
 ...
 */
function bbp_display_replies_feed_rss2($replies_query = []) {
	// User cannot access forum this topic is in
	if (bbp_is_single_topic() && !bbp_user_can_view_forum(['forum_id' => bbp_get_topic_forum_id()])) {
		return;
	}
 ...
 * @since bbPress (r4944)
 */
function bbp_list_replies($args = []) {
