...
function bbp_has_search_results($args = '') {
	global $wp_rewrite;
	/** Defaults **************************************************************/
	$default_post_type = [bbp_get_forum_post_type(), bbp_get_topic_post_type(), bbp_get_reply_post_type()];
	// Default query args
	$default = [
	...
	if (!empty($bbp->search_query->found_posts) && !empty($bbp->search_query->posts_per_page)) {
		...
		$bbp->search_query->pagination_links = paginate_links(
			apply_filters('bbp_search_results_pagination', [
				'base'      => $base,
				'format'    => '',
		...
		if ($wp_rewrite->using_permalinks()) {
			$bbp->search_query->pagination_links = str_replace($wp_rewrite->pagination_base . '/1/', '',
			                                                   $bbp->search_query->pagination_links);
		} else {
			$bbp->search_query->pagination_links = str_replace('&#038;paged=1', '',
			                                                   $bbp->search_query->pagination_links);
		}
	}
...
function bbp_get_search_url() {
	...
	if ($wp_rewrite->using_permalinks()) {
		...
		$url = add_query_arg([bbp_get_search_rewrite_id() => ''], home_url('/'));
	}
...
function bbp_get_search_results_url() {
	...
	if ($wp_rewrite->using_permalinks()) {
		...
		$url = add_query_arg([bbp_get_search_rewrite_id() => urlencode($search_terms)], home_url('/'));
	}
...
function bbp_get_search_pagination_count() {
	...
	$to_num
	           = bbp_number_format(($start_num + ($bbp->search_query->posts_per_page - 1) > $bbp->search_query->found_posts)
		                               ? $bbp->search_query->found_posts
		                               : $start_num + ($bbp->search_query->posts_per_page - 1));
	...
	if (empty($to_num)) {
		...
		$retstr = sprintf(_n('Viewing %2$s results (of %4$s total)',
		                     'Viewing %1$s results - %2$s through %3$s (of %4$s total)', $bbp->search_query->post_count,
		                     'bbpress'), $bbp->search_query->post_count, $from_num, $to_num, $total);
	}
