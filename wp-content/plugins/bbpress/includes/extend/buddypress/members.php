...
if (!class_exists('BBP_Forums_Members')) :
	...
	class BBP_BuddyPress_Members {
		...
		private function setup_actions() {
			...
			add_action('bp_template_redirect', [$this, 'set_member_forum_query_vars']);
			/** Favorites *********************************************************/
		...
		private function setup_filters() {
			add_filter('bbp_pre_get_user_profile_url', [$this, 'user_profile_url']);
			add_filter('bbp_get_favorites_permalink', [$this, 'get_favorites_permalink'], 10, 2);
			add_filter('bbp_get_subscriptions_permalink', [$this, 'get_subscriptions_permalink'], 10, 2);
		}
		...
		public function user_profile_url($user_id) {
			...
			if (bp_is_current_component($component_slug)) {
				...
				if (!bp_current_action() || bp_is_current_action(bbp_get_topic_archive_slug())) {
					$profile_url
						= bp_core_get_user_domain($user_id) . $component_slug . '/' . bbp_get_topic_archive_slug();
					// Empty action or 'topics' action
				} elseif (bp_is_current_action(bbp_get_reply_archive_slug())) {
					$profile_url
						= bp_core_get_user_domain($user_id) . $component_slug . '/' . bbp_get_reply_archive_slug();
					// 'favorites' action
		...
		public function get_favorites_permalink($url, $user_id) {
			$component_slug = bbpress()->extend->buddypress->slug;
			$url
			                = trailingslashit(bp_core_get_user_domain($user_id) . $component_slug . '/' . bbp_get_user_favorites_slug());
		...
		public function get_subscriptions_permalink($url, $user_id) {
			$component_slug = bbpress()->extend->buddypress->slug;
			$url
			                = trailingslashit(bp_core_get_user_domain($user_id) . $component_slug . '/' . bbp_get_user_subscriptions_slug());
