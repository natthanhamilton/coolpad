...
if (!class_exists('BBP_Forums_Group_Extension') && class_exists('BP_Group_Extension')) :
	...
	class BBP_Forums_Group_Extension extends BP_Group_Extension {
		...
		private function setup_actions() {
			// Possibly redirect
			add_action('bbp_template_redirect', [$this, 'redirect_canonical']);
			// Remove group forum cap map when view is done
			add_action('bbp_after_group_forum_display', [$this, 'remove_group_forum_meta_cap_map']);
			// bbPress needs to listen to BuddyPress group deletion.
			add_action('groups_before_delete_group', [$this, 'disconnect_forum_from_group']);
			// Adds a bbPress metabox to the new BuddyPress Group Admin UI
			add_action('bp_groups_admin_meta_boxes', [$this, 'group_admin_ui_edit_screen']);
			// Saves the bbPress options if they come from the BuddyPress Group Admin UI
			add_action('bp_group_admin_edit_after', [$this, 'edit_screen_save']);
			// Adds a hidden input value to the "Group Settings" page
			add_action('bp_before_group_settings_admin', [$this, 'group_settings_hidden_field']);
		}
		...
		private function setup_filters() {
			...
			add_filter('bbp_topic_pagination', [$this, 'topic_pagination']);
			add_filter('bbp_replies_pagination', [$this, 'replies_pagination']);
			// Tweak the redirect field
			add_filter('bbp_new_topic_redirect_to', [$this, 'new_topic_redirect_to'], 10, 3);
			add_filter('bbp_new_reply_redirect_to', [$this, 'new_reply_redirect_to'], 10, 3);
			// Map forum/topic/replys permalinks to their groups
			add_filter('bbp_get_forum_permalink', [$this, 'map_forum_permalink_to_group'], 10, 2);
			add_filter('bbp_get_topic_permalink', [$this, 'map_topic_permalink_to_group'], 10, 2);
			add_filter('bbp_get_reply_permalink', [$this, 'map_reply_permalink_to_group'], 10, 2);
			// Map reply edit links to their groups
			add_filter('bbp_get_reply_edit_url', [$this, 'map_reply_edit_url_to_group'], 10, 2);
			...
			add_filter('bbp_before_record_activity_parse_args', [$this, 'map_activity_to_group']);
			/** Caps **************************************************************/
			...
			if (bp_is_single_item() && bp_is_groups_component() && bp_is_current_action('forum')) {
				...
				add_filter('bbp_map_meta_caps', [$this, 'map_group_forum_meta_caps'], 10, 4);
				// Group member permissions to view the topic and reply forms
				add_filter('bbp_current_user_can_access_create_topic_form', [$this, 'form_permissions']);
				add_filter('bbp_current_user_can_access_create_reply_form', [$this, 'form_permissions']);
			}
		}
		...
		public function display($group_id = NULL) {
			...
			add_action('bbp_theme_before_topic_form_forum', [$this, 'ob_start']);
			add_action('bbp_theme_after_topic_form_forum', [$this, 'ob_end_clean']);
			add_action('bbp_theme_after_topic_form_forum', [$this, 'topic_parent']);
			// Prevent Forum Parent from appearing
			add_action('bbp_theme_before_forum_form_parent', [$this, 'ob_start']);
			add_action('bbp_theme_after_forum_form_parent', [$this, 'ob_end_clean']);
			add_action('bbp_theme_after_forum_form_parent', [$this, 'forum_parent']);
			// Hide breadcrumb
		 ...
		 * @return array
		 */
		public function map_group_forum_meta_caps($caps = [], $cap = '', $user_id = 0, $args = []) {
			switch ($cap) {
		...
		public function remove_group_forum_meta_cap_map() {
			remove_filter('bbp_map_meta_caps', [$this, 'map_group_forum_meta_caps'], 99, 4);
		}
		...
		public function edit_screen($group = FALSE) {
			...
			$checked = is_admin() ? bp_group_is_forum_enabled($group)
				: bp_get_new_group_enable_forum() || bp_group_is_forum_enabled(bp_get_group_id()); ?>
			...
			<fieldset>
				...
				<p><?php esc_html_e('Create a discussion forum to allow members of this group to communicate in a structured, bulletin-board style fashion.',
				                    'bbpress'); ?></p>

				<div class="field-group">
					<div class="checkbox">
						<label><input type="checkbox" name="bbp-edit-group-forum" id="bbp-edit-group-forum"
						              value="1"<?php checked($checked); ?> /> <?php esc_html_e('Yes. I want this group to have a forum.',
						                                                                       'bbpress'); ?></label>
					</div>

					<p class="description"><?php esc_html_e('Saying no will not delete existing forum content.',
					                                        'bbpress'); ?></p>
				</div>
				...
				<?php if (bbp_is_user_keymaster()) : ?>
					<div class="field-group">
						...
						?>
						<p class="description"><?php esc_html_e('Network administrators can reconfigure which forum belongs to this group.',
						                                        'bbpress'); ?></p>
					</div>
				<?php endif; ?>
		...
		public function edit_screen_save($group_id = 0) {
			...
			if (is_admin()) {
				...
				bbp_add_error('bbp_edit_group_forum_screen_save',
			...
			if (bbp_is_user_keymaster()) {
				$forum_ids = !empty($_POST['bbp_group_forum_id']) ? (array)(int)$_POST['bbp_group_forum_id'] : [];
			...
			if (!empty($forum_ids)) {
				...
				foreach ($forum_ids as $forum_id) {
					...
					if (empty($forum)) {
						$this->remove_forum(['forum_id' => $forum_id]);
			...
			if (empty($forum_id) && (TRUE === $edit_forum)) {
				...
				$new_forum_args = ['forum_id' => $forum_id];
		...
		public function group_admin_ui_edit_screen() {
			add_meta_box(
				...
				[$this, 'group_admin_ui_display_metabox'],
		...
		public function create_screen($group_id = 0) {
			...
			<div class="checkbox">
				<label><input type="checkbox" name="bbp-create-group-forum" id="bbp-create-group-forum"
				              value="1"<?php checked($checked); ?> /> <?php esc_html_e('Yes. I want this group to have a forum.',
				                                                                       'bbpress'); ?></label>
			</div>
		...
		public function create_screen_save($group_id = 0) {
			...
			if (!bbp_verify_nonce_request('groups_create_save_' . $this->slug)) {
				bbp_add_error('bbp_create_group_forum_screen_save',
				              __('<strong>ERROR</strong>: Are you sure you wanted to do that?', 'bbpress'));
			...
			switch ($create_forum) {
				case TRUE  :
					...
					// Run the BP-specific functions for new groups
					$this->new_forum(['forum_id' => $forum_id]);
					// Update forum active
		 ...
		 * @return if no forum_id is available
		 */
		public function remove_forum($forum_args = []) {
			// Bail if no forum_id was passed
			if (empty($forum_args['forum_id'])) {
				return;
			}
			...
			<div id="bbpress-forums">
				...
				<?php switch ($forum_action) :
					...
					case 'page' :
						...
						add_filter('bbp_get_super_stickies', [$this, 'no_super_stickies'], 10, 1);
						// Unset the super sticky option on topic form
						add_filter('bbp_get_topic_types', [$this, 'unset_super_sticky'], 10, 1);
					...
					case $this->topic_slug :
						...
						add_filter('bbp_get_topic_stick_link', [$this, 'hide_super_sticky_admin_link'], 10, 2);
						// Get the topic
						bbp_has_topics([
		 ...
		 * @return array (empty)
		 */
		public function no_super_stickies($super = []) {
			$super = [];
		...
		public function new_topic_redirect_to($redirect_url = '', $redirect_to = '', $topic_id = 0) {
			if (bp_is_group()) {
				...
				$redirect_url
				            = trailingslashit(bp_get_group_permalink(groups_get_current_group())) . trailingslashit($this->slug) . trailingslashit($this->topic_slug) . trailingslashit($topic->post_name) . $topic_hash;
			}
		...
		public function new_reply_redirect_to($redirect_url = '', $redirect_to = '', $reply_id = 0) {
			...
			if (bp_is_group()) {
				...
				$topic_url
				                = trailingslashit(bp_get_group_permalink(groups_get_current_group())) . trailingslashit($this->slug) . trailingslashit($this->topic_slug) . trailingslashit($topic->post_name);
		...
		public function edit_redirect_to($redirect_url = '') {
			...
			if (bp_is_group_admin_screen($this->slug)) {
				$redirect_url
					= trailingslashit(bp_get_root_domain() . '/' . bp_get_groups_root_slug() . '/' . $group->slug . '/admin/' . $this->slug);
			}
		...
		public function forum_parent() {
			...
			<input type="hidden" name="bbp_forum_parent_id" id="bbp_forum_parent_id"
			       value="<?php bbp_group_forums_root_id(); ?>"/>
		...
		public function topic_parent() {
			...
			<p>
				...
				<?php bbp_dropdown(['include' => $forum_ids, 'selected' => bbp_get_form_topic_forum()]); ?>
			</p>
		 ...
		 * @return array
		 */
		public function map_activity_to_group($args = []) {
