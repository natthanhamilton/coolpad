...
if (!class_exists('BBP_BuddyPress_Activity')) :
	...
	class BBP_BuddyPress_Activity {
		...
		private function setup_actions() {
			...
			add_action('bbp_new_topic', [$this, 'topic_create'], 10, 4);
			add_action('bbp_new_reply', [$this, 'reply_create'], 10, 5);
			// Hook into topic and reply status changes
			add_action('edit_post', [$this, 'topic_update'], 10, 2);
			add_action('edit_post', [$this, 'reply_update'], 10, 2);
			// Hook into topic and reply deletion
			add_action('bbp_delete_topic', [$this, 'topic_delete'], 10, 1);
			add_action('bbp_delete_reply', [$this, 'reply_delete'], 10, 1);
			// Append forum filters in site wide activity streams
			add_action('bp_activity_filter_options', [$this, 'activity_filter_options'], 10);
			// Append forum filters in single member activity streams
			add_action('bp_member_activity_filter_options', [$this, 'activity_filter_options'], 10);
			// Append forum filters in single group activity streams
			add_action('bp_group_activity_filter_options', [$this, 'activity_filter_options'], 10);
		}
		...
		private function setup_filters() {
			...
			add_filter('bp_activity_can_comment', [$this, 'activity_can_comment']);
			// Link directly to the topic or reply
			add_filter('bp_activity_get_permalink', [$this, 'activity_get_permalink'], 10, 2);
		}
		...
		private function fully_loaded() {
			do_action_ref_array('bbp_buddypress_activity_loaded', [$this]);
		}
		...
		private static function get_activity_id($post_id = 0) {
			...
			$existing
				= bp_activity_get_specific(['activity_ids' => $activity_id, 'show_hidden' => TRUE, 'spam' => 'all',]);
		 ...
		 */
		public function topic_create($topic_id = 0, $forum_id = 0, $anonymous_data = [], $topic_author_id = 0) {
			...
			$activity_text    = sprintf(esc_html__('%1$s started the topic %2$s in the forum %3$s', 'bbpress'),
			                            $user_link, $topic_link, $forum_link);
			$activity_action  = apply_filters('bbp_activity_topic_create', $activity_text, $user_id, $topic_id,
			                                  $forum_id);
			...
			$activity_id = $this->record_activity([
				                                      ...
				                                      'recorded_time'     => get_post_time('Y-m-d H:i:s', TRUE,
				                                                                           $topic_id),
		...
		public function topic_delete($topic_id = 0) {
			...
			if ($activity_id = $this->get_activity_id($topic_id)) {
				return bp_activity_delete(['id' => $activity_id]);
			}
		...
		public function topic_update($topic_id = 0, $post = NULL) {
			...
			if (in_array($post->post_status, [bbp_get_public_status_id(), bbp_get_closed_status_id()])) {
				// Validate topic data
				$forum_id        = bbp_get_topic_forum_id($topic_id);
				$topic_author_id = bbp_get_topic_author_id($topic_id);
				$this->topic_create($topic_id, $forum_id, [], $topic_author_id);
			} else {
		 ...
		 */
		public function reply_create($reply_id = 0, $topic_id = 0, $forum_id = 0, $anonymous_data = [],
		                             $reply_author_id = 0) {
			...
			$forum_link      = '<a href="' . $forum_permalink . '">' . $forum_title . '</a>';
			// Activity action & text
			$activity_text    = sprintf(esc_html__('%1$s replied to the topic %2$s in the forum %3$s', 'bbpress'),
			                            $user_link, $topic_link, $forum_link);
			$activity_action  = apply_filters('bbp_activity_reply_create', $activity_text, $user_id, $reply_id,
			                                  $topic_id);
		...
		public function reply_update($reply_id, $post) {
			...
			if (bbp_get_public_status_id() === $post->post_status) {
				...
				$this->reply_create($reply_id, $topic_id, $forum_id, [], $reply_author_id);
			} else {
