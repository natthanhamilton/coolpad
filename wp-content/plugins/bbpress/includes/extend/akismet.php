	...
	class BBP_Akismet {
		...
		private function setup_actions() {
			...
			foreach ($checks as $type => $functions) {
				foreach ($functions as $function => $priority) {
					add_filter($function, [$this, $type . '_post'], $priority);
				}
			}
			// Update post meta
			add_action('wp_insert_post', [$this, 'update_post_meta'], 10, 2);
			// Admin
			if (is_admin()) {
		...
		public function check_post($post_data) {
			...
			if ('true' === $post_data['bbp_akismet_result']) {
				...
				add_filter('bbp_new_reply_pre_set_terms', [$this, 'filter_post_terms'], 1, 3);
				// @todo Spam counter?
			}
		...
		public function submit_post($post_id = 0) {
			...
			$post_data = [
				...
				'comment_author'       => $_post->post_author ? get_the_author_meta('display_name', $_post->post_author)
					: get_post_meta($post_id, '_bbp_anonymous_name', TRUE),
				'comment_author_email' => $_post->post_author ? get_the_author_meta('email', $_post->post_author)
					: get_post_meta($post_id, '_bbp_anonymous_email', TRUE),
				'comment_author_url'   => $_post->post_author ? bbp_get_user_profile_url($_post->post_author)
					: get_post_meta($post_id, '_bbp_anonymous_website', TRUE),
				...
				switch ($request_type) {
					...
					case 'spam' :
						$this->update_post_history($post_id,
						                           sprintf(esc_html__('%1$s reported this %2$s as spam', 'bbpress'),
						                                   $post_data['reporter'], $post_data['comment_type']),
						                           'report-spam');
					...
					case 'ham'  :
						$this->update_post_history($post_id,
						                           sprintf(esc_html__('%1$s reported this %2$s as not spam', 'bbpress'),
						                                   $post_data['reporter'], $post_data['comment_type']),
						                           'report-ham');
		...
		public function update_post_meta($post_id = 0, $_post = FALSE) {
			...
			if (is_object($_post) && !empty($this->last_post) && is_array($as_submitted)) {
				...
				if (intval($as_submitted['comment_post_ID']) === intval($_post->post_parent)
				    && $as_submitted['comment_author'] === ($anonymous_data ? $anonymous_data['bbp_anonymous_name']
						: $userdata->display_name)
				    && $as_submitted['comment_author_email'] === ($anonymous_data
						? $anonymous_data['bbp_anonymous_email'] : $userdata->user_email)
				) {
					// Normal result: true
					if ($this->last_post['bbp_akismet_result'] === 'true') {
						...
						$this->update_post_history($post_id, esc_html__('Akismet caught this post as spam', 'bbpress'),
						                           'check-spam');
						// If post_status isn't the spam status, as expected, leave a note
						if (bbp_get_spam_status_id() !== $_post->post_status) {
							$this->update_post_history($post_id,
							                           sprintf(esc_html__('Post status was changed to %s', 'bbpress'),
							                                   $_post->post_status),
						...
						$this->update_post_history($post_id,
						                           sprintf(esc_html__('Akismet was unable to check this post (response: %s), will automatically retry again later.',
						                                              'bbpress'),
						                                   $this->last_post['bbp_akismet_result']), 'check-error');
					}
		...
		public function filter_post_terms($terms = '', $topic_id = 0, $reply_id = 0) {
			...
			$existing_terms = wp_get_object_terms($topic_id, bbp_get_topic_tag_tax_id(), ['fields' => 'names']);
						...
						<?php foreach ($history as $row) : ?>
							...
							<tr>
								<td style="color: #999; text-align: right; white-space: nowrap;">
									<span title="<?php echo esc_attr(date('D d M Y @ h:i:m a',
									                                      $row['time']) . ' GMT'); ?>">
