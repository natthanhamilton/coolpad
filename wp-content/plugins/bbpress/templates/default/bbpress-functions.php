...
if (!class_exists('BBP_Default')) :
	...
	class BBP_Default extends BBP_Theme_Compat {
		...
		public function __construct($properties = []) {
			parent::__construct(bbp_parse_args($properties, [
				'id'      => 'default',
		...
		private function setup_actions() {
			...
			add_action('bbp_ajax_favorite', [$this, 'ajax_favorite']); // Handles the topic ajax favorite/unfavorite
			add_action('bbp_ajax_subscription',
			           [$this, 'ajax_subscription']); // Handles the topic ajax subscribe/unsubscribe
			add_action('bbp_ajax_forum_subscription',
			           [$this, 'ajax_forum_subscription']); // Handles the forum ajax subscribe/unsubscribe
			/** Template Wrappers *************************************************/
			add_action('bbp_before_main_content', [$this, 'before_main_content']); // Top wrapper HTML
			add_action('bbp_after_main_content', [$this, 'after_main_content']); // Bottom wrapper HTML
			/** Override **********************************************************/
		...
		public function enqueue_styles() {
			...
			// RTL helpers
			if (is_rtl()) {
				$styles['bbp-default-rtl'] = [
					'file'         => 'css/bbpress-rtl.css',
					'dependencies' => ['bbp-default']
				];
			}
			// Filter the scripts
		...
		public function enqueue_scripts() {
			...
			if (bbp_is_single_topic()) {
				...
				$scripts['bbpress-topic'] = [
					'file'         => 'js/topic.js',
					'dependencies' => ['jquery']
				];
				// Hierarchical replies
				if (bbp_thread_replies()) {
					$scripts['bbpress-reply'] = [
						'file'         => 'js/reply.js',
						'dependencies' => ['jquery']
					];
				}
		...
		public function localize_topic_script() {
			// Single forum
			if (bbp_is_single_forum()) {
				wp_localize_script('bbpress-forum', 'bbpForumJS', [
				...
				wp_localize_script('bbpress-topic', 'bbpTopicJS', [
					'bbp_ajaxurl'        => bbp_get_ajax_url(),
					'generic_ajax_error' => __('Something went wrong. Refresh your browser and try again.', 'bbpress'),
					'is_user_logged_in'  => is_user_logged_in(),
		...
		public function ajax_forum_subscription() {
			...
			// Take action
			$status = bbp_is_user_subscribed($user_id, $forum->ID) ? bbp_remove_user_subscription($user_id, $forum->ID)
		...
		public function ajax_favorite() {
			...
			// Take action
			$status = bbp_is_user_favorite($user_id, $topic->ID) ? bbp_remove_user_favorite($user_id, $topic->ID)
				: bbp_add_user_favorite($user_id, $topic->ID);
			// Bail if action failed
			if (empty($status)) {
				bbp_ajax_response(FALSE, __('The request was unsuccessful. Please try again.', 'bbpress'), 305);
			}
		...
		public function ajax_subscription() {
			...
			// Take action
			$status = bbp_is_user_subscribed($user_id, $topic->ID) ? bbp_remove_user_subscription($user_id, $topic->ID)
				: bbp_add_user_subscription($user_id, $topic->ID);
			// Bail if action failed
			if (empty($status)) {
				bbp_ajax_response(FALSE, __('The request was unsuccessful. Please try again.', 'bbpress'), 305);
			}
