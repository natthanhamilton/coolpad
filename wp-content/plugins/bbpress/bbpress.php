	...
	final class bbPress {
		...
		private function setup_actions() {
			...
			foreach ($actions as $class_action) {
				add_action('bbp_' . $class_action, [$this, $class_action], 5);
			}
			// All bbPress actions are setup (includes bbp-core-hooks.php)
			do_action_ref_array('bbp_after_setup_actions', [&$this]);
		}
		...
		public static function register_post_types() {
			...
			register_post_type(
				...
				apply_filters('bbp_register_topic_post_type', [
					                                            ...
					                                            'description'         => __('bbPress Topics',
					                                                                        'bbpress'),
					                                            'capabilities'        => bbp_get_topic_caps(),
					                                            'capability_type'     => ['topic', 'topics'],
					                                            'menu_position'       => 555555,
					                                            'has_archive'         => ('forums' === bbp_show_on_root())
						                                            ? bbp_get_topic_archive_slug() : FALSE,
					                                            'exclude_from_search' => TRUE,
		...
		public static function register_post_statuses() {
			...
			register_post_status(
				...
				apply_filters('bbp_register_closed_post_status', [
					...
					'label_count'       => _nx_noop('Closed <span class="count">(%s)</span>',
					                                'Closed <span class="count">(%s)</span>', 'post', 'bbpress'),
					'public'            => TRUE,
			...
			register_post_status(
				...
				apply_filters('bbp_register_spam_post_status', [
					...
					'label_count'               => _nx_noop('Spam <span class="count">(%s)</span>',
					                                        'Spam <span class="count">(%s)</span>', 'post', 'bbpress'),
					'protected'                 => TRUE,
					'exclude_from_search'       => TRUE,
			...
			register_post_status(
				...
				apply_filters('bbp_register_orphan_post_status', [
					...
					'label_count'               => _nx_noop('Orphan <span class="count">(%s)</span>',
					                                        'Orphans <span class="count">(%s)</span>', 'post',
					                                        'bbpress'),
					'protected'                 => TRUE,
					'exclude_from_search'       => TRUE,
					'show_in_admin_status_list' => TRUE,
			...
			register_post_status(
				...
				apply_filters('bbp_register_hidden_post_status', [
					'label'                     => _x('Hidden', 'post', 'bbpress'),
					'label_count'               => _nx_noop('Hidden <span class="count">(%s)</span>',
		...
		public static function register_views() {
			...
			bbp_register_view(
				...
				apply_filters('bbp_register_view_popular', [
			...
			bbp_register_view(
				...
				apply_filters('bbp_register_view_no_replies', [
					                                            'meta_key'      => '_bbp_reply_count',
					                                            'meta_value'    => 1,
					                                            'meta_compare'  => '<',
					                                            'orderby'       => '',
		...
		public static function add_rewrite_rules() {
			...
			add_rewrite_rule($forum_slug . $edit_rule,
			                 'index.php?' . bbp_get_forum_post_type() . '=$matches[1]&' . $edit_id . '=1', $priority);
			add_rewrite_rule($topic_slug . $edit_rule,
			                 'index.php?' . bbp_get_topic_post_type() . '=$matches[1]&' . $edit_id . '=1', $priority);
			add_rewrite_rule($reply_slug . $edit_rule,
			                 'index.php?' . bbp_get_reply_post_type() . '=$matches[1]&' . $edit_id . '=1', $priority);
			add_rewrite_rule($ttag_slug . $edit_rule,
			                 'index.php?' . bbp_get_topic_tag_tax_id() . '=$matches[1]&' . $edit_id . '=1', $priority);
			// User Pagination|Edit|View
			add_rewrite_rule($user_slug . $tops_paged_rule,
			                 'index.php?' . $user_id . '=$matches[1]&' . $user_tops_id . '=1&' . $paged_id . '=$matches[2]',
			                 $priority);
			add_rewrite_rule($user_slug . $reps_paged_rule,
			                 'index.php?' . $user_id . '=$matches[1]&' . $user_reps_id . '=1&' . $paged_id . '=$matches[2]',
			                 $priority);
			add_rewrite_rule($user_slug . $favs_paged_rule,
			                 'index.php?' . $user_id . '=$matches[1]&' . $user_favs_id . '=1&' . $paged_id . '=$matches[2]',
			                 $priority);
			add_rewrite_rule($user_slug . $subs_paged_rule,
			                 'index.php?' . $user_id . '=$matches[1]&' . $user_subs_id . '=1&' . $paged_id . '=$matches[2]',
			                 $priority);
			add_rewrite_rule($user_slug . $tops_rule, 'index.php?' . $user_id . '=$matches[1]&' . $user_tops_id . '=1',
			                 $priority);
			add_rewrite_rule($user_slug . $reps_rule, 'index.php?' . $user_id . '=$matches[1]&' . $user_reps_id . '=1',
			                 $priority);
			add_rewrite_rule($user_slug . $favs_rule, 'index.php?' . $user_id . '=$matches[1]&' . $user_favs_id . '=1',
			                 $priority);
			add_rewrite_rule($user_slug . $subs_rule, 'index.php?' . $user_id . '=$matches[1]&' . $user_subs_id . '=1',
			                 $priority);
			add_rewrite_rule($user_slug . $edit_rule, 'index.php?' . $user_id . '=$matches[1]&' . $edit_id . '=1',
			                 $priority);
			add_rewrite_rule($user_slug . $root_rule, 'index.php?' . $user_id . '=$matches[1]', $priority);
			// Topic-View Pagination|Feed|View
			add_rewrite_rule($view_slug . $paged_rule,
			                 'index.php?' . $view_id . '=$matches[1]&' . $paged_id . '=$matches[2]', $priority);
			add_rewrite_rule($view_slug . $feed_rule,
			                 'index.php?' . $view_id . '=$matches[1]&' . $feed_id . '=$matches[2]', $priority);
		...
		public static function add_permastructs() {
			...
			$search_slug = bbp_get_search_slug();
			// User Permastruct
			add_permastruct($user_id, $user_slug . '/%' . $user_id . '%', [
				'with_front'  => FALSE,
				'ep_mask'     => EP_NONE,
				'paged'       => FALSE,
				'feed'        => FALSE,
			...
			add_permastruct($view_id, $view_slug . '/%' . $view_id . '%', [
				'with_front'  => FALSE,
				'ep_mask'     => EP_NONE,
				'paged'       => FALSE,
				'feed'        => FALSE,
			...
			// Search Permastruct
			add_permastruct($user_id, $search_slug . '/%' . $search_id . '%', [
				'with_front'  => FALSE,
				'ep_mask'     => EP_NONE,
				'paged'       => TRUE,
				'feed'        => FALSE,
