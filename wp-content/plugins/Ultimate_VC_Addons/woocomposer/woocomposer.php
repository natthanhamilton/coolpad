...
if (!class_exists("WooComposer")) {
    class WooComposer {
        ...
        function __construct() {
            ...
            add_action('wp_enqueue_scripts', [$this, 'front_scripts'], 1);
            add_action('admin_init', [$this, 'generate_shortcode_params']);
        } /* end constructor */
        function generate_shortcode_params() {
            /* Generate param type "woocomposer" */
            if (defined('WPB_VC_VERSION') && version_compare(WPB_VC_VERSION, 4.8) >= 0) {
                if (function_exists('vc_add_shortcode_param')) {
                    vc_add_shortcode_param('woocomposer', [$this, 'woo_query_builder'],
                                           plugins_url("admin/js/mapping.js", __FILE__));
                    vc_add_shortcode_param('product_search', [$this, 'woo_product_search']);
                    vc_add_shortcode_param('product_categories', [$this, 'woo_product_categories']);
                }
            } else {
                if (function_exists('add_shortcode_param')) {
                    add_shortcode_param('woocomposer', [$this, 'woo_query_builder'],
                                        plugins_url("admin/js/mapping.js", __FILE__));
                    add_shortcode_param('product_search', [$this, 'woo_product_search']);
                    add_shortcode_param('product_categories', [$this, 'woo_product_categories']);
                }
            }
        }
        ...
        function woo_query_builder($settings, $value) {
            ...
            $output .= '<div class="display_type"><label for="display_type"><strong>' . __($labels['products_from'],
                                                                                           'ultimate_vc') . '</strong></label>';
            $output .= '<select id="display_type">';
            foreach ($displays as $title => $display) {
                if ($display == $display_type) {
                    $output .= '<option value="' . $display . '" selected="selected">' . __($title,
                                                                                            'ultimate_vc') . '</option>';
                } else {
            ...
            $output .= '</select></div>';
            $output .= '<div class="per_page"><label for="per_page"><strong>' . __($labels['per_page'],
                                                                                   'ultimate_vc') . '</strong></label>';
            $output .= '<input type="number" min="2" max="1000" id="per_page" value="' . $post_count . '"></div>';
            if ($module == "grid") {
                $output .= '<div class="columns"><label for="columns"><strong>' . __($labels['columns'],
                                                                                     'ultimate_vc') . '</strong></label>';
                $output .= '<input type="number" min="2" max="4" id="columns" value="' . $columns . '"></div>';
            }
            $output .= '<div class="orderby"><label for="orderby"><strong>' . __($labels['order_by'],
                                                                                 'ultimate_vc') . '</strong></label>';
            $output .= '<select id="orderby">';
            foreach ($orderby_arr as $key => $val) {
                if ($orderby == $val) {
                    $output .= '<option value="' . $val . '" selected="selected">' . __($key,
                                                                                        'ultimate_vc') . '</option>';
                } else {
            ...
            $output .= '</select></div>';
            $output .= '<div class="order"><label for="order"><strong>' . __($labels['order'],
                                                                             'ultimate_vc') . '</strong></label>';
            ...
            $output .= '</select></div>';
            $output .= '<div class="cat"><label for="cat"><strong>' . __($labels['category'],
                                                                         'ultimate_vc') . '</strong></label>';
            $output .= wp_dropdown_categories(['taxonomy' => 'product_cat', 'selected' => $cat_id, 'echo' => FALSE,]) . '</div>';
            $output .= '<!-- ' . $value . ' -->';
        ...
        function woo_product_search($settings, $value) {
            ...
            while ($products_array->have_posts()) : $products_array->the_post();
                ...
                $output .= '<option ' . $selected . ' value="' . get_the_ID() . '">' . __(get_the_title(),
                                                                                          'ultimate_vc') . '</option>';
            endwhile;
            $output .= '</select>';
            $output
        ...
        function woo_product_categories($settings, $value) {
            ...
            foreach ($product_categories as $cat) {
                ...
                $output .= '<option ' . $selected . ' value="' . $cat->term_id . '">' . __($cat->name,
                                                                                           'ultimate_vc') . '</option>';
            }
            $output .= '</select>';
            ...
            if ($hook == "post.php" || $hook == "post-new.php") {
                if (defined('WOOCOMMERCE_VERSION') && version_compare('2.1.0', WOOCOMMERCE_VERSION, '<')) {
                    ...
                    if ($bsf_dev_mode === 'enable') {
                        ...
                        wp_register_style("woocomposer-select2-bootstrap",
                                          plugins_url("admin/css/select2-bootstrap.css", __FILE__));
                        ...
                        wp_register_script("woocomposer-select2-js", plugins_url("admin/js/select2.js", __FILE__),
                                           ['jquery'], '', TRUE);
                        ...
                        wp_register_style("woocomposer-admin-style",
                                          plugins_url("admin/css/ultimate-woocomposer-backend.min.css", __FILE__));
                        wp_enqueue_style("woocomposer-admin-style");
                        wp_enqueue_script("woocomposer-admin-script",
                                          plugins_url("admin/js/ultimate-woocomposer-backend.min.js", __FILE__),
                                          ['jquery'], '', TRUE);
        ...
        function front_scripts($post) {
            if (!is_404() && !is_search() && !is_archive()) {
                ...
                if (NULL !== $post) {
                    ...
                    $shortcodes
                             = ['woocomposer_product', 'woocomposer_list', 'woocomposer_grid', 'woocomposer_grid_cat', 'woocomposer_carousel_cat', 'woocomposer_carousel'];
                    foreach ($shortcodes as $shortcode) {
                        if (has_shortcode($content, $shortcode)) {
                            $count++;
                        }
                    }
                    wp_register_script('woocomposer-unveil', plugins_url('assets/js/unveil.js', __FILE__), ['jquery'],
                                       WOOCOMPOSER_VERSION, TRUE);
                    wp_register_script('woocomposer-slick', plugins_url('assets/js/slick.js', __FILE__), ['jquery'],
                                       WOOCOMPOSER_VERSION, TRUE);
                    wp_register_script('woocomposer-js', plugins_url('assets/js/custom.js', __FILE__),
                                       ['jquery', 'woocomposer-slick'], WOOCOMPOSER_VERSION, TRUE);
                    wp_register_style('woocomposer-front', plugins_url('assets/css/style.css', __FILE__), [],
                                      WOOCOMPOSER_VERSION);
                    wp_register_style('woocomposer-front-wooicon', plugins_url('assets/css/wooicon.css', __FILE__), [],
                                      WOOCOMPOSER_VERSION);
                    wp_register_style('woocomposer-front-slick', plugins_url('assets/css/slick.css', __FILE__), [],
                                      WOOCOMPOSER_VERSION);
                    wp_register_style('woocomposer-animate', plugins_url('assets/css/animate.min.css', __FILE__), [],
                                      WOOCOMPOSER_VERSION);
                    wp_register_script('woocomposer-script', plugins_url('assets/js/woocomposer.min.js', __FILE__),
                                       ['jquery'], WOOCOMPOSER_VERSION, TRUE);
                    wp_register_style('woocomposer-style', plugins_url('assets/css/woocomposer.min.css', __FILE__), [],
                                      WOOCOMPOSER_VERSION);
                    if (defined('WOOCOMMERCE_VERSION') && version_compare('2.1.0', WOOCOMMERCE_VERSION,
                                                                          '<') && $count !== 0
                    ) {
    ...
    function woocomposer_admin_notice_for_version() {
        ...
        if (($is_multisite && $is_network_admin) || !$is_multisite) {
            echo '<div class="updated"><p>' . __('The',
                                                 'ultimate_vc') . ' <strong>WooComposer</strong> ' . __('plugin requires',
                                                                                                        'ultimate_vc') . ' <strong>Visual Composer</strong> ' . __('version 3.7.2 or greater.',
                                                                                                                                                                   'ultimate_vc') . '</p></div>';
        }
    }
    ...
    function woocomposer_admin_notice_for_vc_activation() {
        ...
        if (($is_multisite && $is_network_admin) || !$is_multisite) {
            echo '<div class="updated"><p>' . __('The',
                                                 'ultimate_vc') . ' <strong>WooComposer</strong> ' . __('plugin requires',
                                                                                                        'ultimate_vc') . ' <strong>Visual Composer</strong> ' . __('Plugin installed and activated.') . '</p></div>';
        }
    }
}
