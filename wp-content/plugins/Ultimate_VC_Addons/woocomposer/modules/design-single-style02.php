...
function WooComposer_Single_style02($atts) {
    $product_id
        = $product_style
        = $display_elements = $img_animate = $text_align = $color_heading = $color_categories = $color_price = '';
    $color_rating
        = $color_rating_bg = $color_quick_bg = $color_quick = $color_cart_bg = $color_cart = $color_product_desc = '';
    $color_product_desc_bg
        =
    $size_title = $size_cat = $size_price = $color_on_sale = $color_on_sale_bg = $label_on_sale = $border_style = '';
    $border_color
        = $border_size = $border_radius = $sale_price = $on_sale_alignment = $on_sale_style = $product_img_disp = '';
    ...
    $categories = $product->get_categories(', ',
                                           '<span class="posted_in">' . _n('', '', $cat_count, 'woocommerce') . ' ',
                                           '.</span>');
    $tags       = $product->get_tags(', ', '<span class="tagged_as">' . _n('', '', $tag_count, 'woocommerce') . ' ',
                                     '.</span>');
    ...
    $output .= "\n\t" . '</div>';
    if (in_array("quick", $elemets)) {
        $query = new WP_Query(['post_type' => 'product', 'post__in' => [$product_id]]);
        if ($query->have_posts()):
