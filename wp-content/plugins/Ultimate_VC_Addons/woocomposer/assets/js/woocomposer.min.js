function animate_products() {
    jQuery(".wooproduct").each(function (a, b) {
        var c = jQuery(this).attr("id"), d = jQuery(this).data("animation");
        "animated no-animation" !== d && jQuery("#" + c).addClass(d)
    })
}
function isOnScreen(a) {
    var b = jQuery(window), c = {top: b.scrollTop(), left: b.scrollLeft()}, d = jQuery(a).outerHeight() - 80;
    c.right = c.left + b.width(), c.bottom = c.top + b.height() - d;
    var e = jQuery(a).offset();
    return e.right = e.left + jQuery(a).outerWidth(), e.bottom = e.top + jQuery(a).outerHeight(), !(c.right < e.left || c.left > e.right || c.bottom < e.top || c.top > e.bottom)
}
function add_woocomposer_popupclass() {
    var a = jQuery(".quick-view-single-product > div").height(), b = jQuery(window).height();
    a >= b ? jQuery("html").addClass("woocomposer-popup") : jQuery("html").removeClass("woocomposer-popup")
}
!function (a) {
    a.fn.unveil = function (b, c) {
        function d() {
            var b = j.filter(function () {
                var b = a(this), c = f.scrollTop(), d = c + f.height(), e = b.offset().top, h = e + b.height();
                return h >= c - g && d + g >= e
            });
    ...
    b = function () {
        ...
        c.$slides.each(function (d, e) {
            b = c.slideWidth * d * -1, a(e).css({position: "relative", left: b, top: 0, zIndex: 800, opacity: 0})
        ...
        var a = this;
        a.listWidth = a.$list.width(), a.listHeight = a.$list.height(), a.options.vertical === !1 ? a.slideWidth = Math.ceil(a.listWidth / a.options.slidesToShow) : a.slideWidth = Math.ceil(a.listWidth)
    }, b.prototype.setSlideClasses = function (a) {
        var b, c, d, e = this, f = e.$slider.find(".slick-slide").data("animation");
        if (e.$slider.find(".slick-slide").removeClass(f), e.$slider.find(".slick-slide").removeClass("slick-active").removeClass("slick-center"), c = e.$slider.find(".slick-slide"), e.options.centerMode === !0)b = Math.floor(e.options.slidesToShow / 2), a >= b && a <= e.slideCount - 1 - b ? e.$slides.slice(a - b, a + b + 1).addClass("slick-active") : (d = e.options.slidesToShow + a, c.slice(d - b + 1, d + b + 2).addClass("slick-active")), 0 === a ? c.eq(c.length - 1 - e.options.slidesToShow).addClass("slick-center") : a === e.slideCount - 1 && c.eq(e.options.slidesToShow).addClass("slick-center"), e.$slides.eq(a).addClass("slick-center"); else if (a > 0 && a < e.slideCount - e.options.slidesToShow) {
            e.$slides.slice(a, a + e.options.slidesToShow).addClass("slick-active");
            var f = e.$slides.slice(a, a + e.options.slidesToShow).data("animation");
            e.$slides.slice(a, a + e.options.slidesToShow).addClass(f)
        } else {
            d = e.options.infinite === !0 ? e.options.slidesToShow + a : a, c.slice(d, d + e.options.slidesToShow).addClass("slick-active");
            var f = c.slice(d, d + e.options.slidesToShow).data("animation");
            c.slice(d, d + e.options.slidesToShow).addClass(f)
        }
        ...
        return g.animating === !0 ? !1 : (b = a, f = g.getLeft(b), d = g.getLeft(g.currentSlide), e = g.slideCount % g.options.slidesToScroll !== 0 ? g.options.slidesToScroll : 0, g.currentLeft = null === g.swipeLeft ? d : g.swipeLeft, g.options.infinite === !1 && (0 > a || a > g.slideCount - g.options.slidesToShow + e) ? (b = g.currentSlide, g.animateSlide(d, function () {
            g.postSlide(b)
    ...
    /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) && jQuery(".wcmp-product").addClass("wcmp-mobile"), jQuery(".wcmp-sale-circle").each(function (a, b) {
        var c = jQuery(".wcmp-sale-circle").find("span"), d = c.outerWidth();
        c.css({height: d + "px", "line-height": d + "px"})
    }), jQuery("body").append('<div class="quick-view-single-product"></div>');
    var b = jQuery(".woocomposer_grid > .woocomposer"), c = 0;
    jQuery("body").on("click", ".quick-view-loop", function (a) {
        a.preventDefault();
        var d = jQuery(this).parent().parent().parent(), d = jQuery(this).closest(".wooproduct"), e = d.find("selected").length;
        if (e >= 1)d.removeClass("selected"), jQuery(".wcmp-view-wrap").slideUp(function () {
            jQuery(this).remove()
        }); else {
            jQuery(".wcmp-view-wrap").remove();
            var f = b.children();
            b.children().removeClass("selected"), d.addClass("selected"), jQuery(".wcmp-view-wrap").slideUp(function () {
                jQuery(this).remove()
            });
            var g = (d.height() - 3, d.outerWidth(!0)), h = d.index() + 1, i = jQuery(".wcmp-quick-view-wrapper").data("columns"), j = h / i, k = "";
            Math.floor(j) == j && jQuery.isNumeric(j) ? (c = j, k = c * i - 1) : (c = Math.floor(h / i) + 1, k = c * i - 1);
            var l = f.length;
            k >= l && (k = l - 1);
            var m = g / 2 - 5 + (h - 1) % i * g;
            f.removeClass("edge"), jQuery(f[k]).addClass("edge");
            var n = d.children(".wcmp-quick-view-wrapper").html();
            jQuery(".edge").after('<div class="wcmp-view-wrap vc_span12 wpb_column column_container woocommerce" style="height:0px; margin-top: -20px; width:100%;"><div class="info-pc" style="left:' + m + 'px; float:right; top:-10px;"></div><div class="wcmp-view-content product">' + n + '</div><div class="wcmp-close"><i class="wooicon-cross2"></i></div></div>');
            var o = jQuery(".wcmp-view-content").height();
            jQuery(".wcmp-view-wrap").animate({height: o}, {
                duration: 500, complete: function () {
                    jQuery(this).css("overflow", "visible"), jQuery(this).trigger("viewLoaded")
                }
            })
        }
    }), jQuery("body").click(function (a) {
        a.target == this && jQuery(".woocomposer_grid").find(".wcmp-view-wrap").length > 0 && (jQuery(".wcmp-view-wrap").addClass("animated fadeInOut"), setTimeout(function () {
            var a = jQuery(".wcmp-view-content").height(), b = (jQuery(window).innerHeight() + a) / 2, c = jQuery(".wcmp-view-wrap").offset().top - b;
        ...
        }, 500), b.children().removeClass("selected"))
    }), jQuery(document).ready(function (a) {
        a(".woocomposer_grid").each(function (b, c) {
            var d = a(this).find(".woocomposer").attr("data-columns");
            a(this).find(".wooproduct").each(function (b, c) {
                var e = b % d;
                0 == e && a(c).addClass("wooproduct-first"), e == d - 1 && a(c).addClass("wooproduct-last")
            })
        }), a(".wcmp-img").unveil();
        var b = jQuery(".woocomposer_carousel").find(".wcmp_vc_inline").length;
        if (1 == b) {
            var c = jQuery(".woocomposer_carousel");
            c.each(function (a, b) {
                var c = jQuery(this).attr("id"), d = jQuery(this).attr("data-slick");
                jQuery("#" + c + " > .woocomposer").slick(d)
            })
        }
    }), jQuery("body").on("click", ".wcmp-close", function (a) {
        a.preventDefault();
        var b = jQuery(".woocomposer_grid").children(".wooproduct");
        b.removeClass("selected");
        var c = jQuery(".wcmp-view-content").height(), d = (jQuery(window).innerHeight() + c) / 2, e = jQuery(".wcmp-view-wrap").offset().top - d;
        jQuery(".wcmp-view-wrap").slideUp(function () {
            jQuery(this).remove()
        }), jQuery("html, body").animate({scrollTop: e}, {
            duration: 500, complete: function () {
            }
        })
    }), jQuery("body").on("click", "a.swatch-anchor", function (a) {
        a.preventDefault();
        for (var b = "", c = jQuery(this).parent().parent().data("attribute-name"), d = jQuery(this).parent().data("value"), e = jQuery(this).closest(".variations_form").data("product_variations"), f = jQuery(this).closest(".variations_form"), g = jQuery(this).closest(".product").find(".wcmp-quickview-img"), h = 0; h < e.length; h++) {
            var i = !0;
            for (attribute in e[h].attributes)if (c == attribute) {
                var j = e[h].attributes[attribute];
                j != d && (i = !1)
            }
            i && (b = e[h])
        }
        var k = b.image_src;
        f.find(".single_variation").html(b.price_html + b.availability_html), console.log(b), g.find("img").attr("src", k), jQuery(this).parent().parent().find("#" + c).val(d), jQuery(this).parent().siblings(".swatch-wrapper").removeClass("selected"), jQuery(this).parent().addClass("selected"), jQuery("form input[name=variation_id]").val(b.variation_id).change()
    }), jQuery(".woo-msg").css({
        position: "fixed",
        width: "100%",
        left: "0",
        ...
        "z-index": "9999999",
        top: "0px"
    }), jQuery(".woo-msg > .woocommerce-message").append('<a href="#" class="button wc-backward wcmp-continue" style="margin-right: 15px;">Continue Shopping</a>'), jQuery(".wcmp-continue").click(function () {
        jQuery(".woocommerce-message").slideUp(500, function () {
            jQuery(this).remove()
        })
    }), jQuery(".quick-view-single").click(function (a) {
        a.preventDefault();
        var b = jQuery(this).parent().parent().find(".wcmp-quick-view-wrapper"), c = b.html();
        jQuery(".quick-view-single-product").html(c), jQuery(".quick-view-single-product").fadeIn("fast"), jQuery(".quick-view-single-product .wcmp-image-carousel").slick({
            arrows: !0,
            infinite: !0,
            speed: 500,
            slidesToShow: 1,
        ...
        var d = jQuery(".quick-view-single-product").find(".variations_form").length;
        0 != d && (jQuery(".variations_form").wc_variation_form(), jQuery(".variations_form .variations select").change()), add_woocomposer_popupclass()
    }), jQuery(".quick-view-loop-popup").click(function (a) {
        a.preventDefault();
        var b = jQuery(this).closest(".wooproduct").find(".wcmp-quick-view-wrapper"), c = b.html();
        jQuery(".quick-view-single-product").html(c), jQuery(".quick-view-single-product").fadeIn("fast"), jQuery(".quick-view-single-product .wcmp-image-carousel").slick({
            arrows: !0,
            infinite: !0,
            speed: 500,
        ...
        jQuery(".quick-view-single-product").fadeOut(500), jQuery("html").removeClass("woocomposer-popup")
    ...
    var a = jQuery(".wcmp-view-content").height();
    jQuery(".wcmp-view-wrap").animate({
        height: a
    }, {
        duration: 500, complete: function () {
            var b = (jQuery(window).innerHeight() - a) / 2, c = jQuery(".wcmp-view-wrap").offset().top - b;
            jQuery("html, body").animate({scrollTop: c}, {
                duration: 500, complete: function () {
                }
            })
    ...
    var b = jQuery(".wcmp-view-wrap").find(".variations_form").length;
    0 != b && (jQuery(".variations_form").wc_variation_form(), jQuery(".variations_form .variations select").change())
}), jQuery(window).scroll(function () {
    animate_products()
}), jQuery(window).load(function () {
    animate_products();
    var a = jQuery(".woocomposer_carousel").find(".wcmp_vc_inline").length;
    if (a >= 1) {
        var b = jQuery(".woocomposer_carousel");
        b.each(function (b, c) {
            var d = jQuery(this).attr("id"), e = jQuery(this).attr("data-slick");
            jQuery("#" + d + " > .woocomposer").slick(e);
            var f = jQuery("#" + d + " > .woocomposer").find(".wooproduct").length;
            jQuery("#" + d + " > .woocomposer").find("slick-track").css("width", 300 * f), jQuery("#" + d + " > .woocomposer").find(".wooproduct").each(function (a, b) {
                jQuery(this).css("width", "300px")
            }), console.log(a)
        }), jQuery(".quick-view-single-product .wcmp-image-carousel, .wcmp-single-image-carousel").each(function (a, b) {
            jQuery(this).slick({
                arrows: !0,
                infinite: !0,
                draggable: !1,
