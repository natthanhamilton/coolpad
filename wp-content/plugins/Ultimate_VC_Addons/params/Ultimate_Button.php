...
if (!class_exists('Ultimate_Button_Param')) {
    class Ultimate_Button_Param {
        function __construct() {
            if (defined('WPB_VC_VERSION') && version_compare(WPB_VC_VERSION, 4.8) >= 0) {
                if (function_exists('vc_add_shortcode_param')) {
                    vc_add_shortcode_param('ult_button', [$this, 'button_prev_param']);
                }
            } else {
                if (function_exists('add_shortcode_param')) {
                    add_shortcode_param('ult_button', [$this, 'button_prev_param']);
                }
            }
        }
        ...
        function button_prev_param($settings, $value) {
            ...
            $selector
                = '<select name="' . $param_name . '" class="wpb_vc_param_value ' . $param_name . ' ' . $type . ' ' . $class . '">';
            foreach ($jsonIterator as $key => $val) {
                if (is_array($val)) {
                    ...
                    foreach ($val as $label => $style) {
                        ...
                        if ($style == $value) {
                            $selector .= '<option selected value="' . $style . '">' . __($label,
                                                                                         'ultimate_vc') . '</option>';
                        } else {
