	...
	class Ultimate_Navigation {
		function __construct() {
			if (defined('WPB_VC_VERSION') && version_compare(WPB_VC_VERSION, 4.8) >= 0) {
				if (function_exists('vc_add_shortcode_param')) {
					vc_add_shortcode_param('ultimate_navigation', [&$this, 'icon_settings_field']);
				}
			} else {
				if (function_exists('add_shortcode_param')) {
					add_shortcode_param('ultimate_navigation', [&$this, 'icon_settings_field']);
				}
			}
		}
		...
		function icon_settings_field($settings, $value) {
			...
			if ($param_name == "next_icon") {
				$icons
					= ['ultsl-arrow-right', 'ultsl-arrow-right2', 'ultsl-arrow-right3', 'ultsl-arrow-right4', 'ultsl-arrow-right6'];
			}
			if ($param_name == "prev_icon") {
				$icons
					= ['ultsl-arrow-left', 'ultsl-arrow-left2', 'ultsl-arrow-left3', 'ultsl-arrow-left4', 'ultsl-arrow-left6'];
			}
			if ($param_name == "dots_icon") {
				$icons
					= ['ultsl-checkbox-unchecked', 'ultsl-checkbox-partial', 'ultsl-stop', 'ultsl-radio-checked', 'ultsl-radio-unchecked', 'ultsl-record'];
			}
			...
			$output
				= '<input type="hidden" name="' . $param_name . '" class="wpb_vc_param_value ' . $param_name . ' ' . $type . ' ' . $class . '" value="' . $value . '" id="trace-' . $uid . '"/>';
