...
if (!class_exists('Ultimate_Responsive')) {
    class Ultimate_Responsive {
        function __construct() {
            add_action('admin_enqueue_scripts', [$this, 'ultimate_admin_responsive_param_scripts']);
            if (defined('WPB_VC_VERSION') && version_compare(WPB_VC_VERSION, 4.8) >= 0) {
                if (function_exists('vc_add_shortcode_param')) {
                    vc_add_shortcode_param('ultimate_responsive', [$this, 'ultimate_responsive_callback'],
                                           plugins_url('../admin/vc_extend/js/ultimate-responsive.js', __FILE__));
                }
            } else {
                if (function_exists('add_shortcode_param')) {
                    add_shortcode_param('ultimate_responsive', [$this, 'ultimate_responsive_callback'],
                                        plugins_url('../admin/vc_extend/js/ultimate-responsive.js', __FILE__));
                }
            }
        }
            ...
            foreach ($medias as $key => $default_value) {
                ...
                switch ($key) {
                    ...
                    case 'Desktop':
                        ...
                        $html .= $this->ultimate_responsive_param_media($class, $dashicon, $key, $default_value, $unit,
                                                                        $data_id);
                        $html
                            .= "<div class='simplify'>
										<div class='ult-tooltip simplify-options'>" . __("Responsive Options",
                                                                                         "ultimate_vc") . "</div>
        ...
        function ultimate_admin_responsive_param_scripts($hook) {
            if ($hook == "post.php" || $hook == "post-new.php") {
                ...
                if ($bsf_dev_mode === 'enable') {
                    wp_register_style('ultimate_responsive_param_css',
