...
if (!class_exists('Ult_Spacing')) {
    class Ult_Spacing {
        function __construct() {
            add_action('admin_enqueue_scripts', [$this, 'ultimate_spacing_param_scripts']);
            if (defined('WPB_VC_VERSION') && version_compare(WPB_VC_VERSION, 4.8) >= 0) {
                if (function_exists('vc_add_shortcode_param')) {
                    vc_add_shortcode_param('ultimate_spacing', [$this, 'ultimate_spacing_callback'],
                                           plugins_url('../admin/vc_extend/js/ultimate-spacing.js', __FILE__));
                }
            } else {
                if (function_exists('add_shortcode_param')) {
                    add_shortcode_param('ultimate_spacing', [$this, 'ultimate_spacing_callback'],
                                        plugins_url('../admin/vc_extend/js/ultimate-spacing.js', __FILE__));
                }
            }
        }
        ...
        function ultimate_spacing_callback($settings, $value) {
            ...
            foreach ($positions as $key => $default_value) {
                switch ($key) {
                    case 'Top':
                        ...
                        $html .= $this->ultimate_spacing_param_item($dashicon, $mode, $unit, /*$default_value,*/
                                                                    $default_value, $key);
                        break;
                    case 'Right':
            ...
            $html
                .= '<div class="ultimate-spacing-input-block ult-spacing-all " data-status="hide-all">
        ...
        function ultimate_spacing_param_item($dashicon, $mode, $unit,/* $default_value,*/
                                             $default_value, $key) {
        ...
        function ultimate_spacing_param_scripts($hook) {
            if ($hook == "post.php" || $hook == "post-new.php") {
                ...
                if ($bsf_dev_mode === 'enable') {
                    wp_register_style('ultimate_spacing_css',
                                      plugins_url('../admin/vc_extend/css/ultimate_spacing.css', __FILE__));
