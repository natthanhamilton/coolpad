...
if (!class_exists('Ult_Image_Single')) {
	class Ult_Image_Single {
		function __construct() {
			add_action('admin_enqueue_scripts', [$this, 'image_single_scripts']);
			add_filter('ult_get_img_single', [$this, 'ult_img_single_init'], 10, 3);
			if (defined('WPB_VC_VERSION') && version_compare(WPB_VC_VERSION, 4.8) >= 0) {
				if (function_exists('vc_add_shortcode_param')) {
					vc_add_shortcode_param('ult_img_single', [$this, 'ult_img_single_callback'],
					                       plugins_url('../admin/vc_extend/js/ultimate-image_single.js', __FILE__));
				}
			} else {
				if (function_exists('add_shortcode_param')) {
					add_shortcode_param('ult_img_single', [$this, 'ult_img_single_callback'],
					                    plugins_url('../admin/vc_extend/js/ultimate-image_single.js', __FILE__));
				}
			}
			add_action('wp_ajax_ult_get_attachment_url', [$this, 'get_attachment_url_init']);
		}
		...
		function image_single_scripts() {
			...
			wp_enqueue_style('ultimate_image_single_css',
			                 plugins_url('../admin/vc_extend/css/ultimate_image_single.css', __FILE__));
		}
				...
				if ($data != '') {
					switch ($data) {
						case 'url':     // First  - Priority for ID
							...
							break;
						case 'title':
							$final = isset($mainArr['title']) ? $mainArr['title']
								: get_post_meta($mainArr['id'], '_wp_attachment_image_title', TRUE);
							break;
						case 'caption':
							$final = isset($mainArr['caption']) ? $mainArr['caption']
								: get_post_meta($mainArr['id'], '_wp_attachment_image_caption', TRUE);
							break;
						case 'alt':
							$final = isset($mainArr['alt']) ? $mainArr['alt']
								: get_post_meta($mainArr['id'], '_wp_attachment_image_alt', TRUE);
							break;
						case 'description':
							$final = isset($mainArr['description']) ? $mainArr['description']
								: get_post_meta($mainArr['id'], '_wp_attachment_image_description', TRUE);
							break;
						case 'json':
