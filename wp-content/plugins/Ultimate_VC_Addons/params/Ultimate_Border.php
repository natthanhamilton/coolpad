...
if (!class_exists('Ultimate_Border')) {
    class Ultimate_Border {
        function __construct() {
            add_action('admin_enqueue_scripts', [$this, 'ultimate_border_param_scripts']);
            if (defined('WPB_VC_VERSION') && version_compare(WPB_VC_VERSION, 4.8) >= 0) {
                if (function_exists('vc_add_shortcode_param')) {
                    vc_add_shortcode_param('ultimate_border', [$this, 'ultimate_border_callback'],
                                           plugins_url('../admin/vc_extend/js/ultimate-border.js', __FILE__));
                }
            } else {
                if (function_exists('add_shortcode_param')) {
                    add_shortcode_param('ultimate_border', [$this, 'ultimate_border_callback'],
                                        plugins_url('../admin/vc_extend/js/ultimate-border.js', __FILE__));
                }
            }
        }
            ...
            foreach ($positions as $key => $default_value) {
                switch ($key) {
                    case 'Top':
                        ...
                        $html .= $this->ultimate_border_param_item($dashicon, /*$mode,*/
                                                                   $unit, /*$default_value,*/
                ...
                foreach ($radius as $key => $default_value) {
                    switch ($key) {
                        case 'Top Left':
                            ...
                            $dashicon    = 'dashicons dashicons-arrow-up-alt';
                            $placeholder = 'T. Left';
                            $html .= $this->ultimate_border_radius_item($dashicon, /*$mode,*/
                                                                        $unit, $default_value, /*$default_value,*/
                                /*$default_value,*/
                                                                        $key, $placeholder);
                            break;
                        case 'Top Right':
        ...
        function ultimate_border_radius_item($dashicon, /*$mode,*/
                                             $unit,/* $default_value,*/
                                             $default_value, $key, $placeholder) {
        ...
        function ultimate_border_param_item($dashicon, /*$mode,*/
                                            $unit,/* $default_value,*/
                                            $default_value, $key, $id) {
        ...
        function ultimate_border_param_scripts($hook) {
            wp_register_style("ultimate-border-style",
                              plugins_url('../admin/vc_extend/css/ultimate_border.css', __FILE__));
