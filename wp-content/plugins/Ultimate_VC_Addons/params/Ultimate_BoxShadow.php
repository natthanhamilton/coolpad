...
if (!class_exists('Ultimate_BoxShadow')) {
    class Ultimate_BoxShadow {
        function __construct() {
            if (defined('WPB_VC_VERSION') && version_compare(WPB_VC_VERSION, 4.8) >= 0) {
                if (function_exists('vc_add_shortcode_param')) {
                    vc_add_shortcode_param('ultimate_boxshadow', [$this, 'ultimate_boxshadow_callback'],
                                           plugins_url('../admin/vc_extend/js/vc-box-shadow-param.js', __FILE__));
                }
            } else {
                if (function_exists('add_shortcode_param')) {
                    add_shortcode_param('ultimate_boxshadow', [$this, 'ultimate_boxshadow_callback'],
                                        plugins_url('../admin/vc_extend/js/vc-box-shadow-param.js', __FILE__));
                }
            }
            add_action('admin_enqueue_scripts', [$this, 'ultimate_boxshadow_param_scripts']);
            add_filter('Ultimate_GetBoxShadow', [$this, 'ultimate_get_box_shadow'], 10, 3);
        }
        ...
        function ultimate_boxshadow_param_item($dashicon, /*$mode,*/
                                               $unit,/* $default_value,*/
                                               $default_value, $key) {
        ...
        function ultimate_get_box_shadow($content = NULL, $data = '') {
            ...
            if ($content != '') {
                ...
                $rmkeys   = str_replace("outset", "",
                                        $rmkeys);     // Remove outset from style - To apply {outset} box shadow
                if ($data != '') {
                    switch ($data) {
                        case 'data':
                            $final = $rmkeys;
        ...
        function ultimate_boxshadow_param_scripts($hook) {
            if ($hook == "post.php" || $hook == "post-new.php") {
                ...
                if ($bsf_dev_mode === 'enable') {
                    ...
                    wp_register_style('ultimate_boxshadow_param_css',
                                      plugins_url('../admin/vc_extend/css/vc_param_boxshadow.css', __FILE__));
