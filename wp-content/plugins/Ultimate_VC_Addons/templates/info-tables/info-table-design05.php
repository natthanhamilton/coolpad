...
if (!function_exists('ult_info_table_generate_design05')) {
    function ult_info_table_generate_design05($atts, $content = NULL) {
        $icon_type
            ...
            = $icon_border_style = $icon_border_radius = $icon_color_border = $icon_border_size = $icon_border_spacing
            = $el_class = $package_heading = $package_sub_heading = $package_price = $package_unit = $package_btn_text
            = $package_link = $package_featured = $color_bg_main
            = $color_txt_main = $color_bg_highlight = $color_txt_highlight = $color_scheme = $use_cta_btn = '';
        ...
        $css_info_tables = apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG,
                                         vc_shortcode_custom_css_class($css_info_tables, ' '), "ultimate_info_table",
                                         $atts);
        $css_info_tables = esc_attr($css_info_tables);
        $output
                         =
        $link = $target = $featured = $featured_style = $normal_style = $dynamic_style = $box_icon = '';
        if ($icon_type !== "none") {
            $box_icon
