...
function uvc_plugin_activate() {
	...
	if ($allowed_memory - $peak_memory <= 14436352) {
		...
		trigger_error($pre . ' ' . $sub . ' <a href="https://support.brainstormforce.com/">' . __('plugin support',
		                                                                                          'ultimate_vc') . '</a>.',
		              E_USER_ERROR);
	}
...
function load_ulitmate_presets() {
	...
	foreach (glob($ultimate_preset_path . "/*.php") as $filename) {
		...
		$presets = (isset($array['presets'])) ? $array['presets'] : [];
		if (empty($presets)) {
			continue;
		}
		foreach ($presets as $key => $preset) {
			...
			$settings = (isset($preset['settings'])) ? $preset['settings'] : [];
...
if (!class_exists('Ultimate_VC_Addons')) {
	...
	function ultimate_plugins_page_link($links) {
		$tutorial_link = '<a href="http://bsf.io/y7ajc" target="_blank">' . __('Video Tutorials',
		                                                                       'ultimate_vc') . '</a>';
		if (is_multisite()) {
			$settins_link
				= '<a href="' . network_admin_url('admin.php?page=ultimate-dashboard') . '" target="_blank">' . __('Settings',
				                                                                                                   'ultimate_vc') . '</a>';
		} else {
			$settins_link
				= '<a href="' . admin_url('admin.php?page=ultimate-dashboard') . '" target="_blank">' . __('Settings',
				                                                                                           'ultimate_vc') . '</a>';
		}
	...
	function admin_notice_for_version() {
		...
		if (($is_multisite && $is_network_admin) || !$is_multisite) {
			echo '<div class="updated"><p>' . __('The',
			                                     'ultimate_vc') . ' <strong>Ultimate addons for Visual Composer</strong> ' . __('plugin requires',
	...
	function admin_notice_for_vc_activation() {
		...
		if (($is_multisite && $is_network_admin) || !$is_multisite) {
			echo '<div class="updated"><p>' . __('The',
			                                     'ultimate_vc') . ' <strong>Ultimate addons for Visual Composer</strong> ' . __('plugin requires',
			                                                                                                                    'ultimate_vc') . ' <strong>Visual Composer</strong> ' . __('Plugin installed and activated.',
			                                                                                                                                                                               'ultimate_vc') . '</p></div>';
		}
	}
	...
	class Ultimate_VC_Addons {
		...
		function __construct() {
			...
			add_action('init', [$this, 'load_vc_translation']);
			...
			add_action('after_setup_theme', [$this, 'aio_init']);
			add_action('admin_enqueue_scripts', [$this, 'aio_admin_scripts']);
			add_action('wp_enqueue_scripts', [$this, 'aio_front_scripts'], 99);
			add_action('admin_init', [$this, 'toggle_updater'], 1);
			...
			add_action('wp_head', [$this, 'ultimate_init_vars']);
		...
		function ultimate_init_vars() {
			...
			$step
			                                = (isset($ultimate_smooth_scroll_options['step']) && $ultimate_smooth_scroll_options['step'] != '')
				? $ultimate_smooth_scroll_options['step'] : 80;
			$speed
			                                = (isset($ultimate_smooth_scroll_options['speed']) && $ultimate_smooth_scroll_options['speed'] != '')
				? $ultimate_smooth_scroll_options['speed'] : 480;
		...
		function woocomposer_admin_notice_for_woocommerce() {
			echo '<div class="error"><p>' . _('The',
			                                  'ultimate_vc') . ' <strong>WooComposer</strong> ' . __('plugin requires',
			                                                                                         'ultimate_vc') . ' <strong>WooCommerce</strong> ' . __('plugin installed and activated with version greater than 2.1.0.',
			                                                                                                                                                'ultimate_vc') . '</p></div>';
		}
		...
		function aio_admin_scripts($hook) {
			...
			if ($hook == "post.php" || $hook == "post-new.php" || $hook == 'visual-composer_page_vc-roles') {
				...
				if (wp_script_is('vc_inline_custom_view_js', 'enqueued')) {
					wp_enqueue_script('vc-inline-editor', $this->assets_js . 'vc-inline-editor.js',
					                  ['vc_inline_custom_view_js'], '1.5', TRUE);
				}
		...
		function aio_front_scripts() {
			...
			$ultimate_smooth_scroll_compatible = get_option('ultimate_smooth_scroll_compatible');
			// register js
			wp_register_script('ultimate-script', plugins_url('assets/min-js/ultimate.min.js', __FILE__), ['jquery'],
			                   ULTIMATE_VERSION, FALSE);
			wp_register_script('ultimate-appear', plugins_url($js_path . 'jquery-appear' . $ext . '.js', __FILE__),
			                   ['jquery'], ULTIMATE_VERSION);
			wp_register_script('ultimate-custom', plugins_url($js_path . 'custom' . $ext . '.js', __FILE__), ['jquery'],
			                   ULTIMATE_VERSION);
			wp_register_script('ultimate-vc-params', plugins_url($js_path . 'ultimate-params' . $ext . '.js', __FILE__),
			                   ['jquery'], ULTIMATE_VERSION);
			if ($ultimate_smooth_scroll_compatible === 'enable') {
			...
			wp_register_script('ultimate-smooth-scroll', plugins_url('assets/min-js/' . $smoothScroll, __FILE__),
			                   ['jquery'], ULTIMATE_VERSION, TRUE);
			wp_register_script("ultimate-modernizr",
			                   plugins_url($js_path . 'modernizr-custom' . $ext . '.js', __FILE__), ['jquery'],
			                   ULTIMATE_VERSION);
			wp_register_script("ultimate-tooltip", plugins_url($js_path . 'tooltip' . $ext . '.js', __FILE__),
			                   ['jquery'], ULTIMATE_VERSION);
			// register css
			wp_register_style('ultimate-animate', plugins_url($css_path . 'animate' . $ext . '.css', __FILE__), [],
			                  ULTIMATE_VERSION);
			wp_register_style('ultimate-style', plugins_url($css_path . 'style' . $ext . '.css', __FILE__), [],
			                  ULTIMATE_VERSION);
			wp_register_style('ultimate-style-min', plugins_url('assets/min-css/ultimate.min.css', __FILE__), [],
			                  ULTIMATE_VERSION);
			wp_register_style('ultimate-tooltip', plugins_url($css_path . 'tooltip' . $ext . '.css', __FILE__), [],
			                  ULTIMATE_VERSION);
			$ultimate_smooth_scroll = get_option('ultimate_smooth_scroll');
			...
			if (!is_404() && !is_search()) {
				...
				if ($ultimate_css == "enable") {
					...
					if (stripos($post_content, '[interactive_banner') && !stripos($post_content,
