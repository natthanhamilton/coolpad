...
if (!function_exists('bsf_check_product_update')) {
    function bsf_check_product_update() {
        $brainstrom_products = (get_option('brainstrom_products')) ? get_option('brainstrom_products') : [];
        $bsf_users           = (get_option('brainstrom_users')) ? get_option('brainstrom_users') : [];
        $mix = $bsf_product_plugins = $bsf_product_themes = $registered = $check_license = [];
        if (!empty($bsf_users)) {
...
if (!function_exists('get_bsf_product_upgrade_link')) {
    function get_bsf_product_upgrade_link($product) {
        ...
        $free
                = (isset($product['is_product_free']) && ($product['is_product_free'] == TRUE || $product['is_product_free'] == 'true'))
            ? $product['is_product_free'] : 'false';
        $id          = $product['id'];
        $original_id = $id;
        ...
        if ($product['bundled']) {
            ...
            $not_registered_msg
                = 'This is bundled with ' . $product_name . ', Activate ' . $product_name . '\'s licence for one click update.';
        }
        if ($status === 'registered' || ($free === TRUE || $free === 'true')) {
            ...
            if (is_multisite()) {
                $link
                    = '<a href="' . wp_nonce_url(network_admin_url($request)) . '" data-pid="' . $original_id . '" data-bundled="' . $product['bundled'] . '" data-bid="' . $id . '" class="bsf-update-product-button">' . __('Update ' . $name . '.',
                                                                                                                                                                                                                              'bsf') . '</a><span class="spinner bsf-update-spinner"></span>';
            } else {
                $link
                    = '<a href="' . wp_nonce_url(admin_url($request)) . '" data-pid="' . $original_id . '" data-bundled="' . $product['bundled'] . '" data-bid="' . $id . '" class="bsf-update-product-button">' . __('Update ' . $name . '.',
                                                                                                                                                                                                                      'bsf') . '</a><span class="spinner bsf-update-spinner"></span>';
            }
        } else {
            if (is_multisite()) {
                $link
                    = '<a href="' . wp_nonce_url(network_admin_url('index.php?page=bsf-registration&id=' . $id)) . '">' . __($not_registered_msg,
                                                                                                                             'bsf') . '</a>';
            } else {
                if (defined('BSF_REG_MENU_TO_SETTINGS') && (BSF_REG_MENU_TO_SETTINGS == TRUE || BSF_REG_MENU_TO_SETTINGS == 'true')) {
                    $link
                        = '<a href="' . wp_nonce_url(admin_url('options-general.php?page=bsf-registration&id=' . $id)) . '">' . __($not_registered_msg,
                                                                                                                                   'bsf') . '</a>';
                } else {
                    $link
                        = '<a href="' . wp_nonce_url(admin_url('index.php?page=bsf-registration&id=' . $id)) . '">' . __($not_registered_msg,
                                                                                                                         'bsf') . '</a>';
                }
            }
        }
...
if (!function_exists('list_bsf_products_updates')) {
    function list_bsf_products_updates() {
        ...
        $brainstrom_bundled_products = (get_option('brainstrom_bundled_products'))
            ? get_option('brainstrom_bundled_products') : [];
        $mix_products = $update_ready = $bsf_product_plugins = $bsf_product_themes = $temp_bundled = [];
        if (!empty($brainstrom_products)) :
            $bsf_product_plugins = (isset($brainstrom_products['plugins'])) ? $brainstrom_products['plugins'] : [];
            $bsf_product_themes  = (isset($brainstrom_products['themes'])) ? $brainstrom_products['themes'] : [];
        endif;
        ...
        foreach ($mix_products as $product) {
            ...
            if ($is_bundled) {
                ...
                $type     = (isset($brainstrom_bundled_products[ $bundled_key ]->type))
                    ? $brainstrom_bundled_products[ $bundled_key ]->type : 'plugin';
            } else {
                ...
                foreach ($update_ready as $key => $product) :
                    ...
                    <tr class='active' id="bsf-row-<?php echo $key ?>">
                        <td class='plugin-title'>
                            <strong><?php echo ($product['bundled']) ? $product['name']
                                    : $product['product_name'] ?></strong>
                            <span><?php _e('You have version ' . $product['version'] . ' installed. Update to ' . $product['remote'],
                                           'bsf'); ?></span>
                        </td>
                        <td style='vertical-align:middle'><strong><?php echo __($product['version'], 'bsf'); ?></strong>
                        </td>
                        <td style='vertical-align:middle'><strong><?php echo __($product['remote'], 'bsf'); ?></strong>
                        </td>
