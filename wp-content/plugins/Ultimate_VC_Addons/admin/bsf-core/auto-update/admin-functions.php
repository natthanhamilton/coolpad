			...
			if (!empty($bsf_products_array)) :
				foreach ($bsf_products_array as $key => $bsf_product) {
					...
					if ($product == $bsf_template) {
						...
						$is_edd
						                                                            = (isset($brainstrom_products[ $type . 's' ][ $key ]['edd']))
							? $brainstrom_products[ $type . 's' ][ $key ]['edd'] : '';
					}
				}
			endif;
		}
		...
		if (!is_wp_error($request) || wp_remote_retrieve_response_code($request) === 200) {
			...
			if ($status === 'registered') {
				...
				$temp_info['product_info']
				                                                            = $brainstrom_products[ $type . 's' ][ $product_key ];
				$user_array = [
					'email' => $bsf_useremail,
					'token' => $token
				];
			...
			$arr = ['response' => $request->get_error_message()];
...
if (!function_exists('bsf_update_all_product_version')) {
	function bsf_update_all_product_version() {
		...
		$mix_products = $update_ready = $bsf_product_plugins = $bsf_product_themes = [];
		if (!empty($brainstrom_products)) :
			$bsf_product_plugins = (isset($brainstrom_products['plugins'])) ? $brainstrom_products['plugins'] : [];
			$bsf_product_themes  = (isset($brainstrom_products['themes'])) ? $brainstrom_products['themes'] : [];
		endif;
		...
		if (!empty($bsf_product_plugins)) :
			foreach ($bsf_product_plugins as $key => $plugin) :
				...
				if ($current_version !== '') {
					if (version_compare($version, $current_version) === -1 || version_compare($version,
					                                                                          $current_version) === 1
					) {
...
if (!function_exists('bsf_notices')) {
	function bsf_notices() {
		...
		if ($pagenow === 'plugins.php' || $pagenow === 'post-new.php' || $pagenow === 'edit.php' || $pagenow === 'post.php') {
			...
			$mix = [];
			$plugins = (isset($brainstrom_products['plugins'])) ? $brainstrom_products['plugins'] : [];
			$themes  = (isset($brainstrom_products['themes'])) ? $brainstrom_products['themes'] : [];
			...
			foreach ($mix as $product) :
				...
				if ($status !== 'registered') :
					...
					$message = __('Please', 'bsf') . ' <a href="' . $url . '">' . __('activate',
					                                                                 'bsf') . '</a> ' . __('your copy of the',
					                                                                                       'bsf') . ' ' . $product['product_name'] . ' ' . __('to get update notifications, access to support features & other resources!',
					                                                                                                                                          'bsf');
	...
	function upgrade_bsf_product($request_product_id, $bundled_id) {
		...
		if (!is_wp_error($request) || wp_remote_retrieve_response_code($request) === 200) {
			...
			if (isset($result->error) && !$result->error) {
				...
				$call
					  = 'file=' . $download_path . '&hashtime=' . strtotime(date('d-m-Y h:i:s a')) . '&timezone=' . $timezone;
				$hash = $call;
				...
				if (!$res) {
					...
					if ($type === 'theme' && $bundled_id === FALSE) {
						...
						add_filter('upgrader_pre_install', [&$Theme_Upgrader, 'current_before'], 10, 2);
						add_filter('upgrader_post_install', [&$Theme_Upgrader, 'current_after'], 10, 2);
						add_filter('upgrader_clear_destination', [&$Theme_Upgrader, 'delete_old_theme'], 10, 4);
						$Theme_Upgrader->run([
						...
						remove_filter('upgrader_pre_install', [&$Theme_Upgrader, 'current_before']);
						remove_filter('upgrader_post_install', [&$Theme_Upgrader, 'current_after']);
						remove_filter('upgrader_clear_destination', [&$Theme_Upgrader, 'delete_old_theme']);
						...
						remove_filter('upgrader_pre_install', [&$Plugin_Upgrader, 'deactivate_plugin_before_upgrade']);
						remove_filter('upgrader_clear_destination', [&$Plugin_Upgrader, 'delete_old_plugin']);
...
if (!function_exists('bsf_grant_developer_access')) {
	function bsf_grant_developer_access($action) {
		...
		$token = bsf_generate_rand_token();
		$url
		       = wp_nonce_url(get_site_url() . '/wp-login.php?developer_access=true&access_id=' . $user . '&access_token=' . $token);
...
if (!function_exists('bsf_update_counter')) {
	function bsf_update_counter() {
		...
		$mix_products
			=
		$update_ready = $bsf_product_plugins = $bsf_product_themes = $temp_bundled = $temp_theme_update_ready = [];
		if (!empty($brainstrom_products)) :
			$bsf_product_plugins = (isset($brainstrom_products['plugins'])) ? $brainstrom_products['plugins'] : [];
			$bsf_product_themes  = (isset($brainstrom_products['themes'])) ? $brainstrom_products['themes'] : [];
		endif;
		...
		foreach ($mix_products as $product) {
			...
			if ($is_bundled) {
				//echo '['.$bundled_key.']';
				$version  = (isset($brainstrom_bundled_products[ $bundled_key ]->version))
					? $brainstrom_bundled_products[ $bundled_key ]->version : '';
				$remote   = (isset($brainstrom_bundled_products[ $bundled_key ]->remote))
					? $brainstrom_bundled_products[ $bundled_key ]->remote : '';
				$template = (isset($brainstrom_bundled_products[ $bundled_key ]->init))
					? $brainstrom_bundled_products[ $bundled_key ]->init : '';
			} else {
		...
		<script type="text/javascript">
			(function ($) {
				$(window).load(function (e) {
					...
					if ($changelog_url !== '') {
						$message_changelog
							= 'or view <a href="' . $changelog_url . '" target="_blank">Changelog here</a>';
					}
					if ($ur['bundled']) {
						...
						$message
							= 'There is a new version of ' . $name . ', bundled with <strong>' . $parent_name . '</strong>.';
					} else {
						$id      = str_replace(' ', '-', strtolower($plugin_main_name));
