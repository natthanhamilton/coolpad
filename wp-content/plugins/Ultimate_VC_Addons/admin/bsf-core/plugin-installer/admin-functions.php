...
if (!function_exists('install_bsf_product')) {
    function install_bsf_product($install_id) {
        ...
        $call
                  = 'file=' . $install_product_data->download_url . '&hashtime=' . strtotime(date('d-m-Y h:i:s a')) . '&timezone=' . $timezone;
        $hash     = $call;
        ...
        add_filter('upgrader_source_selection', [$Plugin_Upgrader, 'check_package']);
        $Plugin_Upgrader->run([
        ...
        remove_filter('upgrader_source_selection', [$Plugin_Upgrader, 'check_package']);
