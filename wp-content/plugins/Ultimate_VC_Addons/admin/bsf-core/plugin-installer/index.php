	...
	if ($request_product_id !== '') {
		...
		<div class="wrap">
			...
			<?php if (isset($installed['status']) && $installed['status'] === TRUE) : ?>
				...
				<a href="<?php echo (is_multisite()) ? network_admin_url('plugins.php#' . $current_name)
					: admin_url('plugins.php#' . $current_name) ?>"><?php echo __('Manage plugin here', 'bsf') ?></a>
			<?php endif; ?>
		</div>
		<?php
...
?>
	...
	<div class="wrap about-wrap bsf-sp-screen bend">
		...
		<div class="bend-heading-section extension-about-header">
			...
			<h3><?php _e('iMedica is already very flexible & feature rich theme. It further aims to be all-in-one solution for your WordPress needs. Install any necessary extensions you like from below and take it on the steroids.',
			             'bsf'); ?></h3>
		...
		<div class="bend-content-wrap">
			...
			?>
			<?php $brainstrom_bundled_products = (get_option('brainstrom_bundled_products'))
			...
			if (!empty($brainstrom_bundled_products)) :
				...
				<ul class="bsf-extensions-list">
						...
						<li id="ext-<?php echo $key ?>" class="bsf-extension <?php echo $class ?>">
							<?php if (!$is_plugin_installed) : ?>
								<div class="bsf-extension-start-install">
									<div class="bsf-extension-start-install-content">
										<h2><?php echo __('Downloading', 'bsf') ?>
											<div class="bsf-css-loader"></div>
										</h2>
									</div>
								</div>
							<?php endif; ?>
							<div class="top-section">
								...
								<div class="bsf-extension-info">
                                    ...
                                    */
									?>
									<p class="desc"><?php echo $plugin->description; ?><span
											class="author"><cite>By <?php echo $plugin->author ?></cite></span></p>
								</div>
							</div>
							<div class="bottom-section">
								...
								if (!$is_plugin_installed) {
									if ((!$plugin->licence_require || $plugin->licence_require === 'false') || $status === 'registered') {
										if (is_multisite()) {
											$link
												= network_admin_url('admin.php?page=bsf-extensions&action=install&id=' . $plugin->id . '&bundled=true');
										} else {
											$link
												= admin_url('admin.php?page=bsf-extensions&action=install&id=' . $plugin->id . '&bundled=true');
										}
										...
										if (is_multisite()) {
											$link
												= network_admin_url('index.php?page=bsf-registration&id=' . $product_id);
										} else {
											if (defined('BSF_REG_MENU_TO_SETTINGS') && (BSF_REG_MENU_TO_SETTINGS == TRUE || BSF_REG_MENU_TO_SETTINGS == 'true')) {
												$link
													= admin_url('options-general.php?page=bsf-registration&id=' . $product_id);
											} else {
								...
								?>
								<a class="button button-primary extension-button <?php echo $button_class; ?>"
								   href="<?php echo $link ?>" data-ext="<?php echo $key ?>"
								   data-pid="<?php echo $plugin->id ?>" data-bundled="true"
								   data-action="install"><?php echo $button ?></a>
							</div>
						</li>
					<?php endforeach; ?>
					<?php
					if ($total_bundled_plugins === $global_plugin_installed) : ?>
						<div class="bsf-extensions-no-active">
							<div class="bsf-extensions-title-icon"><span class="dashicons dashicons-smiley"></span>
							</div>
							<p class="bsf-text-light"><em><?php echo __('All available extensions have been installed!',
							                                            'bsf'); ?></em></p>
						</div>
					<?php endif; ?>
				</ul>
				...
				<ul class="bsf-extensions-list">
					...
					if ($global_plugin_installed != 0) :
							...
							<li id="ext-<?php echo $key ?>" class="bsf-extension <?php echo $class ?>">
								...
								<div class="top-section">
									<?php if (!empty($plugin->product_image)) : ?>
										<div class="bsf-extension-product-image">
											<div class="bsf-extension-product-image-stick">
												<img src="<?php echo $plugin->product_image; ?>" class="img"
												     alt="image"/>
											</div>
										</div>
									<?php endif; ?>
									<div class="bsf-extension-info">
										<?php $name = (isset($plugin->short_name)) ? $plugin->short_name
											: $plugin->name ?>
										<h4 class="title"><?php echo $name; ?></h4>
										<?php /*
                                    <span class="status">
								...
								<div class="bottom-section">
										...
										if ((!$plugin->licence_require || $plugin->licence_require === 'false') || $status === 'registered') {
											...
											if (is_multisite()) {
												...
												if (defined('BSF_REG_MENU_TO_SETTINGS') && (BSF_REG_MENU_TO_SETTINGS == TRUE || BSF_REG_MENU_TO_SETTINGS == 'true')) {
													...
													$link
														= admin_url('index.php?page=bsf-registration&id=' . $product_id);
												}
											}
											$button       = __('Validate Purchase', 'bsf');
											$button_class = 'bsf-validate-licence-button';
									...
									<a class="button button-primary extension-button <?php echo $button_class; ?>"
									   href="<?php echo $link ?>"
									   data-ext="<?php echo $key ?>"><?php echo $button ?></a>
								</div>
							</li>
							<?php
						endforeach;
						...
						<div class="bsf-extensions-no-active">
							<div class="bsf-extensions-title-icon"><span class="dashicons dashicons-download"></span>
							</div>
							<p class="bsf-text-light"><em><?php echo __('No extensions installed yet!', 'bsf'); ?></em>
							</p>
						</div>
					<?php endif; ?>
				</ul>
