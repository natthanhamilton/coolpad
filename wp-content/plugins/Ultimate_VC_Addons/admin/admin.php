...
if (!function_exists('bsf_core_admin_notice')) {
    function bsf_core_admin_notice() {
        ...
        <div class="bsf-notice update-nag notice is-dismissible" data-bsf-notice="hide-bsf-core-notice">
            <p><?php _e('License registration and extensions are not part of plugin/theme anymore. Kindly download and install "BSF CORE" plugin to manage your licenses and extensins.',
                        'bsf'); ?></p>
        </div>
        <?php
    }
...
if (!class_exists('Ultimate_Admin_Area')) {
    class Ultimate_Admin_Area {
        function __construct() {
            ...
            /* add admin menu */
            add_action('admin_menu', [$this, 'register_brainstorm_menu'], 99);
            //add_action( 'network_admin_menu', array( $this, 'register_brainstorm_network_menu' ) );
            add_action('admin_enqueue_scripts', [$this, 'bsf_admin_scripts_updater'], 1);
            add_action('wp_ajax_update_ultimate_options', [$this, 'update_settings']);
            add_action('wp_ajax_update_ultimate_debug_options', [$this, 'update_debug_settings']);
            add_action('wp_ajax_update_ultimate_modules', [$this, 'update_modules']);
            add_action('wp_ajax_update_css_options', [$this, 'update_css_options']);
            add_action('wp_ajax_update_dev_notes', [$this, 'update_dev_notes']);
        }
        ...
        function bsf_admin_scripts_updater($hook) {
            ...
            if ($hook == "post.php" ||
                ...
                wp_register_script("ultimate-vc-backend-script",
                                   plugins_url("../admin/js/ultimate-vc-backend.min.js", __FILE__), ['jquery'],
                                   ULTIMATE_VERSION, TRUE);
                wp_register_style("ultimate-vc-backend-style",
                                  plugins_url('../admin/css/ultimate-vc-backend.min.css', __FILE__));
        ...
        function register_brainstorm_menu() {
            ...
            $icon_manager_page = add_submenu_page(
                ...
                [$this, 'ultimate_icon_manager_menu']
            );
            $AIO_Icon_Manager = new AIO_Icon_Manager;
            add_action('admin_print_scripts-' . $icon_manager_page, [$AIO_Icon_Manager, 'admin_scripts']);
            ...
            add_action('admin_print_scripts-' . $google_font_manager_page,
                       [$Ultimate_Google_Font_Manager, 'admin_google_font_scripts']);
        ...
        function update_debug_settings() {
            ...
            $bsf_options_array     = ['dev_mode', 'ultimate_global_scripts'];
        ...
        function display_notice() {
            ...
            if ($status == "Deactivated" || $status == "not-activated" || $status == "not-verified") {
                if ($hook_suffix == 'plugins.php') {
                    ...
                    $reg_link = (is_multisite()) ? network_admin_url('index.php?page=bsf-dashboard')
                        : admin_url('index.php?page=bsf-dashboard');
                    if (!$hide_notice) :
                        ?>
                        <div class="updated" style="padding: 0; margin: 0; border: none; background: none;">
                            <style type="text/css">
                                ...
                                .ult_activate .ult_button_border {
                                    border: 1px solid #0096A3;
                                    -moz-border-radius: 2px;
                                    border-radius: 2px;
                                    -webkit-border-radius: 2px;
                                ...
                                .ult_activate .ult_button_container {
                                    cursor: pointer;
                                ...
                                .ult_activate .ult_description strong {
                                    color: #0096A3;
                                    font-weight: normal
                                }
                            </style>
                            <div class="ult_activate">
                                <div class="ult_a"><img style="width:1em;"
                                                        src="<?php echo plugins_url("img/logo-icon.png", __FILE__); ?>"
                                                        alt=""></div>
                                <?php
                                ?>
                                <div class="ult_button_container"
                                     onclick="document.location='<?php echo $reg_link; ?>'">
                                    <div class="ult_button_border">
                                        <div class="ult_button"><span class="dashicons-before dashicons-admin-network"
                                                                      style="padding-right: 6px;"></span><?php __('Activate your license',
                                                                                                                  'ultimate_vc'); ?>
                                        </div>
                                    </div>
                                </div>
                                <div class="ult_description"><h3 style="margin:0;padding: 2px 0px;">
                                        <strong><?php _e('Almost done!', 'ultimate_vc'); ?></strong></h3>
                                    <p style="margin: 0;"><?php _e('Please activate your copy of the Ultimate Addons for Visual Composer to receive automatic updates & get premium support',
                                                                   'ultimate_vc'); ?></p>
                                </div>
                            </div>
                        </div>
                    ...
                    if ($hook_suffix == 'post-new.php' || $hook_suffix == 'edit.php' || $hook_suffix == 'post.php') {
                        ...
                        if (!$hide_notice) :
                            ...
                            <div class="updated fade">
                                ...
                                <p><?php echo _e('Howdy! Please',
                                                 'ultimate_vc') . ' <a href="' . $reg_link . '">' . __('activate your copy',
                                                                                                       'ultimate_vc') . ' </a> ' . __('of the Ultimate Addons for Visual Composer to receive automatic updates & get premium support.',
                                                                                                                                      'ultimate_vc'); ?>
                                    <span style="float: right; padding: 0px 4px; cursor: pointer;"
                                          class="uavc-activation-notice">X</span>
                                </p>
                            </div>
                            <script type="text/javascript">
