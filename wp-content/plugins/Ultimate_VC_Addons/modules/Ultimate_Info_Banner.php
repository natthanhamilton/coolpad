...
if (!class_exists('Ultimate_Info_Banner')) {
	class Ultimate_Info_Banner {
		function __construct() {
			...
			add_shortcode('ultimate_info_banner', [$this, 'banner_shortcode']);
			add_action('wp_enqueue_scripts', [$this, 'register_info_banner_assets'], 1);
		}
...
//			}
			...
			$css_path = '../assets/css/';
			$ext      = '';
			wp_register_script('utl-info-banner-script', plugins_url($js_path . 'info-banner' . $ext . '.js', __FILE__),
			                   ['jquery', 'ultimate-appear'], ULTIMATE_VERSION);
			wp_register_style('utl-info-banner-style', plugins_url($css_path . 'info-banner' . $ext . '.css', __FILE__),
			                  [], ULTIMATE_VERSION);
		}
					...
					[
						...
						"params"      => [
							...
							[
								...
								"value"      => "1",
								"group"      => "Design",
								"dependency" => ["element" => "ib3_border", "value" => ["solid", "dashed", "dotted", "double"]]
							],
							[
								"type"       => "colorpicker",
		...
		function banner_shortcode($atts) {
			...
			$infobnr_design = apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG,
			                                vc_shortcode_custom_css_class($css_infobanner, ' '), "ultimate_info_banner",
			                                $atts);
