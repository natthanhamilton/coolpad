...
if (!class_exists("Ultimate_Buttons")) {
	class Ultimate_Buttons {
		function __construct() {
			...
			add_shortcode('ult_buttons', [$this, 'ult_buttons_shortcode']);
			add_action('admin_enqueue_scripts', [$this, 'button_admin_scripts']);
			add_action("wp_enqueue_scripts", [$this, "advanced_button_scripts"], 1);
		}
		...
		function advanced_button_scripts() {
			...
			wp_register_style("ult-btn", plugins_url($css_path . 'advanced-buttons' . $ext . '.css', __FILE__), [],
		...
		function button_admin_scripts($hook) {
			if ($hook == "post.php" || $hook == "post-new.php") {
				...
				if ($bsf_dev_mode === 'enable') {
					...
					$css_path = '../assets/css/';
					$ext      = '';
					wp_enqueue_style('ult-button',
					                 plugins_url($css_path . 'advanced-buttons' . $ext . '.css', __FILE__));
				}
			}
		}
		...
		function ult_buttons_shortcode($atts) {
			$output
				              = $btn_title = $btn_link
				= $btn_size = $btn_width = $btn_height = $btn_hover = $btn_bg_color = $btn_radius = $btn_shadow = '';
			$btn_shadow_color
				              = $btn_bg_color_hover
				= $btn_border_style = $btn_color_border = $btn_border_size = $btn_shadow_size = $el_class = '';
			$btn_font_family
				              = $btn_font_style = $btn_title_color
				= $btn_font_size = $icon = $icon_size = $icon_color = $btn_icon_pos = $btn_anim_effect = '';
			$btn_padding_left
				              = $btn_padding_top = $button_bg_img
				= $btn_title_color_hover = $btn_align = $btn_color_border_hover = $btn_shadow_color_hover = '';
			...
			$style
				              = $hover_style = $btn_style_inline = $link_sufix
				=
			$link_prefix = $img = $shadow_hover = $shadow_click = $shadow_color = $box_shadow = $main_extra_class = '';
			...
			$css_btn_design = apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG,
			                                vc_shortcode_custom_css_class($css_adv_btn, ' '), "ult_buttons", $atts);
			...
			if ($img !== '') {
				...
				$output
					= '<div class="' . $css_btn_design . ' ' . $wrapper_class . ' ' . $main_extra_class . '">' . $output . '</div>';
			}
			if ($img !== '') {
					...
					[
						...
						"params"          => [
							...
							[
								...
								"suffix"      => "px",
								"description" => "",
								"dependency"  => ["element" => "btn_size", "value" => "ubtn-custom"],
								"group"       => "General"
							],
							[
								"type"        => "number",
							...
							[
								...
								"param_name"  => "rel",
								"description" => __("This is useful when you want to trigger third party features. Example- preetyPhoto, thickbox etc",
								                    "ultimate_vc"),
								"group"       => "General",
							],
							[
								...
								"value"       => [
									...
									__("Fill Background from Center Horizontally",
									   "ultimate_vc")                                             => "ubtn-center-hz-bg",
									__("Fill Background from Center Vertically",
									   "ultimate_vc")                                             => "ubtn-center-vt-bg",
									__("Fill Background from Center Diagonal",
									   "ultimate_vc")                                             => "ubtn-center-dg-bg",
								],
								"description" => "",
								"group"       => "Background"
							],
							...
							[
								...
								"value"       => "",
								"description" => __("Upload the image on which you want to place the button.",
								                    "ultimate_vc"),
								"group"       => "Background"
							],
							[
							...
							*/
							...
							[
								...
								"value"       => "",
								"description" => "",
								"dependency"  => ["element" => "btn_shadow", "not_empty" => TRUE],
								"group"       => "Shadow"
							],
							[
								"type"        => "colorpicker",
							...
							[
								...
								"value"      => ["Enable tooltip on button" => "yes"],
								"group"      => "Tooltip"
							],
							[
								"type"       => "textfield",
								...
								"value"      => "",
								"dependency" => ["element" => "enable_tooltip", "value" => "yes"],
								"group"      => "Tooltip",
							],
							[
								"type"        => "dropdown",
							...
							[
								...
								"text"             => "<span style='display: block;'><a href='http://bsf.io/0n-7p' target='_blank'>" . __("Watch Video Tutorial",
