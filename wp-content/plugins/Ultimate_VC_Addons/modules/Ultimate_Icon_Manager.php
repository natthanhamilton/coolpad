...
if (!class_exists('AIO_Icon_Manager')) {
	class AIO_Icon_Manager {
		...
		function __construct() {
			...
			add_action('wp_ajax_smile_ajax_add_zipped_font', [$this, 'add_zipped_font']);
			add_action('wp_ajax_smile_ajax_remove_zipped_font', [$this, 'remove_zipped_font']);
			//add_action('admin_menu',array($this,'icon_manager_menu'));
			$defaults = get_option('smile_fonts');
			if (!$defaults) {
				add_action('admin_init', [$this, 'AIO_move_fonts']);
			}
		}
		...
		function admin_scripts() {
			...
			wp_enqueue_script('aio-admin-media', $this->admin_js . 'admin-media.js', ['jquery']);
		...
		function icon_manager_dashboard() {
			...
			<h2>
				...
				<a href="#smile_upload_icon" class="add-new-h2 smile_upload_icon" data-target="iconfont_upload"
				   ...
				   data-button="<?php echo __("Insert Fonts Zip File", "ultimate_vc"); ?>"
		...
		public function get_font_manager($id) {
			...
			$fonts = get_option('smile_fonts');
			$output
			       = '<p><div class="preview-icon preview-icon-' . $id . '"><i class=""></i></div><input class="search-icon" type="text" placeholder="Search for a suitable icon.." /></p>';
		...
		public function add_zipped_font() {
			...
			if (!current_user_can($cap)) {
				die(__("Using this feature is reserved for Super Admins. You unfortunately don't have the necessary permissions.",
				       "ultimate_vc"));
			}
			//get the file path of the zip file
			$attachment = $_POST['values'];
			$path       = realpath(get_attached_file($attachment['id']));
			$unzipped   = $this->zip_flatten($path, ['\.eot', '\.svg', '\.ttf', '\.woff', '\.json', '\.css']);
		...
		function create_config() {
			...
			if (empty($this->json_file) || empty($this->svg_file)) {
				$this->delete_folder($this->paths['tempdir']);
				die(__('selection.json or SVG file not found. Was not able to create the necessary config files',
				       'ultimate_vc'));
			}
			...
			if (empty($response)) $response
				= file_get_contents(trailingslashit($this->paths['tempdir']) . $this->svg_file);
			if (!is_wp_error($json) && !empty($json)) {
				...
				if (is_dir($font_folder)) {
					$this->delete_folder($this->paths['tempdir']);
					die(__("It seems that the font with the same name is already exists! Please upload the font with different name.",
					       "ultimate_vc"));
				}
				$file_contents = json_decode($json);
				if (!isset($file_contents->IcoMoonType)) {
					$this->delete_folder($this->paths['tempdir']);
					die(__('Uploaded font is not from IcoMoon. Please upload fonts created with the IcoMoon App Only.',
					       'ultimate_vc'));
				}
		...
		function rename_files() {
			$extensions = ['eot', 'svg', 'ttf', 'woff', 'css'];
			$folder     = trailingslashit($this->paths['tempdir']);
			foreach (glob($folder . '*') as $file) {
				$path_parts = pathinfo($file);
				if (strpos($path_parts['filename'], '.dev') === FALSE && in_array($path_parts['extension'],
				                                                                  $extensions)
				) {
					if ($path_parts['filename'] !== $this->font_name) {
						rename($file,
						       trailingslashit($path_parts['dirname']) . $this->font_name . '.' . $path_parts['extension']);
					}
				}
			}
		}
		...
		static function load_iconfont_list() {
			...
			if (empty($extra_fonts)) $extra_fonts = [];
...
add_action('wp_ajax_smile_ajax_add_zipped_font', [$AIO_Icon_Manager, 'add_zipped_font']);
add_action('wp_ajax_smile_ajax_remove_zipped_font', [$AIO_Icon_Manager, 'remove_zipped_font']);