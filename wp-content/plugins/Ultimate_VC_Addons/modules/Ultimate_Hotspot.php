...
if (!class_exists('ULT_HotSpot')) {
    class ULT_HotSpot {
        function __construct() {
            ...
            add_shortcode('ult_hotspot', [$this, 'ult_hotspot_callback']);
            add_shortcode('ult_hotspot_items', [$this, 'ult_hotspot_items_callback']);
            // We safely integrate with VC with this hook
            add_action('init', [$this, 'ult_hotspot_init'], 99);
            add_action('admin_enqueue_scripts', [$this, 'enqueue_admin_assets'], 999);
            // Register CSS and JS
            add_action('wp_enqueue_scripts', [$this, 'ult_hotspot_scripts'], 1);
            if (defined('WPB_VC_VERSION') && version_compare(WPB_VC_VERSION, 4.8) >= 0) {
                if (function_exists('vc_add_shortcode_param')) {
                    vc_add_shortcode_param('ultimate_hotspot_param', [$this, 'ultimate_hotspot_param_callback'],
                                           plugins_url('../admin/vc_extend/js/vc-hotspot-param.js', __FILE__));
                }
            } else {
                if (function_exists('add_shortcode_param')) {
                    add_shortcode_param('ultimate_hotspot_param', [$this, 'ultimate_hotspot_param_callback'],
                                        plugins_url('../admin/vc_extend/js/vc-hotspot-param.js', __FILE__));
                }
            }
        }
        ...
        function enqueue_admin_assets($hook) {
            if ($hook == "post.php" || $hook == "post-new.php" || $hook == "edit.php") {
                ...
                if ($bsf_dev_mode === 'enable') {
                    wp_register_script('hotspt-admin-js',
                                       plugins_url('../admin/vc_extend/js/admin_enqueue_js.js', __FILE__),
                                       ['jquery', 'jquery-ui-core', 'jquery-ui-draggable', 'jquery-ui-slider', 'jquery-touch-punch'],
        ...
        function ult_hotspot_callback($atts, $content = NULL) {
            ...
            $output
                = "<div class='ult_hotspot_container " . $is_vc_49_plus . " ult-hotspot-tooltip-wrapper " . $el_class . "' style=" . $cust_size . ">";
        ...
        function ult_hotspot_items_callback($atts, $content = NULL) {
            ...
            if ($link_style == 'link') {
                ...
                $output .= $icon_inline;
                $output .= "<span class='hotspot-tooltip-content'>" . esc_html(str_replace('"', '\'',
        ...
        function ult_hotspot_init() {
            if (function_exists("vc_map")) {
                vc_map([
                           ...
                           "base"                    => "ult_hotspot",
                           "as_parent"               => ['only' => 'ult_hotspot_items'],
                           ...
                           "params"                  => [
                               ...
                               [
                                   ...
                                   "dependency" => ["element" => "main_img_size", "value" => "main_img_custom"],
                               ],
                               [
                                   "type"        => "textfield",
                ...
                vc_map([
                           ...
                           "params"          => [
                               ...
                               [
                                   ...
                                   "description" => __("Display tooltip or just link for the hotspot icon.",
                                                       "ultimate_vc"),
                               ],
                               [
                                   "type"        => "vc_link",
                                   ...
                                   "description" => __("Add a custom link or select existing page. You can remove existing link as well.",
                                                       "ultimate_vc"),
                                   "dependency"  => ["element" => "link_style", "value" => "link"],
                               ],
                               // Link style
                               // TOOLTIP
                               [
                                   ...
                                   "param_name"       => "content",
                                   "dependency"       => ["element" => "link_style", "value" => "tooltip"],
                                   'admin_label'      => TRUE,
                               ...
                               [
                                   ...
                                   "label_color"  => __("Border Color",
                                                        "ultimate_vc"),       //  label for 'border color'   default 'Border Color'
                                   "label_radius" => __("Border Radius",
                                                        "ultimate_vc"),        //  label for 'radius'  default 'Border Redius'
                               ...
                               [
                                   ...
                                   "description" => __("Select animation effect for hotspot icon/image.",
                                                       'ultimate_vc'),
                                   "group"       => "Animation"
                               ],
                               [
                                   ...
                                   "value"      => "",
                                   "group"      => "Animation",
                                   "dependency" => ["element" => "tooltip_continuous_animation", "value" => "glow"],
                               ],
                           ]
                       ]);
            }
        ...
        function ult_hotspot_scripts() {
            ...
            wp_register_style('ult_hotspot_css', plugins_url($css_path . "hotspot" . $ext . ".css", __FILE__), [],
                              ULTIMATE_VERSION, FALSE);
            wp_register_style('ult_hotspot_tooltipster_css',
                              plugins_url($css_path . "hotspot-tooltipster" . $ext . ".css", __FILE__), [],
                              ULTIMATE_VERSION, FALSE);
            //  js
            wp_register_script('ult_hotspot_js', plugins_url($js_path . "hotspot" . $ext . ".js", __FILE__), ['jquery'],
                               ULTIMATE_VERSION, FALSE);
            wp_register_script('ult_hotspot_tooltipster_js',
                               plugins_url($js_path . "hotspot-tooltipster" . $ext . ".js", __FILE__), ['jquery'],
                               ULTIMATE_VERSION, FALSE);
        }
    }
