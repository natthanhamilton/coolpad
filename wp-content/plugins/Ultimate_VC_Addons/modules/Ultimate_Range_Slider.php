...
if (!class_exists("Ultimate_Range_Slider")) {
	...
	class Ultimate_Range_Slider {
		function __construct() {
			add_action('init', [$this, 'init_range_slider']);
			add_shortcode('ult_range_slider', [$this, 'ult_range_slider_shortcode']);
			add_action("wp_enqueue_scripts", [$this, "range_slider_scripts"], 1);
		}
		...
		function ult_range_slider_shortcode($atts, $content = NULL) {
			...
			if ($title_box == 'custom') {
				...
				$slider_padding
					= ' style = "padding:' . $title_box_height_t . 'px ' . (($title_box_width_t / 2) + 10) . 'px 35px;"';
			}
			//slider size data
...
//////////design data
					...
					[
						...
						"params"          => [
							...
							[
								...
								"description" => __("Add content here steps will generate based on content",
								                    "ultimate_vc"),
							...
							[
								...
								"value"       => "",
								"suffix"      => "px",
								"description" => __("If title box text or width is too long then slider width will reduce according to title box width",
								                    "ultimate_vc"),
								//"group" => "Slider Setting",
							],
							[
							...
							[
								...
								"description" => __("If you have different height of descriptions. It will automatically adapt the maximum height.  ",
								                    "smile"),
								"dependency"  => "",
								"group"       => "Description",
							],
							...
							[
								...
								"value"       => "",
								"description" => __("Click and select font of your choice. If you can't find the one that suits for your purpose",
								                    "ultimate_vc") . ", " . __("you can",
								                                               "ultimate_vc") . " <a href='admin.php?page=bsf-google-font-manager' target='_blank'>" . __("add new here",
								                                                                                                                                          "ultimate_vc") . "</a>.",
								"group"       => "Typography"
							],
							[
		...
		function range_slider_scripts() {
			...
			wp_register_style('ult_range_slider_css', plugins_url($css_path . 'range-slider' . $ext . '.css', __FILE__),
			                  [], ULTIMATE_VERSION, FALSE);
			//wp_register_style('ult_range_slider_jqueryui_css',"https://ajax.googleapis.com/ajax/libs/jqueryui/1.11.4/themes/smoothness/jquery-ui.css", array(), ULTIMATE_VERSION, false);
			wp_register_script('ult_range_slider_js', plugins_url($js_path . 'range-slider' . $ext . '.js', __FILE__),
			                   ['jquery'], ULTIMATE_VERSION, TRUE);
			wp_register_script('ult_ui_touch_punch',
			                   plugins_url($js_path . 'range-slider-touch-punch' . $ext . '.js', __FILE__), ['jquery'],
			                   ULTIMATE_VERSION, TRUE);
			//jquery.ui.labeledslider
			wp_register_script('ult_range_tick',
			                   plugins_url($js_path . 'jquery-ui-labeledslider' . $ext . '.js', __FILE__),
			                   ['jquery', 'jquery-ui-core', 'jquery-ui-widget', 'jquery-ui-slider'], ULTIMATE_VERSION,
			                   TRUE);
			//wp_register_style( 'jscss', plugins_url($css_path.'jquery.ui.labeledslider'.$ext.'.css',__FILE__), array(), ULTIMATE_VERSION, false);
		}
	}
