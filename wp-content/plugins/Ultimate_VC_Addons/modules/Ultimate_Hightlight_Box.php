...
if (!class_exists('Ultimate_Highlight_Box')) {
	class Ultimate_Highlight_Box {
		function __construct() {
			add_action('init', [$this, 'ctaction_init']);
			add_shortcode('ultimate_ctation', [$this, 'call_to_action_shortcode']);
			add_action('wp_enqueue_scripts', [$this, 'register_cta_assets'], 1);
		}
		...
		function register_cta_assets() {
			...
			wp_register_style('utl-ctaction-style', plugins_url($css_path . 'highlight-box' . $ext . '.css', __FILE__),
			                  [], ULTIMATE_VERSION);
			wp_register_script('utl-ctaction-script', plugins_url($js_path . 'highlight-box' . $ext . '.js', __FILE__),
					...
					[
						...
						"params"   => [
							...
							[
								...
								"dependency"  => ["element" => "enable_icon", "value" => ["enable_icon_value"]],
								"group"       => "Icon"
							],
							[
								"type"        => "dropdown",
		...
		function call_to_action_shortcode($atts, $content) {
			...
			$output
				= $el_class = $style = $data = $text_style_inline = $ctaction_link_html = $icon_inline = $effect = '';
