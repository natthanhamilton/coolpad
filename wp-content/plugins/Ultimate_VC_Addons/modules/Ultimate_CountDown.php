...
if (!class_exists('Ultimate_CountDown')) {
    class Ultimate_CountDown {
        function __construct() {
            add_action('init', [$this, 'countdown_init']);
            add_shortcode('ult_countdown', [$this, 'countdown_shortcode']);
            add_action('admin_enqueue_scripts', [$this, 'admin_scripts']);
            add_action('wp_enqueue_scripts', [$this, 'count_down_scripts'], 1);
        }
        ...
        function count_down_scripts() {
            ...
            wp_register_script("jquery.timecircle", plugins_url($js_path . "countdown" . $ext . ".js", __FILE__),
                               ['jquery'], ULTIMATE_VERSION);
            wp_register_script("jquery.countdown", plugins_url($js_path . "count-timer" . $ext . ".js", __FILE__),
                               ['jquery'], ULTIMATE_VERSION);
            wp_register_style("ult-countdown", plugins_url($css_path . "countdown" . $ext . ".css", __FILE__), [],
                              ULTIMATE_VERSION);
        }
        ...
        function admin_scripts($hook) {
            if ($hook == "post.php" || $hook == "post-new.php") {
                ...
                if ($bsf_dev_mode === 'enable') {
                    ...
                    wp_register_style("ult-colorpicker-style",
                    ...
                    [
                        ...
                        "params"      => [
                            ...
                            [
                                ...
                                "dependency" => ["element" => "br_style", "value" => ["solid", "dotted", "dashed", "double", "inset", "outset",]],
                                "group"      => "General Settings",
                            ],
                            [
                                "type"       => "colorpicker",
                            ...
                            [
                                ...
                                "text"             => "<span style='display: block;'><a href='http://bsf.io/szdd2' target='_blank'>" . __("Watch Video Tutorial",
        ...
        function countdown_shortcode($atts) {
            $count_style
                = $datetime = $ult_tz = $countdown_opts = $tick_col
                =
            $tick_size = $tick_line_height = $tick_style = $tick_sep_col = $tick_sep_size = $tick_sep_line_height = '';
            $tick_sep_style
                = $br_color = $br_style = $br_size = $timer_bg_color = $br_radius = $br_time_space = $el_class = '';
            $string_days
                =
            $string_weeks = $string_months = $string_years = $string_hours = $string_minutes = $string_seconds = '';
            $string_days2
                = $string_weeks2
                = $string_months2 = $string_years2 = $string_hours2 = $string_minutes2 = $string_seconds2 = '';
            ...
            $labels
                           = $string_years2 . ',' . $string_months2 . ',' . $string_weeks2 . ',' . $string_days2 . ',' . $string_hours2 . ',' . $string_minutes2 . ',' . $string_seconds2;
            $labels2
                           = $string_years . ',' . $string_months . ',' . $string_weeks . ',' . $string_days . ',' . $string_hours . ',' . $string_minutes . ',' . $string_seconds;
            ...
            $countdown_design_style = apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG,
                                                    vc_shortcode_custom_css_class($css_countdown, ' '), "ult_countdown",
                                                    $atts);
            ...
            $output
                = '<div "' . $count_down_sep_data_list . '" class="ult-responsive ult_countdown ' . $countdown_design_style . ' ' . $el_class . ' ' . $count_style . '">';
            //Responsive param
            if (is_numeric($tick_size)) {
                $tick_size = 'desktop:' . $tick_size . 'px;';
            }
            ...
            if ($datetime != '') {
                $output .= '<div id="' . $count_down_id . '"  class="ult-responsive ult_countdown-div ult_countdown-dateAndTime ' . $ult_tz . '" data-labels="' . $labels . '" data-labels2="' . $labels2 . '"  data-terminal-date="' . $datetime . '" data-countformat="' . $count_frmt . '" data-time-zone="' . get_option('gmt_offset') . '" data-time-now="' . str_replace('-',
                                                                                                                                                                                                                                                                                                                                                                               '/',
                                                                                                                                                                                                                                                                                                                                                                               current_time('mysql')) . '"  data-tick-col="' . $tick_col . '"  ' . $count_down_data_list . ' data-tick-p-col="' . $tick_sep_col . '" ' . $data_attr . '>' . $datetime . '</div>';
            }
            $output .= '</div>';
            $is_preset = FALSE;
