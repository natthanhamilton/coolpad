...
if (!class_exists('Ultimate_FancyText')) {
	class Ultimate_FancyText {
		function __construct() {
			add_action('init', [$this, 'ultimate_fancytext_init']);
			add_shortcode('ultimate_fancytext', [$this, 'ultimate_fancytext_shortcode']);
			add_action('wp_enqueue_scripts', [$this, 'register_fancytext_assets'], 1);
		}
		...
		function register_fancytext_assets() {
			...
			wp_register_style('ultimate-fancytext-style',
			                  plugins_url($css_path . 'fancytext' . $ext . '.css', __FILE__), [], ULTIMATE_VERSION);
			wp_register_script('ultimate-typed-js', plugins_url($js_path . 'typed' . $ext . '.js', __FILE__),
			                   ['jquery'], ULTIMATE_VERSION);
			wp_register_script('ultimate-vticker-js', plugins_url($js_path . 'vticker' . $ext . '.js', __FILE__),
			                   ['jquery'], ULTIMATE_VERSION);
		}
					...
					[
						...
						"params"      => [
							...
							[
								...
								"dependency"  => ["element" => "fancytext_effect", "value" => ["typewriter"]],
								"description" => __("Speed at which line progresses / Speed of typing effect.",
								                    "ultimate_vc")
							],
							[
								"type"        => "number",
							...
							[
								...
								"group"       => "Advanced Settings",
								"dependency"  => ["element" => "fancytext_effect", "value" => ["typewriter"]],
								"description" => __("Example - If set to 5000, the first string will appear after 5 seconds.",
								                    "ultimate_vc")
							],
							[
								"type"        => "number",
								...
								"group"       => "Advanced Settings",
								"dependency"  => ["element" => "fancytext_effect", "value" => ["typewriter"]],
								"description" => __("Example - If set to 5000, the string will remain visible for 5 seconds before backspace effect.",
								                    "ultimate_vc")
							],
							[
								"type"        => "ult_switch",
							...
							[
								...
								"value"      => "|",
								"group"      => "Advanced Settings",
								"dependency" => ["element" => "typewriter_cursor", "value" => ["true"]]
							],
							[
								"type"        => "number",
								...
								"suffix"      => __("In Miliseconds", "ultimate_vc"),
								"group"       => "Advanced Settings",
								"dependency"  => ["element" => "fancytext_effect", "value" => ["ticker", "ticker-down"]],
							...
							[
								...
								"group"      => "Advanced Settings",
								"group"      => "Typography",
								"dependency" => ["element" => "fancytext_effect", "value" => ["typewriter", "ticker", "ticker-down"]]
							],
							[
								"type"       => "colorpicker",
							...
							[
								"type"             => "ult_param_heading",
								"text"             => "<span style='display: block;'><a href='http://bsf.io/t5ir4' target='_blank'>" . __("Watch Video Tutorial",
		...
		function ultimate_fancytext_shortcode($atts, $content = NULL) {
			$output
				= $fancytext_strings = $fancytext_prefix = $fancytext_suffix = $fancytext_effect
				= $strings_textspeed = $strings_tickerspeed = $typewriter_cursor = $typewriter_cursor_text
				= $typewriter_loop = $fancytext_align = $strings_font_family = $strings_font_style = $strings_font_size
				= $sufpref_color = $strings_line_height = $strings_startdelay = $strings_backspeed
				= $strings_backdelay = $ticker_wait_time = $ticker_show_items = $ticker_hover_pause = $ex_class = '';
			$prefsuf_font_family
				= $prefsuf_font_style = $prefix_suffix_font_size = $prefix_suffix_line_height = $sufpref_bg_color = '';
			$id = uniqid(rand());
			...
			$css_design_style = apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG,
			                                  vc_shortcode_custom_css_class($css_fancy_design, ' '),
			                                  "ultimate_fancytext", $atts);
			...
			$output
				= '<' . $fancytext_tag . ' id="' . $fancy_text_id . '" ' . $data_list . ' class="uvc-type-wrap ' . $css_design_style . ' ' . $is_vc_49_plus . ' ult-responsive ' . $ex_class . ' uvc-wrap-' . $id . '" style="' . $string_inline_style . '">';
