...
if (!class_exists('Ultimate_Video_Banner')) {
    class Ultimate_Video_Banner {
        function __construct() {
            add_action('init', [$this, 'ultimate_video_banner_init']);
            add_shortcode('ultimate_video_banner', [$this, 'ultimate_video_banner_shortcode']);
            add_action("wp_enqueue_scripts", [$this, "register_video_banner_assets"], 1);
        }
        ...
        function register_video_banner_assets() {
            ...
            wp_register_style('ultimate-video-banner-style',
                              plugins_url($css_path . 'video-banner' . $ext . '.css', __FILE__), [], ULTIMATE_VERSION);
            wp_register_script('ultimate-video-banner-script',
                               plugins_url($js_path . 'video-banner' . $ext . '.js', __FILE__), ['jquery'],
                               ULTIMATE_VERSION);
        }
        ...
        function ultimate_video_banner_shortcode($atts, $content = NULL) {
            ...
            $vc_css_class = apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG,
                                          vc_shortcode_custom_css_class($video_banner_vc_css, ' '),
                                          'ultimate_video_banner', $atts);
            ...
            {
                $video_banner_overlay_hover_color = hex2rgbUltParallax($video_banner_overlay_hover_color,
                                                                       $opacity = 0.4);
            }
            /* ---- main heading styles ---- */
            $title_style_inline = '';
            ...
            $output
                = '<div id="' . $video_id . '" class="' . $vc_css_class . ' ult-video-banner ult-vdo-effect ' . $video_banner_effect . ' utl-video-banner-item ult-responsive" ' . $banner_height_responsive_data . ' data-current-time="' . $video_banner_start_time . '" data-placeholder="' . $placeholder . '" style="' . $placeholder_css . '">';
            ...
            if ($video_banner_title != '' || $content != '') :
                ...
                if ($video_banner_title != '') :
                    $output .= '<h2 class="ult-video-banner-title" style="' . $title_style_inline . '">' . __($video_banner_title,
                                                                                                              'ultimate_vc') . '</h2>';
                endif;
                if ($video_banner_content != '') :
                    $output .= '<div class="ult-video-banner-content" style="' . $desc_style_inline . '">' . __($video_banner_content,
                                                                                                                'ultimate_vc') . '</div>';
                endif;
                $output .= '</div>';
            endif;
