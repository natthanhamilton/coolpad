...
if (!class_exists("Ultimate_Carousel")) {
	class Ultimate_Carousel {
		function __construct() {
			add_action("init", [$this, "init_carousel_addon"]);
			add_shortcode("ultimate_carousel", [$this, "ultimate_carousel_shortcode"]);
			add_action("wp_enqueue_scripts", [$this, "ultimate_front_scripts"], 1);
			add_action("admin_enqueue_scripts", [$this, "custom_param_styles"]);
			add_action("admin_enqueue_scripts", [$this, "ultimate_admin_scripts"], 100);
		}
		...
		function ultimate_front_scripts() {
			...
			wp_register_script("ult-slick", plugins_url($js_path . "slick" . $ext . ".js", __FILE__), ['jquery'],
			                   ULTIMATE_VERSION, FALSE);
			wp_register_script("ult-slick-custom", plugins_url($js_path . "slick-custom" . $ext . ".js", __FILE__),
			                   ['jquery', 'ult-slick'], ULTIMATE_VERSION, FALSE);
			wp_register_style("ult-slick", plugins_url($css_path . "slick" . $ext . ".css", __FILE__), [],
			                  ULTIMATE_VERSION);
			wp_register_style("ult-icons", plugins_url("../assets/css/icons.css", __FILE__), [], ULTIMATE_VERSION);
		}
			...
			if (function_exists("vc_map")) {
				vc_map(
					[
						...
						"class"                   => "ultimate_carousel",
						"as_parent"               => ['except' => 'ultimate_carousel'],
						...
						"params"                  => [
							...
							[
								...
								"value"      => ["All visible" => "all", "One at a Time" => "single"],
								//"description" => __("","smile"),
								"group"      => "General",
							],
							[
							...
							*/
							[
								...
								"options"     => [
									"on" => [
										"label" => __("Restart the slider automatically as it passes the last slide.",
										              "ultimate_vc"),
										"on"    => "Yes",
										"off"   => "No",
									],
							...
							*/
							...
							[
								...
								"suffix"     => "ms",
								//"description" => __("","smile"),
								"dependency" => ["element" => "autoplay", "value" => ["on"]],
								"group"      => "General",
							],
							/*
							array(
							...
							*/
							...
							[
								...
								//"description" => __("","smile"),
								"dependency" => ["element" => "arrows", "value" => ["show"]],
								"group"      => "Navigation",
							],
							[
								"type"       => "colorpicker",
								...
								"value"      => "",
								//"description" => __("","smile"),
								"dependency" => ["element" => "arrow_style", "value" => ["circle-bg", "square-bg"]],
								"group"      => "Navigation",
							],
							[
								"type"       => "colorpicker",
								...
								"value"      => "",
								//"description" => __("","smile"),
								"dependency" => ["element" => "arrow_style", "value" => ["circle-border", "square-border"]],
								"group"      => "Navigation",
							],
							[
								"type"       => "number",
							...
							*/
							...
							[
								...
								"value"      => "#333333",
								//"description" => __("","smile"),
								"dependency" => ["element" => "dots", "value" => ["show"]],
								"group"      => "Navigation",
							],
							[
								"type"       => "ultimate_navigation",
							...
							*/
							[
								...
								//"description" => __("", "smile"),
								"dependency"  => ["element" => "draggable", "value" => ["on"]],
								"default_set" => TRUE,
								"group"       => "Advanced",
							],
							[
							...
							[
								...
								"text"             => "<span style='display: block;'><a href='http://bsf.io/bzyci' target='_blank'>" . __("Watch Video Tutorial",
		...
		function ultimate_carousel_shortcode($atts, $content) {
			$slider_type
				            = $slides_on_desk = $slides_on_tabs
				= $slides_on_mob = $slide_to_scroll = $speed = $infinite_loop = $autoplay = $autoplay_speed = '';
			$lazyload
				            = $arrows
				= $dots = $dots_icon = $next_icon = $prev_icon = $dots_color = $draggable = $swipe = $touch_move = '';
			$rtl
				            = $arrow_color = $arrow_size
				= $arrow_style = $arrow_bg_color = $arrow_border_color = $border_size = $item_space = $el_class = '';
			$item_animation = '';
			...
			$desing_style = apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG,
			                              vc_shortcode_custom_css_class($css_ad_caraousel, ' '), "ultimate_carousel",
			                              $atts);
...
if (!function_exists('ultimate_wrap_shortcode_in_div')) {
	function ultimate_wrap_shortcode_in_div($attr, $content = NULL, $tag) {
		...
		return '<div class="ult-item-wrap" data-animation="animated ' . $_shortcode_tags["item_animation"] . '">' . call_user_func($_shortcode_tags[ $tag ],
		                                                                                                                           $attr,
		                                                                                                                           $content,
		                                                                                                                           $tag) . '</div>';
	}
