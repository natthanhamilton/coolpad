...
if (!class_exists('Ultimate_Swatch_Book')) {
	class Ultimate_Swatch_Book {
		...
		function __construct() {
			add_action('init', [$this, 'swatch_book_init']);
			add_action("wp_enqueue_scripts", [$this, "register_swatch_assets"], 1);
			if (function_exists('vc_is_inline')) {
				if (!vc_is_inline()) {
					add_shortcode('swatch_container', [$this, 'swatch_container']);
					add_shortcode('swatch_item', [$this, 'swatch_item']);
				}
			} else {
				add_shortcode('swatch_container', [$this, 'swatch_container']);
				add_shortcode('swatch_item', [$this, 'swatch_item']);
		...
		function register_swatch_assets() {
			...
			wp_register_script("swatchbook-js", plugins_url($js_path . "swatchbook" . $ext . ".js", __FILE__),
			                   ['jquery'], ULTIMATE_VERSION);
			wp_register_style("swatchbook-css", plugins_url($css_path . "swatchbook" . $ext . ".css", __FILE__), [],
			                  ULTIMATE_VERSION);
		}
			...
			if (function_exists('vc_map')) {
					...
					[
						...
						"params"                  => [
							...
							[
								...
								"max"         => 100,
								"suffix"      => "",
								"description" => __("The index of the “centered” item, the one that will have an angle of 0 degrees when the swatch book is opened",
								                    "ultimate_vc"),
								"dependency"  => ["element" => "swatch_style", "value" => "custom"],
								"group"       => "Initial Settings",
							],
							[
								"type"        => "number",
							...
							[
								...
								"max"         => 10000,
								"suffix"      => "",
								"description" => __("Distance From Opened item’s next siblings (neighbor : 4)",
								                    "ultimate_vc"),
								"dependency"  => ["element" => "swatch_style", "value" => "custom"],
								"group"       => "Initial Settings",
							],
				...
				vc_map(
					[
						...
						"icon"            => "vc_swatch_item",
						"content_element" => TRUE,
						"as_child"        => ['only' => 'swatch_container'],
						"is_container"    => FALSE,
						"params"          => [
							[
								"type"       => "textfield",
		...
		function swatch_container($atts, $content = NULL) {
			$swatch_style
				= $swatch_index_center = $swatch_space_degree
				= $swatch_trans_speed = $swatch_distance_sibling = $swatch_init_closed = $swatch_open_at
				= $swatch_width = $swatch_height = $swatch_trans_bg_img = $swatch_main_strip_text
				= $swatch_main_strip_highlight_text = $swatch_main_strip_font_size = $swatch_main_strip_font_style
				= $swatch_main_strip_color = $swatch_main_strip_highlight_font_size
				= $swatch_main_strip_highlight_font_weight = $swatch_main_strip_highlight_color
				= $swatch_main_strip_bg_color = $main_strip_font_family = $main_strip_font_style = '';
		...
		function swatch_item($atts, $content = NULL) {
			$icon_type
				...
				= $icon_border_spacing
				= $el_class = $icon_animation = $swatch_strip_font_size = $swatch_strip_font_weight
				= $swatch_strip_font_color = $swatch_strip_bg_color = $swatch_strip_title_bg_color = '';
			extract(shortcode_atts([
				                       'swatch_strip_text'           => '',
				                       'icon_type'                   => '',
				                       'icon'                        => '',
