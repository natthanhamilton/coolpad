...
if (!class_exists('ULT_TAB_ELEMENT')) {
	class ULT_TAB_ELEMENT {
		function __construct() {
			...
			add_action('init', [$this, 'ult_tab_init']);
			//add_action('admin_init', 'ult_tab_init');
			add_action('admin_print_scripts', 'ultimate_tabs_admin', 999);
			add_shortcode('ult_tab_element', [$this, 'ultimate_main_tab']);
			add_shortcode('single_tab', [$this, 'ultimate_single_tab']);
		}
		...
		function ultimate_single_tab($atts, $content = NULL) {
			$output = $title = $tab_id = $font_icons_position = $icon_type = $icon = $icon_color = $icon_hover_color
				= $icon_size
				= $icon_background_color = $icon_margin_bottom = $icon_margin_left = $icon_margin = $ul_sub_class = '';
			...
			$css_class = apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG,
			                           'wpb_tab ui-tabs-panel wpb_ui-tabs-hide vc_clearfix ult_back', 'single_tab',
			                           $atts);
...
//return $actab;
		...
		function ult_tab_init() {
			...
			$settings = [
				...
				'admin_enqueue_css' => preg_replace('/\s/', '%20',
				                                    plugins_url('../admin/vc_extend/css/sub-tab.css', __FILE__)),
				...
				'params'            => [
					[
						...
						"value"      => ['Style 1' => 'Style_1', 'Style 2' => 'Style_2', 'Style 3' => 'Style_3', 'Style 4' => 'Style_4', 'Style 5' => 'Style_5', 'Style 6' => 'Style_6'],
						"group"      => "General ",
					],
					[
						"type"       => "dropdown",
					...
					[
						...
						"value"      => ["Disable" => "Disables", "Enable" => "Enable"],
						"group"      => "General ",
					],
					[
						'type'       => 'number',
						...
						'value'      => '',
						"suffix"     => "seconds",
						"dependency" => ["element" => "auto_rotate", "value" => ["Enable"]],
						"group"      => "General ",
					],
					[
						'type'        => 'number',
					...
					[
						...
						"dependency"       => ["element" => "tab_style", "value" => ["Style_4", "Style_1", "Style_3", "Style_5", "Style_6"]],
						"edit_field_class" => "vc_col-sm-12 vc_column ult_space_border",
					],
					[
						"type"       => "dropdown",
						...
						"value"      => ['Disable' => 'Disable', 'Enable' => 'Enable'],
						"dependency" => ["element" => "tab_style", "value" => ["Style_1", "Style_3", "Style_4"]],
						"group"      => "Tab",
					],
					[
						"type"       => "colorpicker",
						...
						"value"      => "",
						"dependency" => ["element" => "tab_bottom_border", "value" => ["Enable"]],
						"group"      => "Tab",
					],
					[
						'type'       => 'number',
					...
					[
						...
						"max"              => 100,
						"suffix"           => "px",
						"description"      => __("This border will assign to border-top-left and border-top-right for first and last tab element . As you increase the value, the shape convert in circle slowly. (e.g 100 pixels).",
						                         "ultimate_vc"),
						"group"            => "Tab",
						"edit_field_class" => "vc_col-sm-12 vc_column ult_space_border",
					],
					[
						...
						"value"      => "",
						"dependency" => ["element" => "tab_style", "value" => ["Style_5", "Style_6"]],
						"group"      => "Tab",
					],
					/*---------icon----------------*/
					[
						...
						"value"       => ["Enable" => "Enable", "Disable" => "Disables"],
						"group"       => "Icon ",
					...
					[
						...
						"group"      => "Icon ",
						"dependency" => ["element" => "disp_icon", "value" => ["Enable"]],
					],
					[
						"type"             => "number",
					...
					[
						...
						"label_color"  => __("Container Border Color",
						                     "ultimate_vc"),       //  label for 'border color'   default 'Border Color'
						"label_radius" => __("Container Border Radius",
					...
					[
						...
						"group"       => "Responsive",
						"description" => __("Display normal tab or convert them into accordion on responsive devices.",
						                    "ultimate_vc"),
					],
					[
						"type"        => "number",
						...
						"dependency"  => ["element" => "resp_type", "value" => ["Accordion"]],
						"group"       => "Responsive",
					],
					[
						"type"       => "dropdown",
						...
						"group"       => "Responsive",
						"dependency"  => ["element" => "resp_type", "value" => ["Tabs"]],
					...
					[
						...
						"description" => __("Select the font of your choice. You can <a target='_blank' href='" . admin_url('admin.php?page=bsf-google-font-manager') . "'>add new in the collection here</a>.",
						                    "imedica"),
						"group"       => "Typography",
					],
					[
				...
				vc_map([
					       ...
					       'params'                    => [
						       ...
						       [
							       ...
							       "value"       => "",
							       "group"       => "Icon",
							       "description" => __("Click and select icon of your choice. If you can't find the one that suits for your purpose, you can <a href='admin.php?page=bsf-font-icon-manager' target='_blank'>add new here</a>.",
							                           "smile"),
			...
			function ultimate_tabs() {
				...
				wp_register_script('ult_tabs_rotate', plugins_url($js_path . 'tabs' . $ext . '.js', __FILE__),
				                   ['jquery'], ULTIMATE_VERSION, TRUE);
				wp_register_style('ult_tabs', plugins_url($css_path . 'tabs' . $ext . '.css', __FILE__));
				wp_register_script('ult_tabs_acordian_js',
				                   plugins_url($js_path . 'tabs-accordion' . $ext . '.js', __FILE__), ['jquery'],
				                   ULTIMATE_VERSION, TRUE);
				wp_register_style('ult_tabs_acordian',
				                  plugins_url($css_path . 'tabs-accordion' . $ext . '.css', __FILE__));
			}
			...
			function ultimate_tabs_admin() {
				...
				wp_register_script('tab-js-1',
				                   plugins_url('../admin/vc_extend/js/ult_tab_admin_enqueue_js.js', __FILE__),
				                   ['jquery'], ULTIMATE_VERSION, TRUE);
				wp_register_script('tab-js-2', plugins_url('../admin/vc_extend/js/single_element_js.js', __FILE__),
				                   ['jquery'], ULTIMATE_VERSION, TRUE);
	...
	class WPBakeryShortCode_ULT_TAB_ELEMENT extends WPBakeryShortCode {
		...
		public function contentAdmin($atts, $content = NULL) {
			...
			extract(shortcode_atts(
				        ...
				        , $atts));
			// Extract tab titles
			preg_match_all('/vc_tab title="([^\"]+)"(\stab_id\=\"([^\"]+)\"){0,1}/i', $content, $matches,
			               PREG_OFFSET_CAPTURE);
			$output     = '';
			$tab_titles = [];
			if (isset($matches[0])) {
			...
			if (count($tab_titles)) {
				...
				foreach ($tab_titles as $tab) {
					preg_match('/title="([^\"]+)"(\stab_id\=\"([^\"]+)\"){0,1}/i', $tab[0], $tab_matches,
					           PREG_OFFSET_CAPTURE);
					if (isset($tab_matches[1][0])) {
						$tmp .= '<li><a href="#tab-' . (isset($tab_matches[3][0]) ? $tab_matches[3][0]
								: sanitize_title($tab_matches[1][0])) . '">' . $tab_matches[1][0] . '</a></li>';
					}
				}
				$tmp .= '</ul>' . "\n";
			} else {
			...
			if (isset($this->settings["custom_markup"]) && $this->settings["custom_markup"] != '') {
				if ($content != '') {
					...
					if ($content == '' && isset($this->settings["default_content_in_template"]) && $this->settings["default_content_in_template"] != '') {
						$custom_markup = str_ireplace("%content%", $this->settings["default_content_in_template"],
