...
if (!class_exists('Ultimate_Image_Separator')) {
	class Ultimate_Image_Separator {
		function __construct() {
			add_action('init', [$this, 'ultimate_img_separator_init']);
			add_shortcode('ultimate_img_separator', [$this, 'ultimate_img_separator_shortcode']);
			add_action('wp_enqueue_scripts', [$this, 'register_easy_separator_assets'], 1);
		}
		...
		function register_easy_separator_assets() {
			...
			wp_register_style('ult-easy-separator-style',
			                  plugins_url($css_path . 'image-separator' . $ext . '.css', __FILE__), [],
			                  ULTIMATE_VERSION);
			wp_register_script('ult-easy-separator-script',
			                   plugins_url($js_path . 'image-separator' . $ext . '.js', __FILE__), ['jquery'],
			                   ULTIMATE_VERSION);
		}
					...
					[
						...
						"params"      => [
							...
							[
								...
								"max"         => 100,
								"suffix"      => "s",
								"description" => __("How long the animation effect should last. Decides the speed of effect.",
								                    "ultimate_vc"),
								"group"       => "Animation",
							],
							[
								...
								"max"         => 100,
								"suffix"      => "s",
								"description" => __("Delays the animation effect for seconds you enter above.",
								                    "ultimate_vc"),
								"group"       => "Animation"
							],
							[
								...
								"max"         => 100,
								"suffix"      => "",
								"description" => __("The animation effect will repeat to the count you enter above. Enter 0 if you want to repeat it infinitely.",
								                    "ultimate_vc"),
								"group"       => "Animation"
							],
							[
								...
								//"admin_label" => true,
								"value"       => "90",
								"description" => __("The area of screen from top where animation effects will start working.",
								                    "ultimate_vc"),
								"group"       => "Animation"
							],
							[
							...
							[
								...
								'suffix'      => '%',
								'description' => __('50% is default. Increase to push the image outside or decrease to pull the image inside.',
								                    'ultimate_vc')
							],
							[
								"type"        => "vc_link",
		...
		function ultimate_img_separator_shortcode($atts, $content) {
			$output
				          = $wrapper_class = $custom_position = $opacity_start_effect_data
				= $animation_style = $animation_el_class = $animation_data = $href = $url = $link_title = $target = '';
			$is_animation = FALSE;
			...
			if (strtolower($animation) !== strtolower('No Animation')) {
				$is_animation = TRUE;
				$inifinite_arr
				              = ["InfiniteRotate", "InfiniteDangle", "InfiniteSwing", "InfinitePulse", "InfiniteHorizontalShake", "InfiniteBounce", "InfiniteFlash", "InfiniteTADA"];
			...
			$output
				= '<div id="' . $id . '" class="ult-easy-separator-wrapper ult-responsive ' . $img_separator_position . ' ' . $wrapper_class . '" style="' . $custom_position . '" data-vc-row="' . $ultimate_custom_vc_row . '" ' . $data_list . '>';
