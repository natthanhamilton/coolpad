...
if (!class_exists('Ultimate_Google_Font_Manager')) {
    class Ultimate_Google_Font_Manager {
        function __construct() {
            add_option('ultimate_google_fonts', []);
            //delete_option('ultimate_google_fonts');
            add_option('ultimate_selected_google_fonts', []);
            //add_action('admin_menu',array($this, 'google_font_manager_menu'));
            add_action('wp_ajax_ultimate_google_fonts_refresh', [$this, 'refresh_google_fonts_list']);
            add_action('wp_ajax_get_google_fonts', [$this, 'get_google_fonts_list']);
            add_action('wp_ajax_add_google_font', [$this, 'add_selected_google_font']);
            add_action('wp_ajax_delete_google_font', [$this, 'delete_selected_google_font']);
            add_action('wp_ajax_update_google_font', [$this, 'update_selected_google_font']);
            add_action('wp_ajax_get_font_variants', [$this, 'get_font_variants_callback']);
            add_action('admin_enqueue_scripts', [$this, 'enqueue_selected_ultimate_google_fonts']);
        }
        ...
        function google_font_manager_menu() {
            ...
            if ($current_theme == "Smile") {
                $page = add_submenu_page(
                    ...
                    [$this, 'ultimate_font_manager_dashboard']
                );
            } else {
                $page = add_submenu_page(
                    "bsf-dashboard",
            ...
            add_action('admin_print_scripts-' . $page, [$this, 'admin_google_font_scripts']);
        }

        function admin_google_font_scripts() {
            wp_register_script('ultimate-google-fonts-script',
                               plugins_url("../admin/js/google-fonts-admin.js", __FILE__), ['jquery']);
            wp_enqueue_script('ultimate-google-fonts-script');
            wp_register_style('ultimate-google-fonts-style',
                              plugins_url("../admin/css/google-fonts-admin.css", __FILE__));
        ...
        function enqueue_selected_ultimate_google_fonts() {
            ...
            if (!empty($selected_fonts)) {
                ...
                wp_register_style('ultimate-selected-google-fonts-style', $font_api_call, [], NULL);
        ...
        function ultimate_font_manager_dashboard() {
            ...
            <div class="wrap uavc-gfont">
                <h2>
                    ...
                    <input style="cursor:pointer" type="button" class="add-new-h2" id="refresh-google-fonts"
                           value="<?php _e('Refresh Font List', 'ultimate_vc'); ?>"/>
                    &nbsp;<span class="spinner"></span>
                </h2>
                <div id="vc-gf-msg"></div>
                <div class="nav">
                    <input type="text" id="search_gfont" name="search_gfont"
                           placeholder="<?php echo __('Search font..', 'ultimate_vc') ?>"/>
                </div>
                <div>
                            ...
                            if (!empty($selected_fonts)) {
                                foreach ($selected_fonts as $key => $sfont) {
                                    ...
                                    <div class="selected-font">
                                        <div
                                            class="selected-font-top <?php echo (!empty($sfont['variants']) || !empty($sfont['subsets']))
                                                ? 'have-variants' : ''; ?>">
                                            <div class="font-header"
                                                 style="font-family:'<?php echo $sfont['font_name'] ?>'"><?php echo $sfont['font_name'] ?></div>
                                            <?php if (!empty($sfont['variants']) || !empty($sfont['subsets'])) : ?>
                                                <i class="dashicons dashicons-arrow-down"></i>
                                            <?php endif; ?>
                                            <div class="clear"></div>
                                        </div>
                                        <span class="font-delete" data-font_name="<?php echo $sfont['font_name'] ?>"><i
                                        ...
                                        if (!empty($sfont['variants']) || !empty($sfont['subsets'])) :
                                            ...
                                            <div class="selected-font-content">
                                                ...
                                                if (!empty($sfont['variants'])) :
                                                    ...
                                                    <div class="selected-font-varient-wrapper">
                                                        ...
                                                        foreach ($sfont['variants'] as $svkey => $svariants) {
                                                            ...
                                                            $weight = 'normal';
                                                            if ($weight = preg_replace('/\D/', '',
                                                                                       $svariants['variant_value'])
                                                            ) {
                                                            ...
                                                            <span class="font-variant">
                                                                            <input type="checkbox"
                                                                                   id="<?php echo $tlid ?>"
                                                                                   value="<?php echo $svariants['variant_value'] ?>"
                                                                                   class="selected-variant-checkbox" <?php echo ($svariants['variant_selected'] == 'true')
                                                                                ? 'checked' : ''; ?> />
                                                                            <label style="<?php echo $variant_style; ?>"
                                                                                   for="<?php echo $tlid ?>"><?php echo $svariants['variant_value'] ?></label>
                                                                        </span>
                                                            <?php
                                                        }
                                                        ?>
                                                    ...
                                                    <div
                                                        ...
                                                        foreach ($sfont['subsets'] as $sbkey => $ssubset) {
                                                            ...
                                                            <span class="font-subset">
                                                                            <input type="checkbox"
                                                                                   id="<?php echo $slid ?>"
                                                                                   value="<?php echo $ssubset['subset_value'] ?>"
                                                                                   class="selected-subset-checkbox" <?php echo ($ssubset['subset_selected'] == 'true')
                                                                                ? 'checked' : '' ?> />
                                                                            <label style=""
                                                                                   for="<?php echo $slid ?>"><?php echo $ssubset['subset_value'] ?></label>
                                                                        </span>
                                                            <?php
                                                        }
                                                        ?>
                                                ...
                                                ?>
                                                <input type="button" class="button alignleft update-google-font-button"
                                                       value="<?php echo __('Update font', 'ultimate_vc') ?>"
                                                       data-font_name="<?php echo $sfont['font_name'] ?>"/>
        ...
        function refresh_google_fonts_list() {
            ...
            try {
                $fonts = file_get_contents($filename
                                               = 'https://www.googleapis.com/webfonts/v1/webfonts?key=AIzaSyD_6TR2RyX2VRf8bABDRXCcVqdMXB5FQvs');
            ...
            if ($error != TRUE || count($fonts) == 0) {
                ...
                $response['count'] = 0;
                $response['message']
                                   = __('Fonts could not be downloaded as there might be some issue with file_get_contents or wp_remote_get due to your server configuration.',
                                        'ultimate_vc');
            }
        ...
        function get_font_variants_callback() {
            ...
            if (!empty($font_variants)) {
                ...
                if ($is_italic == FALSE) {
                    array_push($pre_default_variants, $default_variant_styles[1]);
                } //2 for bold
                if ($is_weight == FALSE) {
                    array_push($pre_default_variants, $default_variant_styles[2]);
                } //2 for bold
    ...
    function enquque_ultimate_google_fonts($enqueue_fonts) {
        ...
        if (!empty($enqueue_fonts)) {
            foreach ($enqueue_fonts as $key => $efont) {
                ...
                $font_arr  = $font_call_arr = $font_weight_arr = [];
...
if (!function_exists('enquque_ultimate_google_fonts_optimzed')) {
    function enquque_ultimate_google_fonts_optimzed($enqueue_fonts) {
        ...
        if (!empty($enqueue_fonts)) {
                        ...
                        if (isset($font_call_arr[1]) && $font_call_arr[1] != '') {
                            ...
                            if ($font_variant != '' && $font_variant !== 'regular') {
                                ...
                                if (!empty($main[ $font_name ]['varients'])) {
                                    $main[ $font_name ]['varients']
                                        = array_values(array_unique($main[ $font_name ]['varients']));
                                }
                            }
                        }
                    }
            ...
            $font_api_call = $link . $subset_string;
            if ($font_count > 0) {
                wp_enqueue_style('ultimate-google-fonts', $font_api_call, [], NULL);
            }
        }
    }
}