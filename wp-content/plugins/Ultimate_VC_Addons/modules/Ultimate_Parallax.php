...
if (!class_exists('VC_Ultimate_Parallax')) {
	class VC_Ultimate_Parallax {
		function __construct() {
			...
			add_action('admin_init', [$this, 'parallax_init']);
			$ultimate_row = get_option('ultimate_row');
			//echo WPB_VC_VERSION;
			if ($ultimate_row == "enable") {
				if (defined('WPB_VC_VERSION') && version_compare(WPB_VC_VERSION, '4.4', '>=')) {
					add_filter('vc_shortcode_output', [$this, 'execute_ultimate_vc_shortcode'], 10, 3);
				}
			}
		...
		public static function parallax_shortcode($atts, $content) {
			$bg_type
				= $bg_image = $bg_image_new = $bsf_img_repeat = $parallax_style = $video_opts = $video_url
				= $video_url_2 = $video_poster = $bg_image_size = $bg_image_posiiton = $u_video_url = $parallax_sense
				= $bg_cstm_size = $bg_override = $bg_img_attach = $u_start_time = $u_stop_time = $layer_image = $css
				= $animation_type = $horizontal_animation = $vertical_animation = $animation_speed = $animated_bg_color
				= $fadeout_row = $fadeout_start_effect = $parallax_content = $parallax_content_sense
				= $disable_on_mobile
				= $disable_on_mobile_img_parallax = $animation_repeat = $animation_direction = $enable_overlay
				= $overlay_color
				=
			$overlay_pattern = $overlay_pattern_opacity = $overlay_pattern_size = $multi_color_overlay = $overlay = "";
			$seperator_html
				= $seperator_bottom_html = $seperator_top_html = $seperator_css = $seperator_enable = $seperator_type
				= $seperator_position = $seperator_shape_size = $seperator_shape_background
				= $seperator_shape_border = $seperator_shape_border_color = $seperator_shape_border_width = '';
			$ult_hide_row
				= $ult_hide_row_large_screen = $ult_hide_row_desktop
				=
			$ult_hide_row_tablet = $ult_hide_row_tablet_small = $ult_hide_row_mobile = $ult_hide_row_mobile_large = '';
			...
			if ((get_option('ultimate_row') == "enable" && $bg_type !== "") || $parallax_content != '' || $fadeout_row != '') {
				...
				if ($ultimate_js != 'enable') :
					if ($bg_type == 'no_bg' && ($parallax_content != '' || $fadeout_row != '')) {
						...
						if ($bg_type != 'no_bg' && ($parallax_content != '' || $fadeout_row != '')) {
							...
							wp_enqueue_script('ultimate-row-bg',
							                  plugins_url($js_path, __FILE__) . 'ultimate_bg' . $ext . '.js');
							wp_enqueue_script('ultimate-custom');
						} else {
							if ($bg_type != 'no_bg' && ($parallax_content == '' || $fadeout_row == '')) {
								wp_enqueue_script('ultimate-appear');
				...
				$html
					= $autoplay = $muted
					=
				$loop = $pos_suffix = $bg_img = $bg_img_id = $icon_inline = $commom_data_attributes = $vc_version = '';
				...
				if ($enable_overlay == 'enable_overlay_value') {
					...
					$overlay
						= ' data-overlay="true" data-overlay-color="' . $overlay_color . '" data-overlay-pattern="' . $pattern_url . '" data-overlay-pattern-opacity="' . $overlay_pattern_opacity . '" data-overlay-pattern-size="' . $overlay_pattern_size . '" data-overlay-pattern-attachment="' . $overlay_pattern_attachment . '" ';
					...
					$overlay
						= ' data-overlay="false" data-overlay-color="" data-overlay-pattern="" data-overlay-pattern-opacity="" data-overlay-pattern-size="" ';
				}
				// for seperator
				...
				if ($bg_type == "no_bg") {
					...
					if ($parallax_style == 'vcpb-fs-jquery' || $parallax_style == "vcpb-mlvp-jquery") {
						if ($parallax_style == 'vcpb-fs-jquery') {
							wp_enqueue_script('jquery.shake',
							                  plugins_url($js_path . 'jparallax' . $ext . '.js', __FILE__));
						}
						if ($parallax_style == "vcpb-mlvp-jquery") {
							wp_enqueue_script('jquery.vhparallax',
							                  plugins_url($js_path . 'vhparallax' . $ext . '.js', __FILE__));
						}
					...
					wp_enqueue_script('jquery.ytplayer',
					                  plugins_url($js_path . 'mb-YTPlayer' . $ext . '.js', __FILE__));
					$v_opts = explode(",", $video_opts);
					$v_img  = apply_filters('ult_get_img_single', $video_poster, 'url');
					if (is_array($v_opts)) {
						foreach ($v_opts as $opt) {
		...
		function parallax_init() {
			...
			foreach ($patterns_list as $pattern) {
				$patterns[ basename($pattern) ]
					= plugins_url() . '/' . $pluginname . '/assets/images/patterns/' . basename($pattern);
			}
			/*
			...
			if (function_exists('vc_add_param')) {
				vc_add_param('vc_row', [
					                     ...
					                     "description" => __("Select the kind of background would you like to set for this row.",
					                                         "ultimate_vc") . " " . __("Not sure?",
					                                                                   "ultimate_vc") . " " . __("See Narrated",
					                                                                                             "ultimate_vc") . " <a href='https://www.youtube.com/watch?v=Qxs8R-uaMWk&list=PL1kzJGWGPrW981u5caHy6Kc9I1bG1POOx' target='_blank'>" . __("Video Tutorials",
					                                                                                                                                                                                                                                     "ultimate_vc") . "</a>",
					                     "group"       => $group_name,
				                     ]
				);
				vc_add_param('vc_row', [
					                     ...
					                     "param_name"  => "bg_grad",
					                     "description" => __('At least two color points should be selected.',
					                                         'ultimate_vc') . ' <a href="https://www.youtube.com/watch?v=yE1M4AKwS44" target="_blank">' . __('Video Tutorial',
					                                                                                                                                         'ultimate_vc') . '</a>',
					                     "dependency"  => ["element" => "bg_type", "value" => ["grad"]],
					                     "group"       => $group_name,
				                     ]
				);
				vc_add_param('vc_row', [
					                     ...
					                     "dependency" => ["element" => "bg_type", "value" => ["bg_color"]],
					                     "group"      => $group_name,
				                     ]
				);
				vc_add_param("vc_row", [
				...
				vc_add_param('vc_row', [
					                     ...
					                     "value"       => "",
					                     "description" => __("Upload or select background image from media gallery.",
					                                         "ultimate_vc"),
					                     "dependency"  => ["element" => "parallax_style", "value" => ["vcpb-default", "vcpb-animated", "vcpb-vz-jquery", "vcpb-hz-jquery",]],
					                     "group"       => $group_name,
				                     ]
				);
				vc_add_param('vc_row', [
					                     ...
					                     "value"       => "",
					                     "description" => __("Upload or select background images from media gallery.",
					                                         "ultimate_vc"),
					                     "dependency"  => ["element" => "parallax_style", "value" => ["vcpb-fs-jquery", "vcpb-mlvp-jquery"]],
					                     "group"       => $group_name,
				                     ]
				);
				vc_add_param('vc_row', [
					                     ...
					                     "description" => __("Options to control repeatation of the background image.",
					                                         "ultimate_vc") . " " . __("Learn on",
					                                                                   "ultimate_vc") . " <a href='http://www.w3schools.com/cssref/playit.asp?filename=playcss_background-repeat' target='_blank'>" . __("W3School",
					                                                                                                                                                                                                     "ultimate_vc") . "</a>",
					                     "dependency"  => ["element" => "parallax_style", "value" => ["vcpb-default", "vcpb-fix", "vcpb-vz-jquery", "vcpb-hz-jquery"]],
					                     "group"       => $group_name,
				                     ]
				);
				vc_add_param('vc_row', [
					                     ...
					                     "value"       => [
						                     ...
						                     __("Contain - Image will try to fit inside the container area",
						                        "ultimate_vc")                                             => "contain",
					                     ...
					                     "description" => __("Options to control repeatation of the background image.",
					                                         "ultimate_vc") . " " . __("Learn on",
					                                                                   "ultimate_vc") . " <a href='http://www.w3schools.com/cssref/playit.asp?filename=playcss_background-size&preval=50%25' target='_blank'>" . __("W3School",
					                                                                                                                                                                                                                "ultimate_vc") . "</a>",
					                     "dependency"  => ["element" => "parallax_style", "value" => ["vcpb-default", "vcpb-animated", "vcpb-fix", "vcpb-vz-jquery", "vcpb-hz-jquery"]],
					                     "group"       => $group_name,
				                     ]
				);
				vc_add_param('vc_row', [
					                     ...
					                     "value"       => "",
					                     "description" => __("You can use initial, inherit or any number with px, em, %, etc. Example- 100px 100px",
					                                         "ultimate_vc"),
					                     "dependency"  => ["element" => "bg_image_size", "value" => ["cstm"]],
					                     "group"       => $group_name,
				                     ]
				);
				vc_add_param('vc_row', [
					                     ...
					                     "description" => __("Options to set whether a background image is fixed or scroll with the rest of the page.",
					                                         "ultimate_vc"),
					                     "dependency"  => ["element" => "parallax_style", "value" => ["vcpb-default", "vcpb-animated", "vcpb-hz-jquery", "vcpb-vz-jquery"]],
					                     "group"       => $group_name,
				                     ]
				);
				vc_add_param('vc_row', [
					                     ...
					                     "value"       => "",
					                     "max"         => "100",
					                     "description" => __("Control speed of parallax. Enter value between 1 to 100 (Default 30)",
					                                         "ultimate_vc"),
					                     "dependency"  => ["element" => "parallax_style", "value" => ["vcpb-vz-jquery", "vcpb-animated", "vcpb-hz-jquery", "vcpb-vs-jquery", "vcpb-hs-jquery", "vcpb-fs-jquery", "vcpb-mlvp-jquery"]],
					                     "group"       => $group_name,
				                     ]
				);
				vc_add_param('vc_row', [
					                     ...
					                     "value"       => "",
					                     "description" => __("You can use any number with px, em, %, etc. Example- 100px 100px.",
					                                         "ultimate_vc"),
					                     "dependency"  => ["element" => "parallax_style", "value" => ["vcpb-default", "vcpb-fix"]],
					                     "group"       => $group_name,
				                     ]
				);
				vc_add_param('vc_row', [
					                     ...
					                     "dependency" => ["element" => "parallax_style", "value" => ["vcpb-animated"]],
					                     "group"      => $group_name,
				                     ]
				);
				vc_add_param('vc_row', [
				...
				vc_add_param('vc_row', [
					                     ...
					                     "dependency" => ["element" => "bg_type", "value" => ["video"]],
					                     "group"      => $group_name,
				                     ]
				);
				vc_add_param('vc_row', [
					                     ...
					                     "value"       => "",
					                     "description" => __("IE, Chrome & Safari",
					                                         "ultimate_vc") . " <a href='http://www.w3schools.com/html/html5_video.asp' target='_blank'>" . __("support",
					                                                                                                                                           "ultimate_vc") . "</a> " . __("MP4 format, while Firefox & Opera prefer WebM / Ogg formats.",
					                                                                                                                                                                         "ultimate_vc") . " " . __("You can upload the video through",
					                                                                                                                                                                                                   "ultimate_vc") . " <a href='" . home_url() . "/wp-admin/media-new.php' target='_blank'>" . __("WordPress Media Library",
					                                                                                                                                                                                                                                                                                                 "ultimate_vc") . "</a>.",
					                     "dependency"  => ["element" => "bg_type", "value" => ["video"]],
					                     "group"       => $group_name,
				                     ]
				...
				vc_add_param('vc_row', [
					                     ...
					                     "value"       => "",
					                     "description" => __("Enter YouTube url. Example - YouTube (https://www.youtube.com/watch?v=tSqJIIcxKZM) ",
					                                         "ultimate_vc"),
					                     "dependency"  => ["element" => "bg_type", "value" => ["u_iframe"]],
					                     "group"       => $group_name,
				                     ]
				);
				vc_add_param('vc_row', [
					                     ...
					                     "dependency" => ["element" => "bg_type", "value" => ["video", "u_iframe"]],
					                     "group"      => $group_name,
				                     ]
				);
				vc_add_param('vc_row', [
					                     ...
					                     "value"       => "",
					                     "description" => __("Placeholder image is displayed in case background videos are restricted (Ex - on iOS devices).",
					                                         "ultimate_vc"),
					                     "dependency"  => ["element" => "bg_type", "value" => ["video", "u_iframe"]],
					                     "group"       => $group_name,
				                     ]
				...
				vc_add_param('vc_row', [
					                     ...
					                     "value"       => "",
					                     "suffix"      => "seconds",
					                     "description" => __("You may start / stop the video at any point you would like.",
					                                         "ultimate_vc"),
					                     "dependency"  => ["element" => "bg_type", "value" => ["u_iframe"]],
					                     "group"       => $group_name,
				                     ]
				...
				vc_add_param('vc_row', [
					                     ...
					                     "description" => __("Video will be played only when user is on the particular screen position. Once user scroll away, the video will pause.",
					                                         "ultimate_vc"),
					                     "dependency"  => ["element" => "bg_type", "value" => ["video", "u_iframe"]],
					                     "group"       => $group_name,
				                     ]
				...
				vc_add_param('vc_row', [
					                     ...
					                     "description" => __("Display play / pause controls for the video on bottom right position.",
					                                         "ultimate_vc"),
					                     "dependency"  => ["element" => "bg_type", "value" => ["video"]],
					                     "group"       => $group_name,
				                     ]
				...
				vc_add_param('vc_row', [
					                     ...
					                     "dependency" => ["element" => "enable_controls", "value" => ["display_control"]],
					                     "group"      => $group_name,
				                     ]
				);
				vc_add_param('vc_row', [
					                     ...
					                     "description" => __("By default, the background will be given to the Visual Composer row. However, in some cases depending on your theme's CSS - it may not fit well to the container you are wishing it would. In that case you will have to select the appropriate value here that gets you desired output..",
					                                         "ultimate_vc"),
					                     "dependency"  => ["element" => "bg_type", "value" => ["u_iframe", "image", "video", "grad", "bg_color", "animated"]],
					                     "group"       => $group_name,
				                     ]
				);
				vc_add_param('vc_row', [
					                     ...
					                     "value"      => "",
					                     "options"    => [
						                     "disable_on_mobile_img_parallax_value" => [
							                     "label" => "",
							                     "on"    => __("Yes", "ultimate_vc"),
							                     "off"   => __("No", "ultimate_vc"),
						                     ]
					                     ...
					                     "group"      => $group_name,
					                     "dependency" => ["element" => "parallax_style", "value" => ["vcpb-animated", "vcpb-vz-jquery", "vcpb-hz-jquery", "vcpb-fs-jquery", "vcpb-mlvp-jquery"]],
				                     ]
				);
				vc_add_param('vc_row', [
					                     "type"             => "ult_switch",
					                     ...
					                     "description"      => __("If enabled, the elements inside row - will move slowly as user scrolls.",
					                                              "ultimate_vc")
				                     ]
				);
				vc_add_param('vc_row', [
					                     ...
					                     "value"       => "",
					                     "group"       => $group_effects,
					                     "description" => __("Enter value between 0 to 100 (Default 30)",
					                                         "ultimate_vc"),
					                     "dependency"  => ["element" => "parallax_content", "value" => ["parallax_content_value"]]
				                     ]
				);
				vc_add_param('vc_row', [
					                     "type"             => "ult_switch",
					                     ...
					                     "group"            => $group_effects,
					                     'edit_field_class' => 'uvc-divider last-uvc-divider vc_column vc_col-sm-12',
					                     "description"      => __("If enabled, the the content inside row will fade out slowly as user scrolls down.",
					                                              "ultimate_vc")
				                     ]
				);
				vc_add_param('vc_row', [
					                     ...
					                     "value"       => "",
					                     "group"       => $group_effects,
					                     "description" => __("The area of screen from top where fade out effect will take effect once the row is completely inside that area. (Default 30)",
					                                         "ultimate_vc"),
					                     "dependency"  => ["element" => "fadeout_row", "value" => ["fadeout_row_value"]]
				                     ]
				);
				...
				vc_add_param('vc_row', [
					...
					'value'       => '',
					'group'       => $group_effects,
					'dependency'  => ['element' => 'enable_overlay', 'value' => ['enable_overlay_value']],
				...
				vc_add_param(
					...
					[
						...
						'group'            => $group_effects,
						'dependency'       => ['element' => 'enable_overlay', 'value' => ['enable_overlay_value']],
						'description'      => __('Enter value between 0 to 100 (0 is maximum transparency, while 100 is minimum & default is 80)',
						                         'ultimate_vc'),
				...
				vc_add_param(
					...
					[
						...
						'group'            => $group_effects,
						'dependency'       => ['element' => 'enable_overlay', 'value' => ['enable_overlay_value']],
						'description'      => __('This is optional; sets the size of the pattern image manually.',
						                         'ultimate_vc'),
						'edit_field_class' => 'vc_column vc_col-sm-4',
					]
				);
				...
				vc_add_param(
					...
					[
						...
						'suffix'           => '%',
						'group'            => $group_effects,
						'dependency'       => ['element' => 'multi_color_overlay', 'value' => ['uvc-multi-color-bg']],
				...
				vc_add_param(
					...
					[
						...
						'group'            => $group_effects,
						'dependency'       => ['element' => 'seperator_enable', 'value' => ['seperator_enable_value']],
				...
				vc_add_param(
					...
					[
						...
						'suffix'      => 'px',
						'group'       => $group_effects,
						'dependency'  => ['element' => 'seperator_type', 'value' => ['triangle_seperator', 'circle_seperator', 'round_split_seperator']],
				...
				vc_add_param(
					...
					[
						...
						'description' => __('Mostly, this should be background color of your adjacent row section. (Default - White)',
						                    'ultimate_vc')
					]
				);
				vc_add_param(
				...
				vc_add_param(
					...
					[
						...
						"value"       => "",
						'group'       => $group_effects,
						"description" => __("Click and select icon of your choice. If you can't find the one that suits for your purpose",
						                    "ultimate_vc") . ", " . __("you can",
						                                               "ultimate_vc") . " <a href='admin.php?page=bsf-font-icon-manager' target='_blank'>" . __("add new here",
						                                                                                                                                        "ultimate_vc") . "</a>.",
						"dependency"  => ["element" => "icon_type", "value" => ["selector"]],
					]
				);
				vc_add_param(
					'vc_row',
				...
				vc_add_param(
					...
					[
						...
						'group'       => $group_effects,
						"description" => __("We have given three quick preset if you are in a hurry. Otherwise, create your own with various options.",
						                    "ultimate_vc"),
						"dependency"  => ["element" => "icon_type", "value" => ["selector"]],
					]
				);
				vc_add_param(
					...
					[
						...
						"dependency"  => ["element" => "icon_style", "value" => ["circle", "square", "advanced"]],
					]
				);
				vc_add_param(
					'vc_row',
					[
						...
						"dependency"  => ["element" => "icon_style", "value" => ["advanced"]],
					]
				);
				vc_add_param(
					'vc_row',
					[
						...
						"dependency"  => ["element" => "icon_border_style", "not_empty" => TRUE],
					]
				);
				vc_add_param(
					'vc_row',
				...
				vc_add_param(
					...
					[
						...
						"suffix"      => "px",
						'group'       => $group_effects,
						"description" => __("0 pixel value will create a square border. As you increase the value, the shape convert in circle slowly. (Default 500).",
						                    "ultimate_vc"),
						"dependency"  => ["element" => "icon_border_style", "not_empty" => TRUE],
					]
				);
				vc_add_param(
					...
					[
						...
						"suffix"      => "px",
						'group'       => $group_effects,
						"description" => __("Spacing from center of the icon till the boundary of border / background (Default - 50)",
						                    "ultimate_vc"),
						"dependency"  => ["element" => "icon_style", "value" => ["advanced"]],
					]
				);
				vc_add_param(
					...
					[
						...
						"dependency"  => ["element" => "icon_type", "value" => ["custom"]],
					]
				);
				vc_add_param(
					'vc_row',
				...
				vc_add_param(
					...
					[
						'type'             => 'ult_switch',
						'heading'          => '<i class="dashicons dashicons-welcome-view-site"></i> ' . __('Large Screen',
						                                                                                    'ultimate_vc'),
						...
						'group'            => $group_effects,
						"dependency"       => ["element" => "ult_hide_row", "value" => ["ult_hide_row_value"]],
						'edit_field_class' => 'vc_column vc_col-sm-4',
					]
				);
				vc_add_param(
					...
					[
						'type'             => 'ult_switch',
						'heading'          => '<i class="dashicons dashicons-desktop"></i> ' . __('Desktop',
						                                                                          'ultimate_vc'),
				...
				vc_add_param(
					...
					[
						'type'             => 'ult_switch',
						'heading'          => '<i class="dashicons dashicons-tablet" style="transform: rotate(90deg);"></i> ' . __('Tablet',
						                                                                                                           'ultimate_vc'),
				...
				vc_add_param(
					...
					[
						'type'             => 'ult_switch',
						'heading'          => '<i class="dashicons dashicons-tablet"></i> ' . __('Tablet Portrait',
						                                                                         'ultimate_vc'),
				...
				vc_add_param(
					...
					[
						'type'             => 'ult_switch',
						'heading'          => '<i class="dashicons dashicons-smartphone"></i> ' . __('Mobile',
						                                                                             'ultimate_vc'),
				...
				vc_add_param(
					...
					[
						'type'             => 'ult_switch',
						'heading'          => '<i class="dashicons dashicons-smartphone" style="transform: rotate(90deg);"></i> ' . __('Mobile Landscape',
						                                                                                                               'ultimate_vc'),
				...
				vc_add_param(
					...
					[
						...
						'text'             => __('In order for Effects below to work, you must select something except "default" in background tab.',
						                         'ultimate_vc') . ' ' . __('May be single color.') . ' <br> ' . __('Screenshot',
						                                                                                           'ultimate_vc') . ' - <a href="https://cloudup.com/cc1J8ZlcdZW" target="_blank">https://cloudup.com/cc1J8ZlcdZW</a>',
		...
		function admin_scripts($hook) {
			if ($hook == "post.php" || $hook == "post-new.php") {
				wp_register_script("ult-colorpicker", plugins_url("../admin/js/jquery-colorpicker.js ", __FILE__),
				                   ['jquery'], ULTIMATE_VERSION);
				wp_register_script("ult-classygradient",
				                   plugins_url("../admin/js/jquery-classygradient-min.js", __FILE__), ['jquery'],
				                   ULTIMATE_VERSION);
				wp_register_style("ult-classycolorpicker-style",
				                  plugins_url("../admin/css/jquery-colorpicker.css", __FILE__));
				wp_register_style("ult-classygradient-style",
				                  plugins_url("../admin/css/jquery-classygradient-min.css", __FILE__));
				$bsf_dev_mode = bsf_get_option('dev_mode');
				if ($bsf_dev_mode === 'enable') {
		...
		static function front_scripts() {
			...
			if ($ultimate_css != "enable" || $bsf_dev_mode === 'enable') {
				wp_enqueue_style('background-style',
				                 plugins_url($css_path . 'background-style' . $ext . '.css', __FILE__));
			}
		} /* end front_scripts */
	}
...
if (!function_exists('hex2rgbUltParallax')) {
	function hex2rgbUltParallax($hex, $opacity) {
		$hex = str_replace("#", "", $hex);
		if (preg_match("/^([a-f0-9]{3}|[a-f0-9]{6})$/i",
		               $hex)):      // check if input string is a valid hex colour code
			...
			return 'rgba(' . implode(",",
			                         [$r, $g, $b]) . ',' . $opacity . ')';         // returns the rgb values separated by commas, ready for usage in a rgba( rr,gg,bb,aa ) CSS rule
