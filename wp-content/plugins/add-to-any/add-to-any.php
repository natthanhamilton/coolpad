...
// HTTPS?
$A2A_SHARE_SAVE_plugin_url_path = is_ssl() ? str_replace('http:', 'https:', $A2A_SHARE_SAVE_plugin_url_path)
    : $A2A_SHARE_SAVE_plugin_url_path;
...
function ADDTOANY_SHARE_SAVE_KIT($args = []) {
    ...
    $custom_icons
        = (isset($options['custom_icons']) && $options['custom_icons'] == 'url' && isset($options['custom_icons_url']))
        ? TRUE : FALSE;
    ...
    if ($custom_icons) {
        ...
        if (strpos($kit_additional_classes, 'a2a_vertical_style') !== FALSE) {
            ...
            $icon_size_classname = isset($options['custom_icons_width'])
                ? ' a2a_kit_size_' . $options['custom_icons_width'] : '';
        } else {
            // Use height (if specified) for .a2a_kit_size_## class name to size default service counters
            $icon_size_classname = isset($options['custom_icons_height'])
                ? ' a2a_kit_size_' . $options['custom_icons_height'] : '';
        }
    ...
    if (!isset($args['html_container_open'])) {
        $args['html_container_open']
            = '<div class="a2a_kit' . $icon_size_classname . $kit_additional_classes . ' a2a_target"';
...
function ADDTOANY_SHARE_SAVE_ICONS($args = []) {
    ...
    if ($is_follow) {
        ...
        $service_codes = (is_array($services)) ? array_keys($services) : [];
        // Services set by "buttons" arg
        $active_services = !empty ($buttons) ? array_keys($buttons) : [];
        // Else Share kit
    } else {
        // Make available services extensible via plugins, themes (functions.php), etc.
        ...
        $active_services = (isset($options['active_services'])) ? $options['active_services']
    ...
    foreach ($active_services as $active_service) {
        ...
        if ($active_service == 'facebook_like' || $active_service == 'twitter_tweet' || $active_service == 'google_plusone' || $active_service == 'google_plus_share' || $active_service == 'pinterest_pin') {
            ...
            $counter_enabled = (!$is_follow // Disable counters on Follow Kits
                                && in_array($active_service, ['facebook', 'pinterest', 'linkedin', 'reddit'])
            ...
            $height_attr = isset($service['icon_height']) ? ' height="' . $service['icon_height'] . '"'
                : ' height="16"';
            $height_attr = $is_amp && !empty($icon_size) ? ' height="' . $icon_size . '"' : $height_attr;
            $url          = (isset($href)) ? $href
                : $https_or_http . '://www.addtoany.com/add_to/' . $safe_name . '?linkurl=' . $linkurl_enc . '&amp;linkname=' . $linkname_enc;
            ...
            $rel_nofollow = $is_follow ? ''
                : ' rel="nofollow"'; // ($is_follow indicates a Follow Kit. 'nofollow' is for search crawlers. Different things)
            ...
            $link .= ($large_icons && !isset($custom_icons) && !$custom_service) ? ""
                : "<img src=\"$src\"" . $width_attr . $height_attr . " alt=\"$name\"/>";
...
function ADDTOANY_SHARE_SAVE_BUTTON($args = []) {
    ...
    $linkurl           = isset($args['linkurl']) ? $args['linkurl'] : FALSE;
    $linkmedia         = isset($args['linkmedia']) ? $args['linkmedia'] : FALSE;
    $_addtoany_targets = (isset($_addtoany_targets)) ? $_addtoany_targets : [];
    $args = array_merge($args, A2A_SHARE_SAVE_link_vars($linkname, $linkurl, $linkmedia)); // linkname_enc, etc.
    $defaults = [
        'linkname'             => '',
        'linkurl'              => '',
    ...
    if (!$args['no_universal_button']) {
        ...
        if (isset($button_fname) && ($button_fname == 'favicon.png' || $button_fname == 'share_16_16.png')) {
            if (!$is_feed) {
                $style_bg
                    = 'background:url(' . $A2A_SHARE_SAVE_plugin_url_path . '/' . $button_fname . ') no-repeat scroll 4px 0px;';
                $style
                    = ' style="' . $style_bg . 'padding:0 0 0 25px;display:inline-block;height:16px;vertical-align:middle"'; // padding-left:21+4 (4=other icons padding)
        ...
        $button_html
            = $html_container_open . $html_wrap_open . '<a class="a2a_dd' . $button_class . ' addtoany_share_save" href="https://www.addtoany.com/share' . $button_href_querystring . '"' . $button_id
              . $style . $button_target
              . '>' . $button . '</a>';
    } else {
    ...
    if (!$is_feed && !$script_disabled && (!is_admin() || $ajax)) {
        if ($use_current_page) {
            ...
            $linkname_sanitized  = function_exists('wp_json_encode') ? wp_json_encode($linkname)
                : json_encode($linkname);
...
function ADDTOANY_SHARE_SAVE_SPECIAL($special_service_code, $args = []) {
    ...
    if ($special_service_code == 'facebook_like') {
        $custom_attributes .= ($options['special_facebook_like_options']['verb'] == 'recommend')
        ...
        $custom_attributes .= ($options['special_pinterest_pin_options']['show_count'] == '1') ? ''
...
function ADDTOANY_FOLLOW_KIT($args = []) {
...
function ADDTOANY_SHARE_SAVE_FLOATING($args = []) {
    $options = get_option('addtoany_options');
    $floating_html = '';
    ...
    $horizontal_type = (isset($options['floating_horizontal']) && 'none' != $options['floating_horizontal'])
        ? $options['floating_horizontal'] : FALSE;
    if (is_singular()) {
    ...
    if ($vertical_type || $horizontal_type) {
        ...
        if ($vertical_type) {
            // Top position
            $position = (isset($options['floating_vertical_position'])) ? $options['floating_vertical_position'] . 'px'
                : '100px';
            // Left or right offset
            $offset = (isset($options['floating_vertical_offset'])) ? $options['floating_vertical_offset'] . 'px'
                : '0px';
        ...
        if ($horizontal_type) {
            ...
            $position = (isset($options['floating_horizontal_position']))
                ? $options['floating_horizontal_position'] . 'px' : '0px';
            // Bottom offset
            $offset = (isset($options['floating_horizontal_offset'])) ? $options['floating_horizontal_offset'] . 'px'
                : '0px';
...
function A2A_SHARE_SAVE_head_script() {
    ...
    $additional_js                         = (isset($options['additional_js_variables']))
        ? $options['additional_js_variables'] : '';
...
function A2A_SHARE_SAVE_footer_script() {
    ...
    $_addtoany_targets = (isset($_addtoany_targets)) ? $_addtoany_targets : [];
    $floating_html = ADDTOANY_SHARE_SAVE_FLOATING(['output_later' => TRUE]);
    $javascript_footer = "\n" . '<script type="text/javascript">' . "<!--\n"
                         . "wpa2a.targets=["
...
function A2A_SHARE_SAVE_add_to_content($content) {
    ...
    if ($options['position'] == 'both' || $options['position'] == 'top') {
        // Prepend to content
        $content = sprintf($container_wrap_open,
                           'addtoany_content_top') . ADDTOANY_SHARE_SAVE_KIT($kit_args) . $container_wrap_close . $content;
    }
    if ($options['position'] == 'bottom' || $options['position'] == 'both') {
        // Append to content
        $content .= sprintf($container_wrap_open,
                            'addtoany_content_bottom') . ADDTOANY_SHARE_SAVE_KIT($kit_args) . $container_wrap_close;
    }
...
function A2A_SHARE_SAVE_shortcode($attributes) {
    ...
    $buttons   = !empty($buttons) ? explode(',', $buttons) : [];
    $output_later = TRUE;
...
function A2A_SHARE_SAVE_refresh_cache() {
    ...
    if (!wp_mkdir_p(dirname($upload_dir['basedir'] . '/addtoany/foo'))) {
        $message = sprintf(__('Unable to create directory %s. Is its parent directory writable by the server?'),
                           dirname($new_file));
