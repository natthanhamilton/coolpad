...
do_action('wpas_before_original_post'); ?>
	...
	<table id="original_ticket" class="table wpas-ticket-responses">
		...
		<tr class="wpas_role wpas_client">
			<td class="tbl_col1">
				<div class="ticket_profile">
					...
					<div>
						...
						<time class="visible-xs wpas-timestamp"
						      datetime="<?php echo str_replace(' ', 'T', $post->post_date); ?>Z">
							<?php printf(__('%s ago', 'awesome-support'),
							             human_time_diff(get_the_time('U', $post->ID), current_time('timestamp'))); ?>
						</time>
			...
			<td class="tbl_col2"
			    <?php if (wpas_get_option('date_position',
			                              'right_side') == 'under_avatar'): ?>colspan="2"<?php endif; ?>>
				<?php
				/**
