...
class WPAS_CF_Select extends WPAS_Custom_Field {
    ...
    public function __construct($field_id, $field) {
        ...
        $this->field_args['multiple'] = isset($this->field_args['multiple']) ? (bool)$this->field_args['multiple']
            : FALSE;
        $this->field_args['select2']  = isset($this->field_args['select2']) ? (bool)$this->field_args['select2']
            : FALSE;
        /* Change the field name if multiple upload is enabled */
        if (TRUE === $this->field_args['multiple']) {
            add_filter('wpas_cf_field_atts', [$this, 'edit_field_atts'], 10, 3);
        }
        if (TRUE === $this->field_args['select2']) {
            add_filter('wpas_cf_field_class', [$this, 'add_select2_class'], 10, 2);
        }
    }
    ...
    public function display_no_edit() {
        ...
        return sprintf('<div class="wpas-cf-noedit-wrapper"><div id="%s-label" class="wpas-cf-label">%s</div><div id="%s-value" class="wpas-cf-value">%s</div></div>',
                       $this->get_field_id(), $this->get_field_title(), $this->get_field_id(), $label);
    }
