...
class WPAS_CF_Taxonomy extends WPAS_Custom_Field {
    ...
    public function __construct($field_id, $field) {
        ...
        $this->terms                 = get_terms($this->field_id, ['hide_empty' => 0]);
        $this->ordered_terms         = [];
    ...
    public function display() {
        ...
        return sprintf('<label {{label_atts}}>{{label}}</label><select {{atts}}><option value="">%s</option>%s</select>',
                       __('Please select', 'awesome-support'), $options);
    }
    ...
    public function display_no_edit() {
        return sprintf('<div class="wpas-cf-noedit-wrapper"><div id="%s-label" class="wpas-cf-label">%s</div><div id="%s-value" class="wpas-cf-value">%s</div></div>',
                       $this->get_field_id(), $this->get_field_label(), $this->get_field_id(),
