...
class WPAS_Custom_Field {
	...
	public function __construct($field_id = '', $field = []) {
		 ...
		 */
		if (empty($field)) {
			$field = ['name' => $field_id, 'args' => $this->get_field_defaults()];
		}
		$this->field      = $field;
	...
	public static function get_field_defaults() {
		...
		$defaults = [
			...
			// @since 3.3
			'column_attributes'     => [],               // User-defined attributes to add to the list table columns (required show_column to be true)
	...
	protected function require_field_type_class() {
		$field_class_path
			= WPAS_PATH . "includes/custom-fields/field-types/class-cf-{$this->field['args']['field_type']}.php";
	...
	public function get_class_name() {
		if (!isset($this->class_name)) {
			$type             = str_replace(' ', '_', ucwords(str_replace(['-', '_'], ' ', $this->field_type)));
	...
	public function get_wrapper_class($class = []) {
		...
		if (isset($_SESSION['wpas_submission_error']) && is_array($_SESSION['wpas_submission_error']) && in_array($this->get_field_id(),
	...
	protected function get_field_markup() {
		...
		if (class_exists($this->get_class_name())) {
			...
			if (method_exists($class_name, 'display')) {
				...
				$instance = new $class_name($this->field_id, $this->field);
				if (is_admin()) {
					if (!current_user_can($this->field['args']['capability']) && method_exists($instance,
					                                                                           'display_no_edit')
					) {
						$field = $instance->display_no_edit();
					} elseif (current_user_can($this->field['args']['capability']) && method_exists($instance,
					                                                                                'display_admin')
					) {
						$field = apply_filters('wpas_cf_display_admin_markup', $instance->display_admin(), $this->field,
						                       $this->populate());
					} else {
				...
				$field = '<!-- ' . __('The custom field class does not contain the mandatory method "display"',
				                      'awesome-support') . ' -->';
				$error = TRUE;
			}
		} /* In case the field type / callback function does not exist */
		else {
			$field = '<!-- ' . __('The type of custom field you are trying to use does not exist',
			                      'awesome-support') . ' -->';
			$error = TRUE;
		}

		return FALSE === $error ? $this->process_field_markup(apply_filters('wpas_cf_field_markup', $field,
		                                                                    $this->populate(), $this->field)) : $field;
	}
	...
	public function populate() {
		...
		if (empty($value)) {
			if (isset($_GET[ $this->get_field_id() ])) {
				$value = is_array($_GET[ $this->get_field_id() ]) ? filter_input(INPUT_GET, $this->get_field_id(),
	...
	public function get_sanitized_value($value) {
		$sanitize_function = function_exists($this->field['args']['sanitize']) ? $this->field['args']['sanitize']
			: 'sanitize_text_field';
		if (is_array($value)) {
	...
	protected function process_field_markup($field = '') {
		...
		$field = str_replace('{{atts}}', implode(' ', apply_filters('wpas_cf_field_atts', $atts, $field, $this->field)),
		                     $field);
	 ...
	 * @return string             The class tag with appropriate classes
	 */
	public function get_field_class($class = []) {
		/**
		 * Set the classes array with the default class.
		 *
		 * @var $classes array
	...
	public function get_field_description() {
		 ...
		 * @var $classes array
		 */
		$classes = apply_filters('wpas_cf_field_description_class',
		                         ['backend' => 'description', 'frontend' => 'wpas-help-block'], $this->field);
	...
	public function update_value($value, $post_id) {
		 ...
		 */
			...
			if ($this->require_field_type_class() && class_exists($class_name) && method_exists($class_name,
			                                                                                    'update')
			) {
				/* Instantiate the field type class */
				$instance = new $class_name($this->field_id, $this->field);
