	...
	public function create_log() {
		...
		if (!empty($this->contents)) {
			...
			foreach ($this->contents as $key => $update) {
				...
				switch ($action):
					case 'updated':
						$content .= sprintf(_x('updated %s to %s', 'Custom field value was updated', 'awesome-support'),
						                    $label, $value);
						break;
					case 'deleted':
						$content .= sprintf(_x('deleted %s', 'Custom field value was deleted', 'awesome-support'),
						                    $label);
						break;
					case 'added':
						$content .= sprintf(_x('added %s to %s', 'Custom field value was added', 'awesome-support'),
						                    $value, $label);
						break;
				endswitch;
				$content .= "</li>";
