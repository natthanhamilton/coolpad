	...
	foreach ($ordered as $id => $priority) {
		...
		if (is_array(WPAS()->addons[ $id ]['callback'])) {
			if (isset(WPAS()->addons[ $id ]['callback'][0]) && isset(WPAS()->addons[ $id ]['callback'][1])) {
				if (is_object(WPAS()->addons[ $id ]['callback'][0]) && method_exists(WPAS()->addons[ $id ]['callback'][0],
				                                                                     WPAS()->addons[ $id ]['callback'][1])
				) {
					$wpas_addon_{$id}
						= call_user_func([WPAS()->addons[ $id ]['callback'][0], WPAS()->addons[ $id ]['callback'][1]]);
				} elseif (class_exists(WPAS()->addons[ $id ]['callback'][0])) {
					$wpas_addon_{$id} = call_user_func(WPAS()->addons[ $id ]['callback'][0],
					                                   WPAS()->addons[ $id ]['callback'][1]);
				}
			}
		} else {
...
function wpas_register_addon($id, $callback, $priority = 10) {
	...
	WPAS()->addons[ $id ] = ['status' => 'registered', 'priority' => $priority, 'callback' => $callback];
}
