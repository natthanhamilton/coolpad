<p><?php _e('The system status is a built-in debugging tool. If you contacted the support and you\'re asked to provide the system status, <strong>click the button below</strong> to copy your system report:',
            'awesome-support'); ?></p>

<div class="wpas-system-status">
	<textarea id="wpas-system-status-output" rows="10" style="display: none;"></textarea>
	<button id="wpas-system-status-generate-json" class="button-secondary"><?php _e('Copy Report',
	                                                                                'awesome-support'); ?> - JSON
	</button>
	<button id="wpas-system-status-generate-wporg" class="button-secondary"><?php _e('Copy Report',
	                                                                                 'awesome-support'); ?> -
		WordPress.org
	</button>
</div>
...
<table class="widefat wpas-system-status-table" id="wpas-system-status-wordpress">
	...
	<tr class="alt">
		...
		<td><?php if (defined('WP_DEBUG') && WP_DEBUG) _e('Yes', 'awesome-support'); else _e('No',
		                                                                                     'awesome-support'); ?></td>
	</tr>
	<tr>
	...
	<tr class="alt">
		...
		<td>
			...
			if ($wp_upload_max <= $server_upload_max) {
				...
				echo '<span class="wpas-alert-danger">' . sprintf(__('%s (The server only allows %s)',
				                                                     'awesome-support'), size_format($wp_upload_max),
				                                                  size_format($server_upload_max)) . '</span>';
			}
			?>
		</td>
	</tr>
	...
	<tr class="alt">
		...
		<td>
			...
			if (empty($timezone)) {
				echo '<span class="wpas-alert-danger">' . __('The timezone hasn\'t been set',
...
<table class="widefat wpas-system-status-table" id="wpas-system-status-settings">
	...
	<tr>
		...
		<td><?php TRUE === boolval(wpas_get_option('support_products')) ? _e('Enabled', 'awesome-support')
				: _e('Disabled', 'wpas '); ?></td>
	</tr>
	<tr class="alt">
		<td class="row-title">Registration Status</td>
		<td><?php 'allow' === wpas_get_option('allow_registrations') ? _e('Open', 'awesome-support')
				: _e('Closed', 'wpas '); ?></td>
	</tr>
	<tr>
		...
		<td>
			...
			if (!is_dir(ABSPATH . 'wp-content/uploads/awesome-support')) {
				if (!is_writable(ABSPATH . 'wp-content/uploads')) {
					echo '<span class="wpas-alert-danger">' . __('The upload folder doesn\'t exist and can\'t be created',
					                                             'awesome-support') . '</span>';
				} else {
					echo '<span class="wpas-alert-success">' . __('The upload folder doesn\'t exist but can be created',
					                                              'awesome-support') . '</span>';
				}
			} else {
				if (!is_writable(ABSPATH . 'wp-content/uploads/awesome-support')) {
					echo '<span class="wpas-alert-danger">' . __('The upload folder exists but isn\'t writable',
					                                             'awesome-support') . '</span>';
				} else {
					echo '<span class="wpas-alert-success">' . __('The upload folder exists and is writable',
					                                              'awesome-support') . '</span>';
				}
			}
			?>
	...
	<tr>
		...
		<td>
			...
			if (empty($filetypes)) {
				echo '<span class="wpas-alert-danger">' . _x('None', 'Allowed file types for attachments',
				                                             'awesome-support') . '</span>';
			} else {
	...
	<tr class="alt">
		...
		<td><?php TRUE === boolval(wpas_get_option('frontend_wysiwyg_editor')) ? _e('Yes', 'awesome-support')
				: _e('No', 'wpas '); ?></td>
	</tr>
	</tbody>
</table>
<table class="widefat wpas-system-status-table" id="wpas-system-status-pages">
		...
		<td>
			...
			if (empty($page_submit)) {
				...
				foreach ($page_submit as $page_submit_id) {
					...
					array_push($submission_pages,
					           "<span class='wpas-alert-success'>" . esc_url($page_submit_url) . " (#$page_submit_id)</span>");
				}
	...
	<tr>
		...
		?>
		<td><?php echo empty($page_list) ? '<span class="wpas-alert-danger">Not set</span>'
				: "<span class='wpas-alert-success'>" . get_permalink($page_list) . " (#$page_list)</span>"; ?></td>
	</tr>
	</tbody>
</table>
<table class="widefat wpas-system-status-table" id="wpas-system-status-email-notifications">
	...
	<tr>
		...
		<td>
			<?php echo TRUE === boolval(wpas_get_option('enable_confirmation'))
				? '<span class="wpas-alert-success">Enabled</span>'
				: '<span class="wpas-alert-danger">Disabled</span>'; ?>
		</td>
	</tr>
	<tr class="alt">
		<td class="row-title">New Assignment</td>
		<td>
			<?php echo TRUE === boolval(wpas_get_option('enable_assignment'))
				? '<span class="wpas-alert-success">Enabled</span>'
				: '<span class="wpas-alert-danger">Disabled</span>'; ?>
		</td>
	</tr>
	<tr>
		...
		<td>
			<?php echo TRUE === boolval(wpas_get_option('enable_reply_agent'))
				? '<span class="wpas-alert-success">Enabled</span>'
				: '<span class="wpas-alert-danger">Disabled</span>'; ?>
		</td>
	</tr>
	<tr class="alt">
		...
		<td>
			<?php echo TRUE === boolval(wpas_get_option('enable_reply_client'))
				? '<span class="wpas-alert-success">Enabled</span>'
				: '<span class="wpas-alert-danger">Disabled</span>'; ?>
		</td>
	</tr>
	<tr>
		...
		<td>
			<?php echo TRUE === boolval(wpas_get_option('enable_closed'))
				? '<span class="wpas-alert-success">Enabled</span>'
				: '<span class="wpas-alert-danger">Disabled</span>'; ?>
		</td>
	</tr>
...
<table class="widefat wpas-system-status-table" id="wpas-system-status-custom-fields">
	...
	<?php } else {
		...
		foreach ($fields as $field_id => $field) {
			...
			$values                                  = [];
			$attributes                              = [__('Capability',
			                                               'awesome-support') => '<code>' . $field['args']['capability'] . '</code>'];
			$attributes[ __('Core', 'wpas') ]        = TRUE === boolval($field['args']['core']) ? __('Yes',
			                                                                                         'awesome-support')
				: __('No', 'awesome-support');
			$attributes[ __('Required', 'wpas') ]    = TRUE === boolval($field['args']['required']) ? __('Yes',
			                                                                                             'awesome-support')
				: __('No', 'awesome-support');
			$attributes[ __('Logged', 'wpas') ]      = TRUE === boolval($field['args']['log']) ? __('Yes',
			                                                                                        'awesome-support')
				: __('No', 'awesome-support');
			$attributes[ __('Show Column', 'wpas') ] = TRUE === boolval($field['args']['show_column']) ? __('Yes',
			                                                                                                'awesome-support')
				: __('No', 'awesome-support');
			...
			foreach ($active_plugins as $plugin) {
				...
				if (!empty($plugin_data['Name'])) {
					...
					if (!empty($plugin_data['PluginURI'])) {
						$plugin_name
							= '<a href="' . esc_url($plugin_data['PluginURI']) . '" title="Visit plugin homepage">' . $plugin_name . '</a>';
					}
					$wp_plugins[]
