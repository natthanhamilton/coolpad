...
function wpas_delete_synced_products($resync = FALSE) {
    ...
    $posts = new WP_Query(['post_type' => $post_type, 'posts_per_page' => -1, 'post_status' => 'any']);
    ...
    global $wpdb;
    $metas = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->postmeta} WHERE meta_key = '%s'",
                                               '_wpas_product_term'));
    if (!empty($metas)) {
...
function wpas_clear_agents_metas() {
    $agents = wpas_get_users(['cap' => 'edit_ticket']);
