    ...
    if (isset($_GET['wpas-message'])) {
        switch ($_GET['wpas-message']) {
            case 'opened':
                ?>
                <div class="updated">
                    <p><?php printf(__('The ticket #%s has been (re)opened.', 'awesome-support'),
                                    intval($_GET['post'])); ?></p>
                </div>
                <?php
                break;
            case 'closed':
                ?>
                <div class="updated">
                    <p><?php printf(__('The ticket #%s has been closed.', 'awesome-support'),
                                    intval($_GET['post'])); ?></p>
                </div>
                <?php
                break;
...
function wpas_ticket_reply_controls($controls, $ticket_id, $reply) {
    if (0 !== $ticket_id && get_current_user_id() == $reply->post_author) {
        ...
        $delete         = wpas_do_url(admin_url('post.php'), 'admin_trash_reply',
                                      ['post' => $ticket_id, 'action' => 'edit', 'reply_id' => $reply->ID]);
        $edit           = wp_nonce_url(add_query_arg([
                                                         'post'   => $ticket_id,
                                                         'rid'    => $reply->ID,
                                                         'action' => 'edit_reply'
                                                     ], admin_url('post.php')), 'delete_reply_' . $reply->ID);
        $controls['delete_reply'] = sprintf('<a class="%1$s" href="%2$s" title="%3$s">%3$s</a>', 'wpas-delete',
                                            esc_url($delete),
                                            esc_html_x('Delete', 'Link to delete a ticket reply', 'awesome-support'));
        $controls['edit_reply']
                                  = sprintf('<a class="%1$s" href="%2$s" data-origin="%3$s" data-replyid="%4$d" data-reply="%5$s" data-wysiwygid="%6$s" title="%7$s">%7$s</a>',
                                            'wpas-edit', '#', "#wpas-reply-$reply->ID", $reply->ID,
                                            "wpas-editwrap-$reply->ID", "wpas-editreply-$reply->ID",
                                            esc_html_x('Edit', 'Link ot edit a ticket reply', 'awesome-support'));
    }
    if (get_current_user_id() !== $reply->post_author && 'unread' === $reply->post_status) {
        $controls['mark_read'] = sprintf('<a class="%1$s" href="%2$s" data-replyid="%3$d" title="%4$s">%4$s</a>',
                                         'wpas-mark-read', '#', $reply->ID,
                                         esc_html_x('Mark as Read', 'Mark a user reply as read', 'awesome-support'));
    }
...
function wpas_is_ticket_old($post_id, $replies = NULL) {
    ...
    if (is_null($replies) || is_object($replies) && !is_a($replies, 'WP_Query')) {
        $replies = WPAS_Tickets_List::get_instance()
                                    ->get_replies_query($post_id);
    }
    /**
...
function wpas_is_reply_needed($post_id, $replies = NULL) {
    ...
    if (empty($replies->posts)) {
        ...
        if (user_can($replies->posts[ $last ]->post_author,
                     'edit_ticket') && (int)$replies->posts[ $last ]->post_author !== get_current_user_id()
        ) {
            return TRUE;
        }
        // If the last reply is not from an agent and the reply is still unread we need the ticket to stand out
        if (!user_can($replies->posts[ $last ]->post_author,
                      'edit_ticket') && 'unread' === $replies->posts[ $last ]->post_status
        ) {
            return TRUE;
        }
    }
...
function wpas_admin_footer_text($text) {
    ...
    return sprintf(__('If you like Awesome Support <a %s>please leave us a %s rating</a>. Many thanks from ThemeAvenue in advance :)',
                      'awesome-support'),
                   'href="https://wordpress.org/support/view/plugin-reviews/awesome-support?rate=5#postform" target="_blank"',
