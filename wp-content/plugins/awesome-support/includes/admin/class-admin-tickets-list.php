...
class WPAS_Tickets_List {
	...
	public function __construct() {
		add_action('manage_ticket_posts_columns', [$this, 'add_core_custom_columns'], 16, 1);
		add_action('manage_ticket_posts_custom_column', [$this, 'core_custom_columns_content'], 10, 2);
		add_action('admin_menu', [$this, 'hide_closed_tickets'], 10, 0);
		add_filter('the_excerpt', [$this, 'remove_excerpt'], 10, 1);
		add_filter('post_row_actions', [$this, 'remove_quick_edit'], 10, 2);
		add_filter('the_title', [$this, 'add_ticket_id_title']);
		add_action('pre_get_posts', [$this, 'filter_staff']);
		add_filter('post_class', [$this, 'ticket_row_class'], 10, 3);
		add_action('pre_get_posts', [$this, 'set_ordering_query_var']);
		add_filter('posts_results', [$this, 'apply_ordering_criteria'], 10, 2);
	}
...
SQL;
			$no_replies = $client_replies = $agent_replies = [];
	...
	public function core_custom_columns_content($column, $post_id) {
		switch ($column) {
			case 'wpas-assignee':
				...
				$link     = add_query_arg(['post_type' => 'ticket', 'staff' => $assignee], admin_url('edit.php'));
			...
			case 'wpas-client':
				...
				$link   = add_query_arg(['post_type' => 'ticket', 'author' => $client->ID], admin_url('edit.php'));
			...
			case 'wpas-activity':
				...
				if (0 === $replies->post_count) {
					...
					$last_user_link = add_query_arg(['user_id' => $last_reply->post_author],
					                                admin_url('user-edit.php'));
					$last_user      = get_user_by('id', $last_reply->post_author);
					$role           = TRUE === user_can($last_reply->post_author, 'edit_ticket') ? _x('agent',
					                                                                                  'User role',
					                                                                                  'awesome-support')
						: _x('client', 'User role', 'awesome-support');
					echo _x(sprintf(_n('%s reply.', '%s replies.', $replies->post_count, 'awesome-support'),
					                $replies->post_count), 'Number of replies to a ticket', 'awesome-support');
					echo '<br>';
					printf(_x('<a href="%s">Last replied</a> %s ago by %s (%s).', 'Last reply ago', 'awesome-support'),
					       add_query_arg([
						                     'post'   => $post_id,
						                     'action' => 'edit'
					                     ], admin_url('post.php')) . '#wpas-post-' . $last_reply->ID,
					       human_time_diff(strtotime($last_reply->post_date), current_time('timestamp')),
					       '<a href="' . $last_user_link . '">' . $last_user->user_nicename . '</a>', $role);
				}
				...
				if (TRUE === wpas_is_reply_needed($post_id, $replies)) {
					...
					array_push($tags,
					           "<span class='wpas-label' style='background-color:$color;'>" . __('Awaiting Support Reply',
					                                                                             'awesome-support') . "</span>");
				}
				// Maybe add the "Old" tag
				if (TRUE === wpas_is_ticket_old($post_id, $replies)) {
					$old_color = wpas_get_option('color_old');
					array_push($tags, "<span class='wpas-label' style='background-color:$old_color;'>" . __('Old',
					                                                                                        'awesome-support') . "</span>");
				}
				if (!empty($tags)) {
	...
	public function hide_closed_tickets() {
		...
		if (is_array($submenu) && array_key_exists('edit.php?post_type=ticket', $submenu) && isset($submenu[5])) {
			$submenu["edit.php?post_type=ticket"][5][2]
				= $submenu["edit.php?post_type=ticket"][5][2] . '&amp;wpas_status=open';
		}
