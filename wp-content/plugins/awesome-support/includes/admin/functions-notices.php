...
class AS_Admin_Notices {
    ...
    public function display_notices() {
        ...
        foreach ($notices as $notice_id => $notice) {
            ...
            $url = wpas_do_url(add_query_arg($_GET, ''), 'dismiss_notice', ['notice_id' => $notice_id]);
            printf('<div class="%s"><p>%s <a href="%s"><small>(%s)</small></a></p></div>', $notice[0], $notice[1],
                   esc_url($url), _x('Dismiss', 'Dismiss link for admin notices', 'awesome-support'));
        }
    }
