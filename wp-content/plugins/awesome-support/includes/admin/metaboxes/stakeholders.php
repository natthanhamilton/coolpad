...
if (isset($post) && is_a($post, 'WP_Post') && 'auto-draft' !== $post->post_status) {
    ...
    $client_link   = esc_url(admin_url(add_query_arg([
                                                         'post_type' => 'ticket',
                                                         'author'    => $client_id
                                                     ], 'edit.php')));
    // Staff
...
<div id="wpas-stakeholders">
    <label for="wpas-issuer"><strong
            data-hint="<?php esc_html_e('This user who raised this ticket', 'awesome-support'); ?>"
            class="hint-left hint-anim"><?php _e('Ticket Creator', 'awesome-support'); ?></strong></label>
    <p>
        <?php if (current_user_can('create_ticket')):
            $users_atts
                = ['agent_fallback' => TRUE, 'select2' => TRUE, 'name' => 'post_author_override', 'id' => 'wpas-issuer', 'data_attr' => ['capability' => 'create_ticket']];
            if (isset($post)) {
    ...
    <label for="wpas-assignee"><strong
            data-hint="<?php esc_html_e('The agent currently responsible for this ticket', 'awesome-support'); ?>"
            class="hint-left hint-anim"><?php _e('Support Staff', 'awesome-support'); ?></strong></label>
    <p>
        <?php
        $staff_atts = [
            'name'      => 'wpas_assignee',
            ...
            'select2'   => TRUE,
            'data_attr' => ['capability' => 'edit_ticket']
        ];
