...
function wpas_register_assets_front_end() {
	// Styles
	wp_register_style('wpas-plugin-styles', WPAS_URL . 'assets/public/css/public.css', [], WPAS_VERSION);
	wp_register_style('wpas-select2', WPAS_URL . 'assets/admin/css/vendor/select2.min.css', NULL, '3.5.2',
	                  'all'); // NOTE: This asset is duplicated in the back-end
	// Scripts
	wp_register_script('wpas-plugin-script', WPAS_URL . 'assets/public/js/public-dist.js', ['jquery'], WPAS_VERSION,
	                   TRUE);
	wp_register_script('wpas-select2', WPAS_URL . 'assets/admin/js/vendor/select2.min.js', ['jquery'], '4.0.0',
	                   TRUE); // NOTE: This asset is duplicated in the back-end
	// JS Objects
...
function wpas_register_assets_back_end() {
	// Styles
	wp_register_style('wpas-select2', WPAS_URL . 'assets/admin/css/vendor/select2.min.css', NULL, '3.5.2',
	                  'all'); // NOTE: This asset is duplicated in the front-end
	wp_register_style('wpas-flexboxgrid', WPAS_URL . 'assets/admin/css/vendor/flexboxgrid.min.css', NULL, '6.2.0',
	                  'all');
	wp_register_style('wpas-admin-styles', WPAS_URL . 'assets/admin/css/admin.css', ['wpas-select2'], WPAS_VERSION);
	wp_register_style('wpas-simple-hint', 'https://cdn.jsdelivr.net/simple-hint/2.1.1/simple-hint.min.css', NULL,
	                  '2.1.1');
	// Scripts
	wp_register_script('wpas-admin-about-linkify', WPAS_URL . 'assets/admin/js/vendor/linkify.min.js', ['jquery'],
	                   WPAS_VERSION);
	wp_register_script('wpas-admin-about-linkify-jquery', WPAS_URL . 'assets/admin/js/vendor/linkify-jquery.min.js',
	                   ['jquery'], WPAS_VERSION);
	wp_register_script('wpas-admin-about-moment', WPAS_URL . 'assets/admin/js/vendor/moment.min.js', ['jquery'],
	                   WPAS_VERSION);
	wp_register_script('wpas-admin-about-script', WPAS_URL . 'assets/admin/js/admin-about.js', ['jquery'],
	                   WPAS_VERSION);
	wp_register_script('wpas-select2', WPAS_URL . 'assets/admin/js/vendor/select2.min.js', ['jquery'], '4.0.0',
	                   TRUE); // NOTE: This asset is duplicated in the front-end
	wp_register_script('wpas-admin-script', WPAS_URL . 'assets/admin/js/admin.js', ['jquery', 'wpas-select2'],
	                   WPAS_VERSION);
	wp_register_script('wpas-admin-tabletojson', WPAS_URL . 'assets/admin/js/vendor/jquery.tabletojson.min.js',
	                   ['jquery'], WPAS_VERSION);
	wp_register_script('wpas-admin-reply', WPAS_URL . 'assets/admin/js/admin-reply.js', ['jquery'], WPAS_VERSION);
	wp_register_script('wpas-autolinker', WPAS_URL . 'assets/public/vendor/Autolinker/Autolinker.min.js', NULL,
	                   '0.19.0', TRUE);
	wp_register_script('wpas-users', WPAS_URL . 'assets/admin/js/admin-users.js', NULL, WPAS_VERSION, TRUE);
	wp_register_script('wpas-admin-helpers_functions', WPAS_URL . 'assets/public/js/helpers_functions.js', NULL,
	                   WPAS_VERSION);
	wp_register_script('wpas-admin-upload', WPAS_URL . 'assets/public/js/component_upload.js', ['jquery'],
	                   WPAS_VERSION);
	// JS Objects
...
function wpas_assets_front_end() {
	...
	if (wpas_is_plugin_page()) {
		...
		if (file_exists($stylesheet) && TRUE === boolval(wpas_get_option('theme_stylesheet'))) {
			wp_register_style('wpas-theme-styles', wpas_get_theme_stylesheet_uri(), [], WPAS_VERSION);
...
function wpas_get_javascript_object() {
	...
	if (in_array($post->ID, wpas_get_submission_pages())) {
		$empty_editor = _x("You can't submit an empty ticket", 'JavaScript validation error message',
		                   'awesome-support');
	} else {
	...
	$object = [
		...
		'fileUploadMaxError'     => __(sprintf('You can only upload a maximum of %d files', $upload_max_files),
		                               'awesome-support'),
		'fileUploadMaxSizeError' => [
