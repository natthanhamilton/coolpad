...
class WPAS_Email_Notification {
    ...
    public function __construct($post_id) {
        ...
        if (!in_array(get_post_type($post_id), ['ticket', 'ticket_reply'])) {
            return new WP_Error('incorrect_post_type',
                                __('The post ID provided does not match any of the plugin post types',
                                   'awesome-support'));
        }
        /* Set the e-mail content type to HTML */
        add_filter('wp_mail_content_type', [$this, 'set_html_mime_type']);
        /* Set the post ID */
    ...
    public function __destruct() {
         ...
         */
        add_filter('wp_mail_content_type', [$this, 'set_text_mime_type']);
    }
    ...
    public function notify($case) {
        if (!$this->notification_exists($case)) {
            return new WP_Error('unknown_notification',
                                __('The requested notification does not exist', 'awesome-support'));
        }
        // Only check the higher level 'ticket_closed' for both ticket_closed_agent and ticket_closed_client
        $check = in_array($case, ['ticket_closed_agent', 'ticket_closed_client']) ? 'ticket_closed' : $case;
        if (!$this->is_active($check)) {
            return new WP_Error('disabled_notification',
                                __('The requested notification is disabled', 'awesome-support'));
        }
        /**
         ...
         */
        $email = apply_filters('wpas_email_notifications_email', [
    ...
    private function get_content($part, $case) {
        if (!in_array($part, ['subject', 'content'])) {
            return FALSE;
        }
    ...
    public function get_tags_values() {
        ...
        $url_private = add_query_arg(['post' => $this->ticket_id, 'action' => 'edit'], admin_url('post.php'));
        ...
        foreach ($tags as $key => $tag) {
            ...
            switch ($name) {
                ...
                case 'admin_email':
                    ...
                    break;
                case 'message':
                    $tag['value'] = $this->ticket_id === $this->post_id ? $this->get_ticket()->post_content
                        : $this->get_reply()->post_content;
                    break;
            }
            array_push($new, $tag);
        }
    ...
    public static function get_tags() {
        $tags = [
            ...
            [
                'tag'  => '{ticket_url}',
                'desc' => __('Displays the URL <strong>only</strong> (not a link link) to public ticket',
                             'awesome-support')
            ],
            [
            ...
            [
                ...
                'desc' => __('Displays the URL <strong>only</strong> (not a link link) to ticket details in admin (for agents)',
                             'awesome-support')
            ],
            [
                'tag'  => '{date}',
            ...
            [
                'tag'  => '{admin_email}',
                'desc' => sprintf(__('Converts into WordPress admin e-mail (<em>currently: %s</em>)',
                                     'awesome-support'), get_bloginfo('admin_email'))
            ],
            [
                'tag'  => '{message}',
    ...
    private function get_body($case) {
        return apply_filters('wpas_email_notifications_body', stripcslashes($this->get_content('content', $case)),
                             $this->post_id);
    }
}
