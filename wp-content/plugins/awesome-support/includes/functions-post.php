...
function wpas_open_ticket($data) {
	 ...
	 * Prepare vars
	 */
	$submit = isset($_POST['_wp_http_referer'])
		? wpas_get_submission_page_url(url_to_postid($_POST['_wp_http_referer'])) : wpas_get_submission_page_url();
	...
	if (!current_user_can('create_ticket')) {
		...
		wpas_add_error('cannot_open_ticket',
		               __('You do not have the capacity to open a new ticket.', 'awesome-support'));
		wp_redirect($submit);
		// Break
		exit;
	}
	...
	if (TRUE === ($description_mandatory = apply_filters('wpas_ticket_submission_description_mandatory',
	                                                     TRUE)) && (FALSE === $content || empty($content))
	) {
		// Save the input
		wpas_save_values();
		// Redirect to submit page
		wpas_add_error('missing_description',
		               __('It is mandatory to provide a description for your issue.', 'awesome-support'));
		wp_redirect($submit);
		// Break
		exit;
	}
	...
	if (is_user_logged_in()) {
		...
		wpas_save_values();
		// Redirect to submit page
		wpas_add_error('unknown_user',
		               __('Only registered accounts can submit a ticket. Please register first.', 'awesome-support'));
		wp_redirect($submit);
		exit;
	}
	/**
	 ...
	 */
	$post = apply_filters('wpas_open_ticket_data', [
...
function wpas_new_ticket_submission($data) {
	if (!is_admin() && isset($data['wpas_title'])) {
		...
		if (!isset($data['wpas_nonce']) || !wp_verify_nonce($data['wpas_nonce'], 'new_ticket')) {
			...
			wpas_save_values();
			// Redirect to submit page
			wpas_add_error('nonce_verification_failed',
			               __('The authenticity of your submission could not be validated. If this ticket is legitimate please try submitting again.',
			                  'awesome-support'));
			wp_redirect(wp_sanitize_redirect(home_url($_POST['_wp_http_referer'])));
			exit;
		}
		$ticket_id = wpas_open_ticket(['title' => $data['wpas_title'], 'message' => $data['wpas_message']]);
		/* Submission failure */
		if (FALSE === $ticket_id) {
			/* Save the input */
			wpas_save_values();
			 ...
			 */
			wpas_add_error('submission_error',
			               __('The ticket couldn\'t be submitted for an unknown reason.', 'awesome-support'));
 ...
 */
function wpas_insert_ticket($data = [], $post_id = FALSE, $agent_id = FALSE) {
	if (!current_user_can('create_ticket')) {
		return FALSE;
	}
	$update = FALSE;
 ...
 */
function wpas_get_tickets($ticket_status = 'open', $args = [], $post_status = 'any', $cache = FALSE) {
	...
	if ('any' !== $ticket_status) {
		if (in_array($ticket_status, ['open', 'closed'])) {
			$args['meta_query'][] = [
				'key'     => '_wpas_status',
				'value'   => $ticket_status,
				'compare' => '=',
...
function wpas_new_reply_submission($data) {
	...
	if ('ticket' !== get_post_type($parent_id)) {
		wpas_add_error('reply_added_failed',
		               __('Something went wrong. We couldn&#039;t identify your ticket. Please try again.',
		                  'awesome-support'));
	...
	if (!empty($data['wpas_user_reply'])) {
		/* Sanitize the data */
		$data = ['post_content' => wp_kses($data['wpas_user_reply'], wp_kses_allowed_html('post'))];
		/* Add the reply */
	...
	if (isset($reply_id)) {
		if (FALSE === $reply_id) {
			wpas_add_error('reply_added_failed',
			               __('Your reply could not be submitted for an unknown reason.', 'awesome-support'));
			wpas_redirect('reply_added_failed', get_permalink($parent_id));
			exit;
		} else {
			if ($close) {
				wpas_add_notification('reply_added_closed',
				                      __('Thanks for your reply. The ticket is now closed.', 'awesome-support'));
			} else {
				wpas_add_notification('reply_added', __('Your reply has been submitted. Your agent will reply ASAP.',
				                                        'awesome-support'));
			}
...
function wpas_edit_reply($reply_id = NULL, $content = '') {
	...
	$data = apply_filters('wpas_edit_reply_data', [
		'ID'             => $reply_id,
		'post_content'   => $content,
		'post_status'    => 'read',
		'comment_status' => 'closed',
...
function wpas_mark_reply_read($reply_id = NULL) {
	...
	$data = apply_filters('wpas_mark_reply_read_data', [
		'ID'             => $reply_id,
		'post_status'    => 'read',
		'comment_status' => 'closed',
		'ping_status'    => 'closed',
 ...
 */
function wpas_get_replies($post_id, $status = 'any', $args = [], $output = 'replies') {
	$allowed_status = [
		'any',
		'read',
		'unread'
...
function wpas_find_agent($ticket_id = FALSE) {
	...
	$users = shuffle_assoc(wpas_get_users(apply_filters('wpas_find_agent_get_users_args', ['cap' => 'edit_ticket'])));
	$agent = [];
	foreach ($users->members as $user) {
		...
		if (empty($agent)) {
			$agent = ['tickets' => $count, 'user_id' => $user->ID];
		} else {
...
function wpas_assign_ticket($ticket_id, $agent_id = NULL, $log = TRUE) {
	...
	if (!user_can($agent_id, 'edit_ticket')) {
		return new WP_Error('incorrect_agent',
		                    __('The chosen agent does not have the sufficient capabilities to be assigned a ticket',
		                       'awesome-support'));
	}
...
function wpas_close_ticket($ticket_id, $user_id = 0) {
	...
	if (!current_user_can('close_ticket')) {
		wp_die(__('You do not have the capacity to close this ticket', 'awesome-support'),
		       __('Can’t close ticket', 'awesome-support'), ['back_link' => TRUE]);
	}
...
function wpas_get_ticket_count_by_status($state = '', $status = 'open') {
	...
	// Sanitize the status
	if (!in_array($status, ['open', 'closed', 'any'])) {
		$status = 'open';
	}
...
function wpas_get_ticket_replies_ajax() {
	...
	if (!isset($_POST['ticket_id'])) {
		echo json_encode(['error' => esc_html__('No ticket ID given', 'awesome-support')]);
		die();
	}
	...
	if (!is_object($ticket) || !is_a($ticket, 'WP_Post')) {
		echo json_encode(['error' => esc_html__('Invalid ticket ID', 'awesome-support')]);
		die();
	}
	// Make sure the post is actually a ticket
	if ('ticket' !== $ticket->post_type) {
		echo json_encode(['error' => esc_html__('Given ID is not a ticket', 'awesome-support')]);
		die();
	}
	$number_replies = apply_filters('wpas_get_ticket_replies_ajax_replies', wpas_get_option('replies_per_page', 10));
	$replies        = wpas_get_replies($ticket_id, 'any', [
	...
	while ($replies->have_posts()) {
		...
		ob_start();
		wpas_get_template('partials/ticket-reply', [
			'time_ago' => $time_ago,
			'user'     => $user,
			'post'     => $replies->post
		]);
