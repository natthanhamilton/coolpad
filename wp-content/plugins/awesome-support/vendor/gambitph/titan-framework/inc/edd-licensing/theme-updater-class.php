...
class TITAN_EDD_Theme_Updater {
	...
	function __construct($args = [], $strings = []) {
	...
	function update_nag() {
		...
		$update_url = wp_nonce_url('update.php?action=upgrade-theme&amp;theme=' . urlencode($this->theme_slug),
		                           'upgrade-theme_' . $this->theme_slug);
		$update_onclick
		            = ' onclick="if ( confirm(\'' . esc_js($strings['update-notice']) . '\') ) {return true;}return false;"';
	...
	function check_for_update() {
		...
		if (FALSE == $update_data) {
			...
			$response = wp_remote_post($this->remote_api_url, ['timeout' => 15, 'body' => $api_params]);
