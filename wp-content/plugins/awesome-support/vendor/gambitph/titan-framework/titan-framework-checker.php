...
if (!class_exists('TitanFrameworkChecker')) {
	...
	class TitanFrameworkChecker {
		...
		function __construct() {
			add_action('admin_notices', [$this, 'display_install_or_active_notice'], 2);
			add_action('tgmpa_register', [$this, 'tgm_plugin_activation_include']);
		}
		...
		public function display_install_or_active_notice() {
			...
			if (!$this->plugin_exists()) {
				echo "<div class='error'><p><strong>"
				     . esc_html(apply_filters('titan_checker_installation_notice',
				     ...
				     . sprintf(" <a href='%s'>%s</a>",
				               ...
				               esc_html(apply_filters('titan_checker_search_plugin_notice',
				...
				if (!class_exists(apply_filters('tf_framework_checker_titan_class', self::TITAN_CLASS))) {
					echo "<div class='error'><p><strong>"
					     . esc_html(apply_filters('titan_checker_activation_notice',
					                              __('Titan Framework needs to be activated.', 'default')))
					     . sprintf(" <a href='%s'>%s</a>",
					               esc_url(admin_url('plugins.php')),
					               esc_html(apply_filters('titan_checker_activate_plugin_notice',
					                                      __('Click here to go to the plugins page and activate it.',
					                                         'default')))
					     )
					     . '</strong></p></div>';
				}
