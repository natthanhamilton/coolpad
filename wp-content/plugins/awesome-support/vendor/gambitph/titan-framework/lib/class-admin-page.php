...
class TitanFrameworkAdminPage {
	...
	public function register() {
		...
		add_action('load-' . $this->panelID, [$this, 'saveOptions']);
		add_action('load-' . $this->panelID, [$this, 'addTitanCredit']);
	}

	public function addTitanCredit() {
		add_filter('admin_footer_text', [$this, 'addTitanCreditText']);
	}

	public function addTitanCreditText() {
		echo __("<em>Options Page Created with <a href='http://titanframework.net?utm_source=admin&utm_medium=admin footer'>Titan Framework</a></em>",
		        TF_I18NDOMAIN);
	}
				...
				<div class='options-container'>
					...
					if (!empty($_GET['message'])) {
						if ($_GET['message'] == 'saved') {
							echo TitanFrameworkAdminNotification::formNotification(__('Settings saved.', TF_I18NDOMAIN),
							                                                       esc_html($_GET['message']));
						} else {
							if ($_GET['message'] == 'reset') {
								echo TitanFrameworkAdminNotification::formNotification(__('Settings reset to default.',
								                                                          TF_I18NDOMAIN),
								                                                       esc_html($_GET['message']));
							}
						}
