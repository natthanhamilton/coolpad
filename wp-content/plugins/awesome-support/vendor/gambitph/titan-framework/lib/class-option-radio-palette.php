...
class TitanFrameworkOptionRadioPalette extends TitanFrameworkOption {
	...
	public function registerCustomizerControl($wp_customize, $section, $priority = 1) {
		$wp_customize->add_control(new TitanFrameworkOptionRadioPaletteControl($wp_customize, $this->getID(), [
...
function registerTitanFrameworkOptionRadioPaletteControl() {
	class TitanFrameworkOptionRadioPaletteControl extends WP_Customize_Control {
		...
		<label>
			<input type="radio" name="<?php echo $this->id ?>" value="<?php echo esc_attr($key) ?>" <?php $this->link();
			checked($value, $key); ?>/>
			<span>
				<?php
				foreach ($colorSet as $color) {
