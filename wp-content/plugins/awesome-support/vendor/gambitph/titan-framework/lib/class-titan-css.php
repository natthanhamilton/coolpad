...
class TitanFrameworkCSS {
    ...
    function __construct($frameworkInstance) {
        ...
        add_action('tf_create_option_' . $frameworkInstance->optionNamespace, [$this, 'getOptionsWithCSS']);
        // display our CSS
        add_action('wp_head', [$this, 'printCSS'], 99);
        add_action('wp_enqueue_scripts', [$this, 'enqueueCSS']);
        // Trigger new compile when theme customizer settings were saved
        add_action('customize_save_after', [$this, 'generateSaveCSS']);
        // Trigger new compile when admin option settings were saved
        add_action('tf_admin_options_saved_' . $frameworkInstance->optionNamespace, [$this, 'generateSaveCSS']);
        // Trigger compile when there are no default options saved yet
        add_action('tf_init_no_options_' . $frameworkInstance->optionNamespace, [$this, 'generateMissingCSS']);
    }
    ...
    public function generateCSS() {
        ...
        foreach ($this->allOptionsWithIDs as $option) {
            ...
            if (!apply_filters('tf_continue_generate_css_' . $option->settings['type'] . '_' . $option->getOptionNamespace(),
                               TRUE, $option)
            ) {
                continue;
            }
            ...
            $generatedCSS
                = apply_filters('tf_generate_css_' . $option->settings['type'] . '_' . $option->getOptionNamespace(),
                                '', $option);
    ...
    public function enqueueCSS() {
        ...
        if ($this->frameworkInstance->settings['css'] == 'generate') {
            ...
            if (file_exists($generatedCss) && empty($css)) {
                wp_enqueue_style('tf-compiled-options-' . $this->frameworkInstance->optionNamespace,
                                 $this->getCSSFileURL(), __FILE__);
            }
        }
    }
    ...
    public function generateMissingCSS() {
        add_action('admin_init', [$this, '_generateMissingCSS'], 1000);
    }
