...
if (class_exists('TitanFrameworkOption')) {
    class TitanFrameworkOptionEddLicense extends TitanFrameworkOption {
        ...
        function __construct($settings, $owner) {
            ...
            add_action('admin_init', [$this, 'checkUpdates'], 10, 0);
            add_action('tf_create_option_' . $this->getOptionNamespace(), [$this, 'activateLicense']);
        }
        ...
        public function display() {
            ...
            if (strlen($license) > 0) {
                ...
                switch ($status) {
                    case 'valid':
                        ?><p
                        class="description"><?php esc_html_e('Your license is valid and active.',
                                                             TF_I18NDOMAIN); ?></p><?php
                        break;
                    case 'invalid':
                        ?><p
                    ...
                    case 'inactive':
                        ...
                        <p class="description"><?php esc_html_e('Your license is valid but inactive. Click the button above to activate it.',
                                                                TF_I18NDOMAIN); ?></p><?php
                        break;
                    case 'no_response':
                        ?><p
                        class="description"><?php esc_html_e('The remote server did not return a valid response. You can retry by hitting the &laquo;Save&raquo; button again.',
                                                             TF_I18NDOMAIN); ?></p><?php
                        break;
                }
            } else {
                ?><p
                    class="description"><?php esc_html_e('Entering your license key is mandatory to get the product updates.',
                                                         TF_I18NDOMAIN); ?></p><?php
            }
        ...
        public function check($license = '', $action = 'check_license') {
            ...
            $status_lifetime     = apply_filters('tf_edd_license_status_lifetime',
                                                 48 * 60 * 60);         // Default is set to two days
            $activation_lifetime = apply_filters('tf_edd_license_activation_lifetime',
                                                 365 * 24 * 60 * 60); // Default is set to one year
            ...
            /* Call the API. */
            $response = wp_remote_get(add_query_arg($api_params, $this->settings['server']),
            ...
            if ('activate_license' == $action) {
                 ...
                 */
                $status = $this->check($license);
                if (in_array($status, ['valid', 'inactive'])) {
        ...
        public function checkUpdates() {
            ...
            /* Item name */
            $item_name = isset($this->settings['item_name']) ? sanitize_text_field($this->settings['item_name'])
                : FALSE;
            ...
            elseif (in_array($item_is, ['theme-parent', 'theme-child'])) {
