...
class TitanFrameworkOptionFont extends TitanFrameworkOption {
	...
	private static $optionsToEnqueue = [];
	...
	function __construct($settings, $owner) {
		...
		tf_add_action_once('admin_enqueue_scripts', [$this, 'loadAdminScripts']);
		tf_add_action_once('customize_controls_enqueue_scripts', [$this, 'loadAdminScripts']);
		tf_add_action_once('admin_head', [__CLASS__, 'createFontScript']);
		tf_add_action_once('wp_enqueue_scripts', [$this, 'enqueueGooglefonts']);
		add_filter('tf_generate_css_font_' . $this->getOptionNamespace(), [$this, 'generateCSS'], 10, 2);
		// Customizer preview handling
		tf_add_action_once('tf_generate_customizer_preview_js', [$this, 'generateCustomizerPreviewJS']);
		tf_add_filter_once('tf_generate_customizer_preview_css_' . $this->getOptionNamespace(),
		                   [$this, 'generateCustomizerPreviewCSS']);
		...
		<script>
			...
			function tf_select_font_update_preview($container, doTrigger) {
				...
				if ($container.find('iframe').is(':not([data-visible=false])')) {
					$container.find('iframe').attr('src', '<?php echo TitanFramework::getURL('iframe-font-preview.php?',
					                                                                         __FILE__) ?>' + $.param(params));
				}
	...
	public function getGoogleFontURLs() {
		...
		foreach (self::$optionsToEnqueue as $option) {
			...
			if (empty($fontsToLoad[ $fontValue['font-family'] ])) {
				$fontsToLoad[ $fontValue['font-family'] ] = [];
			}
			// Get the weight
		...
		$subsets = apply_filters('tf_google_font_subsets_' . $this->getOptionNamespace(), ['latin', 'latin-ext']);
		...
		foreach ($fontsToLoad as $fontName => $variants) {
			...
			$fontUrl    = apply_filters('tf_enqueue_google_webfont_' . $this->getOptionNamespace(), $fontUrl,
			                            $fontName);
			if ($fontUrl != FALSE) {
	...
	public function generateCSS($css, $option) {
		...
		$skip
			= ['dark', 'font-type', 'text-shadow-distance', 'text-shadow-blur', 'text-shadow-color', 'text-shadow-opacity'];
		...
		$cssVariables = '';
		$cssChecking
		              = ['font_family', 'color', 'font_size', 'font_weight', 'font_style', 'line_height', 'letter_spacing', 'text_transform', 'font_variant', 'text_shadow'];
		...
		<div>
			...
			<label <?php echo $visibilityAttrs ?>>
				...
				<select class='tf-font-sel-location'>
					<?php
					$options
						= ['none', 'top', 'bottom', 'left', 'right', 'top-left', 'top-right', 'bottom-left', 'bottom-right'];
					foreach ($options as $option) {
						printf("<option value='%s'%s>%s</option>",
						       esc_attr($option),
			...
			<label style='display: none'>
				...
				<select class='tf-font-sel-blur'>
					<?php
					$options = ['0px', '1px', '2px', '3px', '4px', '5px', '10px', '20px'];
					foreach ($options as $option) {
						printf("<option value='%s'%s>%s</option>",
						       esc_attr($option),
	...
	public function cleanValueForGetting($value) {
		...
		if (!empty($value['font-family'])) {
			$value['font-type'] = in_array($value['font-family'], array_keys(self::$webSafeFonts)) ? 'websafe'
				: 'google';
		}
	...
	public function registerCustomizerControl($wp_customize, $section, $priority = 1) {
		$wp_customize->add_control(new TitanFrameworkOptionFontControl($wp_customize, $this->getID(), [
