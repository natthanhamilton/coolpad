...
function duplicate_post_plugin_upgrade() {
    ...
    if (empty($installed_version)) { // first install
        ...
        if ($installed_version == duplicate_post_get_current_version()) { //re-install
            // do nothing
...
function duplicate_post_make_duplicate_link_row($actions, $post) {
    if (duplicate_post_is_current_user_allowed_to_copy()) {
        $actions['clone']             = '<a href="' . duplicate_post_get_clone_post_link($post->ID, 'display',
                                                                                         FALSE) . '" title="'
...
function duplicate_post_save_as_new_post($status = '') {
    ...
    if (isset($post) && $post != NULL) {
        ...
        wp_die(esc_attr(__('Copy creation failed, could not find original:',
                           DUPLICATE_POST_I18N_DOMAIN)) . ' ' . htmlspecialchars($id));
    }
}
...
function duplicate_post_copy_post_meta_info($new_id, $post) {
    ...
    if ($meta_blacklist == "") $meta_blacklist = [];
...
function duplicate_post_copy_children($new_id, $post) {
    ...
    // get children
    $children
        = get_posts(['post_type' => 'any', 'numberposts' => -1, 'post_status' => 'any', 'post_parent' => $post->ID]);
...
function duplicate_post_add_plugin_links($links, $file) {
    if ($file == plugin_basename(dirname(__FILE__) . '/duplicate-post.php')) {
        $links[] = '<a href="http://lopo.it/duplicate-post-plugin">' . __('Donate',
                                                                          DUPLICATE_POST_I18N_DOMAIN) . '</a>';
        $links[] = '<a href="http://lopo.it/duplicate-post-plugin">' . __('Translate',
                                                                          DUPLICATE_POST_I18N_DOMAIN) . '</a>';
    }
