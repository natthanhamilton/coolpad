...
class WPAS_Save_Email {
	...
	public function get_raw_content() {
		...
		switch ( $this->strip_all_html_tags_option ) {
			...
			case '1':
				$text_contents = strip_tags( $this->get_contents( 'text' ), $this->allowed_html_tags_option  );
				$html_contents = strip_tags( $this->get_contents( 'html' ), $this->allowed_html_tags_option );
				break;
		...
		if ( ( $similarity_level > 50 && '1' <> $this->strip_all_html_tags_option ) || empty( $this->get_contents( 'html' ) )  ) {			
			...
			switch ( $this->strip_all_html_tags_option ) {
				case '0':
					...
					break;
				case '1':
					return strip_tags( apply_filters( 'ases_get_raw_content_filter', $this->get_contents( 'text' ), $this ), $this->allowed_html_tags_option  );
					break; 
				default:
					return wp_strip_all_tags( apply_filters( 'ases_get_raw_content_filter', $this->get_contents( 'text' ), $this ) );
					break;
			...
			if ( '1' == $this->strip_all_html_tags_option  && false == empty( $this->get_contents( 'html' ) ) ) {
				...
				switch ( $this->strip_all_html_tags_option ) {
					case '0':
						...
						break;
					case '1':
						return strip_tags( apply_filters( 'ases_get_raw_content_filter', $this->get_contents( 'html' ), $this ), $this->allowed_html_tags_option  );
						break; 
					default:
						return wp_strip_all_tags( apply_filters( 'ases_get_raw_content_filter', $this->get_contents( 'html' ), $this ) );
						break;
				...
				switch ( $this->strip_all_html_tags_option ) {
					case '0':
						...
						break;
					case '1':
						return strip_tags( apply_filters( 'ases_get_raw_content_filter', $this->get_contents( 'text' ) . $this->get_contents( 'html' ), $this ), $this->allowed_html_tags_option );
						break ;
					default:
						return wp_strip_all_tags( apply_filters( 'ases_get_raw_content_filter', $this->get_contents( 'text' ) . $this->get_contents( 'html' ), $this ) );
						break;				
	 ...
	 */
	 function apply_inbox_update_rules( $rule_id = null, $in_ticket_id = null ) {
		...
		if ( ( 1 <> (int) $rule_active ) or ( 'update' <> $rule_action) ) {
			return ;
		}
...
//		}
	...
	*/
	...
	public function add_new_ticket( $post_author = null ) {
		...
		add_action( 'wpas_open_ticket_after', array( $this, 'save_attachments' ), 8, 2 );
		add_action( 'wpas_open_ticket_after', array( $this, 'save_new_ticket_mailbox_defaults' ), 8, 2 );
		add_action( 'wpas_open_ticket_after', array( $this, 'save_new_ticket_mailbox_rules_defaults' ), 9, 2 );
		
		$this->item_id = wpas_insert_ticket( $post_param, false, false, 'email' );

		remove_action( 'wpas_open_ticket_after', array( $this, 'save_attachments' ), 8 );
		remove_action( 'wpas_open_ticket_after', array( $this, 'save_new_ticket_mailbox_defaults' ), 8 );
		remove_action( 'wpas_open_ticket_after', array( $this, 'save_new_ticket_mailbox_rules_defaults' ), 9 );
	}
