...
<div id="multi-mailbox-config">
    ...
    <table id="wpas-es-config-wrapper" class="form-table">
        ...
        <tr valign="top">
            <th scope="row" class="first">
                <label for="html_multimailbox_secureportflag"> <?php _e( 'Secure Port', 'as-email-support' ) ?> </label>
            </th>
            <td class="second tf-checkbox">
                <fieldset>
                <input type="checkbox"
                       id="html_multimailbox_secureportflag"
                       name="html_multimailbox_secureportflag"
                       value="1" <?php echo( $secureportflag == '1' && false === empty( $secureportflag ) ? 'checked' : '' ) ?> />
                <p class="description"> <?php _e( 'Usually: <b>995</b> for pop3 and <b>993</b> for imap when used with SSL security. This item should NOT be blank!', 'as-email-support' ) ?> </p>
                </fieldset>
            </td>
        </tr>
        ...
        <!-- /*	removing for now since the default agent gets overwritten by the autoassign programs...
		<div class="wpas-multi-inbox-config-item-row">
			<div class="wpas-multi-inbox-config-item-label-wrap">
				<label class="wpas-multi-inbox-config-item-label"> <?php _e( 'Default Agent', 'as-email-support' ) ?> </label>
			</div>
			<div class="wpas-multi-inbox-config-item-wrap">
				<input class="wpas-multi-inbox-config-item wpas-multi-inbox-config-item-text" type ="text" name = "html_multimailbox_default_assignee", value = "<?php echo $defaultassignee ?>" />
			</div>
			<div class="wpas-multi-inbox-desc-wrap">
				<div class="wpas-multi-inbox-item-desc"> <?php _e( 'This must be the WordPress ID for the agent. It is a NUMBER, not a text string!', 'as-email-support' ) ?> </div>
			</div>
		</div>
*/ -->
