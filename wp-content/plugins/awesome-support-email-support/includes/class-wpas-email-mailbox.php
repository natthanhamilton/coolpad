...
class ASES_Mailbox {
	 ...
	 * @param string $insecure secure flag (1 or 0)
	...
	public function __construct($inserver, $inprotocol, $inusername, $inpassword, $inport, $insecure, $inother = array()) {
		...
		if ( false === is_null( $insecure ) ) {
			// If a value was passed in convert to boolean...
			$this->secure = (1 === (int) $insecure or true === (boolean) $insecure ? true : false ) ;
		}
		if ( true === is_null( $this->secure ) ) {
			// If no value was passed in then get the ssl flag from the TICKETS->SETTINGS->Email Piping settings instead...
			$this->secure = (bool) wpas_get_option( 'email_secure', false );
		}
	...
	protected function mailbox() {
		...
		$args = array(
			...
			'port'     => $this->port,
			'ssl'      => true === $this->secure ? 'ssl' : false, // Set ssl to actual SSL if security is enabled.
		);
