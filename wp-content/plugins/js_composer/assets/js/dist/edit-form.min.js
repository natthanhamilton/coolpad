...
if (function ($) {
        ...
        vc.atts.column_offset = {
            parse: function (param) {
                var $field = this.content().find("input.wpb_vc_param_value." + param.param_name), column_offset = $field.data("vcColumnOffset"), result = column_offset.save();
                return result.join(" ")
    ...
    }(window.jQuery), _.isUndefined(window.vc))var vc = {atts: {}};
if (function ($) {
        function parseOldDesignOptions() {
            ...
            }, params = vc.edit_element_block_view.model.get("params"), cssString = _.reduce(keys, function (memo, css_name, attr_name) {
        ...
        var removeOldDesignOptions, media = wp.media, preloaderUrl = ajaxurl.replace(/admin\-ajax\.php/, "images/wpspin_light.gif");
        media.controller.VcCssSingleImage = media.controller.VcSingleImage.extend({
            setCssEditor: function (view) {
                ...
                var attachments, selection = this.get("selection"), ids = this._css_editor.getBackgroundImage();
        ...
        VcCssEditor = vc.CssEditor = Backbone.View.extend({
            ...
            addBackgroundImage: function (e) {
                return e.preventDefault(), window.vc_selectedFilters = {}, this.image_media ? this.image_media.open("vc_editor") : (this.image_media = media({
                    ...
                    this.createSelectToolbar(toolbar, {text: window.i18nLocale.set_image})
            ...
            setCurrentBgImage: function (value) {
                var image_split, template, image_regexp = /([^\?]+)(\?id=\d+){0,1}/, url = "", id = "";
                value.match(/^\d+$/) ? (template = vc.template($("#vc_css-editor-image-block").html(), _.defaults({}, {variable: "img"}, vc.templateOptions.custom)), this.$el.find(".vc_background-image .vc_image").html(template({
                    url: preloaderUrl,
                    id: value,
                    ...
                    type: "POST",
                    url: window.ajaxurl,
                    data: {action: "wpb_single_image_src", content: value, size: "full", _vcnonce: window.vcAdminNonce},
                    dataType: "html",
                    context: this
                }).done(function (url) {
                    this.$el.find(".vc_ce-image").attr("src", url + "?id=" + value).removeClass("vc_preview")
                })) : value.match(image_regexp) && (image_split = value.split(image_regexp), url = image_split[1], image_split[2] && (id = image_split[2].replace(/[^\d]+/, "")), template = vc.template($("#vc_css-editor-image-block").html(), _.defaults({}, {variable: "img"}, vc.templateOptions.custom)), this.$el.find(".vc_background-image .vc_image").html(template({
                    url: url,
                    id: id
                })))
            },
            ...
            parseAtts: function (string) {
                ...
                border_regex = /(\d+\S*)\s+(\w+)\s+([\d\w#\(,]+)/, background_regex = /^([^\s]+)\s+url\(([^\)]+)\)([\d\w]+\s+[\d\w]+)?$/, background_size = !1, _.map(string.split(";"), function (val) {
                    var val_pos, border_split, background_split, val_s = val.split(/:\s/), value = val_s[1] || "", name = val_s[0] || "";
            ...
            getBackground: function () {
                var color = this.getBackgroundColor(), image = this.getBackgroundImageSrc(), style = this.getBackgroundStyle();
            ...
            getBorder: function () {
                var style = this.$el.find("[name=border_style]").val(), radius = this.$el.find("[name=border_radius]").val(), color = this.$el.find("[name=border_color]").val(), sides = ["left", "right", "top", "bottom"];
            ...
            getSimplifiedField: function (type) {
                var pos = "top", val = this.$el.find("[data-name=" + type + "-" + pos + "]").val().replace(/\s+/, "");
            ...
            parse: function (param) {
                ...
                $("[data-css-editor=true]", this.content()).each(function () {
                    var $editor = $(this), $param = $editor.find('input.wpb_vc_param_value[name="' + param.param_name + '"]'), value = $param.val();
                    value || (value = parseOldDesignOptions()), $param.data("vcFieldManager", new VcCssEditor({el: $editor}).render(value))
...
if (function ($) {
        var ParamsPresetView = Backbone.View.extend({
            events: {change: "setParams"}, setParams: function () {
                var data, val = this.save(), $option = this.$el.find('[value="' + val + '"]'), $edit_form = vc.edit_element_block_view.content(), $params_preset = this.$el;
                if ($option.length) {
        ...
        vc.atts.params_preset = {
            parse: function (param) {
                ...
                $(".vc_params-preset-select", $field).each(function () {
                    var $this = $(this);
                    _.isUndefined($this.data("fieldManager")) && $this.data("fieldManager", new ParamsPresetView({el: $this}).render())
                })
            }
        }
    ...
    vc.atts.vc_grid_item = {
        init: function (param, $field) {
            !0 === vc_user_access().getState("grid_builder") || null === vc_user_access().getState("grid_builder") ? this.content().find('[data-vc-shortcode-param-name="' + param.param_name + '"] [data-vc-grid-element="value"]').change(function () {
                var value = $(this).val(), url = $(this).find("[value=" + value + "]").data("vcLink");
