        ...
        $(function () {
            ...
            $('[data-vc-roles="part-state"]').click(function () {
                ...
                e.preventDefault(), $submitButton = $("#submit_btn"), $submitButton.attr("disabled", !0), $("#vc_wp-spinner").show(), data.action = $("#vc_settings-roles-action").val(), data.vc_nonce_field = $("#vc_nonce_field").val(), data.vc_roles = {}, $("[data-vc-role]").each(function () {
                    ...
                    var $messageHtml;
                    $("#vc_wp-spinner").hide(), data.message && ($messageHtml = $('<div id="vc_roles-message" class="updated vc_updater-result-message hidden"><p><strong></strong></p></div>'), $messageHtml.find("strong").text(data.message), $messageHtml.insertBefore($submitButton).fadeIn(100), window.setTimeout(function () {
            ...
            var filterValue, $control = $(this), $fieldset = $control.parents("fieldset").first(), filter = ".vc_wp-form-table";
            ...
            $this = $(this), checked = $this.is(":checked"), $parent = $this.parents('[data-vc-roles="table"]').first(), $relatedControl = $parent.find($this.data("vcRelatedControls")), value = $this.data("vcRolesSelectAllCheckbox"), $parent.find('[data-vc-cap="' + value + '"]:visible').prop("checked", checked), $relatedControl.prop("checked", checked), checked && _.defer(function () {
...
jQuery(document).ready(function ($) {
    "use strict";
    _.isUndefined(window.less) || (vc.less = {}, vc.less.options = {
        relativeUrls: !1,
        rootpath: !1
    }, less.options.env = vcData.debug ? "development" : "production", less.options.logLevel = vcData.debug ? 4 : 0, vc.less.generateLessFormData = function (formData, variablesData) {
        var lessData = {};
        return _.isEmpty(variablesData) || _.each(variablesData, function (value, key) {
            var object, objectValue;
            _.isString(value) ? (object = _.first(_.where(formData, {name: value})), _.isObject(object) && (objectValue = object.value, 0 < objectValue.length && (lessData[key] = objectValue))) : _.isObject(value) && !_.isUndefined(value.key) && (object = _.first(_.where(formData, {name: value.key})), _.isObject(object) || _.isUndefined(value.default_key) ? _.isObject(object) || _.isUndefined(value["default"]) || (object = {value: value["default"]}) : object = _.isUndefined(lessData[value.default_key]) ? _.first(_.where(formData, {name: value.default_key})) : {value: lessData[value.default_key]}, _.isObject(object) && (objectValue = object.value, _.isUndefined(value.modify_output) || !_.isObject(value.modify_output) || _.isEmpty(value.modify_output) || _.each(value.modify_output, function (modifier) {
                ...
                }), _.isUndefined(modifier.js_function) || !_.isObject(modifier.js_function) || _.isEmpty(modifier.js_function) || _.each(modifier.js_function, function (data) {
                    if (!_.isUndefined(data["function"])) {
                        var localValue, func;
                        func = new Function("value", "args", data["function"]), localValue = _.isUndefined(data.arguments) ? func(objectValue) : func(objectValue, data.arguments), objectValue = localValue
                    }
                })
            }), objectValue && 0 < objectValue.length && (lessData[key] = objectValue)))
        }), lessData
    }, vc.less.fileManager = less.FileManager.prototype.extractUrlParts, less.FileManager.prototype.extractUrlParts = function (url, baseUrl) {
        var output;
        return url += "?v=" + (window.vcData && window.vcData.version ? window.vcData.version : "4.5"), output = vc.less.fileManager(url, baseUrl)
...
var vc_am = {current_form: !1};
window.i18nLocaleVcAutomapper = window.i18nLocaleSettings, function ($) {
    "use strict";
    function VCS4() {
    ...
    function show_message(text, type) {
        ...
        $message.text(text), $message.prependTo($("#vc_settings-automapper")).fadeIn(500, function () {
            var $message = $(this);
    ...
    function showValidationError(text, $el) {
        (_.isUndefined($el) || !$el.length) && ($el = $(".tab_intro")), showMessageMore(text, "error", void 0, !0).insertBefore($el).fadeIn(500)
    }
    ...
    vc_am.vcGuid = function () {
        ...
        taggedString: function () {
            ...
            return _.each(this.attrs.numeric, function (value) {
                text += /\s/.test(value) ? ' <span class="vc_preview-param">"' + value + '"</span>' : ' <span class="vc_preview-param">' + value + "</span>"
            }), _.each(this.attrs.named, function (value, name) {
                text += ' <span class="vc_preview-param">' + name + '="' + value + '"</span>'
            }), "single" === this.type ? text + "]" : "self-closing" === this.type ? text + " /]" : (text += "]", this.content && (text += '<span class="vc_preview-content">' + this.content + "</span>"), text + '[/<span class="vc_preview-tag">' + this.tag + "</span>]")
        }
    ...
    to_title = function (string) {
        return string = string.replace(/_|-/, " "), string.charAt(0).toUpperCase() + string.slice(1)
    };
    var question = "?";
    window.ajaxurl.indexOf("?") > -1 && (question = "&");
    var request_url = window.ajaxurl + question + "vc_action=automapper", sync_callback = function (method, model, options) {
        var data;
        "create" === method ? (model.set("id", vc_am.vcGuid()), data = {
            vc_action: "create",
            data: model.toJSON()
        }) : data = "update" === method ? {
            vc_action: "update",
    ...
    }, ShortcodeModel = Backbone.Model.extend({
        defaults: function () {
            return {tag: "", name: "", category: "", description: "", params: []}
        }, sync: sync_callback
    }), ShortcodesCollection = Backbone.Collection.extend({model: ShortcodeModel, sync: sync_callback});
    ...
    var FormView = (Backbone.View.extend({
        ...
        edit: function (e) {
            e && e.preventDefault(), new EditFormView({model: this.model}).render()
        },
        clear: function (e) {
            e && e.preventDefault(), confirm(window.i18nLocaleVcAutomapper.are_you_sure_delete) && this.model.destroy()
        },
        removeView: function () {
            this.$el.remove()
        }
        ...
        render: function () {
            return vc_am.current_form && vc_am.current_form.close(), vc_am.current_form = this, this
        }, getType: function () {
            return "form"
            ...
            return _.each(attrs.params, function (param) {
                _.each(fields_required, function (field) {
                    "" === param[field] ? result = window.i18nLocaleVcAutomapper.error_enter_required_fields : "param_name" !== field || param[field].match(/^[a-z0-9_]+$/g) || (result = window.i18nLocaleVcAutomapper.error_enter_required_fields)
                }, this)
            }, this), result || null
        }, isValid: function (data) {
            return this.validationError = this.validate(data), this.validationError ? !1 : !0
        }, close: function (e) {
            e && e.preventDefault(), vc_am.current_form = !1, this.remove()
        }
        ...
        tagName: "li",
        className: "widget",
        events: {"click .vc_automapper-edit-btn": "edit", "click h4, .widget-action": "edit"},
        ...
        edit: function (e) {
            return e && e.preventDefault(), this.$editForm().is(":animated") ? !1 : (this.$el.addClass("vc_opened"), void(this.edit_view ? this.close() : this.edit_view = new EditFormInnerView({model: this.model}).render()))
        },
        ...
        clear: function (e) {
            e && e.preventDefault(), this.model.destroy()
        },
        removeView: function () {
            this.remove()
        }
        ...
        events: {"click #vc_atm-parse-string": "parseShortcode", "click .vc_atm-cancel": "close"},
        getType: function () {
            return "create"
        },
        render: function () {
        ...
        changeParamType: function (e) {
            var $this, $parent;
            $this = $(e.currentTarget), $parent = $this.parents(".vc_fields"), "hidden" === $this.val() ? ($parent.find('[name="heading"]').attr("disabled", !0), $parent.find('[name="description"]').attr("disabled", !0)) : ($parent.find('[name="heading"]').attr("disabled", !1), $parent.find('[name="description"]').attr("disabled", !1))
        },
        ...
        setParamFieldActive: function (e) {
            ...
            this.active_preview && $(this.active_preview).removeClass("vc_active"), this.active_preview = "#vc_shortcode-preview .vc_preview-param:eq(" + index + ")", $(this.active_preview).addClass("vc_active")
        },
        ...
        getPreview: function (data) {
            var params = data.params, content = !1, params_to_string = {};
        ...
        setPreview: function () {
            var data = {params: this.getParams(), tag: $("#vc_atm-tag").val()};
        ...
        validate: function (attrs) {
            ...
            var result = !1, added_param_names = {};
            if (!attrs.name)return $("#vc_atm-name").addClass("vc_error"), $("#vc_atm-header-name").parent().addClass("form-invalid"), window.i18nLocaleVcAutomapper.error_shortcode_name_is_required;
            ...
            return _.each(attrs.params, function (param, index) {
                ...
                }, this)) : (result = window.i18nLocaleVcAutomapper.error_content_param_not_manually, $field_el.addClass("vc_error"), void $field_el.closest(".vc_param-field").addClass("form-invalid"))
            }, this), result || null
        },
        setContentParam: function (e) {
            var $control = $(e.currentTarget);
            $control[0].checked ? (this.addParamField({
                type: "textarea",
                heading: "Content",
                description: "",
                param_name: "content",
        ...
        addParam: function (e) {
            e && e.preventDefault(), this.addParamField({
                type: "",
                heading: "",
                description: "",
                param_name: "",
        ...
        addParamField: function (attr) {
            var $block = $('<div class="vc_param wpb_vc_row' + ("content" === attr.param_name ? " vc_content" : "") + '"/>').appendTo("#vc_atm-params-list"), template = vc.template(this.param_template_html, vc.templateOptions.custom);
        ...
        deleteParam: function (e) {
            var $control;
            e && e.preventDefault(), confirm(window.i18nLocaleVcAutomapper.are_you_sure_delete_param) && ($control = $(e.currentTarget), $control.parents(".vc_param").remove(), this.setPreview())
        },
        close: function (e) {
            e && e.preventDefault(), this.model && $("[data-item-id=" + this.model.get("id") + "]").show(), vc_am.current_form = !1, $(".vc_atm-message").remove(), this.remove()
        },
        clear: function (e) {
            e && e.preventDefault(), confirm(window.i18nLocaleVcAutomapper.are_you_sure_delete) && (this.model.destroy(), this.close())
        }
    }), EditFormInnerView = EditFormView.extend({
        template_html: $("#vc_automapper-form-tpl").html(),
        getType: function () {
            return "edit"
        },
        ...
        render: function () {
            ...
            params = this.model.get("params"), EditFormView.__super__.render.call(this);
            var template = vc.template(this.template_html, vc.templateOptions.custom);
        ...
        save: function (e) {
            e && e.preventDefault(), this.model.view.beforeSave(), EditFormInnerView.__super__.save.call(this)
        },
        close: function (e) {
            e && e.preventDefault(), vc_am.current_form = !1, this.model.view.close()
        },
        clear: function (e) {
            e && e.preventDefault(), confirm(window.i18nLocaleVcAutomapper.are_you_sure_delete) && (this.model.view.clear(), this.remove())
        }
    }), AppView = Backbone.View.extend({
        events: {"click #vc_automapper-add-btn": "create", submit: "formSubmit"},
        ...
        formSubmit: function (e) {
            if (e && e.preventDefault && e.preventDefault(), _.isObject(e) && this.addFormView && !_.isEmpty(e.currentTarget) && !_.isEmpty(e.currentTarget[0])) {
                var node, $el;
                node = e.currentTarget[0], $el = $(node), $el.is("#vc_atm-shortcode-string") && this.addFormView.parseShortcode()
            }
        },
    ...
    $(".wpb_settings_accordion").accordion({
        active: getCookie("wpb_js_composer_settings_group_tab") ? getCookie("wpb_js_composer_settings_group_tab") : !1,
        collapsible: !0,
        change: function (event, ui) {
            "undefined" != typeof ui.newHeader.attr("id") ? setCookie("wpb_js_composer_settings_group_tab", "#" + ui.newHeader.attr("id"), 31536e3) : setCookie("wpb_js_composer_settings_group_tab", "", 31536e3)
        },
        heightStyle: "content"
        ...
        return $helper.is(":animated") ? !1 : void $helper.toggle(100)
    }), $(".color-control").wpColorPicker(), $("#vc_settings-color-restore-default").click(function (e) {
        ...
        return $parent.addClass("loading"), $this.attr("disabled", !0), $.getJSON(window.ajaxurl, {
            ...
            response.status && (window.location = response.url)
        }).error(function () {
            $parent.removeClass("loading"), $this.removeAttr("disabled")
        }), e.preventDefault(), !1
    });
    var $css_editor = $("#wpb_csseditor"), $css_editor_input = $("textarea.custom_css.wpb_csseditor");
    ...
    $("#vc_settings-color").submit(function (e) {
        if (e.preventDefault(), !lessBuilding) {
            var form, $submitButton, $designCheckBox;
            if (form = this, $submitButton = $("#submit_btn"), $designCheckBox = $("#wpb_js_use_custom"), $designCheckBox.prop("checked") && "restore_color" !== $("#vc_settings-color-action").val()) {
                var modifyVars, variablesDataLinker, $spinner;
                lessBuilding = !0, modifyVars = $(form).serializeArray(), variablesDataLinker = $submitButton.data("vc-less-variables"), $spinner = $('<span class="vc_settings-spinner vc_ui-wp-spinner"></span>'), $submitButton.val(window.i18nLocaleSettings.saving), $spinner.insertBefore($submitButton).show(), _.delay(function () {
                    vc.less.build({
