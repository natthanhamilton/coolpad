...
!function ($) {
    vc_iframe.showNoContent = function (show) {
        ...
        var $el, el_height, position_y, position, window_height = $(window).height(), window_scroll_top = $(window).scrollTop();
        if (id && ($el = $("[data-model-id=" + id + "]"))) {
            if (position = $el.offset(), !1 === (position_y = position ? position.top : !1))return !1;
            el_height = $el.height(), (position_y > window_scroll_top + window_height || window_scroll_top > position_y + el_height) && $.scrollTo($el, 500, {offset: -50})
        }
        ...
        $vc_row_el.parent().sortable({
            ...
            over: function (event, ui) {
                var tag = ui.item.data("tag"), vc_map = window.parent.vc.map || !1, parent_tag = ui.placeholder.closest("[data-tag]").data("tag"), allowed_container_element = "undefined" == typeof vc_map[parent_tag].allowed_container_element ? !0 : vc_map[parent_tag].allowed_container_element;
                if (ui.placeholder.removeClass("vc_hidden-placeholder"), ui.placeholder.css({maxWidth: ui.placeholder.parent().width()}), tag && vc_map) {
                    ...
                    is_container && !0 !== allowed_container_element && allowed_container_element !== tag.replace(/_inner$/, "") && ui.placeholder.addClass("vc_hidden-placeholder")
                }
            },
            ...
            stop: function (event, ui) {
                var item_model, tag = ui.item.data("tag"), vc_map = window.parent.vc.map || !1, parent_tag = ui.item.parents("[data-tag]:first").data("tag"), allowed_container_element = vc_map[parent_tag].allowed_container_element ? vc_map[parent_tag].allowed_container_element : !0, trig_changed = !0;
                window.parent.vc.checkRelevance(parent_tag, tag) || (ui.placeholder.removeClass("vc_hidden-placeholder"), $(this).sortable("cancel"), trig_changed = !1);
                var is_container = vc_map[tag] === Object(vc_map[tag]) && ((!0 === vc_map[tag].is_container || !1 === vc_map[tag].is_container || "[object Boolean]" === toString.call(vc_map[tag].is_container)) && !0 === vc_map[tag].is_container || null != vc_map[tag].as_parent && "[object Array]" === Object.prototype.toString.call(vc_map[tag].as_parent) && 0 != vc_map[tag].as_parent);
                is_container && !0 !== allowed_container_element && allowed_container_element !== tag.replace(/_inner$/, "") && (ui.placeholder.removeClass("vc_hidden-placeholder"), $(this).sortable("cancel"), trig_changed = !1), trig_changed && (item_model = parent.vc.shortcodes.get(ui.item.data("modelId")), item_model.view.parentChanged()), vc_iframe.stopSorting()
            }
        }), $(".wpb_row").sortable({
            forcePlaceholderSize: !0,
            tolerance: "pointer",
            ...
            start: function (event, ui) {
                ...
                var id = ui.item.data("modelId"), model = parent.vc.shortcodes.get(id), css_class = model.view.convertSize(model.getParam("width"));
        ...
        var i, scripts_string, ls_rc, scripts = [], scripts_to_add = [];
        ...
        return vc_iframe.loaded_script[window.parent.vc_globalHashCode(src)] ? !1 : (vc_iframe.loaded_script[window.parent.vc_globalHashCode(src)] = src, src)
        ...
        $elements.each(function () {
            $("<style/>", {type: "text/css"}).html($(this).html()).appendTo("body")
        })
        ...
        return vc_iframe.scripts_to_wait && vc_iframe.scripts_to_load ? (vc_iframe.scripts_to_load.each(function () {
            ...
            if (vc_iframe.reload_safety_call = !0, $element.is("script")) {
                ...
                }) : (vc_iframe.scripts_to_wait -= 1, 1 > vc_iframe.scripts_to_wait && vc_iframe.reload()); else if ($element) {
                    try {
                        $("<script/>", {type: "text/javascript"}).html($element.html()).appendTo("body")
                    } catch (e) {
        ...
        var ver = $.ui.version.split("."), old_version = 1 === parseInt(ver[0]) && 9 > parseInt(ver[1]);
        return $tab.each(function (index) {
            var $tabs, tabs_array = ($(this).attr("data-interval"), []), $wrapper = $(this).find(".wpb_tour_tabs_wrapper");
        ...
        $el.each(function (index) {
            var $tabs, $this = $(this), $wrapper = $this.find(".wpb_accordion_wrapper"), active_tab = ($this.attr("data-interval"), !isNaN($this.data("active-tab")) && 0 < parseInt($this.data("active-tab")) ? parseInt($this.data("active-tab")) - 1 : !1), collapsible = !1 === active_tab || "yes" === $this.data("collapsible");
            $wrapper.hasClass("ui-widget") ? (!1 === active && (active = $wrapper.accordion("option", "active")), $wrapper.accordion("refresh"), $wrapper.accordion("option", "active", active)) : $tabs = $this.find(".wpb_accordion_wrapper").accordion({
        ...
        var $el = $("[data-model-id=" + model_id + "]"), $carousel = ($el.find("img").length, $el.find('[data-ride="vc_carousel"]'));
        ...
        var $el = $("[data-model-id=" + model_id + "]"), $gallery = $el.find(".wpb_gallery_slides");
        ...
        if ($gallery.hasClass("wpb_flexslider")) {
            var sliderSpeed = 800, sliderTimeout = 1e3 * parseInt($gallery.attr("data-interval")), sliderFx = $gallery.attr("data-flex_fx"), slideshow = !0;
            ...
            var sliderSpeed = 800, sliderTimeout = 1e3 * $gallery.attr("data-interval");
        ...
        }) : $gallery.find(".wpb_image_grid_ul").isotope({itemSelector: ".isotope-item", layoutMode: "fitRows"}))
        ...
        $grid.each(function () {
            var $grid = $(this), vcGrid = $(this).data("vcGrid");
