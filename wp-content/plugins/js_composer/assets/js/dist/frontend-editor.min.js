...
function vc_convert_column_size(width) {
    var prefix = "vc_col-sm-", numbers = width ? width.split("/") : [1, 1], range = _.range(1, 13), num = !_.isUndefined(numbers[0]) && 0 <= _.indexOf(range, parseInt(numbers[0], 10)) ? parseInt(numbers[0], 10) : !1, dev = !_.isUndefined(numbers[1]) && 0 <= _.indexOf(range, parseInt(numbers[1], 10)) ? parseInt(numbers[1], 10) : !1;
...
function vc_get_column_mask(cells) {
    var i, columns = cells.split("_"), columns_count = columns.length, numbers_sum = 0;
    for (i in columns)if (!isNaN(parseFloat(columns[i])) && isFinite(columns[i])) {
...
function vc_button_param_target_callback() {
    var $ = jQuery, $link_target = this.$content.find("[name=target]").parents('[data-vc-ui-element="panel-shortcode-param"]:first'), $link_field = $(".wpb-edit-form [name=href]"), key_up_callback = _.debounce(function () {
        var val = $(this).val();
...
function vc_grid_exclude_dependency_callback() {
    var $ = jQuery, exclude_el = $(".wpb_vc_param_value[name=exclude]", this.$content), exclude_obj = exclude_el.data("vc-param-object");
    if (void 0 === exclude_obj)return !1;
    var post_type_object = $('select.wpb_vc_param_value[name="post_type"]', this.$content), val = post_type_object.val();
...
function vc_wpnop(content) {
    var blocklist1, blocklist2, preserve_linebreaks = !1, preserve_br = !1;
...
function vcAddShortcodeDefaultParams(model) {
    ...
    params = _.extend({}, vc.getDefaults(model.get("shortcode")), params), model.set({params: params}, {silent: !0})
}
...
if (function () {
        ...
        TabsLine = function (element, options) {
            ...
            return _this.tabs = [], _this.dropdownTabs = [], _this.$element.find(_this.options.tabSelector).each(function () {
                _this.tabs.push({$tab: $(this), width: $(this).outerWidth()})
            }), _this.$dropdown.find(_this.options.dropdownTabSelector).each(function () {
                var $tempElement = $(this).clone().css({visibility: "hidden", position: "fixed"});
            ...
            return this.each(function () {
                var $this = $(this), optionsData = $this.data("vcUiTabsLine"), data = $this.data("vc.tabsLine"), options = $.extend(!0, {}, TabsLine.DEFAULTS, $this.data(), optionsData, "object" == typeof option && option), action = "string" == typeof option ? option : options.action;
...
!function ($) {
    ...
    _.isUndefined(window.vc) && (window.vc = {}), window.Vc_postSettingsEditor = Backbone.View.extend({
        ...
        setEditorTextarea: function (value) {
            return this.$editor || (this.$editor = $("<textarea></textarea>").css({
                ...
                width: "100%",
                height: "100%"
            })), this.$editor.val(value), this.$editor.focus(), this.$editor.parent().css({overflow: "auto"}), this.$editor
        },
        setSize: function () {
            ...
            this.aceEnabled() ? $("#" + this.sel).css({
                height: height,
                minHeight: height
            }) : (this.$editor.parent().css({height: height, minHeight: height}), this.$editor.css({
                height: "98%",
                width: "98%"
            }))
        },
...
}(window.jQuery), function (_, Backbone, vc) {
    "use strict";
    ...
    for (i = 0; i < ARRcookies.length; i++)if (x = ARRcookies[i].substr(0, ARRcookies[i].indexOf("=")), y = ARRcookies[i].substr(ARRcookies[i].indexOf("=") + 1), x = x.replace(/^\s+|\s+$/g, ""), x == c_name)return decodeURIComponent(y)
}), function ($) {
...
if (vc.memoizeWrapper = function (func, resolver) {
        ...
        if (value) {
            var params = model.get("params");
            params && params.css && (params.css = params.css.replace(/(background(\-position)?\s*\:\s*[\S]+(\s*[^\!\s]+)?)[\s*\!important]*/g, "$1"), model.set("params", params, {silent: !0}))
        }
    }), vc.events.on("shortcodes:vc_single_image:sync shortcodes:vc_single_image:add", function (model) {
        var params = model.get("params");
        params.link && !params.onclick && (params.onclick = "custom_link", model.save({params: params}))
        ...
        var attachmentCompatRender, attachCb = [], media = wp.media, origFeaturedImageSet = media.featuredImage.set, origEditorSendAttachment = media.editor.send.attachment, l10n = i18nLocale, workflows = {};
        attachmentCompatRender = _.extend(media.view.AttachmentCompat.prototype.render), media.view.AttachmentCompat.prototype.render = function () {
            ...
            defaults: _.defaults({
                ...
                priority: 60,
                syncSelection: !1
            }, media.controller.Library.prototype.defaults), updateSelection: function () {
                var attachments, selection = this.get("selection"), ids = media.vc_editor.getData();
                this.get("library");
            ...
            getData: function () {
                ...
                return window.vc_selectedFilters = {}, this.element = element, this.$button = $(this.element), this.$block = this.$button.closest(".edit_form_line"), this.$hidden_ids = this.$block.find(".gallery_widget_attached_images_ids"), this.$img_ul = this.$block.find(".gallery_widget_attached_images_list"), this.$clear_button = this.$img_ul.next(), this._frame ? this._frame : (this._frame = media({
                    ...
                    this.createSelectToolbar(toolbar, {text: l10n.set_image, close: !1})
            ...
            getData: function () {
                var $button = media.vc_editor.$vc_editor_element, $block = $button.closest(".edit_form_line"), $hidden_ids = $block.find(".gallery_widget_attached_images_ids");
                return $hidden_ids.val()
            },
            insert: function (images) {
                var $button = media.vc_editor.$vc_editor_element, $block = $button.closest(".edit_form_line"), $hidden_ids = $block.find(".gallery_widget_attached_images_ids"), $img_ul = $block.find(".gallery_widget_attached_images_list"), $thumbnails_string = "", template = vc.template($("#vc_settings-image-block").html(), vc.templateOptions.custom);
...
if (window.vc.filters = {templates: []}, window.vc.addTemplateFilter = function (callback) {
        ...
        window.init_textarea_html = function ($element) {
            ...
            if (6 > hex.length)for (var i = 6 - hex.length - 1; i >= 0; i--)hex = "0" + hex;
            return "#" + hex
        }, vc.loop_partial = function (template_name, key, loop, settings) {
            var data = _.isObject(loop) && !_.isUndefined(loop[key]) ? loop[key] : "", template = vc.template($("#_vcl-" + template_name).html(), vc.templateOptions.custom);
            return template({name: key, data: data, settings: settings})
            ...
            constructor: Suggester, init: function () {
                ...
                var position = this.$el_wrap.position(), block_position = this.$block.position();
                ...
                var $label, exclude_css, index = this.selected_items.push(item) - 1, remove = !0 === this.options.check_locked_callback(this.$el, item) ? "" : ' <a class="remove">&times;</a>';
                ...
                e.preventDefault();
                var $label = $(e.currentTarget), index = parseInt($label.data("index"), 10), $target = $(e.target);
            ...
            return this.each(function () {
                var $this = $(this), data = $this.data("suggester");
        ...
        var VcLoopEditorView = Backbone.View.extend({
            ...
            render: function (controller) {
                var template = vc.template($("#vcl-loop-frame").html(), _.extend({}, vc.templateOptions.custom, {variable: "loop"}));
                return this.controller = controller, this.$el.html(template(this.model)), this.controller.$el.append(this.$el), _.each($("[data-suggest]"), function (object) {
                    var $field = $(object), current_value = window.decodeURIComponent($("[data-suggest-prefill=" + $field.data("suggest") + "]").val());
                    $field.suggester({
            ...
            updateCheckbox: function (e) {
                var $checkbox = $(e.currentTarget), input_name = $checkbox.data("input"), $input = $("[data-name=" + input_name + "]", this.$el), value = [];
            ...
            suggestionLocked: function ($elem, data) {
                var value = data.value, field = $elem.closest("[data-block=suggestion]").find("[data-suggest-value]").data("suggest-value");
            ...
            events: {"click .vc_loop-build": "showEditor"}, initialize: function () {
                ...
                this.loop_editor_view = new VcLoopEditorView({model: _.isEmpty(data) ? {} : data}), this.loop_editor_view.render(this).show()
            ...
            events: {
                ...
                "change input, select": "save",
                "keyup input": "save"
            }, data: {}, fields: {}, initialize: function () {
                ...
                _.each(this.$input.val().split("|"), function (data) {
                    if (data.match(/\:/)) {
                        var split = data.split(":"), name = split[0], value = split[1];
        ...
        VcSortedList.prototype = {
            constructor: VcSortedList, init: function () {
                ...
                for (var i in selected_data) {
                    var control_settings = selected_data[i].split("|"), $control = control_settings.length ? this.$toolbar.find("[data-element=" + decodeURIComponent(control_settings[0]) + "]") : !1;
                ...
                this.$current_control.sortable({stop: this.save}).on("change", "select", this.save)
            }, createControl: function (data) {
                var sub_control = "", selected_sub_value = _.isUndefined(data.sub_value) ? [] : data.sub_value;
            ...
            return this.each(function () {
                var $this = $(this), data = $this.data("vc_sorted_list");
        ...
        var GoogleFonts = Backbone.View.extend({
            ...
            fontFamilyDropdownChange: function () {
                ...
                return this.font_family_url = $font_family_selected.val(), this.font_family = $font_family_selected.attr("data[font_family]"), this.font_types = $font_family_selected.attr("data[font_types]"), this.$font_style_dropdown_el_container.parent().hide(), this.font_family_url && 0 < this.font_family_url.length && WebFont.load({
                    google: {families: [this.font_family_url]},
            ...
            fontStyleDropdownRender: function () {
                var str = this.font_types, str_arr = str.split(","), oel = "", default_f_style = this.$font_family_dropdown_el.attr("default[font_style]");
                for (var str_inner in str_arr) {
                    var str_arr_inner = str_arr[str_inner].split(":"), sel = "";
            ...
            resize: function () {
                var position = this.$el_wrap.position(), block_position = this.$block.position(), widget = this.$el.autocomplete("widget");
            ...
            sortableUpdate: function (event, ui) {
                var elems = this.$sortable_wrapper.sortable("toArray", {attribute: "data-index"}), items = [];
            ...
            open: function (event, ui) {
                var widget = this.getWidget().menu(), widget_position = widget.position();
                ...
                if (this.selected = 1, ui.item) {
                    if (this.options.unique_values) {
                        ...
                        if (this.options.groups) {
                            var $prev_el = $li_el.prev(), $next_el = $li_el.next();
            ...
            createBox: function (item) {
                var $label, index = this.selected_items.push(item) - 1, remove = '<a class="vc_autocomplete-remove">&times;</a>';
            ...
            $content: {}, initialize: function () {
                ...
                return self = this, $('[data-vc-ui-element="panel-shortcode-param"]', this.content()).each(function () {
                    var _this = $(this), param = _this.data("param_settings");
                    vc.atts.init.call(self, param, _this)
                }), this
            }
        }), VC_ParamGroup = Backbone.View.extend({
            options: {max_items: 0, sortable: !0, deletable: !0, collapsible: !0},
            items: 0,
            $ul: !1,
            initializer: {},
            mappedParams: {},
            adminLabelParams: [],
            groupParamName: "",
            events: {"click > .edit_form_line > .vc_param_group-list > .vc_param_group-add_content": "addNew"},
            ...
            dependentElements: !1, mappedParams: !1, groupParamName: "", adminLabelParams: [], events: {
                ...
                for (callback = this.hookAdminLabel, i = 0; i < this.adminLabelParams.length; i++)$fields = $("[name=" + this.adminLabelParams[i] + "].wpb_vc_param_value", this.$el), $fields.each(function () {
                    var $field = $(this);
                    $field.data("vc_admin_labels") || ($field.data("vc_admin_labels", !0), $field.bind("keyup change", callback), callback({currentTarget: this}))
                })
                ...
                if (ev.preventDefault(), this.options.parent.addAllowed()) {
                    var param = this.options.parent.$ul.data("settings"), $content = this.$content;
                ...
                ev.preventDefault();
                var $parent = this.$el, $elem = $parent.find("> .wpb_element_wrapper");
        ...
        vc.edit_form_callbacks = [], vc.atts = {
            ...
            parse: function (param) {
                var _window = this.window(), $field = this.content().find(".textarea_html." + param.param_name);
                try {
            ...
            clone: function (clonedModel, paramValue, paramSettings) {
                ...
                if (_.isEmpty(paramValue) && !_.isUndefined(paramSettings) && !_.isUndefined(paramSettings.settings) && !_.isUndefined(paramSettings.settings.auto_generate) && 1 == paramSettings.settings.auto_generate) {
                    var shortcodeParams;
                    shortcodeParams = shortcodeModel.get("params"), shortcodeParams[paramSettings.param_name] = Date.now() + "-" + vc_guid(), shortcodeModel.set({params: shortcodeParams}, {silent: !0})
                }
            }
        }, vc.events.on("shortcodes:add:param:type:el_id", vc.atts.el_id.create), vc.atts.posttypes = {
            parse: function (param) {
                var posstypes_arr = [], new_value = "";
            ...
            render: function (param, value) {
                ...
                $(".wpb_vc_param_value.dropdown", $field).change(function () {
                    var $this = $(this), $options = $this.find(":selected"), prev_option_class = $this.data("option"), option_class = $options.length ? $options.attr("class").replace(/\s/g, "_") : "";
            ...
            parse: function (param) {
                var $field = this.content().find(".wpb_vc_param_value[name=" + param.param_name + "]"), thumbnails_html = "";
                ...
                return "external_link" === this.model.getParam("source") && (value = this.model.getParam("custom_srcs")), _.isEmpty(value) ? (this.$el.removeData("field-" + param.param_name + "-attach-images"), vc.atts.attach_images.updateImages($thumbnails, "")) : $.ajax({
                    type: "POST",
                    url: window.ajaxurl,
                    data: {action: "wpb_gallery_html", content: value, _vcnonce: window.vcAdminNonce},
                    dataType: "html",
                    context: this
                }).done(function (html) {
            ...
            parse: function (param) {
                var $field = this.content().find(".wpb_vc_param_value[name=" + param.param_name + "]"), val = "";
                return $field.length && "http://" !== $field.val() && (val = $field.val()), val
            }
        }, vc.atts.attach_image = {
            parse: function (param) {
                var $field = this.content().find(".wpb_vc_param_value[name=" + param.param_name + "]"), image_src = "";
                ...
                var $model = $("[data-model-id=" + this.model.id + "]"), image_src = $model.data("field-" + param.param_name + "-attach-image"), $thumbnail = this.$el.find(".attachment-thumbnail[data-name=" + param.param_name + "]"), $post_id = $("#post_ID"), post_id = $post_id.length ? $post_id.val() : 0;
            ...
            parse: function (param) {
                var string_pieces, string, $field = this.content().find(".wpb_vc_param_value[name=" + param.param_name + "]"), $block = $field.parent(), options = {};
                ...
                var $g_fonts = $field;
                $g_fonts.length && ("undefined" != typeof WebFont ? $field.data("vc-param-object", new GoogleFonts({el: $g_fonts})) : $g_fonts.find("> .edit_form_line").html(window.i18nLocale.gfonts_unable_to_load_google_fonts || "Unable to load Google Fonts"))
            }
            ...
            parse: function (param) {
                ...
                $field.data("vc-param-object", new VC_ParamGroup({el: $field, settings: {param: param}}))
            }
        }, vc.atts.colorpicker = {
            init: function (param, $field) {
                var $content = $field;
                $(".vc_color-control", $content).each(function () {
                    var $alpha, $alpha_output, $pickerContainer, $control = $(this), value = $control.val().replace(/\s+/g, ""), alpha_val = 100;
                    value.match(/rgba\(\d+\,\d+\,\d+\,([^\)]+)\)/) && (alpha_val = 100 * parseFloat(value.match(/rgba\(\d+\,\d+\,\d+\,([^\)]+)\)/)[1])), $control.wpColorPicker({
                        ...
                        var alpha_val = parseFloat($alpha.val()), iris = $control.data("a8c-iris"), color_picker = $control.data("wp-wpColorPicker");
                        $alpha_output.val($alpha.val() + "%"), iris._color._alpha = alpha_val / 100, $control.val(iris._color.toString()), color_picker.toggler.css({backgroundColor: $control.val()})
            ...
            init: function (param, $field) {
                ...
                $el_type_autocomplete.length && $el_type_autocomplete.each(function () {
                    var options, ac, $param = $(".wpb_vc_param_value", this), param_name = $param.attr("name"), $el = $(".vc_auto_complete_param", this);
                    options = $.extend({
                        $param_input: $param,
            ...
            change: function (param, $field) {
                ...
                var $field = this.content().find(".wpb_vc_param_value[name=" + param.param_name + "]"), $block = $field.parent(), val = $block.find(".vc-iconpicker").val();
                return val
            }, init: function (param, $field) {
                var $el = $field.find(".wpb_vc_param_value"), settings = $.extend({
                    iconsPerPage: 100,
            ...
            init: function (param, $field) {
                ...
                var content = $field, $field_input = $(".wpb_vc_param_value[name=" + param.param_name + "]", content);
            ...
            params = model.get("params"), settings = vc.map[model.get("shortcode")], _.isArray(settings.params) && _.each(settings.params, function (p) {
                p && !_.isUndefined(p.type) && ("tab_id" === p.type && _.isEmpty(params[p.param_name]) ? (_changed = !0, params[p.param_name] = vc_guid() + "-" + Math.floor(11 * Math.random())) : "vc_grid_id" === p.type && (_changed = !0, params[p.param_name] = vc.atts.vc_grid_id.parse.call({model: model})))
            }), _changed && model.save("params", params, {silent: !0})
        ...
        _.isUndefined(window.vc) && (window.vc = {}), vc.ShortcodesBuilder = function (models) {
            ...
            _ajax: !1, message: !1, isBuildComplete: function () {
                ...
                tag = model.get("shortcode"), shortcode = this.toString(model), vc.setDataChanged(), this.ajax({
                    action: "vc_load_shortcode",
                    shortcodes: [{id: model.get("id"), string: shortcode, tag: tag}],
                    _vcnonce: window.vcAdminNonce
                }, vc.frame_window.location.href).done(function (html) {
                    var old_view;
                    old_view = model.view, _.each($(html), function (block) {
                        this._renderBlockCallback(block)
                    }, this), model.view && (model.view.$el.insertAfter(old_view.$el), vc.shortcodes.where({parent_id: model.get("id")}).length && old_view.content().find("> *").appendTo(model.view.content()), old_view.remove(), vc.frame_window.vc_iframe.loadScripts(), model.view.changed(), vc.frame.setSortable(), model.view.updated())
                })
                ...
                return this._ajax = $.ajax({
                    ...
                    type: "POST",
                    dataType: "html",
                    data: _.extend({post_id: vc.post_id, vc_inline: !0, _vcnonce: window.vcAdminNonce}, data),
                    context: this
                })
                ...
                var models = _.sortBy(vc.shortcodes.where({parent_id: !1}), function (model) {
                ...
                var string = this.getContent(), post_data = $("#post").serializeArray(), data = {};
                ...
                var tags = _.keys(vc.map).join("|"), reg = window.wp.shortcode.regexp(tags), matches = content.trim().match(reg);
                return _.isNull(matches) ? data : (_.each(matches, function (raw) {
                    var shortcode, map_settings, sub_matches = raw.match(this.regexp(tags)), sub_content = sub_matches[5], sub_regexp = new RegExp("^[\\s]*\\[\\[?(" + _.keys(vc.map).join("|") + ")(?![\\w-])"), atts_raw = window.wp.shortcode.attrs(sub_matches[3]), atts = {}, id = vc_guid();
        ...
        var Shortcode = Backbone.Model.extend({
            defaults: function () {
                var id = vc_guid();
                return {id: id, shortcode: "vc_text_block", order: vc.shortcodes.nextOrder(), params: {}, parent_id: !1}
            }, settings: !1, getParam: function (key) {
            ...
            model: Shortcode, sync: function () {
                ...
                var models, params;
                return models = _.sortBy(vc.shortcodes.where({parent_id: parent.get("id")}), function (model) {
                    return model.get("order")
        ...
        _.isUndefined(window.vc) && (window.vc = {}), vc.showSpinner = function () {
            ...
            events: {"hidden.bs.modal": "hide", "shown.bs.modal": "shown"},
            initialize: function () {
            ...
            buildFiltering: function () {
                ...
                var mapped = vc.getMapped(tag), as_parent = tag && !_.isUndefined(mapped.as_parent) ? mapped.as_parent : !1;
            ...
            createElement: function (e) {
                ...
                if (!this.preventDoubleExecution) {
                    ...
                    "vc_row" === tag ? this.builder.create({
                        ...
                        parent_id: this.builder.lastID(),
                        params: column_params
                    }) : "vc_row_inner" === tag && (column_params = {width: "1/1"}, "undefined" != typeof window.vc_settings_presets.vc_column_inner && (column_params = _.extend(column_params, window.vc_settings_presets.vc_column_inner)), this.builder.create({
            ...
            filterElements: function (e) {
                ...
                var $control = $(e.currentTarget), filter = '[data-vc-ui-element="add-element-button"]', name_filter = $("#vc_elements_name_filter").val();
            ...
            render: function (model, prepend) {
                ...
                if (!this.preventDoubleExecution) {
                    ...
                    row_params = {}, "undefined" != typeof window.vc_settings_presets.vc_row && (row_params = _.extend(row_params, window.vc_settings_presets.vc_row)), column_params = {width: "1/1"}, "undefined" != typeof window.vc_settings_presets.vc_column && (column_params = _.extend(column_params, window.vc_settings_presets.vc_column)), !1 === this.model ? (row = vc.shortcodes.create({
                        ...
                        parent_id: column.id,
                        root_id: row.id
                    }) : row) : "vc_row" === tag ? (row_inner_params = {}, "undefined" != typeof window.vc_settings_presets.vc_row_inner && (row_inner_params = _.extend(row_inner_params, window.vc_settings_presets.vc_row_inner)), column_inner_params = {width: "1/1"}, "undefined" != typeof window.vc_settings_presets.vc_column_inner && (column_inner_params = _.extend(column_inner_params, window.vc_settings_presets.vc_column_inner)), row = vc.shortcodes.create({
            ...
            _vcUIEventsHooks: [{resize: "setResize"}],
            options: {startTab: 0},
            clicked: !1,
            ...
            fixElContainment: function () {
                ...
                var el_w = this.$el.width(), container_w = this.$body.width(), container_h = this.$body.height(), containment = [-el_w + 20, 0, container_w - 20, container_h - 30], positions = this.$el.position(), new_positions = {};
            ...
            saveSettings: function (title, is_default) {
                var shortcode_name = this.model.get("shortcode"), data = JSON.stringify(this.getParamsForSettingsPreset()), success = !1;
            ...
            fetchSaveSettingsDialog: function (callback) {
                var $dropdown = this.$el.find(this.settingsDropdownSelector), success = !1;
            ...
            showSaveSettingsDialog: function (is_default) {
                ...
                this.isSettingsPresetDefault = !!is_default, this.fetchSaveSettingsDialog(function (created) {
                    var $dropdown = that.$el.find(that.settingsDropdownSelector), $button = that.$el.find(that.settingsButtonSelector), $prompt = $dropdown.find(".vc_ui-prompt"), $title = $prompt.find(".textfield");
                    $prompt.addClass("vc_visible"), $button.prop("disabled", !0), $title.focus(), created && ($prompt.on("submit", function () {
                        var title = $title.val(), $button = that.$el.find(that.settingsButtonSelector);
            ...
            loadSettingsAjaxData: function (id) {
                return {action: "vc_action_get_settings_preset", vc_inline: !0, id: id, _vcnonce: window.vcAdminNonce}
            },
            ...
            deleteSettings: function (id) {
                var shortcode_name = this.model.get("shortcode"), success = !1;
                confirm(window.i18nLocale.delete_preset_confirmation) && $.ajax({
                    type: "POST", dataType: "json",
                    url: window.ajaxurl, data: this.deleteSettingsAjaxData(shortcode_name, id), context: this
                }).done(function (response) {
            ...
            setSettingsMenuContent: function (html) {
                var $button = this.$el.find(this.settingsButtonSelector), $menu = this.$el.find(this.settingsMenuSelector), shortcode_name = this.model.get("shortcode"), that = this;
            ...
            reloadSettingsMenuContent: function () {
                var shortcode_name = this.model.get("shortcode"), $button = this.$el.find(this.settingsButtonSelector), success = !1;
            ...
            closeSettings: function (destroy) {
                ...
                var $menu = this.$el.find(this.settingsMenuSelector), $button = this.$el.find(this.settingsButtonSelector);
            ...
            init: function () {
                ...
                $(".wpb_edit_form_elements .textarea_html").each(function () {
                    ...
                    var $dropdown = self.$el.find(self.settingsDropdownSelector), $prompt = $dropdown.find(".vc_ui-prompt");
                    ...
                    var $this = $(this), shortcode_name = _this.model.get("shortcode");
                    $this.data("vcSettingsMenuLoaded") && shortcode_name === $this.data("vcShortcodeName") || _this.reloadSettingsMenuContent()
                }), this.panelInit = !0
            },
            initParams: function () {
                var _this = this, $content = this.content().find('#vc_edit-form-tabs [data-vc-ui-element="panel-edit-element-tab"]:eq(' + this.active_tab_index + ")");
            ...
            hookDependent: function (e) {
                ...
                return $master = $(e.currentTarget), $master_container = $master.closest(".vc_column"), dependent_elements = this.dependent_elements[$master.attr("name")], master_value = $master.is(":checkbox") ? _.map(this.$content.find("[name=" + $(e.currentTarget).attr("name") + "].wpb_vc_param_value:checked"), function (element) {
                    ...
                    var param_name = $element.attr("name"), rules = _.isObject(this.mapped_params[param_name]) && _.isObject(this.mapped_params[param_name].dependency) ? this.mapped_params[param_name].dependency : {}, $param_block = $element.closest(".vc_column");
            ...
            getParamsForSettingsPreset: function () {
                var shortcode = this.model.get("shortcode"), params = this.getParams();
            ...
            save: function () {
                if (this.panelInit) {
                    var shortcode = this.model.get("shortcode"), params = this.getParams(), mergedParams = _.extend({}, vc.getDefaults(shortcode), vc.getMergedParams(shortcode, params));
                    _.isUndefined(params.content) || (mergedParams.content = params.content), this.model.save({params: mergedParams}), this.showMessage(window.sprintf(window.i18nLocale.inline_element_saved, vc.getMapped(shortcode).name), "success"), !vc.frame_window && this.hide(), this.trigger("save")
                }
            },
            show: function () {
            ...
            events: {
                ...
                e && e.preventDefault();
                var $button = $(e.currentTarget), template_name = $button.closest('[data-vc-ui-element="template-title"]').text(), answer = confirm(window.i18nLocale.confirm_deleting_template.replace("{template_name}", template_name));
            ...
            ajaxData: function ($button) {
                ...
                $.ajax({
                    ...
                    }), vc.storage.append(shortcodes), vc.shortcodes.fetch({reset: !0}), vc.closeActivePanel()
                })
            }, loadDefaultTemplate: function (e) {
                e.preventDefault();
                var $button = $(e.currentTarget);
                $.ajax({
                    ...
                    _.each(vc.filters.templates, function (callback) {
                        shortcodes = callback(shortcodes)
                    }), vc.storage.append(shortcodes), vc.shortcodes.fetch({reset: !0})
                })
            }, getPostContent: function () {
            ...
            saveTemplate: function (e) {
                ...
                var data, shortcodes, name = this.$name.val(), _this = this;
            ...
            removeTemplate: function (e) {
                ...
                var $button = $(e.target), $template = $button.closest("[data-template_id]"), template_name = $template.find('[data-vc-ui-element="template-title"]').text(), answer = confirm(window.i18nLocale.confirm_deleting_template.replace("{template_name}", template_name));
                if (answer) {
            ...
            reloadTemplateList: function (data, successCallback, errorCallback) {
                var _this = this;
                $.ajax({type: "POST", url: window.ajaxurl, data: data, context: this}).done(function (html) {
            ...
            buildTemplatePreview: function (e) {
                e && e.preventDefault && e.preventDefault();
                try {
                    var url, $el = $(e.currentTarget), $wrapper = $el.closest("[data-template_unique_id]");
                    if ($wrapper.hasClass("vc_active") || $wrapper.hasClass("vc_loading"))$el.vcAccordion("collapseTemplate"); else {
                        var $localContent = $wrapper.find("[data-js-content]"), localContentChilds = $localContent.children().length > 0;
                        if (this.$content = $localContent, this.$content.find("iframe").length)return $el.vcAccordion("collapseTemplate"), !0;
                        var templateId = $wrapper.data("template_unique_id"), templateType = $wrapper.data("template_type");
            ...
            events: {
            ...
            }, _builder: !1, render: function (model) {
                ...
                var $control = $(e.currentTarget), layout = $control.attr("data-cells"), columns = this.model.view.convertRowColumns(layout, this.builder());
                ...
                var b, num, denom, return_cells = [], split = cells.replace(/\s/g, "").split("+"), sum = _.reduce(_.map(split, function (c) {
            ...
            builder: function () {
                ...
                e && e.preventDefault();
                var $control = $(e.currentTarget), layout = $control.attr("data-cells"), columns = this.model.view.convertRowColumns(layout);
                this.$input.val(columns.join(" + "))
            }
        }), $(window).on("orientationchange", function () {
            vc.active_panel && vc.active_panel.$el.css({top: "", left: "auto", height: "auto", width: "auto"})
            ...
            var _this = $(this), $target = $(_this.data("vcDisableEmpty"));
        ...
        vc.HelperPanelViewHeaderFooter = {
            ...
            uiEvents: {save: "setButtonMessage", render: "clearButtonMessage"},
            resetMinimize: function () {
                this.$el.removeClass("vc_panel-opacity"), this.$el.removeClass("vc_minimized")
            },
            toggleOpacity: function (e) {
                e.preventDefault();
                var animationClass = "vc_animating", minimizedClass = "vc_minimized", _this = this, $target = this.$el, $panel = $target.find($target.data("vcPanel")), $panelContainer = $panel.closest($panel.data("vcPanelContainer")), timing = 400, $trigger = $(e.currentTarget);
                "undefined" == typeof $target.data("vcHasHeight") && $target.data("vcHasHeight", function ($element, property) {
                    var styles = $element.attr("style"), hasStyle = !1;
                    return styles && styles.split(";").forEach(function (e) {
                        var style = e.split(":");
                        $.trim(style[0]) === property && (hasStyle = !0)
                    }), hasStyle
                }($target, "height")), $target.hasClass(minimizedClass) ? ("undefined" == typeof $target.data("vcMinimizeHeight") && $target.data("vcMinimizeHeight", $(window).height() - .2 * $(window).height()), $target.animate({height: $target.data("vcMinimizeHeight")}, {
                    duration: timing,
                    start: function () {
                        $trigger.prop("disabled", !0), $target.addClass(animationClass), _this.tabsMenu && _this.tabsMenu() && _this.tabsMenu().vcTabsLine("moveTabs")
                    },
                    complete: function () {
                        $target.removeClass(minimizedClass), $target.removeClass(animationClass), $target.data("vcHasHeight") || $target.css({height: ""}), _this.trigger("afterUnminimize"), $trigger.prop("disabled", !1)
                    }
                })) : ($target.data("vcMinimizeHeight", $target.height()), $target.animate({height: $panel.outerHeight() + $panelContainer.outerHeight() - $panelContainer.height()}, {
                    duration: timing, start: function () {
                        $trigger.prop("disabled", !0), $target.addClass(animationClass)
                    }, complete: function () {
                        $target.addClass(minimizedClass), $target.removeClass(animationClass), $target.css({height: ""}), _this.trigger("afterMinimize"),
        ...
        vc.HelperPanelViewDraggable = {
            draggable: !0,
            draggableOptions: {iframeFix: !0, handle: '[data-vc-ui-element="panel-heading"]'},
            uiEvents: {show: "initDraggable"},
            initDraggable: function () {
        ...
        vc.TemplateWindowUIPanelBackendEditor = vc.TemplatesPanelViewBackend.vcExtendUI(vc.HelperPanelViewHeaderFooter).vcExtendUI(vc.HelperTemplatesPanelViewSearch).vcExtendUI(vc.HelperPanelViewResizable).vcExtendUI(vc.HelperPanelViewDraggable).extend({
            ...
            if (i = 0, $allTriggers = this.getContainer().find("[data-vc-preview-handler]").each(function () {
                    ...
                    return $this[0] !== this
                }), $triggers.length && $.fn.vcAccordion.call($triggers, "hide"), this.isActive())$.fn.vcAccordion.call($this, "hide"); else {
                $.fn.vcAccordion.call($this, "show");
                var $triggerPanel = $this.closest(".vc_ui-list-bar-item"), $wrapper = $this.closest("[data-template_unique_id]"), $panel = $wrapper.closest("[data-vc-ui-element=panel-content]").parent();
                setTimeout(function () {
        ...
        vc.element_start_index = 0, vc.AddElementUIPanelBackendEditor = vc.PanelView.vcExtendUI(vc.HelperPanelViewHeaderFooter).extend({
            ...
            filterElements: function (e) {
                ...
                var filterValue, $visibleElements, $control = $(e.currentTarget), filter = '[data-vc-ui-element="add-element-button"]', nameFilter = $(this.searchSelector).val();
            ...
            createElement: function (e) {
                ...
                row_params = {}, "undefined" != typeof window.vc_settings_presets.vc_row && (row_params = _.extend(row_params, window.vc_settings_presets.vc_row)), column_params = {width: "1/1"}, "undefined" != typeof window.vc_settings_presets.vc_column && (column_params = _.extend(column_params, window.vc_settings_presets.vc_column)), !1 === this.model ? (vc.storage.lock(), row = vc.shortcodes.create({
                    shortcode: "vc_row",
                    params: row_params
                }), column = vc.shortcodes.create({
                    shortcode: "vc_column",
                    ...
                    parent_id: column.id,
                    root_id: row.id
                }) : row) : "vc_row" === tag ? (row_params = {}, "undefined" != typeof window.vc_settings_presets.vc_row_inner && (row_params = _.extend(row_params, window.vc_settings_presets.vc_row_inner)), column_params = {width: "1/1"}, "undefined" != typeof window.vc_settings_presets.vc_column_inner && (column_params = _.extend(column_params, window.vc_settings_presets.vc_column_inner)), vc.storage.lock(), row = vc.shortcodes.create({
                    shortcode: "vc_row_inner",
                    params: row_params,
                    parent_id: this.model.id,
                    order: this.prepend ? this.getFirstPositionIndex() : vc.shortcodes.getNextOrder()
                    ...
                    order: this.prepend ? this.getFirstPositionIndex() : vc.shortcodes.getNextOrder(),
                    root_id: this.model.get("root_id")
                }), this.model = model, showSettings = !(_.isBoolean(vc.getMapped(tag).show_settings_on_create) && !1 === vc.getMapped(tag).show_settings_on_create), shortcode = this.model.get("shortcode"), "undefined" != typeof window.vc_settings_presets[shortcode] && this.model.save({params: _.extend({}, this.model.attributes.params, window.vc_settings_presets[shortcode])}), this.hide(), showSettings && this.showEditForm()
            },
            getFirstPositionIndex: function () {
                return vc.element_start_index -= 1, vc.element_start_index
            },
        ...
        vc.PostSettingsUIPanelFrontendEditor = vc.PostSettingsPanelView.vcExtendUI(vc.HelperPanelViewHeaderFooter).vcExtendUI(vc.HelperPanelViewResizable).vcExtendUI(vc.HelperPanelViewDraggable).vcExtendUI({
            ...
            uiEvents: {setSize: "setEditorSize", show: "setEditorSize"},
            setSize: function () {
                this.trigger("setSize")
            },
            setDefaultHeightSettings: function () {
...
if (_.extend(vc, {
        ...
        vc.map = {}, vc.setFrameSize = function (size) {
            var $vc_navbar = $("#vc_navbar"), height = $(window).height() - $vc_navbar.height();
            vc.$frame.width(size), vc.$frame_wrapper.css({top: $vc_navbar.height()}), vc.$frame.height(height)
            ...
            }), {defaults: defaults, dependencyMap: dependencyMap}
            ...
            return newOrder = _.isBoolean(child_of_clone) && !0 === child_of_clone ? model.get("order") : parseFloat(model.get("order")) + vc.clone_index, params = _.extend({}, model.get("params")), tag = model.get("shortcode"), data = {
                ...
                params: params
            }, vc["cloneMethod_" + tag] && (data = vc["cloneMethod_" + tag](data, model)), _.isBoolean(child_of_clone) && !0 === child_of_clone || (data.place_after_id = model.get("id")), builder.create(data), newModel = builder.last(), _.each(vc.shortcodes.where({parent_id: model.get("id")}), function (shortcode) {
                vc.CloneModel(builder, shortcode, newModel.get("id"), !0)
            }, this), newModel
        }, vc.getColumnSize = function (column) {
            var mod = 12 % column, is_odd = function (n) {
            ...
            checkControlsPosition: function () {
                if (this.$controls_buttons) {
                    var window_top, control_top, element_position_top, new_position, element_height = this.$el.height(), frame_height = vc.$frame.height();
            ...
            addControls: function () {
                ...
                var compiledTemplate = vc.template(template, _.extend({}, vc.templateOptions.custom, {evaluate: /\{#([\s\S]+?)#}/g}));
            ...
            editCloned: function (e) {
                e && e.preventDefault();
                var $control = $(e.currentTarget), model_id = $control.data("modelId"), model = vc.shortcodes.get(model_id);
            ...
            addShortcodes: function (models) {
                _.each(models, function (model) {
                    this.addShortcode(model), this.addShortcodes(vc.shortcodes.where({parent_id: model.get("id")}))
                }, this)
            },
            createShortcodeHtml: function (model) {
                var $template = $("#vc_template-" + model.get("shortcode")), template = $template.length ? $template.html() : '<div class="vc_block"></div>', compiledTemplate = vc.template(template, vc.templateOptions.custom);
                return $(compiledTemplate(model.toJSON()).trim())
            },
            addAll: function (models) {
                this.addShortcodes(models.where({parent_id: !1}))
            },
            createRow: function (e) {
                _.isObject(e) && e.preventDefault();
                var row_params, column_params, builder = new vc.ShortcodesBuilder;
                row_params = {}, "undefined" != typeof window.vc_settings_presets.vc_row && (row_params = _.extend(row_params, window.vc_settings_presets.vc_row)), column_params = {width: "1/1"}, "undefined" != typeof window.vc_settings_presets.vc_column && (column_params = _.extend(column_params, window.vc_settings_presets.vc_column)), builder.create({
                    shortcode: "vc_row",
                    params: row_params
                }).create({shortcode: "vc_column", parent_id: builder.lastID(), params: column_params}).render()
            },
            addElement: function (e) {
            ...
            saveRowOrder: function () {
                _.defer(function (app) {
                    var row_params, column_params, $rows = vc.$page.find("> [data-tag=vc_row]"), builder = new vc.ShortcodesBuilder;
                    $rows.each(function (key, value) {
                        var $this = $(this);
                        if ($this.is(".droppable")) {
                            row_params = {}, "undefined" != typeof window.vc_settings_presets.vc_row && (row_params = _.extend(row_params, window.vc_settings_presets.vc_row)), column_params = {width: "1/1"}, "undefined" != typeof window.vc_settings_presets.vc_column && (column_params = _.extend(column_params, window.vc_settings_presets.vc_column)), $this.remove();
                            var row_data = {shortcode: "vc_row", params: row_params, order: key};
                        ...
                        } else vc.shortcodes.get($this.data("modelId")).save({order: key}, {silent: !0})
                    }), vc.setDataChanged()
                }, this)
            },
            saveElementOrder: function (event, ui) {
                _.defer(function (app, e, ui) {
                    if (_.isNull(ui.sender)) {
                        var $column = ui.item.parent(), $elements = $column.find("> [data-model-id]");
                        $column.find("> [data-model-id]").each(function (key, value) {
                            var model, prev_parent, current_parent, $element = $(this), prepend = !1;
                            ...
                            }, {silent: !0}), prev_parent !== current_parent && (vc.builder.notifyParent(current_parent), vc.builder.notifyParent(prev_parent)))
                        })
                    }
                    vc.setDataChanged()
                }, this, event, ui)
        ...
        function TTaMapChildEvents(model) {
            ...
            vc.events.on("shortcodes:" + childTag + ":add:parent:" + model.get("id"), function (model) {
                ...
                return parentModel = vc.shortcodes.get(model.get("parent_id")), activeTabIndex = parseInt(parentModel.getParam("active_section")), "undefined" == typeof activeTabIndex && (activeTabIndex = 1), models = _.pluck(_.sortBy(vc.shortcodes.where({parent_id: parentModel.get("id")}), function (model) {
                    return model.get("order")
        ...
        _.isUndefined(window.vc) && (window.vc = {}), window.InlineShortcodeViewContainer = window.InlineShortcodeView.extend({
            ...
            multi_edit: function (e) {
                ...
                _.isObject(e) && e.preventDefault(), this.model.get("parent_id") && (parent = vc.shortcodes.get(this.model.get("parent_id"))), parent ? (models.push(parent), children = vc.shortcodes.where({parent_id: parent.get("id")}), vc.multi_edit_element_block_view.render(models.concat(children), this.model.get("id"))) : vc.edit_element_block_view.render(this.model)
            },
            ...
            switchControls: function (e) {
                ...
                var $current, $control = $(e.currentTarget), $parent = $control.parent();
            ...
            initialize: function (options) {
                ...
                params.content = tinyMCE.activeEditor.getContent(), this.model.save({params: params}, {silent: !0})
            }
        }), window.InlineShortcodeView_vc_row = window.InlineShortcodeView.extend({
            column_tag: "vc_column",
            events: {mouseenter: "removeHoldActive"},
            layout: 1,
            addControls: function () {
            ...
            addColumn: function () {
                vc.builder.create({shortcode: this.column_tag, parent_id: this.model.get("id")}).render()
            },
            addElement: function (e) {
            ...
            layoutEditor: function () {
                return _.isUndefined(vc.row_layout_editor) && (vc.row_layout_editor = new vc.RowLayoutUIPanelFrontendEditor({el: $("#vc_ui-panel-row-layout")})), vc.row_layout_editor
            },
            ...
            addLayoutClass: function () {
                this.$el.removeClass("vc_layout_" + this.layout), this.layout = _.reject(vc.shortcodes.where({parent_id: this.model.get("id")}), function (model) {
                    return model.get("deleted")
                }).length, this.$el.addClass("vc_layout_" + this.layout)
            },
            convertRowColumns: function (layout, builder) {
                if (!layout)return !1;
                var column_params, new_model, columns_contents = [], columns = this.convertToWidthsArray(layout);
                return vc.layout_change_shortcodes = [], vc.layout_old_columns = vc.shortcodes.where({parent_id: this.model.get("id")}), _.each(vc.layout_old_columns, function (column) {
                    ...
                    _.isObject(prev_settings) ? (new_model = builder.create({
                        ...
                        shortcode.save({
                            ...
                            order: vc.shortcodes.nextOrder()
                        }, {silent: !0}), vc.layout_change_shortcodes.push(shortcode)
                    }, this)) : (column_params = {width: column}, "undefined" != typeof window.vc_settings_presets[this.column_tag] && (column_params = _.extend(column_params, window.vc_settings_presets[this.column_tag])), new_model = builder.create({
                        shortcode: this.column_tag,
                        parent_id: this.model.get("id"),
                        order: vc.shortcodes.nextOrder(),
                        params: column_params
                    ...
                    _.each(column.shortcodes, function (shortcode) {
                        shortcode.save({
                            parent_id: new_model.get("id"),
                            order: vc.shortcodes.nextOrder()
                        }, {silent: !0}), vc.layout_change_shortcodes.push(shortcode), shortcode.view.rowsColumnsConverted && shortcode.view.rowsColumnsConverted()
                    }, this)
            ...
            destroy: function (e) {
                ...
                window.InlineShortcodeView_vc_column.__super__.destroy.call(this, e), vc.shortcodes.where({parent_id: parent_id}).length || vc.shortcodes.get(parent_id).destroy()
            },
            ...
            setColumnClasses: function () {
                var offset = this.getParam("offset") || "", width = this.getParam("width") || "1/1", $content = this.$el.find("> .wpb_column");
        ...
        }), window.InlineShortcodeView_vc_row_inner = window.InlineShortcodeView_vc_row.extend({column_tag: "vc_column_inner"}), window.InlineShortcodeView_vc_column_inner = window.InlineShortcodeView_vc_column.extend({}), window.InlineShortcodeView_vc_tabs = window.InlineShortcodeView_vc_row.extend({
            events: {
                "click > :first > .vc_empty-element": "addElement",
                "click > :first > .wpb_wrapper > .ui-tabs-nav > li": "setActiveTab"
            }, already_build: !1, active_model_id: !1, $tabsNav: !1, active: 0, render: function () {
                ...
                if (1 === vc.shortcodes.where({parent_id: this.model.get("id")}).length)return this.model.destroy();
            ...
            clone: function (e) {
                ...
                var clone = this.model.clone(), builder = (clone.get("params"), new vc.ShortcodesBuilder), newmodel = vc.CloneModel(builder, this.model, this.model.get("parent_id")), that = (this.parent_view.active_model_id, this);
            ...
            events: {"click > .wpb_accordion > .vc_empty-element": "addElement"},
            render: function () {
            ...
            stopSorting: function () {
                this.$accordion.find("> .wpb_accordion_wrapper > .vc_element[data-tag]").each(function () {
                    ...
                    model.save({order: $(this).index()}, {silent: !0})
                })
            },
            addElement: function (e) {
                e && e.preventDefault(), (new vc.ShortcodesBuilder).create({
            ...
            events: {
                ...
                var parent_id = this.model.get("parent_id");
                window.InlineShortcodeView_vc_accordion_tab.__super__.destroy.call(this, e), vc.shortcodes.where({parent_id: parent_id}).length || vc.shortcodes.get(parent_id).destroy()
            }, allowAddControl: function () {
            ...
            addSection: function (prepend) {
                ...
                for (shortcode = this.childTag, params = {
                    ...
                    parent_id: this.model.get("id"),
                    isActiveSection: !0,
                    params: {title: this.defaultSectionTitle}
            ...
            getSiblingsFirstPositionIndex: function () {
                ...
                return order = 0, shortcodeFirst = vc.shortcodes.sort().findWhere({parent_id: this.model.get("id")}), shortcodeFirst && (order = shortcodeFirst.get("order") - 1), order
            },
            changed: function () {
            ...
            buildSortable: function () {
                this.$el && this.$el.find(".vc_tta-panels").sortable({
                    ...
                    over: function (event, ui) {
                        ui.placeholder.css({maxWidth: ui.placeholder.parent().width()}), ui.placeholder.removeClass("vc_hidden-placeholder")
                    },
                    items: "> .vc_element",
            ...
            updateSorting: function (event, ui) {
                ...
                this.getPanelsList().find("> .vc_element").each(function () {
                    ...
                    $this = $(this), modelId = $this.data("modelId"), shortcode = vc.shortcodes.get(modelId), shortcode.save({order: self.getIndex($this)}, {silent: !0})
            ...
            render: function () {
                ...
                var models = _.sortBy(vc.shortcodes.where({parent_id: this.model.get("id")}), function (model) {
                    return model.get("order")
                });
                _.each(models, function (model) {
                    this.sectionUpdated(model, !0)
                }, this)
            }, defaultSectionTitle: window.i18nLocale.tab, addIcon: function (model, html) {
                ...
                if (tabAdded = !1, sectionId = model.get("id"), $navigation = this.$el.find(".vc_tta-tabs-container .vc_tta-tabs-list"), $tabEl = $navigation.find('[data-vc-target="[data-model-id=' + sectionId + ']"]'), title = model.getParam("title"), $tabEl.length)html = '<span class="vc_tta-title-text">' + title + "</span>", html = this.addIcon(model, html), $tabEl.html(html); else {
                    var $element;
                    html = '<span class="vc_tta-title-text">' + title + "</span>", html = this.addIcon(model, html), $element = $('<li class="vc_tta-tab" data-vc-target-model-id="' + sectionId + '" data-vc-tab><a href="javascript:;" data-vc-use-cache="false" data-vc-tabs data-vc-target="[data-model-id=' + sectionId + ']" data-vc-container=".vc_tta">' + html + "</a></li>"), !0 !== justAppend && (models = _.pluck(_.sortBy(vc.shortcodes.where({parent_id: this.model.get("id")}), function (childModel) {
                ...
                self = this, $tabs = this.$el.find(".vc_tta-tabs-list"), $tabs.find("> .vc_tta-tab").each(function () {
                    ...
                    $li = $(this).removeAttr("style"), modelId = $li.data("vcTargetModelId"), shortcode = vc.shortcodes.get(modelId), shortcode.save({order: self.getIndex($li)}, {silent: !0})
                }), this.updatePanelsPositions($tabs)
            }, updateTabsPositions: function ($panels) {
                var $tabs, $elements, tabSortableData;
                $tabs = this.$el.find(".vc_tta-tabs-list"), $tabs.length && ($elements = [], tabSortableData = $panels.sortable("toArray", {attribute: "data-model-id"}), _.each(tabSortableData, function (value) {
                ...
                $panels = this.getPanelsList(), $elements = [], tabSortableData = $tabs.sortable("toArray", {attribute: "data-vc-target-model-id"}), _.each(tabSortableData, function (value) {
