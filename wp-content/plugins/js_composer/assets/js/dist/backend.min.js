...
!function () {
    ...
    vc.HelperPanelViewHeaderFooter = {
        ...
        toggleOpacity: function (e) {
            ...
            "undefined" == typeof $target.data("vcHasHeight") && $target.data("vcHasHeight", function ($element, property) {
                ...
                complete: function () {
                    $target.addClass(minimizedClass), $target.removeClass(animationClass), $target.css({height: ""}), _this.trigger("afterMinimize"), $trigger.prop("disabled", !1)
                }
            }))
        },
    ...
    vc.PostSettingsUIPanelFrontendEditor = vc.PostSettingsPanelView.vcExtendUI(vc.HelperPanelViewHeaderFooter).vcExtendUI(vc.HelperPanelViewResizable).vcExtendUI(vc.HelperPanelViewDraggable).vcExtendUI({
        ...
        uiEvents: {
            setSize: "setEditorSize", show: "setEditorSize", render: "removeChangeTitleField"
        }, setSize: function () {
            this.trigger("setSize")
        }, setEditorSize: function () {
            this.editor.setSizeResizable()
...
window.vc.filters = {templates: []}, window.vc.addTemplateFilter = function (callback) {
    ...
    var GoogleFonts = Backbone.View.extend({
        ...
        min_length: 2, delay: 500, auto_focus: !0, ajax_url: window.ajaxurl, source_data: function () {
            return {}
        }, replace_values_on_select: !1, initialize: function (params) {
    ...
    vc.debug = !1, vc.map = _.isUndefined(window.vc_mapper) ? {} : window.vc_mapper, vc.roles = _.isUndefined(window.vc_roles) ? {} : window.vc_roles, vc.Storage = function () {
        ...
        url: window.ajaxurl, checksum: !1, locked: !1, create: function (model) {
            ...
            var tiny = _.isObject(window.tinymce) && tinymce.editors.content && !_.isUndefined(window.switchEditors), editor_hidden = tiny && window.tinyMCE.get("content") && window.tinyMCE.get("content").isHidden();
            ...
            return _.isNull(matches) ? data : (_.each(matches, function (raw) {
                var shortcode, map_settings, sub_matches = raw.match(this.regexp(tags)), sub_content = sub_matches[5], sub_regexp = new RegExp("^[\\s]*\\[\\[?(" + _.keys(vc.map).join("|") + ")(?![\\w-])"), id = window.vc_guid(), atts_raw = window.wp.shortcode.attrs(sub_matches[3]), atts = {};
            ...
            var tags = _.keys(vc.map).join("|"), reg = this.regexp_split("vc_row"), starts_with_shortcode = new RegExp("^\\[(\\[?)(" + tags + ")", "g"), matches = _.filter(content.trim().split(reg), function (value) {
    ...
    vc.clone_index = 1, vc.shortcode_view = Backbone.View.extend({
        ...
        checkControlsPosition: function () {
            if (this.$controls_buttons) {
                var window_top, element_position_top, new_position, element_height = this.$el.height(), window_height = $(window).height();
        ...
        html2element: function (html) {
            var $template, attributes = {}, template = vc.template(html);
        ...
        render: function () {
            ...
            if ($shortcode_template_el.is("script"))this.html2element($shortcode_template_el.html()); else {
        ...
        _loadDefaults: function () {
            var tag, hasChilds;
            tag = this.model.get("shortcode"), hasChilds = !!vc.shortcodes.where({parent_id: this.model.get("id")}).length, !hasChilds && !0 === this.use_default_content && _.isObject(vc.map[tag]) && _.isString(vc.map[tag].default_content) && vc.map[tag].default_content.length && (this.use_default_content = !1, vc.shortcodes.createFromString(vc.map[tag].default_content, this.model))
        },
        ...
        changeShortcodeParent: function (model) {
            ...
            var $parent_view = $("[data-model-id=" + this.model.get("parent_id") + "]"), view = vc.app.views[this.model.get("parent_id")];
    ...
    vc.saved_custom_css = !1, vc.visualComposerView = Backbone.View.extend({
        ...
        getView: function (model) {
            var view;
            return view = _.isObject(vc.map[model.get("shortcode")]) && _.isString(vc.map[model.get("shortcode")].js_view) && vc.map[model.get("shortcode")].js_view.length && !_.isUndefined(window[window.vc.map[model.get("shortcode")].js_view]) ? new (window[window.vc.map[model.get("shortcode")].js_view])({model: model}) : new vc.shortcode_view({model: model}), model.set({view: view}), view
        },
        setDraggable: function () {
            $("#wpb-add-new-element, #wpb-add-new-row").draggable({
                helper: function () {
                    return $('<div id="drag_placeholder"></div>').appendTo("body")
                }, zIndex: 99999, cursor: "move", revert: "invalid", start: function (event, ui) {
        ...
        appendShortcode: function (model) {
            ...
            if (view = this.getView(model), params = _.extend(vc.getDefaults(model.get("shortcode")), model.get("params")), model.set("params", params, {silent: !0}), parentModelView = !1 !== model.get("parent_id") ? this.views[model.get("parent_id")] : !1, this.views[model.id] = view, model.get("parent_id")) {
                var parentView;
        ...
        addShortcode: function (model) {
            var view, parentModelView, params;
            if (params = _.extend(vc.getDefaults(model.get("shortcode")), model.get("params")),
                    model.set("params", params, {silent: !0}), view = this.getView(model), parentModelView = !1 !== model.get("parent_id") ? this.views[model.get("parent_id")] : !1, view.use_default_content = !0 !== model.get("cloned"), this.views[model.id] = view, parentModelView) {
        ...
        addTextBlock: function (e) {
            ...
            return e.preventDefault(), row_params = {}, "undefined" != typeof window.vc_settings_presets.vc_row && (row_params = _.extend(row_params, window.vc_settings_presets.vc_row)), row = vc.shortcodes.create({
                shortcode: "vc_row",
                params: row_params
            }), column_params = {width: "1/1"}, "undefined" != typeof window.vc_settings_presets.vc_column && (column_params = _.extend(column_params, window.vc_settings_presets.vc_column)), column = vc.shortcodes.create({
                shortcode: "vc_column",
                params: column_params,
                parent_id: row.id,
                root_id: row.id
        ...
        createRow: function () {
            ...
            return row_params = {}, "undefined" != typeof window.vc_settings_presets.vc_row && (row_params = _.extend(row_params, window.vc_settings_presets.vc_row)), row = vc.shortcodes.create({
                shortcode: "vc_row",
                params: row_params
            }), column_params = {width: "1/1"}, "undefined" != typeof window.vc_settings_presets.vc_column && (column_params = _.extend(column_params, window.vc_settings_presets.vc_column)), vc.shortcodes.create({
                shortcode: "vc_column",
                params: column_params,
                parent_id: row.id,
                root_id: row.id
        ...
        updateElementsSorting: function (event, ui) {
            _.defer(function (app, event, ui) {
                var $current_container = ui.item.parent().closest("[data-model-id]"), parent = $current_container.data("model"), model = ui.item.data("model"), models = app.views[parent.id].$content.find("> [data-model-id]"), i = 0;
                if (!_.isNull(ui.sender)) {
                    var old_parent_id = model.get("parent_id");
                    vc.storage.lock(), model.save({parent_id: parent.id}), app.views[old_parent_id].checkIsEmpty(), app.views[parent.id].checkIsEmpty()
                }
                models.each(function () {
                    var shortcode = $(this).data("model");
                    vc.storage.lock(), shortcode.save({order: i++})
                }), model.save()
            }, this, event, ui)
        },
        updateRowsSorting: function () {
            _.defer(function (app) {
                ...
                $rows.each(function () {
                    ...
                    $rows.length - 1 > index && vc.storage.lock(), $(this).data("model").save({order: index})
                })
            }, this)
        },
        renderPlaceholder: function (event, element) {
            var tag = $(element).data("element_type"), is_container = _.isObject(vc.map[tag]) && (_.isBoolean(vc.map[tag].is_container) && !0 === vc.map[tag].is_container || !_.isEmpty(vc.map[tag].as_parent)), $helper = $('<div class="vc_helper vc_helper-' + tag + '"><i class="vc_general vc_element-icon' + (vc.map[tag].icon ? " " + vc.map[tag].icon : "") + '"' + (is_container ? ' data-is-container="true"' : "") + "></i> " + vc.map[tag].name + "</div>").prependTo("body");
            return $helper
        },
        ...
        setSortable: function () {
            return $(".wpb_main_sortable").sortable({
                ...
                stop: function (event, ui) {
                    ...
                    var tag = ui.item.data("element_type"), parent_tag = ui.item.parent().closest("[data-element_type]").data("element_type"), allowed_container_element = _.isUndefined(vc.map[parent_tag].allowed_container_element) ? !0 : vc.map[parent_tag].allowed_container_element;
        ...
        show: function () {
            this.$el.show(), this.$post.hide(), this.$vcStatus.val("true"), this.navOnScroll(), vc.storage.isContentChanged() && (vc.app.setLoading(), vc.app.views = {}, _.defer(function () {
                vc.shortcodes.fetch({reset: !0}), vc.events.trigger("backendEditor.show")
            }))
        },
        setLoading: function () {
        ...
        $wpbVisualComposer = $("#wpb_visual_composer"), $wpbVisualComposer.is("div") && (vc.app = new vc.visualComposerView({el: $wpbVisualComposer}), vc.accessPolicy.can("be_editor") ? vc.app.checkVcStatus() : vc.app.$el.remove(), vc.events.on("vc:backend_editor:show", function () {
    ...
    window.VcRowView = vc.shortcode_view.extend({
        ...
        convertRowColumns: function (layout) {
            var layout_split = layout.toString().split(/_/), columns = Shortcodes.where({parent_id: this.model.id}), new_columns = [], new_layout = [], new_width = "";
            return _.each(layout_split, function (value, i) {
                ...
                new_width = 3 < column_data.length ? column_data[0] + "" + column_data[1] + "/" + column_data[2] + column_data[3] : 2 < column_data.length ? column_data[0] + "/" + column_data[1] + column_data[2] : column_data[0] + "/" + column_data[1], new_layout.push(new_width), new_column_params = _.extend(_.isUndefined(columns[i]) ? {} : columns[i].get("params"), {width: new_width}), "undefined" != typeof window.vc_settings_presets[this.getChildTag()] && (new_column_params = _.extend(new_column_params, window.vc_settings_presets[this.getChildTag()])), vc.storage.lock(), new_column = Shortcodes.create({
                    shortcode: this.getChildTag(),
                    params: new_column_params,
                    parent_id: this.model.id
                }), _.isObject(columns[i]) && _.each(Shortcodes.where({parent_id: columns[i].id}), function (shortcode) {
                    vc.storage.lock(), shortcode.save({parent_id: new_column.id}), vc.storage.lock(), shortcode.trigger("change_parent_id")
                ...
                _.each(Shortcodes.where({parent_id: column.id}), function (shortcode) {
                    vc.storage.lock(), shortcode.save({parent_id: _.last(new_columns).id}), vc.storage.lock(), shortcode.trigger("change_parent_id")
                })
        ...
        setRowClasses: function () {
            var disable = this.model.getParam("disable_element"), disableClass = "vc_hidden-xs vc_hidden-sm  vc_hidden-md vc_hidden-lg";
            ...
            1 < this.$content.find(this.sortingSelector).length ? this.$content.removeClass("wpb-not-sortable").sortable({
                ...
                update: function () {
                    ...
                    $columns.each(function () {
                        var model = $(this).data("model"), index = $(this).index();
        ...
        validateCellsList: function (cells) {
            var b, return_cells = [], split = cells.replace(/\s/g, "").split("+"), sum = _.reduce(_.map(split, function (c) {
        ...
        setActiveLayoutButton: function (column_layout) {
            column_layout || (column_layout = _.map(vc.shortcodes.where({parent_id: this.model.get("id")}), function (model) {
                var width = model.getParam("width");
        ...
        layoutEditor: function () {
            return _.isUndefined(vc.row_layout_editor) && (vc.row_layout_editor = new vc.RowLayoutUIPanelBackendEditor({el: $("#vc_ui-panel-row-layout")})), vc.row_layout_editor
        },
        setColumns: function (e) {
            _.isObject(e) && e.preventDefault();
            var $button = $(e.currentTarget);
            if ("custom" === $button.data("cells"))this.layoutEditor().render(this.model).show(); else {
                if (vc.is_mobile) {
        ...
        events: {
        ...
        }, current_column_width: !1, initialize: function (options) {
        ...
        }, designHelpersSelector: "> .vc_controls .column_add", buildDesignHelpers: function () {
            var matches, image, color, css = this.model.getParam("css"), $column_toggle = this.$el.find(this.designHelpersSelector).get(0);
            ...
            var current_css_class_width, offset = this.model.getParam("offset") || "", width = this.model.getParam("width") || "1/1", css_class_width = this.convertSize(width);
            ...
            ui.draggable.is("#wpb-add-new-element") ? new vc.element_block_view({model: {position_to_add: "end"}}).show(this) : ui.draggable.is("#wpb-add-new-row") && this.createRow()
        }, setEmpty: function () {
            ...
            Shortcodes.where({parent_id: this.model.id}).length ? this.unsetEmpty() : this.setEmpty(), window.VcColumnView.__super__.checkIsEmpty.call(this)
        }, createRow: function () {
            var row_params, column_params, row;
            return row_params = {}, "undefined" != typeof window.vc_settings_presets.vc_row_inner && (row_params = _.extend(row_params, window.vc_settings_presets.vc_row_inner)), column_params = {width: "1/1"}, "undefined" != typeof window.vc_settings_presets.vc_column_inner && (column_params = _.extend(column_params, window.vc_settings_presets.vc_column_inner)), row = Shortcodes.create({
                shortcode: "vc_row_inner",
                params: row_params,
                parent_id: this.model.id
            }), Shortcodes.create({shortcode: "vc_column_inner", params: column_params, parent_id: row.id}), row
            ...
            return window.VcAccordionView.__super__.render.call(this), vc_user_access().shortcodeAll("vc_accordion_tab") ? (this.$content.sortable({
                ...
                stop: function (event, ui) {
                    ui.item.prev().triggerHandler("focusout"), $(this).find("> .wpb_sortable").each(function () {
                        var shortcode = $(this).data("model");
                        shortcode.save({order: $(this).index()})
                    })
                }
        ...
        events: function () {
            return _.extend({"click button": "buttonClick"}, window.VcToggleView.__super__.events)
        ...
        events: function () {
            ...
            if (window.VcButton2View.__super__.changeShortcodeParams.call(this, model), params = model.get("params"), _.isObject(params)) {
                var el_class;
                el_class = (params.color ? "vc_btn_" + params.color : "") + " " + (params.color ? "vc_btn-" + params.color : "") + " " + (params.size ? "vc_btn-" + params.size : "") + " " + (params.size ? "vc_btn_" + params.size : "") + " " + (params.style ? "vc_btn_" + params.style : ""), this.$el.find(".wpb_element_wrapper").removeClass(el_class), this.$el.find("button.title").attr({"class": "title textfield vc_btn  " + el_class})
            }
        }
        ...
        $wrapper: !1,
        events: function () {
            return _.extend({"click .vc_btn3": "buttonClick"}, window.VcToggleView.__super__.events)
        },
        buttonClick: function (e) {
            e.preventDefault()
        },
        ...
        updateSorting: function (event, ui) {
            ...
            return vc_user_access().shortcodeAll("vc_tta_section") ? (self = this, this.$sortable.find(this.sortableSelector).each(function () {
                ...
                $this = $(this), modelId = $this.attr(self.sortableUpdateModelIdSelector), shortcode = vc.shortcodes.get(modelId), vc.storage.lock(), shortcode.save({order: self.getIndex($this)})
    ...
    }), window.VcBackendTtaTourView = window.VcBackendTtaTabsView.extend({defaultSectionTitle: window.i18nLocale.section}), window.VcBackendTtaPageableView = window.VcBackendTtaTabsView.extend({defaultSectionTitle: window.i18nLocale.section}), window.VcBackendTtaSectionView = window.VcColumnView.extend({
        parentObj: null,
        events: {
        ...
        replaceTemplateVars: function () {
            ...
            $panelHeading.html(template({model_id: this.model.get("id"), section_title: title}))
        },
        getIndex: function () {
    ...
    attachmentCompatRender = _.extend(media.view.AttachmentCompat.prototype.render), media.view.AttachmentCompat.prototype.render = function () {
        ...
        $vc_editor_element: null, getData: function () {
