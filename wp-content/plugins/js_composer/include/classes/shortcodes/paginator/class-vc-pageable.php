...
abstract class WPBakeryShortCode_Vc_Pageable extends WPBakeryShortCode {
    ...
    public function shortcodeScripts() {
        wp_register_script('vc_pageable_owl-carousel', vc_asset_url('lib/owl-carousel2-dist/owl.carousel.min.js'), [
            'jquery',
        ], WPB_VC_VERSION, TRUE);
        wp_register_script('waypoints', vc_asset_url('lib/waypoints/waypoints.min.js'), ['jquery'], WPB_VC_VERSION,
                           TRUE);
        wp_register_style('vc_pageable_owl-carousel-css', vc_asset_url('lib/owl-carousel2-dist/assets/owl.min.css'), [],
    ...
    public function renderPagination($grid_style, $settings, $content = '', $css_class = '') {
        ...
        $content        = method_exists($this, $content_method) ? $this->$content_method($grid_style, $settings,
                                                                                         $content) : $content;
    ...
    protected function contentLoadMore($grid_style, $settings, $content) {
        if (!isset($settings['btn_data']) && isset($settings['button_style']) && isset($settings['button_size']) && isset($settings['button_color'])) {
            ...
            return '<div class="vc_pageable-slide-wrapper vc_clearfix" data-vc-grid-content="true">'
                   ...
                   . do_shortcode('[vc_button2 size="' . $settings['button_size'] . '" title="' . __('Load more',
                                                                                                     'js_composer') . '" style="' . $settings['button_style'] . '" color="' . $settings['button_color'] . '" el_class="vc_grid-btn-load_more"]')
                   . '</div>';
            ...
            return '<div class="vc_pageable-slide-wrapper vc_clearfix" data-vc-grid-content="true">'
                   ...
                   . '</div>'
                   . '<div class="vc_pageable-load-more-btn" data-vc-grid-load-more-btn="true">'
                   . apply_filters('vc_gitem_template_attribute_' . 'vc_btn', '', [
                'post' => new stdClass(),
                'data' => str_replace(['{{ vc_btn:', '}}'], '', $button3->output($data)),
            ])
                   . '</div>';
        }
