...
function vc_gitem_template_attribute_acf($value, $data) {
    ...
    if (strstr($data, 'field_from_group_')) {
        ...
        $fields   = function_exists('acf_get_fields') ? acf_get_fields($group_id)
            : apply_filters('acf/field_group/get_fields', [], $group_id);
    ...
    if (preg_match('/_labeled$/', $data)) {
        ...
        $label = is_array($field) && isset($field['label'])
            ? '<span class="vc_gitem-acf-label">' . $field['label'] . ':</span> ' : '';
    }
    $value = '';
    if ($data) {
