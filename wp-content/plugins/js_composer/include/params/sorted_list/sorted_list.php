...
class Vc_Teaser_Box {
    ...
    public function init() {
        add_action('admin_init', [&$this, 'jsComposerEditPage'], 6);
    }
    ...
    public function jsComposerEditPage() {
        ...
        foreach ($pt_array as $pt) {
            add_meta_box('vc_teaser', __('VC: Custom Teaser', 'js_composer'), [
                &$this,
                'outputTeaser',
            ], $pt, 'side');
        }
    ...
    public function outputTeaser() {
        ...
        wp_localize_script('wpb_jscomposer_teaser_js', 'i18nVcTeaser', [
        ...
        $output
            = '<div class="vc_teaser-switch"><label><input type="checkbox" name="' . self::$meta_data_name . '[enable]" value="1" id="vc_teaser-checkbox"' . ('1' === $this->getTeaserData('enable')
                ? ' checked="true"' : '') . '> ' . __('Enable custom teaser', 'js_composer') . '</label></div>';
        ...
        $output .= '<p>' . __('Customize teaser block design to overwrite default settings used in "Carousel" content element.',
                              'js_composer') . '</p>';
        $output .= '</div>';
...
function vc_sorted_list_parts_list($list) {
    ...
    foreach ($list as $control) {
        $output .= '<div class="vc_sorted-list-checkbox"><label><input type="checkbox" name="vc_sorted_list_element" value="' . $control[0] . '" data-element="' . $control[0] . '" data-subcontrol="' . (count($control) > 1
                ? htmlspecialchars(json_encode(array_slice($control, 2)))
                : '') . '"> <span>' . htmlspecialchars($control[1]) . '</span></label></div>';
    }
...
function vc_sorted_list_parse_value($value) {
    ...
    foreach ($split as $v) {
        ...
        if (count($v_split) > 0) {
            $data[] = [$v_split[0], count($v_split) > 1 ? array_slice($v_split, 1) : []];
        }
    }
