...
if (!class_exists('WooCommerce')) :
	...
	final class WooCommerce {
		...
		private function init_hooks() {
			register_activation_hook(__FILE__, ['WC_Install', 'install']);
			...
			add_action('init', [$this, 'wpdb_table_fix'], 0);
		...
		public function init() {
			...
			$this->product_factory
				                = new WC_Product_Factory();                      // Product Factory to create new product instances
			$this->order_factory
				                = new WC_Order_Factory();                        // Order Factory to create new order instances
			...
			if ($this->is_request('frontend')) {
				...
				$this->customer
				            = new WC_Customer();                                // Customer class, handles data such as customer location
			}
			$this->load_webhooks();
			// Init action.
		...
		private function add_image_sizes() {
			...
			add_image_size('shop_thumbnail', $shop_thumbnail['width'], $shop_thumbnail['height'],
			               $shop_thumbnail['crop']);
