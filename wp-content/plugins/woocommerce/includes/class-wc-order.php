...
class WC_Order extends WC_Abstract_Order {
	...
	public function get_formatted_order_total($tax_display = '', $display_refunded = TRUE) {
			...
			if ('itemized' == get_option('woocommerce_tax_total_display')) {
				foreach ($this->get_tax_totals() as $code => $tax) {
					$tax_amount         = ($total_refunded && $display_refunded)
						? wc_price(WC_Tax::round($tax->amount - $this->get_total_tax_refunded_by_rate_id($tax->rate_id)),
						           ['currency' => $this->get_order_currency()]) : $tax->formatted_amount;
					$tax_string_array[] = sprintf('%s %s', $tax_amount, $tax->label);
				}
			} else {
				$tax_amount         = ($total_refunded && $display_refunded)
					? $this->get_total_tax() - $this->get_total_tax_refunded() : $this->get_total_tax();
				$tax_string_array[] = sprintf('%s %s',
				                              wc_price($tax_amount, ['currency' => $this->get_order_currency()]),
		...
		if ($total_refunded && $display_refunded) {
			$formatted_total
				= '<del>' . strip_tags($formatted_total) . '</del> <ins>' . wc_price($order_total - $total_refunded,
				                                                                     ['currency' => $this->get_order_currency()]) . $tax_string . '</ins>';
		} else {
