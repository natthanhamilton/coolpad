...
class WC_Breadcrumb {
	...
	private function add_crumbs_single($post_id = 0, $permalink = '') {
		...
		if ('product' === get_post_type($post)) {
			...
			if ($terms = wc_get_product_terms($post->ID, 'product_cat', ['orderby' => 'parent', 'order' => 'DESC'])) {
	...
	private function endpoint_trail() {
		...
		if (is_wc_endpoint_url() && ($endpoint = WC()->query->get_current_endpoint()) && ($endpoint_title
	...
	private function add_crumbs_tag() {
		...
		$this->add_crumb(sprintf(__('Posts tagged &ldquo;%s&rdquo;', 'woocommerce'), single_tag_title('', FALSE)),
