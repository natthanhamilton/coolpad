...
class WC_Shipping_Rate {
	 ...
	 */
	public function __construct($id = '', $label = '', $cost = 0, $taxes = [], $method_id = '') {
		$this->id        = $id;
		$this->label     = $label;
		$this->cost      = $cost;
		$this->taxes     = !empty($taxes) && is_array($taxes) ? $taxes : [];
	...
	public function get_shipping_tax() {
		return apply_filters('woocommerce_get_shipping_tax',
		                     sizeof($this->taxes) > 0 && !WC()->customer->is_vat_exempt() ? array_sum($this->taxes) : 0,
		                     $this);
	}
