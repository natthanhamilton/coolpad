...
class WC_Shortcodes {
	...
	public static function product_add_to_cart($atts) {
		...
		$product = is_object($product_data) && in_array($product_data->post_type, ['product', 'product_variation'])
	...
	<p class="product woocommerce add_to_cart_inline <?php echo esc_attr($atts['class']); ?>"<?php echo $styles; ?>>
		...
		<?php woocommerce_template_loop_add_to_cart(['quantity' => $atts['quantity']]); ?>
	...
	public static function top_rated_products($atts) {
		...
		$query_args = self::_maybe_add_category_args($query_args, $atts['category'], $atts['operator']);
		add_filter('posts_clauses', [__CLASS__, 'order_by_rating_post_clauses']);
		$return = self::product_loop($query_args, $atts, 'top_rated_products');
		remove_filter('posts_clauses', [__CLASS__, 'order_by_rating_post_clauses']);
	...
	public static function product_attribute($atts) {
		...
		$query_args = [
			...
			'tax_query'           => [
				[
					'taxonomy' => strstr($atts['attribute'], 'pa_') ? sanitize_title($atts['attribute'])
