 ...
 *
 * Note: get_option( 'woocommerce_lock_down_admin', true ) is a deprecated option here for backwards compat. Defaults
 * to true.
 *
 * @access public
 *
...
function wc_disable_admin_bar($show_admin_bar) {
	if (apply_filters('woocommerce_disable_admin_bar', get_option('woocommerce_lock_down_admin',
	                                                              'yes') === 'yes') && !(current_user_can('edit_posts') || current_user_can('manage_woocommerce'))
	) {
	...
	function wc_create_new_customer($email, $username = '', $password = '') {
		...
		if ('yes' === get_option('woocommerce_registration_generate_password') && empty($password)) {
			...
			return new WP_Error('registration-error-missing-password',
			                    __('Please enter an account password.', 'woocommerce'));
		} else {
		...
		if (is_wp_error($customer_id)) {
			return new WP_Error('registration-error', '<strong>' . __('ERROR',
			                                                          'woocommerce') . '</strong>: ' . __('Couldn&#8217;t register you&hellip; please contact us if you continue to have problems.',
			                                                                                              'woocommerce'));
		}
...
function wc_reset_order_customer_id_on_deleted_user($user_id) {
	global $wpdb;
	$wpdb->update($wpdb->postmeta, ['meta_value' => 0], ['meta_key' => '_customer_user', 'meta_value' => $user_id]);
}
