...
class Emogrifier {
	 ...
	 *
	 * 1. Because of client incompatibilities, it is better practice to send out HTML entities rather than unicode over
	 * email.
	 *
	...
	/**
	 * the styles to apply to the nodes with the XPath paths as array keys for the outer array and the attribute
	 * names/values. as key/value pairs for the inner array.
	 *
	...
	public function emogrify() {
		...
		if ($nodesWithStyleAttributes !== FALSE) {
			...
			foreach ($nodesWithStyleAttributes as $node) {
				$normalizedOriginalStyle = preg_replace_callback('/[A-z\\-]+(?=\\:)/S', [$this, 'strtolower'],
				                                                 $node->getAttribute('style'));
				// in order to not overwrite existing style attributes in the HTML, we have to save the original HTML styles
				$nodePath = $node->getNodePath();
				if (!isset($this->styleAttributesForNodes[ $nodePath ])) {
					$this->styleAttributesForNodes[ $nodePath ]
						                             = $this->parseCssDeclarationBlock($normalizedOriginalStyle);
		...
		foreach ($this->caches[ self::CACHE_KEY_CSS ][ $cssKey ] as $value) {
			...
			foreach ($nodesMatchingCssSelectors as $node) {
				...
				$node->setAttribute('style', $this->generateStyleStringFromDeclarationsArrays($oldStyleDeclarations,
		...
		foreach ($this->styleAttributesForNodes as $nodePath => $styleAttributesForNode) {
			...
			$node->setAttribute('style', $this->generateStyleStringFromDeclarationsArrays($currentStyleAttributes,
		...
		$nodesWithStyleDisplayNone
			= $xpath->query('//*[contains(translate(translate(@style," ",""),"NOE","noe"),"display:none")]');
	...
	/**
	 * Returns the HTML with the non-ASCII characters converts into HTML entities and the unprocessable HTML tags
	 * removed.
	 *
	 ...
	 *
	 * @param integer $key the cache key, must be CACHE_KEY_CSS, CACHE_KEY_SELECTOR, CACHE_KEY_XPATH or
	...
	private function clearCache($key) {
		$allowedCacheKeys
			= [self::CACHE_KEY_CSS, self::CACHE_KEY_SELECTOR, self::CACHE_KEY_XPATH, self::CACHE_KEY_CSS_DECLARATION_BLOCK];
	...
	private function translateCssToXpath($paramCssSelector) {
		...
		if (!isset($this->caches[ self::CACHE_KEY_XPATH ][ $xpathKey ])) {
			...
			$cssSelector = preg_replace_callback(self::ID_ATTRIBUTE_MATCHER, [$this, 'matchIdAttributes'],
			                                     $cssSelector);
			$cssSelector = preg_replace_callback(self::CLASS_ATTRIBUTE_MATCHER, [$this, 'matchClassAttributes'],
			                                     $cssSelector);
