...
class WC_Query {
	...
	public function __construct() {
		...
		if (!is_admin()) {
			...
			add_action('wp', [$this, 'remove_product_query']);
	...
	public function init_query_vars() {
		...
		$this->query_vars = [
			...
			'payment-methods'            => get_option('woocommerce_myaccount_payment_methods_endpoint',
			                                           'payment-methods'),
			...
			'add-payment-method'         => get_option('woocommerce_myaccount_add_payment_method_endpoint',
			                                           'add-payment-method'),
			'delete-payment-method'      => get_option('woocommerce_myaccount_delete_payment_method_endpoint',
			                                           'delete-payment-method'),
			'set-default-payment-method' => get_option('woocommerce_myaccount_set_default_payment_method_endpoint',
	...
	public static function get_main_search_query_sql() {
		...
		foreach ($search_terms as $term) {
			...
			$sql[]
			      = $wpdb->prepare("(($wpdb->posts.post_title $like_op %s) $andor_op ($wpdb->posts.post_excerpt $like_op %s) $andor_op ($wpdb->posts.post_content $like_op %s))",
			                       $like, $like, $like);
		}
	...
	public function get_errors() {
		if (!empty($_GET['wc_error']) && ($error = sanitize_text_field($_GET['wc_error'])) && !wc_has_notice($error,
		                                                                                                     'error')
		) {
	...
	protected function get_endpoints_mask() {
		if ('page' === get_option('show_on_front')) {
			...
			if (in_array($page_on_front, [$myaccount_page_id, $checkout_page_id])) {
	...
	public function pre_get_posts($q) {
		...
		if (is_search()) {
			add_filter('posts_where', [$this, 'search_post_excerpt']);
			add_filter('wp', [$this, 'remove_posts_where']);
		}
	...
	public function product_query($q) {
		...
		$q->set('posts_per_page', $q->get('posts_per_page')
			? $q->get('posts_per_page') : apply_filters('loop_shop_per_page', get_option('posts_per_page')));
		$q->set('wc_query', 'product_query');
		$q->set('post__in', array_unique(apply_filters('loop_shop_post_in', [])));
	 ...
	 * @return array
	 */
	public function get_meta_query($meta_query = []) {
	...
	public function visibility_meta_query($compare = 'IN') {
		return [
			'key'     => '_visibility',
			'value'   => is_search() ? ['visible', 'search'] : ['visible', 'catalog'],
			'compare' => $compare,
		];
	}
	 ...
	 * @return array
	 */
	public function get_tax_query($tax_query = []) {
			...
			if ($attribute_taxonomies = wc_get_attribute_taxonomies()) {
				foreach ($attribute_taxonomies as $tax) {
					...
					$filter_terms = !empty($_GET[ 'filter_' . $attribute ]) ? explode(',',
					                                                                  wc_clean($_GET[ 'filter_' . $attribute ]))
						: [];
					if (empty($filter_terms) || !taxonomy_exists($taxonomy)) {
						continue;
					}
					$query_type
						                                                 = !empty($_GET[ 'query_type_' . $attribute ]) && in_array($_GET[ 'query_type_' . $attribute ],
						                                                                                                           ['and', 'or'])
						? wc_clean($_GET[ 'query_type_' . $attribute ]) : '';
					self::$_chosen_attributes[ $taxonomy ]['terms']      = array_map('sanitize_title',
					                                                                 $filter_terms); // Ensures correct encoding
					self::$_chosen_attributes[ $taxonomy ]['query_type'] = $query_type ? $query_type
	...
	public function remove_posts_where() {
		remove_filter('posts_where', [$this, 'search_post_excerpt']);
	}
