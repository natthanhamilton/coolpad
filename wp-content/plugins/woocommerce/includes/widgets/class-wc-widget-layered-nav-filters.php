...
class WC_Widget_Layered_Nav_Filters extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __('Shows active layered nav filters so users can see and deactivate them.',
		                               'woocommerce');
			...
			if (!empty($_chosen_attributes)) {
				foreach ($_chosen_attributes as $taxonomy => $data) {
					foreach ($data['terms'] as $term_slug) {
						...
						$link = remove_query_arg(['add-to-cart', $filter_name]);
						if (sizeof($new_filter) > 0) {
							$link = add_query_arg($filter_name, implode(',', $new_filter), $link);
						}
						echo '<li class="chosen"><a title="' . esc_attr__('Remove filter',
						                                                  'woocommerce') . '" href="' . esc_url($link) . '">' . esc_html($term->name) . '</a></li>';
					}
				}
			}
			if ($min_price) {
				...
				echo '<li class="chosen"><a title="' . esc_attr__('Remove filter',
				                                                  'woocommerce') . '" href="' . esc_url($link) . '">' . __('Min',
				                                                                                                           'woocommerce') . ' ' . wc_price($min_price) . '</a></li>';
			}
			if ($max_price) {
				$link = remove_query_arg('max_price');
				echo '<li class="chosen"><a title="' . esc_attr__('Remove filter',
				                                                  'woocommerce') . '" href="' . esc_url($link) . '">' . __('Max',
				                                                                                                           'woocommerce') . ' ' . wc_price($max_price) . '</a></li>';
			}
			if ($min_rating) {
				$link = remove_query_arg('min_rating');
				echo '<li class="chosen"><a title="' . esc_attr__('Remove filter',
				                                                  'woocommerce') . '" href="' . esc_url($link) . '">' . sprintf(__('Rated %s and above',
				                                                                                                                   'woocommerce'),
				                                                                                                                $min_rating) . '</a></li>';
			}
			echo '</ul>';
