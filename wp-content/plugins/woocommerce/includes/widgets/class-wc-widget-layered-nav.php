...
class WC_Widget_Layered_Nav extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description
		                       = __('Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories.',
		                            'woocommerce');
	...
	protected function layered_nav_dropdown($terms, $taxonomy, $query_type) {
		...
		if ($taxonomy !== $this->get_current_taxonomy()) {
			...
			echo '<option value="">' . sprintf(__('Any %s', 'woocommerce'),
			...
			foreach ($terms as $term) {
				...
				$current_values = isset($_chosen_attributes[ $taxonomy ]['terms'])
				...
				echo '<option value="' . esc_attr($term->slug) . '" ' . selected($option_is_set, TRUE,
			...
			wc_enqueue_js("
				jQuery( '.dropdown_layered_nav_" . esc_js($taxonomy_filter_name) . "' ).change( function() {
					...
					location.href = '" . preg_replace('%\/page\/[0-9]+%', '', str_replace(['&amp;', '%2C'], ['&', ','],
			                                                                              esc_js(add_query_arg('filtering',
			                                                                                                   '1',
			                                                                                                   remove_query_arg(['page', 'filter_' . $taxonomy_filter_name]))))) . "&filter_" . esc_js($taxonomy_filter_name) . "=' + slug;
				});
			");
		}
		...
		foreach ($terms as $term) {
			...
			if (!empty($current_filter)) {
				...
				if ($query_type === 'or' && !(1 === sizeof($current_filter) && $option_is_set)) {
					$link = add_query_arg('query_type_' . sanitize_title(str_replace('pa_', '', $taxonomy)), 'or',
					                      $link);
				}
			}
			echo '<li class="wc-layered-nav-term ' . ($option_is_set ? 'chosen' : '') . '">';
			echo ($count > 0 || $option_is_set) ? '<a href="' . esc_url(apply_filters('woocommerce_layered_nav_link',
			                                                                          $link)) . '">' : '<span>';
