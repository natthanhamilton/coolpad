...
class WC_Widget_Recent_Reviews extends WC_Widget {
	...
	public function widget($args, $instance) {
		...
		$comments
		        = get_comments(['number' => $number, 'status' => 'approve', 'post_status' => 'publish', 'post_type' => 'product']);
		if ($comments) {
			...
			foreach ((array)$comments as $comment) {
				...
				echo $rating_html;
				printf('<span class="reviewer">' . _x('by %1$s', 'by comment author', 'woocommerce') . '</span>',
				       get_comment_author());
				echo '</li>';
			}
			echo '</ul>';
