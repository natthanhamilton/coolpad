...
function wc_create_order($args = []) {
	...
	if ($args['order_id'] > 0) {
		...
		$order_data['post_title']    = sprintf(__('Order &ndash; %s', 'woocommerce'),
	...
	if (!$updating) {
		update_post_meta($order_id, '_order_key',
		                 'wc_' . apply_filters('woocommerce_generate_order_key', uniqid('order_')));
		update_post_meta($order_id, '_order_currency', get_woocommerce_currency());
		update_post_meta($order_id, '_prices_include_tax', get_option('woocommerce_prices_include_tax'));
		update_post_meta($order_id, '_customer_ip_address', WC_Geolocation::get_ip_address());
		update_post_meta($order_id, '_customer_user_agent',
...
function wc_get_template_part($slug, $name = '') {
	...
	if ($name && !WC_TEMPLATE_DEBUG_MODE) {
		$template = locate_template(["{$slug}-{$name}.php", WC()->template_path() . "{$slug}-{$name}.php"]);
	}
	...
	if (!$template && !WC_TEMPLATE_DEBUG_MODE) {
		$template = locate_template(["{$slug}.php", WC()->template_path() . "{$slug}.php"]);
	}
...
function get_woocommerce_currency_symbol($currency = '') {
	...
	$symbols = apply_filters('woocommerce_currency_symbols', [
		'AED' => '&#x62f;.&#x625;',
		'AFN' => '&#x60b;',
		'ALL' => 'L',
		'AMD' => 'AMD',
...
function wc_get_image_size($image_size) {
	...
	} elseif (in_array($image_size, ['shop_thumbnail', 'shop_catalog', 'shop_single'])) {
		$size           = get_option($image_size . '_image_size', []);
...
function wc_fix_rewrite_rules($rules) {
	...
	if (preg_match('`/(.+)(/%product_cat%)`', $product_permalink, $matches)) {
		foreach ($rules as $rule => $rewrite) {
			if (preg_match('`^' . preg_quote($matches[1], '`') . '/\(`',
			               $rule) && preg_match('/^(index\.php\?product_cat)(?!(.*product))/', $rewrite)
			) {
				unset($rules[ $rule ]);
			}
		}
...
function wc_get_core_supported_themes() {
	return ['twentysixteen', 'twentyfifteen', 'twentyfourteen', 'twentythirteen', 'twentyeleven', 'twentytwelve', 'twentyten'];
}
...
function wc_template_debug_mode() {
	if (!defined('WC_TEMPLATE_DEBUG_MODE')) {
		$status_options = get_option('woocommerce_status_options', []);
...
function wc_get_credit_card_type_label($type) {
	...
	$labels = apply_filters('wocommerce_credit_card_type_labels', [
	...
	return apply_filters('woocommerce_get_credit_card_type_label',
		(array_key_exists($type, $labels) ? $labels[ $type ] : ucfirst($type)));
}
...
function wc_get_shipping_method_count($include_legacy = FALSE) {
	global $wpdb;
	$transient_name = 'wc_shipping_method_count_' . ($include_legacy ? 1
			: 0) . '_' . WC_Cache_Helper::get_transient_version('shipping');
 ...
 * Will increase the precision of wc_get_price_decimals by 2 decimals, unless WC_ROUNDING_PRECISION is set to a higher
 * number.
 *
 * @since 2.6.3
 * @return int
