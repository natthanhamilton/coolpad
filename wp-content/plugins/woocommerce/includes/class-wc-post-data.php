...
class WC_Post_Data {
	...
	public static function init() {
		...
		add_action('pre_post_update', [__CLASS__, 'pre_post_update']);
		add_action('update_post_meta', [__CLASS__, 'sync_product_stock_status'], 10, 4);
	}
	...
	public static function transition_post_status($new_status, $old_status, $post) {
		if (('publish' === $new_status || 'publish' === $old_status) && in_array($post->post_type,
		...
		if (!is_null(self::$editing_term) && strpos($taxonomy, 'pa_') === 0) {
			...
			if ($edited_term->slug !== self::$editing_term->slug) {
				...
				$wpdb->query($wpdb->prepare("UPDATE {$wpdb->postmeta} SET meta_value = %s WHERE meta_key = %s AND meta_value = %s;",
				                            $edited_term->slug, 'attribute_' . sanitize_title($taxonomy),
	...
	public static function update_post_metadata($check, $object_id, $meta_key, $meta_value, $prev_value) {
		if (!empty($meta_value) && is_float($meta_value) && in_array(get_post_type($object_id),
		                                                             array_merge(wc_get_order_types(),
		                                                                         ['shop_coupon', 'product', 'product_variation']))
		) {
			// Convert float to string
			$meta_value = wc_float_to_string($meta_value);
			// Update meta value with new string
