...
function wc_create_page($slug, $option = '', $page_title = '', $page_content = '', $post_parent = 0) {
	...
	if ($option_value > 0) {
		...
		if ('page' === $page_object->post_type && !in_array($page_object->post_status,
	...
	if (strlen($page_content) > 0) {
		...
		$valid_page_found
			= $wpdb->get_var($wpdb->prepare("SELECT ID FROM $wpdb->posts WHERE post_type='page' AND post_status NOT IN ( 'pending', 'trash', 'future', 'auto-draft' ) AND post_content LIKE %s LIMIT 1;",
			                                "%{$page_content}%"));
	} else {
		// Search for an existing page with the specified page slug
		$valid_page_found
			= $wpdb->get_var($wpdb->prepare("SELECT ID FROM $wpdb->posts WHERE post_type='page' AND post_status NOT IN ( 'pending', 'trash', 'future', 'auto-draft' )  AND post_name = %s LIMIT 1;",
			                                $slug));
	}
	...
	if (strlen($page_content) > 0) {
		// Search for an existing page with the specified page content (typically a shortcode)
		$trashed_page_found
			= $wpdb->get_var($wpdb->prepare("SELECT ID FROM $wpdb->posts WHERE post_type='page' AND post_status = 'trash' AND post_content LIKE %s LIMIT 1;",
			                                "%{$page_content}%"));
	} else {
		// Search for an existing page with the specified page slug
		$trashed_page_found
			= $wpdb->get_var($wpdb->prepare("SELECT ID FROM $wpdb->posts WHERE post_type='page' AND post_status = 'trash' AND post_name = %s LIMIT 1;",
			                                $slug));
	}
...
function wc_save_order_items($order_id, $items) {
		...
		foreach ($items['order_item_id'] as $item_id) {
			...
			if (isset($items['order_item_name'][ $item_id ])) {
				$wpdb->update(
					...
					['order_item_name' => wc_clean(wp_unslash($items['order_item_name'][ $item_id ]))],
			...
			$line_total[ $item_id ]        = isset($items['line_total'][ $item_id ]) ? $items['line_total'][ $item_id ]
				: 0;
			$line_subtotal[ $item_id ]     = isset($items['line_subtotal'][ $item_id ])
				? $items['line_subtotal'][ $item_id ] : $line_total[ $item_id ];
			$line_tax[ $item_id ]          = isset($items['line_tax'][ $item_id ]) ? $items['line_tax'][ $item_id ]
				: [];
			$line_subtotal_tax[ $item_id ] = isset($items['line_subtotal_tax'][ $item_id ])
				? $items['line_subtotal_tax'][ $item_id ] : $line_tax[ $item_id ];
			// Format taxes
	...
	// Save meta
	$meta_keys   = isset($items['meta_key']) ? $items['meta_key'] : [];
	$meta_values = isset($items['meta_value']) ? $items['meta_value'] : [];
	foreach ($meta_keys as $id => $meta_key) {
		...
		if ($meta_key === '' && $meta_value === '') {
			$wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->prefix}woocommerce_order_itemmeta WHERE meta_id = %d",
			                            $id));
		} else {
			$wpdb->update(
	...
	if (isset($items['shipping_method_id'])) {
		foreach ($items['shipping_method_id'] as $item_id) {
			...
			$method_id    = isset($items['shipping_method'][ $item_id ])
				? wc_clean($items['shipping_method'][ $item_id ]) : '';
			$method_title = isset($items['shipping_method_title'][ $item_id ])
				? wc_clean(wp_unslash($items['shipping_method_title'][ $item_id ])) : '';
			$cost         = isset($items['shipping_cost'][ $item_id ])
				? wc_format_decimal($items['shipping_cost'][ $item_id ]) : '';
			$ship_taxes   = isset($items['shipping_taxes'][ $item_id ]) ? array_map('wc_format_decimal',
			                                                                        $items['shipping_taxes'][ $item_id ])
				: [];
			$wpdb->update(
				$wpdb->prefix . 'woocommerce_order_items',
	...
	// Taxes
	$order_taxes        = isset($items['order_taxes']) ? $items['order_taxes'] : [];
	$taxes_items        = [];
	$taxes_shipping     = [];
	$total_tax          = 0;
