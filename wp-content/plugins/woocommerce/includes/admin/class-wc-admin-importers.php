...
if (!class_exists('WC_Admin_Importers')) :
	...
	class WC_Admin_Importers {
		...
		public function register_importers() {
			register_importer('woocommerce_tax_rate_csv', __('WooCommerce Tax Rates (CSV)', 'woocommerce'),
								...
								foreach ($post['terms'] as $term) {
									if (strstr($term['domain'], 'pa_')) {
										if (!taxonomy_exists($term['domain'])) {
											...
											register_taxonomy(
												$term['domain'],
												apply_filters('woocommerce_taxonomy_objects_' . $term['domain'],
												              ['product']),
												apply_filters('woocommerce_taxonomy_args_' . $term['domain'], [
													'hierarchical' => TRUE,
													'show_ui'      => FALSE,
													'query_var'    => TRUE,
													'rewrite'      => FALSE,
