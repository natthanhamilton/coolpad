...
if (!class_exists('WC_Settings_Payment_Gateways')) :
	...
	class WC_Settings_Payment_Gateways extends WC_Settings_Page {
		...
		public function get_sections() {
			...
			if (!defined('WC_INSTALLING')) {
				...
				foreach ($payment_gateways as $gateway) {
					$title                                = empty($gateway->method_title) ? ucfirst($gateway->id)
		...
		public function get_settings() {
			$settings = apply_filters('woocommerce_payment_gateways_settings', [
				[
				...
				[
					...
					'default'       => 'no',
					'type'          => 'checkbox',
					'desc_tip'      => __('When applying multiple coupons, apply the first coupon to the full price and the second coupon to the discounted price and so on.',
					                      'woocommerce'),
					'checkboxgroup' => 'end',
					'autoload'      => FALSE,
				],
				...
				[
					...
					'desc_tip'        => __('Force SSL (HTTPS) on the checkout pages (an SSL Certificate is required).',
					                        'woocommerce'),
				],
				...
				[
					...
					'desc'  => __('These pages need to be set so that WooCommerce knows where to send users to checkout.',
					              'woocommerce'),
					'type'  => 'title',
				...
				[
					...
					'desc'     => __('Page contents:',
					                 'woocommerce') . ' [' . apply_filters('woocommerce_cart_shortcode_tag',
					                                                       'woocommerce_cart') . ']',
				...
				[
					...
					'desc'     => __('Page contents:',
					                 'woocommerce') . ' [' . apply_filters('woocommerce_checkout_shortcode_tag',
				...
				[
					...
					'desc'     => __('If you define a "Terms" page the customer will be asked if they accept them when checking out.',
					                 'woocommerce'),
				...
				['title' => __('Checkout Endpoints',
				               'woocommerce'), 'type' => 'title', 'desc' => __('Endpoints are appended to your page URLs to handle specific actions during the checkout process. They should be unique.',
				                                                               'woocommerce'), 'id' => 'account_endpoint_options'],
				[
				...
				[
					...
					'desc'  => __('Installed gateways are listed below. Drag and drop gateways to control their display order on the frontend.',
					              'woocommerce'),
					'type'  => 'title',
		...
		public function output() {
			...
			if ($current_section) {
				foreach ($payment_gateways as $gateway) {
					if (in_array($current_section, [$gateway->id, sanitize_title(get_class($gateway))])) {
			...
			<tr valign="top">
				...
				<td class="forminp">
					<table class="wc_gateways widefat" cellspacing="0">
						...
						<tr>
							<?php
							$columns = apply_filters('woocommerce_payment_gateways_setting_columns', [
								'sort'   => '',
							...
							foreach ($columns as $key => $column) {
								switch ($key) {
									case 'sort' :
										...
										break;
									case 'name' :
										$method_title = $gateway->get_title() ? $gateway->get_title()
									...
									case 'status' :
										...
										if ($gateway->enabled == 'yes') {
											echo '<span class="status-enabled tips" data-tip="' . __('Yes',
											                                                         'woocommerce') . '">' . __('Yes',
											                                                                                    'woocommerce') . '</span>';
										} else {
											echo '-';
