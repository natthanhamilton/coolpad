...
if (!class_exists('WC_Settings_Tax')) :
	...
	class WC_Settings_Tax extends WC_Settings_Page {
		...
		public function output() {
			...
			$tax_classes = WC_Tax::get_tax_classes();
			if ($current_section == 'standard' || in_array($current_section,
			                                               array_map('sanitize_title', $tax_classes))
			) {
		...
		public function output_tax_rates() {
			...
			$base_url = admin_url(add_query_arg([
				                                    'page'    => 'wc-settings',
				                                    'tab'     => 'tax',
				                                    'section' => $current_section,
			                                    ], 'admin.php'));
			// Localize and enqueue our js.
			wp_localize_script('wc-settings-tax', 'htmlSettingsTaxLocalizeScript', [
		...
		public function save_tax_rates() {
			...
			$tax_rate_order
				= absint($wpdb->get_var($wpdb->prepare("SELECT tax_rate_order FROM {$wpdb->prefix}woocommerce_tax_rates WHERE tax_rate_id = %s",
				                                       $first_tax_rate_id)));
