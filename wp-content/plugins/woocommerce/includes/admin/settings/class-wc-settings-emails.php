		...
		public function get_settings() {
			$settings = apply_filters('woocommerce_email_settings', [
				['title' => __('Email Notifications',
				               'woocommerce'), 'desc' => __('Email notifications sent from WooCommerce are listed below. Click on an email to configure it.',
				                                            'woocommerce'), 'type' => 'title', 'id' => 'email_notification_settings'],
				['type' => 'email_notification'],
				['type' => 'sectionend', 'id' => 'email_notification_settings'],
				['type' => 'sectionend', 'id' => 'email_recipient_options'],
				['title' => __('Email Sender Options',
				               'woocommerce'), 'type' => 'title', 'desc' => '', 'id' => 'email_options'],
				[
				...
				[
					...
					'desc'              => __('How the sender\'s email appears in outgoing WooCommerce emails.',
					                          'woocommerce'),
				...
				['title' => __('Email Template',
				               'woocommerce'), 'type' => 'title', 'desc' => sprintf(__('This section lets you customize the WooCommerce emails. <a href="%s" target="_blank">Click here to preview your email template</a>.',
				                                                                       'woocommerce'),
				                                                                    wp_nonce_url(admin_url('?preview_woocommerce_mail=true'),
				                                                                                 'preview-mail')), 'id' => 'email_template_options'],
				[
				...
				[
					...
					'placeholder' => __('N/A', 'woocommerce'),
					'type'        => 'textarea',
					'default'     => get_bloginfo('name', 'display') . ' - ' . __('Powered by WooCommerce',
					                                                              'woocommerce'),
					'autoload'    => FALSE,
					'desc_tip'    => TRUE
				],
				[
					...
					'desc'     => __('The base colour for WooCommerce email templates. Default <code>#557da1</code>.',
					                 'woocommerce'),
				...
				[
					...
					'desc'     => __('The background colour for WooCommerce email templates. Default <code>#f5f5f5</code>.',
					                 'woocommerce'),
						...
						foreach ($email_templates as $email_key => $email) {
							...
							foreach ($columns as $key => $column) {
								switch ($key) {
									...
									case 'recipient' :
										echo '<td class="wc-email-settings-table-' . esc_attr($key) . '">
											' . esc_html($email->is_customer_email() ? __('Customer', 'woocommerce')
											                                                                                                                       : $email->get_recipient()) . '
										</td>';
										break;
									case 'status' :
										echo '<td class="wc-email-settings-table-' . esc_attr($key) . '">';
										if ($email->is_manual()) {
											echo '<span class="status-manual tips" data-tip="' . __('Manually sent',
											                                                        'woocommerce') . '">' . __('Manual',
											                                                                                   'woocommerce') . '</span>';
										} elseif ($email->is_enabled()) {
											echo '<span class="status-enabled tips" data-tip="' . __('Enabled',
											                                                         'woocommerce') . '">' . __('Yes',
											                                                                                    'woocommerce') . '</span>';
										} else {
											echo '<span class="status-disabled tips" data-tip="' . __('Disabled',
											                                                          'woocommerce') . '">-</span>';
										}
										echo '</td>';
										break;
									...
									case 'actions' :
										echo '<td class="wc-email-settings-table-' . esc_attr($key) . '">
											<a class="button alignright tips" data-tip="' . __('Configure',
										                                                       'woocommerce') . '" href="' . admin_url('admin.php?page=wc-settings&tab=email&section=' . strtolower($email_key)) . '">' . __('Configure',
										                                                                                                                                                                                     'woocommerce') . '</a>
										</td>';
										break;
									default :
