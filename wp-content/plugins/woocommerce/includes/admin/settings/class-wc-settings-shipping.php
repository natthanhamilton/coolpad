...
if (!class_exists('WC_Settings_Shipping')) :
	...
	class WC_Settings_Shipping extends WC_Settings_Page {
		...
		public function get_sections() {
			...
			if (!defined('WC_INSTALLING')) {
				...
				foreach ($shipping_methods as $method) {
					...
					$title                               = empty($method->method_title) ? ucfirst($method->id)
						: $method->method_title;
		...
		public function get_settings() {
			$settings = apply_filters('woocommerce_shipping_settings', [
				['title' => __('Shipping Options', 'woocommerce'), 'type' => 'title', 'id' => 'shipping_options'],
				[
			...
			if ('' === $current_section) {
				...
				foreach ($shipping_methods as $method) {
					if (in_array($current_section,
					             [$method->id, sanitize_title(get_class($method))]) && $method->has_settings()
					) {
			...
			switch ($current_section) {
				...
				default :
					...
					foreach ($wc_shipping->get_shipping_methods() as $method_id => $method) {
						if (in_array($current_section, [$method->id, sanitize_title(get_class($method))])) {
			...
			wp_localize_script('wc-shipping-zone-methods', 'shippingZoneMethodsLocalizeScript', [
				...
				'strings'                 => [
					'unload_confirmation_msg' => __('Your changed data will be lost if you leave this page without saving.',
					                                'woocommerce'),
					'save_changes_prompt'     => __('Do you wish to save your changes first? Your changed data will be discarded if you choose to cancel.',
					                                'woocommerce'),
		...
		protected function zones_screen() {
			...
			wp_localize_script('wc-shipping-zones', 'shippingZonesLocalizeScript', [
		...
		protected function instance_settings_screen($instance_id) {
			...
			if (!empty($_POST['save'])) {
				if (empty($_REQUEST['_wpnonce']) || !wp_verify_nonce($_REQUEST['_wpnonce'], 'woocommerce-settings')) {
					echo '<div class="updated error"><p>' . __('Edit failed. Please try again.',
					                                           'woocommerce') . '</p></div>';
				}
		...
		protected function output_shipping_class_screen() {
			...
			$shipping_class_columns = apply_filters('woocommerce_shipping_classes_columns', [
