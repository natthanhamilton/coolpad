	...
	<table id="api-keys-options" class="form-table">
		...
		<tr valign="top">
			...
			<td class="forminp">
				...
				?>
				<input type="hidden" class="wc-customer-search" id="key_user"
				       data-placeholder="<?php esc_attr_e('Search for a customer&hellip;', 'woocommerce'); ?>"
				       data-selected="<?php echo esc_attr($user_string); ?>" value="<?php echo esc_attr($user_id); ?>"
			...
			<td class="forminp">
				<select id="key_permissions" class="wc-enhanced-select">
					...
					foreach ($permissions as $permission_id => $permission_name) : ?>
						<option value="<?php echo esc_attr($permission_id); ?>" <?php selected($key_data['permissions'],
						                                                                       $permission_id,
						                                                                       TRUE); ?>><?php echo esc_html($permission_name); ?></option>
					<?php endforeach; ?>
				</select>
			</td>
		</tr>
				...
				<td class="forminp">
						<span><?php
							if (!empty($key_data['last_access'])) {
								$date = sprintf(_x('%1$s at %2$s', 'date and time', 'woocommerce'),
								                date_i18n(wc_date_format(), strtotime($key_data['last_access'])),
								                date_i18n(wc_time_format(), strtotime($key_data['last_access'])));
								echo apply_filters('woocommerce_api_key_last_access_datetime', $date,
	...
	if (0 == $key_id) {
		...
		<p class="submit">
			...
			<a style="color: #a00; text-decoration: none; margin-left: 10px;"
			   href="<?php echo esc_url(wp_nonce_url(add_query_arg(['revoke-key' => $key_id],
			                                                       admin_url('admin.php?page=wc-settings&tab=api&section=keys')),
			                                         'revoke')); ?>"><?php _e('Revoke Key', 'woocommerce'); ?></a>
		</p>
		<?php
	}
	?>
		...
		<tr valign="top">
			...
			<td class="forminp">
				...
				<button type="button" class="button-secondary copy-key"
				        data-tip="<?php esc_attr_e('Copied!', 'woocommerce'); ?>"><?php _e('Copy',
				                                                                           'woocommerce'); ?></button>
			</td>
		</tr>
		<tr valign="top">
			...
			<td class="forminp">
				<input id="key_consumer_secret" type="text" value="{{ data.consumer_secret }}" size="55"
				       readonly="readonly">
