	...
	<table class="form-table">
		...
		<tr valign="top">
			<th scope="row" class="titledesc">
				...
				<?php echo wc_help_tip(sprintf(__('Friendly name for identifying this webhook, defaults to Webhook created on %s.',
				                                  'woocommerce'), strftime(_x('%b %d, %Y @ %I:%M %p',
		...
		<tr valign="top">
			<th scope="row" class="titledesc">
				...
				<?php wc_help_tip(__('The options are &quot;Active&quot; (delivers payload), &quot;Paused&quot; (does not deliver), or &quot;Disabled&quot; (does not deliver due delivery failures).',
				                     'woocommerce')); ?>
			</th>
			<td class="forminp">
				<select name="webhook_status" id="webhook_status" class="wc-enhanced-select">
					...
					foreach ($statuses as $status_slug => $status_name) : ?>
						<option value="<?php echo esc_attr($status_slug); ?>" <?php selected($current_status,
						                                                                     $status_slug,
						                                                                     TRUE); ?>><?php echo esc_html($status_name); ?></option>
					<?php endforeach; ?>
				</select>
			</td>
		</tr>
			...
			<td class="forminp">
				<select name="webhook_topic" id="webhook_topic" class="wc-enhanced-select">
					...
					foreach ($topics as $topic_slug => $topic_name) : ?>
						<option value="<?php echo esc_attr($topic_slug); ?>" <?php selected($topic_data['topic'],
						                                                                    $topic_slug,
						                                                                    TRUE); ?>><?php echo esc_html($topic_name); ?></option>
					<?php endforeach; ?>
				</select>
			</td>
		</tr>
		...
		<tr valign="top">
			<th scope="row" class="titledesc">
				...
				<?php echo wc_help_tip(__('The Secret Key is used to generate a hash of the delivered webhook and provided in the request headers.',
				                          'woocommerce')); ?>
			</th>
			<td class="forminp">
		...
		<?php if ('0000-00-00 00:00:00' != $webhook->post_data->post_modified_gmt) : ?>
			<?php if ('0000-00-00 00:00:00' == $webhook->post_data->post_date_gmt) : ?>
				<tr valign="top">
					...
					<td class="forminp">
						<?php echo date_i18n(__('M j, Y @ G:i', 'woocommerce'),
						                     strtotime($webhook->post_data->post_modified_gmt)); ?>
					</td>
				</tr>
			<?php else : ?>
				<tr valign="top">
					...
					<td class="forminp">
						<?php echo date_i18n(__('M j, Y @ G:i', 'woocommerce'),
						                     strtotime($webhook->post_data->post_date_gmt)); ?>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row" class="titledesc">
			...
			<td colspan="2" scope="row" style="padding-left: 0;">
				<p class="submit">
					<input type="submit" class="button button-primary button-large" name="save" id="publish"
					       accesskey="p" value="<?php esc_attr_e('Save Webhook', 'woocommerce'); ?>"/>
					<?php if (current_user_can('delete_post', $webhook->id)) : ?>
						<a style="color: #a00; text-decoration: none; margin-left: 10px;"
						   href="<?php echo esc_url(get_delete_post_link($webhook->id)); ?>"><?php echo (!EMPTY_TRASH_DAYS)
								? __('Delete Permanently', 'woocommerce') : __('Move to Trash', 'woocommerce'); ?></a>
					<?php endif; ?>
				</p>
			</td>
		</tr>
