...
if (!class_exists('WC_Admin_Duplicate_Product')) :
	...
	class WC_Admin_Duplicate_Product {
		...
		public function dupe_link($actions, $post) {
			...
			$actions['duplicate']
				= '<a href="' . wp_nonce_url(admin_url('edit.php?post_type=product&action=duplicate_product&amp;post=' . $post->ID),
				                             'woocommerce-duplicate-product_' . $post->ID) . '" title="' . esc_attr__('Make a duplicate from this product',
				                                                                                                      'woocommerce')
		...
		public function dupe_button() {
			...
			if (isset($_GET['post'])) {
				$notify_url
					= wp_nonce_url(admin_url("edit.php?post_type=product&action=duplicate_product&post=" . absint($_GET['post'])),
					               'woocommerce-duplicate-product_' . $_GET['post']);
				?>
				<div id="duplicate-action"><a class="submitduplicate duplication"
				                              href="<?php echo esc_url($notify_url); ?>"><?php _e('Copy to a new draft',
				                                                                                  'woocommerce'); ?></a>
				</div>
				<?php
			}
		...
		public function duplicate_product($post, $parent = 0, $post_status = '') {
			...
			if ('product_variation' === $post->post_type) {
				$post_title = sprintf(__('Variation #%s of %s', 'woocommerce'), absint($new_post_id),
				                      esc_html(get_the_title($post_parent)));
				$wpdb->update(
					$wpdb->posts,
					[
						'post_title' => $post_title,
			...
			if (!in_array($post_status, ['draft', 'pending', 'auto-draft'])) {
				$wpdb->update(
					$wpdb->posts,
					[
						'post_name' => wp_unique_post_slug(sanitize_title($post_title, $new_post_id), $new_post_id,
						                                   $post_status, $post->post_type, $post_parent),
			...
			if (!$exclude && ($children_products
					= get_children('post_parent=' . $post->ID . '&post_type=product_variation'))
			) {
				foreach ($children_products as $child) {
					$this->duplicate_product($this->get_product_to_duplicate($child->ID), $new_post_id,
					                         $child->post_status);
				}
			}
			// Clear cache
		...
		private function duplicate_post_meta($id, $new_id) {
			...
			$sql     = $wpdb->prepare("SELECT meta_key, meta_value FROM $wpdb->postmeta WHERE post_id = %d",
			                          absint($id));
			$exclude = array_map('esc_sql', array_filter(apply_filters('woocommerce_duplicate_product_exclude_meta',
			...
			if (sizeof($post_meta)) {
				...
				foreach ($post_meta as $post_meta_row) {
					$sql_query_sel[] = $wpdb->prepare("SELECT %d, %s, %s", $new_id, $post_meta_row->meta_key,
					                                  $post_meta_row->meta_value);
				}
