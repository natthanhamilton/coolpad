...
class WC_Admin_Notices {
	...
	public static function reset_admin_notices() {
		if (!current_theme_supports('woocommerce') && !in_array(get_option('template'),
		...
		if (!class_exists('WC_Gateway_Simplify_Commerce_Loader') && !empty($simplify_options['enabled']) && 'yes' === $simplify_options['enabled'] && in_array($location['country'],
	...
	public static function add_notices() {
		...
		if (!empty($notices)) {
			...
			foreach ($notices as $notice) {
				if (!empty(self::$core_notices[ $notice ]) && apply_filters('woocommerce_show_admin_notice', TRUE,
				                                                            $notice)
				) {
	...
	public static function simplify_commerce_notice() {
		$location = wc_get_base_location();
		if (class_exists('WC_Gateway_Simplify_Commerce_Loader') || !in_array($location['country'],
		                                                                     apply_filters('woocommerce_gateway_simplify_commerce_supported_countries',
		                                                                                   ['US', 'IE']))
		) {
