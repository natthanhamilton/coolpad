...
class WC_Report_Customer_List extends WP_List_Table {
	...
	public function column_default($user, $column_name) {
		...
		switch ($column_name) {
			...
			case 'location' :
				...
				$state   = isset(WC()->countries->states[ $country_code ][ $state_code ])
					? WC()->countries->states[ $country_code ][ $state_code ] : $state_code;
				$country = isset(WC()->countries->countries[ $country_code ])
			...
			case 'last_order' :
				...
				if (!empty($orders)) {
					...
					return '<a href="' . admin_url('post.php?post=' . $order->id . '&action=edit') . '">' . _x('#',
					                                                                                           'hash before order number',
					                                                                                           'woocommerce') . $order->get_order_number() . '</a> &ndash; ' . date_i18n(get_option('date_format'),
	...
	public function output_report() {
		...
		if (!empty($_GET['link_orders']) && wp_verify_nonce($_REQUEST['_wpnonce'], 'link_orders')) {
			...
			echo '<div class="updated"><p>' . sprintf(_n('%s previous order linked', '%s previous orders linked',
			                                             $linked, 'woocommerce'), $linked) . '</p></div>';
		}
		if (!empty($_GET['refresh']) && wp_verify_nonce($_REQUEST['_wpnonce'], 'refresh')) {
			...
			echo '<div class="updated"><p>' . sprintf(__('Refreshed stats for %s', 'woocommerce'),
	...
	public function order_by_last_name($query) {
		...
		if ($s) {
			...
			$query->query_where .= " AND ( user_login LIKE '%" . esc_sql(str_replace('*', '',
			                                                                         $s)) . "%' OR user_nicename LIKE '%" . esc_sql(str_replace('*',
			                                                                                                                                    '',
			                                                                                                                                    $s)) . "%' OR meta3.meta_value LIKE '%" . esc_sql(str_replace('*',
			                                                                                                                                                                                                  '',
			                                                                                                                                                                                                  $s)) . "%' ) ";
