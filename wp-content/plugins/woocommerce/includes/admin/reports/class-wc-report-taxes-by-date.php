	...
	public function get_main_chart() {
		...
		foreach ($tax_rows_orders as $tax_row) {
			...
			$tax_rows[ $key ] = isset($tax_rows[ $key ]) ? $tax_rows[ $key ]
				: (object)['tax_amount' => 0, 'shipping_tax_amount' => 0, 'total_sales' => 0, 'total_shipping' => 0, 'total_orders' => 0];
		...
		<table class="widefat">
			...
			<tr>
				...
				<th class="total_row"><?php _e('Total Sales',
				                               'woocommerce'); ?><?php echo wc_help_tip(__("This is the sum of the 'Order Total' field within your orders.",
				                                                                           'woocommerce')); ?></th>
				<th class="total_row"><?php _e('Total Shipping',
				                               'woocommerce'); ?><?php echo wc_help_tip(__("This is the sum of the 'Shipping Total' field within your orders.",
				                                                                           'woocommerce')); ?></th>
				<th class="total_row"><?php _e('Total Tax',
				                               'woocommerce'); ?><?php echo wc_help_tip(__('This is the total tax for the rate (shipping tax + product tax).',
				                                                                           'woocommerce')); ?></th>
				<th class="total_row"><?php _e('Net profit',
				                               'woocommerce'); ?><?php echo wc_help_tip(__("Total sales minus shipping and tax.",
				                                                                           'woocommerce')); ?></th>
			</tr>
			</thead>
			<?php if (!empty($tax_rows)) : ?>
				...
				<tfoot>
				<?php
				$gross     = array_sum(wp_list_pluck((array)$tax_rows,
				                                     'total_sales')) - array_sum(wp_list_pluck((array)$tax_rows,
				                                                                               'total_shipping'));
				$total_tax = array_sum(wp_list_pluck((array)$tax_rows,
				...
				<tr>
					...
					<th class="total_row"><?php echo wc_price(array_sum(wp_list_pluck((array)$tax_rows,
					                                                                  'total_shipping'))); ?></th>
