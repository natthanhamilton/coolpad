...
class WC_Report_Sales_By_Product extends WC_Admin_Report {
	...
	public function get_chart_legend() {
		...
		$total_items = absint($this->get_order_report_data([
			                                                   'data'         => [
				                                                   '_qty' => [
					                                                   'type'            => 'order_item_meta',
					                                                   'order_item_type' => 'line_item',
		...
		$legend[] = [
			'title'            => sprintf(__('%s sales for the selected items', 'woocommerce'),
			                              '<strong>' . wc_price($total_sales) . '</strong>'),
	...
	public function get_main_chart() {
		...
		if (empty($this->product_ids)) {
			...
			$order_item_counts  = $this->prepare_chart_data($order_item_counts, 'post_date', 'order_item_count',
			                                                $this->chart_interval, $this->start_date,
			                                                $this->chart_groupby);
			$order_item_amounts = $this->prepare_chart_data($order_item_amounts, 'post_date', 'order_item_amount',
			                                                $this->chart_interval, $this->start_date,
			                                                $this->chart_groupby);
	...
	public function current_filters() {
		...
		echo '<p><a class="button" href="' . esc_url(remove_query_arg('product_ids')) . '">' . __('Reset',
		                                                                                          'woocommerce') . '</a></p>';
	}
				...
				if ($top_sellers) {
					foreach ($top_sellers as $product) {
						echo '<tr class="' . (in_array($product->product_id, $this->product_ids) ? 'active' : '') . '">
							...
							<td class="name"><a href="' . esc_url(add_query_arg('product_ids',
						                                                        $product->product_id)) . '">' . get_the_title($product->product_id) . '</a></td>
