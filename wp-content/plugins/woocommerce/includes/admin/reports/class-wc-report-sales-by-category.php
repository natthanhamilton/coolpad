...
class WC_Report_Sales_By_Category extends WC_Admin_Report {
	...
	public function __construct() {
		if (isset($_GET['show_categories'])) {
			$this->show_categories = is_array($_GET['show_categories']) ? array_map('absint', $_GET['show_categories'])
				: [absint($_GET['show_categories'])];
		}
	}
	...
	public function get_chart_legend() {
		...
		foreach ($this->show_categories as $category) {
			...
			$legend[] = [
				'title'            => sprintf(__('%s sales in %s', 'woocommerce'),
				                              '<strong>' . wc_price($total) . '</strong>', $category->name),
				'color'            => isset($this->chart_colours[ $index ]) ? $this->chart_colours[ $index ]
					: $this->chart_colours[0],
	...
	public function get_products_in_category($category_id) {
		...
		return array_unique(apply_filters('woocommerce_report_sales_by_category_get_products_in_category', $product_ids,
		                                  $category_id));
	}
		...
		if (!empty($this->show_categories)) {
			...
			if (is_array($order_items)) {
				foreach ($order_items as $order_item) {
					...
					$this->item_sales_and_times[ $time ][ $order_item->product_id ]
						= isset($this->item_sales_and_times[ $time ][ $order_item->product_id ])
						? $this->item_sales_and_times[ $time ][ $order_item->product_id ] + $order_item->order_item_amount
						: $order_item->order_item_amount;
					$this->item_sales[ $order_item->product_id ] = isset($this->item_sales[ $order_item->product_id ])
						? $this->item_sales[ $order_item->product_id ] + $order_item->order_item_amount
						: $order_item->order_item_amount;
				}
			}
		}
		...
		if (empty($this->show_categories)) {
			...
			foreach ($this->show_categories as $category) {
				...
				for ($i = 0; $i <= $this->chart_interval; $i++) {
					...
					$category_chart_data[] = [$time, (float)wc_format_decimal($interval_total,
		...
		<form method="GET">
			<div>
				<select multiple="multiple"
				        data-placeholder="<?php esc_attr_e('Select categories&hellip;', 'woocommerce'); ?>"
