	...
	<td class="name">
		...
		<div class="edit" style="display: none;">
			...
			<input type="text" class="shipping_method_name"
			       ...
			       value="<?php echo (isset($item['name'])) ? wc_clean($item['name']) : ''; ?>"/>
			<select class="shipping_method" name="shipping_method[<?php echo esc_attr($item_id); ?>]">
				<optgroup label="<?php esc_attr_e('Shipping Method', 'woocommerce'); ?>">
					...
					foreach ($shipping_methods as $method) {
						...
						echo '<option value="' . esc_attr($current_method) . '" ' . selected($method_id == $current_method,
						                                                                     TRUE,
						                                                                     FALSE) . '>' . esc_html($method->get_title()) . '</option>';
						if ($method_id == $current_method) {
							$found_method = TRUE;
						}
					}
					if (!$found_method && !empty($method_id)) {
						echo '<option value="' . esc_attr($method_id) . '" selected="selected">' . __('Other',
						                                                                              'woocommerce') . '</option>';
					} else {
			...
			<td class="line_tax" width="1%">
				<div class="view">
					...
					echo ('' != $tax_item_total) ? wc_price(wc_round_tax_total($tax_item_total),
					                                        ['currency' => $order->get_order_currency()]) : '&ndash;';
				...
				<div class="edit" style="display: none;">
					<input type="text"
					       ...
					       placeholder="<?php echo wc_format_localized_price(0); ?>"
					       value="<?php echo (isset($tax_item_total))
						       ? esc_attr(wc_format_localized_price($tax_item_total)) : ''; ?>"
