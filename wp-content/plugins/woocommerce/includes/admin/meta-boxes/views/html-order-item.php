...
$thumbnail     = $_product ? apply_filters('woocommerce_admin_order_item_thumbnail',
                                           $_product->get_image('thumbnail', ['title' => ''], FALSE), $item_id, $item)
	: '';
$tax_data      = empty($legacy_order) && wc_tax_enabled() ? maybe_unserialize(isset($item['line_tax_data'])
	                                                                              ? $item['line_tax_data'] : '')
	: FALSE;
...
?>
<tr class="item <?php echo apply_filters('woocommerce_admin_html_order_item_class', (!empty($class) ? $class : ''),
                                         $item, $order); ?>" data-order_item_id="<?php echo $item_id; ?>">
	<td class="thumb">
		<?php
	...
	<td class="name" data-sort-value="<?php echo esc_attr($item['name']); ?>">
		<?php
		echo $product_link
			? '<a href="' . esc_url($product_link) . '" class="wc-order-item-name">' . esc_html($item['name']) . '</a>'
			: '<div class="class="wc-order-item-name"">' . esc_html($item['name']) . '</div>';
		if ($_product && $_product->get_sku()) {
			echo '<div class="wc-order-item-sku"><strong>' . __('SKU:',
			                                                    'woocommerce') . '</strong> ' . esc_html($_product->get_sku()) . '</div>';
		}
	...
	<td class="item_cost" width="1%"
		...
		<div class="view">
			...
			if (isset($item['line_total'])) {
				echo wc_price($order->get_item_total($item, FALSE, TRUE), ['currency' => $order->get_order_currency()]);
				if (isset($item['line_subtotal']) && $item['line_subtotal'] != $item['line_total']) {
					echo '<span class="wc-order-item-discount">-' . wc_price(wc_format_decimal($order->get_item_subtotal($item,
					                                                                                                     FALSE,
					                                                                                                     FALSE) - $order->get_item_total($item,
					                                                                                                                                     FALSE,
					                                                                                                                                     FALSE),
					                                                                           ''),
	...
	<td class="line_cost" width="1%"
		...
		<div class="view">
			<?php
			if (isset($item['line_total'])) {
				echo wc_price($item['line_total'], ['currency' => $order->get_order_currency()]);
			}
			if (isset($item['line_subtotal']) && $item['line_subtotal'] !== $item['line_total']) {
				echo '<span class="wc-order-item-discount">-' . wc_price(wc_format_decimal($item['line_subtotal'] - $item['line_total'],
				                                                                           ''),
				                                                         ['currency' => $order->get_order_currency()]) . '</span>';
			}
			if ($refunded = $order->get_total_refunded_for_item($item_id)) {
				echo '<small class="refunded">' . wc_price($refunded,
				                                           ['currency' => $order->get_order_currency()]) . '</small>';
			}
			?>
		</div>
		<div class="edit" style="display: none;">
			<div class="split-input">
				<div class="input">
					...
					<input type="text" name="line_subtotal[<?php echo absint($item_id); ?>]"
					       ...
					       value="<?php echo $item_subtotal; ?>" class="line_subtotal wc_input_price"
		...
		foreach ($order_taxes as $tax_item) {
			...
			<td class="line_tax" width="1%">
				<div class="view">
					...
					if (isset($item['line_subtotal']) && $item['line_subtotal'] !== $item['line_total']) {
						echo '<span class="wc-order-item-discount">-' . wc_price(wc_round_tax_total($tax_item_subtotal - $tax_item_total),
						                                                         ['currency' => $order->get_order_currency()]) . '</span>';
					}
		...
		<div class="wc-order-edit-line-item-actions">
			<?php if ($order->is_editable()) : ?>
				<a class="edit-order-item tips" href="#"
				   data-tip="<?php esc_attr_e('Edit item', 'woocommerce'); ?>"></a><a class="delete-order-item tips"
				                                                                      href="#"
				                                                                      data-tip="<?php esc_attr_e('Delete item',
				                                                                                                 'woocommerce'); ?>"></a>
			<?php endif; ?>
		</div>
	</td>
