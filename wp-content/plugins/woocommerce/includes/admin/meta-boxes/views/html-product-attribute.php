				...
				<td rowspan="3">
					...
					<?php if ($attribute['is_taxonomy']) : ?>
						<?php if ('select' === $attribute_taxonomy->attribute_type) : ?>
							...
							<select multiple="multiple"
								...
								if ($all_terms) {
									foreach ($all_terms as $term) {
										echo '<option value="' . esc_attr($term->slug) . '" ' . selected(has_term(absint($term->term_id),
										                                                                          $taxonomy,
										                                                                          $thepostid),
										                                                                 TRUE,
										                                                                 FALSE) . '>' . esc_attr(apply_filters('woocommerce_product_attribute_term_name',
										                                                                                                       $term->name,
										                                                                                                       $term)) . '</option>';
									}
								}
							...
							<button class="button plus select_all_attributes"><?php _e('Select all',
							                                                           'woocommerce'); ?></button>
							<button class="button minus select_no_attributes"><?php _e('Select none',
							                                                           'woocommerce'); ?></button>
							<button class="button fr plus add_new_attribute"><?php _e('Add new',
							                                                          'woocommerce'); ?></button>
						...
						<?php elseif ('text' == $attribute_taxonomy->attribute_type) : ?>
							...
							echo esc_attr(implode(' ' . WC_DELIMITER . ' ',
							                      wp_get_post_terms($thepostid, $taxonomy, ['fields' => 'names'])));
							?>" placeholder="<?php echo esc_attr(sprintf(__('"%s" separate terms', 'woocommerce'),
							                                             WC_DELIMITER)); ?>"/>
					...
					<?php else : ?>
						...
						<textarea name="attribute_values[<?php echo $i; ?>]" cols="5" rows="5"
						          placeholder="<?php echo esc_attr(sprintf(__('Enter some text, or some attributes by "%s" separating values.',
						                                                      'woocommerce'),
						                                                   WC_DELIMITER)); ?>"><?php echo esc_textarea($attribute['value']); ?></textarea>
