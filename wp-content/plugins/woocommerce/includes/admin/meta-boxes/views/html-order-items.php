		...
		<tr>
			...
			if (empty($legacy_order) && !empty($order_taxes)) :
				foreach ($order_taxes as $tax_id => $tax_item) :
					...
					$tax_class_name = isset($classes_options[ $tax_class ]) ? $classes_options[ $tax_class ]
						: __('Tax', 'woocommerce');
...
<div class="wc-order-data-row wc-order-item-bulk-edit" style="display:none;">
	<button type="button" class="button bulk-delete-items"><?php _e('Delete selected row(s)',
	                                                                'woocommerce'); ?></button>
...
<div class="wc-order-data-row wc-order-totals-items wc-order-items-editable">
		...
		<div class="wc-used-coupons">
			<ul class="wc_coupon_list"><?php
				...
				foreach ($coupons as $item_id => $item) {
					$post_id
						= $wpdb->get_var($wpdb->prepare("SELECT ID FROM {$wpdb->posts} WHERE post_title = %s AND post_type = 'shop_coupon' AND post_status = 'publish' LIMIT 1;",
						                                $item['name']));
					$link = $post_id
						? add_query_arg(['post' => $post_id, 'action' => 'edit'], admin_url('post.php'))
						: add_query_arg(['s' => $item['name'], 'post_status' => 'all', 'post_type' => 'shop_coupon'],
						                admin_url('edit.php'));
					echo '<li class="code"><a href="' . esc_url($link) . '" class="tips" data-tip="' . esc_attr(wc_price($item['discount_amount'],
					                                                                                                     ['currency' => $order->get_order_currency()])) . '"><span>' . esc_html($item['name']) . '</span></a></li>';
				}
				?></ul>
		</div>
		<?php
	...
	<table class="wc-order-totals">
		<tr>
			<td class="label"><?php echo wc_help_tip(__('This is the total discount. Discounts are defined per line item.',
			                                            'woocommerce')); ?> <?php _e('Discount', 'woocommerce'); ?>:
			</td>
			<td width="1%"></td>
			<td class="total">
				<?php echo wc_price($order->get_total_discount(), ['currency' => $order->get_order_currency()]); ?>
			</td>
		</tr>
		...
		<tr>
			<td class="label"><?php echo wc_help_tip(__('This is the shipping and handling total costs for the order.',
			                                            'woocommerce')); ?> <?php _e('Shipping', 'woocommerce'); ?>:
			</td>
			<td width="1%"></td>
			<td class="total"><?php
				if (($refunded = $order->get_total_shipping_refunded()) > 0) {
					echo '<del>' . strip_tags(wc_price($order->get_total_shipping(),
					                                   ['currency' => $order->get_order_currency()])) . '</del> <ins>' . wc_price($order->get_total_shipping() - $refunded,
					                                                                                                              ['currency' => $order->get_order_currency()]) . '</ins>';
				} else {
					echo wc_price($order->get_total_shipping(), ['currency' => $order->get_order_currency()]);
				}
				?></td>
		</tr>
				...
				<tr>
					...
					<td class="total"><?php
						if (($refunded = $order->get_total_tax_refunded_by_rate_id($tax->rate_id)) > 0) {
							echo '<del>' . strip_tags($tax->formatted_amount) . '</del> <ins>' . wc_price(WC_Tax::round($tax->amount,
							                                                                                            wc_get_price_decimals()) - WC_Tax::round($refunded,
			...
			<td class="total">
				...
				<div class="edit" style="display: none;">
					<input type="text" class="wc_input_price" id="_order_total" name="_order_total"
					       ...
					       value="<?php echo (isset($data['_order_total'][0]))
						       ? esc_attr(wc_format_localized_price($data['_order_total'][0])) : ''; ?>"/>
					<div class="clear"></div>
				</div>
			</td>
		</tr>
		...
		<tr>
			...
			<td width="1%"></td>
			<td class="total refunded-total">-<?php echo wc_price($order->get_total_refunded(),
			                                                      ['currency' => $order->get_order_currency()]); ?></td>
		</tr>
...
<div class="wc-order-data-row wc-order-bulk-actions wc-order-data-row-toggle">
	<p class="add-items">
		...
		<?php else : ?>
			<span
				class="description"><?php echo wc_help_tip(__('To edit this order change the status back to "Pending"',
			                                                  'woocommerce')); ?><?php _e('This order is no longer editable.',
			                                                                              'woocommerce'); ?></span>
		<?php endif; ?>
		...
		?>
		<?php if ($order->is_editable()) : ?>
			<button type="button" class="button button-primary calculate-tax-action"><?php _e('Calculate Taxes',
			                                                                                  'woocommerce'); ?></button>
			<button type="button" class="button button-primary calculate-action"><?php _e('Calculate Total',
			                                                                              'woocommerce'); ?></button>
		<?php endif; ?>
	</p>
</div>
	...
	<div class="wc-order-data-row wc-order-refund-items wc-order-data-row-toggle" style="display: none;">
		<table class="wc-order-totals">
			<tr style="display:none;">
				<td class="label"><label for="restock_refunded_items"><?php _e('Restock refunded items',
			...
			<tr>
				...
				<td class="total">-<?php echo wc_price($order->get_total_refunded(),
				                                       ['currency' => $order->get_order_currency()]); ?></td>
			</tr>
			<tr>
				<td class="label"><?php _e('Total available to refund', 'woocommerce'); ?>:</td>
				<td class="total"><?php echo wc_price($order->get_total() - $order->get_total_refunded(),
				                                      ['currency' => $order->get_order_currency()]); ?></td>
			</tr>
			<tr>
			...
			<tr>
				<td class="label"><label
						for="refund_reason"><?php echo wc_help_tip(__('Note: the refund reason will be visible by the customer.',
				                                                      'woocommerce')); ?> <?php _e('Reason for refund (optional)',
				                                                                                   'woocommerce'); ?>
						:</label></td>
				<td class="total">
		...
		<div class="refund-actions">
			<?php
			$refund_amount            = '<span class="wc-order-refund-amount">' . wc_price(0,
			                                                                               ['currency' => $order->get_order_currency()]) . '</span>';
			$gateway_supports_refunds = FALSE !== $payment_gateway && $payment_gateway->supports('refunds');
			$gateway_name             = FALSE !== $payment_gateway ? (!empty($payment_gateway->method_title)
				? $payment_gateway->method_title : $payment_gateway->get_title())
				: __('Payment Gateway', 'woocommerce');
			?>
			<button type="button" class="button <?php echo $gateway_supports_refunds ? 'button-primary do-api-refund'
				: 'tips disabled'; ?>" <?php echo $gateway_supports_refunds
				? ''
				: 'data-tip="' . esc_attr__('The payment gateway used to place this order does not support automatic refunds.',
				                            'woocommerce') . '"'; ?>><?php printf(_x('Refund %s via %s',
			                                                                         'Refund $amount', 'woocommerce'),
			                                                                      $refund_amount,
			                                                                      $gateway_name); ?></button>
			<button type="button" class="button button-primary do-manual-refund tips"
			        data-tip="<?php esc_attr_e('You will need to manually issue a refund through your payment gateway after using this.',
			                                   'woocommerce'); ?>"><?php printf(_x('Refund %s manually',
			                                                                       'Refund $amount manually',
			                                                                       'woocommerce'),
			                                                                    $refund_amount); ?></button>
			<button type="button" class="button cancel-action"><?php _e('Cancel', 'woocommerce'); ?></button>
			<div class="clear"></div>
		</div>
	</div>
	...
	<div class="wc-backbone-modal">
		<div class="wc-backbone-modal-content">
			<section class="wc-backbone-modal-main" role="main">
				...
				<footer>
					<div class="inner">
						<button id="btn-ok" class="button button-primary button-large"><?php _e('Add',
						                                                                        'woocommerce'); ?></button>
					</div>
				</footer>
			</section>
				...
				<article>
					<form action="" method="post">
						<table class="widefat">
							...
							<?php
							$rates
								= $wpdb->get_results("SELECT * FROM {$wpdb->prefix}woocommerce_tax_rates ORDER BY tax_rate_name LIMIT 100");
							foreach ($rates as $rate) {
								echo '
									<tr>
										...
										<td>' . (isset($classes_options[ $rate->tax_rate_class ])
										? $classes_options[ $rate->tax_rate_class ] : '-') . '</td>
						...
						<?php if (absint($wpdb->get_var("SELECT COUNT(tax_rate_id) FROM {$wpdb->prefix}woocommerce_tax_rates;")) > 100) : ?>
							<p>
								<label for="manual_tax_rate_id"><?php _e('Or, enter tax rate ID:',
								                                         'woocommerce'); ?></label><br/>
								<input type="number" name="manual_tax_rate_id" id="manual_tax_rate_id" step="1"
								       placeholder="<?php esc_attr_e('Optional', 'woocommerce'); ?>"/>
							</p>
						<?php endif; ?>
					</form>
				</article>
