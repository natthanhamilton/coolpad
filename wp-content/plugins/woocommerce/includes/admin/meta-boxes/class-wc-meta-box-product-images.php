...
class WC_Meta_Box_Product_Images {
	...
	public static function output($post) {
		...
		<div id="product_images_container">
			<ul class="product_images">
				...
				if (metadata_exists('post', $post->ID, '_product_image_gallery')) {
					...
					// Backwards compat
					$attachment_ids
						                   = get_posts('post_parent=' . $post->ID . '&numberposts=-1&post_type=attachment&orderby=menu_order&order=ASC&post_mime_type=image&fields=ids&meta_key=_woocommerce_exclude_image&meta_value=0');
		...
		<p class="add_product_images hide-if-no-js">
			<a href="#" data-choose="<?php esc_attr_e('Add Images to Product Gallery', 'woocommerce'); ?>"
			   ...
			   data-text="<?php esc_attr_e('Delete', 'woocommerce'); ?>"><?php _e('Add product gallery images',
			                                                                      'woocommerce'); ?></a>
		</p>
		<?php
	}
	...
	public static function save($post_id, $post) {
		$attachment_ids = isset($_POST['product_image_gallery']) ? array_filter(explode(',',
