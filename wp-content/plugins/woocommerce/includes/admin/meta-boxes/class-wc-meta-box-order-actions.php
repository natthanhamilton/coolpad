		...
		<ul class="order_actions submitbox">
			...
			<li class="wide" id="actions">
				<select name="wc_order_action">
					...
					<optgroup label="<?php esc_attr_e('Resend order emails', 'woocommerce'); ?>">
						<?php
						$mailer           = WC()->mailer();
						$available_emails = apply_filters('woocommerce_resend_order_emails_available',
						                                  ['new_order', 'cancelled_order', 'customer_processing_order', 'customer_completed_order', 'customer_invoice', 'customer_refunded_order']);
					...
					<option value="regenerate_download_permissions"><?php _e('Regenerate download permissions',
					                                                         'woocommerce'); ?></option>

					<?php foreach (apply_filters('woocommerce_order_actions', []) as $action => $title) { ?>
						<option value="<?php echo $action; ?>"><?php echo $title; ?></option>
					<?php } ?>
				</select>

				<button class="button wc-reload" title="<?php esc_attr_e('Apply', 'woocommerce'); ?>">
			...
			<li class="wide">
				...
				<input type="submit" class="button save_order button-primary tips" name="save"
				       ...
				       data-tip="<?php printf(__('Save/update the %s', 'woocommerce'),
				...
				if (!empty($mails)) {
					foreach ($mails as $mail) {
						if ($mail->id == $email_to_send) {
							...
							$order->add_order_note(sprintf(__('%s email notification manually sent.', 'woocommerce'),
							                               $mail->title), FALSE, TRUE);
						}
					}
				}
