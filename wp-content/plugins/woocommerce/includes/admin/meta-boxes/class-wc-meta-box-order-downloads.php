			...
			<div class="toolbar">
				<p class="buttons">
					<input type="hidden" id="grant_access_id" name="grant_access_id" data-multiple="true"
					       ...
					       data-placeholder="<?php esc_attr_e('Search for a downloadable product&hellip;',
					                                          'woocommerce'); ?>"
					       data-action="woocommerce_json_search_downloadable_products_and_variations"/>
					<button type="button" class="button grant_access"><?php _e('Grant Access',
					                                                           'woocommerce'); ?></button>
				</p>
				<div class="clear"></div>
			</div>
	...
	public static function save($post_id, $post) {
		...
		if (isset($_POST['download_id'])) {
			...
			for ($i = 0; $i <= $product_ids_max; $i++) {
				...
				$expiry = (array_key_exists($i, $access_expires) && '' != $access_expires[ $i ]) ? date_i18n('Y-m-d',
				                                                                                             strtotime($access_expires[ $i ]))
					: NULL;
