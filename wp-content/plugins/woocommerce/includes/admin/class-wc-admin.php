...
class WC_Admin {
	...
	public function admin_redirects() {
		...
		if (!empty($_GET['wc-install-plugin-redirect'])) {
			...
			if (current_user_can('install_plugins') && in_array($plugin_slug, ['woocommerce-gateway-stripe'])) {
				$nonce = wp_create_nonce('install-plugin_' . $plugin_slug);
				$url
				       = self_admin_url('update.php?action=install-plugin&plugin=' . $plugin_slug . '&_wpnonce=' . $nonce);
			} else {
		...
		if (get_transient('_wc_activation_redirect')) {
			...
			if ((!empty($_GET['page']) && in_array($_GET['page'],
			                                       ['wc-setup'])) || is_network_admin() || isset($_GET['activate-multi']) || !current_user_can('manage_woocommerce') || apply_filters('woocommerce_prevent_automatic_wizard_redirect',
			                                                                                                                                                                          FALSE)
			) {
				return;
			}
	...
	public function prevent_admin_access() {
		...
		if ('yes' === get_option('woocommerce_lock_down_admin',
		                         'yes') && !is_ajax() && basename($_SERVER["SCRIPT_FILENAME"]) !== 'admin-post.php'
		) {
			$has_cap = FALSE;
	...
	public function preview_emails() {
		if (isset($_GET['preview_woocommerce_mail'])) {
			...
			$message = apply_filters('woocommerce_mail_content',
			                         $email->style_inline($mailer->wrap_message($email_heading, $message)));
	...
	public function admin_footer_text($footer_text) {
		...
		if (isset($current_screen->id) && apply_filters('woocommerce_display_admin_footer_text',
		                                                in_array($current_screen->id, $wc_pages))
		) {
			// Change the footer text
			if (!get_option('woocommerce_admin_footer_text_rated')) {
				$footer_text
					= sprintf(__('If you like <strong>WooCommerce</strong> please leave us a %s&#9733;&#9733;&#9733;&#9733;&#9733;%s rating. A huge thanks in advance!',
					             'woocommerce'),
					          '<a href="https://wordpress.org/support/view/plugin-reviews/woocommerce?filter=5#postform" target="_blank" class="wc-rating-link" data-rated="' . esc_attr__('Thanks :)',
					                                                                                                                                                                       'woocommerce') . '">',
					          '</a>');
				wc_enqueue_js("
