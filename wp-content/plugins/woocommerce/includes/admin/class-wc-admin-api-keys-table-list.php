...
class WC_Admin_API_Keys_Table_List extends WP_List_Table {
	...
	public function column_description($key) {
		...
		$actions = [
			...
			'trash' => '<a class="submitdelete" title="' . esc_attr__('Revoke API Key',
			                                                          'woocommerce') . '" href="' . esc_url(wp_nonce_url(add_query_arg(['revoke-key' => $key['key_id']],
	...
	public function column_user($key) {
		...
		if (current_user_can('edit_user', $user->ID)) {
			return '<a href="' . esc_url(add_query_arg(['user_id' => $user->ID],
			                                           admin_url('user-edit.php'))) . '">' . esc_html($user->display_name) . '</a>';
		}
