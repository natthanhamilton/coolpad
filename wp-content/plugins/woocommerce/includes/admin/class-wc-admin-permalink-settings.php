...
if (!class_exists('WC_Admin_Permalink_Settings')) :
	...
	class WC_Admin_Permalink_Settings {
		...
		public function settings_init() {
			...
			add_settings_section('woocommerce-permalink', __('Product Permalinks', 'woocommerce'), [$this, 'settings'],
			                     'permalink');
			// Add our settings
			add_settings_field(
				'woocommerce_product_category_slug',            // id
				__('Product category base', 'woocommerce'),   // setting title
				[$this, 'product_category_slug_input'],  // display callback
			...
			add_settings_field(
				...
				[$this, 'product_tag_slug_input'],       // display callback
				'permalink',                                    // settings page
				'optional'                                      // settings section
			);
			add_settings_field(
				...
				[$this, 'product_attribute_slug_input'], // display callback
				'permalink',                                    // settings page
				'optional'                                      // settings section
			);
		}
		...
		public function product_category_slug_input() {
			...
			<input name="woocommerce_product_category_slug" type="text" class="regular-text code"
			       ...
			       placeholder="<?php echo esc_attr_x('product-category', 'slug', 'woocommerce') ?>"/>
			<?php
		}
		...
		public function product_tag_slug_input() {
			...
			<input name="woocommerce_product_tag_slug" type="text" class="regular-text code"
			       ...
			       placeholder="<?php echo esc_attr_x('product-tag', 'slug', 'woocommerce') ?>"/>
			<?php
		}
		...
		public function settings() {
			...
			$base_slug    = urldecode(($shop_page_id > 0 && get_post($shop_page_id)) ? get_page_uri($shop_page_id)
			...
			<table class="form-table wc-permalink-structure">
				<tbody>
				<tr>
					<th><label><input name="product_permalink" type="radio"
					                  value="<?php echo esc_attr($structures[0]); ?>"
					                  class="wctog" <?php checked($structures[0],
					                                              $product_permalink); ?> /> <?php _e('Default',
					                                                                                  'woocommerce'); ?>
						</label></th>
					<td><code class="default-example"><?php echo esc_html(home_url()); ?>/?product=sample-product</code>
						<code class="non-default-example"><?php echo esc_html(home_url()); ?>
							/<?php echo esc_html($product_base); ?>/sample-product/</code></td>
				</tr>
				<?php if ($shop_page_id) : ?>
					<tr>
						<th><label><input name="product_permalink" type="radio"
						                  value="<?php echo esc_attr($structures[1]); ?>"
						                  class="wctog" <?php checked($structures[1],
						                                              $product_permalink); ?> /> <?php _e('Shop base',
						                                                                                  'woocommerce'); ?>
							</label></th>
						<td><code><?php echo esc_html(home_url()); ?>/<?php echo esc_html($base_slug); ?>
								/sample-product/</code></td>
					</tr>
					<tr>
						<th><label><input name="product_permalink" type="radio"
						                  value="<?php echo esc_attr($structures[2]); ?>"
						                  class="wctog" <?php checked($structures[2],
						                                              $product_permalink); ?> /> <?php _e('Shop base with category',
						                                                                                  'woocommerce'); ?>
							</label></th>
						<td><code><?php echo esc_html(home_url()); ?>/<?php echo esc_html($base_slug); ?>
				...
				<tr>
					<th><label><input name="product_permalink" id="woocommerce_custom_selection" type="radio"
					                  value="custom" class="tog" <?php checked(in_array($product_permalink,
					                                                                    $structures), FALSE); ?> />
					...
					<td>
						<input name="product_permalink_structure" id="woocommerce_permalink_structure" type="text"
							...
							class="description"><?php _e('Enter a custom base to use. A base <strong>must</strong> be set or WordPress will use default instead.',
						                                 'woocommerce'); ?></span>
					</td>
				</tr>
				</tbody>
		...
		public function settings_save() {
			...
			if (isset($_POST['permalink_structure'])) {
				...
				$permalinks['attribute_base']
				                             = wc_sanitize_permalink(trim($_POST['woocommerce_product_attribute_slug']));
				// Product base.
				...
				if ('custom' === $product_permalink) {
					if (isset($_POST['product_permalink_structure'])) {
						$product_permalink = preg_replace('#/+#', '/', '/' . str_replace('#', '',
						                                                                 trim($_POST['product_permalink_structure'])));
					} else {
						$product_permalink = '/';
					}
