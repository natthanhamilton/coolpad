...
function wc_template_redirect() {
	...
	elseif (is_search() && is_post_type_archive('product') && apply_filters('woocommerce_redirect_single_search_result',
	                                                                        TRUE) && 1 === absint($wp_query->found_posts)
	) {
...
/**
 * When loading sensitive checkout or account pages, send a HTTP header to limit rendering of pages to same origin
 * iframes for security reasons.
 *
...
function wc_setup_product_data($post) {
	...
	if (empty($post->post_type) || !in_array($post->post_type, ['product', 'product_variation'])) {
		return;
	}
...
function wc_products_rss_feed() {
	...
	if (is_post_type_archive('product') || is_singular('product')) {
		...
		echo '<link rel="alternate" type="application/rss+xml"  title="' . esc_attr__('New products',
		                                                                              'woocommerce') . '" href="' . esc_url($feed) . '" />';
	} elseif (is_tax('product_cat')) {
		$term = get_term_by('slug', esc_attr(get_query_var('product_cat')), 'product_cat');
		if ($term) {
			$feed = add_query_arg('product_cat', $term->slug, get_post_type_archive_feed_link('product'));
			echo '<link rel="alternate" type="application/rss+xml"  title="' . esc_attr(sprintf(__('New products added to %s',
			                                                                                       'woocommerce'),
			                                                                                    $term->name)) . '" href="' . esc_url($feed) . '" />';
		}
	} elseif (is_tax('product_tag')) {
		$term = get_term_by('slug', esc_attr(get_query_var('product_tag')), 'product_tag');
		if ($term) {
			$feed = add_query_arg('product_tag', $term->slug, get_post_type_archive_feed_link('product'));
			echo '<link rel="alternate" type="application/rss+xml"  title="' . sprintf(__('New products tagged %s',
			                                                                              'woocommerce'),
			                                                                           urlencode($term->name)) . '" href="' . esc_url($feed) . '" />';
		}
	}
}
...
function wc_get_loop_class() {
	...
	$woocommerce_loop['columns'] = !empty($woocommerce_loop['columns']) ? $woocommerce_loop['columns']
...
if (!function_exists('woocommerce_demo_store')) {
	...
	function woocommerce_demo_store() {
		...
		if (empty($notice)) {
			$notice = __('This is a demo store for testing purposes &mdash; no orders shall be fulfilled.',
			             'woocommerce');
		}
		...
		<h3>
			...
			if ($category->count > 0) {
				echo apply_filters('woocommerce_subcategory_count_html',
				                   ' <mark class="count">(' . $category->count . ')</mark>', $category);
			}
			?>
		</h3>
		<?php
...
if (!function_exists('woocommerce_taxonomy_archive_description')) {
	...
	function woocommerce_taxonomy_archive_description() {
		if (is_tax(['product_cat', 'product_tag']) && 0 === absint(get_query_var('paged'))) {
...
if (!function_exists('woocommerce_catalog_ordering')) {
	...
	function woocommerce_catalog_ordering() {
		...
		wc_get_template('loop/orderby.php',
		                ['catalog_orderby_options' => $catalog_orderby_options, 'orderby' => $orderby, 'show_default_orderby' => $show_default_orderby]);
	}
}
...
if (!function_exists('woocommerce_variable_add_to_cart')) {
	...
	function woocommerce_variable_add_to_cart() {
		...
		$get_variations = sizeof($product->get_children()) <= apply_filters('woocommerce_ajax_variation_threshold', 30,
		                                                                    $product);
		// Load the template
...
if (!function_exists('woocommerce_comments')) {
	...
	function woocommerce_comments($comment, $args, $depth) {
		...
		wc_get_template('single-product/review.php', ['comment' => $comment, 'args' => $args, 'depth' => $depth]);
	}
}
...
if (!function_exists('woocommerce_related_products')) {
	 ...
	 */
	function woocommerce_related_products($args = [], $columns = FALSE, $orderby = FALSE) {
		if (!is_array($args)) {
			_deprecated_argument(__FUNCTION__, '2.1',
			                     __('Use $args argument as an array instead. Deprecated argument will be removed in WC 2.2.',
			                        'woocommerce'));
			$argsvalue = $args;
			$args = [
...
if (!function_exists('woocommerce_checkout_login_form')) {
	...
	function woocommerce_checkout_login_form() {
		wc_get_template('checkout/form-login.php', ['checkout' => WC()->checkout()]);
	}
}
...
if (!function_exists('woocommerce_products_will_display')) {
	...
	function woocommerce_products_will_display() {
		...
		$transient_name
			= 'wc_products_will_display_' . $term->term_id . '_' . WC_Cache_Helper::get_transient_version('product_query');
		if (FALSE === ($products_will_display = get_transient($transient_name))) {
			$has_children
				= $wpdb->get_col($wpdb->prepare("SELECT term_id FROM {$wpdb->term_taxonomy} WHERE parent = %d AND taxonomy = %s",
...
if (!function_exists('woocommerce_order_again_button')) {
	...
	function woocommerce_order_again_button($order) {
		...
		wc_get_template('order/order-again.php', [
			'order' => $order
		]);
	}
}
	...
	function woocommerce_form_field($key, $args, $value = NULL) {
		...
		switch ($args['type']) {
			case 'country' :
				...
				if (1 === sizeof($countries)) {
					...
					$field
						= '<select name="' . esc_attr($key) . '" id="' . esc_attr($args['id']) . '" ' . $args['autocomplete'] . ' class="country_to_state country_select ' . esc_attr(implode(' ',
					...
					$field .= '</select>';
					$field .= '<noscript><input type="submit" name="woocommerce_checkout_update_totals" value="' . esc_attr__('Update country',
					                                                                                                          'woocommerce') . '" /></noscript>';
				}
				break;
			case 'state' :
				/* Get Country */
				...
				if (is_array($states) && empty($states)) {
					...
					$field .= '<select name="' . esc_attr($key) . '" id="' . esc_attr($args['id']) . '" class="state_select ' . esc_attr(implode(' ',
					                                                                                                                             $args['input_class'])) . '" ' . implode(' ',
					                                                                                                                                                                     $custom_attributes) . ' data-placeholder="' . esc_attr($args['placeholder']) . '" ' . $args['autocomplete'] . '>
					...
					$field .= '<input type="text" class="input-text ' . esc_attr(implode(' ',
					                                                                     $args['input_class'])) . '" value="' . esc_attr($value) . '"  placeholder="' . esc_attr($args['placeholder']) . '" ' . $args['autocomplete'] . ' name="' . esc_attr($key) . '" id="' . esc_attr($args['id']) . '" ' . implode(' ',
					                                                                                                                                                                                                                                                                                                   $custom_attributes) . ' />';
				}
				break;
			case 'textarea' :
				$field .= '<textarea name="' . esc_attr($key) . '" class="input-text ' . esc_attr(implode(' ',
				                                                                                          $args['input_class'])) . '" id="' . esc_attr($args['id']) . '" placeholder="' . esc_attr($args['placeholder']) . '" ' . $args['maxlength'] . ' ' . $args['autocomplete'] . ' ' . (empty($args['custom_attributes']['rows'])
						? ' rows="2"' : '') . (empty($args['custom_attributes']['cols']) ? ' cols="5"'
			...
			case 'number' :
				$field .= '<input type="' . esc_attr($args['type']) . '" class="input-text ' . esc_attr(implode(' ',
				                                                                                                $args['input_class'])) . '" name="' . esc_attr($key) . '" id="' . esc_attr($args['id']) . '" placeholder="' . esc_attr($args['placeholder']) . '" ' . $args['maxlength'] . ' ' . $args['autocomplete'] . ' value="' . esc_attr($value) . '" ' . implode(' ',
				                                                                                                                                                                                                                                                                                                                                                        $custom_attributes) . ' />';
				break;
			case 'select' :
				$options = $field = '';
					...
					foreach ($args['options'] as $option_key => $option_text) {
						if ('' === $option_key) {
							...
							if (empty($args['placeholder'])) {
								$args['placeholder'] = $option_text ? $option_text
				...
				if (!empty($args['options'])) {
					foreach ($args['options'] as $option_key => $option_text) {
						$field .= '<input type="radio" class="input-radio ' . esc_attr(implode(' ',
						                                                                       $args['input_class'])) . '" value="' . esc_attr($option_key) . '" name="' . esc_attr($key) . '" id="' . esc_attr($args['id']) . '_' . esc_attr($option_key) . '"' . checked($value,
						                                                                                                                                                                                                                                                   $option_key,
						                                                                                                                                                                                                                                                   FALSE) . ' />';
		...
		if (!empty($field)) {
			...
			if ($args['label'] && 'checkbox' != $args['type']) {
				$field_html .= '<label for="' . esc_attr($label_id) . '" class="' . esc_attr(implode(' ',
...
if (!function_exists('wc_dropdown_variation_attribute_options')) {
	...
	function wc_dropdown_variation_attribute_options($args = []) {
		...
		$html
			= '<select id="' . esc_attr($id) . '" class="' . esc_attr($class) . '" name="' . esc_attr($name) . '" data-attribute_name="attribute_' . esc_attr(sanitize_title($attribute)) . '">';
		...
		if (!empty($options)) {
			if ($product && taxonomy_exists($attribute)) {
				...
				$terms = wc_get_product_terms($product->id, $attribute, ['fields' => 'all']);
				foreach ($terms as $term) {
					if (in_array($term->slug, $options)) {
						$html .= '<option value="' . esc_attr($term->slug) . '" ' . selected(sanitize_title($args['selected']),
						                                                                     $term->slug,
						                                                                     FALSE) . '>' . esc_html(apply_filters('woocommerce_variation_option_name',
						                                                                                                           $term->name)) . '</option>';
					}
				}
			} else {
				foreach ($options as $option) {
					...
					$html .= '<option value="' . esc_attr($option) . '" ' . $selected . '>' . esc_html(apply_filters('woocommerce_variation_option_name',
					                                                                                                 $option)) . '</option>';
				}
			}
		}
