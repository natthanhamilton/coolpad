...
class WC_Gateway_Paypal_PDT_Handler extends WC_Gateway_Paypal_Response {
	...
	public function __construct($sandbox = FALSE, $identity_token = '') {
		add_action('woocommerce_thankyou_paypal', [$this, 'check_response']);
	...
	public function check_response() {
		...
		if ($transaction_result && 'completed' === $status) {
			if ($order->get_total() != $amount) {
				...
				$this->payment_on_hold($order, sprintf(__('Validation error: PayPal amounts do not match (amt %s).',
				                                          'woocommerce'), $amount));
			} else {
	...
	protected function validate_transaction($transaction) {
		...
		$response = wp_safe_remote_post($this->sandbox ? 'https://www.sandbox.paypal.com/cgi-bin/webscr'
			                                : 'https://www.paypal.com/cgi-bin/webscr', $pdt);
