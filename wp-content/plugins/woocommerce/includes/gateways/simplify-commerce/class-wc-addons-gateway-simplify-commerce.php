...
class WC_Addons_Gateway_Simplify_Commerce extends WC_Gateway_Simplify_Commerce {
	...
	public function __construct() {
		...
		if (class_exists('WC_Subscriptions_Order')) {
			...
			add_filter('woocommerce_subscription_validate_payment_meta', [$this, 'validate_subscription_payment_meta'],
			           10, 2);
		}
		if (class_exists('WC_Pre_Orders_Order')) {
			add_action('wc_pre_orders_process_pre_order_completion_payment_' . $this->id,
			           [$this, 'process_pre_order_release_payment']);
		}
		add_filter('woocommerce_simplify_commerce_hosted_args', [$this, 'hosted_payment_args'], 10, 2);
		add_action('woocommerce_api_wc_addons_gateway_simplify_commerce', [$this, 'return_handler']);
	}
	...
	public function scheduled_subscription_payment($amount_to_charge, $renewal_order) {
		...
		if (is_wp_error($result)) {
			$renewal_order->update_status('failed', sprintf(__('Simplify Transaction Failed (%s)', 'woocommerce'),
	...
	public function update_failing_payment_method($subscription, $renewal_order) {
		update_post_meta($subscription->id, '_simplify_customer_id',
		                 get_post_meta($renewal_order->id, '_simplify_customer_id', TRUE));
	}

	/**
	 * Include the payment meta data required to process automatic recurring payments so that store managers can.
	 * manually set up automatic recurring payments for a customer via the Edit Subscription screen in Subscriptions
	 * v2.0+.
	 *
	 * @since 2.4
	 *
