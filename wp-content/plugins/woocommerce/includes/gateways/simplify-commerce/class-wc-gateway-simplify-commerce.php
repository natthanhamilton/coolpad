...
class WC_Gateway_Simplify_Commerce extends WC_Payment_Gateway_CC {
	...
	public function __construct() {
		...
		$this->method_description
		                            = __('Take payments via Simplify Commerce - uses simplify.js to create card tokens and the Simplify Commerce SDK. Requires SSL when sandbox is disabled.',
		                                 'woocommerce');
		...
		$this->public_key  = $this->sandbox == 'no' ? $this->get_option('public_key')
			: $this->get_option('sandbox_public_key');
		$this->private_key = $this->sandbox == 'no' ? $this->get_option('private_key')
		...
		add_action('woocommerce_api_wc_gateway_simplify_commerce', [$this, 'return_handler']);
	}
		...
		$this->form_fields = [
			...
			'mode'                => [
				...
				'description' => sprintf(__('Standard will display the credit card fields on your store (SSL required). %1$s Hosted Payments will display a Simplify Commerce modal dialog on your store (if SSL) or will redirect the customer to Simplify Commerce hosted page (if not SSL). %1$s Note: Hosted Payments need a new API Key pair with the hosted payments flag selected. %2$sFor more details check the Simplify Commerce docs%3$s.',
				                            'woocommerce'), '<br />',
				                         '<a href="https://simplify.desk.com/customer/portal/articles/1792405-how-do-i-enable-hosted-payments" target="_blank">',
				                         '</a>'),
				'default'     => 'standard',
				'options'     => [
	...
	public function save_token($customer_token, $cart_token, $customer_info) {
		if (!is_null($customer_token)) {
			...
			$updates  = ['token' => $cart_token];
			$customer->setAll($updates);
			$customer->updateCustomer();
			$customer
				   = Simplify_Customer::findCustomer($customer_token->get_token()); // get updated customer with new set card
	...
	protected function process_standard_payments($order, $cart_token = '', $customer_token = '') {
		try {
			if (empty($cart_token) && empty($customer_token)) {
				$error_msg
					= __('Please make sure your card details have been entered correctly and that your browser supports JavaScript.',
					     'woocommerce');
				if ('yes' == $this->sandbox) {
	 ...
	 */
	public function do_payment($order, $amount = 0, $token = []) {
		if ($amount * 100 < 50) {
		...
		try {
			...
			$data = [
				...
				'description' => sprintf(__('%s - Order #%s', 'woocommerce'), esc_html(get_bloginfo('name', 'display')),
			...
			return new WP_Error('simplify_payment_declined', $e->getMessage(), ['status' => $e->getCode()]);
		}
	...
	public function get_icon() {
		$icon
			= '<img src="' . WC_HTTPS::force_https_url(WC()->plugin_url() . '/assets/images/icons/credit-cards/visa.svg') . '" alt="Visa" width="32" />';
	...
	public function checks() {
		...
		elseif ('standard' == $this->mode && !wc_checkout_is_https()) {
			echo '<div class="error"><p>' . sprintf(__('Simplify Commerce is enabled, but the <a href="%s">force SSL option</a> is disabled; your checkout may not be secure! Please enable SSL and ensure your server has a valid SSL certificate - Simplify Commerce will only work in sandbox mode.',
			                                           'woocommerce'),
			                                        admin_url('admin.php?page=wc-settings&tab=checkout')) . '</p></div>';
		}
	}
	...
	public function payment_scripts() {
		...
		wp_enqueue_script('simplify-commerce', 'https://www.simplify.com/commerce/v1/simplify.js', ['jquery'],
		                  WC_VERSION, TRUE);
		wp_enqueue_script('wc-simplify-commerce',
		                  WC()->plugin_url() . '/includes/gateways/simplify-commerce/assets/js/simplify-commerce.js',
		                  ['simplify-commerce', 'wc-credit-card-form'], WC_VERSION, TRUE);
		wp_localize_script('wc-simplify-commerce', 'Simplify_commerce_params', [
			'key'           => $this->public_key,
	...
	public function receipt_page($order_id) {
		$order = wc_get_order($order_id);
		echo '<p>' . __('Thank you for your order, please click the button below to pay with credit card using Simplify Commerce by MasterCard.',
		                'woocommerce') . '</p>';
		...
		echo '<script type="text/javascript" src="https://www.simplify.com/commerce/simplify.pay.js"></script>
			<button class="button alt" id="simplify-payment-button" ' . implode(' ', $button_args) . '>' . __('Pay Now',
	...
	public function return_handler() {
		...
		if (isset($_REQUEST['reference']) && isset($_REQUEST['paymentId']) && isset($_REQUEST['signature'])) {
			$signature
				      = strtoupper(md5($_REQUEST['amount'] . $_REQUEST['reference'] . $_REQUEST['paymentId'] . $_REQUEST['paymentDate'] . $_REQUEST['paymentStatus'] . $this->private_key));
			$order_id = absint($_REQUEST['reference']);
			$order    = wc_get_order($order_id);
			if ($signature === $_REQUEST['signature']) {
				$order_complete = $this->process_order_status($order, $_REQUEST['paymentId'],
				                                              $_REQUEST['paymentStatus'], $_REQUEST['paymentDate']);
				if (!$order_complete) {
