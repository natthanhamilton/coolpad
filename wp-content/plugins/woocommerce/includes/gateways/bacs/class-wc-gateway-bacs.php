...
class WC_Gateway_BACS extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_description = __('Allows payments by BACS, more commonly known as direct bank/wire transfer.',
		                               'woocommerce');
		// Load the settings.
		...
		$this->form_fields = [
			...
			'description'     => [
				...
				'description' => __('Payment method description that the customer will see on your checkout.',
				                    'woocommerce'),
				'default'     => __('Make your payment directly into our bank account. Please use your Order ID as the payment reference. Your order won\'t be shipped until the funds have cleared in our account.',
				                    'woocommerce'),
				'desc_tip'    => TRUE,
			],
			'instructions'    => [
	...
	public function generate_account_details_html() {
		...
		$sortcode = isset($locale[ $country ]['sortcode']['label']) ? $locale[ $country ]['sortcode']['label']
				...
				<table class="widefat wc_input_table sortable" cellspacing="0">
					...
					<tr>
						<th colspan="7"><a href="#" class="add button"><?php _e('+ Add Account', 'woocommerce'); ?></a>
							<a href="#" class="remove_rows button"><?php _e('Remove selected account(s)',
							                                                'woocommerce'); ?></a></th>
					</tr>
					</tfoot>
				</table>
	...
	public function get_country_locale() {
		if (empty($this->locale)) {
			...
			$this->locale = apply_filters('woocommerce_get_bacs_locale', [
				'AU' => [
					'sortcode' => [
	...
	private function bank_details($order_id = '') {
		...
		if (!empty($bacs_accounts)) {
			echo '<h2 class="wc-bacs-bank-details-heading">' . __('Our Bank Details',
			                                                      'woocommerce') . '</h2>' . PHP_EOL;
			foreach ($bacs_accounts as $bacs_account) {
				$bacs_account = (object)$bacs_account;
				if ($bacs_account->account_name || $bacs_account->bank_name) {
					echo '<h3>' . wp_unslash(implode(' - ',
					                                 array_filter([$bacs_account->account_name, $bacs_account->bank_name]))) . '</h3>' . PHP_EOL;
				}
