...
class WC_Shortcode_Checkout {
	...
	public static function output($atts) {
		...
		if (isset($_GET['order']) && isset($_GET['key'])) {
			_deprecated_argument(__CLASS__ . '->' . __FUNCTION__, '2.1',
			                     '"order" is no longer used to pass an order ID. Use the order-pay or order-received endpoint instead.');
	...
	private static function order_pay($order_id) {
		...
		if (isset($_GET['pay_for_order']) && isset($_GET['key']) && $order_id) {
			...
			if (!current_user_can('pay_for_order', $order_id)) {
				echo '<div class="woocommerce-error">' . __('Invalid order. If you have an account please log in and try again.',
				                                            'woocommerce') . ' <a href="' . wc_get_page_permalink('myaccount') . '" class="wc-forward">' . __('My Account',
				                                                                                                                                              'woocommerce') . '</a>' . '</div>';
			...
			if ($order->id == $order_id && $order->order_key == $order_key) {
				if ($order->needs_payment()) {
					...
					wc_get_template('checkout/form-pay.php', [
						...
						'order_button_text'  => apply_filters('woocommerce_pay_order_button_text',
						                                      __('Pay for order', 'woocommerce'))
					]);
				} else {
					wc_add_notice(sprintf(__('This order&rsquo;s status is &ldquo;%s&rdquo;&mdash;it cannot be paid for. Please contact us if you need assistance.',
					...
					<ul class="order_details">
						...
						<li class="date">
							...
							<strong><?php echo date_i18n(get_option('date_format'),
							                             strtotime($order->order_date)); ?></strong>
						</li>
						<li class="total">
	...
	private static function checkout() {
		...
		if (empty($_POST) && wc_notice_count('error') > 0) {
			...
			if (wc_notice_count('error') == 0 && $non_js_checkout) {
				wc_add_notice(__('The order totals have been updated. Please confirm your order by pressing the Place Order button at the bottom of the page.',
				                 'woocommerce'));
			}
