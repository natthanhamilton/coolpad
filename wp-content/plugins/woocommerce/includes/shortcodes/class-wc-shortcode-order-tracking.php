...
class WC_Shortcode_Order_Tracking {
	...
	public static function output($atts) {
		...
		global $post;
		if (!empty($_REQUEST['orderid']) && isset($_POST['_wpnonce']) && wp_verify_nonce($_POST['_wpnonce'],
			...
			if (!$order_id) {
				...
				if ($order && $order->id && $order_email) {
					if (strtolower($order->billing_email) == strtolower($order_email)) {
						...
						wc_get_template('order/tracking.php', [
							'order' => $order
						]);
					...
					echo '<p class="woocommerce-error">' . sprintf(__('Sorry, we could not find that order ID in our database.',
					                                                  'woocommerce'),
					                                               get_permalink($post->ID)) . '</p>';
				}
			}
		}
