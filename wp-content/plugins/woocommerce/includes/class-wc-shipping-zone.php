...
class WC_Shipping_Zone extends WC_Data {
	...
	public function read($id) {
		global $wpdb;
		if ($zone_data
			= $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}woocommerce_shipping_zones WHERE zone_id = %d LIMIT 1;",
			                                $id))
		) {
	...
	private function read_zone_locations($zone_id) {
		global $wpdb;
		if ($locations
			= $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}woocommerce_shipping_zone_locations WHERE zone_id = %d;",
			                                    $zone_id))
		) {
	...
	public function add_location($code, $type) {
		if ($this->is_valid_location_type($type)) {
			if ('postcode' === $type) {
				$code = trim(strtoupper(str_replace(chr(226) . chr(128) . chr(166), '...',
	...
	public function is_valid_location_type($type) {
		return in_array($type, ['postcode', 'state', 'country', 'continent']);
	}	/**
	...
	public function get_shipping_methods($enabled_only = FALSE) {
		global $wpdb;
		$raw_methods_sql = $enabled_only
			? "SELECT method_id, method_order, instance_id, is_enabled FROM {$wpdb->prefix}woocommerce_shipping_zone_methods WHERE zone_id = %d AND is_enabled = 1 order by method_order ASC;"
			: "SELECT method_id, method_order, instance_id, is_enabled FROM {$wpdb->prefix}woocommerce_shipping_zone_methods WHERE zone_id = %d order by method_order ASC;";
		...
		foreach ($raw_methods as $raw_method) {
			if (in_array($raw_method->method_id, array_keys($allowed_classes), TRUE)) {
				...
				if (is_object($methods[ $raw_method->instance_id ])) {
					...
					$methods[ $raw_method->instance_id ]->has_settings
					                                                   = $methods[ $raw_method->instance_id ]->has_settings();
					$methods[ $raw_method->instance_id ]->settings_html
					                                                   = $methods[ $raw_method->instance_id ]->supports('instance-settings-modal')
						? $methods[ $raw_method->instance_id ]->get_admin_options_html() : FALSE;
				}
			}
		}

		return apply_filters('woocommerce_shipping_zone_shipping_methods', $methods, $raw_methods, $allowed_classes,
		                     $this);
	}
	...
	public function delete() {
		if ($this->get_id()) {
			global $wpdb;
			$wpdb->delete($wpdb->prefix . 'woocommerce_shipping_zone_methods', ['zone_id' => $this->get_id()]);
			$wpdb->delete($wpdb->prefix . 'woocommerce_shipping_zone_locations', ['zone_id' => $this->get_id()]);
			$wpdb->delete($wpdb->prefix . 'woocommerce_shipping_zones', ['zone_id' => $this->get_id()]);
	...
	public function add_shipping_method($type) {
		...
		$allowed_classes = $wc_shipping->get_shipping_method_class_names();
		$count
		                 = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->prefix}woocommerce_shipping_zone_methods WHERE zone_id = %d",
		                                                 $this->get_zone_id()));
	...
	public function get_formatted_location($max = 10) {
		...
		if (sizeof($location_parts) > $max) {
			...
			return sprintf(_n('%s and %d other region', '%s and %d other regions', $remaining, 'woocommerce'),
			               implode(', ', array_splice($location_parts, 0, $max)), $remaining);
	...
	private function save_locations() {
		...
		global $wpdb;
		$wpdb->delete($wpdb->prefix . 'woocommerce_shipping_zone_locations', ['zone_id' => $this->get_zone_id()]);
