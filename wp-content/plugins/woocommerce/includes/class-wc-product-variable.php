...
class WC_Product_Variable extends WC_Product {
	...
	public function add_to_cart_text() {
		$text = $this->is_purchasable() && $this->is_in_stock() ? __('Select options', 'woocommerce')
	...
	public function get_variation_prices($display = FALSE) {
		 ...
		 */
		if ($display) {
			$price_hash = [get_option('woocommerce_tax_display_shop', 'excl'), WC_Tax::get_rates()];
		} else {
			$price_hash = [FALSE];
		}
		$filter_names
		...
		$price_hash = md5(json_encode(apply_filters('woocommerce_get_variation_prices_hash', $price_hash, $this,
		                                            $display)));
		...
		if (empty($this->prices_array[ $price_hash ])) {
			...
			if (empty($prices_array['version']) || $prices_array['version'] !== WC_Cache_Helper::get_transient_version('product')) {
				$this->prices_array = ['version' => WC_Cache_Helper::get_transient_version('product')];
			}
			...
			if (empty($prices_array[ $price_hash ])) {
				...
				foreach ($variation_ids as $variation_id) {
					if ($variation = $this->get_child($variation_id)) {
						$price         = apply_filters('woocommerce_variation_prices_price', $variation->price,
						                               $variation, $this);
						$regular_price = apply_filters('woocommerce_variation_prices_regular_price',
						                               $variation->regular_price, $variation, $this);
						$sale_price    = apply_filters('woocommerce_variation_prices_sale_price',
						                               $variation->sale_price, $variation, $this);
						// Skip empty prices
						if ('' === $price) {
							continue;
						}
						...
						if ($display) {
							if ('incl' === get_option('woocommerce_tax_display_shop')) {
								...
								$regular_price = '' === $regular_price ? ''
									: $variation->get_price_including_tax(1, $regular_price);
								$sale_price    = '' === $sale_price ? ''
									: $variation->get_price_including_tax(1, $sale_price);
							} else {
								$price         = '' === $price ? '' : $variation->get_price_excluding_tax(1, $price);
								$regular_price = '' === $regular_price ? ''
									: $variation->get_price_excluding_tax(1, $regular_price);
								$sale_price    = '' === $sale_price ? ''
									: $variation->get_price_excluding_tax(1, $sale_price);
							}
						}
						...
						$sale_prices[ $variation_id ]    = wc_format_decimal($sale_price . '.00',
			...
			$this->prices_array[ $price_hash ] = apply_filters('woocommerce_variation_prices',
			                                                   $prices_array[ $price_hash ], $this, $display);
		}
	...
	public function get_variation_attributes() {
		...
		if (!empty($child_ids)) {
			foreach ($attributes as $attribute) {
				...
				if (in_array('', $values)) {
					$values = $attribute['is_taxonomy'] ? wp_get_post_terms($this->id, $attribute['name'],
					                                                        ['fields' => 'slugs'])
	...
	public function get_variation_default_attributes() {
		...
		return apply_filters('woocommerce_product_default_attributes', array_filter((array)maybe_unserialize($default)),
		                     $this);
	}
	...
	public function get_matching_variation($match_attributes = []) {
		...
		if ($matches && !is_wp_error($matches)) {
			...
			return $match_attributes === array_map('sanitize_title', $match_attributes) ? 0
				: $this->get_matching_variation(array_map('sanitize_title', $match_attributes));
		} else {
			return 0;
		}
	}
	...
	public function get_available_variations() {
		...
		foreach ($this->get_children() as $child_id) {
			...
			if (apply_filters('woocommerce_hide_invisible_variations', FALSE, $this->id,
	...
	public function get_available_variation($variation) {
		...
		if (has_post_thumbnail($variation->get_variation_id())) {
			...
			$image_srcset      = function_exists('wp_get_attachment_image_srcset')
				? wp_get_attachment_image_srcset($attachment_id, 'shop_single') : FALSE;
			$image_sizes       = function_exists('wp_get_attachment_image_sizes')
				? wp_get_attachment_image_sizes($attachment_id, 'shop_single') : FALSE;
			if (empty($image_alt)) {
		...
		$availability_html = empty($availability['availability']) ? ''
			: '<p class="stock ' . esc_attr($availability['class']) . '">' . wp_kses_post($availability['availability']) . '</p>';
		$availability_html = apply_filters('woocommerce_stock_html', $availability_html, $availability['availability'],
		                                   $variation);

		return apply_filters('woocommerce_available_variation', [
			...
			'price_html'            => apply_filters('woocommerce_show_variation_price',
			                                         $variation->get_price() === "" || $this->get_variation_price('min') !== $this->get_variation_price('max'),
			                                         $this, $variation)
	...
	public function variable_product_sync($product_id = '') {
		...
		foreach (['price', 'regular_price', 'sale_price'] as $price_type) {
	...
	public static function sync($product_id) {
		...
		if (!$children) {
			...
			if (is_admin() && 'publish' === get_post_status($product_id)) {
				WC_Admin_Meta_Boxes::add_error(__('This variable product has no active variations. Add or enable variations to allow this product to be purchased.',
				                                  'woocommerce'));
			}
			// Loop the variations
		} else {
			// Set the variable product to be virtual/downloadable if all children are virtual/downloadable
			foreach (['_downloadable', '_virtual'] as $meta_key) {
