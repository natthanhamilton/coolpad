...
if (!class_exists('WC_Auth')) :
	class WC_Auth {
		...
		public static function add_endpoint() {
			add_rewrite_rule('^wc-auth/v([1]{1})/(.*)?',
			                 'index.php?wc-auth-version=$matches[1]&wc-auth-route=$matches[2]', 'top');
		}
		...
		protected function make_validation() {
			...
			if (!in_array($_REQUEST['scope'], ['read', 'write', 'read_write'])) {
		...
		protected function create_keys($app_name, $app_user_id, $scope) {
			global $wpdb;
			$description = sprintf(__('%s - API %s (created on %s at %s).', 'woocommerce'), wc_clean($app_name),
			                       $this->get_i18n_scope($scope), date_i18n(wc_date_format()),
			                       date_i18n(wc_time_format()));
			$user        = wp_get_current_user();
			// Created API keys.
			$permissions     = (in_array($scope, ['read', 'write', 'read_write'])) ? sanitize_text_field($scope)
				: 'read';
		...
		protected function post_consumer_data($consumer_data, $url) {
			...
			if (is_wp_error($response)) {
				...
				if (200 != $response['response']['code']) {
					throw new Exception(__('An error occurred in the request and at the time were unable to send the consumer data',
					                       'woocommerce'));
				}
			}
			...
			try {
				...
				if ('login' == $route && !is_user_logged_in()) {
					wc_get_template('auth/form-login.php', [
						...
						'return_url'   => add_query_arg(['success' => 0, 'user_id' => wc_clean($_REQUEST['user_id'])],
					...
					if ('login' == $route && is_user_logged_in()) {
						...
						if ('authorize' == $route && !is_user_logged_in()) {
							wp_redirect(esc_url_raw($this->build_url($_REQUEST, 'login')));
							exit;
							// Authorize endpoint
						} else {
							if ('authorize' == $route && current_user_can('manage_woocommerce')) {
								wc_get_template('auth/form-grant-access.php', [
									'app_name'    => $_REQUEST['app_name'],
									'return_url'  => add_query_arg(['success' => 0, 'user_id' => wc_clean($_REQUEST['user_id'])],
									                               $this->get_formatted_url($_REQUEST['return_url'])),
									'scope'       => $this->get_i18n_scope(wc_clean($_REQUEST['scope'])),
									'permissions' => $this->get_permissions_in_scope(wc_clean($_REQUEST['scope'])),
									'granted_url' => wp_nonce_url($this->build_url($_REQUEST, 'access_granted'),
									                              'wc_auth_grant_access', 'wc_auth_nonce'),
								...
								if ('access_granted' == $route && current_user_can('manage_woocommerce')) {
									if (!isset($_GET['wc_auth_nonce']) || !wp_verify_nonce($_GET['wc_auth_nonce'],
									                                                       'wc_auth_grant_access')
									) {
										throw new Exception(__('Invalid nonce verification', 'woocommerce'));
									}
									$consumer_data = $this->create_keys($_REQUEST['app_name'], $_REQUEST['user_id'],
									                                    $_REQUEST['scope']);
									$response      = $this->post_consumer_data($consumer_data,
									                                           $this->get_formatted_url($_REQUEST['callback_url']));
									if ($response) {
										wp_redirect(esc_url_raw(add_query_arg(['success' => 1, 'user_id' => wc_clean($_REQUEST['user_id'])],
										                                      $this->get_formatted_url($_REQUEST['return_url']))));
										exit;
									}
								} else {
									throw new Exception(__('You do not have permissions to access this page!',
									                       'woocommerce'));
								}
							}
						}
				...
				wp_die(sprintf(__('Error: %s', 'woocommerce'), $e->getMessage()), __('Access Denied', 'woocommerce'),
				       ['response' => 401]);
			}
		}
