 ...
 * @return WC_Product
 */
function wc_get_product($the_product = FALSE, $args = []) {
	if (!did_action('woocommerce_init')) {
		_doing_it_wrong(__FUNCTION__,
		                __('wc_get_product should not be called before the woocommerce_init action.', 'woocommerce'),
		                '2.5');
...
function wc_get_product_ids_on_sale() {
	...
	$on_sale_posts = $wpdb->get_results("
		...
		GROUP BY post.ID;
	");
	$product_ids_on_sale = array_unique(array_map('absint', array_merge(wp_list_pluck($on_sale_posts, 'ID'),
	                                                                    array_diff(wp_list_pluck($on_sale_posts,
	                                                                                             'post_parent'),
	                                                                               [0]))));
...
function wc_placeholder_img($size = 'shop_thumbnail') {
	...
	return apply_filters('woocommerce_placeholder_img',
	                     '<img src="' . wc_placeholder_img_src() . '" alt="' . esc_attr__('Placeholder',
	                                                                                      'woocommerce') . '" width="' . esc_attr($dimensions['width']) . '" class="woocommerce-placeholder wp-post-image" height="' . esc_attr($dimensions['height']) . '" />',
	                     $size, $dimensions);
}
...
function wc_get_formatted_variation($variation, $flat = FALSE) {
	...
	if (is_array($variation)) {
		...
		foreach ($variation as $name => $value) {
			...
			if ($flat) {
				$variation_list[] = wc_attribute_label(str_replace('attribute_', '',
				                                                   $name)) . ': ' . rawurldecode($value);
			} else {
				$variation_list[] = '<dt>' . wc_attribute_label(str_replace('attribute_', '',
				                                                            $name)) . ':</dt><dd>' . rawurldecode($value) . '</dd>';
			}
		}
		if ($flat) {
...
function wc_get_product_types() {
	return (array)apply_filters('product_type_selector', [
...
function wc_get_product_variation_attributes($variation_id) {
	...
	foreach ($all_meta as $name => $value) {
		 ...
		 */
		if (sanitize_title($value[0]) === $value[0] && version_compare(get_post_meta($parent_id, '_product_version',
		                                                                             TRUE), '2.4.0', '<')
		) {
...
function wc_get_product_attachment_props($attachment_id, $product = FALSE) {
	...
	if ($attachment_id) {
		...
		$props['alt'] = empty($props['alt']) && $product ? trim(strip_tags(get_the_title($product->ID)))
			: $props['alt'];
	}
