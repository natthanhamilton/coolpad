 ...
 *
 * NOTE: At this point in time, ordering by menu_order for example isn't possible with this function. wp_get_post_terms
 * has no. filters which we can utilise to modify it's query. https://core.trac.wordpress.org/ticket/19094.
 *
...
function wc_get_product_terms($product_id, $taxonomy, $args = []) {
	...
	if (!empty($args['orderby']) && in_array($args['orderby'], ['name_num', 'parent'])) {
		...
		$args['include'] = wp_get_post_terms($product_id, $taxonomy, ['fields' => 'ids']);
 ...
 * @return string
 */
function wc_product_dropdown_categories($args = [], $deprecated_hierarchical = 1, $deprecated_show_uncategorized = 1,
                                        $deprecated_orderby = '') {
	global $wp_query;
	if (!is_array($args)) {
		_deprecated_argument('wc_product_dropdown_categories()', '2.1',
		                     'show_counts, hierarchical, show_uncategorized and orderby arguments are invalid - pass a single array of values instead.');
	...
	$output .= '<option value="" ' . selected($current_product_cat, '', FALSE) . '>' . __('Select a category',
	                                                                                      'woocommerce') . '</option>';
	$output .= wc_walk_category_dropdown_tree($terms, 0, $args);
	if ($args['show_uncategorized']) {
		$output .= '<option value="0" ' . selected($current_product_cat, '0', FALSE) . '>' . __('Uncategorized',
		                                                                                        'woocommerce') . '</option>';
	}
	$output .= "</select>";
	echo $output;
...
function wc_taxonomy_metadata_update_content_for_split_terms($old_term_id, $new_term_id, $term_taxonomy_id, $taxonomy) {
	...
	if (get_option('db_version') < 34370) {
		if ('product_cat' === $taxonomy || taxonomy_is_product_attribute($taxonomy)) {
			$old_meta_data
				= $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}woocommerce_termmeta WHERE woocommerce_term_id = %d;",
				                                    $old_term_id));
 ...
 * We do this when the new version is >= 34370, and the old version is < 34370 (34370 is when term meta table was
 * added).
 *
 ...
 *
 * @todo These functions should be deprecated with notices in a future WC version, allowing users a chance to upgrade
 *       WordPress.
 *
 * @param mixed  $term_id
...
function update_woocommerce_term_meta($term_id, $meta_key, $meta_value, $prev_value = '') {
	return function_exists('update_term_meta') ? update_term_meta($term_id, $meta_key, $meta_value, $prev_value)
		: update_metadata('woocommerce_term', $term_id, $meta_key, $meta_value, $prev_value);
}
...
function add_woocommerce_term_meta($term_id, $meta_key, $meta_value, $unique = FALSE) {
	return function_exists('add_term_meta') ? add_term_meta($term_id, $meta_key, $meta_value, $unique)
		: add_metadata('woocommerce_term', $term_id, $meta_key, $meta_value, $unique);
}
...
function delete_woocommerce_term_meta($term_id, $meta_key, $meta_value = '', $deprecated = FALSE) {
	return function_exists('delete_term_meta') ? delete_term_meta($term_id, $meta_key, $meta_value)
...
function get_woocommerce_term_meta($term_id, $key, $single = TRUE) {
	return function_exists('get_term_meta') ? get_term_meta($term_id, $key, $single)
		: get_metadata('woocommerce_term', $term_id, $key, $single);
}
...
function wc_terms_clauses($clauses, $taxonomies, $args) {
	...
	foreach ((array)$taxonomies as $taxonomy) {
		if (taxonomy_is_product_attribute($taxonomy) || in_array($taxonomy,
	...
	// Default to ASC.
	if (!isset($args['menu_order']) || !in_array(strtoupper($args['menu_order']), ['ASC', 'DESC'])) {
		$args['menu_order'] = 'ASC';
	}
...
function wc_change_term_counts($terms, $taxonomies) {
	...
	if (!isset($taxonomies[0]) || !in_array($taxonomies[0], apply_filters('woocommerce_change_term_counts',
	...
	foreach ($terms as &$term) {
		if (is_object($term)) {
			$term_counts[ $term->term_id ] = isset($term_counts[ $term->term_id ]) ? $term_counts[ $term->term_id ]
				: get_woocommerce_term_meta($term->term_id, 'product_count_' . $taxonomies[0], TRUE);
