...
class WC_Cache_Helper {
	...
	public static function init() {
		...
		add_action('before_woocommerce_init', [__CLASS__, 'prevent_caching']);
		add_action('admin_notices', [__CLASS__, 'notices']);
		add_action('delete_version_transients', [__CLASS__, 'delete_version_transients']);
	}
	 ...
	 *
	 * Note; this only works on transients appended with the transient version, and when object caching is not being
	 * used.
	 *
	 * @since  2.3.10
	 */
	public static function delete_version_transients($version = '') {
		if (!wp_using_ext_object_cache() && !empty($version)) {
			...
			$affected
			       = $wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->options} WHERE option_name LIKE %s ORDER BY option_id LIMIT %d;",
			                                     "\_transient\_%" . $version, $limit));
			// If affected rows is equal to limit, there are more rows to delete. Delete in 10 secs.
			if ($affected === $limit) {
				wp_schedule_single_event(time() + 10, 'delete_version_transients', [$version]);
			}
		}
	}
	...
	public static function notices() {
		...
		if ($enabled && !in_array('_wc_session_', $settings)) {
			?>
			<div class="error">
				<p><?php printf(__('In order for <strong>database caching</strong> to work with WooCommerce you must add <code>_wc_session_</code> to the "Ignored Query Strings" option in W3 Total Cache settings <a href="%s">here</a>.',
				                   'woocommerce'), admin_url('admin.php?page=w3tc_dbcache')); ?></p>
			</div>
			<?php
		}
	}
