 ...
 *            particular user. Accepts array of values. Array of values is OR'ed. If array of array is passed, each
 *            array will be AND'ed. e.g. test@test.com, 1, array( 1, 2, 3 ), array( array( 1, 'test@test.com' ), 2, 3 )
 *            limit int Maximum of orders to retrieve. offset int Offset of orders to retrieve. page int Page of orders
 *            to retrieve. Ignored when using the 'offset' arg. exclude array Order IDs to exclude from the query.
 *            orderby string Order by date, title, id, modified, rand etc order string ASC or DESC return string Type
 *            of data to return. Allowed values: ids array of order ids objects array of order objects (default)
...
function wc_get_orders($args) {
	...
	if (!empty($args['customer'])) {
		$values                        = is_array($args['customer']) ? $args['customer'] : [$args['customer']];
...
function wc_get_order($the_order = FALSE) {
	if (!did_action('woocommerce_init')) {
		_doing_it_wrong(__FUNCTION__,
		                __('wc_get_order should not be called before the woocommerce_init action.', 'woocommerce'),
		                '2.5');
...
function wc_get_order_id_by_order_key($order_key) {
	...
	$order_id
		= $wpdb->get_var($wpdb->prepare("SELECT post_id FROM {$wpdb->prefix}postmeta WHERE meta_key = '_order_key' AND meta_value = %s",
		                                $order_key));
...
function wc_downloadable_file_permission($download_id, $product_id, $order, $qty = 1) {
	...
	$data = apply_filters('woocommerce_downloadable_file_permission_data', [
	...
	$format = apply_filters('woocommerce_downloadable_file_permission_format', [
		'%s',
		'%s',
		'%s',
		'%s',
		...
		foreach ($order->get_items() as $item) {
			...
			if ($_product && $_product->exists() && $_product->is_downloadable()) {
				...
				foreach (array_keys($downloads) as $download_id) {
					wc_downloadable_file_permission($download_id, $item['variation_id'] > 0 ? $item['variation_id']
...
function wc_update_order_item($item_id, $args) {
	global $wpdb;
	$update = $wpdb->update($wpdb->prefix . 'woocommerce_order_items', $args, ['order_item_id' => $item_id]);
	if (FALSE === $update) {
		return FALSE;
	}
...
function wc_delete_order_item($item_id) {
	...
	$wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->prefix}woocommerce_order_items WHERE order_item_id = %d",
	                            $item_id));
	$wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->prefix}woocommerce_order_itemmeta WHERE order_item_id = %d",
	                            $item_id));
...
function wc_cancel_unpaid_orders() {
	...
	if ($unpaid_orders) {
		foreach ($unpaid_orders as $unpaid_order) {
			...
			if (apply_filters('woocommerce_cancel_unpaid_order',
			                  'checkout' === get_post_meta($unpaid_order, '_created_via', TRUE), $order)) {
...
function wc_create_refund($args = []) {
	...
	if ($args['refund_id'] > 0) {
		...
		$refund_data['post_title']    = sprintf(__('Refund &ndash; %s', 'woocommerce'),
		                                        strftime(_x('%b %d, %Y @ %I:%M %p', 'Order date parsed by strftime',
		                                                    'woocommerce')));
						...
						case 'line_item' :
							$line_item_args = [
								'totals' => [
									...
									'tax_data'     => ['total' => array_map('wc_format_refund_total',
									                                        $refund_item['refund_tax']), 'subtotal' => array_map('wc_format_refund_total',
									                                                                                             $refund_item['refund_tax'])]
								]
							];
							$new_item_id
							                = $refund->add_product($order->get_product_from_item($order_items[ $refund_item_id ]),
							                                       isset($refund_item['qty']) ? $refund_item['qty'] * -1
								                                       : 0, $line_item_args);
							wc_add_order_item_meta($new_item_id, '_refunded_item_id', $refund_item_id);
							break;
						case 'shipping' :
							$shipping    = new WC_Shipping_Rate($order_items[ $refund_item_id ]['method_id'],
							                                    $order_items[ $refund_item_id ]['name'],
							                                    wc_format_refund_total($refund_item['refund_total']),
							                                    array_map('wc_format_refund_total',
							                                              $refund_item['refund_tax']),
							                                    $order_items[ $refund_item_id ]['method_id']);
...
function wc_get_tax_class_by_tax_id($tax_id) {
	global $wpdb;
	$tax_class
		= $wpdb->get_var($wpdb->prepare("SELECT tax_rate_class FROM {$wpdb->prefix}woocommerce_tax_rates WHERE tax_rate_id = %d",
		                                $tax_id));
...
function wc_order_search($term) {
	...
	$post_ids = [];
	// Search fields.
	$search_fields = array_map('wc_clean', apply_filters('woocommerce_shop_order_search_fields', [
		'_order_key',
		'_billing_company',
		'_billing_address_1',
		'_billing_address_2',
	...
	if (is_numeric($term)) {
		$post_ids = array_unique(array_merge(
			                         $wpdb->get_col(
				                         $wpdb->prepare("SELECT DISTINCT p1.post_id FROM {$wpdb->postmeta} p1 WHERE p1.meta_key IN ('" . implode("','",
				                                                                                                                                 array_map('esc_sql',
				                                                                                                                                           $search_fields)) . "') AND p1.meta_value LIKE '%%%s%%';",
				                                        wc_clean($term))
			                         ),
			                         [absint($term)]
		                         ));
