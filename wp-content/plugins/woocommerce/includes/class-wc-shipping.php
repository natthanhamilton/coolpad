...
class WC_Shipping {
	...
	public function get_shipping_classes() {
		if (empty($this->shipping_classes)) {
			$classes                = get_terms('product_shipping_class', ['hide_empty' => '0', 'orderby' => 'name']);
		...
		foreach ($this->packages as $i => $package) {
			...
			if (sizeof($package['rates']) > 0) {
				...
				if (empty($chosen_method) || !isset($package['rates'][ $chosen_method ]) || $method_count !== sizeof($package['rates'])) {
					$chosen_method        = apply_filters('woocommerce_shipping_chosen_method',
					                                      $this->get_default_method($package['rates'], FALSE),
				...
				if ($chosen_method) {
					...
					if (!empty($rate->taxes) && is_array($rate->taxes)) {
						foreach (array_keys($this->shipping_taxes + $rate->taxes) as $key) {
							$this->shipping_taxes[ $key ] = (isset($rate->taxes[ $key ]) ? $rate->taxes[ $key ]
									: 0) + (isset($this->shipping_taxes[ $key ]) ? $this->shipping_taxes[ $key ] : 0);
						}
					}
				}
			}
	 ...
	 *
	 * Calculates each shipping methods cost. Rates are stored in the session based on the package hash to avoid
	...
	/**
	 * Loads all shipping methods which are hooked in. If a $package is passed some methods may add themselves
	 * conditionally.
	 *
	...
	/**
	 * Shipping methods register themselves by returning their main class name through the woocommerce_shipping_methods
	 * filter.
	 *
	 * @return array
	 */
	public function get_shipping_method_class_names() {
		...
		$maybe_load_legacy_methods
			= ['flat_rate', 'free_shipping', 'international_delivery', 'local_delivery', 'local_pickup'];
	...
	/**
	 * @deprecated 2.6.0 Was previously used to determine sort order of methods, but this is now controlled by zones
	 *             and thus unused.
	 */
