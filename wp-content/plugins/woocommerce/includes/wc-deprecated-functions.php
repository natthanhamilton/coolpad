...
/**
 * Alias functions/soft-deprecated function names (moving from woocommerce_ to wc_). These will be deprecated with
 * notices in future updates.
 */
/**
 ...
 * @deprecated
 */
function woocommerce_product_dropdown_categories($args = [], $deprecated_hierarchical = 1,
                                                 $deprecated_show_uncategorized = 1, $deprecated_orderby = '') {
	return wc_product_dropdown_categories($args, $deprecated_hierarchical, $deprecated_show_uncategorized,
	                                      $deprecated_orderby);
}
...
function wc_shop_order_status_backwards_compatibility($q) {
	...
	if (
		...
		$tax_query    = isset($q->query_vars['tax_query']) ? $q->query_vars['tax_query'] : [];
		$order_status = [];
		$tax_key      = '';
		...
		if ($order_status) {
			...
			_doing_it_wrong('WP_Query',
			                sprintf(__('The shop_order_status taxonomy is no more in WooCommerce 2.2! You should use the new WooCommerce post_status instead, <a href="%s">read more...</a>',
			                           'woocommerce'),
			                        'https://woocommerce.wordpress.com/2014/08/wc-2-2-order-statuses-plugin-compatibility/'),
			                'WooCommerce 2.2');
		} else {
			$q->set('post_status', array_keys(wc_get_order_statuses()));
			_doing_it_wrong('WP_Query',
			                sprintf(__('The "publish" order status is no more in WooCommerce 2.2! You should use the new WooCommerce post_status instead, <a href="%s">read more...</a>',
			                           'woocommerce'),
			                        'https://woocommerce.wordpress.com/2014/08/wc-2-2-order-statuses-plugin-compatibility/'),
			                'WooCommerce 2.2');
		}
...
/**
 * woocommerce_calc_shipping was an option used to determine if shipping was enabled prior to version 2.6.0. This has
 * since been replaced with wc_shipping_enabled() function and the woocommerce_ship_to_countries setting.
 *
 * @since 2.6.0
 * @return string
 */
