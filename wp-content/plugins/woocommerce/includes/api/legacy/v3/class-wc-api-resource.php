...
class WC_API_Resource {
	...
	public function __construct(WC_API_Server $server) {
		...
		$response_names = ['order', 'coupon', 'customer', 'product', 'report',
	...
	protected function validate_request($id, $type, $context) {
		...
		if ('customer' !== $type) {
			$post = get_post($id);
			if (NULL === $post) {
				return new WP_Error("woocommerce_api_no_{$resource_name}_found",
				                    sprintf(__('No %s found with the ID equal to %s', 'woocommerce'), $resource_name,
				                            $id), ['status' => 404]);
			}
	...
	private function check_permission($post, $context) {
		...
		if ('read' === $context) {
			$permission = 'revision' !== $post->post_type && current_user_can($post_type->cap->read_private_posts,
			                                                                  $post->ID);
		} elseif ('edit' === $context) {
