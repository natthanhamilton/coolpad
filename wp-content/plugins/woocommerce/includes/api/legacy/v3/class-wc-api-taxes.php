...
class WC_API_Taxes extends WC_API_Resource {
	...
	public function get_tax($id, $fields = NULL) {
		...
		try {
			...
			if (!current_user_can('manage_woocommerce')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_read_tax',
				                           __('You do not have permission to read tax rate', 'woocommerce'), 401);
			}
			// Get tax rate details
			$tax = WC_Tax::_get_tax_rate($id);
			if (is_wp_error($tax) || empty($tax)) {
				throw new WC_API_Exception('woocommerce_api_invalid_tax_id',
				                           __('A tax rate with the provided ID could not be found', 'woocommerce'),
				                           404);
			}
			$tax_data = [
	...
	public function delete_tax($id) {
		...
		try {
			// Check permissions
			if (!current_user_can('manage_woocommerce')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_delete_tax',
				                           __('You do not have permission to delete tax rates', 'woocommerce'), 401);
			}
			$id = absint($id);
			WC_Tax::_delete_tax_rate($id);
			if (0 === $wpdb->rows_affected) {
				throw new WC_API_Exception('woocommerce_api_cannot_delete_tax',
				                           __('Could not delete the tax rate', 'woocommerce'), 401);
			}
	...
	public function get_taxes_count($class = NULL, $filter = []) {
		try {
			if (!current_user_can('manage_woocommerce')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_read_taxes_count',
				                           __('You do not have permission to read the taxes count', 'woocommerce'),
				                           401);
			}
			if (!empty($class)) {
				$filter['tax_rate_class'] = $class;
	...
	public function bulk($data) {
		try {
			if (!isset($data['taxes'])) {
				throw new WC_API_Exception('woocommerce_api_missing_taxes_data',
			...
			if (count($data) > $limit) {
				throw new WC_API_Exception('woocommerce_api_taxes_request_entity_too_large',
	...
	public function edit_tax($id, $data) {
		try {
			if (!isset($data['tax'])) {
				throw new WC_API_Exception('woocommerce_api_missing_tax_data',
				                           sprintf(__('No %1$s data specified to edit %1$s', 'woocommerce'), 'tax'),
				                           400);
			}
			// Check permissions
			if (!current_user_can('manage_woocommerce')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_edit_tax',
				                           __('You do not have permission to edit tax rates', 'woocommerce'), 401);
			}
			$data = $data['tax'];
	...
	public function create_tax($data) {
		try {
			if (!isset($data['tax'])) {
				throw new WC_API_Exception('woocommerce_api_missing_tax_data',
				                           sprintf(__('No %1$s data specified to create %1$s', 'woocommerce'), 'tax'),
				                           400);
			}
			// Check permissions
			if (!current_user_can('manage_woocommerce')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_create_tax',
				                           __('You do not have permission to create tax rates', 'woocommerce'), 401);
			}
	...
	public function get_tax_classes($fields = NULL) {
		try {
			// Permissions check
			if (!current_user_can('manage_woocommerce')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_read_tax_classes',
				                           __('You do not have permission to read tax classes', 'woocommerce'), 401);
			}
			$tax_classes = [];
			// Add standard class
			$tax_classes[] = [
			...
			return ['tax_classes' => apply_filters('woocommerce_api_tax_classes_response', $tax_classes, $classes,
			                                       $fields, $this)];
		} catch (WC_API_Exception $e) {
			return new WP_Error($e->getErrorCode(), $e->getMessage(), ['status' => $e->getCode()]);
		}
	...
	public function create_tax_class($data) {
		try {
			if (!isset($data['tax_class'])) {
				throw new WC_API_Exception('woocommerce_api_missing_tax_class_data',
				                           sprintf(__('No %1$s data specified to create %1$s', 'woocommerce'),
				                                   'tax_class'), 400);
			}
			// Check permissions
			if (!current_user_can('manage_woocommerce')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_create_tax_class',
				                           __('You do not have permission to create tax classes', 'woocommerce'), 401);
			}
			$data = $data['tax_class'];
			if (empty($data['name'])) {
				throw new WC_API_Exception('woocommerce_api_missing_tax_class_name',
			...
			if ($exists) {
				throw new WC_API_Exception('woocommerce_api_cannot_create_tax_class',
				                           __('Tax class already exists', 'woocommerce'), 401);
			}
			// Add the new class.
			$classes[] = $name;
	...
	public function delete_tax_class($slug) {
		...
		try {
			// Check permissions
			if (!current_user_can('manage_woocommerce')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_delete_tax_class',
				                           __('You do not have permission to delete tax classes', 'woocommerce'), 401);
			}
			...
			if (!$deleted) {
				throw new WC_API_Exception('woocommerce_api_cannot_delete_tax_class',
				                           __('Could not delete the tax class', 'woocommerce'), 401);
			}
			update_option('woocommerce_tax_classes', implode("\n", $classes));
			...
			$wpdb->delete($wpdb->prefix . 'woocommerce_tax_rates', ['tax_rate_class' => $slug], ['%s']);
	...
	public function get_tax_classes_count() {
		try {
			if (!current_user_can('manage_woocommerce')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_read_tax_classes_count',
				                           __('You do not have permission to read the tax classes count',
				                              'woocommerce'), 401);
			}
