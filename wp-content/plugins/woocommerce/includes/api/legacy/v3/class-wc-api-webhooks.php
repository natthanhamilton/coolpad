...
class WC_API_Webhooks extends WC_API_Resource {
	...
	public function get_webhook($id, $fields = NULL) {
		...
		return ['webhook' => apply_filters('woocommerce_api_webhook_response', $webhook_data, $webhook, $fields,
		                                   $this)];
	}
	...
	public function get_webhooks_count($status = NULL, $filter = []) {
		try {
			if (!current_user_can('read_private_shop_webhooks')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_read_webhooks_count',
				                           __('You do not have permission to read the webhooks count', 'woocommerce'),
				                           401);
			}
			if (!empty($status)) {
				$filter['status'] = $status;
	...
	public function create_webhook($data) {
		try {
			if (!isset($data['webhook'])) {
				throw new WC_API_Exception('woocommerce_api_missing_webhook_data',
			...
			if (!current_user_can('publish_shop_webhooks')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_create_webhooks',
				                           __('You do not have permission to create webhooks', 'woocommerce'), 401);
			}
			...
			if (empty($data['topic']) || !wc_is_webhook_valid_topic(strtolower($data['topic']))) {
				throw new WC_API_Exception('woocommerce_api_invalid_webhook_topic',
				                           __('Webhook topic is required and must be valid', 'woocommerce'), 400);
			}
			// validate delivery URL
			if (empty($data['delivery_url']) || !wc_is_valid_url($data['delivery_url'])) {
				throw new WC_API_Exception('woocommerce_api_invalid_webhook_delivery_url',
				                           __('Webhook delivery URL must be a valid URL starting with http:// or https://',
				                              'woocommerce'), 400);
			}
			...
			if (is_wp_error($webhook_id) || !$webhook_id) {
				throw new WC_API_Exception('woocommerce_api_cannot_create_webhook',
				                           sprintf(__('Cannot create webhook: %s', 'woocommerce'),
				                                   is_wp_error($webhook_id) ? implode(', ',
				                                                                      $webhook_id->get_error_messages())
					                                   : '0'), 500);
			}
	...
	public function edit_webhook($id, $data) {
		try {
			if (!isset($data['webhook'])) {
				throw new WC_API_Exception('woocommerce_api_missing_webhook_data',
				                           sprintf(__('No %1$s data specified to edit %1$s', 'woocommerce'), 'webhook'),
				                           400);
			}
			$data = $data['webhook'];
			...
			if (!empty($data['topic'])) {
				if (wc_is_webhook_valid_topic(strtolower($data['topic']))) {
					...
					throw new WC_API_Exception('woocommerce_api_invalid_webhook_topic',
					                           __('Webhook topic must be valid', 'woocommerce'), 400);
				}
			}
			// update delivery URL
	...
	public function get_webhook_deliveries($webhook_id, $fields = NULL) {
		...
		return ['webhook_deliveries' => $delivery_logs];
	}
	...
	public function get_webhook_delivery($webhook_id, $id, $fields = NULL) {
		try {
			...
			$id = absint($id);
			if (empty($id)) {
				throw new WC_API_Exception('woocommerce_api_invalid_webhook_delivery_id',
				                           __('Invalid webhook delivery ID', 'woocommerce'), 404);
			}
			$webhook = new WC_Webhook($webhook_id);
			$log = $webhook->get_delivery_log($id);
			if (!$log) {
				throw new WC_API_Exception('woocommerce_api_invalid_webhook_delivery_id',
				                           __('Invalid webhook delivery', 'woocommerce'), 400);
			}
			$delivery_log = $log;
			// Add timestamp
			...
			return ['webhook_delivery' => apply_filters('woocommerce_api_webhook_delivery_response', $delivery_log, $id,
			                                            $fields, $log, $webhook_id, $this)];
		} catch (WC_API_Exception $e) {
			return new WP_Error($e->getErrorCode(), $e->getMessage(), ['status' => $e->getCode()]);
		}
	}
