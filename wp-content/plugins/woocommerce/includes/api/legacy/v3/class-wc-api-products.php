...
class WC_API_Products extends WC_API_Resource {
	...
	public function get_product($id, $fields = NULL) {
		...
		return ['product' => apply_filters('woocommerce_api_product_response', $product_data, $product, $fields,
		                                   $this->server)];
	}
	...
	private function get_product_data($product) {
		return [
			...
			'featured_src'       => (string)wp_get_attachment_url(get_post_thumbnail_id($product->is_type('variation')
			...
			'total_sales'        => metadata_exists('post', $product->id, 'total_sales')
				? (int)get_post_meta($product->id, 'total_sales', TRUE) : 0,
			'variations'         => [],
			'parent'             => [],
			'grouped_products'   => [],
	...
	private function get_variation_data($product) {
		...
		foreach ($product->get_children() as $child_id) {
			...
			$variations[] = [
				...
				'shipping_class_id'  => (0 !== $variation->get_shipping_class_id())
					? $variation->get_shipping_class_id() : NULL,
	...
	public function get_products_count($type = NULL, $filter = []) {
		try {
			if (!current_user_can('read_private_products')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_read_products_count',
				                           __('You do not have permission to read the products count', 'woocommerce'),
				                           401);
			}
			if (!empty($type)) {
				$filter['type'] = $type;
	...
	public function delete_product($id, $force = FALSE) {
		...
		if (!$result) {
			return new WP_Error('woocommerce_api_cannot_delete_product',
			                    sprintf(__('This %s cannot be deleted', 'woocommerce'), 'product'), ['status' => 500]);
		}
	...
	public function get_product_reviews($id, $fields = NULL) {
		...
		return ['product_reviews' => apply_filters('woocommerce_api_product_reviews_response', $reviews, $id, $fields,
		                                           $comments, $this->server)];
	}
	...
	public function get_product_orders($id, $fields = NULL, $filter = [], $status = NULL, $page = 1) {
		...
		return ['orders' => apply_filters('woocommerce_api_product_orders_response', $orders['orders'], $id, $filter,
		                                  $fields, $this->server)];
	}
	...
	public function get_product_categories($fields = NULL) {
		try {
			...
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_read_product_categories',
			...
			return ['product_categories' => apply_filters('woocommerce_api_product_categories_response',
			                                              $product_categories, $terms, $fields, $this)];
		} catch (WC_API_Exception $e) {
			return new WP_Error($e->getErrorCode(), $e->getMessage(), ['status' => $e->getCode()]);
		}
	}
	...
	public function get_product_category($id, $fields = NULL) {
		try {
			...
			// Validate ID
			if (empty($id)) {
				throw new WC_API_Exception('woocommerce_api_invalid_product_category_id',
				                           __('Invalid product category ID', 'woocommerce'), 400);
			}
			// Permissions check
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_read_product_categories',
			...
			if (is_wp_error($term) || is_null($term)) {
				throw new WC_API_Exception('woocommerce_api_invalid_product_category_id',
				                           __('A product category with the provided ID could not be found',
				                              'woocommerce'), 404);
			}
			...
			return ['product_category' => apply_filters('woocommerce_api_product_category_response', $product_category,
			                                            $id, $fields, $term, $this)];
		} catch (WC_API_Exception $e) {
			return new WP_Error($e->getErrorCode(), $e->getMessage(), ['status' => $e->getCode()]);
		}
	...
	public function create_product_category($data) {
		...
		try {
			if (!isset($data['product_category'])) {
				throw new WC_API_Exception('woocommerce_api_missing_product_category_data',
				                           sprintf(__('No %1$s data specified to create %1$s', 'woocommerce'),
				                                   'product_category'), 400);
			}
			// Check permissions
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_create_product_category',
				                           __('You do not have permission to create product categories', 'woocommerce'),
				                           401);
			}
			$defaults = [
				'name'        => '',
			...
			if ($data['parent']) {
				...
				if (!$parent) {
					throw new WC_API_Exception('woocommerce_api_invalid_product_category_parent',
					                           __('Product category parent is invalid', 'woocommerce'), 400);
				}
			}
			...
			if (is_wp_error($insert)) {
				throw new WC_API_Exception('woocommerce_api_cannot_create_product_category',
				                           $insert->get_error_message(), 400);
			}
			$id = $insert['term_id'];
			update_woocommerce_term_meta($id, 'display_type',
			                             'default' === $data['display'] ? '' : sanitize_text_field($data['display']));
	...
	protected function upload_image_from_url($image_url, $upload_for = 'product_image') {
		...
		if (!$parsed_url || !is_array($parsed_url)) {
			throw new WC_API_Exception('woocommerce_api_invalid_' . $upload_for,
			                           sprintf(__('Invalid URL %s', 'woocommerce'), $image_url), 400);
		}
		// Ensure url is valid.
		...
		if (is_wp_error($response)) {
			throw new WC_API_Exception('woocommerce_api_invalid_remote_' . $upload_for,
			...
			throw new WC_API_Exception('woocommerce_api_invalid_remote_' . $upload_for,
			                           sprintf(__('Error getting remote image %s.', 'woocommerce'), $image_url), 400);
		}
		...
		if (!$wp_filetype['type']) {
			...
			if (!$wp_filetype['type']) {
				throw new WC_API_Exception('woocommerce_api_invalid_' . $upload_for,
				                           __('Invalid image type.', 'woocommerce'), 400);
			}
		}
		// Upload the file.
		...
		if (0 == $filesize) {
			@unlink($upload['file']);
			unset($upload);
			throw new WC_API_Exception('woocommerce_api_' . $upload_for . '_upload_file_error',
			                           __('Zero size file downloaded', 'woocommerce'), 400);
		}
		unset($response);
	...
	public function edit_product_category($id, $data) {
		...
		try {
			if (!isset($data['product_category'])) {
				throw new WC_API_Exception('woocommerce_api_missing_product_category',
				                           sprintf(__('No %1$s data specified to edit %1$s', 'woocommerce'),
				                                   'product_category'), 400);
			}
			$id   = absint($id);
			...
			// Check permissions.
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_edit_product_category',
				                           __('You do not have permission to edit product categories', 'woocommerce'),
				                           401);
			}
			...
			if (is_wp_error($update)) {
				throw new WC_API_Exception('woocommerce_api_cannot_edit_product_catgory',
				                           __('Could not edit the category', 'woocommerce'), 400);
			}
			if (!empty($data['display'])) {
				update_woocommerce_term_meta($id, 'display_type', 'default' === $data['display'] ? ''
					: sanitize_text_field($data['display']));
			}
			if (isset($image_id)) {
				update_woocommerce_term_meta($id, 'thumbnail_id', $image_id);
			}
	...
	public function delete_product_category($id) {
		...
		try {
			// Check permissions
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_delete_product_category',
				                           __('You do not have permission to delete product category', 'woocommerce'),
				                           401);
			}
			$id      = absint($id);
			$deleted = wp_delete_term($id, 'product_cat');
			if (!$deleted || is_wp_error($deleted)) {
				throw new WC_API_Exception('woocommerce_api_cannot_delete_product_category',
				                           __('Could not delete the category', 'woocommerce'), 401);
			}
			// When a term is deleted, delete its meta.
			if (get_option('db_version') < 34370) {
				$wpdb->delete($wpdb->woocommerce_termmeta, ['woocommerce_term_id' => $id], ['%d']);
			}
	...
	public function get_product_tags($fields = NULL) {
		try {
			// Permissions check
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_read_product_tags',
				                           __('You do not have permission to read product tags', 'woocommerce'), 401);
			}
			$product_tags = [];
			...
			return ['product_tags' => apply_filters('woocommerce_api_product_tags_response', $product_tags, $terms,
			                                        $fields, $this)];
		} catch (WC_API_Exception $e) {
			return new WP_Error($e->getErrorCode(), $e->getMessage(), ['status' => $e->getCode()]);
		}
	...
	public function get_product_tag($id, $fields = NULL) {
		try {
			...
			// Validate ID
			if (empty($id)) {
				throw new WC_API_Exception('woocommerce_api_invalid_product_tag_id',
				                           __('Invalid product tag ID', 'woocommerce'), 400);
			}
			// Permissions check
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_read_product_tags',
			...
			if (is_wp_error($term) || is_null($term)) {
				throw new WC_API_Exception('woocommerce_api_invalid_product_tag_id',
				                           __('A product tag with the provided ID could not be found', 'woocommerce'),
				                           404);
			}
			$term_id = intval($term->term_id);
			$tag = [
			...
			return ['product_tag' => apply_filters('woocommerce_api_product_tag_response', $tag, $id, $fields, $term,
			                                       $this)];
		} catch (WC_API_Exception $e) {
			return new WP_Error($e->getErrorCode(), $e->getMessage(), ['status' => $e->getCode()]);
		}
	...
	public function create_product_tag($data) {
		try {
			if (!isset($data['product_tag'])) {
				throw new WC_API_Exception('woocommerce_api_missing_product_tag_data',
				                           sprintf(__('No %1$s data specified to create %1$s', 'woocommerce'),
				                                   'product_tag'), 400);
			}
			// Check permissions
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_create_product_tag',
				                           __('You do not have permission to create product tags', 'woocommerce'), 401);
			}
			$defaults = [
				'name'        => '',
				'slug'        => '',
			...
			if (is_wp_error($insert)) {
				throw new WC_API_Exception('woocommerce_api_cannot_create_product_tag', $insert->get_error_message(),
				                           400);
			}
			$id = $insert['term_id'];
	...
	public function edit_product_tag($id, $data) {
		try {
			if (!isset($data['product_tag'])) {
				throw new WC_API_Exception('woocommerce_api_missing_product_tag',
				                           sprintf(__('No %1$s data specified to edit %1$s', 'woocommerce'),
				                                   'product_tag'), 400);
			}
			$id   = absint($id);
			...
			// Check permissions.
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_edit_product_tag',
				                           __('You do not have permission to edit product tags', 'woocommerce'), 401);
			}
			...
			if (is_wp_error($update)) {
				throw new WC_API_Exception('woocommerce_api_cannot_edit_product_tag',
				                           __('Could not edit the tag', 'woocommerce'), 400);
			}
	...
	public function delete_product_tag($id) {
		try {
			// Check permissions
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_delete_product_tag',
				                           __('You do not have permission to delete product tag', 'woocommerce'), 401);
			}
			$id      = absint($id);
			$deleted = wp_delete_term($id, 'product_tag');
			if (!$deleted || is_wp_error($deleted)) {
				throw new WC_API_Exception('woocommerce_api_cannot_delete_product_tag',
				                           __('Could not delete the tag', 'woocommerce'), 401);
			}
	...
	public function get_product_attributes($fields = NULL) {
		try {
			// Permissions check.
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_read_product_attributes',
				                           __('You do not have permission to read product attributes', 'woocommerce'),
				                           401);
			}
			$product_attributes   = [];
			...
			return ['product_attributes' => apply_filters('woocommerce_api_product_attributes_response',
			                                              $product_attributes, $attribute_taxonomies, $fields, $this)];
		} catch (WC_API_Exception $e) {
			return new WP_Error($e->getErrorCode(), $e->getMessage(), ['status' => $e->getCode()]);
		}
	}
	...
	public function create_product_attribute($data) {
		...
		try {
			if (!isset($data['product_attribute'])) {
				throw new WC_API_Exception('woocommerce_api_missing_product_attribute_data',
				                           sprintf(__('No %1$s data specified to create %1$s', 'woocommerce'),
				                                   'product_attribute'), 400);
			}
			$data = $data['product_attribute'];
			// Check permissions.
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_create_product_attribute',
				                           __('You do not have permission to create product attributes', 'woocommerce'),
				                           401);
			}
			...
			if (is_wp_error($insert)) {
				throw new WC_API_Exception('woocommerce_api_cannot_create_product_attribute',
				                           $insert->get_error_message(), 400);
			}
			$id = $wpdb->insert_id;
	...
	protected function validate_attribute_data($name, $slug, $type, $order_by, $new_data = TRUE) {
		if (empty($name)) {
			throw new WC_API_Exception('woocommerce_api_missing_product_attribute_name',
			                           sprintf(__('Missing parameter %s', 'woocommerce'), 'name'), 400);
		}
		if (strlen($slug) >= 28) {
			throw new WC_API_Exception('woocommerce_api_invalid_product_attribute_slug_too_long',
			                           sprintf(__('Slug "%s" is too long (28 characters max). Shorten it, please.',
			                                      'woocommerce'), $slug), 400);
		} else {
			if (wc_check_if_attribute_name_is_reserved($slug)) {
				throw new WC_API_Exception('woocommerce_api_invalid_product_attribute_slug_reserved_name',
				                           sprintf(__('Slug "%s" is not allowed because it is a reserved term. Change it, please.',
				                                      'woocommerce'), $slug), 400);
			} else {
				if ($new_data && taxonomy_exists(wc_attribute_taxonomy_name($slug))) {
					throw new WC_API_Exception('woocommerce_api_invalid_product_attribute_slug_already_exists',
					                           sprintf(__('Slug "%s" is already in use. Change it, please.',
					                                      'woocommerce'), $slug), 400);
				}
			}
		}
		...
		if (!in_array(wc_clean($type), array_keys(wc_get_attribute_types()))) {
			throw new WC_API_Exception('woocommerce_api_invalid_product_attribute_type',
			                           sprintf(__('Invalid product attribute type - the product attribute type must be any of these: %s',
			                                      'woocommerce'), implode(', ', array_keys(wc_get_attribute_types()))),
			                           400);
		}
		// Validate the attribute order by
		if (!in_array(wc_clean($order_by), ['menu_order', 'name', 'name_num', 'id'])) {
			throw new WC_API_Exception('woocommerce_api_invalid_product_attribute_order_by',
			                           sprintf(__('Invalid product attribute order_by type - the product attribute order_by type must be any of these: %s',
			                                      'woocommerce'),
			                                   implode(', ', ['menu_order', 'name', 'name_num', 'id'])), 400);
		}
	...
	public function get_product_attribute($id, $fields = NULL) {
		...
		try {
			...
			// Validate ID
			if (empty($id)) {
				throw new WC_API_Exception('woocommerce_api_invalid_product_attribute_id',
				                           __('Invalid product attribute ID', 'woocommerce'), 400);
			}
			// Permissions check
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_read_product_attributes',
			...
			if (is_wp_error($attribute) || is_null($attribute)) {
				throw new WC_API_Exception('woocommerce_api_invalid_product_attribute_id',
				                           __('A product attribute with the provided ID could not be found',
				                              'woocommerce'), 404);
			}
			$product_attribute = [
				'id'           => intval($attribute->attribute_id),
			...
			return ['product_attribute' => apply_filters('woocommerce_api_product_attribute_response',
			                                             $product_attribute, $id, $fields, $attribute, $this)];
		} catch (WC_API_Exception $e) {
			return new WP_Error($e->getErrorCode(), $e->getMessage(), ['status' => $e->getCode()]);
		}
	}
	...
	public function edit_product_attribute($id, $data) {
		...
		try {
			...
			// Check permissions.
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_edit_product_attribute',
				                           __('You do not have permission to edit product attributes', 'woocommerce'),
				                           401);
			}
			...
			$attribute_order_by = isset($data['order_by']) ? $data['order_by']
				: $attribute['product_attribute']['order_by'];
			if (isset($data['slug'])) {
			...
			// Validate the attribute data.
			$this->validate_attribute_data($attribute_name, $attribute_slug, $attribute_type, $attribute_order_by,
			                               FALSE);
			$update = $wpdb->update(
				$wpdb->prefix . 'woocommerce_attribute_taxonomies',
				[
			...
			// Checks for an error in the product creation.
			if (FALSE === $update) {
				throw new WC_API_Exception('woocommerce_api_cannot_edit_product_attribute',
				                           __('Could not edit the attribute', 'woocommerce'), 400);
			}
	...
	public function delete_product_attribute($id) {
		...
		try {
			// Check permissions.
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_delete_product_attribute',
				                           __('You do not have permission to delete product attributes', 'woocommerce'),
				                           401);
			}
			$id = absint($id);
			...
			if (FALSE === $deleted) {
				throw new WC_API_Exception('woocommerce_api_cannot_delete_product_attribute',
				                           __('Could not delete the attribute', 'woocommerce'), 401);
			}
	...
	public function get_product_attribute_terms($attribute_id, $fields = NULL) {
		try {
			// Permissions check.
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_read_product_attribute_terms',
				                           __('You do not have permission to read product attribute terms',
				                              'woocommerce'), 401);
			}
			...
			return ['product_attribute_terms' => apply_filters('woocommerce_api_product_attribute_terms_response',
			                                                   $attribute_terms, $terms, $fields, $this)];
		} catch (WC_API_Exception $e) {
			return new WP_Error($e->getErrorCode(), $e->getMessage(), ['status' => $e->getCode()]);
		}
	}
	...
	public function create_product_attribute_term($attribute_id, $data) {
		...
		try {
			if (!isset($data['product_attribute_term'])) {
				throw new WC_API_Exception('woocommerce_api_missing_product_attribute_term_data',
				                           sprintf(__('No %1$s data specified to create %1$s', 'woocommerce'),
				                                   'product_attribute_term'), 400);
			}
			...
			if (!isset($data['name'])) {
				throw new WC_API_Exception('woocommerce_api_missing_product_attribute_term_name',
				                           sprintf(__('Missing parameter %s', 'woocommerce'), 'name'), 400);
			}
			$args = [];
	...
	public function get_product_attribute_term($attribute_id, $id, $fields = NULL) {
		...
		try {
			...
			// Validate ID
			if (empty($id)) {
				throw new WC_API_Exception('woocommerce_api_invalid_product_attribute_term_id',
				                           __('Invalid product attribute ID', 'woocommerce'), 400);
			}
			// Permissions check
			if (!current_user_can('manage_product_terms')) {
			...
			if (is_wp_error($term) || is_null($term)) {
				throw new WC_API_Exception('woocommerce_api_invalid_product_attribute_term_id',
				                           __('A product attribute term with the provided ID could not be found',
				                              'woocommerce'), 404);
			}
			$attribute_term = [
				'id'    => $term->term_id,
			...
			return ['product_attribute_term' => apply_filters('woocommerce_api_product_attribute_response',
			                                                  $attribute_term, $id, $fields, $term, $this)];
		} catch (WC_API_Exception $e) {
			return new WP_Error($e->getErrorCode(), $e->getMessage(), ['status' => $e->getCode()]);
		}
	}
	...
	public function edit_product_attribute_term($attribute_id, $id, $data) {
		...
		try {
			...
			if (is_wp_error($term)) {
				throw new WC_API_Exception('woocommerce_api_cannot_edit_product_attribute_term',
				                           $term->get_error_message(), 400);
			}
	...
	public function delete_product_attribute_term($attribute_id, $id) {
		...
		try {
			// Check permissions.
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_delete_product_attribute_term',
				                           __('You do not have permission to delete product attribute terms',
				                              'woocommerce'), 401);
			}
			$taxonomy = wc_attribute_taxonomy_name_by_id($attribute_id);
			if (!$taxonomy) {
			...
			if (!$term) {
				throw new WC_API_Exception('woocommerce_api_cannot_delete_product_attribute_term',
				                           sprintf(__('This %s cannot be deleted', 'woocommerce'),
				                                   'product_attribute_term'), 500);
			} else {
				if (is_wp_error($term)) {
					throw new WC_API_Exception('woocommerce_api_cannot_delete_product_attribute_term',
					                           $term->get_error_message(), 400);
	...
	public function bulk($data) {
		try {
			if (!isset($data['products'])) {
				throw new WC_API_Exception('woocommerce_api_missing_products_data',
			...
			if (count($data) > $limit) {
				throw new WC_API_Exception('woocommerce_api_products_request_entity_too_large',
	...
	public function edit_product($id, $data) {
		try {
			if (!isset($data['product'])) {
				throw new WC_API_Exception('woocommerce_api_missing_product_data',
				                           sprintf(__('No %1$s data specified to edit %1$s', 'woocommerce'), 'product'),
				                           400);
			}
			$data = $data['product'];
			...
			if (isset($data['short_description'])) {
				...
				$post_excerpt
					= (isset($data['enable_html_short_description']) && TRUE === $data['enable_html_short_description'])
					? $data['short_description'] : wc_clean($data['short_description']);
				wp_update_post(['ID' => $id, 'post_excerpt' => $post_excerpt]);
			}
			// Product description.
			if (isset($data['description'])) {
				// Enable description html tags.
				$post_content = (isset($data['enable_html_description']) && TRUE === $data['enable_html_description'])
					? $data['description'] : wc_clean($data['description']);
				wp_update_post(['ID' => $id, 'post_content' => $post_content]);
			}
			// Menu order.
			if (isset($data['menu_order'])) {
				wp_update_post(['ID' => $id, 'menu_order' => intval($data['menu_order'])]);
			}
			// Validate the product type.
			if (isset($data['type']) && !in_array(wc_clean($data['type']), array_keys(wc_get_product_types()))) {
				throw new WC_API_Exception('woocommerce_api_invalid_product_type',
				                           sprintf(__('Invalid product type - the product type must be any of these: %s',
				                                      'woocommerce'),
				                                   implode(', ', array_keys(wc_get_product_types()))), 400);
			}
				...
				if (isset($image['position']) && $image['position'] == 0) {
					...
					if (0 === $attachment_id && isset($image['src'])) {
						...
						if (is_wp_error($upload)) {
							throw new WC_API_Exception('woocommerce_api_cannot_upload_product_image',
							                           $upload->get_error_message(), 400);
						}
		...
		if (isset($data['sku'])) {
			...
			if ('' == $new_sku) {
				...
				if (!empty($new_sku)) {
					...
					if (!$unique_sku) {
						throw new WC_API_Exception('woocommerce_api_product_sku_already_exists',
						                           __('The SKU already exists on another product', 'woocommerce'), 400);
					} else {
				...
				if ($is_taxonomy) {
					...
					if (!empty($values)) {
						...
						$attributes[ $taxonomy ] = [
							'name'         => $taxonomy,
							'value'        => '',
							'position'     => isset($attribute['position']) ? (string)absint($attribute['position'])
								: '0',
					...
					if (is_array($attribute['options'])) {
						...
						$values = implode(' ' . WC_DELIMITER . ' ',
						                  array_map('wc_clean', explode(WC_DELIMITER, $attribute['options'])));
					}
	...
	private function save_product_shipping_data($id, $data) {
		...
		if (isset($data['dimensions'])) {
			...
			if (isset($data['dimensions']['width'])) {
				update_post_meta($id, '_width', ('' === $data['dimensions']['width']) ? ''
					: wc_format_decimal($data['dimensions']['width']));
			}
			// Length
			if (isset($data['dimensions']['length'])) {
				update_post_meta($id, '_length', ('' === $data['dimensions']['length']) ? ''
					: wc_format_decimal($data['dimensions']['length']));
			}
		}
		// Virtual
		if (isset($data['virtual'])) {
	...
	protected function save_variations($id, $data) {
		...
		foreach ($variations as $menu_order => $variation) {
			...
			$variation_post_title = sprintf(__('Variation #%s of %s', 'woocommerce'), $variation_id,
			                                esc_html(get_the_title($id)));
			// Update or Add post
			if (!$variation_id) {
				$post_status = (isset($variation['visible']) && FALSE === $variation['visible']) ? 'private'
					: 'publish';
				$new_variation = [
			...
			if (is_wp_error($variation_id)) {
				throw new WC_API_Exception('woocommerce_api_cannot_save_product_variation',
				                           $variation_id->get_error_message(), 400);
			}
			// SKU
			if (isset($variation['sku'])) {
				...
				if ('' == $new_sku) {
					...
					if (!empty($new_sku)) {
						...
						if (!$unique_sku) {
							throw new WC_API_Exception('woocommerce_api_product_sku_already_exists',
							                           __('The SKU already exists on another product', 'woocommerce'),
							                           400);
						} else {
				...
				foreach ($variation['attributes'] as $attribute_key => $attribute) {
					...
					if (isset($_attribute['is_variation']) && $_attribute['is_variation']) {
						...
						if (isset($_attribute['is_taxonomy']) && $_attribute['is_taxonomy']) {
							...
							$_attribute_value = isset($attribute['option'])
								? sanitize_title(stripslashes($attribute['option'])) : '';
						} else {
							$_attribute_value = isset($attribute['option'])
								? wc_clean(stripslashes($attribute['option'])) : '';
						}
	...
	public function create_product($data) {
		...
		try {
			...
			if (!current_user_can('publish_products')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_create_product',
				                           __('You do not have permission to create products', 'woocommerce'), 401);
			}
			$data = apply_filters('woocommerce_api_create_product_data', $data, $this);
			// Check if product title is specified.
			if (!isset($data['title'])) {
				throw new WC_API_Exception('woocommerce_api_missing_product_title',
				                           sprintf(__('Missing parameter %s', 'woocommerce'), 'title'), 400);
			}
			// Check product type.
			if (!isset($data['type'])) {
				$data['type'] = 'simple';
	...
	public function get_product_shipping_classes($fields = NULL) {
		try {
			// Permissions check
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_read_product_shipping_classes',
				                           __('You do not have permission to read product shipping classes',
				                              'woocommerce'), 401);
			}
			...
			return ['product_shipping_classes' => apply_filters('woocommerce_api_product_shipping_classes_response',
			                                                    $product_shipping_classes, $terms, $fields, $this)];
		} catch (WC_API_Exception $e) {
			return new WP_Error($e->getErrorCode(), $e->getMessage(), ['status' => $e->getCode()]);
		}
	}
	...
	public function get_product_shipping_class($id, $fields = NULL) {
		try {
			$id = absint($id);
			if (!$id) {
				throw new WC_API_Exception('woocommerce_api_invalid_product_shipping_class_id',
				                           __('Invalid product shipping class ID', 'woocommerce'), 400);
			}
			// Permissions check
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_read_product_shipping_classes',
			...
			if (is_wp_error($term) || is_null($term)) {
				throw new WC_API_Exception('woocommerce_api_invalid_product_shipping_class_id',
				                           __('A product shipping class with the provided ID could not be found',
				                              'woocommerce'), 404);
			}
			$term_id = intval($term->term_id);
			...
			return ['product_shipping_class' => apply_filters('woocommerce_api_product_shipping_class_response',
			                                                  $product_shipping_class, $id, $fields, $term, $this)];
		} catch (WC_API_Exception $e) {
			return new WP_Error($e->getErrorCode(), $e->getMessage(), ['status' => $e->getCode()]);
		}
	}
		...
		try {
			if (!isset($data['product_shipping_class'])) {
				throw new WC_API_Exception('woocommerce_api_missing_product_shipping_class_data',
				                           sprintf(__('No %1$s data specified to create %1$s', 'woocommerce'),
				                                   'product_shipping_class'), 400);
			}
			// Check permissions
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_create_product_shipping_class',
				                           __('You do not have permission to create product shipping classes',
				                              'woocommerce'), 401);
			}
			$defaults = [
				'name'        => '',
			...
			if ($data['parent']) {
				...
				if (!$parent) {
					throw new WC_API_Exception('woocommerce_api_invalid_product_shipping_class_parent',
					                           __('Product shipping class parent is invalid', 'woocommerce'), 400);
				}
			}
			$insert = wp_insert_term($data['name'], 'product_shipping_class', $data);
			if (is_wp_error($insert)) {
				throw new WC_API_Exception('woocommerce_api_cannot_create_product_shipping_class',
				                           $insert->get_error_message(), 400);
			}
			$id = $insert['term_id'];
	...
	public function edit_product_shipping_class($id, $data) {
		...
		try {
			if (!isset($data['product_shipping_class'])) {
				throw new WC_API_Exception('woocommerce_api_missing_product_shipping_class',
				                           sprintf(__('No %1$s data specified to edit %1$s', 'woocommerce'),
				                                   'product_shipping_class'), 400);
			}
			$id   = absint($id);
			...
			// Check permissions
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_edit_product_shipping_class',
				                           __('You do not have permission to edit product shipping classes',
				                              'woocommerce'), 401);
			}
			...
			if (is_wp_error($update)) {
				throw new WC_API_Exception('woocommerce_api_cannot_edit_product_shipping_class',
				                           __('Could not edit the shipping class', 'woocommerce'), 400);
			}
	...
	public function delete_product_shipping_class($id) {
		...
		try {
			// Check permissions
			if (!current_user_can('manage_product_terms')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_delete_product_shipping_class',
				                           __('You do not have permission to delete product shipping classes',
				                              'woocommerce'), 401);
			}
			$id      = absint($id);
			...
			if (!$deleted || is_wp_error($deleted)) {
				throw new WC_API_Exception('woocommerce_api_cannot_delete_product_shipping_class',
				                           __('Could not delete the shipping class', 'woocommerce'), 401);
			}
