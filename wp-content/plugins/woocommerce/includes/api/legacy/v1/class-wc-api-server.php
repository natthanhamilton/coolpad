 ...
 *
 * This class and related code (JSON response handler, resource classes) are based on WP-API v0.6
 * (https://github.com/WP-API/WP-API) Many thanks to Ryan McCue and any other contributors!
 *
 * @author      WooThemes
 * @category    API
...
class WC_API_Server {
	...
	public function __construct($path) {
		...
		if ($this->is_json_request()) {
			...
			$handler_class = apply_filters('woocommerce_api_default_response_handler', 'WC_API_JSON_Handler',
			                               $this->path, $this);
		}
	...
	public function serve_request() {
		...
		if (!apply_filters('woocommerce_api_enabled', TRUE,
		                   $this) || ('no' === get_option('woocommerce_api_enabled'))
		) {
	...
	public function check_authentication() {
		...
		elseif (!is_wp_error($user)) {
			$user = new WP_Error('woocommerce_api_authentication_error',
			                     __('Invalid authentication method', 'woocommerce'), ['code' => 500]);
		}
	...
	public function dispatch() {
		switch ($this->method) {
			...
			case 'DELETE':
				...
				break;
			default:
				return new WP_Error('woocommerce_api_unsupported_method',
				                    __('Unsupported request method', 'woocommerce'), ['status' => 400]);
		}
		foreach ($this->get_routes() as $route => $handlers) {
			foreach ($handlers as $handler) {
				...
				if (!is_callable($callback)) {
					return new WP_Error('woocommerce_api_invalid_handler',
					                    __('The handler for the route is invalid', 'woocommerce'), ['status' => 500]);
				}
		...
		return new WP_Error('woocommerce_api_no_route',
	 ...
	 *
	 * @since 2.1
	 * @return array `'/path/regex' => array( $callback, $bitmask )` or `'/path/regex' => array( array( $callback,
	 *               $bitmask ), ...)`
	 */
	...
	protected function sort_callback_params($callback, $provided) {
		...
		foreach ($wanted as $param) {
			if (isset($provided[ $param->getName() ])) {
				...
				$ordered_parameters[] = is_array($provided[ $param->getName() ]) ? array_map('urldecode',
				                                                                             $provided[ $param->getName() ])
					: urldecode($provided[ $param->getName() ]);
			} elseif ($param->isDefaultValueAvailable()) {
				// We don't have this parameter, but it's optional
				$ordered_parameters[] = $param->getDefaultValue();
			} else {
				// We don't have this parameter and it wasn't optional, abort!
				return new WP_Error('woocommerce_api_missing_callback_param',
				                    sprintf(__('Missing parameter %s', 'woocommerce'), $param->getName()),
				                    ['status' => 400]);
			}
		}
