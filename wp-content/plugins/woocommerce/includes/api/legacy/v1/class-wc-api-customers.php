...
class WC_API_Customers extends WC_API_Resource {
	...
	protected function validate_request($id, $type, $context) {
		...
		// validate ID
		if (empty($id)) {
			return new WP_Error('woocommerce_api_invalid_customer_id', __('Invalid customer ID', 'woocommerce'),
			                    ['status' => 404]);
		}
		// non-existent IDs return a valid WP_User object with the user ID = 0
		$customer = new WP_User($id);
		if (0 === $customer->ID) {
			return new WP_Error('woocommerce_api_invalid_customer', __('Invalid customer', 'woocommerce'),
			                    ['status' => 404]);
		}
		...
		switch ($context) {
			case 'read':
				if (!current_user_can('list_users')) {
					return new WP_Error('woocommerce_api_user_cannot_read_customer',
					                    __('You do not have permission to read this customer', 'woocommerce'),
					                    ['status' => 401]);
				}
				break;
			case 'edit':
				if (!current_user_can('edit_users')) {
					return new WP_Error('woocommerce_api_user_cannot_edit_customer',
					                    __('You do not have permission to edit this customer', 'woocommerce'),
					                    ['status' => 401]);
				}
				break;
			case 'delete':
				if (!current_user_can('delete_users')) {
					return new WP_Error('woocommerce_api_user_cannot_delete_customer',
					                    __('You do not have permission to delete this customer', 'woocommerce'),
					                    ['status' => 401]);
				}
				break;
		}
	...
	public function get_customers_count($filter = []) {
		$query = $this->query_customers($filter);
		if (!current_user_can('list_users')) {
			return new WP_Error('woocommerce_api_user_cannot_read_customers_count',
	...
	public function create_customer($data) {
		if (!current_user_can('create_users')) {
			return new WP_Error('woocommerce_api_user_cannot_create_customer',
