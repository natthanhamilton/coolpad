...
class WC_API_Reports extends WC_API_Resource {
	...
	public function get_sales_report($fields = NULL, $filter = []) {
		...
		for ($i = 0; $i <= $this->report->chart_interval; $i++) {
			...
			foreach ($customers as $customer) {
				if (date(('day' == $this->report->chart_groupby) ? 'Y-m-d' : 'Y-m',
				         strtotime($customer->user_registered)) == $time
				) {
					$customer_count++;
				}
			}
		...
		foreach ($orders as $order) {
			$time = ('day' === $this->report->chart_groupby) ? date('Y-m-d', strtotime($order->post_date))
		...
		foreach ($order_items as $order_item) {
			$time = ('day' === $this->report->chart_groupby) ? date('Y-m-d', strtotime($order_item->post_date))
		...
		foreach ($discounts as $discount) {
			$time = ('day' === $this->report->chart_groupby) ? date('Y-m-d', strtotime($discount->post_date))
		...
		$sales_data = [
			...
			'total_discount'    => is_null($total_discount) ? wc_format_decimal(0.00, 2)
		...
		return ['sales' => apply_filters('woocommerce_api_report_response', $sales_data, $this->report, $fields,
		                                 $this->server)];
	}
	...
	protected function validate_request($id = NULL, $type = NULL, $context = NULL) {
		if (!current_user_can('view_woocommerce_reports')) {
			return new WP_Error('woocommerce_api_user_cannot_read_report',
			                    __('You do not have permission to read this report', 'woocommerce'), ['status' => 401]);
		} else {
			return TRUE;
		}
	}
	...
	private function setup_report($filter) {
		...
		if (empty($filter['period'])) {
			...
			if (!empty($filter['date_min']) || !empty($filter['date_max'])) {
				...
				$_GET['end_date']   = isset($filter['date_max']) ? $this->server->parse_datetime($filter['date_max'])
					: NULL;
			} else {
	...
	public function get_top_sellers_report($fields = NULL, $filter = []) {
		...
		return ['top_sellers' => apply_filters('woocommerce_api_report_response', $top_sellers_data, $this->report,
		                                       $fields, $this->server)];
	}
}
