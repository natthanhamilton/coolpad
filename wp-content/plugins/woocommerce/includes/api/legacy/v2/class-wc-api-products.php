...
class WC_API_Products extends WC_API_Resource {
	...
	private function get_product_data($product) {
		...
		return [
			...
			'sale_price'         => $product->get_sale_price() ? wc_format_decimal($product->get_sale_price(),
			                                                                       $prices_precision) : NULL,
	...
	private function get_variation_data($product) {
		...
		foreach ($product->get_children() as $child_id) {
			...
			$variations[] = [
				...
				'sale_price'        => $variation->get_sale_price() ? wc_format_decimal($variation->get_sale_price(),
				                                                                        $prices_precision) : NULL,
	...
	public function get_product_by_sku($sku, $fields = NULL) {
		try {
			...
			if (empty($id)) {
				throw new WC_API_Exception('woocommerce_api_invalid_product_sku',
				                           __('Invalid product SKU', 'woocommerce'), 404);
			}
	...
	public function upload_product_image($image_url) {
		...
		if (!$parsed_url || !is_array($parsed_url)) {
			throw new WC_API_Exception('woocommerce_api_invalid_product_image',
		...
		if (is_wp_error($response)) {
			throw new WC_API_Exception('woocommerce_api_invalid_remote_product_image',
		...
		if (0 == $filesize) {
			@unlink($upload['file']);
			unset($upload);
			throw new WC_API_Exception('woocommerce_api_product_image_upload_file_error',
