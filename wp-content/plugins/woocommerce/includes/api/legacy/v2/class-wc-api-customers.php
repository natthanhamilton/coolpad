...
class WC_API_Customers extends WC_API_Resource {
	...
	public function get_customer($id, $fields = NULL) {
		...
		$customer_data = [
			...
			'last_order_date'  => is_object($last_order) ? $this->server->format_datetime($last_order->post_date_gmt)
				: NULL,
		...
		return ['customer' => apply_filters('woocommerce_api_customer_response', $customer_data, $customer, $fields,
		                                    $this->server)];
	}
		...
		try {
			...
			if (empty($id)) {
				throw new WC_API_Exception('woocommerce_api_invalid_customer_id',
				                           __('Invalid customer ID', 'woocommerce'), 404);
			}
			// non-existent IDs return a valid WP_User object with the user ID = 0
			$customer = new WP_User($id);
			if (0 === $customer->ID) {
				throw new WC_API_Exception('woocommerce_api_invalid_customer', __('Invalid customer', 'woocommerce'),
				                           404);
			}
			...
			switch ($context) {
				case 'read':
					if (!current_user_can('list_users')) {
						throw new WC_API_Exception('woocommerce_api_user_cannot_read_customer',
						                           __('You do not have permission to read this customer',
						                              'woocommerce'), 401);
					}
					break;
				case 'edit':
					if (!current_user_can('edit_users')) {
						throw new WC_API_Exception('woocommerce_api_user_cannot_edit_customer',
						                           __('You do not have permission to edit this customer',
						                              'woocommerce'), 401);
					}
					break;
				case 'delete':
					if (!current_user_can('delete_users')) {
						throw new WC_API_Exception('woocommerce_api_user_cannot_delete_customer',
						                           __('You do not have permission to delete this customer',
						                              'woocommerce'), 401);
					}
					break;
			}
			...
			if (is_email($email)) {
				...
				if (!is_object($customer)) {
					throw new WC_API_Exception('woocommerce_api_invalid_customer_email',
					                           __('Invalid customer Email', 'woocommerce'), 404);
				}
			} else {
				throw new WC_API_Exception('woocommerce_api_invalid_customer_email',
				                           __('Invalid customer Email', 'woocommerce'), 404);
	...
	public function get_customers_count($filter = []) {
		try {
			if (!current_user_can('list_users')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_read_customers_count',
				                           __('You do not have permission to read the customers count', 'woocommerce'),
				                           401);
			}
			$query = $this->query_customers($filter);
	...
	public function get_customer_orders($id, $fields = NULL) {
		...
		return ['orders' => apply_filters('woocommerce_api_customer_orders_response', $orders, $id, $fields, $order_ids,
		                                  $this->server)];
	}
	...
	public function get_customer_downloads($id, $fields = NULL) {
		...
		return ['downloads' => apply_filters('woocommerce_api_customer_downloads_response', $downloads, $id, $fields,
		                                     $this->server)];
	}
	...
	public function modify_user_query($query) {
		if ($this->created_at_min) {
			$query->query_where .= sprintf(" AND user_registered >= STR_TO_DATE( '%s', '%%Y-%%m-%%d %%H:%%i:%%s' )",
			                               esc_sql($this->created_at_min));
		}
		if ($this->created_at_max) {
			$query->query_where .= sprintf(" AND user_registered <= STR_TO_DATE( '%s', '%%Y-%%m-%%d %%H:%%i:%%s' )",
			                               esc_sql($this->created_at_max));
		}
	}
	...
	public function bulk($data) {
		try {
			if (!isset($data['customers'])) {
				throw new WC_API_Exception('woocommerce_api_missing_customers_data',
			...
			if (count($data) > $limit) {
				throw new WC_API_Exception('woocommerce_api_customers_request_entity_too_large',
	...
	public function edit_customer($id, $data) {
		try {
			if (!isset($data['customer'])) {
				throw new WC_API_Exception('woocommerce_api_missing_customer_data',
			...
			// Customer email.
			if (isset($data['email'])) {
				wp_update_user(['ID' => $id, 'user_email' => sanitize_email($data['email'])]);
			}
			// Customer password.
			if (isset($data['password'])) {
				wp_update_user(['ID' => $id, 'user_pass' => wc_clean($data['password'])]);
			}
	...
	public function create_customer($data) {
		try {
			...
			if (!current_user_can('create_users')) {
				throw new WC_API_Exception('woocommerce_api_user_cannot_create_customer',
				                           __('You do not have permission to create this customer', 'woocommerce'),
				                           401);
			}
			$data = apply_filters('woocommerce_api_create_customer_data', $data, $this);
			// Checks with the email is missing.
			if (!isset($data['email'])) {
				throw new WC_API_Exception('woocommerce_api_missing_customer_email',
				                           sprintf(__('Missing parameter %s', 'woocommerce'), 'email'), 400);
			}
			// Sets the username.
