...
class WC_API_JSON_Handler implements WC_API_Handler {
	...
	public function get_content_type() {
		return sprintf('%s; charset=%s', isset($_GET['_jsonp']) ? 'application/javascript' : 'application/json',
	...
	public function generate_response($data) {
		if (isset($_GET['_jsonp'])) {
			...
			if (!apply_filters('woocommerce_api_jsonp_enabled', TRUE)) {
				...
				$data
					= [['code' => 'woocommerce_api_jsonp_disabled', 'message' => __('JSONP support is disabled on this site',
					                                                                'woocommerce')]];
			}
			...
			if (preg_match('/\W/', $_GET['_jsonp'])) {
				WC()->api->server->send_status(400);
				$data
					= [['code' => 'woocommerce_api_jsonp_callback_invalid', __('The JSONP callback function is invalid',
					                                                           'woocommerce')]];
			}
