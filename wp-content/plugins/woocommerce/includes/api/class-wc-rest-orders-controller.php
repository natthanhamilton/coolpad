...
class WC_REST_Orders_Controller extends WC_REST_Posts_Controller {
	...
	protected function set_line_item($order, $item, $action = 'create') {
		...
		if (empty($item['product_id']) && empty($item['sku']) && empty($item['variation_id'])) {
			throw new WC_REST_Exception('woocommerce_rest_required_product_reference',
		...
		if ('update' === $action && !empty($item['id'])) {
			...
			if ($product_id !== $item_product_id && $product_id !== $item_variation_id) {
				throw new WC_REST_Exception('woocommerce_rest_required_product_reference',
				                            __('Product ID or variation ID provided does not match this line item.',
				                               'woocommerce'), 400);
			}
		}
		...
		if (!is_object($product)) {
			throw new WC_REST_Exception('woocommerce_rest_invalid_product', __('Product is invalid.', 'woocommerce'),
			                            400);
		}
		// Quantity must be positive float.
		if (isset($item['quantity']) && 0 >= floatval($item['quantity'])) {
			throw new WC_REST_Exception('woocommerce_rest_invalid_product_quantity',
		...
		if ($creating) {
			...
			if (!$item_id) {
				throw new WC_REST_Exception('woocommerce_rest_cannot_create_line_item',
				                            __('Cannot create line item, try again.', 'woocommerce'), 500);
			}
		} else {
			$item_id = $order->update_product($item['id'], $product, $item_args);
			if (!$item_id) {
				throw new WC_REST_Exception('woocommerce_rest_cannot_update_line_item',
	...
	protected function set_shipping($order, $shipping, $action) {
		...
		if (!empty($shipping['total']) && 0 > floatval($shipping['total'])) {
			throw new WC_REST_Exception('woocommerce_rest_invalid_shipping_total',
			                            __('Shipping total must be a positive amount.', 'woocommerce'), 400);
		}
		if ('create' === $action) {
			// Method ID is required.
			if (empty($shipping['method_id'])) {
				throw new WC_REST_Exception('woocommerce_rest_invalid_shipping_item',
			...
			if (!$shipping_id) {
				throw new WC_REST_Exception('woocommerce_rest_cannot_create_shipping',
			...
			if (!$shipping_id) {
				throw new WC_REST_Exception('woocommerce_rest_cannot_update_shipping',
	...
	protected function set_fee($order, $fee, $action) {
		if ('create' === $action) {
			...
			if (empty($fee['name'])) {
				throw new WC_REST_Exception('woocommerce_rest_invalid_fee_item',
				                            __('Fee name is required.', 'woocommerce'), 400);
			}
			...
			if (!$fee_id) {
				throw new WC_REST_Exception('woocommerce_rest_cannot_create_fee',
			...
			if (!$fee_id) {
				throw new WC_REST_Exception('woocommerce_rest_cannot_update_fee',
	...
	protected function set_coupon($order, $coupon, $action) {
		...
		if (isset($coupon['discount']) && 0 > floatval($coupon['discount'])) {
			throw new WC_REST_Exception('woocommerce_rest_invalid_coupon_total',
			                            __('Coupon discount must be a positive amount.', 'woocommerce'), 400);
		}
		if ('create' === $action) {
			// Coupon code is required.
			if (empty($coupon['code'])) {
				throw new WC_REST_Exception('woocommerce_rest_invalid_coupon_coupon',
				                            __('Coupon code is required.', 'woocommerce'), 400);
			}
			$coupon_id = $order->add_coupon($coupon['code'], floatval($coupon['discount']));
			if (!$coupon_id) {
				throw new WC_REST_Exception('woocommerce_rest_cannot_create_order_coupon',
			...
			if (!$coupon_id) {
				throw new WC_REST_Exception('woocommerce_rest_cannot_update_order_coupon',
	...
	protected function create_order($request) {
		...
		try {
			...
			if (0 !== $request['customer_id'] && FALSE === get_user_by('id', $request['customer_id'])) {
				throw new WC_REST_Exception('woocommerce_rest_invalid_customer_id',
			...
			if (is_wp_error($order)) {
				throw new WC_REST_Exception('woocommerce_rest_cannot_create_order',
	...
	protected function set_item($order, $item_type, $item, $action) {
		...
		if ('update' === $action) {
			...
			if (is_null($result)) {
				throw new WC_REST_Exception('woocommerce_rest_invalid_item_id',
		...
		try {
			...
			if (!empty($request['status'])) {
				$order->update_status($request['status'],
				                      isset($request['status_note']) ? $request['status_note'] : '');
			}
	...
	public function update_item($request) {
		...
		if (empty($id) || empty($post->ID) || $this->post_type !== $post->post_type) {
			return new WP_Error("woocommerce_rest_{$this->post_type}_invalid_id", __('ID is invalid.', 'woocommerce'),
			                    ['status' => 400]);
		}
	...
	public function get_item_schema() {
		$schema = [
			...
			'properties' => [
				...
				'set_paid'             => [
					'description' => __('Define if the order is paid. It will set the status to processing and reduce stock items.',
					                    'woocommerce'),
					'type'        => 'boolean',
					'default'     => FALSE,
					'context'     => ['edit'],
