...
class WC_REST_Products_Controller extends WC_REST_Posts_Controller {
	...
	public function get_collection_params() {
		...
		$params['shipping_class'] = [
			'description'       => __('Limit result set to products assigned a specific shipping class.',
			                          'woocommerce'),
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
			'validate_callback' => 'rest_validate_request_arg',
		...
		$params['attribute_term'] = [
			'description'       => __('Limit result set to products with a specific attribute term (required an assigned attribute).',
			                          'woocommerce'),
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
			'validate_callback' => 'rest_validate_request_arg',
	...
	protected function get_default_attributes($product) {
		...
		if ($product->is_type('variable')) {
			foreach (array_filter((array)get_post_meta($product->id, '_default_attributes', TRUE), 'strlen') as $key =>
			         $value) {
				if (0 === strpos($key, 'pa_')) {
					$default[] = [
	...
	protected function get_product_data($product) {
		$data = [
			...
			'parent_id'          => $product->is_type('variation') ? $product->parent->id
				: $product->get_post_data()->post_parent,
	...
	protected function get_variation_data($product) {
		...
		foreach ($product->get_children() as $child_id) {
			...
			$variations[] = [
				...
				'date_on_sale_from'  => $variation->sale_price_dates_from ? date('Y-m-d',
				                                                                 $variation->sale_price_dates_from)
					: '',
	...
	private function save_product_shipping_data($product_id, $data) {
		...
		if (isset($data['virtual']) && TRUE === $data['virtual']) {
			...
			if (isset($data['weight'])) {
				update_post_meta($product_id, '_weight',
				                 '' === $data['weight'] ? '' : wc_format_decimal($data['weight']));
			}
			// Height.
	...
	protected function save_product_meta($product, $request) {
		...
		if (isset($request['attributes'])) {
			...
			foreach ($request['attributes'] as $attribute) {
				...
				if ($attribute_id) {
					...
					$attributes[ sanitize_title($attribute_name) ] = [
						'name'         => $attribute_name,
						'value'        => $values,
		...
		if (isset($request['parent_id'])) {
			$parent_id = wp_update_post(['ID' => $product->id, 'post_parent' => absint($request['parent_id'])]);
		}
		...
		if ('yes' === $is_downloadable) {
			...
			if (isset($request['download_limit'])) {
				update_post_meta($product->id, '_download_limit',
				                 -1 === $request['download_limit'] ? '' : absint($request['download_limit']));
			}
			// Download expiry.
			if (isset($request['download_expiry'])) {
				update_post_meta($product->id, '_download_expiry',
				                 -1 === $request['download_expiry'] ? '' : absint($request['download_expiry']));
			}
			// Download type.
			if (isset($request['download_type'])) {
				update_post_meta($product->id, '_download_type',
				                 'standard' === $request['download_type'] ? '' : wc_clean($request['download_type']));
			}
		}
	...
	protected function save_variations_data($product, $request) {
		...
		foreach ($variations as $menu_order => $variation) {
			...
			if (is_wp_error($variation_id)) {
				throw new WC_REST_Exception('woocommerce_rest_cannot_save_product_variation',
			...
			if ('yes' === $is_downloadable) {
				...
				if (isset($variation['download_limit'])) {
					update_post_meta($variation_id, '_download_limit',
					                 -1 === $variation['download_limit'] ? '' : absint($variation['download_limit']));
				}
				// Download expiry.
				if (isset($variation['download_expiry'])) {
					update_post_meta($variation_id, '_download_expiry',
					                 -1 === $variation['download_expiry'] ? '' : absint($variation['download_expiry']));
				}
			} else {
				update_post_meta($variation_id, '_download_limit', '');
				update_post_meta($variation_id, '_download_expiry', '');
	...
	public function delete_item($request) {
		...
		if (empty($id) || empty($post->ID) || !in_array($post->post_type, $this->get_post_types())) {
			return new WP_Error("woocommerce_rest_{$this->post_type}_invalid_id", __('Invalid post id.', 'woocommerce'),
			                    ['status' => 404]);
		}
		...
		if (!wc_rest_check_post_permissions($this->post_type, 'delete', $post->ID)) {
			return new WP_Error("woocommerce_rest_user_cannot_delete_{$this->post_type}",
			                    sprintf(__('Sorry, you are not allowed to delete %s.', 'woocommerce'),
			                            $this->post_type), ['status' => rest_authorization_required_code()]);
		}
		...
		if ($force) {
			...
			if ('trash' === $post->post_status) {
				return new WP_Error('woocommerce_rest_already_trashed',
	...
	public function get_item_schema() {
		...
		$schema         = [
			...
			'properties' => [
				...
				'date_modified'      => [
					'description' => __("The date the product was last modified, in the site's timezone.",
					                    'woocommerce'),
					'type'        => 'date-time',
					'context'     => ['view', 'edit'],
					'readonly'    => TRUE,
				...
				'download_expiry'    => [
					'description' => __('Number of days that the customer has up to be able to download the product.',
					                    'woocommerce'),
					'type'        => 'integer',
					'default'     => -1,
					'context'     => ['view', 'edit'],
				...
				'download_type'      => [
					...
					'type'        => 'string',
					'default'     => 'standard',
					'enum'        => ['standard', 'application', 'music'],
					'context'     => ['view', 'edit'],
				],
				'external_url'       => [
				...
				'in_stock'           => [
					'description' => __('Controls whether or not the product is listed as "in stock" or "out of stock" on the frontend.',
					                    'woocommerce'),
					'type'        => 'boolean',
					'default'     => TRUE,
					'context'     => ['view', 'edit'],
				...
				'attributes'         => [
					...
					'properties'  => [
						...
						'visible'   => [
							'description' => __("Define if the attribute is visible on the \"Additional Information\" tab in the product's page.",
							                    'woocommerce'),
							'type'        => 'boolean',
							'default'     => FALSE,
							'context'     => ['view', 'edit'],
				...
				'variations'         => [
					...
					'properties'  => [
						...
						'date_created'       => [
							'description' => __("The date the variation was created, in the site's timezone.",
							                    'woocommerce'),
							'type'        => 'date-time',
							'context'     => ['view', 'edit'],
							'readonly'    => TRUE,
						...
						'date_modified'      => [
							'description' => __("The date the variation was last modified, in the site's timezone.",
							                    'woocommerce'),
							'type'        => 'date-time',
							'context'     => ['view', 'edit'],
							'readonly'    => TRUE,
						...
						'download_expiry'    => [
							'description' => __('Number of days that the customer has up to be able to download the variation.',
							                    'woocommerce'),
							'type'        => 'integer',
							'default'     => NULL,
							'context'     => ['view', 'edit'],
						...
						'in_stock'           => [
							'description' => __('Controls whether or not the variation is listed as "in stock" or "out of stock" on the frontend.',
							                    'woocommerce'),
							'type'        => 'boolean',
							'default'     => TRUE,
							'context'     => ['view', 'edit'],
						...
						'backorders'         => [
							'description' => __('If managing stock, this controls if backorders are allowed.',
							                    'woocommerce'),
							'type'        => 'string',
							'default'     => 'no',
							'enum'        => ['no', 'notify', 'yes'],
						...
						'dimensions'         => [
							...
							'properties'  => [
								'length' => [
									'description' => sprintf(__('Variation length (%s).', 'woocommerce'),
									                         $dimension_unit),
									'type'        => 'string',
									'context'     => ['view', 'edit'],
								],
								'width'  => [
									'description' => sprintf(__('Variation width (%s).', 'woocommerce'),
									                         $dimension_unit),
									'type'        => 'string',
									'context'     => ['view', 'edit'],
								],
								'height' => [
									'description' => sprintf(__('Variation height (%s).', 'woocommerce'),
									                         $dimension_unit),
									'type'        => 'string',
									'context'     => ['view', 'edit'],
								],
						...
						'image'              => [
							...
							'properties'  => [
								...
								'position'      => [
									'description' => __('Image position. 0 means that the image is featured.',
									                    'woocommerce'),
									'type'        => 'integer',
									'context'     => ['view', 'edit'],
								],
