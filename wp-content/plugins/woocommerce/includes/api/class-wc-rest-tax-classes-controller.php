...
class WC_REST_Tax_Classes_Controller extends WC_REST_Controller {
	...
	public function register_routes() {
		...
		register_rest_route($this->namespace, '/' . $this->rest_base . '/(?P<slug>\w[\w\s\-]*)', [
			[
	...
	public function create_item($request) {
		...
		if ($exists) {
			return new WP_Error('woocommerce_rest_tax_class_exists',
		...
		$response->header('Location',
		                  rest_url(sprintf('/%s/%s/%s', $this->namespace, $this->rest_base, $tax_class['slug'])));
	...
	public function delete_item($request) {
		...
		$wpdb->delete($wpdb->prefix . 'woocommerce_tax_rates', ['tax_rate_class' => $tax_class['slug']], ['%s']);
		$request->set_param('context', 'edit');
		$response = $this->prepare_item_for_response($tax_class, $request);
		/**
