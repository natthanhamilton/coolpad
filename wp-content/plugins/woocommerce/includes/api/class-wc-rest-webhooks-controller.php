...
class WC_REST_Webhooks_Controller extends WC_REST_Posts_Controller {
	...
	public function create_item($request) {
		...
		if (empty($request['topic']) || !wc_is_webhook_valid_topic(strtolower($request['topic']))) {
			return new WP_Error("woocommerce_rest_{$this->post_type}_invalid_topic",
			                    __('Webhook topic is required and must be valid.', 'woocommerce'), ['status' => 400]);
		}
		// Validate delivery URL.
		if (empty($request['delivery_url']) || !wc_is_valid_url($request['delivery_url'])) {
			return new WP_Error("woocommerce_rest_{$this->post_type}_invalid_delivery_url",
			                    __('Webhook delivery URL must be a valid URL starting with http:// or https://.',
			                       'woocommerce'), ['status' => 400]);
		}
	...
	protected function prepare_item_for_database($request) {
		...
		if ('POST' === $request->get_method() && empty($data->ID)) {
			$data->post_title = !empty($request['name'])
				? $request['name']
	...
	public function update_item($request) {
		...
		if (!empty($request['topic'])) {
			if (wc_is_webhook_valid_topic(strtolower($request['topic']))) {
				...
				return new WP_Error("woocommerce_rest_{$this->post_type}_invalid_topic",
				                    __('Webhook topic must be valid.', 'woocommerce'), ['status' => 400]);
			}
		}
		// Update delivery URL.
	...
	public function delete_item($request) {
		...
		if (!$force) {
			return new WP_Error('woocommerce_rest_trash_not_supported',
			                    __('Webhooks do not support trashing.', 'woocommerce'), ['status' => 501]);
		}
		$post = get_post($id);
		if (empty($id) || empty($post->ID) || $this->post_type !== $post->post_type) {
	...
	public function get_item_schema() {
		$schema = [
			...
			'properties' => [
				...
				'secret'        => [
					'description' => __("Secret key used to generate a hash of the delivered webhook and provided in the request headers. This will default is a MD5 hash from the current user's ID|username if not provided.",
					                    'woocommerce'),
					'type'        => 'string',
					'context'     => ['edit'],
				],
				...
				'date_modified' => [
					'description' => __("The date the webhook was last modified, in the site's timezone.",
					                    'woocommerce'),
					'type'        => 'date-time',
					'context'     => ['view', 'edit'],
					'readonly'    => TRUE,
