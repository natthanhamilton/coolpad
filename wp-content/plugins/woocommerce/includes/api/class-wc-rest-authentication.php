...
class WC_REST_Authentication {
	...
	private function perform_basic_authentication() {
		...
		if (!hash_equals($user->consumer_secret, $consumer_secret)) {
			$wc_rest_authentication_error = new WP_Error('woocommerce_rest_authentication_error',
		...
		switch ($_SERVER['REQUEST_METHOD']) {
			...
			case 'GET' :
				if ('read' !== $permissions && 'read_write' !== $permissions) {
					$wc_rest_authentication_error = new WP_Error('woocommerce_rest_authentication_error',
					                                             __('The API key provided does not have read permissions.',
					                                                'woocommerce'), ['status' => 401]);
					$valid                        = FALSE;
				}
				break;
			case 'POST' :
			...
			case 'DELETE' :
				if ('write' !== $permissions && 'read_write' !== $permissions) {
					$wc_rest_authentication_error = new WP_Error('woocommerce_rest_authentication_error',
					                                             __('The API key provided does not have write permissions.',
					                                                'woocommerce'), ['status' => 401]);
					$valid                        = FALSE;
				}
				break;
	...
	private function perform_oauth_authentication() {
		global $wc_rest_authentication_error;
		$params = ['oauth_consumer_key', 'oauth_timestamp', 'oauth_nonce', 'oauth_signature', 'oauth_signature_method'];
		...
		$wc_rest_authentication_error = $this->check_oauth_timestamp_and_nonce($user, $_GET['oauth_timestamp'],
		                                                                       $_GET['oauth_nonce']);
		if (is_wp_error($wc_rest_authentication_error)) {
			return FALSE;
		}
	...
	private function check_oauth_signature($user, $params) {
		...
		if (!uksort($params, 'strcmp')) {
			return new WP_Error('woocommerce_rest_authentication_error',
			                    __('Invalid Signature - failed to sort parameters.', 'woocommerce'), ['status' => 401]);
		}
		...
		if ($params['oauth_signature_method'] !== 'HMAC-SHA1' && $params['oauth_signature_method'] !== 'HMAC-SHA256') {
			return new WP_Error('woocommerce_rest_authentication_error',
			                    __('Invalid Signature - signature method is invalid.', 'woocommerce'),
			                    ['status' => 401]);
		}
		...
		if (!hash_equals($signature, $consumer_signature)) {
			return new WP_Error('woocommerce_rest_authentication_error',
			                    __('Invalid Signature - provided signature does not match.', 'woocommerce'),
			                    ['status' => 401]);
		}
	...
	private function check_oauth_timestamp_and_nonce($user, $timestamp, $nonce) {
		...
		if (($timestamp < time() - $valid_window) || ($timestamp > time() + $valid_window)) {
			return new WP_Error('woocommerce_rest_authentication_error', __('Invalid timestamp.', 'woocommerce'),
			                    ['status' => 401]);
		}
		...
		if (in_array($nonce, $used_nonces)) {
			return new WP_Error('woocommerce_rest_authentication_error',
			                    __('Invalid nonce - nonce has already been used.', 'woocommerce'), ['status' => 401]);
		}
	...
	public function send_unauthorized_headers($response) {
		...
		if (is_wp_error($wc_rest_authentication_error) && is_ssl()) {
			$auth_message
				= __('WooCommerce API - Use a consumer key in the username field and a consumer secret in the password field.',
				     'woocommerce');
