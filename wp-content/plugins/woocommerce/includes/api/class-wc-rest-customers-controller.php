...
class WC_REST_Customers_Controller extends WC_REST_Controller {
			...
			[
				...
				'args'                => array_merge($this->get_endpoint_args_for_item_schema(WP_REST_Server::CREATABLE),
				                                     [
					                                     ...
					                                     'username' => [
						                                     'required' => 'no' === get_option('woocommerce_registration_generate_username',
						                                                                       'yes'),
					                                     ],
					                                     'password' => [
						                                     'required' => 'no' === get_option('woocommerce_registration_generate_password',
						                                                                       'no'),
					                                     ],
				                                     ]),
			],
	...
	public function prepare_item_for_response($customer, $request) {
		...
		$data = [
			...
			'last_order'    => [
				...
				'date' => is_object($last_order) ? wc_rest_prepare_date_response($last_order->post->post_date_gmt)
					: NULL
			],
	...
	public function create_item($request) {
		if (!empty($request['id'])) {
			return new WP_Error('woocommerce_rest_customer_exists',
			                    __('Cannot create existing resource.', 'woocommerce'), ['status' => 400]);
		}
		// Sets the username.
	...
	public function get_item_schema() {
		$schema = [
			...
			'properties' => [
				...
				'date_modified' => [
					'description' => __("The date the customer was last modified, in the site's timezone.",
					                    'woocommerce'),
					'type'        => 'date-time',
					'context'     => ['view', 'edit'],
					'readonly'    => TRUE,
	...
	public function update_item($request) {
		...
		if (!empty($request['email']) && email_exists($request['email']) && $request['email'] !== $customer->user_email) {
			return new WP_Error('woocommerce_rest_customer_invalid_email',
			                    __('Email address is invalid.', 'woocommerce'), ['status' => 400]);
		}
		if (!empty($request['username']) && $request['username'] !== $customer->user_login) {
			return new WP_Error('woocommerce_rest_customer_invalid_argument',
			                    __("Username isn't editable.", 'woocommerce'), ['status' => 400]);
		}
		// Customer email.
		if (isset($request['email'])) {
			wp_update_user(['ID' => $customer->ID, 'user_email' => sanitize_email($request['email'])]);
		}
		// Customer password.
		if (isset($request['password'])) {
			wp_update_user(['ID' => $customer->ID, 'user_pass' => wc_clean($request['password'])]);
		}
		$this->update_additional_fields_for_object($customer, $request);
	...
	public function delete_item($request) {
		...
		if (!$force) {
			return new WP_Error('woocommerce_rest_trash_not_supported',
			                    __('Customers do not support trashing.', 'woocommerce'), ['status' => 501]);
		}
		$customer = get_userdata($id);
		if (!$customer) {
			return new WP_Error('woocommerce_rest_invalid_id', __('Invalid resource id.', 'woocommerce'),
		...
		if (!empty($reassign)) {
			if ($reassign === $id || !get_userdata($reassign)) {
				return new WP_Error('woocommerce_rest_customer_invalid_reassign',
				                    __('Invalid resource id for reassignment.', 'woocommerce'), ['status' => 400]);
			}
		}
		$request->set_param('context', 'edit');
		$response = $this->prepare_item_for_response($customer, $request);
