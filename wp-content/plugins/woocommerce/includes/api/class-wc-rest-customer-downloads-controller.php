...
class WC_REST_Customer_Downloads_Controller extends WC_REST_Controller {
	...
	public function get_items_permissions_check($request) {
		...
		if (!$customer) {
			return new WP_Error("woocommerce_rest_customer_invalid", __("Resource doesn't exist.", 'woocommerce'),
			                    ['status' => 404]);
		}
	...
	public function prepare_item_for_response($download, $request) {
		$data                        = (array)$download;
		$data['access_expires']      = $data['access_expires'] ? wc_rest_prepare_date_response($data['access_expires'])
			: 'never';
	...
	public function get_item_schema() {
		$schema = [
			...
			'properties' => [
				...
				'access_expires'      => [
					'description' => __("The date when the download access expires, in the site's timezone.",
					                    'woocommerce'),
					'type'        => 'string',
					'context'     => ['view'],
					'readonly'    => TRUE,
