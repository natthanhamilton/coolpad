...
class WC_REST_Order_Notes_Controller extends WC_REST_Controller {
	...
	public function get_items($request) {
		...
		if (empty($order->post_type) || $this->post_type !== $order->post_type) {
			return new WP_Error('woocommerce_rest_{$this->post_type}_invalid_id',
			                    __('Invalid order id.', 'woocommerce'), ['status' => 404]);
		}
		$args = [
			'post_id' => $order->ID,
			'approve' => 'approve',
	...
	public function create_item($request) {
		...
		if (!$note_id) {
			return new WP_Error('woocommerce_api_cannot_create_order_note',
			                    __('Cannot create order note, please try again.', 'woocommerce'), ['status' => 500]);
		}
		...
		$response->header('Location', rest_url(sprintf('/%s/%s/%d', $this->namespace,
		                                               str_replace('(?P<order_id>[\d]+)', $order->id, $this->rest_base),
		                                               $note_id)));
	...
	public function delete_item($request) {
		...
		if (!$result) {
			return new WP_Error('woocommerce_rest_cannot_delete',
			                    sprintf(__('The %s cannot be deleted.', 'woocommerce'), 'order_note'),
			                    ['status' => 500]);
		}
		/**
	...
	public function get_item_schema() {
		$schema = [
			...
			'properties' => [
				...
				'customer_note' => [
					'description' => __('Shows/define if the note is only for reference or for the customer (the user will be notified).',
					                    'woocommerce'),
					'type'        => 'boolean',
					'default'     => FALSE,
					'context'     => ['view', 'edit'],
