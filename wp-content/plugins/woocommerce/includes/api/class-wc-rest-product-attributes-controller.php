...
class WC_REST_Product_Attributes_Controller extends WC_REST_Controller {
	...
	public function create_item_permissions_check($request) {
		if (!wc_rest_check_manager_permissions('attributes', 'create')) {
			return new WP_Error('woocommerce_rest_cannot_create',
			                    __('Sorry, you cannot create new resource.', 'woocommerce'),
			                    ['status' => rest_authorization_required_code()]);
		}
	...
	public function get_item_permissions_check($request) {
		if (!$this->get_taxonomy($request)) {
			return new WP_Error("woocommerce_rest_taxonomy_invalid", __("Resource doesn't exist.", 'woocommerce'),
			                    ['status' => 404]);
		}
		if (!wc_rest_check_manager_permissions('attributes', 'read')) {
	...
	public function update_item_permissions_check($request) {
		...
		if (!wc_rest_check_manager_permissions('attributes', 'edit')) {
			return new WP_Error('woocommerce_rest_cannot_update',
			                    __('Sorry, you cannot update resource.', 'woocommerce'),
			                    ['status' => rest_authorization_required_code()]);
		}
	...
	public function delete_item_permissions_check($request) {
		...
		if (!wc_rest_check_manager_permissions('attributes', 'delete')) {
			return new WP_Error('woocommerce_rest_cannot_delete',
			                    __('Sorry, you cannot delete resource.', 'woocommerce'),
			                    ['status' => rest_authorization_required_code()]);
		}
	...
	public function create_item($request) {
		...
		if (empty($args['attribute_name'])) {
			...
			$args['attribute_name'] = preg_replace('/^pa\_/', '',
			                                       wc_sanitize_taxonomy_name(stripslashes($args['attribute_name'])));
		}
		...
		if (is_wp_error($insert)) {
			return new WP_Error('woocommerce_rest_cannot_create', $insert->get_error_message(), ['status' => 400]);
		}
		...
		$response->header('Location',
		                  rest_url('/' . $this->namespace . '/' . $this->rest_base . '/' . $attribute->attribute_id));
		// Clear transients.
		flush_rewrite_rules();
	...
	protected function validate_attribute_slug($slug, $new_data = TRUE) {
		if (strlen($slug) >= 28) {
			return new WP_Error('woocommerce_rest_invalid_product_attribute_slug_too_long',
			                    sprintf(__('Slug "%s" is too long (28 characters max).', 'woocommerce'), $slug),
			                    ['status' => 400]);
		} elseif (wc_check_if_attribute_name_is_reserved($slug)) {
			return new WP_Error('woocommerce_rest_invalid_product_attribute_slug_reserved_name',
			                    sprintf(__('Slug "%s" is not allowed because it is a reserved term.', 'woocommerce'),
			                            $slug), ['status' => 400]);
		} elseif ($new_data && taxonomy_exists(wc_attribute_taxonomy_name($slug))) {
			return new WP_Error('woocommerce_rest_invalid_product_attribute_slug_already_exists',
			                    sprintf(__('Slug "%s" is already in use.', 'woocommerce'), $slug), ['status' => 400]);
		}
	...
	protected function get_attribute($id) {
		...
		if (is_wp_error($attribute) || is_null($attribute)) {
			return new WP_Error('woocommerce_rest_attribute_invalid', __("Resource doesn't exist.", 'woocommerce'),
			                    ['status' => 404]);
		}
	...
	public function update_item($request) {
		...
		// Checks for errors.
		if (FALSE === $update) {
			return new WP_Error('woocommerce_rest_cannot_edit', __('Could not edit the attribute', 'woocommerce'),
			                    ['status' => 400]);
		}
	...
	public function delete_item($request) {
		...
		if (!$force) {
			return new WP_Error('woocommerce_rest_trash_not_supported',
			                    __('Resource does not support trashing.', 'woocommerce'), ['status' => 501]);
		}
		$attribute = $this->get_attribute($request['id']);
		if (is_wp_error($attribute)) {
			return $attribute;
