...
class WC_REST_Coupons_Controller extends WC_REST_Posts_Controller {
	...
	public function prepare_item_for_response($post, $request) {
		...
		$data    = [
			...
			'expiry_date'                 => (!empty($coupon->expiry_date))
				? wc_rest_prepare_date_response($coupon->expiry_date) : NULL,
	...
	protected function prepare_item_for_database($request) {
		...
		if ('POST' === $request->get_method() && empty($data->ID)) {
			if (empty($request['code'])) {
				return new WP_Error('woocommerce_rest_empty_coupon_code',
				                    sprintf(__('The coupon code cannot be empty.', 'woocommerce'), 'code'),
				                    ['status' => 400]);
			}
		}
		// Code.
		if (!empty($schema['properties']['code']) && !empty($request['code'])) {
			...
			$coupon_found = $wpdb->get_var($wpdb->prepare("
			 ...
			 ", $coupon_code, $id));
			if ($coupon_found) {
				return new WP_Error('woocommerce_rest_coupon_code_already_exists',
				                    __('The coupon code already exists', 'woocommerce'), ['status' => 400]);
			}
	...
	public function get_item_schema() {
		$schema = [
			...
			'properties' => [
				...
				'date_modified'               => [
					'description' => __("The date the coupon was last modified, in the site's timezone.",
					                    'woocommerce'),
					'type'        => 'date-time',
					'context'     => ['view', 'edit'],
					'readonly'    => TRUE,
				...
				'minimum_amount'              => [
					'description' => __('Minimum order amount that needs to be in the cart before coupon applies.',
					                    'woocommerce'),
					'type'        => 'string',
					'context'     => ['view', 'edit'],
				],
	...
	protected function add_post_meta_fields($post, $request) {
		...
		update_post_meta($post->ID, 'product_ids',
		                 implode(',', array_filter(array_map('intval', $data['product_ids']))));
		update_post_meta($post->ID, 'exclude_product_ids',
		...
		update_post_meta($post->ID, 'product_categories',
		                 array_filter(array_map('intval', $data['product_categories'])));
		update_post_meta($post->ID, 'exclude_product_categories',
		                 array_filter(array_map('intval', $data['excluded_product_categories'])));
		...
		update_post_meta($post->ID, 'customer_email',
		                 array_filter(array_map('sanitize_email', $data['email_restrictions'])));
	...
	protected function update_post_meta_fields($post, $request) {
		...
		if (isset($request['product_ids'])) {
			update_post_meta($post->ID, 'product_ids',
			                 implode(',', array_filter(array_map('intval', $request['product_ids']))));
		}
		if (isset($request['exclude_product_ids'])) {
			update_post_meta($post->ID, 'exclude_product_ids',
			                 implode(',', array_filter(array_map('intval', $request['exclude_product_ids']))));
		}
		...
		if (isset($request['expiry_date'])) {
			update_post_meta($post->ID, 'expiry_date',
			                 $this->get_coupon_expiry_date(wc_clean($request['expiry_date'])));
		}
		...
		if (isset($request['product_categories'])) {
			update_post_meta($post->ID, 'product_categories',
			                 array_filter(array_map('intval', $request['product_categories'])));
		}
		if (isset($request['excluded_product_categories'])) {
			update_post_meta($post->ID, 'exclude_product_categories',
			                 array_filter(array_map('intval', $request['excluded_product_categories'])));
		}
		...
		if (isset($request['email_restrictions'])) {
			update_post_meta($post->ID, 'customer_email',
			                 array_filter(array_map('sanitize_email', $request['email_restrictions'])));
		}
