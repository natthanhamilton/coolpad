...
class WC_Form_Handler {
	...
	public static function init() {
		...
		add_action('wp_loaded', [__CLASS__, 'order_again'], 20);
	...
	public static function save_address() {
		...
		if (empty($_POST['action']) || 'edit_address' !== $_POST['action'] || empty($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'],
						...
						switch ($rule) {
							case 'postcode' :
								...
								if (!WC_Validation::is_postcode($_POST[ $key ], $_POST[ $load_address . '_country' ])) {
									...
									$_POST[ $key ] = wc_format_postcode($_POST[ $key ],
									                                    $_POST[ $load_address . '_country' ]);
								}
								break;
							case 'phone' :
	...
	public static function save_account_details() {
		...
		if (empty($_POST['action']) || 'save_account_details' !== $_POST['action'] || empty($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'],
		...
		$required_fields = apply_filters('woocommerce_save_account_details_required_fields', [
	...
	public static function pay_action() {
		global $wp;
		if (isset($_POST['woocommerce_pay']) && isset($_POST['_wpnonce']) && wp_verify_nonce($_POST['_wpnonce'],
		                                                                                     'woocommerce-pay')
		) {
			ob_start();
	...
	public static function add_payment_method_action() {
		if (isset($_POST['woocommerce_add_payment_method'], $_POST['payment_method'], $_POST['_wpnonce']) && wp_verify_nonce($_POST['_wpnonce'],
	...
	public static function update_cart_action() {
		...
		elseif (!empty($_GET['remove_item']) && isset($_GET['_wpnonce']) && wp_verify_nonce($_GET['_wpnonce'],
		                                                                                    'woocommerce-cart')
		) {
			...
			if ($cart_item = WC()->cart->get_cart_item($cart_item_key)) {
				...
				if ($product->is_in_stock() && $product->has_enough_stock($cart_item['quantity'])) {
					...
					$removed_notice .= ' <a href="' . esc_url(WC()->cart->get_undo_url($cart_item_key)) . '">' . __('Undo?',
					                                                                                                'woocommerce') . '</a>';
				} else {
					$removed_notice = sprintf(__('%s removed.', 'woocommerce'), $item_removed_title);
				}
			...
			$referer = wp_get_referer() ? remove_query_arg(['remove_item', 'add-to-cart', 'added-to-cart'],
			                                               add_query_arg('removed_item', '1', wp_get_referer()))
				: wc_get_cart_url();
			wp_safe_redirect($referer);
			exit;
		} // Undo Cart Item
		elseif (!empty($_GET['undo_item']) && isset($_GET['_wpnonce']) && wp_verify_nonce($_GET['_wpnonce'],
		                                                                                  'woocommerce-cart')
		) {
			$cart_item_key = sanitize_text_field($_GET['undo_item']);
			WC()->cart->restore_cart_item($cart_item_key);
			$referer = wp_get_referer() ? remove_query_arg(['undo_item', '_wpnonce'], wp_get_referer())
				: wc_get_cart_url();
			wp_safe_redirect($referer);
			exit;
		}
		// Update Cart - checks apply_coupon too because they are in the same form
		if ((!empty($_POST['apply_coupon']) || !empty($_POST['update_cart']) || !empty($_POST['proceed'])) && isset($_POST['_wpnonce']) && wp_verify_nonce($_POST['_wpnonce'],
		                                                                                                                                                   'woocommerce-cart')
		) {
			$cart_updated = FALSE;
			...
			if (!WC()->cart->is_empty() && is_array($cart_totals)) {
				foreach (WC()->cart->get_cart() as $cart_item_key => $values) {
					...
					$quantity = apply_filters('woocommerce_stock_amount_cart_item',
					                          wc_stock_amount(preg_replace("/[^0-9\.]/", '',
					                                                       $cart_totals[ $cart_item_key ]['qty'])),
					                          $cart_item_key);
					if ('' === $quantity || $quantity == $values['quantity']) {
						continue;
					}
					// Update cart validation
					$passed_validation = apply_filters('woocommerce_update_cart_validation', TRUE, $cart_item_key,
					                                   $values, $quantity);
					// is_sold_individually
					if ($_product->is_sold_individually() && $quantity > 1) {
						wc_add_notice(sprintf(__('You can only have 1 %s in your cart.', 'woocommerce'),
						                      $_product->get_title()), 'error');
	...
	public static function order_again() {
		// Nothing to do
		if (!isset($_GET['order_again']) || !is_user_logged_in() || !isset($_GET['_wpnonce']) || !wp_verify_nonce($_GET['_wpnonce'],
		...
		foreach ($order->get_items() as $item) {
			...
			if (!apply_filters('woocommerce_add_to_cart_validation', TRUE, $product_id, $quantity, $variation_id,
			                   $variations, $cart_item_data)
			) {
				continue;
			}
		...
		if (isset($_GET['cancel_order']) && isset($_GET['order']) && isset($_GET['order_id'])) {
			...
			if ($order->has_status('cancelled')) {
				...
				wc_add_notice(apply_filters('woocommerce_order_cancelled_notice',
				                            __('Your order was cancelled.', 'woocommerce')),
				...
				wc_add_notice(__('Your order can no longer be cancelled. Please contact us if you need assistance.',
				                 'woocommerce'), 'error');
			} else {
	...
	public static function add_to_cart_action($url = FALSE) {
		...
		$add_to_cart_handler = apply_filters('woocommerce_add_to_cart_handler', $adding_to_cart->product_type,
		                                     $adding_to_cart);
	...
	private static function add_to_cart_handler_variable($product_id) {
		...
		if (!empty($missing_attributes)) {
			wc_add_notice(sprintf(_n('%s is a required field', '%s are required fields', sizeof($missing_attributes),
			                         'woocommerce'), wc_format_list_of_items($missing_attributes)), 'error');
	...
	private static function add_to_cart_handler_grouped($product_id) {
		...
		if (!empty($_REQUEST['quantity']) && is_array($_REQUEST['quantity'])) {
			...
			if (!$was_added_to_cart && !$quantity_set) {
				wc_add_notice(__('Please choose the quantity of items you wish to add to your cart&hellip;',
				                 'woocommerce'), 'error');
			...
			try {
				...
				if ($validation_error->get_error_code()) {
					throw new Exception('<strong>' . __('Error',
					                                    'woocommerce') . ':</strong> ' . $validation_error->get_error_message());
				}
				if (empty($username)) {
					throw new Exception('<strong>' . __('Error',
					                                    'woocommerce') . ':</strong> ' . __('Username is required.',
					                                                                        'woocommerce'));
				}
				if (empty($_POST['password'])) {
					throw new Exception('<strong>' . __('Error',
					                                    'woocommerce') . ':</strong> ' . __('Password is required.',
					                                                                        'woocommerce'));
				}
				if (is_email($username) && apply_filters('woocommerce_get_username_from_email', TRUE)) {
					...
					if (isset($user->user_login)) {
						...
						throw new Exception('<strong>' . __('Error',
						                                    'woocommerce') . ':</strong> ' . __('A user could not be found with this email address.',
						                                                                        'woocommerce'));
					}
				} else {
	...
	public static function process_lost_password() {
		if (isset($_POST['wc_reset_password']) && isset($_POST['user_login']) && isset($_POST['_wpnonce']) && wp_verify_nonce($_POST['_wpnonce'],
		                                                                                                                      'lost_password')
		) {
	...
	public static function process_reset_password() {
		$posted_fields = ['wc_reset_password', 'password_1', 'password_2', 'reset_key', 'reset_login', '_wpnonce'];
		...
		$user = WC_Shortcode_My_Account::check_password_reset_key($posted_fields['reset_key'],
		...
		if ($user instanceof WP_User) {
			...
			if (0 === wc_notice_count('error')) {
				...
				wp_redirect(add_query_arg('reset', 'true', remove_query_arg(['key', 'login', 'reset-link-sent'])));
				exit;
			}
		}
	}
	...
	public static function process_registration() {
		...
		if (!empty($_POST['register']) && wp_verify_nonce($nonce_value, 'woocommerce-register')) {
			...
			try {
				$validation_error = new WP_Error();
				$validation_error = apply_filters('woocommerce_process_registration_errors', $validation_error,
				...
				wp_safe_redirect(apply_filters('woocommerce_registration_redirect', wp_get_referer() ? wp_get_referer()
