 ...
 * No personal infomation is tracked, only general WooCommerce settings, general product, order and user counts and
 * admin email for discount code.
 *
 * @class          WC_Tracker
 * @version        2.3.0
...
class WC_Tracker {
	...
	public static function send_tracking_data($override = FALSE) {
		...
		if (!apply_filters('woocommerce_tracker_send_override', $override)) {
			...
			if ($last_send && $last_send > apply_filters('woocommerce_tracker_last_send_interval',
			                                             strtotime('-1 week'))
			) {
				return;
			}
		...
		wp_safe_remote_post(self::$api_url, [
			                                  'method'      => 'POST',
			                                  'timeout'     => 45,
			                                  'redirection' => 5,
			                                  'httpversion' => '1.0',
			                                  'blocking'    => FALSE,
			                                  'headers'     => ['user-agent' => 'WooCommerceTracker/' . md5(esc_url(home_url('/'))) . ';'],
	...
	private static function get_tracking_data() {
		...
		$data['jetpack_connected']
		                           = (class_exists('Jetpack') && is_callable('Jetpack::is_active') && Jetpack::is_active())
			? 'yes' : 'no';
		$data['jetpack_is_staging']
		...
		$data['connect_active']
		                           = (class_exists('WC_Connect_Loader') && wp_next_scheduled('wc_connect_fetch_service_schemas'))
			? 'yes' : 'no';
		// Store count info
	...
	public static function get_theme_info() {
		...
		$theme_wc_support  = (!current_theme_supports('woocommerce') && !in_array($theme_data->template,
		                                                                          wc_get_core_supported_themes()))
			? 'No' : 'Yes';

		return ['name' => $theme_name, 'version' => $theme_version, 'child_theme' => $theme_child_theme, 'wc_support' => $theme_wc_support];
	}
	...
	private static function get_active_shipping_methods() {
		...
		foreach ($shipping_methods as $id => $shipping_method) {
			if (isset($shipping_method->enabled) && $shipping_method->enabled == 'yes') {
				$active_methods[ $id ]
					= ['title' => $shipping_method->title, 'tax_status' => $shipping_method->tax_status];
			}
		}
