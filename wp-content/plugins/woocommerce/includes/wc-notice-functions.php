...
function wc_notice_count($notice_type = '') {
	if (!did_action('woocommerce_init')) {
		_doing_it_wrong(__FUNCTION__, __('This function should not be called before woocommerce_init.', 'woocommerce'),
		                '2.3');
...
function wc_print_notice($message, $notice_type = 'success') {
	...
	wc_get_template("notices/{$notice_type}.php", [
		'messages' => [apply_filters('woocommerce_add_' . $notice_type, $message)]
	]);
}
