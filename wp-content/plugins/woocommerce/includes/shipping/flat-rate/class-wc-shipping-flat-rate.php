...
class WC_Shipping_Flat_Rate extends WC_Shipping_Method {
	...
	public function calculate_shipping($package = []) {
		...
		if ($cost !== '') {
			$has_costs    = TRUE;
			$rate['cost'] = $this->evaluate_cost($cost, [
		...
		foreach ($found_shipping_classes as $shipping_class => $products) {
			...
			$class_cost_string   = $shipping_class_term && $shipping_class_term->term_id
				? $this->get_option('class_cost_' . $shipping_class_term->term_id,
				                    $this->get_option('class_cost_' . $shipping_class, ''))
	...
	protected function evaluate_cost($sum, $args = []) {
		...
		$locale         = localeconv();
		$decimals
		                = [wc_get_price_decimal_separator(), $locale['decimal_point'], $locale['mon_decimal_point'], ','];
