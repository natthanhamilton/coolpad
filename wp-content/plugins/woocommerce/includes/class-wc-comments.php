...
class WC_Comments {
	...
	public static function init() {
		...
		add_action('comment_feed_join', [__CLASS__, 'exclude_order_comments_from_feed_join']);
		add_action('comment_feed_where', [__CLASS__, 'exclude_order_comments_from_feed_where']);
		// Secure webhook comments
		add_filter('comments_clauses', [__CLASS__, 'exclude_webhook_comments'], 10, 1);
		add_action('comment_feed_join', [__CLASS__, 'exclude_webhook_comments_from_feed_join']);
		add_action('comment_feed_where', [__CLASS__, 'exclude_webhook_comments_from_feed_where']);
		// Count comments
	 ...
	 *
	 * This code should exclude shop_order comments from queries. Some queries (like the recent comments widget on the
	 * dashboard) are hardcoded. and are not filtered, however, the code current_user_can( 'read_post',
	 * $comment->comment_post_ID ) should keep them safe since only admin and. shop managers can view orders anyway.
	 *
	 * The frontend view order pages get around this filter by using remove_filter('comments_clauses', array(
	...
	public static function wp_count_comments($stats, $post_id) {
		...
		if (0 === $post_id) {
			...
			if (!$stats) {
				$stats = [];
				$count
					= $wpdb->get_results("SELECT comment_approved, COUNT( * ) AS num_comments FROM {$wpdb->comments} WHERE comment_type != 'order_note' GROUP BY comment_approved",
					                     ARRAY_A);
				$total = 0;
				$approved
				       = ['0' => 'moderated', '1' => 'approved', 'spam' => 'spam', 'trash' => 'trash', 'post-trashed' => 'post-trashed'];
	...
	public static function add_comment_purchase_verification($comment_id) {
		...
		if ('product' === get_post_type($comment->comment_post_ID)) {
			$verified = wc_customer_bought_product($comment->comment_author_email, $comment->user_id,
