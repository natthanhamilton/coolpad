...
class WC_Product_Variation extends WC_Product {
	...
	protected $variation_level_meta_data
		= [
			'downloadable'          => 'no',
			'virtual'               => 'no',
			'manage_stock'          => 'no',
	...
	protected $variation_inherited_meta_data
		= [
			'tax_class'  => '',
			'backorders' => 'no',
			'sku'        => '',
	 ...
	 */
	public function __construct($variation, $args = []) {
		...
		$this->id = !empty($args['parent_id']) ? intval($args['parent_id'])
			: wp_get_post_parent_id($this->variation_id);
		...
		$this->post         = !empty($this->parent->post) ? $this->parent->post : [];
	}
	...
	public function __isset($key) {
		if (in_array($key, array_keys($this->variation_level_meta_data))) {
			...
			return metadata_exists('post', $this->variation_id, '_' . $key) || metadata_exists('post', $this->id,
			                                                                                   '_' . $key);
		} else {
	...
	public function __get($key) {
		if (in_array($key, array_keys($this->variation_level_meta_data))) {
			...
			$value = metadata_exists('post', $this->variation_id, '_' . $key) ? get_post_meta($this->variation_id,
			                                                                                  '_' . $key, TRUE)
				: get_post_meta($this->id, '_' . $key, TRUE);
			// Handle meta data keys which can be empty at variation level to cause inheritance
			if ('' === $value && in_array($key, ['sku', 'weight', 'length', 'width', 'height'])) {
	 ...
	 *
	 * @param  $item_object item array If a cart or order item is passed, we can get a link containing the exact
	 *                      attributes selected for the variation, rather than the default attributes.
	 *
	 * @return string
	 */
	public function get_permalink($item_object = NULL) {
		if (!empty($item_object['variation'])) {
			...
			$data_keys   = array_map('wc_variation_attribute_name',
			                         wp_list_pluck($item_object['item_meta_array'], 'key'));
	...
	public function add_to_cart_url() {
		...
		$url            = $this->is_purchasable() && $this->is_in_stock() ? remove_query_arg('added-to-cart',
		                                                                                     add_query_arg(array_merge(['variation_id' => $this->variation_id, 'add-to-cart' => $this->id],
		                                                                                                               $variation_data)))
	 ...
	 * Override abstract method so that: i) Disabled variations are not be purchasable by admins. ii) Enabled
	 * variations are not purchasable if the parent product is not purchasable.
	 *
	 * @return bool
	 */
	...
	public function get_stock_quantity() {
		return apply_filters('woocommerce_variation_get_stock_quantity',
		                     TRUE === $this->managing_stock() ? wc_stock_amount($this->stock)
			                     : $this->parent->get_stock_quantity(), $this);
	}
	...
	public function add_to_cart_text() {
		$text = $this->is_purchasable() && $this->is_in_stock() ? __('Add to cart', 'woocommerce')
	...
	public function get_price_html($price = '') {
		...
		if ($this->get_price() !== '') {
			if ($this->is_on_sale()) {
				$price = apply_filters('woocommerce_variation_sale_price_html',
				                       '<del>' . wc_price($display_regular_price) . '</del> <ins>' . wc_price($display_sale_price) . '</ins>' . $this->get_price_suffix(),
				                       $this);
			} elseif ($this->get_price() > 0) {
				$price = apply_filters('woocommerce_variation_price_html',
				                       wc_price($display_price) . $this->get_price_suffix(), $this);
			} else {
	...
	/**
	 * Checks if this particular variation is visible. Invisible variations are enabled and can be selected, but no
	 * price / stock info is displayed. Instead, a suitable 'unavailable' message is displayed. Invisible by default:
	 * Disabled variations and variations with an empty price.
	 *
	 * @return bool
	 */
		...
		if (is_array($variation_data)) {
			...
			foreach ($attributes as $attribute) {
				...
				if ($attribute['is_taxonomy']) {
					...
					foreach ($post_terms as $term) {
						if ($variation_selected_value === $term->slug) {
							$description_value = esc_html(apply_filters('woocommerce_variation_option_name',
							                                            $term->name));
						}
					}
				} else {
				...
				if ($flat) {
					...
					$description[]
						= '<dt>' . $description_name . ':</dt><dd>' . rawurldecode($description_value) . '</dd>';
				}
			}
			if ($flat) {
