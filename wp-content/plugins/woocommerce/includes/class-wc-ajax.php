...
class WC_AJAX {
	...
	public static function get_endpoint($request = '') {
		return esc_url_raw(apply_filters('woocommerce_ajax_get_endpoint', add_query_arg('wc-ajax', $request,
		                                                                                remove_query_arg(['remove_item', 'add-to-cart', 'added-to-cart'])),
		                                 $request));
	}
		...
		if (WC()->cart->is_empty()) {
			$data = [
				'fragments' => apply_filters('woocommerce_update_order_review_fragments', [
					'form.woocommerce-checkout' => '<div class="woocommerce-error">' . __('Sorry, your session has expired.',
					                                                                      'woocommerce') . ' <a href="' . esc_url(wc_get_page_permalink('shop')) . '" class="wc-backward">' . __('Return to shop',
					                                                                                                                                                                             'woocommerce') . '</a></div>'
				])
			];
			wp_send_json($data);
	...
	public static function add_to_cart() {
		...
		if ($passed_validation && FALSE !== WC()->cart->add_to_cart($product_id,
		                                                            $quantity) && 'publish' === $product_status
		) {
	...
	public static function get_refreshed_fragments() {
		...
		$data = [
			...
			'cart_hash' => apply_filters('woocommerce_add_to_cart_hash', WC()->cart->get_cart_for_session()
				? md5(json_encode(WC()->cart->get_cart_for_session())) : '', WC()->cart->get_cart_for_session())
		];
		wp_send_json($data);
	}
		...
		if (current_user_can('edit_products') && check_admin_referer('woocommerce-feature-product')) {
			...
			if ('product' === get_post_type($product_id)) {
				update_post_meta($product_id, '_featured',
				                 get_post_meta($product_id, '_featured', TRUE) === 'yes' ? 'no' : 'yes');
				delete_transient('wc_featured_products');
			}
		}
		wp_safe_redirect(wp_get_referer() ? remove_query_arg(['trashed', 'untrashed', 'deleted', 'ids'],
		                                                     wp_get_referer())
				...
				if ($is_taxonomy) {
					if (isset($attribute_values[ $i ])) {
						...
						if (is_array($attribute_values[ $i ])) {
							...
							$values     = [];
							$raw_values = array_map('wc_sanitize_term_text_based',
							                        explode(WC_DELIMITER, $attribute_values[ $i ]));
	...
	public static function add_variation() {
		...
		if ($variation_id) {
			...
			foreach ($variation_fields as $field => $value) {
				$variation_data[ $field ] = isset($variation_meta[ $field ][0])
					? maybe_unserialize($variation_meta[ $field ][0]) : $value;
			}
			...
			$variation_data['image']          = $variation_data['_thumbnail_id']
				? wp_get_attachment_thumb_url($variation_data['_thumbnail_id']) : '';
			$variation_data['shipping_class'] = $shipping_classes && !is_wp_error($shipping_classes)
				? current($shipping_classes)->term_id : '';
	...
	public static function link_all_variations() {
		...
		$variations = [];
		$_product   = wc_get_product($post_id, ['product_type' => 'variable']);
	...
	public static function revoke_access_to_download() {
		...
		$wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->prefix}woocommerce_downloadable_product_permissions WHERE permission_id = %d;",
		                            $permission_id));
			...
			if (!empty($files)) {
				foreach ($files as $download_id => $file) {
					if ($inserted_id = wc_downloadable_file_permission($download_id, $product_id, $order)) {
						...
						$download
							= $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}woocommerce_downloadable_product_permissions WHERE permission_id = %d",
							                                $inserted_id));
						$loop++;
						$file_counter++;
						if (isset($file['name'])) {
	...
	public static function reduce_order_item_stock() {
		...
		$order_id       = absint($_POST['order_id']);
		$order_item_ids = isset($_POST['order_item_ids']) ? $_POST['order_item_ids'] : [];
		$order_item_qty = isset($_POST['order_item_qty']) ? $_POST['order_item_qty'] : [];
		$order          = wc_get_order($order_id);
		...
		if ($order && !empty($order_items) && sizeof($order_item_ids) > 0) {
			foreach ($order_items as $item_id => $order_item) {
				...
				if ($_product->exists() && $_product->managing_stock() && isset($order_item_qty[ $item_id ]) && $order_item_qty[ $item_id ] > 0) {
					$stock_change = apply_filters('woocommerce_reduce_order_stock_quantity',
					                              $order_item_qty[ $item_id ], $item_id);
					$new_stock    = $_product->reduce_stock($stock_change);
					$item_name    = $_product->get_sku() ? $_product->get_sku() : $order_item['product_id'];
					$note         = sprintf(__('Item %s stock reduced from %s to %s.', 'woocommerce'), $item_name,
					                        $new_stock + $stock_change, $new_stock);
					$return[]     = $note;
			...
			if (empty($return)) {
				$return[] = __('No products had their stock reduced - they may not have stock management enabled.',
				               'woocommerce');
			}
			echo implode(', ', $return);
		}
		...
		if ($order && !empty($order_items) && sizeof($order_item_ids) > 0) {
			foreach ($order_items as $item_id => $order_item) {
				...
				if ($_product->exists() && $_product->managing_stock() && isset($order_item_qty[ $item_id ]) && $order_item_qty[ $item_id ] > 0) {
					...
					$stock_change = apply_filters('woocommerce_restore_order_stock_quantity',
					                              $order_item_qty[ $item_id ], $item_id);
					$new_quantity = $_product->increase_stock($stock_change);
					$item_name    = $_product->get_sku() ? $_product->get_sku() : $order_item['product_id'];
					$note         = sprintf(__('Item %s stock increased from %s to %s.', 'woocommerce'), $item_name,
					                        $old_stock, $new_quantity);
					$return[]     = $note;
					$order->add_order_note($note);
				}
			}
			...
			if (empty($return)) {
				$return[] = __('No products had their stock increased - they may not have stock management enabled.',
				               'woocommerce');
			}
			echo implode(', ', $return);
		}
	...
	public static function add_order_item_meta() {
		...
		$meta_id = wc_add_order_item_meta(absint($_POST['order_item_id']), __('Name', 'woocommerce'),
	...
	public static function remove_order_item_meta() {
		...
		global $wpdb;
		$wpdb->delete("{$wpdb->prefix}woocommerce_order_itemmeta", [
	...
	public static function calc_line_taxes() {
		...
		if (wc_tax_enabled() && $is_vat_exempt !== 'yes') {
			...
			if (isset($items['order_item_id'])) {
				...
				foreach ($items['order_item_id'] as $item_id) {
					...
					$line_total[ $item_id ]             = isset($items['line_total'][ $item_id ])
						? wc_format_decimal($items['line_total'][ $item_id ]) : 0;
					$line_subtotal[ $item_id ]          = isset($items['line_subtotal'][ $item_id ])
						? wc_format_decimal($items['line_subtotal'][ $item_id ]) : $line_total[ $item_id ];
					$order_item_tax_classes[ $item_id ] = isset($items['order_item_tax_class'][ $item_id ])
						? sanitize_text_field($items['order_item_tax_class'][ $item_id ]) : '';
					...
					if ('0' !== $order_item_tax_classes[ $item_id ] && 'taxable' === $item_tax_status) {
						...
						foreach (array_keys($taxes + $line_taxes) as $key) {
							$taxes[ $key ] = (isset($line_taxes[ $key ]) ? $line_taxes[ $key ]
									: 0) + (isset($taxes[ $key ]) ? $taxes[ $key ] : 0);
						}
					}
				}
			}
			...
			if (isset($items['shipping_method_id'])) {
				...
				foreach ($items['shipping_method_id'] as $item_id) {
					$item_id                   = absint($item_id);
					$shipping_cost[ $item_id ] = isset($items['shipping_cost'][ $item_id ])
						? wc_format_decimal($items['shipping_cost'][ $item_id ]) : 0;
					$_shipping_taxes           = WC_Tax::calc_shipping_tax($shipping_cost[ $item_id ],
					                                                       $matched_tax_rates);
					// Set the new shipping_taxes
					foreach ($_shipping_taxes as $_tax_id => $_tax_value) {
						$items['shipping_taxes'][ $item_id ][ $_tax_id ] = $_tax_value;
						$shipping_taxes[ $_tax_id ] = isset($shipping_taxes[ $_tax_id ])
							? $shipping_taxes[ $_tax_id ] + $_tax_value : $_tax_value;
					}
				}
			}
		}
		...
		foreach (array_keys($taxes + $shipping_taxes) as $tax_rate_id) {
			$order->add_tax($tax_rate_id, isset($taxes[ $tax_rate_id ]) ? $taxes[ $tax_rate_id ] : 0,
	...
	public static function add_order_note() {
		...
		if ($post_id > 0) {
			...
			echo '</div><p class="meta"><a href="#" class="delete_note">' . __('Delete note',
			                                                                   'woocommerce') . '</a></p>';
			echo '</li>';
		}
		// Quit out
	...
	public static function json_search_products_and_variations() {
		self::json_search_products('', ['product', 'product_variation']);
	}
	 ...
	 */
	public static function json_search_products($term = '', $post_types = ['product']) {
		global $wpdb;
		ob_start();
		...
		if (!empty($_GET['exclude'])) {
			$query .= " AND posts.ID NOT IN (" . implode(',',
	...
	public static function json_search_customers() {
		...
		$found_customers = [];
		add_action('pre_user_query', [__CLASS__, 'json_search_customer_name']);
		$customers_query = new WP_User_Query(apply_filters('woocommerce_json_search_customers_query', [
			'fields'         => 'all',
			'orderby'        => 'display_name',
			'search'         => '*' . $term . '*',
			'search_columns' => ['ID', 'user_login', 'user_email', 'user_nicename']
		]));
		remove_action('pre_user_query', [__CLASS__, 'json_search_customer_name']);
		$customers = $customers_query->get_results();
		if (!empty($customers)) {
			foreach ($customers as $customer) {
				if (!in_array($customer->ID, $exclude)) {
					$found_customers[ $customer->ID ]
						= $customer->display_name . ' (#' . $customer->ID . ' &ndash; ' . sanitize_email($customer->user_email) . ')';
				}
			}
		}
	...
	public static function refund_line_items() {
		...
		try {
			...
			foreach ($item_ids as $item_id) {
				$line_items[ $item_id ] = ['qty' => 0, 'refund_total' => 0, 'refund_tax' => []];
			}
			...
			foreach ($line_item_qtys as $item_id => $qty) {
				if ($restock_refunded_items && $qty && isset($order_items[ $item_id ])) {
					...
					if ($_product && $_product->exists() && $_product->managing_stock()) {
						...
						$order->add_order_note(sprintf(__('Item #%s stock increased from %s to %s.', 'woocommerce'),
						                               $order_item['product_id'], $old_stock, $new_quantity));
			...
			if ($order->get_remaining_refund_amount() > 0 || ($order->has_free_item() && $order->get_remaining_refund_items() > 0)) {
				...
				$order->update_status(apply_filters('woocommerce_order_fully_refunded_status', 'refunded', $order_id,
				                                    $refund->id));
	...
	public static function delete_refund() {
		...
		$refund_ids = array_map('absint', is_array($_POST['refund_id']) ? $_POST['refund_id'] : [$_POST['refund_id']]);
	...
	public static function update_api_key() {
		...
		try {
			...
			$permissions = (in_array($_POST['permissions'], ['read', 'write', 'read_write']))
			...
			if (0 < $key_id) {
				...
				$data['message']
				                         = __('API Key generated successfully. Make sure to copy your new API keys now. You won\'t be able to see it again!',
				                              'woocommerce');
				$data['revoke_url']
				                         = '<a style="color: #a00; text-decoration: none;" href="' . esc_url(wp_nonce_url(add_query_arg(['revoke-key' => $key_id],
				                                                                                                                        admin_url('admin.php?page=wc-settings&tab=api&section=keys')),
				                                                                                                          'revoke')) . '">' . __('Revoke Key',
				                                                                                                                                 'woocommerce') . '</a>';
			}
	...
	public static function load_variations() {
		...
		// Get variations
		$args = apply_filters('woocommerce_ajax_admin_get_variations_args', [
			'post_type'      => 'product_variation',
		...
		if ($variations) {
			foreach ($variations as $variation) {
				...
				$variation_data['shipping_class'] = $shipping_classes && !is_wp_error($shipping_classes)
					? current($shipping_classes)->term_id : '';
				$variation_data['menu_order']     = $variation->menu_order;
				$variation_data['_stock']         = '' === $variation_data['_stock'] ? ''
					: wc_stock_amount($variation_data['_stock']);
				include('admin/meta-boxes/views/html-variation-admin.php');
				$loop++;
			}
		}
	...
	public static function save_variations() {
		...
		if ($errors = WC_Admin_Meta_Boxes::$meta_box_errors) {
			...
			echo '<button type="button" class="notice-dismiss"><span class="screen-reader-text">' . __('Dismiss this notice.',
	...
	public static function tax_rates_save_changes() {
		...
		foreach ($changes as $tax_rate_id => $data) {
			...
			$tax_rate = array_intersect_key($data, [
				'tax_rate_country'  => 1,
				'tax_rate_state'    => 1,
				'tax_rate'          => 1,
				'tax_rate_name'     => 1,
	...
	public static function shipping_zones_save_changes() {
		...
		foreach ($changes as $zone_id => $data) {
			...
			$zone_data = array_intersect_key($data, [
				'zone_id'        => 1,
				'zone_name'      => 1,
				'zone_order'     => 1,
				'zone_locations' => 1,
	...
	public static function shipping_zone_methods_save_changes() {
		...
		foreach ($changes as $instance_id => $data) {
			$method_id
				= $wpdb->get_var($wpdb->prepare("SELECT method_id FROM {$wpdb->prefix}woocommerce_shipping_zone_methods WHERE instance_id = %d",
				                                $instance_id));
			if (isset($data['deleted'])) {
				if ($wpdb->delete("{$wpdb->prefix}woocommerce_shipping_zone_methods",
				                  ['instance_id' => $instance_id])
				) {
			...
			$method_data = array_intersect_key($data, [
				'method_order' => 1,
				'enabled'      => 1
			]);
			if (isset($method_data['method_order'])) {
				$wpdb->update("{$wpdb->prefix}woocommerce_shipping_zone_methods",
				              ['method_order' => absint($method_data['method_order'])],
				              ['instance_id' => absint($instance_id)]);
			}
			if (isset($method_data['enabled'])) {
				$is_enabled = absint('yes' === $method_data['enabled']);
				if ($wpdb->update("{$wpdb->prefix}woocommerce_shipping_zone_methods", ['is_enabled' => $is_enabled],
				                  ['instance_id' => absint($instance_id)])
				) {
					do_action('woocommerce_shipping_zone_method_status_toggled', $instance_id, $method_id, $zone_id,
					          $is_enabled);
				}
			}
		}
