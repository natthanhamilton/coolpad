...
class WC_Download_Handler {
	...
	public static function download_product() {
		...
		$download_data = self::get_download_data([
			                                         ...
			                                         'email'       => sanitize_email(str_replace(' ', '+',
			                                                                                     $_GET['email'])),
			                                         'download_id' => wc_clean(isset($_GET['key'])
				                                                                   ? preg_replace('/\s+/', ' ',
				                                                                                  $_GET['key']) : '')
		                                         ]);
		if ($_product && $download_data) {
			self::check_current_user_can_download($download_data);
			do_action('woocommerce_download_product', $download_data->user_email, $download_data->order_key,
			          $download_data->product_id, $download_data->user_id, $download_data->download_id,
	 ...
	 * @access private
	 */
	private static function get_download_data($args = []) {
		global $wpdb;
		...
		return $wpdb->get_row($wpdb->prepare($query,
		                                     [$args['email'], $args['order_key'], $args['product_id'], $args['download_id']]));
	}
	...
	private static function check_order_is_valid($download_data) {
		if ($download_data->order_id && ($order
				= wc_get_order($download_data->order_id)) && !$order->is_download_permitted()
		) {
	...
	private static function download_error($message, $title = '', $status = 404) {
		if (!strstr($message, '<a ')) {
			$message .= ' <a href="' . esc_url(wc_get_page_permalink('shop')) . '" class="wc-forward">' . __('Go to shop',
			                                                                                                 'woocommerce') . '</a>';
		}
	...
	private static function check_downloads_remaining($download_data) {
		if ('0' == $download_data->downloads_remaining) {
			self::download_error(__('Sorry, you have reached your download limit for this file', 'woocommerce'), '',
			                     403);
		}
	}
	...
	private static function check_download_expiry($download_data) {
		if ($download_data->access_expires > 0 && strtotime($download_data->access_expires) < strtotime('midnight',
		...
		if ($download_data->user_id && 'yes' === get_option('woocommerce_downloads_require_login')) {
			if (!is_user_logged_in()) {
				if (wc_get_page_id('myaccount')) {
					wp_safe_redirect(add_query_arg('wc_error', urlencode(__('You must be logged in to download files.',
					                                                        'woocommerce')),
					...
					self::download_error(__('You must be logged in to download files.',
					                        'woocommerce') . ' <a href="' . esc_url(wp_login_url(wc_get_page_permalink('myaccount'))) . '" class="wc-forward">' . __('Login',
					                                                                                                                                                 'woocommerce') . '</a>',
					                     __('Log in to Download Files', 'woocommerce'), 403);
				}
	...
	public static function count_download($download_data) {
		...
		$wpdb->update(
			...
			[
				...
				'downloads_remaining' => $download_data->downloads_remaining > 0
					? $download_data->downloads_remaining - 1 : $download_data->downloads_remaining,
			],
			[
	...
	public static function download($file_path, $product_id) {
		...
		$file_download_method = apply_filters('woocommerce_file_download_method',
		                                      get_option('woocommerce_file_download_method', 'force'), $product_id);
	...
	public static function download_file_xsendfile($file_path, $filename) {
		...
		if (function_exists('apache_get_modules') && in_array('mod_xsendfile', apache_get_modules())) {
			...
			self::download_headers($parsed_file_path['file_path'], $filename);
			$xsendfile_path = trim(preg_replace('`^' . str_replace('\\', '/', getcwd()) . '`', '',
	...
	public static function parse_file_path($file_path) {
		...
		} elseif ((!isset($parsed_file_path['scheme']) || !in_array($parsed_file_path['scheme'],
	...
	private static function check_server_config() {
		wc_set_time_limit(0);
		if (function_exists('get_magic_quotes_runtime') && get_magic_quotes_runtime() && version_compare(phpversion(),
		                                                                                                 '5.4', '<')
		) {
