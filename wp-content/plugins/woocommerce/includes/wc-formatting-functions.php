 ...
 * @param  mixed        $dp         number of decimal points to use, blank to use woocommerce_price_num_decimals, or
 ...
 *
 * @since 2.3.10 Tooltips are encoded with htmlspecialchars to prevent XSS. Should not be used in conjunction with
 *        esc_attr()
 *
 * @param string $var
 *
...
function wc_price($price, $args = []) {
	extract(apply_filters('wc_price_args', wp_parse_args($args, [
		'ex_tax_label'       => FALSE,
		'currency'           => '',
	...
	$price    = apply_filters('formatted_woocommerce_price',
	                          number_format($price, $decimals, $decimal_separator, $thousand_separator), $price,
...
function wc_format_content($raw_string) {
	return apply_filters('woocommerce_format_content', do_shortcode(shortcode_unautop(wpautop($raw_string))),
	                     $raw_string);
}
...
function wc_format_product_short_description($content) {
	...
	if (class_exists('WPCom_Markdown')) {
		...
		return wpautop($markdown->transform($content, ['unslash' => FALSE]));
	}
...
add_filter('woocommerce_admin_settings_sanitize_option_woocommerce_price_decimal_sep',
           'wc_format_option_price_separators', 10, 3);
add_filter('woocommerce_admin_settings_sanitize_option_woocommerce_price_thousand_sep',
           'wc_format_option_price_separators', 10, 3);
/**
...
add_filter('woocommerce_admin_settings_sanitize_option_woocommerce_price_num_decimals',
           'wc_format_option_price_num_decimals', 10, 3);
/**
...
add_filter('woocommerce_admin_settings_sanitize_option_woocommerce_hold_stock_minutes',
           'wc_format_option_hold_stock_minutes', 10, 3);
/**
