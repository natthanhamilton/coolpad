...
if (!class_exists('WC_Email_New_Order')) :
	...
	class WC_Email_New_Order extends WC_Email {
		...
		public function __construct() {
			...
			$this->description    = __('New order emails are sent to chosen recipient(s) when a new order is received.',
			                           'woocommerce');
			$this->heading        = __('New customer order', 'woocommerce');
			$this->subject        = __('[{site_title}] New customer order ({order_number}) - {order_date}',
			                           'woocommerce');
			$this->template_html  = 'emails/admin-new-order.php';
			$this->template_plain = 'emails/plain/admin-new-order.php';
			// Triggers for this email
			add_action('woocommerce_order_status_pending_to_processing_notification', [$this, 'trigger']);
			add_action('woocommerce_order_status_pending_to_completed_notification', [$this, 'trigger']);
			add_action('woocommerce_order_status_pending_to_on-hold_notification', [$this, 'trigger']);
			add_action('woocommerce_order_status_failed_to_processing_notification', [$this, 'trigger']);
			add_action('woocommerce_order_status_failed_to_completed_notification', [$this, 'trigger']);
			add_action('woocommerce_order_status_failed_to_on-hold_notification', [$this, 'trigger']);
		...
		public function init_form_fields() {
			$this->form_fields = [
				...
				'subject'    => [
					...
					'type'        => 'text',
					'description' => sprintf(__('This controls the email subject line. Leave blank to use the default subject: <code>%s</code>.',
