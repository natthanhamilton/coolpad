...
/**
 * is_woocommerce - Returns true if on a page which uses WooCommerce templates (cart and checkout are standard pages
 * with shortcodes and thus are not included).
 *
 * @return bool
 */
...
if (!function_exists('is_checkout')) {
	...
	function is_checkout() {
		return is_page(wc_get_page_id('checkout')) || wc_post_content_has_shortcode('woocommerce_checkout') || apply_filters('woocommerce_is_checkout',
		                                                                                                                     FALSE);
	}
}
...
if (!function_exists('is_account_page')) {
	...
	function is_account_page() {
		return is_page(wc_get_page_id('myaccount')) || wc_post_content_has_shortcode('woocommerce_my_account') || apply_filters('woocommerce_is_account_page',
		                                                                                                                        FALSE);
	}
}
...
if (!function_exists('is_filtered')) {
	...
	function is_filtered() {
		return apply_filters('woocommerce_is_filtered',
			(sizeof(WC_Query::get_layered_nav_chosen_attributes()) > 0 || isset($_GET['max_price']) || isset($_GET['min_price']) || isset($_GET['min_rating'])));
	}
}
...
if (!function_exists('meta_is_product_attribute')) {
	...
	function meta_is_product_attribute($name, $value, $product_id) {
		...
		if ($product && method_exists($product, 'get_variation_attributes')) {
			...
			return (in_array($name, array_keys($attributes)) && in_array($value,
			                                                             $variation_attributes[ $attributes[ $name ]['name'] ]));
		} else {
			return FALSE;
		}
	}
...
function wc_is_webhook_valid_topic($topic) {
	...
	$valid_events    = apply_filters('woocommerce_valid_webhook_events', ['created', 'updated', 'deleted']);
...
function wc_checkout_is_https() {
	return wc_site_is_https() || 'yes' === get_option('woocommerce_force_ssl_checkout') || class_exists('WordPressHTTPS') || strstr(wc_get_page_permalink('checkout'),
	                                                                                                                                'https:');
}
