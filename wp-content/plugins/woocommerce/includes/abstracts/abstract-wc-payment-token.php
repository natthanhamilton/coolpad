...
abstract class WC_Payment_Token extends WC_Data {
	 ...
	 * user_id      - int     Optional - ID for the user this token is associated with. 0 if this token is not
	...
	public function read($token_id) {
		global $wpdb;
		if ($token
			= $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}woocommerce_payment_tokens WHERE token_id = %d LIMIT 1;",
			                                $token_id))
		) {
	...
	public function delete() {
		...
		$wpdb->delete($wpdb->prefix . 'woocommerce_payment_tokens', ['token_id' => $this->get_id()], ['%d']);
		$wpdb->delete($wpdb->prefix . 'woocommerce_payment_tokenmeta', ['payment_token_id' => $this->get_id()], ['%d']);
