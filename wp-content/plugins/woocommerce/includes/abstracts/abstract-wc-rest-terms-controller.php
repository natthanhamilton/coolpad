...
abstract class WC_REST_Terms_Controller extends WC_REST_Controller {
	...
	protected function check_permissions($request, $context = 'read') {
		...
		if (!$taxonomy) {
			return new WP_Error('woocommerce_rest_taxonomy_invalid', __("Taxonomy doesn't exist.", 'woocommerce'),
			                    ['status' => 404]);
		}
		...
		if ($id = intval($request['id'])) {
			...
			if (!$term || $term->taxonomy !== $taxonomy) {
				return new WP_Error('woocommerce_rest_term_invalid', __("Resource doesn't exist.", 'woocommerce'),
				                    ['status' => 404]);
			}
	...
	protected function get_terms_for_product($prepared_args) {
		...
		// ordering is by `name`.
		if (!in_array($prepared_args['orderby'], ['name', 'none', 'include'])) {
	...
	public function create_item($request) {
		...
		if (isset($request['parent'])) {
			if (!is_taxonomy_hierarchical($taxonomy)) {
				return new WP_Error('woocommerce_rest_taxonomy_not_hierarchical',
				                    __('Can not set resource parent, taxonomy is not hierarchical.', 'woocommerce'),
				                    ['status' => 400]);
			}
			$parent = get_term((int)$request['parent'], $taxonomy);
			if (!$parent) {
				return new WP_Error('woocommerce_rest_term_invalid',
				                    __("Parent resource doesn't exist.", 'woocommerce'), ['status' => 404]);
			}
	...
	public function update_item($request) {
		...
		if (isset($request['parent'])) {
			...
			if (!$parent) {
				return new WP_Error('woocommerce_rest_term_invalid',
				                    __("Parent resource doesn't exist.", 'woocommerce'), ['status' => 400]);
			}
