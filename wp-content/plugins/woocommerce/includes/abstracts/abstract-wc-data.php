...
abstract class WC_Data {
	...
	public function get_data() {
		return array_merge($this->_data, ['meta_data' => $this->get_meta_data()]);
	}
	...
	public function update_meta_data($key, $value, $meta_id = '') {
		...
		if ($array_key) {
			$this->_meta_data[ current($array_key) ] = (object)[
				'key'     => $key,
				'value'   => $value,
				'meta_id' => $meta_id,
			];
	...
	protected function save_meta_data() {
		...
		foreach ($this->_meta_data as $array_key => $meta) {
			if (empty($meta->meta_id)) {
				$new_meta_id                             = add_metadata($this->_meta_type, $this->get_id(), $meta->key,
				                                                        $meta->value, FALSE);
	...
	protected function _get_db_info() {
		...
		if (!in_array($this->_meta_type, ['post', 'user', 'comment', 'term'])) {
			$table .= 'woocommerce_';
		}
	...
	protected function get_internal_meta_keys() {
		return array_merge(array_map([$this, 'prefix_key'], array_keys($this->_data)), $this->_internal_meta_keys);
	}
