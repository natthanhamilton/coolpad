...
class WC_Product {
	...
	public function __get($key) {
		...
		} elseif (in_array($key, ['product_attributes', 'crosssell_ids', 'upsell_ids'])) {
			$value = $value ? $value : [];
	...
	public function supports($feature) {
		return apply_filters('woocommerce_product_supports', in_array($feature, $this->supports) ? TRUE : FALSE,
		                     $feature, $this);
	}
	...
	public function get_gallery_attachment_ids() {
		return apply_filters('woocommerce_product_gallery_attachment_ids',
		                     array_filter(array_filter((array)explode(',', $this->product_image_gallery)),
		                                  'wp_attachment_is_image'), $this);
	}
	...
	public function set_stock($amount = NULL, $mode = 'set') {
		...
		if (!is_null($amount) && $this->managing_stock()) {
			...
			switch ($mode) {
				case 'add' :
					$wpdb->query($wpdb->prepare("UPDATE {$wpdb->postmeta} SET meta_value = meta_value + %f WHERE post_id = %d AND meta_key='_stock'",
					                            $amount, $this->id));
					break;
				case 'subtract' :
					$wpdb->query($wpdb->prepare("UPDATE {$wpdb->postmeta} SET meta_value = meta_value - %f WHERE post_id = %d AND meta_key='_stock'",
					...
					break;
				default :
					$wpdb->query($wpdb->prepare("UPDATE {$wpdb->postmeta} SET meta_value = %f WHERE post_id = %d AND meta_key='_stock'",
					                            $amount, $this->id));
					break;
			}
			// Clear caches
	...
	public function managing_stock() {
		return (!isset($this->manage_stock) || $this->manage_stock == 'no' || get_option('woocommerce_manage_stock') !== 'yes')
			? FALSE : TRUE;
	}
	...
	public function backorders_allowed() {
		return apply_filters('woocommerce_product_backorders_allowed',
		                     $this->backorders === 'yes' || $this->backorders === 'notify' ? TRUE : FALSE, $this->id,
		                     $this);
	}
	...
	public function get_stock_quantity() {
		return apply_filters('woocommerce_get_stock_quantity',
		                     $this->managing_stock() ? wc_stock_amount($this->stock) : NULL, $this);
	}
	...
	public function is_type($type) {
		return ($this->product_type == $type || (is_array($type) && in_array($this->product_type, $type))) ? TRUE
			: FALSE;
	}
	...
	public function get_files() {
		$downloadable_files = array_filter(isset($this->downloadable_files)
			                                   ? (array)maybe_unserialize($this->downloadable_files) : []);
		if (!empty($downloadable_files)) {
			foreach ($downloadable_files as $key => $file) {
				if (!is_array($file)) {
					$downloadable_files[ $key ] = [
						'file' => $file,
						'name' => ''
					];
				}
				...
				// Filter URL
				$downloadable_files[ $key ]['file'] = apply_filters('woocommerce_file_download_path',
				                                                    $downloadable_files[ $key ]['file'], $this, $key);
			}
		}
	...
	public function get_availability() {
		return apply_filters('woocommerce_get_availability', [
	...
	public function is_on_backorder($qty_in_cart = 0) {
		return $this->managing_stock() && $this->backorders_allowed() && ($this->get_total_stock() - $qty_in_cart) < 0
			? TRUE : FALSE;
	}
	...
	public function backorders_require_notification() {
		return apply_filters('woocommerce_product_backorders_require_notification',
		                     $this->managing_stock() && $this->backorders === 'notify' ? TRUE : FALSE, $this);
	}
	...
	public function get_price_html($price = '') {
		...
		if ($this->get_price() > 0) {
			if ($this->is_on_sale() && $this->get_regular_price()) {
				$price .= $this->get_price_html_from_to($display_regular_price,
				                                        $display_price) . $this->get_price_suffix();
	...
	public function get_display_price($price = '', $qty = 1) {
		...
		$display_price    = $tax_display_mode == 'incl' ? $this->get_price_including_tax($qty, $price)
			: $this->get_price_excluding_tax($qty, $price);
		...
		if ($this->is_taxable()) {
			if (get_option('woocommerce_prices_include_tax') === 'no') {
				...
				if (!empty(WC()->customer) && WC()->customer->is_vat_exempt()) {
					 ...
					 */
				} elseif ($tax_rates !== $base_tax_rates && apply_filters('woocommerce_adjust_non_base_location_prices',
				                                                          TRUE)
				) {
					$base_taxes   = WC_Tax::calc_tax($price * $qty, $base_tax_rates, TRUE);
					$modded_taxes = WC_Tax::calc_tax(($price * $qty) - array_sum($base_taxes), $tax_rates, FALSE);
					$price        = round(($price * $qty) - array_sum($base_taxes) + array_sum($modded_taxes),
	...
	/**
	 * Returns the price (excluding tax) - ignores tax_class filters since the price may *include* tax and thus needs
	 * subtracting. Uses store base tax rates. Can work for a specific $qty for more accurate taxes.
	 *
	 * @param  int    $qty
	 * @param  string $price to calculate, left blank to just use get_price()
	 *
	...
	public function is_on_sale() {
		return apply_filters('woocommerce_product_is_on_sale',
			($this->get_sale_price() !== $this->get_regular_price() && $this->get_sale_price() === $this->get_price()),
			                 $this);
	}
	...
	public function get_price_html_from_to($from, $to) {
		$price = '<del>' . ((is_numeric($from)) ? wc_price($from) : $from) . '</del> <ins>' . ((is_numeric($to))
				? wc_price($to) : $to) . '</ins>';
	...
	public function get_rating_html($rating = NULL) {
		...
		if ($rating > 0) {
			$rating_html = '<div class="star-rating" title="' . sprintf(__('Rated %s out of 5', 'woocommerce'),
			                                                            $rating) . '">';
			$rating_html .= '<span style="width:' . (($rating / 5) * 100) . '%"><strong class="rating">' . $rating . '</strong> ' . __('out of 5',
			                                                                                                                           'woocommerce') . '</span>';
			$rating_html .= '</div>';
		}
	...
	public function get_cross_sells() {
		return apply_filters('woocommerce_product_crosssell_ids', (array)maybe_unserialize($this->crosssell_ids),
		                     $this);
	}
	 ...
	 *  - To ensure we always have enough results, it will check $limit before returning the cached result, if not
	 *  recalc.
	 *  - This used to rely on transient version to invalidate cache, but to avoid multiple transients we now just
	 *  expire daily. This means if a related product is edited and no longer related, it won't be removed for 24
	 *  hours. Acceptable trade-off for performance.
	...
	public function get_related($limit = 5) {
		...
		if (FALSE === $related_posts || sizeof($related_posts) < $limit) {
			...
			if (1 === sizeof($cats_array) && 1 === sizeof($tags_array)) {
				...
				// Sanitize
				$exclude_ids = array_map('absint', array_merge([0, $this->id], $this->get_upsells()));
	...
	protected function get_related_terms($term) {
		$terms_array = [0];
		$terms = apply_filters('woocommerce_get_related_' . $term . '_terms', wp_get_post_terms($this->id, $term),
		                       $this->id);
	...
	protected function build_related_query($cats_array, $tags_array, $exclude_ids, $limit) {
		...
		if ($relate_by_category || $relate_by_tag) {
			...
			if ($relate_by_category) {
				$query['where'] .= " ( tt.taxonomy = 'product_cat' AND t.term_id IN ( " . implode(',',
				                                                                                  $cats_array) . " ) ) ";
				if ($relate_by_tag) {
					$query['where'] .= ' OR ';
				}
			...
			if ($relate_by_tag) {
				$query['where'] .= " ( tt.taxonomy = 'product_tag' AND t.term_id IN ( " . implode(',',
				                                                                                  $tags_array) . " ) ) ";
			}
			$query['where'] .= ')';
		}
	...
	public function get_attribute($attr) {
		...
		if (isset($attributes[ $attr ]) || isset($attributes[ 'pa_' . $attr ])) {
			...
			if (isset($attribute['is_taxonomy']) && $attribute['is_taxonomy']) {
				return implode(', ', wc_get_product_terms($this->id, $attribute['name'], ['fields' => 'names']));
			} else {
	...
	public function get_dimensions() {
		$dimensions = implode(' x ', array_filter([
	...
	public function get_weight() {
		return apply_filters('woocommerce_product_weight',
		                     apply_filters('woocommerce_product_get_weight', $this->weight ? $this->weight : ''),
		                     $this);
	}
	 ...
	 * @return string
	 */
	public function get_image($size = 'shop_thumbnail', $attr = [], $placeholder = TRUE) {
		if (has_post_thumbnail($this->id)) {
