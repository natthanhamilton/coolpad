...
abstract class WC_Abstract_Order {
	...
	public function populate($result) {
		...
		if (empty($this->billing_email) && !empty($this->customer_user) && ($user = get_user_by('id',
		                                                                                        $this->customer_user))
		) {
			$this->billing_email = $user->user_email;
		}
		// Orders store the state of prices including tax when created.
		$this->prices_include_tax = metadata_exists('post', $this->id, '_prices_include_tax') ? get_post_meta($this->id,
		                                                                                                      '_prices_include_tax',
		                                                                                                      TRUE) === 'yes'
	...
	public function add_product($product, $qty = 1, $args = []) {
		...
		$item_id = wc_add_order_item($this->id, [
		...
		wc_add_order_item_meta($item_id, '_line_subtotal',
		                       wc_format_decimal(isset($args['totals']['subtotal']) ? $args['totals']['subtotal']
			                                         : $product->get_price_excluding_tax($qty)));
		wc_add_order_item_meta($item_id, '_line_total',
		                       wc_format_decimal(isset($args['totals']['total']) ? $args['totals']['total']
			                                         : $product->get_price_excluding_tax($qty)));
		wc_add_order_item_meta($item_id, '_line_subtotal_tax', wc_format_decimal(isset($args['totals']['subtotal_tax'])
			                                                                         ? $args['totals']['subtotal_tax']
			                                                                         : 0));
		wc_add_order_item_meta($item_id, '_line_tax',
		                       wc_format_decimal(isset($args['totals']['tax']) ? $args['totals']['tax'] : 0));
		...
		if (isset($args['totals']['tax_data'])) {
			...
			wc_add_order_item_meta($item_id, '_line_tax_data', ['total' => [], 'subtotal' => []]);
		}
		// Add variation meta
		...
		if ($product->backorders_require_notification() && $product->is_on_backorder($qty)) {
			wc_add_order_item_meta($item_id, apply_filters('woocommerce_backordered_item_meta_name',
			                                               __('Backordered', 'woocommerce')),
			                       $qty - max(0, $product->get_total_stock()));
		}
	...
	public function update_product($item_id, $product, $args) {
		...
		if (isset($args['qty'])) {
			wc_update_order_item_meta($item_id, '_line_subtotal',
			                          wc_format_decimal(isset($args['totals']['subtotal']) ? $args['totals']['subtotal']
				                                            : $product->get_price_excluding_tax($args['qty'])));
			wc_update_order_item_meta($item_id, '_line_total',
			                          wc_format_decimal(isset($args['totals']['total']) ? $args['totals']['total']
				                                            : $product->get_price_excluding_tax($args['qty'])));
		}
		// set item tax totals
		wc_update_order_item_meta($item_id, '_line_subtotal_tax',
		                          wc_format_decimal(isset($args['totals']['subtotal_tax'])
		...
		if (isset($args['qty']) && $product->backorders_require_notification() && $product->is_on_backorder($args['qty'])) {
			wc_update_order_item_meta($item_id, apply_filters('woocommerce_backordered_item_meta_name',
			                                                  __('Backordered', 'woocommerce')),
			                          $args['qty'] - max(0, $product->get_total_stock()));
		}
	...
	public function update_coupon($item_id, $args) {
		...
		// code
		if (isset($args['code'])) {
			wc_update_order_item($item_id, ['order_item_name' => $args['code']]);
		}
		// amount
	...
	public function set_total($amount, $total_type = 'total') {
		if (!in_array($total_type,
		              ['shipping', 'tax', 'shipping_tax', 'total', 'cart_discount', 'cart_discount_tax'])
		) {
			return FALSE;
		}
		switch ($total_type) {
	...
	public function update_shipping($item_id, $args) {
		...
		if (isset($args['method_title'])) {
			wc_update_order_item($item_id, ['order_item_name' => $args['method_title']]);
		}
		// method ID
		...
		if (isset($args['cost'])) {
			...
			// Update total
			$this->set_total($this->order_shipping - wc_format_decimal($old_cost) + wc_format_decimal($args['cost']),
			                 'shipping');
		}
	...
	public function add_fee($fee) {
		...
		wc_add_order_item_meta($item_id, '_line_tax_data', ['total' => $tax_data]);
	...
	public function update_fee($item_id, $args) {
		...
		// name
		if (isset($args['name'])) {
			wc_update_order_item($item_id, ['order_item_name' => $args['name']]);
		}
		// tax class
		if (isset($args['tax_class'])) {
			wc_update_order_item_meta($item_id, '_tax_class', $args['tax_class']);
	...
	public function update_taxes() {
		...
		foreach ($this->get_items(['shipping']) as $item_id => $item) {
		...
		foreach (array_keys($order_taxes + $order_shipping_taxes) as $tax_rate_id) {
			$this->add_tax($tax_rate_id, isset($order_taxes[ $tax_rate_id ]) ? $order_taxes[ $tax_rate_id ] : 0,
			               isset($order_shipping_taxes[ $tax_rate_id ]) ? $order_shipping_taxes[ $tax_rate_id ] : 0);
		}
		// Save tax totals
	...
	public function get_items($type = '') {
		...
		$items = [];
		$get_items_sql
		       = $wpdb->prepare("SELECT order_item_id, order_item_name, order_item_type FROM {$wpdb->prefix}woocommerce_order_items WHERE order_id = %d ",
		                        $this->id);
		$get_items_sql .= "AND order_item_type IN ( '" . implode("','", array_map('esc_sql',
		                                                                          $type)) . "' ) ORDER BY order_item_id;";
	...
	/**
	 * Get all item meta data in array format in the order it was saved. Does not group meta by key like
	 * get_item_meta().
	 *
	 * @param mixed $order_item_id
	 *
	...
	public function get_item_meta_array($order_item_id) {
		...
		if (FALSE === $item_meta_array) {
			$item_meta_array = [];
			$metadata
			                 = $wpdb->get_results($wpdb->prepare("SELECT meta_key, meta_value, meta_id FROM {$wpdb->prefix}woocommerce_order_itemmeta WHERE order_item_id = %d ORDER BY meta_id",
			                                                     absint($order_item_id)));
			foreach ($metadata as $metadata_row) {
				$item_meta_array[ $metadata_row->meta_id ]
					= (object)['key' => $metadata_row->meta_key, 'value' => $metadata_row->meta_value];
			}
	...
	public function remove_order_items($type = NULL) {
		global $wpdb;
		if (!empty($type)) {
			$wpdb->query($wpdb->prepare("DELETE FROM itemmeta USING {$wpdb->prefix}woocommerce_order_itemmeta itemmeta INNER JOIN {$wpdb->prefix}woocommerce_order_items items WHERE itemmeta.order_item_id = items.order_item_id AND items.order_id = %d AND items.order_item_type = %s",
			                            $this->id, $type));
			$wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->prefix}woocommerce_order_items WHERE order_id = %d AND order_item_type = %s",
			                            $this->id, $type));
		} else {
			$wpdb->query($wpdb->prepare("DELETE FROM itemmeta USING {$wpdb->prefix}woocommerce_order_itemmeta itemmeta INNER JOIN {$wpdb->prefix}woocommerce_order_items items WHERE itemmeta.order_item_id = items.order_item_id and items.order_id = %d",
			                            $this->id));
			$wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->prefix}woocommerce_order_items WHERE order_id = %d",
			                            $this->id));
		}
	}
	...
	public function calculate_totals($and_taxes = TRUE) {
		...
		$grand_total
			= round($cart_total + $fee_total + $this->get_total_shipping() + $this->get_cart_tax() + $this->get_shipping_tax(),
	...
	public function calculate_taxes() {
		...
		foreach ($this->get_items(['line_item', 'fee']) as $item_id => $item) {
			...
			if ('0' !== $tax_class && 'taxable' === $item_tax_status) {
				...
				wc_update_order_item_meta($item_id, '_line_tax_data',
				                          ['total' => $line_taxes, 'subtotal' => $line_subtotal_taxes]);
				// Sum the item taxes
				foreach (array_keys($taxes + $line_taxes) as $key) {
					$taxes[ $key ] = (isset($line_taxes[ $key ]) ? $line_taxes[ $key ] : 0) + (isset($taxes[ $key ])
							? $taxes[ $key ] : 0);
				}
			}
		}
		...
		foreach ($this->get_shipping_methods() as $item_id => $item) {
			...
			if ('' === $shipping_tax_class) {
				...
				$tax_rates = WC_Tax::find_shipping_rates([
					                                         ...
					                                         'postcode'  => $postcode,
					                                         'city'      => $city,
					                                         'tax_class' => 'standard' === $shipping_tax_class ? ''
						                                         : $shipping_tax_class,
				                                         ]);
			}
			...
			wc_update_order_item_meta($item_id, '_line_tax', wc_format_decimal($line_tax));
			wc_update_order_item_meta($item_id, '_line_tax_data', ['total' => $line_taxes]);
			// Sum the item taxes
			foreach (array_keys($shipping_taxes + $line_taxes) as $key) {
				$shipping_taxes[ $key ] = (isset($line_taxes[ $key ]) ? $line_taxes[ $key ]
						: 0) + (isset($shipping_taxes[ $key ]) ? $shipping_taxes[ $key ] : 0);
			}
		}
		// Save tax totals
		...
		foreach (array_keys($taxes + $shipping_taxes) as $tax_rate_id) {
			$this->add_tax($tax_rate_id, isset($taxes[ $tax_rate_id ]) ? $taxes[ $tax_rate_id ] : 0,
	...
	public function get_status() {
		...
		return apply_filters('woocommerce_order_get_status',
		                     'wc-' === substr($this->post_status, 0, 3) ? substr($this->post_status, 3)
			                     : $this->post_status, $this);
	}
	...
	public function get_shipping_address_map_url() {
		$address = apply_filters('woocommerce_shipping_address_map_url_parts', [
			'address_1' => $this->shipping_address_1,
			'address_2' => $this->shipping_address_2,
			'city'      => $this->shipping_city,
			'state'     => $this->shipping_state,
		...
		return apply_filters('woocommerce_shipping_address_map_url',
		                     'https://maps.google.com/maps?&q=' . urlencode(implode(', ', $address)) . '&z=16', $this);
	}
	...
	public function get_formatted_billing_full_name() {
		return sprintf(_x('%1$s %2$s', 'full name', 'woocommerce'), $this->billing_first_name,
	...
	public function get_formatted_shipping_full_name() {
		return sprintf(_x('%1$s %2$s', 'full name', 'woocommerce'), $this->shipping_first_name,
	...
	public function get_total_discount($ex_tax = TRUE) {
		...
		return apply_filters('woocommerce_order_amount_total_discount',
	...
	public function get_formatted_line_subtotal($item, $tax_display = '') {
		...
		if ('excl' == $tax_display) {
			...
			$subtotal = wc_price($this->get_line_subtotal($item),
			                     ['ex_tax_label' => $ex_tax_label, 'currency' => $this->get_order_currency()]);
		} else {
			$subtotal = wc_price($this->get_line_subtotal($item, TRUE), ['currency' => $this->get_order_currency()]);
		}
	...
	public function get_cart_discount_to_display($tax_display = '') {
		...
		return apply_filters('woocommerce_order_cart_discount_to_display', $this->get_discount_to_display($tax_display),
		                     $this);
	}
	...
	public function get_discount_to_display($tax_display = '') {
		...
		return apply_filters('woocommerce_order_discount_to_display',
		                     wc_price($this->get_total_discount($tax_display === 'excl' && $this->display_totals_ex_tax),
		                              ['currency' => $this->get_order_currency()]), $this);
	}
	...
	public function get_order_item_totals($tax_display = '') {
		...
		if ($fees = $this->get_fees()) {
			foreach ($fees as $id => $fee) {
				if (apply_filters('woocommerce_get_order_item_totals_excl_free_fees',
				                  $fee['line_total'] + $fee['line_tax'] == 0, $id)) {
					continue;
				}
				if ('excl' == $tax_display) {
					$total_rows[ 'fee_' . $id ] = [
						'label' => ($fee['name'] ? $fee['name'] : __('Fee', 'woocommerce')) . ':',
						'value' => wc_price($fee['line_total'], ['currency' => $this->get_order_currency()])
					];
				} else {
					$total_rows[ 'fee_' . $id ] = [
						'label' => $fee['name'] . ':',
		...
		if ('excl' === $tax_display) {
			if (get_option('woocommerce_tax_total_display') == 'itemized') {
				...
				$total_rows['tax'] = [
					...
					'value' => wc_price($this->get_total_tax(), ['currency' => $this->get_order_currency()])
				];
			}
		}
		...
		if ($refunds = $this->get_refunds()) {
			foreach ($refunds as $id => $refund) {
				$total_rows[ 'refund_' . $id ] = [
					'label' => $refund->get_refund_reason() ? $refund->get_refund_reason()
						: __('Refund', 'woocommerce') . ':',
					'value' => wc_price('-' . $refund->get_refund_amount(), ['currency' => $this->get_order_currency()])
				];
			}
		}
	...
	public function get_subtotal_to_display($compound = FALSE, $tax_display = '') {
		...
		if (!$compound) {
			...
			$subtotal = wc_price($subtotal, ['currency' => $this->get_order_currency()]);
	...
	public function get_shipping_to_display($tax_display = '') {
		...
		if ($this->order_shipping != 0) {
			if ($tax_display == 'excl') {
				...
				$shipping = wc_price($this->order_shipping, ['currency' => $this->get_order_currency()]);
			...
			$shipping .= apply_filters('woocommerce_order_shipping_to_display_shipped_via',
			                           '&nbsp;<small class="shipped_via">' . sprintf(__('via %s', 'woocommerce'),
			                                                                         $this->get_shipping_method()) . '</small>',
			                           $this);
	...
	public function get_total_tax() {
		return apply_filters('woocommerce_order_amount_total_tax',
		                     wc_round_tax_total($this->get_cart_tax() + $this->get_shipping_tax()), $this);
	}
	...
	public function get_formatted_order_total() {
		$formatted_total = wc_price($this->get_total(), ['currency' => $this->get_order_currency()]);
	 ...
	 * @return string
	 */
	public function email_order_items_table($args = [], $deprecated1 = NULL, $deprecated2 = NULL, $deprecated3 = NULL,
	                                        $deprecated4 = NULL, $deprecated5 = NULL) {
		ob_start();
		...
		wc_get_template($template, apply_filters('woocommerce_email_order_items_args', [
			'order'               => $this,
	...
	public function is_download_permitted() {
		return apply_filters('woocommerce_order_is_download_permitted',
		                     $this->has_status('completed') || (get_option('woocommerce_downloads_grant_access_after_payment') == 'yes' && $this->has_status('processing')),
		                     $this);
	}
	...
	public function has_status($status) {
		return apply_filters('woocommerce_order_has_status', (is_array($status) && in_array($this->get_status(),
		                                                                                    $status)) || $this->get_status() === $status
			? TRUE : FALSE, $this, $status);
	}
	...
	public function is_paid() {
		return apply_filters('woocommerce_order_is_paid',
		                     $this->has_status(apply_filters('woocommerce_order_is_paid_statuses',
		                                                     ['processing', 'completed'])), $this);
	}
	...
	/**
	 * Generates a URL so that a customer can pay for their (unpaid - pending) order. Pass 'true' for the checkout
	 * version which doesn't offer gateway choices.
	 *
	...
	public function get_checkout_payment_url($on_checkout = FALSE) {
		...
		if ($on_checkout) {
			...
			$pay_url = add_query_arg(['pay_for_order' => 'true', 'key' => $this->order_key], $pay_url);
		}
	...
	public function get_cancel_order_url($redirect = '') {
		...
		return apply_filters('woocommerce_get_cancel_order_url', esc_url(add_query_arg([
			                                                                               'cancel_order' => 'true',
	...
	public function get_cancel_order_url_raw($redirect = '') {
		...
		return apply_filters('woocommerce_get_cancel_order_url_raw', add_query_arg([
			                                                                           'cancel_order' => 'true',
			                                                                           'order'        => $this->order_key,
			                                                                           'order_id'     => $this->id,
			                                                                           'redirect'     => $redirect,
	...
	public function display_item_downloads($item) {
		...
		if ($product && $product->exists() && $product->is_downloadable() && $this->is_download_permitted()) {
			...
			foreach ($download_files as $download_id => $file) {
				$i++;
				$prefix = count($download_files) > 1 ? sprintf(__('Download %d', 'woocommerce'), $i)
					: __('Download', 'woocommerce');
				$links[]
				        = '<small class="download-url">' . $prefix . ': <a href="' . esc_url($file['download_url']) . '" target="_blank">' . esc_html($file['name']) . '</a></small>' . "\n";
			}
	...
	public function update_status($new_status, $note = '', $manual = FALSE) {
		...
		if (!wp_update_post($update_post_data)) {
			$this->add_order_note(sprintf(__('Unable to update order from %1$s to %2$s.', 'woocommerce'),
			                              wc_get_order_status_name($old_status), wc_get_order_status_name($new_status)),
			                      0, $manual);
		...
		if ($new_status !== $old_status) {
			$this->add_order_note(trim($note . ' ' . sprintf(__('Order status changed from %1$s to %2$s.',
			                                                    'woocommerce'), wc_get_order_status_name($old_status),
			                                                 wc_get_order_status_name($new_status))), 0, $manual);
			...
			$this->add_order_note(trim($note . ' ' . sprintf(__('Order status changed to %s.', 'woocommerce'),
			                                                 wc_get_order_status_name($new_status))), 0, $manual);
		}
		switch ($new_status) {
			case 'completed' :
	...
	public function add_order_note($note, $is_customer_note = 0, $added_by_user = FALSE) {
		if (is_user_logged_in() && current_user_can('edit_shop_order', $this->id) && $added_by_user) {
			...
			$comment_author_email .= isset($_SERVER['HTTP_HOST']) ? str_replace('www.', '', $_SERVER['HTTP_HOST'])
				: 'noreply.com';
		...
		$commentdata        = apply_filters('woocommerce_new_order_note_data',
		                                    compact('comment_post_ID', 'comment_author', 'comment_author_email',
		                                            ...
		                                            'comment_type', 'comment_parent', 'comment_approved'),
		                                    ['order_id' => $this->id, 'is_customer_note' => $is_customer_note]);
		$comment_id = wp_insert_comment($commentdata);
		if ($is_customer_note) {
			add_comment_meta($comment_id, 'is_customer_note', 1);
			do_action('woocommerce_new_customer_note',
			          ['order_id' => $this->id, 'customer_note' => $commentdata['comment_content']]);
		}
	 ...
	 * If the cart contains only downloadable items then the order is 'completed' since the admin needs to take no
	 * action. Stock levels are reduced at this point. Sales are also recorded for products. Finally, record the date
	 * of payment.
	 *
	...
	public function payment_complete($transaction_id = '') {
		...
		$valid_order_statuses = apply_filters('woocommerce_valid_order_statuses_for_payment_complete',
		                                      ['on-hold', 'pending', 'failed', 'cancelled'], $this);
		if ($this->id && $this->has_status($valid_order_statuses)) {
			...
			if (sizeof($this->get_items()) > 0) {
				foreach ($this->get_items() as $item) {
					if ($_product = $this->get_product_from_item($item)) {
						...
						if (apply_filters('woocommerce_order_item_needs_processing', !$virtual_downloadable_item,
						                  $_product, $this->id)) {
			...
			$this->update_status(apply_filters('woocommerce_payment_complete_order_status',
			                                   $order_needs_processing ? 'processing' : 'completed', $this->id));
			...
			if (apply_filters('woocommerce_payment_complete_reduce_order_stock',
			                  !get_post_meta($this->id, '_order_stock_reduced', TRUE), $this->id)) {
	 ...
	 * Runs if stock management is enabled, but can be disabled on per-order basis by extensions @since 2.4.0 via
	 * woocommerce_can_reduce_order_stock hook.
	 */
	public function reduce_order_stock() {
		if ('yes' === get_option('woocommerce_manage_stock') && apply_filters('woocommerce_can_reduce_order_stock',
		                                                                      TRUE,
		                                                                      $this) && sizeof($this->get_items()) > 0
		) {
			foreach ($this->get_items() as $item) {
				if ($item['product_id'] > 0) {
					$_product = $this->get_product_from_item($item);
					if ($_product && $_product->exists() && $_product->managing_stock()) {
						...
						if (isset($item['variation_id']) && $item['variation_id']) {
							$this->add_order_note(sprintf(__('Item %1$s variation #%2$s stock reduced from %3$s to %4$s.',
							                                 'woocommerce'), $item_name, $item['variation_id'],
							                              $new_stock + $qty, $new_stock));
						} else {
							$this->add_order_note(sprintf(__('Item %1$s stock reduced from %2$s to %3$s.',
							                                 'woocommerce'), $item_name, $new_stock + $qty,
							                              $new_stock));
						}
	...
	public function send_stock_notifications($product, $new_stock, $qty_ordered) {
		// Backorders
		if ($new_stock < 0) {
			do_action('woocommerce_product_on_backorder',
			          ['product' => $product, 'order_id' => $this->id, 'quantity' => $qty_ordered]);
		}
	...
	public function needs_payment() {
		$valid_order_statuses = apply_filters('woocommerce_valid_order_statuses_for_payment', ['pending', 'failed'],
		                                      $this);
	...
	public function is_editable() {
		return apply_filters('wc_order_is_editable',
		                     in_array($this->get_status(), ['pending', 'on-hold', 'auto-draft', 'failed']), $this);
	}
}
