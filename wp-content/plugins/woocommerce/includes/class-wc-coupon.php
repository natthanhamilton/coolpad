...
class WC_Coupon {
		...
		foreach ($defaults as $key => $value) {
			...
			if (empty($this->$key)) {
				$this->$key = $value;
			                   ...
			                   ['product_ids', 'exclude_product_ids', 'product_categories', 'exclude_product_categories', 'customer_email'])) {
				...
				$this->expiry_date = $this->expiry_date && !is_numeric($this->expiry_date)
					? strtotime($this->expiry_date) : $this->expiry_date;
			}
		}
	 ...
	 *
	 * @since 2.5.0 woocommerce_coupon_code_query was removed in favour of woocommerce_get_coupon_id_from_code filter
	 *        on the return. wp_cache was also implemented.
	 *
	 * @param  string $code
	 *
	 * @return int
	...
	private function get_coupon_id_from_code($code) {
		global $wpdb;
		$coupon_id = wp_cache_get(WC_Cache_Helper::get_cache_prefix('coupons') . 'coupon_id_from_code_' . $code,
		                          'coupons');
		if (FALSE === $coupon_id) {
			$sql
				= $wpdb->prepare("SELECT ID FROM $wpdb->posts WHERE post_title = %s AND post_type = 'shop_coupon' AND post_status = 'publish' ORDER BY post_date DESC LIMIT 1;",
				                 $this->code);
			if ($coupon_id = apply_filters('woocommerce_get_coupon_id_from_code', $wpdb->get_var($sql), $this->code)) {
				wp_cache_set(WC_Cache_Helper::get_cache_prefix('coupons') . 'coupon_id_from_code_' . $code, $coupon_id,
				             'coupons');
			}
		}
	...
	public function __isset($key) {
		if (in_array($key, ['coupon_custom_fields', 'type', 'amount'])) {
			return TRUE;
		}
	...
	public function __get($key) {
		...
		if ('coupon_custom_fields' === $key) {
			$value = $this->id ? get_post_meta($this->id) : [];
		} elseif ('type' === $key) {
	...
	public function dcr_usage_count($used_by = '') {
		if ($this->id && $this->usage_count > 0) {
			...
			if ($used_by) {
				 ...
				 */
				$meta_id
					= $wpdb->get_var($wpdb->prepare("SELECT meta_id FROM $wpdb->postmeta WHERE meta_key = '_used_by' AND meta_value = %s AND post_id = %d LIMIT 1;",
					                                $used_by, $this->id));
				if ($meta_id) {
	...
	private function validate_user_usage_limit($user_id = 0) {
		...
		if ($this->usage_limit_per_user > 0 && is_user_logged_in() && $this->id) {
			global $wpdb;
			$usage_count
				= $wpdb->get_var($wpdb->prepare("SELECT COUNT( meta_id ) FROM {$wpdb->postmeta} WHERE post_id = %d AND meta_key = '_used_by' AND meta_value = %d;",
				                                $this->id, $user_id));
	...
	private function validate_minimum_amount() {
		if ($this->minimum_amount > 0 && apply_filters('woocommerce_coupon_validate_minimum_amount',
		                                               wc_format_decimal($this->minimum_amount) > WC()->cart->get_displayed_subtotal(),
		                                               $this)
		) {
	...
	private function validate_maximum_amount() {
		if ($this->maximum_amount > 0 && apply_filters('woocommerce_coupon_validate_maximum_amount',
		                                               wc_format_decimal($this->maximum_amount) < WC()->cart->get_displayed_subtotal(),
		                                               $this)
		) {
		...
		if (sizeof($this->product_ids) > 0) {
			...
			if (!WC()->cart->is_empty()) {
				foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
					if (in_array($cart_item['product_id'], $this->product_ids) || in_array($cart_item['variation_id'],
					                                                                       $this->product_ids) || in_array($cart_item['data']->get_parent(),
					                                                                                                       $this->product_ids)
					) {
						$valid_for_cart = TRUE;
					}
	...
	public function is_type($type) {
		return ($this->discount_type == $type || (is_array($type) && in_array($this->discount_type, $type))) ? TRUE
			: FALSE;
	}
	...
	public function is_valid_for_product($product, $values = []) {
		...
		$product_ids  = [$product->id, (isset($product->variation_id) ? $product->variation_id
			: 0), $product->get_parent()];
		...
		if (sizeof($this->exclude_product_categories) && sizeof(array_intersect($product_cats,
			...
			if (!WC()->cart->is_empty()) {
				foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
					if (in_array($cart_item['product_id'],
					             $this->exclude_product_ids) || in_array($cart_item['variation_id'],
					                                                     $this->exclude_product_ids) || in_array($cart_item['data']->get_parent(),
	...
	public function get_coupon_error($err_code) {
		switch ($err_code) {
			...
			case self::E_WC_COUPON_INVALID_REMOVED:
				$err
					= sprintf(__('Sorry, it seems the coupon "%s" is invalid - it has now been removed from your order.',
					             'woocommerce'), $this->code);
				break;
			case self::E_WC_COUPON_NOT_YOURS_REMOVED:
				$err
					= sprintf(__('Sorry, it seems the coupon "%s" is not yours - it has now been removed from your order.',
					             'woocommerce'), $this->code);
				break;
			...
			case self::E_WC_COUPON_ALREADY_APPLIED_INDIV_USE_ONLY:
				$err
					= sprintf(__('Sorry, coupon "%s" has already been applied and cannot be used in conjunction with other coupons.',
					             'woocommerce'), $this->code);
				break;
			...
			case self::E_WC_COUPON_MIN_SPEND_LIMIT_NOT_MET:
				$err = sprintf(__('The minimum spend for this coupon is %s.', 'woocommerce'),
				               wc_price($this->minimum_amount));
				break;
			case self::E_WC_COUPON_MAX_SPEND_LIMIT_MET:
				$err = sprintf(__('The maximum spend for this coupon is %s.', 'woocommerce'),
				               wc_price($this->maximum_amount));
				break;
			...
			case self::E_WC_COUPON_EXCLUDED_PRODUCTS:
				...
				$err = sprintf(__('Sorry, this coupon is not applicable to the products: %s.', 'woocommerce'),
				               implode(', ', $products));
				break;
			case self::E_WC_COUPON_EXCLUDED_CATEGORIES:
				...
				if (!WC()->cart->is_empty()) {
					foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
						$product_cats = wc_get_product_cat_ids($cart_item['product_id']);
						if (sizeof(
							    $intersect = array_intersect($product_cats, $this->exclude_product_categories)) > 0
						) {
				...
				$err = sprintf(__('Sorry, this coupon is not applicable to the categories: %s.', 'woocommerce'),
	...
	public function get_discount_amount($discounting_amount, $cart_item = NULL, $single = FALSE) {
		...
		if ($this->is_type(['percent_product', 'percent'])) {
			...
			if (wc_prices_include_tax()) {
				$discount_percent
					= ($cart_item['data']->get_price_including_tax() * $cart_item_qty) / WC()->cart->subtotal;
			} else {
				$discount_percent
					= ($cart_item['data']->get_price_excluding_tax() * $cart_item_qty) / WC()->cart->subtotal_ex_tax;
			}
		...
		if ($this->is_type(['percent_product', 'fixed_product'])) {
		...
		return apply_filters('woocommerce_coupon_get_discount_amount', $discount, $discounting_amount, $cart_item,
		                     $single, $this);
	}
