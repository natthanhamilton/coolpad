...
function wc_get_text_attributes($raw_attributes) {
	return array_map('trim',
	                 explode(WC_DELIMITER, html_entity_decode($raw_attributes, ENT_QUOTES, get_bloginfo('charset'))));
}
...
function wc_get_attribute_taxonomies() {
	if (FALSE === ($attribute_taxonomies = get_transient('wc_attribute_taxonomies'))) {
		...
		$attribute_taxonomies
			= $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "woocommerce_attribute_taxonomies order by attribute_name ASC;");
...
function wc_attribute_label($name, $product = '') {
	...
	if (taxonomy_is_product_attribute($name)) {
		...
		$label
			= $wpdb->get_var($wpdb->prepare("SELECT attribute_label FROM {$wpdb->prefix}woocommerce_attribute_taxonomies WHERE attribute_name = %s;",
			                                $name));
		if (!$label) {
			$label = $name;
		}
	} elseif ($product && ($attributes
			= $product->get_attributes()) && isset($attributes[ sanitize_title($name) ]['name'])
	) {
...
function wc_attribute_orderby($name) {
	...
	if (isset($wc_product_attributes[ 'pa_' . $name ])) {
		...
		$orderby
			= $wpdb->get_var($wpdb->prepare("SELECT attribute_orderby FROM " . $wpdb->prefix . "woocommerce_attribute_taxonomies WHERE attribute_name = %s;",
			                                $name));
	}
...
function wc_get_attribute_types() {
	return (array)apply_filters('product_attributes_type_selector', [
