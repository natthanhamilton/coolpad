...
class WC_Tax {
	...
	public static function calc_tax($price, $rates, $price_includes_tax = FALSE, $suppress_rounding = FALSE) {
		...
		$taxes = array_map([__CLASS__, 'remove_precision'], $taxes);
	...
	public static function calc_exclusive_tax($price, $rates) {
		...
		if (!empty($rates)) {
			...
			foreach ($rates as $key => $rate) {
				...
				// ADVANCED: Allow third parties to modify this rate
				$tax_amount = apply_filters('woocommerce_price_ex_tax_amount', $tax_amount, $key, $rate, $price,
				                            $the_price_inc_tax, $pre_compound_total);
				// Add rate
				if (!isset($taxes[ $key ])) {
					$taxes[ $key ] = $tax_amount;
				} else {
	...
	public static function find_rates($args = []) {
		...
		$cache_key
		                   = WC_Cache_Helper::get_cache_prefix('taxes') . 'wc_tax_rates_' . md5(sprintf('%s+%s+%s+%s+%s',
		                                                                                                $country,
		                                                                                                $state, $city,
		                                                                                                $postcode,
		                                                                                                $tax_class));
	...
	private static function get_matched_tax_rates($country, $state, $postcode, $city, $tax_class) {
		...
		$postcode_ranges
		                 = $wpdb->get_results("SELECT tax_rate_id, location_code FROM {$wpdb->prefix}woocommerce_tax_rate_locations WHERE location_type = 'postcode' AND location_code LIKE '%...%';");
		if ($postcode_ranges) {
		...
		$locations_criteria[]
		                      = "
			locations.location_type = 'postcode' AND locations.location_code IN ('" . implode("','",
		                                                                                      array_map('esc_sql',
		                                                                                                $postcode_search)) . "')
			AND (
		...
		return apply_filters('woocommerce_matched_tax_rates', $matched_tax_rates, $country, $state, $postcode, $city,
		                     $tax_class);
	}
	...
	public static function get_base_tax_rates($tax_class = '') {
		return apply_filters('woocommerce_base_tax_rates', self::find_rates([
	...
	public static function is_compound($key) {
		...
		return $wpdb->get_var($wpdb->prepare("SELECT tax_rate_compound FROM {$wpdb->prefix}woocommerce_tax_rates WHERE tax_rate_id = %s",
		                                     $key)) ? TRUE : FALSE;
	}
	...
	public static function get_rate_label($key_or_rate) {
		...
		if (is_object($key_or_rate)) {
			...
			$key = $key_or_rate;
			$rate_name
			     = $wpdb->get_var($wpdb->prepare("SELECT tax_rate_name FROM {$wpdb->prefix}woocommerce_tax_rates WHERE tax_rate_id = %s",
			                                     $key));
		}
		if (!$rate_name) {
	...
	public static function get_rate_percent($key_or_rate) {
		...
		if (is_object($key_or_rate)) {
			...
			$key = $key_or_rate;
			$tax_rate
			     = $wpdb->get_var($wpdb->prepare("SELECT tax_rate FROM {$wpdb->prefix}woocommerce_tax_rates WHERE tax_rate_id = %s",
			                                     $key));
		}
	...
	public static function get_rate_code($key_or_rate) {
		...
		if (is_object($key_or_rate)) {
			...
			$key = $key_or_rate;
			$rate
			     = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}woocommerce_tax_rates WHERE tax_rate_id = %s",
			                                     $key));
		}
		$code_string = '';
		if (NULL !== $rate) {
	...
	public static function get_tax_total($taxes) {
		return array_sum(array_map([__CLASS__, 'round'], $taxes));
	}
	...
	public static function _delete_tax_rate($tax_rate_id) {
		global $wpdb;
		$wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->prefix}woocommerce_tax_rate_locations WHERE tax_rate_id = %d;",
		                            $tax_rate_id));
		$wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->prefix}woocommerce_tax_rates WHERE tax_rate_id = %d;",
		                            $tax_rate_id));
		WC_Cache_Helper::incr_cache_prefix('taxes');
	...
	public static function _update_tax_rate_postcodes($tax_rate_id, $postcodes) {
		...
		self::_update_tax_rate_locations($tax_rate_id, array_diff(array_filter($postcodes), ['*']), 'postcode');
	}
	...
	private static function _update_tax_rate_locations($tax_rate_id, $values, $type) {
		...
		if (sizeof($values) > 0) {
			$sql = "( '" . implode("', $tax_rate_id, '" . esc_sql($type) . "' ),( '",
			                       array_map('esc_sql', $values)) . "', $tax_rate_id, '" . esc_sql($type) . "' )";
			$wpdb->query("
	...
	public static function get_rates_for_tax_class($tax_class) {
		...
		$rates
			       = $wpdb->get_results($wpdb->prepare("SELECT * FROM `{$wpdb->prefix}woocommerce_tax_rates` WHERE `tax_rate_class` = %s ORDER BY `tax_rate_order`;",
