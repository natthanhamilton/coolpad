...
class WC_CLI_Command extends WP_CLI_Command {
	...
	protected function merge_wp_query_args($base_args, $assoc_args) {
		...
		if (!empty($assoc_args['created_at_min']) || !empty($assoc_args['created_at_max']) || !empty($assoc_args['updated_at_min']) || !empty($assoc_args['updated_at_max'])) {
			...
			if (!empty($assoc_args['created_at_min'])) {
				$args['date_query'][]
					= ['column' => 'post_date_gmt', 'after' => $this->parse_datetime($assoc_args['created_at_min']), 'inclusive' => TRUE];
			}
			// resources created before specified date
			if (!empty($assoc_args['created_at_max'])) {
				$args['date_query'][]
					= ['column' => 'post_date_gmt', 'before' => $this->parse_datetime($assoc_args['created_at_max']), 'inclusive' => TRUE];
			}
			// resources updated after specified date
			if (!empty($assoc_args['updated_at_min'])) {
				$args['date_query'][]
					= ['column' => 'post_modified_gmt', 'after' => $this->parse_datetime($assoc_args['updated_at_min']), 'inclusive' => TRUE];
			}
			// resources updated before specified date
			if (!empty($assoc_args['updated_at_max'])) {
				$args['date_query'][]
					= ['column' => 'post_modified_gmt', 'before' => $this->parse_datetime($assoc_args['updated_at_max']), 'inclusive' => TRUE];
			}
		}
		// Search.
	...
	protected function merge_wp_user_query_args($base_args, $assoc_args) {
		...
		if (!empty($assoc_args['created_at_min']) || !empty($assoc_args['created_at_max'])) {
			...
			if (!empty($assoc_args['created_at_min'])) {
				$args['date_query'][]
					= ['after' => $this->parse_datetime($assoc_args['created_at_min']), 'inclusive' => TRUE];
			}
			// resources created before specified date
			if (!empty($assoc_args['created_at_max'])) {
				$args['date_query'][]
					= ['before' => $this->parse_datetime($assoc_args['created_at_max']), 'inclusive' => TRUE];
			}
		}
	...
	protected function get_format_args($assoc_args) {
		...
		if (!empty($assoc_args['format']) && in_array($assoc_args['format'],
		                                              ['count', 'ids', 'table', 'csv', 'json'])
		) {
	...
	protected function unflatten_array($arr) {
		...
		foreach ($arr as $key => $value) {
			...
			if (sizeof($key_list) > 0) {
				...
				$subarray       = $this->unflatten_array([$remaining_keys => $value]);
				foreach ($subarray as $sub_key => $sub_value) {
					...
					if (!empty($unflatten[ $first_key ][ $sub_key ])) {
						$unflatten[ $first_key ][ $sub_key ]
							= array_merge_recursive($unflatten[ $first_key ][ $sub_key ], $sub_value);
					} else {
