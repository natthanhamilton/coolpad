...
class WC_CLI_Coupon extends WC_CLI_Command {
	...
	public function create($__, $assoc_args) {
		...
		try {
			...
			if (!isset($assoc_args['code'])) {
				throw new WC_CLI_Exception('woocommerce_cli_missing_coupon_code',
				                           sprintf(__('Missing parameter %s', 'woocommerce'), 'code'));
			}
			...
			$coupon_found = $wpdb->get_var($wpdb->prepare("
			 ...
			 ", $coupon_code));
			if ($coupon_found) {
				throw new WC_CLI_Exception('woocommerce_cli_coupon_code_already_exists',
				                           __('The coupon code already exists', 'woocommerce'));
			}
			$defaults = [
				'type'                         => 'fixed_cart',
				'amount'                       => 0,
			...
			if (!in_array(wc_clean($coupon_data['type']), array_keys(wc_get_coupon_types()))) {
				throw new WC_CLI_Exception('woocommerce_cli_invalid_coupon_type',
			...
			update_post_meta($id, 'free_shipping',
			                 ($this->is_true($coupon_data['enable_free_shipping'])) ? 'yes' : 'no');
			...
			update_post_meta($id, 'exclude_sale_items',
			                 ($this->is_true($coupon_data['exclude_sale_items'])) ? 'yes' : 'no');
	...
	protected function get_coupon_from_id_or_code($coupon_id_or_code, $display_warning = FALSE) {
		global $wpdb;
		$code
			= $wpdb->get_var($wpdb->prepare("SELECT post_title FROM $wpdb->posts WHERE (id = %s OR post_title = %s) AND post_type = 'shop_coupon' AND post_status = 'publish' LIMIT 1",
			                                $coupon_id_or_code, $coupon_id_or_code));
		if (!$code) {
			if ($display_warning) {
	...
	public function get($args, $assoc_args) {
		...
		try {
			...
			if (!$coupon) {
				throw new WC_CLI_Exception('woocommerce_cli_invalid_coupon',
				                           sprintf(__('Invalid coupon ID or code: %s', 'woocommerce'), $args[0]));
			}
			...
			$coupon_data = [
				...
				'expiry_date'                  => (!empty($coupon->expiry_date))
					? $this->format_datetime($coupon->expiry_date) : NULL,
	...
	protected function format_posts_to_items($posts) {
		...
		foreach ($posts as $post) {
			$items[] = [
				...
				'usage_limit_per_user'         => (!empty($post->usage_limit_per_user)) ? $post->usage_limit_per_user
					: NULL,
				'limit_usage_to_x_items'       => (int)$post->limit_usage_to_x_items,
				'usage_count'                  => (int)$post->usage_count,
				'expiry_date'                  => (!empty($post->expiry_date))
					? $this->format_datetime($post->expiry_date) : NULL,
				'free_shipping'                => $post->free_shipping,
				'product_category_ids'         => implode(', ', is_array($post->product_categories)
					? $post->product_categories : []),
				'exclude_product_category_ids' => implode(', ', is_array($post->exclude_product_categories)
					? $post->exclude_product_categories : []),
				...
				'customer_emails'              => implode(', ',
				                                          is_array($post->customer_email) ? $post->customer_email : []),
	...
	public function update($args, $assoc_args) {
		try {
			...
			$id
				= wp_update_post(['ID' => intval($id), 'post_title' => $coupon_code, 'post_excerpt' => isset($data['description'])
				? $data['description'] : '']);
			if (0 === $id) {
				throw new WC_CLI_Exception('woocommerce_cli_cannot_update_coupon',
				                           __('Failed to update coupon', 'woocommerce'));
			}
			if (isset($data['type'])) {
				// Validate coupon types.
			...
			if (isset($data['exclude_sale_items'])) {
				update_post_meta($id, 'exclude_sale_items',
				                 ($this->is_true($data['exclude_sale_items'])) ? 'yes' : 'no');
			}
