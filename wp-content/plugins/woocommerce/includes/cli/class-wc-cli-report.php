...
class WC_CLI_Report extends WC_CLI_Command {
	...
	public function list_($__, $assoc_args) {
		$reports   = ['sales', 'sales/top_sellers'];
	...
	public function sales($__, $assoc_args) {
		...
		for ($i = 0; $i <= $reporter->chart_interval; $i++) {
			...
			foreach ($customers as $customer) {
				if (date(('day' == $reporter->chart_groupby) ? 'Y-m-d' : 'Y-m',
		...
		foreach ($report_data->orders as $order) {
			$time = ('day' === $reporter->chart_groupby) ? date('Y-m-d', strtotime($order->post_date))
		...
		foreach ($report_data->order_items as $order_item) {
			$time = ('day' === $reporter->chart_groupby) ? date('Y-m-d', strtotime($order_item->post_date))
		...
		foreach ($report_data->coupons as $discount) {
			$time = ('day' === $reporter->chart_groupby) ? date('Y-m-d', strtotime($discount->post_date))
	...
	private function get_reporter($assoc_args) {
		...
		if (empty($assoc_args['period'])) {
			...
			if (!empty($assoc_args['date_min']) || !empty($assoc_args['date_max'])) {
				...
				$_GET['end_date']   = isset($assoc_args['date_max']) ? $this->parse_datetime($assoc_args['date_max'])
					: NULL;
			} else {
			...
			if (!in_array($assoc_args['period'], ['week', 'month', 'last_month', 'year'])) {
	...
	public function top_sellers($__, $assoc_args) {
		$reporter    = $this->get_reporter($assoc_args);
		$top_sellers = $reporter->get_order_report_data([
			                                                'data'         => [
				                                                '_product_id' => [
