...
class WC_CLI_Tax extends WC_CLI_Command {
	...
	public function create_class($__, $assoc_args) {
		try {
			...
			if (!isset($assoc_args['name'])) {
				throw new WC_CLI_Exception('woocommerce_cli_missing_name',
			...
			if ($exists) {
				throw new WC_CLI_Exception('woocommerce_cli_cannot_create_tax_class',
				                           __('Tax class already exists', 'woocommerce'));
			}
			// Add the new class
			$classes[] = $name;
			update_option('woocommerce_tax_classes', implode("\n", $classes));
			do_action('woocommerce_cli_create_tax_class', $slug, ['name' => $name]);
			if ($porcelain) {
				WP_CLI::line($slug);
			} else {
	...
	public function get($args, $assoc_args) {
		global $wpdb;
		try {
			$tax_data = $this->format_taxes_to_items([$args[0]]);
			if (empty($tax_data)) {
				throw new WC_CLI_Exception('woocommerce_cli_invalid_tax_rate',
				                           sprintf(__('Invalid tax rate ID: %s', 'woocommerce'), $args[0]));
			}
	...
	public function list_($__, $assoc_args) {
		$query_args = $this->merge_tax_query_args([], $assoc_args);
		$formatter  = $this->get_formatter($assoc_args);
	...
	public function list_class($__, $assoc_args) {
		...
		foreach ($classes as $class) {
			$items[] = apply_filters('woocommerce_cli_tax_class_response', [
