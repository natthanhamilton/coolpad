...
class WC_CLI_Product extends WC_CLI_Command {
	...
	public function create($args, $assoc_args) {
		...
		try {
			...
			if (!isset($data['title'])) {
				throw new WC_CLI_Exception('woocommerce_missing_product_title',
			...
			if (!in_array(wc_clean($data['type']), array_keys(wc_get_product_types()))) {
				throw new WC_CLI_Exception('woocommerce_cli_invalid_product_type',
				...
				if (isset($image['position']) && $image['position'] == 0) {
					...
					if (0 === $attachment_id && isset($image['src'])) {
						...
						if (is_wp_error($upload)) {
							throw new WC_CLI_Exception('woocommerce_cli_cannot_upload_product_image',
	...
	private function upload_product_image($image_url) {
		...
		if (!$parsed_url || !is_array($parsed_url)) {
			throw new WC_CLI_Exception('woocommerce_cli_invalid_product_image',
		...
		if (is_wp_error($response)) {
			throw new WC_CLI_Exception('woocommerce_cli_invalid_remote_product_image',
		...
		if (!$wp_filetype['type']) {
			...
			$wp_filetype = wp_check_filetype($file_name, $this->allowed_image_mime_types());
			if (!$wp_filetype['type']) {
				throw new WC_CLI_Exception('woocommerce_cli_invalid_image_type',
				                           __('Invalid image type.', 'woocommerce'));
			}
		}
		// Upload the file.
		...
		if (0 == $filesize) {
			@unlink($upload['file']);
			unset($upload);
			throw new WC_CLI_Exception('woocommerce_cli_product_image_upload_file_error',
			                           __('Zero size file downloaded', 'woocommerce'));
		}
		unset($response);
	...
	private function save_product_meta($product_id, $data) {
		...
		if (isset($data['sku'])) {
			...
			if ('' == $new_sku) {
				...
				if (!empty($new_sku)) {
					...
					if (!$unique_sku) {
						throw new WC_CLI_Exception('woocommerce_cli_product_sku_already_exists',
		...
		if (in_array($product_type, ['variable', 'grouped'])) {
			...
			$date_from = isset($data['sale_price_dates_from']) ? strtotime($data['sale_price_dates_from'])
				: get_post_meta($product_id, '_sale_price_dates_from', TRUE);
			$date_to   = isset($data['sale_price_dates_to']) ? strtotime($data['sale_price_dates_to'])
				: get_post_meta($product_id, '_sale_price_dates_to', TRUE);
			// Dates
			if ($date_from) {
		...
		if (isset($data['sold_individually'])) {
			update_post_meta($product_id, '_sold_individually',
			                 ($this->is_true($data['sold_individually'])) ? 'yes' : '');
		}
		// Stock status
	...
	private function save_variations($id, $data) {
		...
		foreach ($variations as $menu_order => $variation) {
			...
			if (is_wp_error($variation_id)) {
				throw new WC_CLI_Exception('woocommerce_cli_cannot_save_product_variation',
				                           $variation_id->get_error_message());
			}
			// SKU
			...
			if (isset($variation['regular_price'])) {
				$regular_price = ('' === $variation['regular_price']) ? ''
					: wc_format_decimal($variation['regular_price']);
			...
			$date_from = isset($variation['sale_price_dates_from']) ? strtotime($variation['sale_price_dates_from'])
				: get_post_meta($variation_id, '_sale_price_dates_from', TRUE);
			$date_to   = isset($variation['sale_price_dates_to']) ? strtotime($variation['sale_price_dates_to'])
	...
	public function get($args, $assoc_args) {
		try {
			...
			if (!$product) {
				throw new WC_CLI_Exception('woocommerce_cli_invalid_product',
				                           sprintf(__('Invalid product "%s"', 'woocommerce'), $args[0]));
			}
	...
	private function get_product_data($product) {
		...
		$product_data = [
			...
			'categories'         => implode(', ',
			                                wp_get_post_terms($product->id, 'product_cat', ['fields' => 'names'])),
			'tags'               => implode(', ',
			                                wp_get_post_terms($product->id, 'product_tag', ['fields' => 'names'])),
			'images'             => $this->get_images($product),
			'featured_src'       => wp_get_attachment_url(get_post_thumbnail_id($product->is_type('variation')
	...
	public function reviews($args, $assoc_args) {
		try {
			...
			if (!$product) {
				throw new WC_CLI_Exception('woocommerce_cli_invalid_product',
				                           sprintf(__('Invalid product "%s"', 'woocommerce'), $id));
			}
	...
	public function update($args, $assoc_args) {
		try {
			...
			if (isset($data['short_description'])) {
				// Enable short description html tags.
				$post_excerpt
					= (isset($data['enable_html_short_description']) && $this->is_true($data['enable_html_short_description']))
			...
			if (isset($data['description'])) {
				// Enable description html tags.
				$post_content
					= (isset($data['enable_html_description']) && $this->is_true($data['enable_html_description']))
