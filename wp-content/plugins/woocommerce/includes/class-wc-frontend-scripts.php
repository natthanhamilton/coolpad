...
class WC_Frontend_Scripts {
	...
	public static function load_scripts() {
		...
		self::register_script('select2', $assets_path . 'js/select2/select2' . $suffix . '.js', ['jquery'], '3.5.4');
		self::register_script('jquery-blockui', $assets_path . 'js/jquery-blockui/jquery.blockUI' . $suffix . '.js',
		                      ['jquery'], '2.70');
		self::register_script('jquery-payment', $assets_path . 'js/jquery-payment/jquery.payment' . $suffix . '.js',
		                      ['jquery'], '1.4.1');
		self::register_script('jquery-cookie', $assets_path . 'js/jquery-cookie/jquery.cookie' . $suffix . '.js',
		                      ['jquery'], '1.4.1');
		self::register_script('wc-credit-card-form', $frontend_script_path . 'credit-card-form' . $suffix . '.js',
		                      ['jquery', 'jquery-payment']);
		self::register_script('wc-add-to-cart-variation',
		                      $frontend_script_path . 'add-to-cart-variation' . $suffix . '.js', ['jquery', 'wp-util']);
		...
		self::register_script('wc-country-select', $frontend_script_path . 'country-select' . $suffix . '.js',
		                      ['jquery']);
		self::register_script('wc-address-i18n', $frontend_script_path . 'address-i18n' . $suffix . '.js', ['jquery']);
		self::register_script('wc-password-strength-meter',
		                      $frontend_script_path . 'password-strength-meter' . $suffix . '.js',
		...
		if (is_cart()) {
			self::enqueue_script('wc-cart', $frontend_script_path . 'cart' . $suffix . '.js',
		...
		if (is_checkout()) {
			self::enqueue_script('wc-checkout', $frontend_script_path . 'checkout' . $suffix . '.js',
			                     ['jquery', 'woocommerce', 'wc-country-select', 'wc-address-i18n']);
		}
		if (is_add_payment_method_page()) {
			self::enqueue_script('wc-add-payment-method',
			                     $frontend_script_path . 'add-payment-method' . $suffix . '.js',
			                     ['jquery', 'woocommerce']);
		}
		if (is_lost_password_page()) {
			self::enqueue_script('wc-lost-password', $frontend_script_path . 'lost-password' . $suffix . '.js',
			                     ['jquery', 'woocommerce']);
		}
		if ($lightbox_en && (is_product() || (!empty($post->post_content) && strstr($post->post_content,
		                                                                            '[product_page')))
		) {
			self::enqueue_script('prettyPhoto', $assets_path . 'js/prettyPhoto/jquery.prettyPhoto' . $suffix . '.js',
			                     ['jquery'], '3.1.6', TRUE);
			self::enqueue_script('prettyPhoto-init',
		...
		if ('geolocation_ajax' === get_option('woocommerce_default_customer_address')) {
			...
			if (!strstr($ua, 'bot') && !strstr($ua, 'spider') && !strstr($ua, 'crawl')) {
				self::enqueue_script('wc-geolocation', $frontend_script_path . 'geolocation' . $suffix . '.js',
				                     ['jquery']);
			}
		}
		// Global frontend scripts
		self::enqueue_script('woocommerce', $frontend_script_path . 'woocommerce' . $suffix . '.js',
		                     ['jquery', 'jquery-blockui']);
		self::enqueue_script('wc-cart-fragments', $frontend_script_path . 'cart-fragments' . $suffix . '.js',
	 ...
	 */
	private static function register_script($handle, $path, $deps = ['jquery'], $version = WC_VERSION,
	                                        $in_footer = TRUE) {
		self::$scripts[] = $handle;
	 ...
	 * @param  boolean  $in_footer
	 */
	private static function enqueue_script($handle, $path = '', $deps = ['jquery'], $version = WC_VERSION,
	                                       $in_footer = TRUE) {
	 ...
	 * @param  string   $media
	 */
	private static function enqueue_style($handle, $path = '', $deps = [], $version = WC_VERSION, $media = 'all') {
	 ...
	 * @param  string   $media
	 */
	private static function register_style($handle, $path, $deps = [], $version = WC_VERSION, $media = 'all') {
		self::$styles[] = $handle;
	...
	public static function get_styles() {
		return apply_filters('woocommerce_enqueue_styles', [
			'woocommerce-layout'      => [
			...
			'woocommerce-smallscreen' => [
				...
				'version' => WC_VERSION,
				'media'   => 'only screen and (max-width: ' . apply_filters('woocommerce_style_smallscreen_breakpoint',
				                                                            $breakpoint = '768px') . ')'
			],
			'woocommerce-general'     => [
				'src'     => str_replace(['http:', 'https:'], '', WC()->plugin_url()) . '/assets/css/woocommerce.css',
				'deps'    => '',
				'version' => WC_VERSION,
				'media'   => 'all'
			],
		...
		switch ($handle) {
			...
			case 'wc-geolocation' :
				return [
					'wc_ajax_url'  => WC_AJAX::get_endpoint("%%endpoint%%"),
					'home_url'     => home_url(),
					'is_available' => !(is_cart() || is_account_page() || is_checkout() || is_customize_preview()) ? '1'
						: '0',
			...
			case 'wc-checkout' :
				return [
					...
					'is_checkout'               => is_page(wc_get_page_id('checkout')) && empty($wp->query_vars['order-pay']) && !isset($wp->query_vars['order-received'])
						? 1 : 0,
					'debug_mode'                => defined('WP_DEBUG') && WP_DEBUG,
					'i18n_checkout_error'       => esc_attr__('Error processing checkout. Please try again.',
					                                          'woocommerce'),
				];
				break;
			...
			case 'wc-add-to-cart-variation' :
				...
				return [
					'i18n_no_matching_variations_text' => esc_attr__('Sorry, no products matched your selection. Please choose a different combination.',
					                                                 'woocommerce'),
					'i18n_make_a_selection_text'       => esc_attr__('Please select some product options before adding this product to your cart.',
					                                                 'woocommerce'),
			...
			case 'wc-password-strength-meter' :
				return [
					...
					'i18n_password_hint'    => esc_attr__('The password should be at least seven characters long. To make it stronger, use upper and lower case letters, numbers and symbols like ! " ? $ % ^ &amp; ).',
					                                      'woocommerce')
				];
				break;
		}
