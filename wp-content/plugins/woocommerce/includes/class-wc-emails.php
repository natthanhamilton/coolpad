 ...
 *
 * WooCommerce Emails Class which handles the sending on transactional emails and email templates. This class loads in
 * available emails.
 *
 * @class          WC_Emails
 * @version        2.3.0
...
class WC_Emails {
	...
	public function __construct() {
		...
		add_action('woocommerce_email_order_details', [$this, 'order_schema_markup'], 20, 4);
	...
	public function init() {
		...
		$this->emails['WC_Email_Customer_On_Hold_Order']
		                                               = include('emails/class-wc-email-customer-on-hold-order.php');
		$this->emails['WC_Email_Customer_Processing_Order']
		                                               = include('emails/class-wc-email-customer-processing-order.php');
		$this->emails['WC_Email_Customer_Completed_Order']
		                                               = include('emails/class-wc-email-customer-completed-order.php');
		$this->emails['WC_Email_Customer_Refunded_Order']
		                                               = include('emails/class-wc-email-customer-refunded-order.php');
		...
		$this->emails['WC_Email_Customer_Reset_Password']
		                                               = include('emails/class-wc-email-customer-reset-password.php');
	...
	public static function init_transactional_emails() {
		$email_actions = apply_filters('woocommerce_email_actions', [
	...
	public function order_details($order, $sent_to_admin = FALSE, $plain_text = FALSE, $email = '') {
		if ($plain_text) {
			wc_get_template('emails/plain/email-order-details.php',
			                ['order' => $order, 'sent_to_admin' => $sent_to_admin, 'plain_text' => $plain_text, 'email' => $email]);
		} else {
	...
	public function order_schema_markup($order, $sent_to_admin = FALSE, $plain_text = FALSE) {
		...
		foreach ($order->get_items() as $item) {
			...
			$product        = apply_filters('woocommerce_order_item_product', $order->get_product_from_item($item),
			                                $item);
	...
	public function customer_details($order, $sent_to_admin = FALSE, $plain_text = FALSE) {
		...
		$fields = array_filter(apply_filters('woocommerce_email_customer_details_fields', $fields, $sent_to_admin,
		                                     $order), [$this, 'customer_detail_field_is_valid']);
		if ($plain_text) {
	...
	public function low_stock($product) {
		...
		$message = sprintf(__('%s is low in stock.', 'woocommerce'),
		                   html_entity_decode(strip_tags($product->get_formatted_name()), ENT_QUOTES,
		                                      get_bloginfo('charset'))) . ' ' . sprintf(__('There are %d left',
		                                                                                   'woocommerce'),
		                                                                                html_entity_decode(strip_tags($product->get_total_stock())));
		wp_mail(
			apply_filters('woocommerce_email_recipient_low_stock', get_option('woocommerce_stock_email_recipient'),
			              $product),
			...
			apply_filters('woocommerce_email_attachments', [], 'low_stock', $product)
		);
	}
	...
	public function no_stock($product) {
		...
		$message = sprintf(__('%s is out of stock.', 'woocommerce'),
		                   html_entity_decode(strip_tags($product->get_formatted_name()), ENT_QUOTES,
		                                      get_bloginfo('charset')));
		wp_mail(
			apply_filters('woocommerce_email_recipient_no_stock', get_option('woocommerce_stock_email_recipient'),
			              $product),
			...
			apply_filters('woocommerce_email_attachments', [], 'no_stock', $product)
		);
	}
	...
	public function backorder($args) {
		...
		$message = sprintf(__('%1$s units of %2$s have been backordered in order #%3$s.', 'woocommerce'), $quantity,
		                   html_entity_decode(strip_tags($product->get_formatted_name()), ENT_QUOTES,
		                                      get_bloginfo('charset')), $order->get_order_number());
		wp_mail(
			apply_filters('woocommerce_email_recipient_backorder', get_option('woocommerce_stock_email_recipient'),
			              $args),
			...
			apply_filters('woocommerce_email_attachments', [], 'backorder', $args)
		);
	}
}
