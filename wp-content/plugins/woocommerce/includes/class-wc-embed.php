...
class WC_Embed {
	...
	public static function product_buttons() {
		...
		if ($_product->is_type('simple') && $_product->is_purchasable() && $_product->is_in_stock()) {
			$buttons[] = sprintf($button, esc_url(add_query_arg('add-to-cart', get_the_ID(), wc_get_cart_url())),
	...
	public static function get_ratings() {
		// Make sure we're only affecting embedded products.
		if (self::is_embedded_product() && ($_product
				= wc_get_product(get_the_ID())) && $_product->get_average_rating() > 0
		) {
			?>
			<div class="wc-embed-rating">
				<?php echo esc_html(sprintf(__('Rated %s out of 5', 'woocommerce'),
