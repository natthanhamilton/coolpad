...
class WC_Session_Handler extends WC_Session {
	...
	public function __construct() {
		...
		add_action('woocommerce_cleanup_sessions', [$this, 'cleanup_sessions'], 10);
	...
	public function get_session($customer_id, $default = FALSE) {
		...
		if (FALSE === $value) {
			...
			wp_cache_add($this->get_cache_prefix() . $customer_id, $value, WC_SESSION_CACHE_GROUP,
	...
	public function save_data() {
		...
		if ($this->_dirty && $this->has_session()) {
			...
			// Set cache
			wp_cache_set($this->get_cache_prefix() . $this->_customer_id, $this->_data, WC_SESSION_CACHE_GROUP,
			             $this->_session_expiration - time());
	...
	public function destroy_session() {
		// Clear cookie
		wc_setcookie($this->_cookie, '', time() - YEAR_IN_SECONDS,
		             apply_filters('wc_session_use_secure_cookie', FALSE));
