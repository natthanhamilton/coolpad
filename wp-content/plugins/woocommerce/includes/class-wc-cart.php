 ...
 * WooCommerce cart
 *
 * The WooCommerce cart class stores cart data and active coupons as well as handling customer sessions and some cart
 * related urls. The cart class also has a price calculation function which calls upon other classes to calculate
 * totals.
 *
 * @class          WC_Cart
 * @version        2.1.0
...
class WC_Cart {
	...
	public function __construct() {
		add_action('wp_loaded', [$this, 'init']); // Get cart after WP and plugins are loaded.
		add_action('wp', [$this, 'maybe_set_cart_cookies'], 99); // Set cookies
		add_action('shutdown', [$this, 'maybe_set_cart_cookies'], 0); // Set cookies before shutdown and ob flushing
	...
	public function __get($key) {
		switch ($key) {
			...
			case 'tax' :
				...
				return $this->tax;
			case 'discount_total':
				_deprecated_argument('WC_Cart->discount_total', '2.3',
				                     'After tax coupons are no longer supported. For more information see: https://woocommerce.wordpress.com/2014/12/upcoming-coupon-changes-in-woocommerce-2-3/');
	...
	public function get_cart() {
		if (!did_action('wp_loaded')) {
			_doing_it_wrong(__FUNCTION__,
			                __('Get cart should not be called before the wp_loaded action.', 'woocommerce'), '2.3');
		}
	...
	public function get_cart_from_session() {
		...
		$this->removed_cart_contents = array_filter(WC()->session->get('removed_cart_contents', []));
		$this->applied_coupons       = array_filter(WC()->session->get('applied_coupons', []));
		/**
		 * Load the cart object. This defaults to the persistent cart if null.
		 */
		$cart = WC()->session->get('cart', NULL);
		if (is_null($cart) && ($saved_cart = get_user_meta(get_current_user_id(), '_woocommerce_persistent_cart',
		                                                   TRUE))
		) {
			...
			foreach ($cart as $key => $values) {
				...
				if (!empty($_product) && $_product->exists() && $values['quantity'] > 0) {
					if (!$_product->is_purchasable()) {
						...
						$update_cart_session = TRUE;
						wc_add_notice(sprintf(__('%s has been removed from your cart because it can no longer be purchased. Please contact us if you need assistance.',
						                         'woocommerce'), $_product->get_title()), 'error');
						do_action('woocommerce_remove_cart_item_from_session', $key, $values);
					} else {
						// Put session data into array. Run through filter so other plugins can load their own session data
						$session_data                = array_merge($values, ['data' => $_product]);
						$this->cart_contents[ $key ] = apply_filters('woocommerce_get_cart_item_from_session',
						                                             $session_data, $values, $key);
					}
				}
			}
		}
	...
	public function calculate_totals() {
		...
		foreach ($cart as $cart_item_key => $values) {
			...
			if (!$_product->is_taxable()) {
				 ...
				 */
				if ($item_tax_rates !== $base_tax_rates && apply_filters('woocommerce_adjust_non_base_location_prices',
				                                                         TRUE)
				) {
		...
		uasort($cart, [$this, 'sort_by_subtotal']);
		/**
		...
		foreach ($cart as $cart_item_key => $values) {
			...
			if (!$_product->is_taxable()) {
				...
				foreach (array_keys($this->taxes + $discounted_taxes) as $key) {
					$this->taxes[ $key ] = (isset($discounted_taxes[ $key ]) ? $discounted_taxes[ $key ]
							: 0) + (isset($this->taxes[ $key ]) ? $this->taxes[ $key ] : 0);
				}
				/**
				 * Prices exclude tax.
				 */
			...
			$this->cart_contents[ $cart_item_key ]['line_tax_data']
				= ['total' => $discounted_taxes, 'subtotal' => $taxes];
		}
		...
		if (is_checkout() || is_cart() || defined('WOOCOMMERCE_CHECKOUT') || defined('WOOCOMMERCE_CART')) {
			...
			if ($this->round_at_subtotal) {
				...
				$this->taxes              = array_map(['WC_Tax', 'round'], $this->taxes);
				$this->shipping_taxes     = array_map(['WC_Tax', 'round'], $this->shipping_taxes);
			} else {
			...
			$this->total = max(0, apply_filters('woocommerce_calculated_total',
			                                    round($this->cart_contents_total + $this->tax_total + $this->shipping_tax_total + $this->shipping_total + $this->fee_total,
			                                          $this->dp), $this));
		} else {
		...
		if (!empty($this->coupons)) {
			...
			foreach ($this->coupons as $code => $coupon) {
				if ($coupon->is_valid() && ($coupon->is_valid_for_product($product,
				                                                          $values) || $coupon->is_valid_for_cart())
				) {
					$discount_amount
						             = $coupon->get_discount_amount('yes' === get_option('woocommerce_calc_discounts_sequentially',
						                                                                 'no') ? $price
							                                            : $undiscounted_price, $values, TRUE);
					...
					if ($add_totals) {
						...
						if (wc_tax_enabled()) {
							...
							$taxes              = WC_Tax::calc_tax($discount_amount, $tax_rates,
							                                       $this->prices_include_tax);
							$total_discount_tax = WC_Tax::get_tax_total($taxes) * $values['quantity'];
							$total_discount     = $this->prices_include_tax ? $total_discount - $total_discount_tax
								: $total_discount;
	...
	private function increase_coupon_discount_amount($code, $amount, $tax) {
		$this->coupon_discount_amounts[ $code ]     = isset($this->coupon_discount_amounts[ $code ])
			? $this->coupon_discount_amounts[ $code ] + $amount : $amount;
		$this->coupon_discount_tax_amounts[ $code ] = isset($this->coupon_discount_tax_amounts[ $code ])
			? $this->coupon_discount_tax_amounts[ $code ] + $tax : $tax;
	}
				...
				if ($fee->taxable) {
					...
					if (!empty($fee_taxes)) {
						...
						foreach (array_keys($this->taxes + $fee_taxes) as $key) {
							$this->taxes[ $key ] = (isset($fee_taxes[ $key ]) ? $fee_taxes[ $key ]
									: 0) + (isset($this->taxes[ $key ]) ? $this->taxes[ $key ] : 0);
						}
					}
				}
	...
	public function remove_taxes() {
		...
		foreach ($this->cart_contents as $cart_item_key => $item) {
			$this->cart_contents[ $cart_item_key ]['line_subtotal_tax']
				                                                    =
			$this->cart_contents[ $cart_item_key ]['line_tax'] = 0;
			$this->cart_contents[ $cart_item_key ]['line_tax_data'] = ['total' => [], 'subtotal' => []];
		}
		...
		if (apply_filters('woocommerce_cart_remove_taxes_apply_zero_rate', TRUE)) {
			$this->taxes
				=
			$this->shipping_taxes = [apply_filters('woocommerce_cart_remove_taxes_zero_rate_id', 'zero-rated') => 0];
		} else {
			$this->taxes = $this->shipping_taxes = [];
		}
	}
	...
	public function init() {
		...
		add_action('woocommerce_after_checkout_validation', [$this, 'check_customer_coupons'], 1);
	}
	...
	public function check_cart_item_validity() {
		...
		foreach ($this->get_cart() as $cart_item_key => $values) {
			...
			if (!$_product || !$_product->exists() || 'trash' === $_product->post->post_status) {
				...
				$return = new WP_Error('invalid', __('An item which is no longer available was removed from your cart.',
				                                     'woocommerce'));
			}
		}
	...
	public function check_cart_item_stock() {
		...
		foreach ($this->get_cart() as $cart_item_key => $values) {
			...
			if (!$_product->is_in_stock()) {
				$error->add('out-of-stock',
				            sprintf(__('Sorry, "%s" is not in stock. Please edit your cart and try again. We apologise for any inconvenience caused.',
				                       'woocommerce'), $_product->get_title()));
			...
			$check_qty = $_product->is_type('variation') && TRUE === $_product->managing_stock()
				? $product_qty_in_cart[ $values['variation_id'] ] : $product_qty_in_cart[ $values['product_id'] ];
			/**
			 * Check stock based on all items in the cart.
			 */
			if (!$_product->has_enough_stock($check_qty)) {
				$error->add('out-of-stock',
				            sprintf(__('Sorry, we do not have enough "%s" in stock to fulfill your order (%s in stock). Please edit your cart and try again. We apologise for any inconvenience caused.',
				                       'woocommerce'), $_product->get_title(), $_product->get_stock_quantity()));
			...
			if (get_option('woocommerce_hold_stock_minutes') > 0 && !$_product->backorders_allowed()) {
				$order_id   = isset(WC()->session->order_awaiting_payment)
					? absint(WC()->session->order_awaiting_payment) : 0;
				$held_stock = $wpdb->get_var(
					$wpdb->prepare("
							...
							AND		posts.ID                   != %d;",
					               $_product->is_type('variation') && TRUE === $_product->managing_stock()
						               ? '_variation_id' : '_product_id',
					               $_product->is_type('variation') && TRUE === $_product->managing_stock()
						               ? $values['variation_id'] : $values['product_id'],
					               $order_id
					)
				);
				...
				if ($not_enough_stock) {
					$error->add('out-of-stock',
					            sprintf(__('Sorry, we do not have enough "%s" in stock to fulfill your order right now. Please try again in %d minutes or edit your cart and try again. We apologise for any inconvenience caused.',
					                       'woocommerce'), $_product->get_title(),
	...
	public function get_cart_item_quantities() {
		...
		foreach ($this->get_cart() as $cart_item_key => $values) {
			...
			if ($_product->is_type('variation') && TRUE === $_product->managing_stock()) {
				...
				$quantities[ $values['variation_id'] ] = isset($quantities[ $values['variation_id'] ])
					? $quantities[ $values['variation_id'] ] + $values['quantity'] : $values['quantity'];
			} else {
				$quantities[ $values['product_id'] ] = isset($quantities[ $values['product_id'] ])
					? $quantities[ $values['product_id'] ] + $values['quantity'] : $values['quantity'];
			}
		}
			...
			foreach ($cart_item['variation'] as $name => $value) {
				...
				if (taxonomy_exists($taxonomy)) {
					...
					if (isset($product_attributes[ str_replace('attribute_', '', $name) ])) {
						$label = wc_attribute_label($product_attributes[ str_replace('attribute_', '',
						                                                             $name) ]['name']);
					} else {
						$label = $name;
					}
	...
	public function get_remove_url($cart_item_key) {
		...
		return apply_filters('woocommerce_get_remove_url',
		                     $cart_page_url ? wp_nonce_url(add_query_arg('remove_item', $cart_item_key, $cart_page_url),
		                                                   'woocommerce-cart') : '');
	}
	...
	public function get_undo_url($cart_item_key) {
		...
		return apply_filters('woocommerce_get_undo_url',
		                     $cart_page_url ? wp_nonce_url(add_query_arg($query_args, $cart_page_url),
		                                                   'woocommerce-cart') : '', $cart_item_key);
	}
	...
	public function get_taxes() {
		...
		foreach (array_keys($this->taxes + $this->shipping_taxes) as $key) {
			$taxes[ $key ] = (isset($this->shipping_taxes[ $key ]) ? $this->shipping_taxes[ $key ]
					: 0) + (isset($this->taxes[ $key ]) ? $this->taxes[ $key ] : 0);
		}
	 ...
	 */
	public function add_to_cart($product_id = 0, $quantity = 1, $variation_id = 0, $variation = [],
	                            $cart_item_data = []) {
		...
		try {
			...
			if ($product_data->is_sold_individually()) {
				$quantity         = apply_filters('woocommerce_add_to_cart_sold_individually_quantity', 1, $quantity,
				                                  $product_id, $variation_id, $cart_item_data);
				$in_cart_quantity = $cart_item_key ? $this->cart_contents[ $cart_item_key ]['quantity'] : 0;
				if ($in_cart_quantity > 0) {
					throw new Exception(sprintf('<a href="%s" class="button wc-forward">%s</a> %s', wc_get_cart_url(),
					                            __('View Cart', 'woocommerce'),
					                            sprintf(__('You cannot add another &quot;%s&quot; to your cart.',
					                                       'woocommerce'), $product_data->get_title())));
				}
			}
			...
			if (!$product_data->is_in_stock()) {
				throw new Exception(sprintf(__('You cannot add &quot;%s&quot; to the cart because the product is out of stock.',
				                               'woocommerce'), $product_data->get_title()));
			}
			if (!$product_data->has_enough_stock($quantity)) {
				throw new Exception(sprintf(__('You cannot add that amount of &quot;%s&quot; to the cart because there is not enough stock (%s remaining).',
				                               'woocommerce'), $product_data->get_title(),
			...
			if ($managing_stock = $product_data->managing_stock()) {
				...
				if ($product_data->is_type('variation') && TRUE === $managing_stock) {
					$check_qty = isset($products_qty_in_cart[ $variation_id ]) ? $products_qty_in_cart[ $variation_id ]
						: 0;
				} else {
				...
				if (!$product_data->has_enough_stock($check_qty + $quantity)) {
					throw new Exception(sprintf(
						                    ...
						                    wc_get_cart_url(),
						                    __('View Cart', 'woocommerce'),
						                    sprintf(__('You cannot add that amount to the cart &mdash; we have %s in stock and you already have %s in your cart.',
						                               'woocommerce'), $product_data->get_stock_quantity(), $check_qty)
					                    ));
				}
			}
	...
	public function generate_cart_id($product_id, $variation_id = 0, $variation = [], $cart_item_data = []) {
		...
		return apply_filters('woocommerce_cart_id', md5(implode('_', $id_parts)), $product_id, $variation_id,
	...
	public function restore_cart_item($cart_item_key) {
		if (isset($this->removed_cart_contents[ $cart_item_key ])) {
			...
			$this->cart_contents[ $cart_item_key ]['data']
			                                       = wc_get_product($this->cart_contents[ $cart_item_key ]['variation_id']
				                                                        ? $this->cart_contents[ $cart_item_key ]['variation_id']
	...
	public function add_discount($coupon_code) {
		...
		if ($the_coupon->individual_use == 'yes') {
			$this->applied_coupons = apply_filters('woocommerce_apply_individual_use_coupon', [], $the_coupon,
			                                       $this->applied_coupons);
		}
		if ($this->applied_coupons) {
			foreach ($this->applied_coupons as $code) {
				$coupon = new WC_Coupon($code);
				if ($coupon->individual_use == 'yes' && FALSE === apply_filters('woocommerce_apply_with_individual_use_coupon',
				                                                                FALSE, $the_coupon, $coupon,
				                                                                $this->applied_coupons)
				) {
					// Reject new coupon
	...
	public function has_discount($coupon_code = '') {
		return $coupon_code ? in_array(apply_filters('woocommerce_coupon_code', $coupon_code), $this->applied_coupons)
			: sizeof($this->applied_coupons) > 0;
	}
	...
	public function get_coupon_discount_tax_amount($code) {
		return wc_cart_round_discount(isset($this->coupon_discount_tax_amounts[ $code ])
			                              ? $this->coupon_discount_tax_amounts[ $code ] : 0, $this->dp);
	}
	...
	public function remove_coupons($deprecated = NULL) {
		$this->applied_coupons
			= $this->coupon_discount_amounts = $this->coupon_discount_tax_amounts = $this->coupon_applied_count = [];
		WC()->session->set('applied_coupons', []);
		WC()->session->set('coupon_discount_amounts', []);
		WC()->session->set('coupon_discount_tax_amounts', []);
	}
	 ...
	 * @param string $tax_class (default: '') The tax class for the fee if taxable. A blank string is standard tax
	 *                          class.
	 */
	...
	public function get_cart_subtotal($compound = FALSE) {
		...
		if ($compound) {
			$cart_subtotal = wc_price($this->cart_contents_total + $this->shipping_total + $this->get_taxes_total(FALSE,
			                                                                                                      FALSE));
	 ...
	 *
	 * When on the checkout (review order), this will get the subtotal based on the customer's tax rate rather than the
	 * base rate.
	 *
	 * @param WC_Product $_product
	 * @param int        $quantity
