jQuery(function (a) {
    ...
    var b = {
        ...
        init_payment_methods: function () {
            ...
            1 === b.length && b.eq(0).hide(), 0 === b.filter(":checked").length && b.eq(0).prop("checked", !0), b.filter(":checked").eq(0).trigger("click")
        },
        ...
        update_checkout_action: function (c) {
            if (b.xhr && b.xhr.abort(), 0 !== a("form.checkout").length) {
                ...
                var d = a("#billing_country").val(), e = a("#billing_state").val(), f = a("input#billing_postcode").val(), g = a("#billing_city").val(), h = a("input#billing_address_1").val(), i = a("input#billing_address_2").val(), j = d, k = e, l = f, m = g, n = h, o = i;
                ...
                a(".woocommerce-checkout-payment, .woocommerce-checkout-review-order-table").block({
                    ...
                    success: function (c) {
                        ...
                        var d = a("#terms").prop("checked");
                        if (c && c.fragments && a.each(c.fragments, function (b, c) {
                                a(b).replaceWith(c), a(b).unblock()
                            }), d && a("#terms").prop("checked", !0), "failure" === c.result) {
                            var e = a("form.checkout");
                            a(".woocommerce-error, .woocommerce-message").remove(), c.messages ? e.prepend('<div class="woocommerce-NoticeGroup-updateOrderReview">' + c.messages + "</div>") : e.prepend(c), e.find(".input-text, select, input:checkbox").blur(), a("html, body").animate({scrollTop: a("form.checkout").offset().top - 100}, 1e3)
                        }
        ...
        submit: function () {
            ...
            if (c.triggerHandler("checkout_place_order") !== !1 && c.triggerHandler("checkout_place_order_" + b.get_payment_method()) !== !1) {
                ...
                1 !== d["blockUI.isBlocked"] && c.block({
                    ...
                    success: function (c) {
                        try {
                            if ("success" !== c.result)throw"failure" === c.result ? "Result failure" : "Invalid response";
                            -1 === c.redirect.indexOf("https://") || -1 === c.redirect.indexOf("http://") ? window.location = c.redirect : window.location = decodeURI(c.redirect)
                        } catch (d) {
                            if ("true" === c.reload)return void window.location.reload();
        ...
        submit_error: function (c) {
            a(".woocommerce-error, .woocommerce-message").remove(), b.$checkout_form.prepend(c), b.$checkout_form.removeClass("processing").unblock(), b.$checkout_form.find(".input-text, select, input:checkbox").blur(), a("html, body").animate({scrollTop: a("form.checkout").offset().top - 100}, 1e3), a(document.body).trigger("checkout_error")
        }
    }, c = {
        init: function () {
            ...
            return a.ajax({
                ...
                data: c,
                success: function (c) {
                    a(".woocommerce-error, .woocommerce-message").remove(), b.removeClass("processing").unblock(), c && (b.before(c), b.slideUp(), a(document.body).trigger("update_checkout", {update_shipping_method: !1}))
                },
                dataType: "html"
            }), !1
            ...
            var e = {security: wc_checkout_params.remove_coupon_nonce, coupon: d};
            a.ajax({
                type: "POST",
                url: wc_checkout_params.wc_ajax_url.toString().replace("%%endpoint%%", "remove_coupon"),
                data: e,
                success: function (b) {
                    a(".woocommerce-error, .woocommerce-message").remove(), c.removeClass("processing").unblock(), b && (a("form.woocommerce-checkout").before(b), a(document.body).trigger("update_checkout", {update_shipping_method: !1}), a("form.checkout_coupon").find('input[name="coupon_code"]').val(""))
                },
                error: function (a) {
