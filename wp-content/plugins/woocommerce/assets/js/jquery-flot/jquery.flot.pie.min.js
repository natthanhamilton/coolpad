    ...
    function b(b) {
        ...
        function g(b) {
            for (var c = 0, d = 0, f = 0, g = e.series.pie.combine.color, h = [], i = 0; i < b.length; ++i) {
                var j = b[i].data;
                a.isArray(j) && 1 == j.length && (j = j[0]), a.isArray(j) ? !isNaN(parseFloat(j[1])) && isFinite(j[1]) ? j[1] = +j[1] : j[1] = 0 : j = !isNaN(parseFloat(j)) && isFinite(j) ? [1, +j] : [1, 0], b[i].data = [j]
            }
            ...
            for (var i = 0; i < b.length; ++i) {
                var j = b[i].data[0][1];
                (f < 2 || j / c > e.series.pie.combine.threshold) && h.push({
                    data: [[1, j]],
                    color: b[i].color,
                    label: b[i].label,
                    angle: j * Math.PI * 2 / c,
        ...
        function h(b, f) {
            ...
            function h() {
                ...
                if (!(f >= k / 2 - a || f * e.series.pie.tilt >= l / 2 - b || f <= c)) {
                    y.save(), y.translate(a, b), y.globalAlpha = d, y.fillStyle = "#000", y.translate(v, w), y.scale(1, e.series.pie.tilt);
                    for (var g = 1; g <= c; g++)y.beginPath(), y.arc(0, 0, f, 0, 2 * Math.PI, !1), y.fill(), f -= g;
                    y.restore()
                }
            }
            ...
            function j() {
                function b(a, b, c) {
                    a <= 0 || isNaN(a) || (c ? y.fillStyle = b : (y.strokeStyle = b, y.lineJoin = "round"), y.beginPath(), Math.abs(a - 2 * Math.PI) > 1e-9 && y.moveTo(0, 0), y.arc(0, 0, f, g, g + a / 2, !1), y.arc(0, 0, f, g + a / 2, g + a, !1), y.closePath(), g += a, c ? y.fill() : y.stroke())
                }
                ...
                return i(y), y.restore(), !e.series.pie.label.show || c()
            }

            if (t) {
                var k = b.getPlaceholder().width(), l = b.getPlaceholder().height(), m = t.children().filter(".legend").children().width() || 0;
                y = f, x = !1, u = Math.min(k, l / e.series.pie.tilt) / 2, w = l / 2 + e.series.pie.offset.top, v = k / 2, "auto" == e.series.pie.offset.left ? e.legend.position.match("w") ? v += m / 2 : v -= m / 2 : v += e.series.pie.offset.left, v < u ? v = u : v > k - u && (v = k - u);
                var n = b.getData(), o = 0;
                do o > 0 && (u *= d), o += 1, g(), e.series.pie.tilt <= .8 && h(); while (!j() && o < c);
        ...
        function o(a, c) {
            var d = q(a);
            d == -1 ? (z.push({series: a, auto: c}), b.triggerRedrawOverlay()) : c || (z[d].auto = !1)
        }
