...
class Cloudinary {
    ...
    public static  $JS_CONFIG_PARAMS = ["api_key", "cloud_name", "private_cdn", "secure_distribution", "cdn_subdomain"];
    private static $config           = NULL;

    public static function cloudinary_url($source, &$options = []) {
        ...
        $private_cdn         = Cloudinary::option_consume($options, "private_cdn",
                                                          Cloudinary::config_get("private_cdn"));
        $secure_distribution = Cloudinary::option_consume($options, "secure_distribution",
                                                          Cloudinary::config_get("secure_distribution"));
        $cdn_subdomain       = Cloudinary::option_consume($options, "cdn_subdomain",
        ...
        return preg_replace("/([^:])\/+/", "$1/", implode("/", [$prefix, $resource_type,
                                                                $type, $transformation, $version ? "v" . $version
                                                                    : "", $source]));
    }
    ...
    public static function generate_transformation_string(&$options = []) {
        ...
        $params
                       = ["w" => $width, "h" => $height, "t" => $named_transformation, "c" => $crop, "b" => $background, "e" => $effect, "bo" => $border, "a" => $angle, "fl" => $flags];
        $simple_params = ["x"  => "x", "y" => "y", "r" => "radius", "d" => "default_image", "g" => "gravity",
        ...
        $transformation       = implode(",",
                                        array_map($join_pair_underscore, array_keys($params), array_values($params)));
        $raw_transformation   = Cloudinary::option_consume($options, "raw_transformation");
        $transformation       = implode(",", array_filter([$transformation, $raw_transformation]));
    ...
    private static function smart_escape($str) {
        $revert = ['%21' => '!', '%2A' => '*', '%27' => "'", '%28' => '(', '%29' => ')', '%3A' => ':', '%2F' => '/'];

        return strtr(rawurlencode($str), $revert);
    }

    public static function cloudinary_api_url($action = 'upload', $options = []) {
        ...
        return implode("/", [$cloudinary, "v1_1", $cloud_name, $resource_type, $action]);
    }
    ...
    private static function generate_base_transformation($base_transformation) {
        return is_array($base_transformation)
            ...
            :
            Cloudinary::generate_transformation_string(["transformation" => $base_transformation]);
    }
...
function cl_image_tag($source, $options = []) {
...
function fetch_image_tag($url, $options = []) {
    $options["type"] = "fetch";

    return cl_image_tag($url, $options);
}

function facebook_profile_image_tag($profile, $options = []) {
    $options["type"] = "facebook";

    return cl_image_tag($profile, $options);
}

function gravatar_profile_image_tag($email, $options = []) {
...
function twitter_profile_image_tag($profile, $options = []) {
    $options["type"] = "twitter";

    return cl_image_tag($profile, $options);
}

function twitter_name_profile_image_tag($profile, $options = []) {
...
function cloudinary_url($source, $options = []) {
    return cloudinary_url_internal($source, $options);
}

function cloudinary_url_internal($source, &$options = []) {
