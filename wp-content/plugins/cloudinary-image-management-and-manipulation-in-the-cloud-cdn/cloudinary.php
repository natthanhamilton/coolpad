...
class CloudinaryPlugin {
  function CloudinaryPlugin() {
    ...
    add_action('admin_init', [$this, 'config']);
    add_action('admin_menu', [$this, 'cloudinary_options_page']);
    add_filter('manage_media_columns', [$this, 'media_lib_add_upload_column']);
    add_action('manage_media_custom_column', [$this, 'media_lib_upload_column_value'], 0, 2);
    add_action('admin_footer-upload.php', [$this, 'media_lib_upload_admin_footer']);
    add_action('load-upload.php', [$this, 'media_lib_upload_action']);
    add_action('admin_notices', [$this, 'media_lib_upload_notices']);
    add_action('media_buttons', [$this, 'media_cloudinary'], 11);
    add_action('wp_ajax_cloudinary_update_options', [$this, 'ajax_update_options']);
    add_filter('wp_get_attachment_url', [$this, 'fix_url'], 1, 2);
    add_filter('image_downsize', [$this, 'remote_resize'], 1, 3);
    add_action('wp_ajax_cloudinary_register_image', [$this, 'ajax_register_image']);
  }
  ...
  function ajax_register_image() {
    ...
    if (empty($_POST) || !check_admin_referer('cloudinary_register_image')) {
      $result = ["message" => 'Sorry, your nonce did not verify.', "error" => TRUE];
    } else {
      $post_id       = $_POST["post_id"];
      $attachment_id =& $_POST["attachment_id"];
      $url           = $_POST["url"];
      if (!empty($post_id) && !current_user_can('edit_post', $post_id)) {
        $result = ["message" => 'Permission denied.', "error" => TRUE];
      } else {
        if (!empty($attachment_id) && !current_user_can('edit_post', $attachment_id)) {
          $result = ["message" => 'Permission denied.', "error" => TRUE];
        } else {
          if (empty($url)) {
            $result = ["message" => 'Missing URL.', "error" => TRUE];
          } else {
            $id     = $this->register_image($url, $post_id, $attachment_id, NULL, $_POST["width"], $_POST["height"]);
            $result = ["success" => TRUE, "attachment_id" => $id];
          }
        }
      }
    }
  ...
  function register_image($url, $post_id, $attachment_id, $original_attachment, $width, $height) {
    ...
    $mime_types
               = ["png" => "image/png", "jpg" => "image/jpeg", "pdf" => "application/pdf", "gif" => "image/gif", "bmp" => "image/bmp"];
    ...
    if (!is_wp_error($id)) {
      $metadata = ["image_meta" => $meta, "width" => $width, "height" => $height, "cloudinary" => TRUE];
  ...
  function extract_metadata($remote_meta) {
    ...
    $meta['title'] = $this->extract_meta_value($remote_meta, ['Headline', 'ObjectName']);
    $caption       = $this->extract_meta_value($remote_meta, ['Caption-Abstract']);
    if (!empty($caption)) {
      if (empty($meta['title'])) {
    ...
    $meta['credit'] = $this->extract_meta_value($remote_meta, ['Artist', 'Author', 'Credit', 'By-line']);
    ...
    $meta['copyright'] = $this->extract_meta_value($remote_meta, ['Copyright', 'CopyrightNotice']);
  ...
  function build_resize_url($url, $metadata, $size) {
    if (preg_match('#(.*?)/(v[0-9]+/.*)$#', $url, $matches)) {
      if (!$size) {
        return [$url, $metadata["width"], $metadata["height"], TRUE];
      }
      if (is_array($size)) {
        $wanted = ["width" => $size[0], "height" => $size[1]];
        $crop   = FALSE;
      } else {
      ...
      if ($crop) {
        $resized = image_resize_dimensions($metadata['width'], $metadata['height'], $wanted['width'], $wanted['height'],
                                           TRUE);
        if ($resized) {
  ...
  function cloudinary_options_page() {
    ...
    add_menu_page('Cloudinary Menu', 'Cloudinary', 'manage_options', $main_action, NULL,
                  plugins_url(CLOUDINARY_UNIQUE_ID . '/images/favicon.png'));
    if ($this->configured()) {
  ...
  function ajax_update_options() {
    if (empty($_POST) || !check_admin_referer('cloudinary_update_options')) {
      ...
      if ($this->configured()) {
        ...
        $args
                  = ["method" => "GET", "timeout" => 5, "redirection" => 5, "httpversion" => "1.0", "blocking" => TRUE, "headers" => [],
                     "body"   => NULL, "cookies" => [], "sslverify" => FALSE];
  ...
  function prepare_cloudinary_media_lib_url($mode) {
    ...
    $params              = ["timestamp" => time(), "mode" => $mode, "plugin_version" => cloudinary_VERSION];
  ...
  function media_lib_upload_action() {
    ...
    if ($action === 'upload_cloudinary' || $action === 'migrate_away_cloudinary') {
      ...
      foreach ($post_ids as $k => $post_id) {
        $error = $action === 'upload_cloudinary' ? $this->upload_to_cloudinary($post_id, TRUE)
            : $this->migrate_away($post_id);
        if ($error) {
  ...
  function upload_to_cloudinary($attachment_id, $migrate) {
    ...
    try {
      $result = CloudinaryUploader::upload($full_path, ['use_filename' => TRUE]);
    } catch (Exception $e) {
      return $e->getMessage();
    }
    $post_parent = NULL;
    ...
    $this->register_image($result["url"], $post_parent, $attachment_id, $attachment, $result["width"],
                          $result["height"]);
    if ($migrate) {
      $errors = [];
  ...
  function update_image_src_all($attachment_id, $attachment_metadata, $old_url, $new_url, $migrate_in, &$errors) {
    $posts
        = get_posts(["post_type" => "any", 'numberposts' => -1, "post_status" => 'publish,pending,draft,auto-draft,future,private']);
  ...
  function update_image_src($post, $attachment_id, $attachment_metadata, $old_url, $new_url, $migrate_in, &$errors) {
    ...
    foreach ($images[0] as $img) {
      if (preg_match('~class *= *["\']([^"\']+)["\']~i', $img, $class) && preg_match('~wp-image-(\d+)~i', $class[1],
                                                                                     $id) && $id[1] == $attachment_id
      ) {
        $wanted_size = NULL;
        ...
        if (preg_match('~src *= *["\']([^"\']+)["\']~i', $img, $src)) {
          if ($migrate_in) {
            ...
            if ($old_img_src) {
              ...
              print_r([$old_img_src, $wanted_size, $new_img_src, $src[1]]);
              if (!$new_img_src) {
    ...
    if ($post_content != $post->post_content) {
      return wp_update_post(["post_content" => $post_content, "ID" => $post->ID]);
    }
  ...
  function return_to_media_lib($errors, $successes, $action, $sendback) {
    ...
    if (!empty($errors)) {
      ...
      $message
                            = "$message " . number_format_i18n($errors_count) . " failed because of the following $errors_count_message: ";
    ...
    $location = add_query_arg(["cloudinary_message" => urlencode($message)], $sendback);
  ...
  function migrate_away($attach_id) {
    ...
    $attrs
                = ['post_mime_type', 'post_title', 'post_content', 'post_status', 'post_author', 'post_name', 'post_date'];
  ...
  function media_lib_upload_notices() {
    ...
    if ($pagenow == 'upload.php' && $post_type == 'attachment' && Cloudinary::option_get($_REQUEST,
                                                                                         'cloudinary_message')
    ) {
