...
class CloudinaryUploader {
    private static $TEXT_PARAMS = ["public_id", "font_family", "font_size", "font_color", "text_align", "font_weight", "font_style", "background", "opacity", "text_decoration"];
    ...
    public static function build_upload_params(&$options) {
        $params = ["timestamp"      => time(),
                   ...
                   "headers"        => CloudinaryUploader::build_custom_headers(Cloudinary::option_get($options,
                                                                                                       "headers")),
    ...
    protected static function build_eager($transformations) {
        ...
        foreach (Cloudinary::build_array($transformations) as $trans) {
            ...
            $single_eager   = implode("/",
                                      array_filter([Cloudinary::generate_transformation_string($transformation), $format]));
    ...
    public static function call_api($action, $params, $options = [], $file = NULL) {
    ...
    public static function explicit($public_id, $options = []) {
        $params = [
            "timestamp" => time(),
            "public_id" => $public_id,
            "type"      => Cloudinary::option_get($options, "type"),
    ...
    public static function add_tag($tag, $public_ids = [], $options = []) {
    ...
    public static function call_tags_api($tag, $command, $public_ids = [], &$options = []) {
        $params = [
            "timestamp"  => time(),
            "tag"        => $tag,
    ...
    private static function join_pair_colon($key, $value) {
        return $key . ": " . $value;
    }
}

function cl_image_upload_tag($field, $options = []) {
    $html_options = Cloudinary::option_get($options, "html", []);
    ...
    # Remove blank parameters
    $params = array_filter($params);
    $tag_options = array_merge($html_options, ["type"                  => "file", "name" => "file",
                                               "data-url"              => $cloudinary_upload_url,
                                               "data-form-data"        => json_encode($params),
                                               "data-cloudinary-field" => $field,
                                               "class"                 => implode(" ",
                                                                                  [$html_options["class"], "cloudinary-fileupload"])
    ]);

    return '<input ' . Cloudinary::html_attrs($tag_options) . '/>';
}

function cl_form_tag($callback_url, $options = []) {
    $form_options = Cloudinary::option_get($options, "form", []);
    ...
    $form
        = "<form enctype='multipart/form-data' action='" . $api_url . "' method='POST' " . Cloudinary::html_attrs($form_options) . ">\n";
    foreach ($params as $key => $value) {
        $form .= "<input " . Cloudinary::html_attrs(["name" => $key, "value" => $value, "type" => "hidden"]) . "/>\n";
    }
    $form .= "</form>\n";
