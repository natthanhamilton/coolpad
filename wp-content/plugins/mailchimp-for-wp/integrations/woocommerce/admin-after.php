...
<table class="form-table">
	<?php $config = ['element' => 'mc4wp_integrations[' . $integration->slug . '][implicit]', 'value' => '0']; ?>
		...
		<td>
			<select name="mc4wp_integrations[<?php echo $integration->slug; ?>][position]">
				...
				foreach ($position_options as $value => $label) {
					printf('<option value="%s" %s>%s</option>', esc_attr($value),
					       selected($value, $opts['position'], FALSE), esc_html($label));
				}
				?>
