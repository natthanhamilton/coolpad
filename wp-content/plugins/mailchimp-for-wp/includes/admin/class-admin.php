...
class MC4WP_Admin {
	...
	public function __construct(MC4WP_Admin_Messages $messages, MC4WP_MailChimp $mailchimp) {
		...
		// update opt-in
		$this->update_optin = new MC4WP_Update_Optin('4.0.0', $this->plugin_file,
		                                             MC4WP_PLUGIN_DIR . 'includes/views/parts/update-4.x-notice.php');
	}
	...
	public function renew_lists_cache() {
		...
		if (!empty($lists)) {
			$this->messages->flash(__('Success! The cached configuration for your MailChimp lists has been renewed.',
			                          'mailchimp-for-wp'));
		}
	}
	...
	public function enqueue_assets() {
		...
		// css
		wp_register_style('mc4wp-admin', MC4WP_PLUGIN_URL . 'assets/css/admin-styles' . $suffix . '.css', [],
		                  MC4WP_VERSION);
		wp_enqueue_style('mc4wp-admin');
		// js
		wp_register_script('es5-shim', MC4WP_PLUGIN_URL . 'assets/js/third-party/es5-shim.min.js', [], MC4WP_VERSION);
		$wp_scripts->add_data('es5-shim', 'conditional', 'lt IE 9');
		// @todo: eventually get rid of jQuery here
		wp_register_script('mc4wp-admin', MC4WP_PLUGIN_URL . 'assets/js/admin' . $suffix . '.js',
		                   ['jquery', 'es5-shim'], MC4WP_VERSION, TRUE);
		wp_enqueue_script(['jquery', 'es5-shim', 'mc4wp-admin']);
		wp_localize_script('mc4wp-admin', 'mc4wp_vars',
		                   [
			                   ...
			                   'l10n'      => [
				                   'pro_only' => __('This is a pro-only feature. Please upgrade to the premium version to be able to use it.',
				                                    'mailchimp-for-wp')
			                   ]
		                   ]
		);
	...
	public function build_menu() {
		...
		// add top menu item
		add_menu_page('MailChimp for WP', 'MailChimp for WP', $required_cap, 'mailchimp-for-wp',
		              [$this, 'show_generals_setting_page'], MC4WP_PLUGIN_URL . 'assets/img/icon.png', '99.68491');
		// sort submenu items by 'position'
		uasort($menu_items, [$this, 'sort_menu_items_by_position']);
		// add sub-menu items
		array_walk($menu_items, [$this, 'add_menu_item']);
	}
	...
	public function add_menu_item(array $item) {
		...
		// register page
		$hook = add_submenu_page($parent_slug, $item['title'] . ' - MailChimp for WordPress', $item['text'],
		                         $capability, $slug, $item['callback']);
	...
	public function show_api_key_notice() {
		...
		echo '<p>' . sprintf(__('To get started with MailChimp for WordPress, please <a href="%s">enter your MailChimp API key on the settings page of the plugin</a>.',
		                        'mailchimp-for-wp'), admin_url('admin.php?page=mailchimp-for-wp')) . '</p>';
