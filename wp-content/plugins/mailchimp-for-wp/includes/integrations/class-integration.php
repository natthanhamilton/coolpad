...
abstract class MC4WP_Integration {
	...
	public function get_checkbox_html() {
		...
		<p class="mc4wp-checkbox mc4wp-checkbox-<?php echo esc_attr($this->slug); ?>">
			<label>
				...
				<input type="checkbox" name="<?php echo esc_attr($this->checkbox_name); ?>"
				       value="1" <?php echo $this->get_checkbox_attributes(); ?> />
	...
	protected function get_checkbox_attributes() {
		 ...
		 * @param MC4WP_Integration $integration
		 */
		$attributes = (array)apply_filters('mc4wp_integration_' . $slug . '_checkbox_attributes', $attributes,
		                                   $integration);
		$string = '';
	 ...
	 */
	protected function subscribe($email, array $merge_vars = [], $related_object_id = 0) {
		$integration = $this;
		$slug        = $this->slug;
		/**
		 * @var MC4WP_API $api
		...
		foreach ($map->list_fields as $list_id => $list_field_data) {
			$result = $api->subscribe($list_id, $email, $list_field_data, $email_type, $this->options['double_optin'],
			                          $this->options['update_existing'], $this->options['replace_interests'],
			                          $this->options['send_welcome']);
		}
		// if result failed, show error message
		if (!$result) {
			// log error
			if ($api->get_error_code() === 214) {
				$this->get_log()->warning(sprintf("%s > %s is already subscribed to the selected list(s)", $this->name,
				                                  mc4wp_obfuscate_string($email)));
			} else {
				$this->get_log()->error(sprintf('%s > MailChimp API Error: %s', $this->name,
