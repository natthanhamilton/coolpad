...
class MC4WP_Integration_Admin {
	...
	public function enqueue_assets($suffix, $page = '') {
		...
		wp_register_script('mc4wp-integrations-admin',
		                   MC4WP_PLUGIN_URL . 'assets/js/integrations-admin' . $suffix . '.js', ['mc4wp-admin'],
		                   MC4WP_VERSION, TRUE);
	...
	public function show_integration_settings_page($slug) {
		try {
			...
			echo sprintf('<h3>Integration not found.</h3><p>No integration with slug <strong>%s</strong> was found.</p>',
			             $slug);
