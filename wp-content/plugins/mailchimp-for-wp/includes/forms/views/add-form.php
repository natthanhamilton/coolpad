			...
			<div style="max-width: 480px;">
				...
				<form method="post">
					...
					<div class="small-margin">
						...
						<input type="text" name="mc4wp_form[name]" class="widefat" value="" spellcheck="true"
						       autocomplete="off"
						       placeholder="<?php _e('Enter your form title..', 'mailchimp-for-wp'); ?>">
					</div>
					...
					<div class="small-margin">
							...
							<ul id="mc4wp-lists">
								<?php foreach ($lists as $list) { ?>
									<li>
										<label>
											<input type="checkbox"
											       name="mc4wp_form[settings][lists][<?php echo esc_attr($list->id); ?>]"
											       value="<?php echo esc_attr($list->id); ?>" <?php selected($number_of_lists,
											                                                                 1); ?> >
						...
						<?php } else { ?>
							<p class="mc4wp-notice">
								<?php printf(__('No lists found. Did you <a href="%s">connect with MailChimp</a>?',
								                'mailchimp-for-wp'), admin_url('admin.php?page=mailchimp-for-wp')); ?>
							</p>
						<?php } ?>
