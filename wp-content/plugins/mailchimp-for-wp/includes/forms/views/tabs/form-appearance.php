...
$css_options = [
	...
	__('Form Themes', 'mailchimp-for-wp') => [
...
?>
	...
	<table class="form-table">
		<tr valign="top">
			<th scope="row"><label for="mc4wp_load_stylesheet_select"><?php _e('Form Style',
			...
			<td class="nowrap valigntop">
					...
					<?php foreach ($css_options as $key => $option) {
						if (is_array($option)) {
							...
							foreach ($options as $key => $option) {
								printf('<option value="%s" %s>%s</option>', $key, selected($opts['css'], $key, FALSE),
								       $option);
							}
							print('</optgroup>');
						} else {
				...
				<p class="help">
					<?php _e('If you want to load some default CSS styles, select "basic formatting styles" or choose one of the color themes',
					         'mailchimp-for-wp'); ?>
				</p>
			</td>
		</tr>
