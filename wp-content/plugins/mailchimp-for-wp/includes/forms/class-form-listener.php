...
class MC4WP_Form_Listener {
	...
	public function listen(MC4WP_Request $request) {
		...
		if (!$form->has_errors()) {
			...
			call_user_func([$this, $method], $form);
		} else {
			$this->get_log()->info(sprintf("Form %d > Submitted with errors: %s", $form->ID,
			                               join(', ', $form->errors)));
		}
		$this->respond($form);
	...
	public function process_subscribe_form(MC4WP_Form $form) {
		...
		foreach ($map->list_fields as $list_id => $merge_vars) {
			...
			$result = $api->subscribe($list_id, $email, $merge_vars, $email_type, $form->settings['double_optin'],
			                          $form->settings['update_existing'], $form->settings['replace_interests'],
			                          $form->settings['send_welcome']);
		}
		// do stuff on failure
		if (!$result) {
			...
			if ($api->get_error_code() == $error_code_unsubscribed || $api->get_error_code() == $error_code_bounced) {
				...
				$this->get_log()
				     ->warning(sprintf('Form %d > %s has unsubscribed before and cannot be resubscribed by the plugin.',
				                       $form->ID, $form->data['EMAIL']));
			} elseif ($api->get_error_code() == 214) {
				// handle "already_subscribed" as a soft-error
				$form->errors[] = 'already_subscribed';
				$this->get_log()->warning(sprintf("Form %d > %s is already subscribed to the selected list(s)",
				                                  $form->ID, mc4wp_obfuscate_string($form->data['EMAIL'])));
			} else {
				// log error
				$this->get_log()->error(sprintf('Form %d > MailChimp API error: %s %s', $form->ID,
				                                $api->get_error_code(), $api->get_error_message()));
	...
	public function process_unsubscribe_form(MC4WP_Form $form) {
		...
		if (!$result) {
			...
			if (in_array($api->get_error_code(), [215, 232])) {
				$form->add_error('not_subscribed');
				$this->get_log()->info(sprintf('Form %d > %s is not subscribed to the selected list(s)', $form->ID,
				                               $form->data['EMAIL']));
			} else {
				$form->add_error('error');
				$this->get_log()->error(sprintf('Form %d > MailChimp API error: %s', $form->ID,
