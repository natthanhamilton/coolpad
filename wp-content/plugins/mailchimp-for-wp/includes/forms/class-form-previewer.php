...
class MC4WP_Form_Previewer {
	...
	public static function init() {
		...
		add_filter('mc4wp_form_stylesheets', [$instance, 'set_stylesheet']);
		add_filter('mc4wp_form_css_classes', [$instance, 'add_css_class'], 10, 2);
		add_filter('the_title', [$instance, 'set_page_title']);
		add_filter('the_content', [$instance, 'set_page_content']);
	}
	...
	public function get_preview_page_id() {
		...
		if ($page instanceof WP_Post && in_array($page->post_status, ['draft', 'publish'])) {
			$page_id = $page->ID;
		} else {
	...
	public function set_stylesheet($stylesheets) {
		return [$this->form->get_stylesheet()];
	}
