...
class MC4WP_Form_Element {
	 ...
	 * @param            $config array
	 */
	public function __construct(MC4WP_Form $form, $ID, $config = []) {
		$this->form   = $form;
		$this->ID     = $ID;
		$this->config = $config;
	...
	public function generate_html(array $config = NULL) {
		...
		$opening_html
			= '<!-- MailChimp for WordPress v' . MC4WP_VERSION . ' - https://wordpress.org/plugins/mailchimp-for-wp/ -->';
	...
	protected function get_message_html($key) {
		...
		$html    = sprintf('<div class="mc4wp-alert mc4wp-%s"><p>%s</p></div>', esc_attr($message->type),
		                   $message->text);
	...
	protected function get_hidden_fields() {
		...
		if (!empty($this->config['lists'])) {
			$lists_string = is_array($this->config['lists']) ? join(',', $this->config['lists'])
				: $this->config['lists'];
