...
class MC4WP_Forms_Admin {
	...
	public function add_hooks() {
		...
		add_action('mc4wp_admin_preview_form', [$this, 'prepare_form_preview']);
	...
	public function enqueue_assets($suffix, $page = '') {
		...
		wp_register_script('mc4wp-forms-admin', MC4WP_PLUGIN_URL . 'assets/js/forms-admin' . $suffix . '.js',
		                   ['mc4wp-admin'], MC4WP_VERSION, TRUE);
		wp_enqueue_script('mc4wp-forms-admin');
		wp_localize_script('mc4wp-forms-admin', 'mc4wp_forms_i18n', [
			...
			'formActionDescription' => __('This field will allow your visitors to choose whether they would like to subscribe or unsubscribe',
			                              'mailchimp-for-wp'),
			...
			'listChoiceDescription' => __('This field will allow your visitors to choose a list to subscribe to.',
			                              'mailchimp-for-wp'),
			'min'                   => __('Min', 'mailchimp-for-wp'),
			'max'                   => __('Max', 'mailchimp-for-wp'),
			'noAvailableFields'     => __('No available fields. Did you select a MailChimp list in the form settings?',
			                              'mailchimp-for-wp'),
	...
	public function process_save_form() {
		...
		$this->messages->flash(__("<strong>Success!</strong> Form successfully saved.",
		                          'mailchimp-for-wp') . sprintf(' <a href="%s">',
		                                                        $previewer->get_preview_url()) . __('Preview form',
		                                                                                            'mailchimp-for-wp') . '</a>');
	}
	...
	public function redirect_to_form_action() {
		...
		$forms = mc4wp_get_forms(['numberposts' => 1]);
		if ($forms) {
	...
	public function register_shortcake_ui() {
		...
		shortcode_ui_register_for_shortcode('mc4wp_form', [
			                                                'label'         => esc_html__('MailChimp Sign-Up Form',
			                                                                              'mailchimp-for-wp'),
			                                                'listItemImage' => 'dashicons-feedback',
			                                                'attrs'         => [
				                                                [
					                                                'label'   => esc_html__('Select the form to show',
					                                                                        'mailchimp-for-wp'),
					                                                'attr'    => 'id',
					                                                'type'    => 'select',
					                                                'options' => $options
