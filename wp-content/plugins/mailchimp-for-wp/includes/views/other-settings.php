	...
	<div class="medium-margin">
		...
		<p>
			<label>
				...
				<input type="checkbox" name="mc4wp[allow_usage_tracking]"
				       value="1" <?php checked($opts['allow_usage_tracking'], 1); ?>>
				<?php echo __('Allow us to anonymously track how this plugin is used to help us make it better fit your needs.',
				              'mailchimp-for-wp'); ?>
				<a href="https://mc4wp.com/kb/what-is-usage-tracking/#utm_source=wp-plugin&utm_medium=mailchimp-for-wp&utm_campaign=settings-page"
				   target="_blank">
		...
		<div class="main-content col col-4">
			...
			<div class="medium-margin">
				...
				<style scoped type="text/css">
					...
					.rtl #debug-log-filter {
						float: left;
					}
				</style>

				<h3><?php _e('Debug Log', 'mailchimp-for-wp'); ?> <input type="text" id="debug-log-filter"
				                                                         class="regular-text"
				                                                         placeholder="<?php esc_attr_e('Filter..',
				                                                                                       'mailchimp-for-wp'); ?>"/>
				</h3>
				...
				if (!$log->test()) {
					...
					echo sprintf(__('Please ensure %s has the proper <a href="%s">file permissions</a>.',
					                'mailchimp-for-wp'), '<code>' . $log->file . '</code>',
				...
				if ($log->level >= 300) {
					echo '<p>';
					echo __('Right now, the plugin is configured to only log errors and warnings.',
					        'mailchimp-for-wp') . ' ';
					echo sprintf(__('Would you like to <a href="%s">log all events</a> instead?', 'mailchimp-for-wp'),
					             'https://mc4wp.com/kb/how-to-enable-log-debugging/');
					echo '</p>';
				}
				?>
