...
function wpcf7_editor_panel_form($post) {
	...
	<textarea id="wpcf7-form" name="wpcf7-form" cols="100" rows="24"
...
function wpcf7_editor_box_mail($post, $args = '') {
	$args = wp_parse_args($args, [
		...
		'use'   => NULL]);
	$id = esc_attr($args['id']);
	$mail = wp_parse_args($post->prop($args['name']), [
	...
	<div class="contact-form-editor-box-mail" id="<?php echo $id; ?>">
		...
		if (!empty($args['use'])) :
			?>
			<label for="<?php echo $id; ?>-active"><input type="checkbox" id="<?php echo $id; ?>-active"
			                                              name="<?php echo $id; ?>-active" class="toggle-form-table"
			                                              value="1"<?php echo ($mail['active']) ? ' checked="checked"'
					: ''; ?> /> <?php echo esc_html($args['use']); ?></label>
			<p class="description"><?php echo esc_html(__("Mail (2) is an additional mail template often used as an autoresponder.",
			                                              'contact-form-7')); ?></p>
			<?php
		endif;
		?>
		...
		<fieldset>
			<legend><?php echo esc_html(__("In the following fields, you can use these mail-tags:",
			                               'contact-form-7')); ?><br/>
			...
			<table class="form-table">
				...
				<tr>
					...
					<td>
						...
						<input type="text" id="<?php echo $id; ?>-recipient" name="<?php echo $id; ?>-recipient"
						       class="large-text code" size="70"
						       value="<?php echo esc_attr($mail['recipient']); ?>"<?php if ($do_validate && $config_error) {
				...
				<tr>
					...
					<td>
						...
						<input type="text" id="<?php echo $id; ?>-sender" name="<?php echo $id; ?>-sender"
						       class="large-text code" size="70"
						       value="<?php echo esc_attr($mail['sender']); ?>"<?php if ($do_validate && $config_error) {
							echo ' aria-invalid="true"';
						} ?> />
						<?php if ($do_validate && $config_error) {
							echo sprintf('<br /><span role="alert" class="config-error">%s</span>', $config_error);
				...
				<tr>
					...
					<td>
						...
						<input type="text" id="<?php echo $id; ?>-subject" name="<?php echo $id; ?>-subject"
						       class="large-text code" size="70"
						       value="<?php echo esc_attr($mail['subject']); ?>"<?php if ($do_validate && $config_error) {
							echo ' aria-invalid="true"';
						} ?> />
						<?php if ($do_validate && $config_error) {
							echo sprintf('<br /><span role="alert" class="config-error">%s</span>', $config_error);
				...
				<tr>
					...
					<td>
						...
						<textarea id="<?php echo $id; ?>-additional-headers"
						          name="<?php echo $id; ?>-additional-headers" cols="100" rows="4"
						          class="large-text code"<?php if ($do_validate && $config_error) {
							echo ' aria-invalid="true"';
				...
				<tr>
					...
					<td>
						...
						<textarea id="<?php echo $id; ?>-body" name="<?php echo $id; ?>-body" cols="100" rows="18"
						          class="large-text code"<?php if ($do_validate && $config_error) {
							echo ' aria-invalid="true"';
						...
						<p><label for="<?php echo $id; ?>-exclude-blank"><input type="checkbox"
						                                                        id="<?php echo $id; ?>-exclude-blank"
						                                                        name="<?php echo $id; ?>-exclude-blank"
						                                                        value="1"<?php echo (!empty($mail['exclude_blank']))
									? ' checked="checked"'
									: ''; ?> /> <?php echo esc_html(__('Exclude lines with blank mail-tags from output',
						                                               'contact-form-7')); ?></label></p>

						<p><label for="<?php echo $id; ?>-use-html"><input type="checkbox"
						                                                   id="<?php echo $id; ?>-use-html"
						                                                   name="<?php echo $id; ?>-use-html"
						                                                   value="1"<?php echo ($mail['use_html'])
									? ' checked="checked"' : ''; ?> /> <?php echo esc_html(__('Use HTML content type',
						                                                                      'contact-form-7')); ?>
							</label></p>
					</td>
				</tr>
				...
				<tr>
					...
					<td>
						<textarea id="<?php echo $id; ?>-attachments" name="<?php echo $id; ?>-attachments" cols="100"
						          rows="4"
						          class="large-text code"><?php echo esc_textarea($mail['attachments']); ?></textarea>
					</td>
				</tr>
				</tbody>
			</table>
...
function wpcf7_editor_panel_messages($post) {
	...
	<fieldset>
		<legend><?php echo esc_html(__('Edit messages used in the following situations.',
			...
			<p class="description">
				<label for="<?php echo $field_name; ?>"><?php echo esc_html($arr['description']); ?><br/>
					<input type="text" id="<?php echo $field_name; ?>" name="<?php echo $field_name; ?>"
					       ...
					       value="<?php echo esc_attr($post->message($key, FALSE)); ?>"<?php echo $config_error
						? ' aria-invalid="true"' : ''; ?> />
					<?php
					if ($config_error) {
						echo sprintf('<br /><span role="alert" class="config-error">%s</span>', $config_error);
...
function wpcf7_editor_panel_additional_settings($post) {
	...
	<fieldset>
		...
		<textarea id="wpcf7-additional-settings" name="wpcf7-additional-settings" cols="100" rows="8"
		          class="large-text"><?php echo esc_textarea($post->prop('additional_settings')); ?></textarea>
	</fieldset>
	<?php
}
