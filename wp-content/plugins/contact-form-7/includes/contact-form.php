...
class WPCF7_ContactForm {
	...
	public function __get($name) {
		$message
			= __('<code>%1$s</code> property of a <code>WPCF7_ContactForm</code> object is <strong>no longer accessible</strong>. Use <code>%2$s</code> method instead.',
			     'contact-form-7');
		if ('id' == $name) {
			if (WP_DEBUG) {
	...
	public function form_html($args = '') {
		...
		$html           = sprintf('<div %s>', wpcf7_format_atts([
			                                                        'role'  => 'form',
			                                                        'class' => 'wpcf7',
			                                                        'id'    => $this->unit_tag,
			                                                        (get_option('html_type') == 'text/html') ? 'lang'
				                                                        : 'xml:lang'
...
function wpcf7_form_controls_class($type, $default = '') {
	...
	$classes   = array_merge(['wpcf7-form-control'], $default);
	$typebase  = rtrim($type, '*');
