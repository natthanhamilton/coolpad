...
function wpcf7_quiz_ajax_refill($items) {
	...
	$fes = wpcf7_scan_shortcode(['type' => 'quiz']);
	if (empty($fes)) {
		return $items;
	}
	$refill = [];
	foreach ($fes as $fe) {
		...
		$answer = wpcf7_canonicalize($answer);
		$refill[ $name ] = [$question, wp_hash($answer, 'wpcf7_quiz')];
	}
	if (!empty($refill)) {
		$items['quiz'] = $refill;
	}
...
function wpcf7_quiz_messages($messages) {
	return array_merge($messages, ['quiz_answer_not_correct' => [
		...
		<fieldset>
			...
			<table class="form-table">
				...
				<tr>
					...
					<td>
						<fieldset>
							<legend class="screen-reader-text"><?php echo esc_html(__('Questions and answers',
							                                                          'contact-form-7')); ?></legend>
							<textarea name="values" class="values"
							          id="<?php echo esc_attr($args['content'] . '-values'); ?>"></textarea><br/>
							<label for="<?php echo esc_attr($args['content'] . '-values'); ?>"><span
									class="description"><?php echo esc_html(__("One pipe-separated question-answer pair (e.g. The capital of Brazil?|Rio) per line.",
