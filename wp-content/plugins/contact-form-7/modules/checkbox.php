...
function wpcf7_add_shortcode_checkbox() {
    wpcf7_add_shortcode(['checkbox', 'checkbox*', 'radio'],
    ...
    foreach ($tags as $tag) {
        ...
        if ($tag->has_option('free_text')) {
            ...
            if (in_array($last, $posted_items)) {
                $posted_items = array_diff($posted_items, [$last]);
                $free_text_name = sprintf(
                    '_wpcf7_%1$s_free_text_%2$s', $tag->basetype, $tag->name);
...
function wpcf7_tag_generator_checkbox($contact_form, $args = '') {
    ...
    if ('checkbox' == $type) {
        ...
        $description = __("Generate a form-tag for a group of radio buttons. For more details, see %s.",
                          'contact-form-7');
    }
    $desc_link = wpcf7_link(__('http://contactform7.com/checkboxes-radio-buttons-and-menus/', 'contact-form-7'),
                            __('Checkboxes, Radio Buttons and Menus', 'contact-form-7'));
    ?>
            ...
            <table class="form-table">
                ...
                <?php if ('checkbox' == $type) : ?>
                    <tr>
                        ...
                        <td>
                            <fieldset>
                                <legend
                                    class="screen-reader-text"><?php echo esc_html(__('Field type',
                                                                                      'contact-form-7')); ?></legend>
                                <label><input type="checkbox"
                                              name="required"/> <?php echo esc_html(__('Required field',
                                                                                       'contact-form-7')); ?>
                                </label>
                            </fieldset>
                        </td>
                ...
                <tr>
                    ...
                    <td>
                        <fieldset>
                            <legend
                                class="screen-reader-text"><?php echo esc_html(__('Options',
                                                                                  'contact-form-7')); ?></legend>
                            ...
                            <label for="<?php echo esc_attr($args['content'] . '-values'); ?>"><span
                                    class="description"><?php echo esc_html(__("One option per line.",
                                                                               'contact-form-7')); ?></span></label><br/>
                            <label><input type="checkbox" name="label_first"
                                          class="option"/> <?php echo esc_html(__('Put a label first, a checkbox last',
                                                                                  'contact-form-7')); ?>
                            </label><br/>
                            <label><input type="checkbox" name="use_label_element"
                                          class="option"/> <?php echo esc_html(__('Wrap each item with label element',
                                                                                  'contact-form-7')); ?>
                            </label>
                            <?php if ('checkbox' == $type) : ?>
                                <br/><label><input type="checkbox" name="exclusive"
                                                   class="option"/> <?php echo esc_html(__('Make checkboxes exclusive',
                                                                                           'contact-form-7')); ?>
                                </label>
                            <?php endif; ?>
                        </fieldset>
