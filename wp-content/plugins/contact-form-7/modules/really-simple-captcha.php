...
function wpcf7_add_shortcode_captcha() {
    wpcf7_add_shortcode(['captchac', 'captchar'],
...
function wpcf7_captcha_ajax_refill($items) {
    ...
    $fes = wpcf7_scan_shortcode(['type' => 'captchac']);
    if (empty($fes)) {
        return $items;
    }
    $refill = [];
...
function wpcf7_captcha_messages($messages) {
    return array_merge($messages, ['captcha_not_match' => [
...
function wpcf7_tag_generator_captcha($contact_form, $args = '') {
    ...
    if (!class_exists('ReallySimpleCaptcha')) {
        ...
        <div class="control-box">
            <fieldset>
                <legend><?php echo sprintf(esc_html(__("To use CAPTCHA, you first need to install and activate %s plugin.",
                                                       'contact-form-7')),
                                           wpcf7_link('http://wordpress.org/extend/plugins/really-simple-captcha/',
                                                      'Really Simple CAPTCHA')); ?></legend>
            </fieldset>
        </div>
        <?php
        return;
    ...
    $description
        = __("Generate form-tags for a CAPTCHA image and corresponding response input field. For more details, see %s.",
             'contact-form-7');
    ...
    <div class="control-box">
        <fieldset>
            ...
            <table class="form-table">
                ...
                <tr>
                    <th scope="row"><label
                            for="<?php echo esc_attr($args['content'] . '-name'); ?>"><?php echo esc_html(__('Name',
            ...
            <table class="form-table scope captchac">
                ...
                <tr>
                    <th scope="row"><label
                            for="<?php echo esc_attr($args['content'] . '-captchac-id'); ?>"><?php echo esc_html(__('Id attribute',
                                                                                                                    'contact-form-7')); ?></label>
                    </th>
                    <td><input type="text" name="id" class="idvalue oneline option"
                               id="<?php echo esc_attr($args['content'] . '-captchac-id'); ?>"/></td>
                </tr>

                <tr>
                    <th scope="row"><label
                            for="<?php echo esc_attr($args['content'] . '-captchac-class'); ?>"><?php echo esc_html(__('Class attribute',
                                                                                                                       'contact-form-7')); ?></label>
                    </th>
                    <td><input type="text" name="class" class="classvalue oneline option"
                               id="<?php echo esc_attr($args['content'] . '-captchac-class'); ?>"/></td>
                </tr>
                </tbody>
            </table>
            ...
            <table class="form-table scope captchar">
                ...
                <tr>
                    <th scope="row"><label
                            for="<?php echo esc_attr($args['content'] . '-captchar-id'); ?>"><?php echo esc_html(__('Id attribute',
                                                                                                                    'contact-form-7')); ?></label>
                    </th>
                    <td><input type="text" name="id" class="idvalue oneline option"
                               id="<?php echo esc_attr($args['content'] . '-captchar-id'); ?>"/></td>
                </tr>

                <tr>
                    <th scope="row"><label
                            for="<?php echo esc_attr($args['content'] . '-captchar-class'); ?>"><?php echo esc_html(__('Class attribute',
                                                                                                                       'contact-form-7')); ?></label>
                    </th>
                    <td><input type="text" name="class" class="classvalue oneline option"
                               id="<?php echo esc_attr($args['content'] . '-captchar-class'); ?>"/></td>
                </tr>
                </tbody>
            </table>
        </fieldset>
...
function wpcf7_captcha_display_warning_message() {
    ...
    if (!is_dir($uploads_dir) || !wp_is_writable($uploads_dir)) {
        $message
            = sprintf(__('This contact form contains CAPTCHA fields, but the temporary folder for the files (%s) does not exist or is not writable. You can create the folder or change its permission manually.',
                         'contact-form-7'), $uploads_dir);
        echo '<div class="notice notice-error is-dismissible"><p>' . esc_html($message) . '</p></div>';
    }
    if (!function_exists('imagecreatetruecolor') || !function_exists('imagettftext')) {
        $message
            = __('This contact form contains CAPTCHA fields, but the necessary libraries (GD and FreeType) are not available on your server.',
                 'contact-form-7');
        echo '<div class="notice notice-error is-dismissible"><p>' . esc_html($message) . '</p></div>';
    }
}
...
function wpcf7_init_captcha() {
    ...
    $captcha->tmp_dir = $dir;
    if (is_callable([$captcha, 'make_tmp_dir'])) {
...
function wpcf7_cleanup_captcha_files() {
    ...
    if (is_callable([$captcha, 'cleanup'])) {
