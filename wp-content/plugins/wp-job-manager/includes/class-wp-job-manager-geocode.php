...
class WP_Job_Manager_Geocode {
	...
	public static function get_location_data($raw_address) {
		$invalid_chars = [" " => "+", "," => "", "?" => "", "&" => "", "=" => "", "#" => ""];
		$raw_address   = trim(strtolower(str_replace(array_keys($invalid_chars), array_values($invalid_chars),
		                                             $raw_address)));
		...
		try {
			if (FALSE === $geocoded_address || empty($geocoded_address->results[0])) {
				$result           = wp_remote_get(
					apply_filters('job_manager_geolocation_endpoint',
					              "http://maps.googleapis.com/maps/api/geocode/json?address=" . $raw_address . "&sensor=false&region=" . apply_filters('job_manager_geolocation_region_cctld',
					                                                                                                                                   '',
					                                                                                                                                   $raw_address),
					              $raw_address),
					[
