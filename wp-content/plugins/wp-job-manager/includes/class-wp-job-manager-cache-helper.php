...
class WP_Job_Manager_Cache_Helper {
    public static function init() {
        ...
        add_action('job_manager_clear_expired_transients', [__CLASS__, 'clear_expired_transients'], 10);
    }
    ...
    public static function clear_expired_transients() {
        ...
        if (!wp_using_ext_object_cache() && !defined('WP_SETUP_CONFIG') && !defined('WP_INSTALLING')) {
            ...
            $wpdb->query($wpdb->prepare($sql, $wpdb->esc_like('_transient_jm_') . '%',
                                        $wpdb->esc_like('_transient_timeout_jm_') . '%', time()));
        }
    }
}
