...
abstract class WP_Job_Manager_Form {
        ...
        foreach ($this->fields as $group_key => $group_fields) {
            foreach ($group_fields as $key => $field) {
                ...
                if ($handler = apply_filters("job_manager_get_posted_{$field_type}_field", FALSE)) {
                    ...
                    $values[ $group_key ][ $key ] = call_user_func([$this, "get_posted_{$field_type}_field"], $key,
                                                                   $field);
                } else {
    ...
    protected function sanitize_posted_field($value) {
        // Santize value
        $value = is_array($value) ? array_map([$this, 'sanitize_posted_field'], $value)
            : sanitize_text_field(stripslashes(trim($value)));
    ...
    protected function get_posted_multiselect_field($key, $field) {
        return isset($_POST[ $key ]) ? array_map('sanitize_text_field', $_POST[ $key ]) : [];
    }
    ...
    protected function upload_file($field_key, $field) {
        if (isset($_FILES[ $field_key ]) && !empty($_FILES[ $field_key ]) && !empty($_FILES[ $field_key ]['name'])) {
            ...
            foreach ($files_to_upload as $file_to_upload) {
                $uploaded_file = job_manager_upload_file($file_to_upload, ['file_key' => $field_key]);
