...
class WP_Job_Manager_Post_Types {
    ...
    public function __construct() {
        ...
        // WP ALL Import
        add_action('pmxi_saved_post', [$this, 'pmxi_saved_post'], 10, 1);
        // RP4WP
        add_filter('rp4wp_get_template', [$this, 'rp4wp_template'], 10, 3);
        add_filter('rp4wp_related_meta_fields', [$this, 'rp4wp_related_meta_fields'], 10, 3);
        add_filter('rp4wp_related_meta_fields_weight', [$this, 'rp4wp_related_meta_fields_weight'], 10, 3);
        // Single job content
    ...
    public function register_post_types() {
        ...
        if (get_option('job_manager_enable_categories')) {
            ...
            if (current_theme_supports('job-manager-templates')) {
                $rewrite = [
                    'slug'         => _x('job-category', 'Job category slug - resave permalinks after changing this',
                                         'wp-job-manager'),
                    'with_front'   => FALSE,
                    'hierarchical' => FALSE
                ];
            ...
            register_taxonomy("job_listing_category",
                              ...
                              apply_filters('register_taxonomy_job_listing_category_args', [
                                  'hierarchical'          => TRUE,
        ...
        if (current_theme_supports('job-manager-templates')) {
            $rewrite = [
                'slug'         => _x('job-type', 'Job type slug - resave permalinks after changing this',
                                     'wp-job-manager'),
                'with_front'   => FALSE,
                'hierarchical' => FALSE
            ];
        ...
        register_taxonomy("job_listing_type",
                          ...
                          apply_filters('register_taxonomy_job_listing_type_args', [
                              'hierarchical' => TRUE,
                              'label'        => $plural,
                              'labels'       => [
                                  'name'              => $plural,
        ...
        if (current_theme_supports('job-manager-templates')) {
            $has_archive = _x('jobs', 'Post type archive slug - resave permalinks after changing this',
                              'wp-job-manager');
        } else {
            $has_archive = FALSE;
        }
        ...
        register_post_type("job_listing",
                           apply_filters("register_post_type_job_listing", [
                               'labels'              => [
                                   'name'                  => $plural,
                                   'singular_name'         => $singular,
                                   ...
                                   'not_found_in_trash'    => sprintf(__('No %s found in trash', 'wp-job-manager'),
                                                                      $plural),
                               ...
                               'description'         => sprintf(__('This is where you can create and manage %s.',
                                                                   'wp-job-manager'), $plural),
                               'public'              => TRUE,
                               'show_ui'             => TRUE,
         ...
         * Feeds
         */
        add_feed('job_feed', [$this, 'job_feed']);
        /**
         * Post status
         */
        register_post_status('expired', [
            ...
            'label_count'               => _n_noop('Expired <span class="count">(%s)</span>',
                                                   'Expired <span class="count">(%s)</span>', 'wp-job-manager'),
        ]);
        register_post_status('preview', [
            ...
            'show_in_admin_status_list' => TRUE,
            'label_count'               => _n_noop('Preview <span class="count">(%s)</span>',
                                                   'Preview <span class="count">(%s)</span>', 'wp-job-manager'),
        ]);
    }
    ...
    public function job_feed() {
        ...
        if (!empty($_GET['job_types'])) {
            $query_args['tax_query'][] = [
                ...
                'field'    => 'slug',
                'terms'    => explode(',', sanitize_text_field($_GET['job_types'])) + [0]
            ];
        }
        if (!empty($_GET['job_categories'])) {
            $cats                      = explode(',', sanitize_text_field($_GET['job_categories'])) + [0];
    ...
    public function check_for_expired_jobs() {
        ...
        if (apply_filters('job_manager_delete_expired_jobs', FALSE)) {
			...
			", date('Y-m-d', strtotime('-' . apply_filters('job_manager_delete_expired_jobs_days', 30) . ' days',
    ...
    public function set_expiry($post) {
        ...
        if (!empty($_POST['_job_expires'])) {
            update_post_meta($post->ID, '_job_expires',
                             date('Y-m-d', strtotime(sanitize_text_field($_POST['_job_expires']))));
    ...
    public function pmxi_saved_post($post_id) {
        if ('job_listing' === get_post_type($post_id)) {
            ...
            if (!WP_Job_Manager_Geocode::has_location_data($post_id) && ($location = get_post_meta($post_id,
                                                                                                   '_job_location',
                                                                                                   TRUE))
            ) {
