...
class WP_Job_Manager_Ajax {
	...
	public function get_listings() {
		...
		$filter_job_types  = isset($_REQUEST['filter_job_type']) ? array_filter(array_map('sanitize_title',
		                                                                                  (array)$_REQUEST['filter_job_type']))
			: NULL;
		$types             = get_job_listing_types();
		$post_type_label   = $wp_post_types['job_listing']->labels->name;
		$orderby           = sanitize_text_field($_REQUEST['orderby']);
		if (is_array($search_categories)) {
			$search_categories = array_filter(array_map('sanitize_text_field',
			                                            array_map('stripslashes', $search_categories)));
		} else {
			$search_categories = array_filter([sanitize_text_field(stripslashes($search_categories))]);
		}
		$args = [
			'search_location'   => $search_location,
			'search_keywords'   => $search_keywords,
			'search_categories' => $search_categories,
			'job_types'         => is_null($filter_job_types) || sizeof($types) === sizeof($filter_job_types) ? ''
				: $filter_job_types + [0],
			'orderby'           => $orderby,
		...
		if ($search_categories) {
			...
			foreach ($search_categories as $category) {
				$category_object = get_term_by(is_numeric($category) ? 'id' : 'slug', $category,
		...
		$result['showing'] = apply_filters('job_manager_get_listings_custom_filter_text',
		                                   sprintf(__('Showing all %s', 'wp-job-manager'),
		                                           implode(' ', $result['showing'])));
		// Generate RSS link
		...
		if (!empty($_FILES)) {
			foreach ($_FILES as $file_key => $file) {
				...
				foreach ($files_to_upload as $file_to_upload) {
					$uploaded_file = job_manager_upload_file($file_to_upload, ['file_key' => $file_key]);
					if (is_wp_error($uploaded_file)) {
						$data['files'][] = ['error' => $uploaded_file->get_error_message()];
					} else {
