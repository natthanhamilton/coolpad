...
class WP_Job_Manager_CPT {
    ...
    public function __construct() {
        ...
        add_action('admin_notices', [$this, 'expired_notice']);
        ...
        if ($post_type == 'job_listing') {
            ...
            <script type="text/javascript">
                jQuery(document).ready(function () {
                    jQuery('<option>').val('approve_jobs').text('<?php printf(__('Approve %s', 'wp-job-manager'),
                                                                              $wp_post_types['job_listing']->labels->name); ?>').appendTo("select[name='action']");
                    jQuery('<option>').val('approve_jobs').text('<?php printf(__('Approve %s', 'wp-job-manager'),
                                                                              $wp_post_types['job_listing']->labels->name); ?>').appendTo("select[name='action2']");

                    jQuery('<option>').val('expire_jobs').text('<?php printf(__('Expire %s', 'wp-job-manager'),
                                                                             $wp_post_types['job_listing']->labels->name); ?>').appendTo("select[name='action']");
                    jQuery('<option>').val('expire_jobs').text('<?php printf(__('Expire %s', 'wp-job-manager'),
                                                                             $wp_post_types['job_listing']->labels->name); ?>').appendTo("select[name='action2']");
                });
    ...
    public function do_bulk_actions() {
        ...
        switch ($action) {
            case 'approve_jobs' :
                ...
                if (!empty($post_ids)) {
                    foreach ($post_ids as $post_id) {
                        ...
                        if (in_array(get_post_status($post_id),
                                     ['pending', 'pending_payment']) && current_user_can('publish_post',
                ...
                wp_redirect(add_query_arg('approved_jobs', $approved_jobs,
                                          remove_query_arg(['approved_jobs', 'expired_jobs'],
            ...
            case 'expire_jobs' :
                ...
                wp_redirect(add_query_arg('expired_jobs', $expired_jobs,
                                          remove_query_arg(['approved_jobs', 'expired_jobs'],
                                                           admin_url('edit.php?post_type=job_listing'))));
                exit;
                break;
    ...
    public function approve_job() {
        if (!empty($_GET['approve_job']) && wp_verify_nonce($_REQUEST['_wpnonce'],
                                                            'approve_job') && current_user_can('publish_post',
                                                                                               $_GET['approve_job'])
        ) {
            ...
            wp_redirect(remove_query_arg('approve_job', add_query_arg('approved_jobs', $post_id,
                                                                      admin_url('edit.php?post_type=job_listing'))));
            exit;
        }
    }
    ...
    public function approved_notice() {
        ...
        if ($pagenow == 'edit.php' && $post_type == 'job_listing' && !empty($_REQUEST['approved_jobs'])) {
            ...
            if (is_array($approved_jobs)) {
                ...
                echo '<div class="updated"><p>' . sprintf(__('%s approved', 'wp-job-manager'),
                                                          '&quot;' . implode('&quot;, &quot;',
                                                                             $titles) . '&quot;') . '</p></div>';
            } else {
                echo '<div class="updated"><p>' . sprintf(__('%s approved', 'wp-job-manager'),
                                                          '&quot;' . get_the_title($approved_jobs) . '&quot;') . '</p></div>';
            }
        }
    }
    ...
    public function expired_notice() {
        ...
        if ($pagenow == 'edit.php' && $post_type == 'job_listing' && !empty($_REQUEST['expired_jobs'])) {
            ...
            if (is_array($expired_jobs)) {
                ...
                echo '<div class="updated"><p>' . sprintf(__('%s expired', 'wp-job-manager'),
                                                          '&quot;' . implode('&quot;, &quot;',
                                                                             $titles) . '&quot;') . '</p></div>';
            } else {
                echo '<div class="updated"><p>' . sprintf(__('%s expired', 'wp-job-manager'),
                                                          '&quot;' . get_the_title($expired_jobs) . '&quot;') . '</p></div>';
            }
        }
    }
    ...
    public function jobs_by_category() {
        ...
        $r['hide_empty']   = 0;
        $r['show_count']   = 1;
        $r['selected']     = (isset($wp_query->query['job_listing_category']))
            ? $wp_query->query['job_listing_category'] : '';
        $r['menu_order']   = FALSE;
        ...
        $output .= '<option value="" ' . selected(isset($_GET['job_listing_category']) ? $_GET['job_listing_category']
                                                      : '', '', FALSE) . '>' . __('Select category',
                                                                                  'wp-job-manager') . '</option>';
    ...
    public function post_updated_messages($messages) {
        ...
        $messages['job_listing'] = [
            0  => '',
            1  => sprintf(__('%s updated. <a href="%s">View</a>', 'wp-job-manager'),
                          $wp_post_types['job_listing']->labels->singular_name, esc_url(get_permalink($post_ID))),
            ...
            5  => isset($_GET['revision']) ? sprintf(__('%s restored to revision from %s', 'wp-job-manager'),
                                                     $wp_post_types['job_listing']->labels->singular_name,
                                                     wp_post_revision_title((int)$_GET['revision'], FALSE)) : FALSE,
            6  => sprintf(__('%s published. <a href="%s">View</a>', 'wp-job-manager'),
                          $wp_post_types['job_listing']->labels->singular_name, esc_url(get_permalink($post_ID))),
            7  => sprintf(__('%s saved.', 'wp-job-manager'), $wp_post_types['job_listing']->labels->singular_name),
            8  => sprintf(__('%s submitted. <a target="_blank" href="%s">Preview</a>', 'wp-job-manager'),
                          $wp_post_types['job_listing']->labels->singular_name,
                          esc_url(add_query_arg('preview', 'true', get_permalink($post_ID)))),
            9  => sprintf(__('%s scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview</a>',
                             'wp-job-manager'), $wp_post_types['job_listing']->labels->singular_name,
                          date_i18n(__('M j, Y @ G:i', 'wp-job-manager'), strtotime($post->post_date)),
                          esc_url(get_permalink($post_ID))),
            10 => sprintf(__('%s draft updated. <a target="_blank" href="%s">Preview</a>', 'wp-job-manager'),
                          $wp_post_types['job_listing']->labels->singular_name,
                          esc_url(add_query_arg('preview', 'true', get_permalink($post_ID)))),
        ];
    ...
    public function columns($columns) {
        ...
        $columns['job_status']           = '<span class="tips" data-tip="' . __("Status",
                                                                                'wp-job-manager') . '">' . __("Status",
                                                                                                              'wp-job-manager') . '</span>';
        ...
        $columns['featured_job']         = '<span class="tips" data-tip="' . __("Featured?",
                                                                                'wp-job-manager') . '">' . __("Featured?",
                                                                                                              'wp-job-manager') . '</span>';
        $columns['filled']               = '<span class="tips" data-tip="' . __("Filled?",
                                                                                'wp-job-manager') . '">' . __("Filled?",
                                                                                                              'wp-job-manager') . '</span>';
    ...
    public function custom_columns($column) {
        ...
        switch ($column) {
            ...
            case "job_position" :
                ...
                echo '<a href="' . admin_url('post.php?post=' . $post->ID . '&action=edit') . '" class="tips job_title" data-tip="' . sprintf(__('ID: %d',
                                                                                                                                                 'wp-job-manager'),
                                                                                                                                              $post->ID) . '">' . $post->post_title . '</a>';
                echo '<div class="company">';
                if (get_the_company_website()) {
                    the_company_name('<span class="tips" data-tip="' . esc_attr(get_the_company_tagline()) . '"><a href="' . esc_url(get_the_company_website()) . '">',
                                     '</a></span>');
                } else {
                    the_company_name('<span class="tips" data-tip="' . esc_attr(get_the_company_tagline()) . '">',
                                     '</span>');
                }
                echo '</div>';
                the_company_logo();
            ...
            case "featured_job" :
                ...
                break;
            case "job_posted" :
                echo '<strong>' . date_i18n(__('M j, Y', 'wp-job-manager'),
                                            strtotime($post->post_date)) . '</strong><span>';
                echo (empty($post->post_author)
                        ? __('by a guest', 'wp-job-manager')
                        : sprintf(__('by %s', 'wp-job-manager'), '<a href="' . esc_url(add_query_arg('author',
                                                                                                     $post->post_author)) . '">' . get_the_author() . '</a>')) . '</span>';
                break;
            case "job_expires" :
                if ($post->_job_expires) {
                    echo '<strong>' . date_i18n(__('M j, Y', 'wp-job-manager'),
                                                strtotime($post->_job_expires)) . '</strong>';
                } else {
                    echo '&ndash;';
                }
                break;
            ...
            case "job_actions" :
                ...
                if (in_array($post->post_status, ['pending', 'pending_payment']) && current_user_can('publish_post',
                                                                                                     $post->ID)
                ) {
                ...
                foreach ($admin_actions as $action) {
                    if (is_array($action)) {
                        printf('<a class="button button-icon tips icon-%1$s" href="%2$s" data-tip="%3$s">%4$s</a>',
                               $action['action'], esc_url($action['url']), esc_attr($action['name']),
    ...
    /**
     * Adds post status to the "submitdiv" Meta Box and post type WP List Table screens. Based on
