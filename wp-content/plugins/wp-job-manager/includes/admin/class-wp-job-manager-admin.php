...
class WP_Job_Manager_Admin {
    ...
    public function admin_enqueue_scripts() {
        ...
        if (in_array($screen->id, apply_filters('job_manager_admin_screen_ids',
                                                ['edit-job_listing', 'job_listing', 'job_listing_page_job-manager-settings', 'job_listing_page_job-manager-addons']))) {
            ...
            wp_register_script('jquery-tiptip',
                               JOB_MANAGER_PLUGIN_URL . '/assets/js/jquery-tiptip/jquery.tipTip.min.js', ['jquery'],
                               JOB_MANAGER_VERSION, TRUE);
            wp_enqueue_script('job_manager_admin_js', JOB_MANAGER_PLUGIN_URL . '/assets/js/admin.min.js',
                              ['jquery', 'jquery-tiptip', 'jquery-ui-datepicker'], JOB_MANAGER_VERSION, TRUE);
            wp_localize_script('job_manager_admin_js', 'job_manager_admin', [
    ...
    public function admin_menu() {
        add_submenu_page('edit.php?post_type=job_listing', __('Settings', 'wp-job-manager'),
                         __('Settings', 'wp-job-manager'), 'manage_options', 'job-manager-settings',
                         [$this->settings_page, 'output']);
        if (apply_filters('job_manager_show_addons_page', TRUE)) {
            add_submenu_page('edit.php?post_type=job_listing', __('WP Job Manager Add-ons', 'wp-job-manager'),
                             __('Add-ons', 'wp-job-manager'), 'manage_options', 'job-manager-addons',
                             [$this, 'addons_page']);
        }
    }
