...
class WP_Job_Manager_Shortcodes {
	...
	public function job_dashboard_handler() {
		if (!empty($_REQUEST['action']) && !empty($_REQUEST['_wpnonce']) && wp_verify_nonce($_REQUEST['_wpnonce'],
				...
				switch ($action) {
					case 'mark_filled' :
						...
						$this->job_dashboard_message
							= '<div class="job-manager-message">' . sprintf(__('%s has been filled', 'wp-job-manager'),
							                                                $job->post_title) . '</div>';
						break;
					case 'mark_not_filled' :
						...
						// Message
						$this->job_dashboard_message
							= '<div class="job-manager-message">' . sprintf(__('%s has been marked as not filled',
							                                                   'wp-job-manager'),
							                                                $job->post_title) . '</div>';
						break;
					case 'delete' :
						...
						// Message
						$this->job_dashboard_message
							= '<div class="job-manager-message">' . sprintf(__('%s has been deleted', 'wp-job-manager'),
							                                                $job->post_title) . '</div>';
						break;
					case 'duplicate' :
						...
						if ($new_job_id) {
							wp_redirect(add_query_arg(['job_id' => absint($new_job_id)],
							                          job_manager_get_permalink('submit_job_form')));
							exit;
						}
						break;
					case 'relist' :
						...
						wp_redirect(add_query_arg(['job_id' => absint($job_id)],
						                          job_manager_get_permalink('submit_job_form')));
						exit;
						break;
					default :
	...
	public function job_dashboard($atts) {
		...
		$args = apply_filters('job_manager_get_dashboard_jobs_args', [
		...
		get_job_manager_template('job-dashboard.php',
		                         ['jobs' => $jobs->query($args), 'max_num_pages' => $jobs->max_num_pages, 'job_dashboard_columns' => $job_dashboard_columns]);
	...
	public function output_jobs($atts) {
		ob_start();
		extract($atts = shortcode_atts(apply_filters('job_manager_output_jobs_defaults', [
		...
		if (!is_null($filled)) {
			$filled = (is_bool($filled) && $filled) || in_array($filled, ['1', 'true', 'yes']) ? TRUE : FALSE;
		}
		// Array handling
		$categories         = is_array($categories) ? $categories
			: array_filter(array_map('trim', explode(',', $categories)));
		$job_types          = is_array($job_types) ? $job_types
			: array_filter(array_map('trim', explode(',', $job_types)));
		$selected_job_types = is_array($selected_job_types) ? $selected_job_types
			: array_filter(array_map('trim', explode(',', $selected_job_types)));
		...
		if ($show_filters) {
			get_job_manager_template('job-filters.php',
			                         ['per_page' => $per_page, 'orderby' => $orderby, 'order' => $order, 'show_categories' => $show_categories, 'categories' => $categories, 'selected_category' => $selected_category, 'job_types' => $job_types, 'atts' => $atts, 'location' => $location, 'keywords' => $keywords, 'selected_job_types' => $selected_job_types, 'show_category_multiselect' => $show_category_multiselect]);
			...
			if (!$show_pagination && $show_more) {
				echo '<a class="load_more_jobs" href="#" style="display:none;"><strong>' . __('Load more listings',
				                                                                              'wp-job-manager') . '</strong></a>';
			}
		} else {
			$jobs = get_job_listings(apply_filters('job_manager_output_jobs_args', [
			...
			if ($jobs->have_posts()) : ?>
				...
				<?php if ($jobs->found_posts > $per_page && $show_more) : ?>
					...
					<?php else : ?>
						<a class="load_more_jobs" href="#"><strong><?php _e('Load more listings',
						                                                    'wp-job-manager'); ?></strong></a>
					<?php endif; ?>
	...
	public function job_filter_job_types($atts) {
		...
		get_job_manager_template('job-filter-job-types.php',
		                         ['job_types' => $job_types, 'atts' => $atts, 'selected_job_types' => $selected_job_types]);
	}
	...
	public function output_job_summary($atts) {
		...
		if ($jobs->have_posts()) : ?>
			...
			<?php while ($jobs->have_posts()) : $jobs->the_post(); ?>
				...
				<div class="job_summary_shortcode align<?php echo $align ?>"
				     style="width: <?php echo $width ? $width : auto; ?>">
