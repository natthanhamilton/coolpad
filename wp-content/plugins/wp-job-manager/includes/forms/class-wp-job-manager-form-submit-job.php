...
class WP_Job_Manager_Form_Submit_Job extends WP_Job_Manager_Form {
    ...
    public function __construct() {
        ...
        // Get step/job
        if (isset($_POST['step'])) {
            $this->step = is_numeric($_POST['step']) ? max(absint($_POST['step']), 0)
                : array_search($_POST['step'], array_keys($this->steps));
        } elseif (!empty($_GET['step'])) {
            $this->step = is_numeric($_GET['step']) ? max(absint($_GET['step']), 0)
                : array_search($_GET['step'], array_keys($this->steps));
        }
        ...
        if (!$this->job_id && !empty($_COOKIE['wp-job-manager-submitting-job-id']) && !empty($_COOKIE['wp-job-manager-submitting-job-key'])) {
            ...
            if ('preview' === $job_status && get_post_meta($job_id, '_submitting_key',
                                                           TRUE) === $_COOKIE['wp-job-manager-submitting-job-key']
            ) {
                $this->job_id = $job_id;
            }
        }
        ...
        if ($this->job_id) {
            ...
            } elseif (!in_array($job_status, apply_filters('job_manager_valid_submit_job_statuses', ['preview']))) {
                $this->job_id = 0;
                $this->step   = 0;
            }
        }
                ...
                foreach ($group_fields as $key => $field) {
                    switch ($key) {
                        ...
                        case 'job_description' :
                            ...
                            break;
                        case 'job_type' :
                            $this->fields[ $group_key ][ $key ]['value'] = current(wp_get_object_terms($job->ID,
                                                                                                       'job_listing_type',
                                                                                                       ['fields' => 'ids']));
                            break;
                        case 'job_category' :
                            $this->fields[ $group_key ][ $key ]['value'] = wp_get_object_terms($job->ID,
                                                                                               'job_listing_category',
                                                                                               ['fields' => 'ids']);
                            break;
                        case 'company_logo' :
                            $this->fields[ $group_key ][ $key ]['value'] = has_post_thumbnail($job->ID)
                                ? get_post_thumbnail_id($job->ID) : get_post_meta($job->ID, '_' . $key, TRUE);
                            break;
                        default:
    ...
    public function init_fields() {
        ...
        $this->fields = apply_filters('submit_job_form_fields', [
            'job'     => [
                'job_title'       => [
    ...
    public function submit_handler() {
        try {
                ...
                if (job_manager_enable_registration()) {
                    ...
                    if (!empty($_POST['create_account_email'])) {
                        $create_account = wp_job_manager_create_account([
                                                                            'username' => empty($_POST['create_account_username'])
                                                                                ? ''
                                                                                : $_POST['create_account_username'],
            ...
            // Update the job
            $this->save_job($values['job']['job_title'], $values['job']['job_description'],
                            $this->job_id ? '' : 'preview', $values);
        ...
        foreach ($this->fields as $group_key => $group_fields) {
            foreach ($group_fields as $key => $field) {
                if ($field['required'] && empty($values[ $group_key ][ $key ])) {
                    return new WP_Error('validation-error',
                                        sprintf(__('%s is a required field', 'wp-job-manager'), $field['label']));
                }
                if (!empty($field['taxonomy']) && in_array($field['type'],
                                                           ['term-checklist', 'term-select', 'term-multiselect'])
                ) {
                    ...
                    foreach ($check_value as $term) {
                        if (!term_exists($term, $field['taxonomy'])) {
                            return new WP_Error('validation-error',
                                                sprintf(__('%s is invalid', 'wp-job-manager'), $field['label']));
                        }
                    }
                }
                if ('file' === $field['type'] && !empty($field['allowed_mime_types'])) {
                    ...
                    if (!empty($check_value)) {
                        foreach ($check_value as $file_url) {
                            ...
                            if (!is_numeric($file_url) && $file_info && !in_array($file_info['type'],
                                                                                  $field['allowed_mime_types'])
                            ) {
                                throw new Exception(sprintf(__('"%s" (filetype %s) needs to be one of the following file types: %s',
                                                               'wp-job-manager'), $field['label'], $file_info['ext'],
                                                            implode(', ', array_keys($field['allowed_mime_types']))));
                            }
                        }
                    }
                }
        ...
        if (isset($values['job']['application']) && !empty($values['job']['application'])) {
            ...
            switch ($allowed_application_method) {
                ...
                case 'url' :
                    ...
                    if (!strstr($values['job']['application'], 'http:') && !strstr($values['job']['application'],
                                                                                   'https:')
                    ) {
                ...
                default :
                    if (!is_email($values['job']['application'])) {
                        ...
                        if (!filter_var($values['job']['application'], FILTER_VALIDATE_URL)) {
                            throw new Exception(__('Please enter a valid application email address or URL',
                                                   'wp-job-manager'));
                        }
                    }
                    break;
    ...
    protected function save_job($post_title, $post_content, $status = 'preview', $values = [], $update_slug = TRUE) {
        ...
        if ($update_slug) {
            ...
            if (apply_filters('submit_job_form_prefix_post_name_with_company',
                              TRUE) && !empty($values['company']['company_name'])
            ) {
                $job_slug[] = $values['company']['company_name'];
            }
            // Prepend location
            if (apply_filters('submit_job_form_prefix_post_name_with_location',
                              TRUE) && !empty($values['job']['job_location'])
            ) {
                $job_slug[] = $values['job']['job_location'];
            }
            // Prepend with job type
            if (apply_filters('submit_job_form_prefix_post_name_with_job_type',
                              TRUE) && !empty($values['job']['job_type'])
            ) {
        ...
        $job_data = apply_filters('submit_job_form_save_job_data', $job_data, $post_title, $post_content, $status,
                                  $values);
        if ($this->job_id) {
            ...
            if (!headers_sent()) {
                ...
                setcookie('wp-job-manager-submitting-job-key', $submitting_key, FALSE, COOKIEPATH, COOKIE_DOMAIN,
                          FALSE);
    ...
    protected function update_job_data($values) {
        ...
        foreach ($this->fields as $group_key => $group_fields) {
            foreach ($group_fields as $key => $field) {
                ...
                if (!empty($field['taxonomy'])) {
                    ...
                    $attachment_id = is_numeric($values[ $group_key ][ $key ]) ? absint($values[ $group_key ][ $key ])
                        : $this->create_attachment($values[ $group_key ][ $key ]);
        ...
        if (sizeof($maybe_attach) && apply_filters('job_manager_attach_uploaded_files', TRUE)) {
            // Get attachments
            $attachments
                             = get_posts('post_parent=' . $this->job_id . '&post_type=attachment&fields=ids&post_mime_type=image&numberposts=-1');
            $attachment_urls = [];
        ...
        if (is_user_logged_in()) {
            update_user_meta(get_current_user_id(), '_company_name',
                             isset($values['company']['company_name']) ? $values['company']['company_name'] : '');
            update_user_meta(get_current_user_id(), '_company_website',
                             isset($values['company']['company_website']) ? $values['company']['company_website'] : '');
            update_user_meta(get_current_user_id(), '_company_tagline',
                             isset($values['company']['company_tagline']) ? $values['company']['company_tagline'] : '');
            update_user_meta(get_current_user_id(), '_company_twitter',
                             isset($values['company']['company_twitter']) ? $values['company']['company_twitter'] : '');
            update_user_meta(get_current_user_id(), '_company_video',
                             isset($values['company']['company_video']) ? $values['company']['company_video'] : '');
        }
    ...
    protected function create_attachment($attachment_url) {
        ...
        $attachment_url = str_replace([$upload_dir['baseurl'], WP_CONTENT_URL, site_url('/')],
                                      [$upload_dir['basedir'], WP_CONTENT_DIR, ABSPATH], $attachment_url);
        ...
        if (!is_wp_error($attachment_id)) {
            wp_update_attachment_metadata($attachment_id,
                                          wp_generate_attachment_metadata($attachment_id, $attachment_url));
    ...
    public function preview() {
        ...
        if ($this->job_id) {
            ...
            setup_postdata($post);
            get_job_manager_template('job-preview.php', [
                'form' => $this
            ]);
            wp_reset_postdata();
        }
    ...
    public function preview_handler() {
        ...
        if (!empty($_POST['continue'])) {
            $job = get_post($this->job_id);
            if (in_array($job->post_status, ['preview', 'expired'])) {
                // Reset expiry
                delete_post_meta($job->ID, '_job_expires');
                // Update job listing
                $update_job                  = [];
                $update_job['ID']            = $job->ID;
                $update_job['post_status']   = apply_filters('submit_job_post_status',
                                                             get_option('job_manager_submission_requires_approval')
                                                                 ? 'pending' : 'publish', $job);
