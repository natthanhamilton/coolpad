...
if (!function_exists('get_job_listings')) :
	...
	function get_job_listings($args = []) {
		...
		if (!empty($args['search_categories'])) {
			...
			$operator                  = 'all' === get_option('job_manager_category_filter_type',
			                                                  'all') && sizeof($args['search_categories']) > 1 ? 'AND'
				: 'IN';
			$query_args['tax_query'][] = [
		...
		if (!empty($job_manager_keyword) && strlen($job_manager_keyword) >= apply_filters('job_manager_get_listings_keyword_length_threshold',
		                                                                                  2)
		) {
		...
		$query_args_hash
		         = 'jm_' . md5($to_hash) . WP_Job_Manager_Cache_Helper::get_transient_version('get_job_listings');
...
if (!function_exists('get_job_listings_keyword_search')) :
	...
	function get_job_listings_keyword_search($args) {
		...
		$conditions[]
		              = "{$wpdb->posts}.ID IN ( SELECT post_id FROM {$wpdb->postmeta} WHERE meta_value LIKE '%" . esc_sql($job_manager_keyword) . "%' )";
		$conditions[]
		              = "{$wpdb->posts}.ID IN ( SELECT object_id FROM {$wpdb->term_relationships} AS tr LEFT JOIN {$wpdb->terms} AS t ON tr.term_taxonomy_id = t.term_id WHERE t.name LIKE '%" . esc_sql($job_manager_keyword) . "%' )";
...
if (!function_exists('get_job_listing_post_statuses')) :
	...
	function get_job_listing_post_statuses() {
		return apply_filters('job_listing_post_statuses', [
...
if (!function_exists('get_job_listing_types')) :
	...
	function get_job_listing_types($fields = 'all') {
		return get_terms("job_listing_type", [
			'orderby'    => 'name',
			'order'      => 'ASC',
			'hide_empty' => FALSE,
			'fields'     => $fields
...
if (!function_exists('get_job_listing_categories')) :
	...
	function get_job_listing_categories() {
		...
		return get_terms("job_listing_category", [
			'orderby'    => 'name',
			'order'      => 'ASC',
			'hide_empty' => FALSE,
		]);
...
if (!function_exists('job_manager_get_filtered_links')) :
	...
	function job_manager_get_filtered_links($args = []) {
		...
		$links = apply_filters('job_manager_job_filters_showing_jobs_links', [
			'reset'    => [
			...
			'rss_link' => [
				...
				'url'  => get_job_listing_rss_link(apply_filters('job_manager_get_listings_custom_filter_rss_args', [
		...
		if (sizeof($args['filter_job_types']) === sizeof($types) && !$args['search_keywords'] && !$args['search_location'] && !$args['search_categories'] && !apply_filters('job_manager_get_listings_custom_filter',
		                                                                                                                                                                    FALSE)
		) {
			unset($links['reset']);
		}
...
if (!function_exists('get_job_listing_rss_link')) :
	...
	function get_job_listing_rss_link($args = []) {
		$rss_link = add_query_arg(urlencode_deep(array_merge(['feed' => 'job_feed'], $args)), home_url());
	...
	function wp_job_manager_create_account($args, $deprecated = '') {
		...
		if (email_exists($email)) {
			return new WP_Error('validation-error',
			                    __('This email is already registered, please choose another one.', 'wp-job-manager'));
		}
...
/**
 * True if an the user can post a job. If accounts are required, and reg is enabled, users can post (they signup at the
 * same time).
 *
 * @return bool
 */
...
function job_manager_enable_registration() {
	return apply_filters('job_manager_enable_registration',
	                     get_option('job_manager_enable_registration') == 1 ? TRUE : FALSE);
}
...
function job_manager_generate_username_from_email() {
	return apply_filters('job_manager_generate_username_from_email',
	                     get_option('job_manager_generate_username_from_email') == 1 ? TRUE : FALSE);
}
...
function job_manager_user_requires_account() {
	return apply_filters('job_manager_user_requires_account',
	                     get_option('job_manager_user_requires_account') == 1 ? TRUE : FALSE);
}
...
function job_manager_user_can_edit_pending_submissions() {
	return apply_filters('job_manager_user_can_edit_pending_submissions',
	                     get_option('job_manager_user_can_edit_pending_submissions') == 1 ? TRUE : FALSE);
}
...
function job_manager_dropdown_categories($args = '') {
	...
	$categories_hash
		        = 'jm_cats_' . md5(json_encode($r) . WP_Job_Manager_Cache_Helper::get_transient_version('jm_get_' . $r['taxonomy']));
	...
	$output
		= "<select name='" . esc_attr($name) . "[]' id='" . esc_attr($id) . "' class='" . esc_attr($class) . "' " . ($multiple
			? "multiple='multiple'"
			: '') . " data-placeholder='" . esc_attr($placeholder) . "' data-no_results_text='" . esc_attr($no_results_text) . "' data-multiple_text='" . esc_attr($multiple_text) . "'>\n";
	if ($show_option_all) {
...
function job_manager_upload_dir($pathdata) {
	...
	if (!empty($job_manager_upload)) {
		$dir = untrailingslashit(apply_filters('job_manager_upload_dir',
		                                       'job-manager-uploads/' . sanitize_key($job_manager_uploading_file),
		                                       sanitize_key($job_manager_uploading_file)));
	...
	if (is_array($file_data['name'])) {
		foreach ($file_data['name'] as $file_data_key => $file_data_value) {
			if ($file_data['name'][ $file_data_key ]) {
				$type
					               = wp_check_filetype($file_data['name'][ $file_data_key ]); // Map mime type to one WordPress recognises
				$files_to_upload[] = [
	...
	if (!in_array($file['type'], $args['allowed_mime_types'])) {
		if ($args['file_label']) {
			return new WP_Error('upload',
			                    sprintf(__('"%s" (filetype %s) needs to be one of the following file types: %s',
			                               'wp-job-manager'), $args['file_label'], $file['type'],
			                            implode(', ', array_keys($args['allowed_mime_types']))));
		} else {
			return new WP_Error('upload', sprintf(__('Uploaded files need to be one of the following file types: %s',
			                                         'wp-job-manager'),
			                                      implode(', ', array_keys($args['allowed_mime_types']))));
		}
	} else {
		$upload = wp_handle_upload($file,
		                           apply_filters('submit_job_wp_handle_upload_overrides', ['test_form' => FALSE]));
...
function job_manager_duplicate_listing($post_id) {
	 ...
	 */
	$post_meta
		= $wpdb->get_results($wpdb->prepare("SELECT meta_key, meta_value FROM {$wpdb->postmeta} WHERE post_id=%d",
		                                    $post_id));
	if (!empty($post_meta)) {
		$post_meta = wp_list_pluck($post_meta, 'meta_value', 'meta_key');
		foreach ($post_meta as $meta_key => $meta_value) {
			if (in_array($meta_key, apply_filters('job_manager_duplicate_listing_ignore_keys',
			                                      ['_filled', '_featured', '_job_expires', '_job_duration', '_package_id', '_user_package_id']))) {
				continue;
			}
