...
function get_job_listing_pagination($max_num_pages, $current_page = 1) {
    ob_start();
    get_job_manager_template('job-pagination.php',
                             ['max_num_pages' => $max_num_pages, 'current_page' => absint($current_page)]);
...
function candidates_can_apply($post = NULL) {
    ...
    return apply_filters('job_manager_candidates_can_apply',
        (!is_position_filled() && !in_array($post->post_status, ['preview', 'expired'])), $post);
}
...
function get_the_job_application_method($post = NULL) {
    ...
    if (strstr($apply, '@') && is_email($apply)) {
        ...
        $method->subject   = apply_filters('job_manager_application_email_subject',
                                           sprintf(__('Application via "%s" listing on %s', 'wp-job-manager'),
                                                   $post->post_title, home_url()), $post);
    } else {
...
function the_job_location($map_link = TRUE, $post = NULL) {
    ...
    if ($location) {
        if ($map_link) {
            ...
            echo apply_filters('the_job_location_map_link',
                               '<a class="google_map_link" href="' . esc_url('http://maps.google.com/maps?q=' . urlencode(strip_tags($location)) . '&zoom=14&size=512x512&maptype=roadmap&sensor=false') . '" target="_blank">' . esc_html(strip_tags($location)) . '</a>',
                               $location, $post);
        } else {
...
function the_company_logo($size = 'thumbnail', $default = NULL, $post = NULL) {
    ...
    if (has_post_thumbnail($post)) {
        ...
        echo '<img class="company_logo" src="' . esc_attr(apply_filters('job_manager_default_company_logo',
                                                                        JOB_MANAGER_PLUGIN_URL . '/assets/images/company.png')) . '" alt="' . esc_attr(get_the_company_name($post)) . '" />';
    }
}
...
function job_manager_get_resized_image($logo, $size) {
    ...
    if ($size !== 'full' && strstr($logo,
                                   WP_CONTENT_URL) && (isset($_wp_additional_image_sizes[ $size ]) || in_array($size,
        ...
        $logo_path         = str_replace([$upload_dir['baseurl'], $upload_dir['url'], WP_CONTENT_URL],
                                         [$upload_dir['basedir'], $upload_dir['path'], WP_CONTENT_DIR], $logo);
        $path_parts        = pathinfo($logo_path);
        $resized_logo_path = str_replace('.' . $path_parts['extension'], '-' . $size . '.' . $path_parts['extension'],
                                         $logo_path);
...
function the_company_twitter($before = '', $after = '', $echo = TRUE, $post = NULL) {
    ...
    $company_twitter
                     = $before . '<a href="http://twitter.com/' . $company_twitter . '" class="company_twitter" target="_blank">' . $company_twitter . '</a>' . $after;
    if ($echo) {
