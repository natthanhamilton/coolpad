...
class WP_Job_Manager {
	...
	public function __construct() {
		...
		define('JOB_MANAGER_PLUGIN_URL', untrailingslashit(plugins_url(basename(plugin_dir_path(__FILE__)),
		                                                               basename(__FILE__))));
		// Includes
	...
	public function load_plugin_textdomain() {
		load_textdomain('wp-job-manager',
		                WP_LANG_DIR . "/wp-job-manager/wp-job-manager-" . apply_filters('plugin_locale', get_locale(),
		                                                                                'wp-job-manager') . ".mo");
	...
	public function frontend_scripts() {
		...
		if (apply_filters('job_manager_chosen_enabled', TRUE)) {
			wp_register_script('chosen', JOB_MANAGER_PLUGIN_URL . '/assets/js/jquery-chosen/chosen.jquery.min.js',
			                   ['jquery'], '1.1.0', TRUE);
			wp_register_script('wp-job-manager-term-multiselect',
			                   JOB_MANAGER_PLUGIN_URL . '/assets/js/term-multiselect.min.js', ['jquery', 'chosen'],
			...
			wp_register_script('wp-job-manager-multiselect', JOB_MANAGER_PLUGIN_URL . '/assets/js/multiselect.min.js',
			                   ['jquery', 'chosen'], JOB_MANAGER_VERSION, TRUE);
			...
			wp_localize_script('chosen', 'job_manager_chosen_multiselect_args',
			                   apply_filters('job_manager_chosen_multiselect_args', ['search_contains' => TRUE])
			);
		}
		if (apply_filters('job_manager_ajax_file_upload_enabled', TRUE)) {
			wp_register_script('jquery-iframe-transport',
			                   JOB_MANAGER_PLUGIN_URL . '/assets/js/jquery-fileupload/jquery.iframe-transport.js',
			                   ['jquery'], '1.8.3', TRUE);
			wp_register_script('jquery-fileupload',
			                   JOB_MANAGER_PLUGIN_URL . '/assets/js/jquery-fileupload/jquery.fileupload.js',
			                   ['jquery', 'jquery-iframe-transport', 'jquery-ui-widget'], '9.11.2', TRUE);
			wp_register_script('wp-job-manager-ajax-file-upload',
			                   JOB_MANAGER_PLUGIN_URL . '/assets/js/ajax-file-upload.min.js',
			                   ['jquery', 'jquery-fileupload'], JOB_MANAGER_VERSION, TRUE);
			ob_start();
			get_job_manager_template('form-fields/uploaded-file-html.php',
			                         ['name' => '', 'value' => '', 'extension' => 'jpg']);
			$js_field_html_img = ob_get_clean();
			ob_start();
			get_job_manager_template('form-fields/uploaded-file-html.php',
			                         ['name' => '', 'value' => '', 'extension' => 'zip']);
			$js_field_html = ob_get_clean();
			wp_localize_script('wp-job-manager-ajax-file-upload', 'job_manager_ajax_file_upload', [
				'ajax_url'               => $ajax_url,
		...
		wp_register_script('jquery-deserialize',
		                   JOB_MANAGER_PLUGIN_URL . '/assets/js/jquery-deserialize/jquery.deserialize.js', ['jquery'],
		                   '1.2.1', TRUE);
		wp_register_script('wp-job-manager-ajax-filters', JOB_MANAGER_PLUGIN_URL . '/assets/js/ajax-filters.min.js',
		                   $ajax_filter_deps, JOB_MANAGER_VERSION, TRUE);
		wp_register_script('wp-job-manager-job-dashboard', JOB_MANAGER_PLUGIN_URL . '/assets/js/job-dashboard.min.js',
		                   ['jquery'], JOB_MANAGER_VERSION, TRUE);
		wp_register_script('wp-job-manager-job-application',
		                   JOB_MANAGER_PLUGIN_URL . '/assets/js/job-application.min.js', ['jquery'],
		                   JOB_MANAGER_VERSION, TRUE);
		...
		wp_localize_script('wp-job-manager-job-dashboard', 'job_manager_job_dashboard', [
