 ...
 *
 * Displays a message after install (if not dismissed and GA is not already configured) about how to configure the
 * analytics plugin
 */
class WC_Google_Analytics_Info_Banner {
    ...
    public function __construct($dismissed = FALSE, $ga_id = '') {
        ...
        add_action('admin_init', [$this, 'dismiss_banner']);
    }
    ...
    public function banner() {
        ...
        if (!in_array($screen->base,
                      ['woocommerce_page_wc-settings', 'plugins']) || $screen->is_network || $screen->action
        ) {
            return;
        }
        ...
        $configure
                 = sprintf(__('<a href="%s">Connect WooCommerce to Google Analytics</a> to finish setting up this integration.'),
                           $integration_url);
        // Display the message..
        echo '<div class="updated fade"><p><strong>' . $heading . '</strong> ';
        echo '<a href="' . esc_url($dismiss_url) . '" title="' . __('Dismiss this notice.',
                                                                    'woocommerce-google-analytics-integration') . '"> ' . __('(Dismiss)',
                                                                                                                             'woocommerce-google-analytics-integration') . '</a>';
