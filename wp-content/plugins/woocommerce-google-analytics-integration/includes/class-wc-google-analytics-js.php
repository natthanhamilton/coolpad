...
class WC_Google_Analytics_JS {
    ...
    public static function load_analytics($order = FALSE) {
        ...
        if ('yes' === self::get('ga_use_universal_analytics')) {
            add_action('wp_footer', ['WC_Google_Analytics_JS', 'universal_analytics_footer']);

            return self::load_analytics_universal($logged_in);
        } else {
            add_action('wp_footer', ['WC_Google_Analytics_JS', 'classic_analytics_footer']);
     ...
     *
     * @param  string         $logged_in 'yes' if the user is logged in, no if not (this is a string so we can pass it
     *                                   to GA)
     * @param  boolean|object $order     We don't always need to load order data for currency, so we omit that if false
     *                                   is set, otherwise this is an order object
     *
