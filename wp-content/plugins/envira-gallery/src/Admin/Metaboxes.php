...
class Metaboxes {
	...
	*/
	...
	public function fix_plugin_js_conflicts(){
		...
		// Bail if we're not on the Envira Post Type screen.
		if ( $screen === false || ! isset( $screen->post_type ) || 'envira' !== $screen->post_type ) {
			return;
		}
		
		if ( class_exists( 'Cherry_Banner_Widget' ) ){
			remove_action('admin_enqueue_scripts', array( 'Cherry_Banner_Widget', 'upload_scripts'), 10 );
		}
				...
				<li class="envira-select sorting-options">
					<select id="envira-config-image-sort" name="_envira_gallery[random]" class="envira-chosen" data-envira-chosen-options='{ "disable_search":"true", "width": "100%" }'>
						<?php foreach ( (array) envira_get_sorting_options() as $i => $data ) : 
							  	// change the name to 'manual sorting' ONLY here, because these same options appear for default/dynamic
									if ( strtolower($data['name']) == 'no sorting' ) {
										$data['name'] = "Manual Sort";
									}
						?>
			...
			<table class="form-table" style="margin-bottom: 0;">
				<tbody>
					...
					<tr id="envira-config-lazy-loading-box">
						...
						<td>
							<input id="envira-config-lazy-loading" type="checkbox" name="_envira_gallery[lazy_loading]" value="<?php echo envira_get_config( 'lazy_loading', $gallery_data ); ?>" <?php checked( envira_get_config( 'lazy_loading', $gallery_data ), 1 ); ?> />
						...
						<tr id="envira-config-gallery-justified-theme-box">
							...
							<td>
								<select id="envira-config-gallery-justified-theme" name="_envira_gallery[justified_gallery_theme]">
									<?php foreach ( (array) envira_get_justified_gallery_themes() as $i => $data ) : ?>
										<option value="<?php echo $data['value']; ?>"<?php selected( $data['value'], envira_get_config( 'justified_gallery_theme', $gallery_data, envira_get_config_default('justified_gallery_theme') ) ); ?>><?php echo $data['name']; ?></option>
									<?php endforeach; ?>
								</select>
								<select id="envira-config-gallery-justified-theme-hover" name="_envira_gallery[justified_gallery_theme_detail]">
									<?php foreach ( (array) envira_get_justified_gallery_themes_details() as $i => $data ) : ?>
										<option value="<?php echo $data['value']; ?>"<?php selected( $data['value'], envira_get_config( 'justified_gallery_theme_detail', $gallery_data, envira_get_config_default('justified_gallery_theme_detail') ) ); ?>><?php echo $data['name']; ?></option>
									<?php endforeach; ?>
								</select>
				...
				<table class="form-table" style="margin-bottom: 0;">
					<tbody>
						...
						<tr id="envira-config-css-animations-box">
							...
							<td>
								<input id="envira-config-css-animations" type="checkbox" name="_envira_gallery[css_animations]" value="<?php echo envira_get_config( 'css_animations', $gallery_data ); ?>" <?php checked( envira_get_config( 'css_animations', $gallery_data, envira_get_config_default('css_animations') ), 1 ); ?> />
						...
						<tr id="envira-config-css-opacity-box">
							...
							<td>
								<input id="envira-config-css-opacity" type="number" name="_envira_gallery[css_opacity]" min="0" max="100" step="1" value="<?php echo envira_get_config( 'css_opacity', $gallery_data, envira_get_config_default('css_opacity') ); ?>" /><span class="envira-unit">%</span>
						...
						$ids = array();
						$ids['dynamic_gallery_id']  = get_option( 'envira_dynamic_gallery' );
						$ids['default_gallery_id']  = get_option( 'envira_default_gallery' );

						if ( isset( $_GET['post'] ) && in_array( intval( $_GET['post'] ), $ids ) ) { ?>
						<tr id="envira-config-sorting-box">
								 ...
								 <td>
								  <select id="envira-config-sorting" name="_envira_gallery[random]">
									  <?php
									  foreach ( (array) envira_get_sorting_options() as $i => $data ) {
											?>
											<option value="<?php echo $data['value']; ?>"<?php selected( $data['value'], envira_get_config( 'random', $gallery_data, envira_get_config_default( 'random' ) ) ); ?>><?php echo $data['name']; ?></option>
											<?php
									  }
									  ?>
								  </select>
...
<tr id="envira-config-sorting-direction-box">
						...
						<td>
							<input id="envira-config-crop-width" type="number" name="_envira_gallery[crop_width]" value="<?php echo envira_get_config( 'crop_width', $gallery_data, envira_get_config_default( 'crop_width' ) ); ?>" /> &#215; <input id="envira-config-crop-height" type="number" name="_envira_gallery[crop_height]" value="<?php echo envira_get_config( 'crop_height', $gallery_data, envira_get_config_default( 'crop_height' ) ); ?>" /> <span class="envira-unit"><?php _e( 'px', 'envira-gallery' ); ?></span>
						...
						<tr id="envira-config-dimensions-box">
								...
								<td>
									<input id="envira-config-dimensions" type="checkbox" name="_envira_gallery[dimensions]" value="<?php echo envira_get_config( 'dimensions', $gallery_data ); ?>" <?php checked( envira_get_config( 'dimensions', $gallery_data, $gallery_data, envira_get_config_default( 'dimensions' ) ), 1 ); ?> />
					...
					<tbody>
						...
						<tr id="envira-config-lightbox-title-display-box">
							...
							<tr id="envira-config-lightbox-keyboard-box">
								...
								<td>
									<input id="envira-config-lightbox-keyboard" type="checkbox" name="_envira_gallery[keyboard]" value="<?php echo envira_get_config( 'keyboard', $gallery_data ); ?>" <?php checked( envira_get_config( 'keyboard', $gallery_data ), 1 ); ?> />
							...
							<tr id="envira-config-lightbox-mousewheel-box">
								...
								<td>
									<input id="envira-config-lightbox-mousewheel" type="checkbox" name="_envira_gallery[mousewheel]" value="<?php echo envira_get_config( 'mousewheel', $gallery_data ); ?>" <?php checked( envira_get_config( 'mousewheel', $gallery_data ), 1 ); ?> />
								...
								<td>
									<select id="envira-config-lightbox-open-close-effect" name="_envira_gallery[lightbox_open_close_effect]">
										...
										foreach ( (array) envira_get_transition_effects() as $i => $data ) {
											?>
											<option value="<?php echo $data['value']; ?>"<?php selected( $data['value'], envira_get_config( 'lightbox_open_close_effect', $gallery_data ) ); ?>><?php echo $data['name']; ?></option>
											<?php
										}

										// Easing Effects
										foreach ( (array) envira_get_easing_transition_effects() as $i => $data ) {
											?>
											<option value="<?php echo $data['value']; ?>"<?php selected( $data['value'], envira_get_config( 'lightbox_open_close_effect', $gallery_data ) ); ?>><?php echo $data['name']; ?></option>
											<?php
										}
							...
							<tr id="envira-config-lightbox-effect-box">
								...
								<td>
									<select id="envira-config-lightbox-effect" name="_envira_gallery[effect]">
										...
										foreach ( (array) envira_get_transition_effects() as $i => $data ) {
											?>
											<option value="<?php echo $data['value']; ?>"<?php selected( $data['value'], envira_get_config( 'effect', $gallery_data ) ); ?>><?php echo $data['name']; ?></option>
											<?php
										}
							...
							<tr id="envira-config-lightbox-html5-box">
								...
								<td>
									<input id="envira-config-lightbox-html5" type="checkbox" name="_envira_gallery[html5]" value="<?php echo envira_get_config( 'html5', $gallery_data ); ?>" <?php checked( envira_get_config( 'html5', $gallery_data ), 1 ); ?> />
			...
			<table class="form-table">
				<tbody>
					...
					<tr id="envira-config-mobile-touchwipe-box">
						...
						<td>
							<input id="envira-config-mobile-touchwipe" type="checkbox" name="_envira_gallery[mobile_touchwipe]" value="<?php echo envira_get_config( 'mobile_touchwipe', $gallery_data ); ?>" <?php checked( envira_get_config( 'mobile_touchwipe', $gallery_data, envira_get_config_default( 'mobile_touchwipe' ) ), 1 ); ?> />
			...
			<table class="form-table">
				<tbody>
					<tr id="envira-config-title-box">
						...
						<td>
							<input id="envira-config-title" type="text" name="_envira_gallery[title]" value="<?php echo envira_get_config( 'title', $gallery_data ); ?>" />
	...
	public function save_meta_boxes( $post_id, $post ) {
		...
		$settings['config']['title']	= sanitize_text_field( strip_tags( esc_attr( $_POST['post_title'] ) ) ) != trim( strip_tags( esc_attr( $_POST['_envira_gallery']['title'] ) ) ) ? sanitize_text_field( $_POST['post_title'] ) : trim( strip_tags( esc_attr( $_POST['_envira_gallery']['title'] ) ) );
