...
use Envira\Frontend\Public_Container;
	...
	class Envira_Gallery {
		...
		public function setup_constants(){
			...
			if ( ! defined( 'ENVIRA_VERSION' ) ){
				...
				define( 'ENVIRA_VERSION', '1.7.0' );
			
			}
			
			if ( ! defined( 'ENVIRA_SLUG' ) ){
				
				define( 'ENVIRA_SLUG', 'envira-gallery' );
			
			}

			if ( ! defined( 'ENVIRA_FILE' ) ){
				
				define( 'ENVIRA_FILE', __FILE__ );
		...
		public function init() {
			...
			if ( is_admin() ) {
				...
				if ( class_exists( 'AM_Notification' ) ){
					$notifications = new AM_Notification( 'envira', $this->version );
				}
		...
		public function load_legacy(){
			...
			if ( is_admin() ){
				...
				require_once( trailingslashit( ENVIRA_DIR ) . 'src/Utils/Notifications.php' );
